!     #########
                   SUBROUTINE GET_DIMLEN_NETCDF(HFILE_NAME,HDIM_NAME,KDIM,KDIM_ID)
!
!
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK
USE PARKIND1  ,ONLY : JPRB
!
USE MODI_HANDLE_ERR
IMPLICIT NONE
INCLUDE "netcdf.inc"
CHARACTER(LEN=*), INTENT(IN):: HFILE_NAME
CHARACTER(LEN=*), INTENT(IN):: HDIM_NAME
INTEGER,          INTENT(OUT) :: KDIM
INTEGER, OPTIONAL,INTENT(OUT) :: KDIM_ID

!local variables
INTEGER ::  JRET,IFILE_ID,IDIM_ID
INTEGER,DIMENSION(2) :: IRET
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!----------------------------------
! 1.a Open file
IF (LHOOK) CALL DR_HOOK('GET_DIMLEN_NETCDF',0,ZHOOK_HANDLE)
JRET=NF_OPEN(HFILE_NAME,NF_NOWRITE,IFILE_ID)
!
! 1.b get dim info
IRET(1)=NF_INQ_DIMID(IFILE_ID,HDIM_NAME,IDIM_ID)
IF (PRESENT(KDIM_ID)) THEN
        KDIM_ID=IDIM_ID
ENDIF
IRET(2)=NF_INQ_DIMLEN(IFILE_ID,IDIM_ID,KDIM)

! 1.c close file
JRET=NF_CLOSE(IFILE_ID)
!
! 2. check for error
DO JRET=1,2
   IF (IRET(JRET).NE.NF_NOERR) CALL HANDLE_ERR(IRET(JRET),'GET_DIMLEN_NETCDF')
ENDDO
IF (LHOOK) CALL DR_HOOK('GET_DIMLEN_NETCDF',1,ZHOOK_HANDLE)
!
END SUBROUTINE GET_DIMLEN_NETCDF
