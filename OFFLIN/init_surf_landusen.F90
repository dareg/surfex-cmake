...
USE MODD_SURFEX_MPI, ONLY : NRANK, NPIO, NCOMM, LSFX_MPI
!
...
!
#ifdef SFX_MPI
IF (LSFX_MPI) CALL MPI_BCAST(UG%NGRID_FULL_PAR,KIND(UG%NGRID_FULL_PAR)/4,MPI_INTEGER,NPIO,NCOMM,INFOMPI)
#endif
IF (NRANK/=NPIO) ALLOCATE(UG%XGRID_FULL_PAR(UG%NGRID_FULL_PAR))
#ifdef SFX_MPI
IF (LSFX_MPI)  CALL MPI_BCAST(UG%XGRID_FULL_PAR,&
