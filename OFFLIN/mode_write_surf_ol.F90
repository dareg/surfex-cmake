...
!    
USE MODD_SURF_PAR,   ONLY: XUNDEF
USE MODD_IO_SURF_OL, ONLY: LMASK,NMASK,XSTART,XSTRIDE,LPARTW,XSTARTW,XCOUNTW
!
USE MODI_IO_BUFF_n
USE MODI_OL_FIND_FILE_WRITE
USE MODI_UNPACK_SAME_RANK
...
IF (IFILE_ID /= 0) THEN
  ...
  ENDDO
  !
  IRET(4)=NF_INQ_DIMNAME(IFILE_ID,IDIMIDS(1),YNAME)
  IF (YNAME .EQ. 'Number_of_points') THEN
    CALL WRITE_DATAX1_OL(IDIMLEN(1),1,INDIMS)          
  ELSE
...
CONTAINS
!
SUBROUTINE WRITE_DATAX1_OL(KDIM1,KDIM2,KNDIMS)
!
INTEGER, INTENT(IN) :: KDIM1
INTEGER, INTENT(IN) :: KDIM2
INTEGER, INTENT(IN) :: KNDIMS
!
REAL, DIMENSION(KDIM1*KDIM2) :: ZTAB1D
REAL, DIMENSION(KDIM2,KDIM1) :: ZTAB2D
...
!
IF (LMASK) THEN
  CALL UNPACK_SAME_RANK(NMASK,PFIELD,ZTAB1D)
ELSE 
  ZTAB1D(:)=PFIELD(:)
ENDIF
!
IF (KDIM2.NE.1) THEN
  DO J=1,KDIM1
    ZTAB2D(:,J)=ZTAB1D((J-1)*KDIM2+1:J*KDIM2)
  ENDDO
ENDIF
!
IF  (LPARTW) THEN
...
IF (IFILE_ID /= 0) THEN
  ...
  IRET(4)=NF_INQ_DIMNAME(IFILE_ID,IDIMIDS(1),YNAME)
  IF (YNAME .EQ. 'Number_of_points') THEN
    CALL WRITE_DATAX2_OL(IDIMLEN(1),1         ,IDIMLEN(2),INDIMS)
    print*,IDIMLEN(1),IDIMLEN(2)
  ELSE
...
CONTAINS
!
SUBROUTINE WRITE_DATAX2_OL(KDIM1,KDIM2,KDIM3,KNDIMS)
!
INTEGER, INTENT(IN) :: KDIM1
INTEGER, INTENT(IN) :: KDIM2
...
INTEGER, INTENT(IN) :: KNDIMS
!
REAL, DIMENSION(KDIM2,KDIM1,KDIM3) :: ZTAB3D ! array containing the data field
REAL, DIMENSION(KDIM1*KDIM2,KDIM3) :: ZTAB2D ! work array read in the file
INTEGER,         DIMENSION(KNDIMS) :: ISTART
INTEGER,         DIMENSION(KNDIMS) :: ISTRIDE
INTEGER,         DIMENSION(KNDIMS) :: ICOUNT
INTEGER :: J, I
...
IF (LMASK) THEN
  CALL UNPACK_SAME_RANK(NMASK,PFIELD,ZTAB2D)
ELSE 
  ZTAB2D(:,:)=PFIELD(:,:)
ENDIF
!
IF (KDIM2.NE.1) THEN
  DO J=1,KDIM1 !number of tile
    ZTAB3D(:,J,:)=ZTAB2D((J-1)*KDIM2+1:J*KDIM2,:)
  ENDDO
ENDIF
!
! 2. Put variable
...
IF (LPARTW) THEN
  ...
  !
  IF (KDIM2.NE.1) THEN
    IRET(5)=NF_PUT_VARS_DOUBLE(IFILE_ID,IVAR_ID,ISTART,ICOUNT,ISTRIDE,ZTAB3D)
  ELSE
    IRET(5)=NF_PUT_VARS_DOUBLE(IFILE_ID,IVAR_ID,ISTART,ICOUNT,ISTRIDE,TRANSPOSE(ZTAB2D))
  ENDIF
ELSE
  IF (KDIM2.NE.1) THEN
    IRET(5)=NF_PUT_VAR_DOUBLE(IFILE_ID,IVAR_ID,ZTAB3D)
  ELSE
    IRET(5)=NF_PUT_VAR_DOUBLE(IFILE_ID,IVAR_ID,TRANSPOSE(ZTAB2D))
  ENDIF
  CALL HANDLE_ERR(IRET(5),'X2')
ENDIF
!
