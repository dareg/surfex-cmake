...
TDATE_END%DAY = IDAY
ZDURATION2 = ZDURATION + ZTIME !asmundb 20200730 (+ ZTIME)
...
#ifdef SFX_FA
            ! Won't work on more processes as global value is kept there                     
...
IF ( LRESTART ) THEN
  ...
  DO JNW = 1,INW
    ...
    YSC%IM%ID%O%LPATCH_BUDGET  = .TRUE.
...
IF ( LINQUIRE ) THEN
  ...
  !
  ISERIES = 0
  CALL GET_SURF_VAR_n(YSC%FM, YSC%IM, YSC%SM, YSC%TM, YSC%WM, YSC%DUO, YSC%DU,  YSC%UG, YSC%U, YSC%USS, YSC%SM%S, &
