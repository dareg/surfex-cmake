...
USE MODD_IO_SURF_ASC,ONLY : CFILEIN, CFILEOUT
USE MODD_SURF_PAR
USE MODD_IO_SURF_FA, ONLY : CFILEIN_FA, CFILEOUT_FA, NUNIT_FA,     &
                             CDNOMC, IVERBFA, LFANOCOMPACT  
USE MODD_IO_SURF_LFI,ONLY : CFILEIN_LFI, CLUOUT_LFI, CFILEOUT_LFI, &
                            LMNH_COMPATIBLE  
...
REAL, DIMENSION(:), ALLOCATABLE   :: ZZ0, ZZ0H
...
!
IF((CSURF_FILETYPE=='FA    '.OR.CTIMESERIES_FILETYPE=='FA    ').AND.&
     (CSURF_FILETYPE/='FA    '.OR.CTIMESERIES_FILETYPE/='FA    '))THEN  
   WRITE(ILUOUT,*)' FA IS USED BUT SURFEX INPUT AND TIMESERIES FILETYPE ARE DIFFERENT :'
   WRITE(ILUOUT,*)' CSURF_FILETYPE       = ',CSURF_FILETYPE
   WRITE(ILUOUT,*)' CTIMESERIES_FILETYPE = ',CTIMESERIES_FILETYPE
   CALL ABOR1_SFX(' OFFLINE: FA IS USED SO BOTH INPUT AND TIMESERIES FILETYPE MUST BE IN FA')
ENDIF
!
! --------------------------------------------------------------------------------------
!
...
IF (LLAND_USE) THEN
  CALL SET_VEGTYPES_FRACTIONS(CSURF_FILETYPE)
  CALL COMPUTE_ISBA_PARAMETERS(CSURF_FILETYPE,YINIT, LLAND_USE,   &
                       INI, ISCAL, IBANDS,                        &
                       CSV,XCO2,XRHOA,                            &
                       XZENITH,XAZIM,XSW_BANDS,XDIR_ALB,XSCA_ALB, &
                       XEMIS,XTSRAD,                              &
...
DO JFORC_STEP=1,INB_STEP_ATM
   ...
   DO JSURF_STEP=1,INB_ATM
       ...
       !
       CALL IO_BUFF_CLEAN_n
       CALL COUPLING_SURF_ATM_n(CSURF_FILETYPE, 'E',                         &
       ...
       IF (MOD(ZTIMEC,XTSTEP_OUTPUT) == 0. .AND. CTIMESERIES_FILETYPE/='NONE  ') THEN
          ...
          LTIME_WRITTEN(:)=.FALSE.          
          CALL IO_BUFF_CLEAN_n
          CALL WRITE_SURF_ATM_n(CTIMESERIES_FILETYPE,'ALL')
...
IF ( LRESTART ) THEN
  ...
  CALL IO_BUFF_CLEAN_n
  CALL WRITE_SURF_ATM_n(CSURF_FILETYPE,'ALL')
  IF(CSURF_FILETYPE=='FA    ')THEN
     CALL WRITE_DIAG_SURF_ATM_n(CSURF_FILETYPE,'PGD')
  ELSE
...
!
   ...
   CALL GET_SURF_VAR_n(CSURF_FILETYPE,INI,ISERIES,PSEA=ZSEA,PWATER=ZWATER,PNATURE=ZNATURE,PTOWN=ZTOWN, &
                         PT2M=ZT2M,PQ2M=ZQ2M,PZ0=ZZ0,PZ0H=ZZ0H,PZ0EFF=ZZ0EFF,PQS_SEA=ZQS_SEA,    &
