...
USE MODD_SURFEX_MPI, ONLY : NCOMM, NPROC, NRANK, NPIO, WLOG_MPI, PREP_LOG_MPI,   &
                            END_LOG_MPI, NNUM, NINDEX, NSIZE_TASK, LSFX_MPI
...
!!OV REAL, DIMENSION(0) :: ZZS                                   ! not used
...
IF (NRANK==NPIO) THEN
  IF (CSURF_FILETYPE=='FA    ') THEN
    ...
    IDATEF(4)=FLOOR(YSC%U%TTIME%TIME/3600.) 
    IDATEF(5)=FLOOR(YSC%U%TTIME%TIME/60.) - IDATEF(4) * 60 
    IDATEF(6)=1 
    IDATEF(7:11)=0  
    CALL FAITOU(IRET,NUNIT_FA,.TRUE.,CFILEOUT_FA,'NEW',.TRUE.,.FALSE.,IVERBFA,0,INB,CDNOMC)
    CALL FAUTIF(IRET,NUNIT_FA,'SURFEX')
...
IF (CSURF_FILETYPE=='FA    ' .AND. LMNH_COMPATIBLE .AND. LFAGMAP) CALL WRITE_HEADER_MNH
...
ELSE
#ifdef SFX_MPI
  IF (LSFX_MPI) CALL MPI_FINALIZE(INFOMPI)
#endif
ENDIF
!
!-------------------------------------------------------------------------------
