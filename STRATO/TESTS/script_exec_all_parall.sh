#set -x 

exec_new=$3
exec_old=$4

cp -f OPTIONS.nam OPTIONS.nam_new

echo $1 >> $2

################################

./script_to_old.sh 

./script_base.sh "OLD" "$2"

echo "OLD" >> $2
./pgd.exe $exec_old >> $2
./prep.exe $exec_old >> $2
./offline.exe $exec_old >> $2

./script_test_end.sh "OLD" "$2" "$1"

mv -f *.OUT.nc RES_OLD
mv -f PGD.* RES_OLD
mv -f PREP.* RES_OLD
mv -f SURFOUT.* RES_OLD

rm -f *.fa

################################

cp -f OPTIONS.nam_new OPTIONS.nam

./script_base.sh "NEW" "$2"

echo "NEW" >> $2


#1 thread
echo "SIMPLE" >> $2
export OMP_NUM_THREADS=1
./pgd.exe $exec_new >> $2
./prep.exe $exec_new >> $2
#add test
#mv -f PGD.* RES_NEW
#mv -f PREP.* RES_NEW
#./script_to_old.sh
#./pgd.exe $exec_old >> $2
#./prep.exe $exec_old >> $2
#add test
./offline.exe $exec_new >> $2

./script_test_end.sh "NEW_SIMPLE" "$2" "$1"

mv -f *.OUT.nc RES_NEW
mv -f PGD.nc RES_NEW
mv -f PREP*.nc RES_NEW
mv -f SURFOUT*.nc RES_NEW

cd TESTS/PYTHON

./python.exe "RES_NEW/" "RES_OLD/" "$1_SIMPLE"

cd ../..
rm -f RES_NEW/*

rm -f *.fa

##########1: new MPI4_LIN
echo "MPI4_LIN" >> $2
#4 tasks
./pgd_mpi.exe $exec_new >> $2
./prep_mpi.exe $exec_new >> $2
#add test
#cp -f PGD.* RES_NEW
#cp -f PREP.* RES_NEW
#./script_to_old.sh
#./pgd.exe $exec_old >> $2
#./prep.exe $exec_old >> $2
#add test

cp -f OPTIONS.nam OPTIONS.nam_lin
./offline_mpi.exe $exec_new >> $2

./script_test_end.sh "NEW_MPI4_LIN" "$2" "$1"

mv -f *.OUT.nc RES_NEW
mv -f PGD.nc RES_NEW
mv -f PREP*.nc RES_NEW
mv -f SURFOUT*.nc RES_NEW

cd TESTS/PYTHON

./python.exe "RES_NEW/" "RES_OLD/" "$1_MPI4_LIN"

cd ../..
rm -f RES_NEW/*

rm -f *.fa

##########1: new MPI4_TILL
echo "MPI4_TILL" >> $2
./pgd_mpi.exe $exec_new >> $2
cp -f OPTIONS.nam_lin OPTIONS.nam_save
sed -e "s/YALG\_MPI\ \=\ \"LIN\"/YALG\_MPI\ \=\ \"TILL\"/g" OPTIONS.nam_save > OPTIONS.nam
./prep_mpi.exe $exec_new >> $2
./offline_mpi.exe $exec_new >> $2

./script_test_end.sh "NEW_MPI4_TILL" "$2" "$1"

cp -f PGD.* RES_NEW
cp -f PREP.* RES_NEW
mv -f *.OUT.nc RES_NEW
mv -f SURFOUT*.nc RES_NEW

cd TESTS/PYTHON

./python.exe "RES_NEW/" "RES_OLD/" "$1_MPI4_TILL"

cd ../..
rm -f RES_NEW/*

rm -f *.fa

##########1: new MPI4_ADJ
cp -f OPTIONS.nam_lin OPTIONS.nam
echo "MPI4_ADJ" >> $2
./pgd_mpi.exe $exec_new >> $2
cp -f OPTIONS.nam_lin OPTIONS.nam_save
sed -e "s/YALG\_MPI\ \=\ \"LIN\"/YALG\_MPI\ \=\ \"ADJ\"/g" OPTIONS.nam_save > OPTIONS.nam
./prep_mpi.exe $exec_new >> $2
./offline_mpi.exe $exec_new >> $2

./script_test_end.sh "NEW_MPI4_ADJ" "$2" "$1"

cp -f PGD.* RES_NEW
cp -f PREP.* RES_NEW
mv -f *.OUT.nc RES_NEW
mv -f SURFOUT*.nc RES_NEW

cd TESTS/PYTHON

./python.exe "RES_NEW/" "RES_OLD/" "$1_MPI4_ADJ"

cd ../..
rm -f RES_NEW/*

rm -f *.fa

##########1: new MPI4_TILA
cp -f OPTIONS.nam_lin OPTIONS.nam
echo "MPI4_TILA" >> $2
./pgd_mpi.exe $exec_new >> $2
cp -f OPTIONS.nam_lin OPTIONS.nam_save
sed -e "s/YALG\_MPI\ \=\ \"LIN\"/YALG\_MPI\ \=\ \"TILA\"/g" OPTIONS.nam_save > OPTIONS.nam
./prep_mpi.exe $exec_new >> $2
./offline_mpi.exe $exec_new >> $2

./script_test_end.sh "NEW_MPI4_TILA" "$2" "$1"

cp -f PGD.* RES_NEW
cp -f PREP.* RES_NEW
mv -f *.OUT.nc RES_NEW
mv -f SURFOUT*.nc RES_NEW

cd TESTS/PYTHON

./python.exe "RES_NEW/" "RES_OLD/" "$1_MPI4_TILA"

cd ../..
rm -f RES_NEW/*

rm -f *.fa


#2 tasks 2 threads
cp -f OPTIONS.nam_lin OPTIONS.nam
echo "MPI2OMP2" >> $2
export OMP_NUM_THREADS=2
./pgd_mpiomp.exe $exec_new >> $2
./prep_mpiomp.exe $exec_new >> $2
#add test
#cp -f PGD.* RES_NEW
#cp -f PREP.* RES_NEW
#./script_to_old.sh
#./pgd.exe $exec_old >> $2
#./prep.exe $exec_old >> $2
#add test
./offline_mpiomp.exe $exec_new >> $2

./script_test_end.sh "NEW_MPI2OMP2" "$2" "$1"

mv -f *.OUT.nc RES_NEW
mv -f PGD.nc RES_NEW
mv -f PREP*.nc RES_NEW
mv -f SURFOUT*.nc RES_NEW

cd TESTS/PYTHON

./python.exe "RES_NEW/" "RES_OLD/" "$1_MPI2OMP2"

cd ../..
rm -f RES_NEW/*

rm -f *.fa


#4 thread
#echo "OMP4" >> $2
#export OMP_NUM_THREADS=4
#./pgd_omp.exe $exec_new >> $2
#./prep_omp.exe $exec_new >> $2
##add test
##cp -f PGD.* RES_NEW
##cp -f PREP.* RES_NEW
##./script_to_old.sh
##./pgd.exe $exec_old >> $2
##./prep.exe $exec_old >> $2
##add test
#./offline_omp.exe $exec_new >> $2
#
#./script_test_end.sh "NEW_OMP4" "$2" "$1"
#
#mv -f *.OUT.nc RES_NEW
#mv -f PGD.nc RES_NEW
#mv -f PREP*.nc RES_NEW
#mv -f SURFOUT*.nc RES_NEW
#
#
export OMP_NUM_THREADS=1
#
#
#cd TESTS/PYTHON
#
#./python.exe "RES_NEW/" "RES_OLD/" "$1_OMP4"
#
#cd ../..
rm -f RES_NEW/*
rm -f RES_OLD/*

