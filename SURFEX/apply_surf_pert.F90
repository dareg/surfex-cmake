...
DO JP = 1,IO%NPATCH
    ...
    DO JI = 1,PK%NSIZE_P
      ...
      IF (ASSOCIATED(S%XPERTVEG)) THEN
        IF((S%XPERTVEG(IMASK,JP)<XUNDEF).AND.&
           &(PEK%XVEG(JI)<IO%XPERT_HIGH(JKV)).AND.(PEK%XVEG(JI)>IO%XPERT_LOW(JKV)).AND.&
           &(PEK%XVEG(JI)/=XUNDEF)) THEN 
           PEK%XVEG(JI) = &
           &MAX(MIN(PEK%XVEG(JI) *(1.+ S%XPERTVEG(IMASK,JP) ),IO%XPERT_HIGH(JKV)),IO%XPERT_LOW(JKV))
        ENDIF
      ENDIF
      IF (ASSOCIATED(S%XPERTLAI)) THEN
        IF((S%XPERTLAI(IMASK,JP)<XUNDEF).AND.&
           &(PEK%XLAI(JI)<IO%XPERT_HIGH(JKL)).AND.(PEK%XLAI(JI)>IO%XPERT_LOW(JKL)).AND.&
           &(PEK%XLAI(JI)/=XUNDEF)) THEN 
               PEK%XLAI(JI) = &
               &MAX(MIN(PEK%XLAI(JI) *(1.+ S%XPERTLAI(IMASK,JP) ),IO%XPERT_HIGH(JKL)),IO%XPERT_LOW(JKL))
        ENDIF
      ENDIF
      IF (ASSOCIATED(S%XPERTCV)) THEN
        IF((S%XPERTCV(IMASK,JP)<XUNDEF).AND.&
           &(PEK%XCV(JI)<IO%XPERT_HIGH(JKC)).AND.(PEK%XCV(JI)>IO%XPERT_LOW(JKC)).AND.&
           &PEK%XCV(JI)/=XUNDEF)  THEN 
               PEK%XCV(JI)  = &
