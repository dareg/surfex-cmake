...
!
REAL, DIMENSION(SIZE(PCOVER,1),SIZE(PFIELD,2))   :: ZCOVER_WEIGHT_PATCH
...
INTEGER, DIMENSION(SIZE(PCOVER,1),NVEGTYPE)      :: IDOY
INTEGER, DIMENSION(365)                          :: ICOUNT
   ...
   DO JVEGTYPE=1,NVEGTYPE
      ...
      DO JCOVER=1,ICOVER
         ...
         IF (IDATA_DOY(JCOVER,JVEGTYPE) /= NUNDEF) THEN
            ...
            ICOUNT(IDATA_DOY(JCOVER,JVEGTYPE)) = ICOUNT(IDATA_DOY(JCOVER,JVEGTYPE)) + 1

         END IF

      END DO

      IDOY(JP,JVEGTYPE) = MAXLOC(ICOUNT,1)
      CALL DOY2DATE(IDOY(JP,JVEGTYPE),IMONTH,IDAY)
      TFIELD(JP,JVEGTYPE)%TDATE%MONTH = IMONTH
      TFIELD(JP,JVEGTYPE)%TDATE%DAY   = IDAY
