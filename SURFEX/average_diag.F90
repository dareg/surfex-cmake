...
!       A. Napoly   08/2023 : Add MAKE_AVERAGE_ISBA
...
! Modified weighting giving increased weight to LAND temperature
  ...
  ELSE
    DO JT=1,NTILESFC
      ...
      CALL MAKE_AVERAGE_ISBA(PFRAC_TILE(:,JT),ND%AL(JT)%XT2M,D%XT2M_NAT,JT,PFRAC_TILE(:,3))
...
!
  CALL MAKE_AVERAGE_Z0(PFRAC_TILE(:,JT),D%XUREF,D%XZREF,ND%AL(JT)%XZ0,ND%AL(JT)%XZ0H,D%XZ0,D%XZ0H,JT)
!
  ENDDO
  !
ENDIF
...
!
SUBROUTINE MAKE_AVERAGE_Z0(PFRAC,PUREF,PZREF,PFIELD_IN,PFIELD_INH,PFIELD_OUT,PFIELD_OUTH,KTILE)
!
USE MODD_SURF_PAR, ONLY : XUNDEF
...
REAL, DIMENSION(:),INTENT(IN)   :: PFIELD_IN
REAL, DIMENSION(:),INTENT(IN)   :: PFIELD_INH
REAL, DIMENSION(:), INTENT(OUT) :: PFIELD_OUT
REAL, DIMENSION(:), INTENT(OUT) :: PFIELD_OUTH
INTEGER, INTENT(IN) :: KTILE
REAL :: Z0UNDEF
REAL(KIND=JPHOOK) :: ZHOOK_HANDLE
!
...
ENDIF
!
WHERE ((PFIELD_IN(:)==XUNDEF.OR.PFIELD_INH(:)==XUNDEF) .AND. PFRAC(:)/=0.) 
  PFIELD_OUT (:) = XUNDEF
  PFIELD_OUTH(:) = XUNDEF
END WHERE
!
WHERE (PFIELD_OUT(:)/=XUNDEF) 
  PFIELD_OUT (:) = PFIELD_OUT (:) + PFRAC(:) *  1./(LOG(XZ0_OFFSET + PUREF(:)/MIN(PFIELD_IN (:),Z0UNDEF)))**2
  PFIELD_OUTH(:) = PFIELD_OUTH(:) + PFRAC(:) *  1./(LOG(XZ0_OFFSET + PZREF(:)/MIN(PFIELD_INH(:),Z0UNDEF)) * LOG(XZ0_OFFSET * (1. + PUREF(:)/MIN(PFIELD_IN(:),Z0UNDEF) - PZREF(:)/MIN(PFIELD_INH(:),Z0UNDEF)) + PZREF(:)/MIN(PFIELD_INH(:),Z0UNDEF)))
END WHERE
!
IF (KTILE==NTILESFC) THEN
  WHERE (PFIELD_OUTH(:) == 0.)
    PFIELD_OUTH(:) = XUNDEF
  ELSEWHERE (PFIELD_OUTH(:) /= XUNDEF)
    PFIELD_OUTH(:) = PZREF(:)/(EXP((XZ0_OFFSET * (SQRT(PFIELD_OUT(:)/PFIELD_OUTH(:)) - 1.) + 1.)/SQRT(PFIELD_OUTH(:))) - XZ0_OFFSET)
  ENDWHERE
  !
  WHERE (PFIELD_OUT(:) == 0.)
    PFIELD_OUT(:) = XUNDEF
  ELSEWHERE (PFIELD_OUT(:) /= XUNDEF)
    PFIELD_OUT(:) = PUREF(:)/(EXP( SQRT(1./PFIELD_OUT(:)) ) - XZ0_OFFSET)
  ENDWHERE
ENDIF
!
...
!
SUBROUTINE MAKE_AVERAGE_ISBA(PFRAC,PFIELD_IN,PFIELD_OUT,KTILE,PLAND)
!
USE MODD_SURF_PAR, ONLY : XUNDEF
!
IMPLICIT NONE
!
REAL, DIMENSION(:),INTENT(IN)   :: PFRAC
REAL, DIMENSION(:),INTENT(IN)   :: PLAND
REAL, DIMENSION(:),INTENT(IN)   :: PFIELD_IN
REAL, DIMENSION(:), INTENT(OUT) :: PFIELD_OUT
INTEGER, INTENT(IN) :: KTILE
REAL(KIND=JPHOOK) :: ZHOOK_HANDLE
...
!
IF (KTILE==3) THEN
  WHERE (PFIELD_IN(:)/=XUNDEF .AND. PFIELD_OUT(:)/=XUNDEF) 
    PFIELD_OUT(:)=PFIELD_IN(:)
  END WHERE
ELSE  
  WHERE(PLAND(:)==0. .AND. PFIELD_OUT(:)/=XUNDEF) PFIELD_OUT(:) = PFIELD_OUT(:) + PFRAC(:) * PFIELD_IN(:)
ENDIF  
!
IF (LHOOK) CALL DR_HOOK('AVERAGE_DIAG:MAKE_AVERAGE',1,ZHOOK_HANDLE)
!
END SUBROUTINE MAKE_AVERAGE_ISBA
