...
DO JLAYER=2,KLVL
  ZF(:,JLAYER) = -PK (:,JLAYER) * ZDUDZ(:,JLAYER)
END DO
!
!-------------------------------------------------------------------------------
!
...
!
ZDFDDVDZ(:,2:KLVL) = - PK(:,2:KLVL) - PDKDDVDZ(:,2:KLVL) * ZDUDZ(:,2:KLVL)
!
!
!-------------------------------------------------------------------------------
!
...
!
LIMPL=(KIMPL==1)
CALL TRIDIAG_SURF(PU,ZF,ZDFDDVDZ,ZEXT,ZDEXTDV,PTSTEP, &
                   PDZF,PDZ,ZU,LIMPL,PALFA,PBETA    ) 
!
!-------------------------------------------------------------------------------
!
...
DO JLAYER=2,KLVL
  PUW(:,JLAYER) = PUW(:,JLAYER-1)                                                         &
    +   ( PFORC_U(:,JLAYER-1) + PDFORC_UDU(:,JLAYER-1) * (ZU(:,JLAYER-1)-PU(:,JLAYER-1)) ) &
      * PDZ(:,JLAYER-1) 
END DO
!
!-------------------------------------------------------------------------------
!
!*    9. New value of U (at full levels)
!        --------------
!
!* temporal evolution
PU(:,:) = ZU(:,:)
