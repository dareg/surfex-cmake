!     #########
      SUBROUTINE CARBON_INIT(HPHOTO, HRESPSL, KNBIOMASS,                       &
...
CHARACTER(LEN=*),     INTENT(IN)    :: HRESPSL             ! type of soil respiration
...
IF (HPHOTO=='NCB' .AND. HRESPSL=='CNT') THEN
  !
  !*	 1.     ALLOCATION
  !	
  ...
  ALLOCATE(XTAU_LITTER(KNLITTER))
  ALLOCATE(XFRAC_SOILCARB(KNLITTER,KNSOILCARB,KNLITTLEVS))
  ALLOCATE(XTAU_SOILCARB(KNSOILCARB))
  !
  !
  !*	 2.     BIOMASS CONSTANTS
  !	        -----------------
