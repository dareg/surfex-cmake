!!   ############################################################
     SUBROUTINE CH_AER_EMISSION(PFLUX, PRHODREF, HSV, PCONVERSION, KSV_CHSBEG,  PFCO)
!!   ############################################################
!!
!!    PURPOSE
!!    -------
...
REAL,   DIMENSION(:),        INTENT(IN) :: PRHODREF, PCONVERSION
...
! if MIX then initial aerosols fluxes are in kg.kg-1.m.s-1 (not in ppp.m.s-1)
!  conversion in kg.kg-1.m.s-1

  PFLUX(:,I_CH_SO4i) = PFLUX(:,I_CH_SO4i) / PCONVERSION(:)
  PFLUX(:,I_CH_SO4j) = PFLUX(:,I_CH_SO4j) / PCONVERSION(:)
  PFLUX(:,I_CH_NO3i) = PFLUX(:,I_CH_NO3i) / PCONVERSION(:)
  PFLUX(:,I_CH_NO3j) = PFLUX(:,I_CH_NO3j) / PCONVERSION(:)
  PFLUX(:,I_CH_NH3i) = PFLUX(:,I_CH_NH3i) / PCONVERSION(:)
  PFLUX(:,I_CH_NH3j) = PFLUX(:,I_CH_NH3j) / PCONVERSION(:)
  PFLUX(:,I_CH_H2Oi) = PFLUX(:,I_CH_H2Oi) / PCONVERSION(:)
  PFLUX(:,I_CH_H2Oj) = PFLUX(:,I_CH_H2Oj) / PCONVERSION(:)
  PFLUX(:,I_CH_OCi)  = (PFLUX(:,I_CH_OCi) + ZFCO(:) * ZVALOC / 2.) / PCONVERSION(:) 
  PFLUX(:,I_CH_OCj)  = (PFLUX(:,I_CH_OCj) + ZFCO(:) * ZVALOC ) / PCONVERSION(:)
  PFLUX(:,I_CH_BCi)  = (PFLUX(:,I_CH_BCi) + ZFCO(:) * ZVALBC / 2.) / PCONVERSION(:) 
  PFLUX(:,I_CH_BCj)  = (PFLUX(:,I_CH_BCj) + ZFCO(:) * ZVALBC ) / PCONVERSION(:)
...
!
   ZFM(:,1)= ZFM(:,2) / &
                ((ZEMISRADIUSI**3)*EXP(4.5 * (LOG(XEMISSIGI))**2)) 
!
   ZFM(:,4)= ZFM(:,5) / &
                ((ZEMISRADIUSJ**3)*EXP(4.5 * (LOG(XEMISSIGJ))**2)) 
!
...
!  PFLUX(:,I_CH_SO4i) =  ZFCTOTA(:,JP_AER_SO4,1)*&
!                         6.0221367E+11 / (ZMI(JP_AER_SO4)*ZDEN2MOL * PRHODREF(:))
!  PFLUX(:,I_CH_SO4j) =  ZFCTOTA(:,JP_AER_SO4,2)*&
!                         6.0221367E+11 / (ZMI(JP_AER_SO4)*ZDEN2MOL * PRHODREF(:))
!  PFLUX(:,I_CH_NO3i) = ZFCTOTA(:,JP_AER_NO3,1)*&
!                         6.0221367E+11 / (ZMI(JP_AER_NO3)*ZDEN2MOL * PRHODREF(:))
!  PFLUX(:,I_CH_NO3j) = ZFCTOTA(:,JP_AER_NO3,2)*&
!                         6.0221367E+11 / (ZMI(JP_AER_NO3)*ZDEN2MOL * PRHODREF(:))
!  PFLUX(:,I_CH_NH3i) = ZFCTOTA(:,JP_AER_NH3,1)*&
!                         6.0221367E+11 / (ZMI(JP_AER_NH3)*ZDEN2MOL * PRHODREF(:))
!  PFLUX(:,I_CH_NH3j) =  ZFCTOTA(:,JP_AER_NH3,2)*&
!                         6.0221367E+11 / (ZMI(JP_AER_NH3)*ZDEN2MOL * PRHODREF(:))
!  PFLUX(:,I_CH_H2Oi) = ZFCTOTA(:,JP_AER_H2O,1)*&
!                         6.0221367E+11 / (ZMI(JP_AER_H2O)*ZDEN2MOL * PRHODREF(:))
!  PFLUX(:,I_CH_H2Oj) = ZFCTOTA(:,JP_AER_H2O,2)*&
!                         6.0221367E+11 / (ZMI(JP_AER_H2O)*ZDEN2MOL * PRHODREF(:))
!
!  PFLUX(:,I_CH_OCi) = ZFCTOTA(:,JP_AER_OC,1)*&
!                         6.0221367E+11 / (ZMI(JP_AER_OC)*ZDEN2MOL * PRHODREF(:))
!  PFLUX(:,I_CH_OCj) = ZFCTOTA(:,JP_AER_OC,2)*&
!                         6.0221367E+11 / (ZMI(JP_AER_OC)*ZDEN2MOL * PRHODREF(:))
!  PFLUX(:,I_CH_BCi) = ZFCTOTA(:,JP_AER_BC,1)*&
!                         6.0221367E+11 / (ZMI(JP_AER_BC)*ZDEN2MOL * PRHODREF(:))
!  PFLUX(:,I_CH_BCj) = ZFCTOTA(:,JP_AER_BC,2)*&
!                        6.0221367E+11 / (ZMI(JP_AER_BC)*ZDEN2MOL * PRHODREF(:))
!
 ! then conversion in  molecules.m-2.s-1
  PFLUX(:,I_CH_M0i) = PFLUX(:,I_CH_M0i) * PCONVERSION(:)
  PFLUX(:,I_CH_M0j) = PFLUX(:,I_CH_M0j) * PCONVERSION(:)

  IF (LVARSIGI) PFLUX(:,I_CH_M6i) = PFLUX(:,I_CH_M6i) * PCONVERSION(:) 
  IF (LVARSIGJ) PFLUX(:,I_CH_M6j) = PFLUX(:,I_CH_M6j) * PCONVERSION(:)
  ...
  PFLUX(:,I_CH_H2Oi) = PFLUX(:,I_CH_H2Oi) * PCONVERSION(:)
  PFLUX(:,I_CH_H2Oj) = PFLUX(:,I_CH_H2Oj) * PCONVERSION(:)
  PFLUX(:,I_CH_OCi) = PFLUX(:,I_CH_OCi)   * PCONVERSION(:)
  PFLUX(:,I_CH_OCj) = PFLUX(:,I_CH_OCj)   * PCONVERSION(:)
  PFLUX(:,I_CH_BCi) = PFLUX(:,I_CH_BCi)   * PCONVERSION(:)
  PFLUX(:,I_CH_BCj) = PFLUX(:,I_CH_BCj)   * PCONVERSION(:)
