...
END DO
!
IF ((LCH_AERO_FLUX).AND.(GCO).AND.(NSV_AERBEG > 0)) THEN
     CALL CH_AER_EMISSION(ZEMIS, PRHOA, CSV, XCONVERSION, NSV_CHSBEG, PFCO=ZFCO)
ELSE IF ((LCH_AERO_FLUX).AND.(NSV_AERBEG > 0)) THEN
     CALL CH_AER_EMISSION(ZEMIS, PRHOA, CSV, XCONVERSION, NSV_CHSBEG)
END IF
!
PSFSV(:,:) = PSFSV(:,:) + ZEMIS(:,:)
!
