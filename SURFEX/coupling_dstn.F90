SUBROUTINE COUPLING_DST_n(  &
       ...
       PSFTH,                    &!I [W/m2] surface heat flux
       PSFTQ,                    &!I [kg/m2/s] surface vapor flux
  ...
  USE MODD_CSTS, ONLY :            &
         XRD                    ![J/K/kg] The universal gas constant  
       
  USE MODD_PACK_ISBA, ONLY :       &
         XP_VEGTYPE_PATCH       !Fraction of all vegtypes for all patches  
  ...
  USE MODE_DSTMBL   !Dust mobilization routines
  USE MODE_DSTMBL_MB   !Dust mobilization routines (M. Mokhtari)  
!
...
!
  ...
  REAL, DIMENSION(SIZE(PTA))         :: ZCPS           !I [] surface heat capacity

  INTEGER                            :: JVEG           ![idx] counter for vegetation types

  INTEGER             :: JMODE            !Counter for dust modes
  REAL                :: ZPERC_FIN, ZPERC_ACCU !Percentage of mass within the
  REAL(KIND=JPRB) :: ZHOOK_HANDLE
                                               !fin and accumulation modes
  ...
  CONTAINS
    ...
    SUBROUTINE TREAT_SURF(KSIZE,KMASK)
      ...
      REAL, DIMENSION(KSIZE) :: ZP_SFTH       ![W/m2] surface sensible heat flux
      REAL, DIMENSION(KSIZE) :: ZP_SFTQ       ![kg/m2/s] water vapor flux
      ...
      REAL, DIMENSION(KSIZE) :: ZP_CD         !I [] drag coefficient from isba
      ...
      INTEGER                :: JJ            !Counter for vector points
      INTEGER                :: JMODE         
      REAL(KIND=JPRB) :: ZHOOK_HANDLE
      ...
      DO JJ=1,KSIZE
        ...
        ZP_SFTH(JJ)   =    PSFTH    (KMASK(JJ))
        ZP_SFTQ(JJ)   =    PSFTQ    (KMASK(JJ))
      ...
      ENDDO
      ZP_CD_DST  = 0.  ! output value from surface_cd
      ZP_CDN     = 0.  ! output value from surface_cd
     ...
     IF (CVERMOD=='CMDVER') THEN
       !Get wind friction speed over erodible surface
       ! We use the drag coefficient from isba
       ZP_USTAR(:) =  sqrt(ZP_CD(:))*ZP_VMOD
       !Get zonal friction speed (m/s)
       ZP_SFZON(:) = - sqrt(ZP_CD(:))*ZP_U(:)

       !Get meridional surface stress
       ZP_SFMER(:) = - sqrt(ZP_CD(:))*ZP_V(:)
!
     ...
     CALL PARAM_CLS(                    &
          ...
          ,ZP_PS                        & !I [Pa] atmospheric pressure
          ,ZP_RHOA                      & !I [kg/m3] atmospheric density
          ,ZP_U                         & !I [m/s] zonal wind at atm. height
          ,ZP_V                         & !I [m/s] meridional wind at atm. height
          ,ZP_ZREF                      & !I [m] reference height for wind
          ,ZP_UREF                      & !I [m] reference height for wind
          ,ZP_SFTH                      & !I [W/m2] heat flux
          ,ZP_SFTQ                      & !I [kg/m2/s] water vapor flux
          ,ZP_SFZON                     & !I [??] Zonal friction
          ,ZP_SFMER                     & !I [??] meridional friction
          ,ZP_T2M                       & !O [K] 2M temperature
          ,ZP_Q2M                       & !O [kg_{H2O}/kg_{air}] 2M specific humidity
          ,ZP_HU2M                      & !O [-] 2M relative humidity
          ,ZP_ZON10M                    & !O [m/s] zonal wind at 2M
          ,ZP_MER10M                    & !O [m/s] meridional wind at 2M
          )
!
...
!
     ...
     CALL dustflux_get_MB(                &
          ...
          ,NSIZE_PATCH_DST(JVEG,KPATCH)   & !I [nbr] number of points for which we do calculation
          )
     ...
     ELSE 
       ...
       CALL SURFACE_CD(ZP_RI, ZP_Z0_EROD, ZP_UREF, ZP_Z0_EROD, ZP_Z0H_WITH_SNOW, ZP_CD_DST, ZP_CDN)
       !Get wind friction speed over erodible surface
       ...
       !Use DEAD model (http://dust.ess.uci.edu/dead/) to predict dust fluxes for 
       !the erodible surface
       ZP_WIND10M(:) = MAX(ZP_WIND10M(:), 1E-2)
     ...
     IF (CEMISPARAM == 'EXPLI') THEN
       ...
       !  0 % < mode 2 < 36 %
       !  63 % < mode 3 < 100 %
       WHERE ((ZP_USTAR(:) >= 35E-2).AND.(ZP_USTAR(:) < 42E-2))
          ZP_MSS_FRC_SRC(:,1)  = &
            0.01 * (ZP_USTAR(:) - 35E-2) / (42E-2 - 35E-2)  
          ZP_MSS_FRC_SRC(:,2)  = &
            0.36 * (ZP_USTAR(:) - 35E-2) / (42E-2 - 35E-2)  
          ZP_MSS_FRC_SRC(:,3)  = &
            1. - (1. - 0.63) * (ZP_USTAR(:) - 35E-2) / (42E-2 - 35E-2)  
       ENDWHERE
       ! Case of  42E-2 m/s < u* < 50E-2 m/s
       !  1 % < mode 1 < 8 %
       !  36 % < mode 2 < 43 %
       !  49 % < mode 3 < 63 %
       WHERE ((ZP_USTAR(:) >= 42E-2).AND.(ZP_USTAR(:) < 50E-2))
          ZP_MSS_FRC_SRC(:,1)  = &
            0.01 + (0.08 - 0.01) * (ZP_USTAR(:) - 42E-2) / (50E-2 - 42E-2)  
          ZP_MSS_FRC_SRC(:,2)  = &
            0.36 + (0.43 - 0.36) * (ZP_USTAR(:) - 42E-2) / (50E-2 - 42E-2)  
          ZP_MSS_FRC_SRC(:,3)  = &
            0.63 - (0.63 - 0.49) * (ZP_USTAR(:) - 42E-2) / (50E-2 - 42E-2)  
       ENDWHERE
       ! Case of  50E-2 m/s < u* < 66E-2 m/s
       !  8 % < mode 1 < 15 %
       !  43 % < mode 2 < 76 %
       !  9  % < mode 3 < 49 %
       WHERE ((ZP_USTAR(:) >= 50E-2).AND.(ZP_USTAR(:) < 66E-2))
          ZP_MSS_FRC_SRC(:,1)  = &
            0.08 + (0.15 - 0.08) * (ZP_USTAR(:) - 50E-2) / (66E-2 - 50E-2)  
          ZP_MSS_FRC_SRC(:,2)  = &
            0.43 + (0.76 - 0.43) * (ZP_USTAR(:) - 50E-2) / (66E-2 - 50E-2)  
          ZP_MSS_FRC_SRC(:,3)  = &
            0.49 - (0.49 - 0.09) * (ZP_USTAR(:) - 50E-2) / (66E-2 - 50E-2)  
       ENDWHERE
       ! Case of  u* > 66E-2 m/s 
       !   mode 1 = 15 %
       !   mode 2 = 76 %
       !   mode 3 = 9 %
       WHERE (ZP_USTAR(:) >= 66E-2)
          ZP_MSS_FRC_SRC(:,1)  = 0.15
          ZP_MSS_FRC_SRC(:,2)  = 0.76
          ZP_MSS_FRC_SRC(:,3)  = 0.09
       ENDWHERE

       DO JMODE=1,NDSTMDE
         ! Explicit mass fraction from u*
         ZP_SFDST_MDE(:,JMODE) = ZP_SFDST(:)  &!Total mass flux of dust
             *ZP_MSS_FRC_SRC(:,JORDER(JMODE))   !Fraction of dust going to each mode  
       ENDDO

     ELSEIF (CEMISPARAM == 'AMMA') THEN
       ...
       ! mode 2 = 0
       ! mode 3 = 100
       WHERE (ZP_USTAR(:) < 30E-2)
         ZP_MSS_FRC_SRC(:,3) = 1.
       ENDWHERE
       ! Case of  30E-2 m/s < u* < 35E-2 m/s
       !  0 % < mode 1 < 2*ZPERC_FIN %
       !  0 % < mode 2 < 2*ZPERC_ACCU %
       !  98 % < mode 3 < 100 %
       WHERE ((ZP_USTAR(:) >= 30E-2).AND.(ZP_USTAR(:) < 35E-2))
          ZP_MSS_FRC_SRC(:,1)  = &
            2.*ZPERC_FIN * (ZP_USTAR(:) - 30E-2) / (35E-2 - 30E-2)  
          ZP_MSS_FRC_SRC(:,2)  = &
            2.*ZPERC_ACCU * (ZP_USTAR(:) - 30E-2) / (35E-2 - 30E-2)  
          ZP_MSS_FRC_SRC(:,3)  = &
            1. - (1. - 0.98) * (ZP_USTAR(:) - 30E-2) / (35E-2 - 30E-2)  
       ENDWHERE
       ! Case of  35E-2 m/s < u* < 40E-2 m/s
       !  2 % < mode 1 < 5*ZPERC_FIN %
       !  2 % < mode 2 < 5*ZPERC_ACCU %
       !  95 % < mode 3 < 98%
       WHERE ((ZP_USTAR(:) >= 35E-2).AND.(ZP_USTAR(:) < 40E-2))
          ZP_MSS_FRC_SRC(:,1)  = &
            2.*ZPERC_FIN+3.*ZPERC_FIN * (ZP_USTAR(:) - 35E-2) / (40E-2 - 35E-2)  
          ZP_MSS_FRC_SRC(:,2)  = &
            2.*ZPERC_ACCU+ 3.*ZPERC_ACCU * (ZP_USTAR(:) - 35E-2) / (40E-2 - 35E-2)  
          ZP_MSS_FRC_SRC(:,3)  = &
            0.98 - (0.98 - 0.95) * (ZP_USTAR(:) - 35E-2) / (40E-2 - 35E-2)  
       ENDWHERE
       ! Case of  40E-2 m/s < u* < 55E-2 m/s
       !  5.*ZPERC_FIN % < mode 1 < 36.*ZPERC_FIN %COUPLING_DST_n:
       !  5.*ZPERC_ACCU % < mode 2 < 36.*ZPERC_ACCU %
       !  64 % < mode 3 < 95 %
       WHERE ((ZP_USTAR(:) >= 40E-2).AND.(ZP_USTAR(:) < 55E-2))
          ZP_MSS_FRC_SRC(:,1)  = &
            5.*ZPERC_FIN + 31.*ZPERC_FIN * (ZP_USTAR(:) - 40E-2) / (55E-2 - 40E-2)  
          ZP_MSS_FRC_SRC(:,2)  = &
            5.*ZPERC_ACCU + 31.*ZPERC_ACCU * (ZP_USTAR(:) - 40E-2) / (55E-2 - 40E-2)  
          ZP_MSS_FRC_SRC(:,3)  = &
            0.95 - (0.95 - 0.64) * (ZP_USTAR(:) - 40E-2) / (55E-2 - 40E-2)  
       ENDWHERE
       ! Case of  55E-2 m/s < u* < 80E-2 m/s
       !  36.*ZPERC_FIN % < mode 1 < 56.*ZPERC_FIN %
       !  36.*ZPERC_ACCU % < mode 2 < 56.*ZPERC_ACCU %
       !  44  % < mode 3 < 64 %
       WHERE ((ZP_USTAR(:) >= 55E-2).AND.(ZP_USTAR(:) < 80E-2))
          ZP_MSS_FRC_SRC(:,1)  = &
            36.*ZPERC_FIN + 20.*ZPERC_FIN * (ZP_USTAR(:) - 55E-2) / (80E-2 - 55E-2)  
          ZP_MSS_FRC_SRC(:,2)  = &
            36.*ZPERC_ACCU + 20.*ZPERC_ACCU * (ZP_USTAR(:) - 55E-2) / (80E-2 - 55E-2)  
          ZP_MSS_FRC_SRC(:,3)  = &
            0.64 - (0.64 - 0.44) * (ZP_USTAR(:) - 55E-2) / (80E-2 - 55E-2)  
       ENDWHERE
       ! Case of  u* > 80E-2 m/s 
       !   mode 1 = 56.*ZPERC_FIN %
       !   mode 2 = 56.*ZPERC_ACCU %
       !   mode 3 = 44 %
       WHERE (ZP_USTAR(:) >= 80E-2)
          ZP_MSS_FRC_SRC(:,1)  = 56.*ZPERC_FIN 
          ZP_MSS_FRC_SRC(:,2)  = 56.*ZPERC_ACCU
          ZP_MSS_FRC_SRC(:,3)  = 0.44
       ENDWHERE
     ...
     ELSE
       ! Alf parameterization
       DO JMODE=1,NDSTMDE
         ZP_SFDST_MDE(:,JMODE) = ZP_SFDST(:)  &!Total mass flux of dust
             *XMSS_FRC_SRC(JMODE)               !Fraction of dust going to each mode  
       ENDDO
     END IF
