!     ###############################################################################
SUBROUTINE COUPLING_FLAKE_SBL_n(HPROGRAM, HCOUPLING,                                        &
...
!     ###############################################################################
!
!!****  *COUPLING_FLAKE_SBL_n * - Adds a SBL into FLAKE
!!
!!    PURPOSE
!!    -------
!
...
USE MODD_FLAKE_n,          ONLY : LSBL, XTS, XZ0
...
!
  IF(ANY(XT(:,:) == XUNDEF)) THEN
    CALL INIT_WATER_SBL(NLVL, PPA, PPS, PTA, PQA, PRHOA, PU, PV, PRAIN, PSNOW,     &
                         PSFTH, PSFTQ, PZREF, PUREF, XTS, XZ0, XZ, 1,               &
                         XT, XQ, XU, XTKE, XP) 
  ENDIF  
!
!
...
!              ---------------------
!
IF (.NOT. LSBL .AND. LHOOK) CALL DR_HOOK('COUPLING_FLAKE_SBL_N',1,ZHOOK_HANDLE)
IF (.NOT. LSBL) RETURN
!
!-------------------------------------------------------------------------------------
!
