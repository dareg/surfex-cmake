...
!
  ...
  !LOCAL VARIABLES
  REAL,DIMENSION(size(PSFSLT,1),3) :: ZSFSLT_MDE ! sea salt flux from modes
  INTEGER             :: JN, JI         !Counter for sea salt modes
  REAL                :: ZCONVERTFACM0  ![kg/mole*mole/molec] conversion factor
                                        !for moment fluxes and used fluxes
  REAL                :: ZCONVERTFACM6, ZCONVERTFACM3 
  INTEGER, DIMENSION(JPMODE_SLT) :: NM0, NM3, NM6
...
ZCONVERTFACM3 = 4./3.*XPI*XDENSITY_SLT / 1.d18
...
PSFSLT(:,2+(JN-1)*3) = PSFSLT(:,1+(JN-1)*3) * &
                     (XEMISRADIUS_SLT(JN)**3)*EXP(4.5 * LOG(XEMISSIG_SLT(JN))**2)  
PSFSLT(:,3+(JN-1)*3) = PSFSLT(:,1+(JN-1)*3) * &
                     (XEMISRADIUS_SLT(JN)**6)*EXP(18. * LOG(XEMISSIG_SLT(JN))**2)  

! Conversion into fluxes
PSFSLT(:,1+(JN-1)*3) = PSFSLT(:,1+(JN-1)*3) * ZCONVERTFACM0
PSFSLT(:,2+(JN-1)*3) = PSFSLT(:,1+(JN-1)*3) * ZCONVERTFACM3
PSFSLT(:,3+(JN-1)*3) = PSFSLT(:,3+(JN-1)*3) * ZCONVERTFACM6

ELSE IF (LRGFIX_SLT) THEN
PSFSLT(:,JN) =  ZSFSLT_MDE(:,JORDER_SLT(JN)) * &
                     (XEMISRADIUS_SLT(JN)**3)*EXP(4.5 * LOG(XEMISSIG_SLT(JN))**2) 
! Conversion into fluxes
PSFSLT(:,JN) = PSFSLT(:,JN) * ZCONVERTFACM3

ELSE
PSFSLT(:,1+(JN-1)*2) = ZSFSLT_MDE(:,JORDER_SLT(JN)) 
PSFSLT(:,2+(JN-1)*2) = PSFSLT(:,1+(JN-1)*2) * &
                     (XEMISRADIUS_SLT(JN)**3)*EXP(4.5 * LOG(XEMISSIG_SLT(JN))**2) 

! Conversion into fluxes
PSFSLT(:,1+(JN-1)*2) = PSFSLT(:,1+(JN-1)*2) * ZCONVERTFACM0
PSFSLT(:,2+(JN-1)*2) = PSFSLT(:,1+(JN-1)*2) * ZCONVERTFACM3
