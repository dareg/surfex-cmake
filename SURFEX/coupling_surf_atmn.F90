!     #################################################################################
SUBROUTINE COUPLING_SURF_ATM_n(HPROGRAM, HCOUPLING,                                        &
...
ELSE IF (CROUGH=="BE04") THEN
  CALL SSO_BE04_FRICTION_n(PTSTEP,PUREF,PRHOA,PU,PV,ZPEW_A_COEF,ZPEW_B_COEF,PSFU,PSFV)
END IF
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
!cdir unroll=8
  ...
  ENDDO
ELSE
  ZP_PEW_A_COEF(:) = XUNDEF
  ZP_PEW_B_COEF(:) = XUNDEF
  ZP_PET_A_COEF(:) = XUNDEF
  ZP_PET_B_COEF(:) = XUNDEF
  ZP_PEQ_A_COEF(:) = XUNDEF
  ZP_PEQ_B_COEF(:) = XUNDEF
END IF
!
!--------------------------------------------------------------------------------------------
!
IF (KTILE==1) THEN
  !
  CALL COUPLING_SEA_n(HPROGRAM, HCOUPLING,                                                   &
...
ELSEIF (KTILE==2) THEN
  !
  CALL COUPLING_INLAND_WATER_n(HPROGRAM, HCOUPLING,                                           &
               PTSTEP, KYEAR, KMONTH, KDAY, PTIME,                                            &
               NSIZE_WATER, KSV, KSW,                                                         &
               ZP_TSUN, ZP_ZENITH, ZP_ZENITH2,ZP_AZIM,                                        &
               ZP_ZREF, ZP_UREF, ZP_ZS, ZP_U, ZP_V, ZP_QA, ZP_TA, ZP_RHOA, ZP_SV, ZP_CO2, HSV,&
...
ELSEIF (KTILE==3) THEN
  !
  CALL COUPLING_NATURE_n(HPROGRAM, HCOUPLING,                                                 &
               PTSTEP, KYEAR, KMONTH, KDAY, PTIME,                                            &
...
ELSEIF (KTILE==4) THEN
  !
  CALL COUPLING_TOWN_n(HPROGRAM, HCOUPLING,                                                   &
               PTSTEP, KYEAR, KMONTH, KDAY, PTIME,                                            &
               NSIZE_TOWN, KSV, KSW,                                                          &
