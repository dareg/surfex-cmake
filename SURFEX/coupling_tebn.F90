...
USE MODD_TEB_CANOPY_n, ONLY : XZ, XU, NLVL, XTKE, XT, XQ, XLMO, XLM,XLEPS,XZF, XDZ, XDZF, XP, XQ0
...
REAL, DIMENSION(SIZE(PTA),1):: ZESNOW_GARDEN ! green area snow emissivity
!
...
REAL, DIMENSION(NDSTMDE)    :: ZEMISRADIUS   ! Dust emission radius (um)
REAL, DIMENSION(NDSTMDE)    :: ZEMISSIG      ! Dust emission standard deviation
INTEGER                     :: JMODE_IDX, JMODE ! loop index
...
REAL, DIMENSION(SIZE(PTA))        :: ZUSTAR_ROAD
REAL, DIMENSION(SIZE(PTA))        :: ZUSTAR_ROOF
REAL, DIMENSION(SIZE(PTA))        :: ZU_ROOF   ! wind layer just above roof
REAL, DIMENSION(SIZE(PTA))        :: ZZ_ROOF   ! heigth of layer just above roof
...
!
!-------------------------------------------------------------------------------------
!* Heat flux from the canyon (impacts turbulence in the canyon next timestep 
!-------------------------------------------------------------------------------------
!
XQ0(:) = ZH_GRND (:) / XCPD / PRHOA(:) 
!
END IF
!
!-------------------------------------------------------------------------------------
