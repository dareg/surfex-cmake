!     ###############################################################################
SUBROUTINE COUPLING_WATFLUX_SBL_n(HPROGRAM, HCOUPLING,                                     &
...
!     ###############################################################################
!
!!****  *COUPLING_WATFLUX_SBL_n * - Adds a SBL into WATFLUX
!!
!!    PURPOSE
!!    -------
!
...
USE MODD_WATFLUX_n,        ONLY : LSBL, XTS, XZ0
USE MODD_WATFLUX_SBL_n,    ONLY : XZ, XU, NLVL, XTKE, XT, XQ, XLMO, XZF, XDZ, XDZF, XP
USE MODD_DIAG_WATFLUX_n,   ONLY : N2M, XT2M, XQ2M, XHU2M, XZON10M, XMER10M
!
...
REAL, DIMENSION(SIZE(PTA))   :: ZTEMPO
INTEGER                      :: J
INTEGER                      :: JLAYER
...
!
  IF(ANY(XT(:,:) == XUNDEF)) THEN
    CALL INIT_WATER_SBL(NLVL, PPA, PPS, PTA, PQA, PRHOA, PU, PV, PRAIN, PSNOW,     &
                        PSFTH, PSFTQ, PZREF, PUREF, XTS, XZ0, XZ, 4,               &
                        XT, XQ, XU, XTKE, XP)
  ENDIF
!
!*      1.3    Allocations
...
!
!*      1.4   Computes coefficients for implicitation
!             ---------------------------------------
!
...
!
!*     1.4     Goes from atmospheric forcing to canopy forcing height
!              ------------------------------------------------------
!
  GCOUPLING ='I'
  ZTA = XT(:,1)
...
!              ---------------------
!
IF (.NOT. LSBL .AND. LHOOK) CALL DR_HOOK('COUPLING_WATFLUX_SBL_N',1,ZHOOK_HANDLE)
IF (.NOT. LSBL) RETURN
!
!-------------------------------------------------------------------------------------
!
