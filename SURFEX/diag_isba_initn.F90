...
!    GDIM = (IVERSION>8 .OR. IVERSION==8 .AND. IBUG>0)
    GDIM = ((IVERSION>8 .OR. IVERSION==8 .AND. IBUG>0) .AND. LSPLIT_PATCH)
    !
#ifdef SFX_OL
    IF(DE%LWATER_BUDGET .OR. (LRESTART .AND. .NOT.DGO%LRESET_BUDGETC))THEN
#else
...
!
ALLOCATE(DMA%XSNOWALB      (KLUA))
ALLOCATE(DMA%XSNOWRHO      (KLUA))
ALLOCATE(DMA%XWR           (KLUA))
ALLOCATE(DMA%XWGI          (KLUA,KNLAYER))
ALLOCATE(DMA%XTG           (KLUA,KNLAYER))
!
...
IF(IO%CISBA=='DIF'.AND.DM%LSURF_MISC_DIF)THEN
  ALLOCATE(DMA%XFRD2_TG  (KLUA))
  ALLOCATE(DMA%XFRD2_SWI (KLUA))
  ALLOCATE(DMA%XFRD2_TSWI(KLUA))
  ALLOCATE(DMA%XFRD2_TWG (KLUA))
  ALLOCATE(DMA%XFRD2_TWGI(KLUA))
  ALLOCATE(DMA%XFRD3_TG  (KLUA))
  ALLOCATE(DMA%XFRD3_SWI (KLUA))
  ...
  !
ELSE
  ALLOCATE(DMA%XFRD2_TG  (0))
  ALLOCATE(DMA%XFRD2_SWI (0))
  ALLOCATE(DMA%XFRD2_TSWI(0))
  ALLOCATE(DMA%XFRD2_TWG (0))
  ALLOCATE(DMA%XFRD2_TWGI(0))
  ALLOCATE(DMA%XFRD3_TG  (0))
  ALLOCATE(DMA%XFRD3_SWI (0))
