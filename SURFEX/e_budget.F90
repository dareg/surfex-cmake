...
!
REAL, DIMENSION(SIZE(PALB)) :: ZUSTAR2, ZVMOD, ZFF
!                              ZUSTAR2 = friction
!                              ZVMOD   = wind modulus
!                              ZFF     = Effective flood vraction if ISBA-ES
...
!
   ZCONDAVG(:) = (PD_G(:,1)*PSOILCONDZ(:,1) + (PD_G(:,2)-PD_G(:,1))*PSOILCONDZ(:,2))/     &
                   PD_G(:,2)  
   ZA(:)       = ZA(:) - (2. * XPI / XDAY)                                    &
                   + 2.*ZCONDAVG(:)*PCT(:)/PD_G(:,2)  
...
!
   CALL SOIL_HEATDIF(PTSTEP,PD_G,PSOILCONDZ,PSOILHCAPZ,PCT,                   &
                      ZTERM1,ZTERM2,PTDEEP,PTG                                  )  
!
