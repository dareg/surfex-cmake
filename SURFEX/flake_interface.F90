...
! Switches to configure FLake runs
                               lflk_botsed, LWATFLX, PPEW_A_COEF, PPEW_B_COEF, rho_a )  
!------------------------------------------------------------------------------
!
! Description:
!
...
LOGICAL ::  LWATFLX     ! Switch, .TRUE. -> compute the surface fluxes with water_flux
...
!------------------------------------------------------------------------------
   ...
   Q_w_flk = Q_w_flk - SfcFlx_lwradwsfc(T_sfc(JL))  ! Radiation of the surface (notice the sign)
