...
USE MODD_DIAG_TEB_GARDEN_n, ONLY: XCG, XC1, XC2, XWGEQ, XCT, XRS,          &
                                    ...
                                    XSNOWFREE_ALB, XTS, XTSRAD, XRRVEG,      &
                                    XALBT, XEMIST, XGPP, XRESP_AUTO, XRESP_ECO  
!
USE MODI_ISBA
...
CALL ISBA(CISBA, CPHOTO, CRUNOFF, CC1DRY, CSCOND, CSOILFRZ,      &
          ...
          TPTIME, OFLOOD, OTEMP_ARP, OGLACIER,                   &
          PPEW_A_COEF, PPEW_B_COEF,                              &
          PPET_A_COEF, PPEQ_A_COEF, PPET_B_COEF, PPEQ_B_COEF,    &
          PTSTEP, PZ_LOWCAN, PZ_LOWCAN, ZDIRCOSZW,               &
          PT_LOWCAN, PQ_LOWCAN, PEXNS, PRHOA, PPS, PEXNS,        &
          PRR, PSR, PZENITH, PSW, PLW,                           &
          PU_LOWCAN,                                             &
          XVEG(:,1), XLAI(:,1), XWRMAX_CF(:,1), XRSMIN(:,1),     &
          XGAMMA(:,1), XCV(:,1), XRGL(:,1),                      &
          XRUNOFFD(:,1), XEMIS(:,1), XSNOWFREE_ALB, XZ0(:,1),    &
          XZ0(:,1)/XZ0_O_Z0H(:,1), XZ0(:,1),                     &
          XRUNOFFB, XWDRAIN,                                     &
          XCGSAT, XC1SAT(:,1), XC2REF(:,1), XC3(:,:,1),          &
          XC4B, XC4REF(:,1), XACOEF, XPCOEF, XTAUICE,            &
          XCG, XC1, XC2, XWGEQ, XCT, XRS,                        &
          XTDEEP, XGAMMAT, XWR(:,1), XRESA(:,1),                 &
          XCH, XCD, XCDN, XRI, XHU, XHUG,                        &
          XPSN(:,1), XPSNG(:,1), XPSNV(:,1),                     &
          XRN, XH, XLE(:,1), XLEI, XLEG, XLEGI, XLEV, XLES, XLER,&
          XLETR, XEVAP, XGFLUX, XRESTORE,                        &
          XUSTAR, XDRAIN, XRUNOFF, XMELT,                        &
          XVEGTYPE_PATCH(:,:,1), XAN(:,1), XANF(:,1),            &
          XANFM(:,1), XANDAY(:,1), ZRESP_BIOMASS_INST,           &
          XRESP_BIOMASS(:,:,1), XABC, XPOI, PCO2,                &
          XFZERO(:,1), XEPSO(:,1), XGAMM(:,1), XQDGAMM(:,1),     &
          XGMES(:,1), XQDGMES(:,1), XT1GMES(:,1), XT2GMES(:,1),  &
          XAMAX(:,1), XQDAMAX(:,1), XT1AMAX(:,1), XT2AMAX(:,1),  &
          LSTRESS(:,1), XF2I(:,1), XGC(:,1), XAH(:,1), XBH(:,1), &
          XDMAX(:,1), TSNOW%WSNOW(:,:,1), TSNOW%HEAT(:,:,1),     &
          TSNOW%RHO(:,:,1), TSNOW%ALB(:,1), TSNOW%GRAN1(:,:,1),  &
          TSNOW%GRAN2(:,:,1), TSNOW%HIST(:,:,1),TSNOW%AGE(:,:,1),&
          ZRNSNOW, ZHSNOW,  ZHPSNOW, ZSMELTFLUX,                 &
          ZGFLUXSNOW, ZUSTARSNOW, ZGRNDFLUX, ZSRSFC, ZRRSFC,     &
          ZLESL, TSNOW%EMIS(:,1), ZCDSNOW, ZCHSNOW, XSNOWTEMP,   &
          XSNOWLIQ, XSNOWDZ, PTS_GARDEN, XTS, XSNOWHMASS,        &
          ZRN_ISBA, ZH_ISBA, ZLEG_ISBA, ZLEGI_ISBA, ZLEV_ISBA,   &
          ZLETR_ISBA, ZUSTAR_ISBA, ZLER_ISBA, ZLE_ISBA,          &
          ZLEI_ISBA, ZGFLUX_ISBA, XMELTADV, XDG(:,:,1),          &
          XROOTFRAC(:,:,1), XTG(:,:,1), XWG(:,:,1), XWGI(:,:,1), &
          XPCPS(:,1), XPLVTT(:,1), XPLSTT(:,1),                  &
          XWFC, XWWILT, XWSAT, XBCOEF, XCONDSAT(:,:,1), XMPOTSAT,&
          XHCAPSOIL, XCONDDRY, XCONDSLD,                         &
          XEMIST, XALBT, XIACAN, XHV,                            &
          XSNOWFREE_ALB_VEG, XSNOWFREE_ALB_SOIL,                 &
          XPSNV_A(:,1), XQS, XIRRIG(:,1), XWATSUP(:,1),          &
          XTHRESHOLDSPT(:,1), LIRRIGATE(:,1), LIRRIDAY(:,1),     &
          XCGMAX, CKSAT, HRAIN, CHORT,                           &
          PMUF, PFSAT, XKSAT_ICE(:,1), XD_ICE(:,1), XHORT,       &
          XDRIP, XGPP, ZFFLOOD, ZPIFLOOD, ZIFLOOD,               &
          ZPFLOOD, ZLEFLOOD, ZLEIFLOOD,                          &
          ZFFG, ZFFV, ZFF, ZALBF, ZEMISF,                        &
          ZFFG_NOSNOW, ZFFV_NOSNOW, ZFFROZEN,                    &
          XRRVEG, XCONDSAT_EXP(:,:,1), XEXP_DIF(:,:,1),          &
          ZALPHA, ZNVG, ZMVG, ZLVG, ZWRESVG,                     &
          PAC_AGG_GARDEN, PHU_AGG_GARDEN, PSODELX, XLAT, XLON    )          
!
