...
!     ##############################################################
!
!!**** *GET_GRID_DIM_CARTESIAN* get the grid mesh dimensions
!!
!!    PURPOSE
!!    -------
!!
...
!----------------------------------------------------------------------------
!
IF (LHOOK) CALL DR_HOOK('GET_GRID_DIM_CARTESIAN',0,ZHOOK_HANDLE)
ORECT = .TRUE.
!
 CALL GET_GRIDTYPE_CARTESIAN(PGRID_PAR,KIMAX=KDIM1,KJMAX=KDIM2)
IF (LHOOK) CALL DR_HOOK('GET_GRID_DIM_CARTESIAN',1,ZHOOK_HANDLE)
!
!-------------------------------------------------------------------------------
!
