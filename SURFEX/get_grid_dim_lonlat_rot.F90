...
!     ##############################################################
!
!!**** *GET_GRID_DIM_LONLAT_ROT* get the grid mesh dimensions
!!
!!    PURPOSE
!!    -------
!!
...
!----------------------------------------------------------------------------
!
IF (LHOOK) CALL DR_HOOK('GET_GRID_DIM_LONLAT_ROT',0,ZHOOK_HANDLE)
ORECT = .TRUE.
!
 CALL GET_GRIDTYPE_LONLAT_ROT(PGRID_PAR,KLON=KDIM1,KLAT=KDIM2)
IF (LHOOK) CALL DR_HOOK('GET_GRID_DIM_LONLAT_ROT',1,ZHOOK_HANDLE)
!
!-------------------------------------------------------------------------------
!
