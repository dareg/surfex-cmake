...
IF (LHOOK) CALL DR_HOOK('HORIBL_SURF_GRIDIN_3',0,ZHOOK_HANDLE)
  ...
  IDX_I = IDX_I + 1
#ifdef SFX_MPI  
  IF (LSFX_MPI) CALL MPI_SEND(IEXT,2*KIND(IEXT)/4,MPI_INTEGER,NPIO,IDX_I,NCOMM,INFOMPI)
#endif  
    ...
    IDX_I = IDX_I + 1
#ifdef SFX_MPI    
    IF (LSFX_MPI) CALL MPI_RECV(ZDARIN,SIZE(ZDARIN)*KIND(ZDARIN)/4,MPI_REAL,NPIO,IDX_I,NCOMM,ISTATUS,INFOMPI)
#endif  
  IDX_I = IDX_I + 1
#ifdef SFX_MPI  
    IF (LSFX_MPI) CALL MPI_RECV(IDLS,SIZE(IDLS)*KIND(IDLS)/4,MPI_INTEGER,NPIO,IDX_I,NCOMM,ISTATUS,INFOMPI)
#endif  
...
!$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(J,IEXT,ISIZE,ZDARIN,IDLS,JT,JL,ISTATUS,INFOMPI,ZHOOK_HANDLE_OMP)
  ...
  DO J=0,NPROC-1
    ...
    IF (J/=NPIO) THEN
#ifdef SFX_MPI            
      IF (LSFX_MPI) CALL MPI_RECV(IEXT,2*KIND(IEXT)/4,MPI_INTEGER,J,IDX_I+1,NCOMM,ISTATUS,INFOMPI)
#endif      
    ELSE
      IEXT(1) = MINVAL(KP)
      IEXT(2) = MAXVAL(KP)
