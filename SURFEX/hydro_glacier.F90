...
USE MODD_SNOW_PAR, ONLY : XRHOSMAX, XHGLA, XSNOWDMIN
!
!
...
IF(TSNOW%SCHEME/='3-L' .AND. TSNOW%SCHEME/='CRO')THEN
!
  WHERE(PSNOWSWE(:,1)<=XHGLA*XRHOSMAX)PICEFLUX(:)=0.0
  PSNOWSWE(:,1)=PSNOWSWE(:,1)-PICEFLUX(:)*PTSTEP
!
ELSE
!
  WHERE(ZSWE(:)<=XHGLA*XRHOSMAX)PICEFLUX(:)=0.0
!
! Snow total depth
  ZSNOWD(:) = 0.
  DO JWRK=1,SIZE(PSNOWSWE,2)
...
! Flux
  DO JWRK=1,SIZE(PSNOWSWE,2)
     ZFLUX(:) = PICEFLUX(:)*(PSNOWSWE(:,JWRK)/PSNOWRHO(:,JWRK))/ZSNOWD(:)
