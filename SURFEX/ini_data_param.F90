!     #########################
      SUBROUTINE INI_DATA_PARAM(PTYPE,PSURF, PSURF2, PLAI, PH_TREE,                 &
                                  ...
                                  PROOT_LIN, PROOT_EXTINCTION,                      &
                                  PSOILRC_SO2, PSOILRC_O3, PRE25,                   &
                                  PCE_NITRO,PCF_NITRO,PCNA_NITRO,                   &
                                  PGMES_ST, PGC_ST, PBSLAI_ST, PSEFOLD_ST,          &
                                  PDMAX_ST, PZ0_TOWN, PALB_ROOF, PALB_ROAD,         &
                                  PALB_WALL, PEMIS_ROOF, PEMIS_ROAD, PEMIS_WALL,    &
                                  PHC_ROOF, PHC_ROAD, PHC_WALL,                     &
                                  PTC_ROOF, PTC_ROAD, PTC_WALL,                     &
...
USE MODD_DATA_COVER_PAR, ONLY : NVT_TREE, NVT_CONI, NVT_EVER, NVT_C3, &
                                  ...
                                  NVEGTYPE, JPCOVER, &
                                 NURBTYPE, NUR_DENSE, NUR_SUB, NUR_ZIC, &
                                 NUR_PORT, NUR_RAR, NUR_AIRPT, NUR_MINES, &
                                 NUR_PARK, NUR_SPORT 
!
...
!
    IF (PRESENT(PZ0_TOWN)) THEN
      PZ0_TOWN(JLOOP)=1.   
      IF(PTYPE(JLOOP,1)==NUR_DENSE)  PZ0_TOWN(JLOOP)= 3.
      IF(PTYPE(JLOOP,1)==NUR_ZIC  )  PZ0_TOWN(JLOOP)= 2.
      IF(PTYPE(JLOOP,1)==NUR_PORT )  PZ0_TOWN(JLOOP)= 2.           
      IF(PTYPE(JLOOP,1)==NUR_RAR  )  PZ0_TOWN(JLOOP)= 0.5
      IF(PTYPE(JLOOP,1)==NUR_PARK )  PZ0_TOWN(JLOOP)= 0.5  
      IF(PTYPE(JLOOP,1)==NUR_MINES)  PZ0_TOWN(JLOOP)= 0.1           
      IF(PTYPE(JLOOP,1)==NUR_AIRPT)  PZ0_TOWN(JLOOP)= 0.01
    ENDIF

    IF (PRESENT(PALB_ROOF)) THEN
      PALB_ROOF(JLOOP)=0.15
    ENDIF
    IF (PRESENT(PALB_ROAD)) THEN
      PALB_ROAD(JLOOP)=0.08
    ENDIF
    IF (PRESENT(PALB_WALL)) THEN
      PALB_WALL(JLOOP)=0.25
    ENDIF

    IF (PRESENT(PEMIS_ROOF)) THEN
      PEMIS_ROOF(JLOOP)=0.90
    ENDIF
    IF (PRESENT(PEMIS_ROAD)) THEN
      PEMIS_ROAD(JLOOP)=0.94
    ENDIF
    IF (PRESENT(PEMIS_WALL)) THEN
      PEMIS_WALL(JLOOP)=0.85
    ENDIF

    IF (PRESENT(PHC_ROOF)) THEN
      PHC_ROOF(JLOOP,:)= 1.E6 * 0.29 ! Oke 87   Insulation    
      PHC_ROOF(JLOOP,1)= 1.E6 * 2.11 ! Oke 87   Dense Concrete
      PHC_ROOF(JLOOP,2)= 1.E6 * 0.28 ! Oke 87   Aerated Concrete  
    ENDIF
    IF (PRESENT(PHC_ROAD)) THEN
      PHC_ROAD(JLOOP,:)= 1.E6 * 1.28 ! Mills 93  Dry soil 
      PHC_ROAD(JLOOP,1)= 1.E6 * 1.94 ! Mills 93  Asphalt 
      PHC_ROAD(JLOOP,2)= 1.E6 * 1.28 ! Mills 93  Dry soil
    ENDIF
    IF (PRESENT(PHC_WALL)) THEN
      PHC_WALL(JLOOP,:)= 1.E6 * 0.29 ! 0ke 87    Insulation 
      PHC_WALL(JLOOP,1)= 1.E6 * 1.55 ! Mills 93  Concrete
      PHC_WALL(JLOOP,2)= 1.E6 * 1.55 ! Mills 93  Concrete
    ENDIF
 
    IF (PRESENT(PTC_ROOF)) THEN
      PTC_ROOF(JLOOP,:)= 0.05
      PTC_ROOF(JLOOP,1)= 1.51
      PTC_ROOF(JLOOP,2)= 0.08   
    ENDIF
    IF (PRESENT(PTC_ROAD)) THEN
      PTC_ROAD(JLOOP,:)= 0.2513
      PTC_ROAD(JLOOP,1)= 0.7454
      PTC_ROAD(JLOOP,2)= 0.2513  
    ENDIF
    IF (PRESENT(PTC_WALL)) THEN
      PTC_WALL(JLOOP,:)= 0.05 
      PTC_WALL(JLOOP,1)= 0.9338
      PTC_WALL(JLOOP,2)= 0.9338    
    ENDIF

    IF (PRESENT(PD_ROOF)) THEN
      PD_ROOF(JLOOP,:)= 0.10  
      PD_ROOF(JLOOP,1)= 0.05
      PD_ROOF(JLOOP,2)= 0.4     
    ENDIF
    IF (PRESENT(PD_ROAD)) THEN
      PD_ROAD(JLOOP,:)= 1. 
      PD_ROAD(JLOOP,1)= 0.05
      PD_ROAD(JLOOP,2)= 0.1
    ENDIF
    IF (PRESENT(PD_WALL)) THEN
      PD_WALL(JLOOP,:)= 0.05
      PD_WALL(JLOOP,1)= 0.02
      PD_WALL(JLOOP,2)= 0.125  
    ENDIF

    IF (PRESENT(PBLD_HEIGHT)) THEN
      PBLD_HEIGHT(JLOOP)=10.
      IF(PTYPE(JLOOP,1)==NUR_DENSE)  PBLD_HEIGHT(JLOOP)= 30.
      IF(PTYPE(JLOOP,1)==NUR_ZIC  )  PBLD_HEIGHT(JLOOP)= 20.
      IF(PTYPE(JLOOP,1)==NUR_PORT )  PBLD_HEIGHT(JLOOP)= 20.  
      IF(PTYPE(JLOOP,1)==NUR_SUB  )  PBLD_HEIGHT(JLOOP)= 10. 
      IF(PTYPE(JLOOP,1)==NUR_AIRPT)  PBLD_HEIGHT(JLOOP)= 10.
      IF(PTYPE(JLOOP,1)==NUR_SPORT)  PBLD_HEIGHT(JLOOP)= 10.
      IF(PTYPE(JLOOP,1)==NUR_RAR  )  PBLD_HEIGHT(JLOOP)= 5.
      IF(PTYPE(JLOOP,1)==NUR_PARK )  PBLD_HEIGHT(JLOOP)= 5.  
      IF(PTYPE(JLOOP,1)==NUR_MINES)  PBLD_HEIGHT(JLOOP)= 5.                 
    ENDIF

    ZGARDEN(JLOOP)=0.
    IF(PTYPE(JLOOP,1)==NUR_DENSE)  ZGARDEN(JLOOP)= 0.1
    IF(PTYPE(JLOOP,1)==NUR_SUB  )  ZGARDEN(JLOOP)= 0.4
    IF(PTYPE(JLOOP,1)==NUR_ZIC  )  ZGARDEN(JLOOP)= 0.1  
    IF(PTYPE(JLOOP,1)==NUR_RAR  )  ZGARDEN(JLOOP)= 0.1           
    IF(PTYPE(JLOOP,1)==NUR_PORT )  ZGARDEN(JLOOP)= 0.1
    ...
    IF (PRESENT(PBLD)) THEN
      PBLD(JLOOP)=0.5        
      IF(PTYPE(JLOOP,1)==NUR_RAR  )  PBLD(JLOOP)= 0.1
      IF(PTYPE(JLOOP,1)==NUR_PARK )  PBLD(JLOOP)= 0.1  
      IF(PTYPE(JLOOP,1)==NUR_MINES)  PBLD(JLOOP)= 0.1           
      IF(PTYPE(JLOOP,1)==NUR_AIRPT)  PBLD(JLOOP)= 0.1
      PBLD(JLOOP) = PBLD(JLOOP) / (1. - ZGARDEN(JLOOP))
    ENDIF
    
    IF (PRESENT(PWALL_O_HOR)) THEN
      PWALL_O_HOR(JLOOP)=0.5
      IF(PTYPE(JLOOP,1)==NUR_DENSE)  PWALL_O_HOR(JLOOP)= 1.
      IF(PTYPE(JLOOP,1)==NUR_PORT )  PWALL_O_HOR(JLOOP)= 1.           
      IF(PTYPE(JLOOP,1)==NUR_SPORT)  PWALL_O_HOR(JLOOP)= 1.
    ...
    ELSEIF (PRESENT(PCAN_HW_RATIO) .AND. &
      ...
      CALL ABOR1_SFX("INI_DATA_PARAM: WHEN CALLING WITH CAN_HW_RATIO, BLD AND "// &
        "WALL_O_HOR MUST ALSO BE IN ARGUMENTS")
    ENDIF

    IF (PRESENT(PH_TRAFFIC)) THEN
      PH_TRAFFIC(JLOOP)= 10.
      IF (PTYPE(JLOOP,1)==NUR_RAR  ) PH_TRAFFIC(JLOOP)=30.
      IF (PTYPE(JLOOP,1)==NUR_DENSE) PH_TRAFFIC(JLOOP)=20.
      IF (PTYPE(JLOOP,1)==NUR_MINES) PH_TRAFFIC(JLOOP)=0.
      IF (PTYPE(JLOOP,1)==NUR_PARK ) PH_TRAFFIC(JLOOP)=0.
      IF (PTYPE(JLOOP,1)==NUR_SPORT) PH_TRAFFIC(JLOOP)=0.
    ENDIF
    ...
    IF (PRESENT(PH_INDUSTRY)) THEN
      PH_INDUSTRY(JLOOP)= 5.
      IF (PTYPE(JLOOP,1)==NUR_ZIC  ) PH_INDUSTRY(JLOOP)=20.
      IF (PTYPE(JLOOP,1)==NUR_PORT ) PH_INDUSTRY(JLOOP)=20.
      IF (PTYPE(JLOOP,1)==NUR_DENSE) PH_INDUSTRY(JLOOP)=10.
      IF (PTYPE(JLOOP,1)==NUR_RAR  ) PH_INDUSTRY(JLOOP)=30.
      IF (PTYPE(JLOOP,1)==NUR_AIRPT) PH_INDUSTRY(JLOOP)=0.
      IF (PTYPE(JLOOP,1)==NUR_MINES) PH_INDUSTRY(JLOOP)=0.
      IF (PTYPE(JLOOP,1)==NUR_PARK ) PH_INDUSTRY(JLOOP)=0.
      IF (PTYPE(JLOOP,1)==NUR_SPORT) PH_INDUSTRY(JLOOP)=0.
    ENDIF
