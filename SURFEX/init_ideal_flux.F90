...
USE MODD_IDEAL_FLUX, ONLY : XSFTS, XFREQ, NFORC, XALB, XEMIS, XTSRAD 
...
IF (LNAM_READ) THEN
 ...
 !               --------
 !
 CALL DEFAULT_DIAG_IDEAL(N2M,LSURF_BUDGET,LRAD_BUDGET,XDIAG_TSTEP,LSURF_BUDGETC,&
                           LRESET_BUDGETC)  
...
!----------------------------------------------------------------------------------
!
!*       1.     Frequency of forcing (in sec)
!               ----------------------------
!
!* XFREQ=1800., then 49 values from 00UTC to 24UTC are needed for the surface fluxes
!
XFREQ = XDAY / FLOAT(NFORC)
!
!----------------------------------------------------------------------------------
!
!*       3.    HOURLY surface scalar mixing ratio fluxes (NFORC+1 values per scalar from 00UTC to 24UTC)
!              -----------------------------------------
!
ISV = SIZE(HSV)
!
IF(.NOT. ALLOCATED (XSFTS) )ALLOCATE(XSFTS(0:NFORC,ISV))
!
!* unit: kg/m2/s
!
XSFTS = 0.
