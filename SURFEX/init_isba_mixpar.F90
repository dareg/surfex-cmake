!     #########
      SUBROUTINE INIT_ISBA_MIXPAR(HISBA,PCOVER,HPHOTO,HSFTYPE)
!     ##############################################################
!
!!**** *INIT_ISBA_MIXPAR* 
!!
...
USE MODD_DATA_ISBA_n,    ONLY : XPAR_VEGTYPE,  XPAR_LAI, XPAR_H_TREE,                      &
                                ...
                                XPAR_DG, XPAR_ROOTFRAC, LDATA_DG, LDATA_ROOTFRAC,          &
...
!
USE MODD_DATA_COVER,     ONLY : XDATA_VEGTYPE, XDATA_LAI, XDATA_H_TREE, XDATA_VEG,         &
...
!
REAL, DIMENSION(:,:), ALLOCATABLE :: ZPAR_ROOT_EXTINCTION
REAL, DIMENSION(:,:), ALLOCATABLE :: ZPAR_ROOT_LIN
!
...
IF (.NOT.LDATA_LAI) THEN
  ...
  DO JTIME=1,36 
    CALL AV_PGD(XPAR_LAI(:,JTIME,:),PCOVER,XDATA_LAI(:,JTIME,:),YVEG,'ARI',KDECADE=JTIME)
  ENDDO
ENDIF
!veg
IF (.NOT.LDATA_VEG) THEN
  ...
  ELSE
    DO JTIME=1,36
      CALL AV_PGD(XPAR_VEG(:,JTIME,:),PCOVER,XDATA_VEG(:,JTIME,:),YNAT,'ARI')
    ENDDO
  ENDIF
ENDIF
...
ENDIF
!
!dg: initialization of XPAR_ROOT_FRAC
IF (LDATA_DG .AND. .NOT.LDATA_ROOTFRAC) THEN
  !
  ALLOCATE(XPAR_ROOTFRAC  (NDIM,NGROUND_LAYER,NVEGTYPE))
  !
  ALLOCATE(ZPAR_ROOT_EXTINCTION(SIZE(XPAR_DG,1),SIZE(XPAR_DG,3)))
  ALLOCATE(ZPAR_ROOT_LIN(SIZE(XPAR_DG,1),SIZE(XPAR_DG,3)))
  !
  CALL INI_DATA_PARAM(XPAR_VEGTYPE,&
      PROOT_EXTINCTION=ZPAR_ROOT_EXTINCTION, PROOT_LIN=ZPAR_ROOT_LIN)
  CALL INI_DATA_SOIL(HISBA,&
      PROOT_EXTINCTION=ZPAR_ROOT_EXTINCTION,PROOT_LIN=ZPAR_ROOT_LIN,&
      PDG_IN=XPAR_DG,PCUM_ROOT_FRAC=XPAR_ROOTFRAC)
  LDATA_ROOTFRAC=.TRUE.
  !
  DEALLOCATE(ZPAR_ROOT_EXTINCTION)
  DEALLOCATE(ZPAR_ROOT_LIN)
  !
ENDIF
!
...
IF (LDATA_DG .AND. .NOT.LDATA_DICE) THEN
  ...
  ELSE
    DO JI=1,SIZE(XPAR_DG,1)
      DO JVEGTYPE=1,NVEGTYPE
        IF(SUM(XPAR_VEG(JI,:,JVEGTYPE))==0.0) THEN
          XPAR_DICE(JI,JVEGTYPE)=MIN(0.5,0.8*XPAR_DG(JI,SIZE(XPAR_DG,2),JVEGTYPE))
          XPAR_DICE(JI,JVEGTYPE)=MAX(0.2,XPAR_DICE(JI,JVEGTYPE))
        ELSE
          DO JLAYER=1,SIZE(XPAR_DG,2)
            IF(XPAR_ROOTFRAC(JI,JLAYER,JVEGTYPE)==1.0)THEN
              XPAR_DICE(JI,JVEGTYPE)=0.8*XPAR_DG(JI,JLAYER,JVEGTYPE)
              EXIT
            ENDIF
          ENDDO
        ENDIF
...
IF (LDATA_VEG) THEN
  ...
  IF (HPHOTO == 'AST' .OR. HPHOTO == 'LST' .OR. HPHOTO == 'NIT')  THEN
  ...
  IF (.NOT.LDATA_IRRIG) THEN
    ...
    DO JTIME=1,36
      CALL AV_PGD(XPAR_IRRIG(:,JTIME,:),PCOVER,XDATA_IRRIG,YVEG,'ARI',KDECADE=JTIME)
    ENDDO
    LDATA_IRRIG=.TRUE.
  ENDIF
  !
  IF (.NOT.LDATA_WATSUP) THEN
    ...
    DO JTIME=1,36
      CALL AV_PGD(XPAR_WATSUP(:,JTIME,:),PCOVER,XDATA_WATSUP,YVEG,'ARI',KDECADE=JTIME)
    ENDDO
    LDATA_WATSUP=.TRUE.
  ENDIF
  !
