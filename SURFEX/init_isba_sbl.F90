   ...
   !We choose to set ZZ0EFF and ZZ0_WITH_SNOW equal to ZZ0
    ...
    DO JLAYER=1,SIZE(PTSNOW%WSNOW,2)
      ...
      WHERE(ZSUM_LAYER(:,JLAYER)>0)      
        ZSNOWRHO(:,JLAYER)= &
          SUM(PPATCH(:,:)*PTSNOW%RHO(:,JLAYER,:),DIM=2,MASK=PTSNOW%WSNOW(:,JLAYER,:)>0) / ZSUM_LAYER(:,JLAYER)
      ELSEWHERE
    ...
    WHERE(ZSUM(:)>0)         
      ZSNOWALB(:)= &
        SUM(PPATCH(:,:)*PTSNOW%ALB(:,:),DIM=2,MASK=ZWSNOW(:,:)>0) / ZSUM(:)      
    ELSEWHERE
    ...
    DO JLAYER=1,KLVL
      ...
      PTKE(:,JLAYER)=XALPSBL * ZCD(:) * (ZWIND(:)**2)
