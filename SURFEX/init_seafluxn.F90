...
IF (LNAM_READ) THEN
 ...
 CALL DEFAULT_SEAFLUX(SM%S%XTSTEP,SM%S%XOUT_TSTEP,SM%S%CSEA_ALB,SM%S%CSEA_FLUX,SM%S%LPWG,  &
                      ...
                      SM%O%NTIME_COUPLING,SM%O%XOCEAN_TSTEP,SM%O%XCMO_COEF,SM%S%XICHCE,SM%S%CINTERPOL_SST,&
                      SM%S%CINTERPOL_SSS                            )
 ...
 !            
 CALL DEFAULT_DIAG_SEAFLUX(SM%SD%O%N2M,SM%SD%O%LSURF_BUDGET,SM%SD%O%L2M_MIN_ZS,SM%SD%O%LHU2M_QSAT, &
...
SELECT CASE (HINIT)
  CASE ('PGD')
    ...
    RETURN
  CASE ('PRE')
    SM%S%CSEAICE_SCHEME = CPREP_SEAICE_SCHEME
  CASE DEFAULT
...
END SELECT
CALL SM%S%ICE%INIT(HPROGRAM)
SM%S%ICE%LINTERPOL_SIC = (SM%S%LINTERPOL_SIC .OR. SM%S%LVOLATILE_SIC)
SM%S%ICE%LINTERPOL_SIT = SM%S%LINTERPOL_SIT
!-------------------------------------------------------------------------------
!
