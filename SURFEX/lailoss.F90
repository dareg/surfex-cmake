!     #########
    SUBROUTINE LAILOSS(PRHOA, PLAI,PSEFOLD,PANMAX,PANFM,PANDAY,   &
                         OMASK,PBSLAI,PBIOMASS, PVEG         )  
!   ###############################################################
!!****  *LAILOSS*  
!!
!!    PURPOSE
...
REAL,   DIMENSION(:), INTENT(IN) :: PSEFOLD ! e-folding time for 
!                                             ! senescence (s)
...
!
REAL,    DIMENSION(SIZE(PLAI))  :: ZXSEFOLD, ZXM
REAL                            :: ZBMCOEF
...
! ----------------------------------------
!
! compute time for signaling end of day:
!
WHERE( OMASK(:) .AND. (PVEG(:)>0) )
!
