...
! FLake parameters
  ...
  REAL, POINTER, DIMENSION(:) :: XDEPTH_BS     ! Depth of the thermally active layer of the
                                               !       bottom sediments [m]
  REAL, POINTER, DIMENSION(:) :: XCOR          ! The Coriolis parameter [s^{-1}]
  REAL, POINTER, DIMENSION(:) :: XALB_WATER    ! Water surface albedo
  REAL, POINTER, DIMENSION(:) :: XALB_ICE      ! Ice surface albedo
  REAL, POINTER, DIMENSION(:) :: XALB_SNOW     ! Snow surface albedo
...
!
  ...
  LOGICAL            :: LWATFLX     !flag, if .TRUE. use water_flux to compute surface fluxes
  CHARACTER(LEN=6)   :: CFLAKE_SNOW ! /NON/FLAKE/ISBA-ES/ Snow scheme to use
                                    ! Now, only NON is allowed
!
END TYPE FLAKE_t
...
REAL, POINTER, DIMENSION(:) :: XCOR=>NULL()
!$OMP THREADPRIVATE(XCOR)
REAL, POINTER, DIMENSION(:) :: XALB_WATER=>NULL()
!$OMP THREADPRIVATE(XALB_WATER)
REAL, POINTER, DIMENSION(:) :: XALB_ICE=>NULL()
!$OMP THREADPRIVATE(XALB_ICE)
REAL, POINTER, DIMENSION(:) :: XALB_SNOW=>NULL()
!$OMP THREADPRIVATE(XALB_SNOW)
...
LOGICAL, POINTER            :: LWATFLX=>NULL()  
!$OMP THREADPRIVATE(LWATFLX)
CHARACTER(LEN=6),POINTER    :: CFLAKE_SNOW=>NULL()
!$OMP THREADPRIVATE(CFLAKE_SNOW)
...
FLAKE_MODEL(KFROM)%XCOR=>          XCOR
FLAKE_MODEL(KFROM)%XALB_WATER=>    XALB_WATER
FLAKE_MODEL(KFROM)%XALB_ICE=>      XALB_ICE
FLAKE_MODEL(KFROM)%XALB_SNOW=>     XALB_SNOW
...
XCOR          =>FLAKE_MODEL(KTO)%XCOR
XALB_WATER    =>FLAKE_MODEL(KTO)%XALB_WATER
XALB_ICE      =>FLAKE_MODEL(KTO)%XALB_ICE
XALB_SNOW     =>FLAKE_MODEL(KTO)%XALB_SNOW
...
LWATFLX=>FLAKE_MODEL(KTO)%LWATFLX  
CFLAKE_SNOW=>FLAKE_MODEL(KTO)%CFLAKE_SNOW  
...
DO J=1,KMODEL
  ...
  NULLIFY(FLAKE_MODEL(J)%XCOR)
  NULLIFY(FLAKE_MODEL(J)%XALB_WATER)
  NULLIFY(FLAKE_MODEL(J)%XALB_ICE)
  NULLIFY(FLAKE_MODEL(J)%XALB_SNOW)
...
FLAKE_MODEL(:)%LWATFLX=.FALSE.
FLAKE_MODEL(:)%CFLAKE_SNOW=' '
