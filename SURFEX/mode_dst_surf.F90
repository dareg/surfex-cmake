...
IF (LHOOK) CALL DR_HOOK('MODE_DST_SURF:MASSFLUX2MOMENTFLUX',0,ZHOOK_HANDLE)
ZCONVERTFACM0 = XMOLARWEIGHT_DST/XAVOGADRO !(kg_dst/mol_dst)/(molec/mol)

!Factor which is needed for moment 6, there is a factor 1.d6 transported around in M6 in MESONH
ZCONVERTFACM6 = XMOLARWEIGHT_DST/XAVOGADRO*1.d6
...
DO JMODE=1,NDSTMDE
   ...
   ZFM(:,1)=PFLUX(:,JSV_IDX+2)                &!kg_{dst}/m2/sec
          /(4./3.*XPI*XDENSITY_DUST)            &!/(kg_{dst}/m^{3}_{dst)} ==> m^3_{dst}/m2/sec
   ...
   !
ENDDO !Loop on modes
IF (LHOOK) CALL DR_HOOK('MODE_DST_SURF:MASSFLUX2MOMENTFLUX',1,ZHOOK_HANDLE)
   !
...
SUBROUTINE DUSTMOMENT2SIZE(       &
  ...
  REAL,DIMENSION(SIZE(PSVT,1), JPMODE_DST*3) :: ZM     ! [moments] local array for moments
  INTEGER,DIMENSION(JPMODE_DST) :: NM0                 ! [idx] index for Mode 0 in passed variables
  INTEGER,DIMENSION(JPMODE_DST) :: NM3                 ! [idx] indexes for Mode 3 in passed variables
  INTEGER,DIMENSION(JPMODE_DST) :: NM6                 ! [idx] indexes for Mode 6 in passed variables
  ...
  IF (LHOOK) CALL DR_HOOK('MODE_DST_SURF:DUSTMOMENT2SIZE',0,ZHOOK_HANDLE)
  ZCONVERTFACM0 = XAVOGADRO/XMOLARWEIGHT_DST         !==> to obtain #/m3
  ZCONVERTFACM6 = XAVOGADRO/XMOLARWEIGHT_DST *1.D-6  !==> to get um6/m3
  ...
  DO JN=1,JPMODE_DST
     ...
     IF (LVARSIG) THEN ! give M6 (case of variable standard deviation)
        ...
        ZM(:,NM3(JN)) =                        &
               ...
               /(XPI*4./3.*XDENSITY_DST)          &!==> m3_{dst}/m3_{air}
     ...
     ELSE IF (LRGFIX_DST) THEN ! compute M6 from M3, Rg and SIGMA
        !calculate moment 3 from total aerosol mass in kg/m3 ==> um3/m3
        ZM(:,NM3(JN)) =                        &
        ...
        ZM(:,NM0(JN))= ZM(:,NM3(JN)) /&
         ((XEMISRADIUS(JN)**3)*EXP(4.5 * LOG(ZSIGMA(:))**2))  
    ...
    IF(PRESENT(PMASS1D))THEN
       PMASS1D(:,JN)=     &
              ZM(:,NM0(JN))         &!#/m^3_{air}
              * XPI*4./3.           &
              * XDENSITY_DUST       &!==>kg/m^3_{aeros}/m^3_{air}
              * ZRG(:) * ZRG(:) * ZRG(:) &
...
!
!
END SUBROUTINE DUSTMOMENT2SIZE


END MODULE MODE_DST_SURF
