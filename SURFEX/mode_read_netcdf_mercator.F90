...
DO J1=1,NVARDIMLEN(1)
  DO J2=1,NVARDIMLEN(2)
    IF (ZVALU2D(J1,J2)/=PMISSVALUE) PVALU2D(J1,J2)=ZVALU2D(J1,J2)
  ENDDO
ENDDO
!
...
DO J1=1,NVARDIMLEN(1)
  DO J2=1,NVARDIMLEN(2)
    DO J3=1,NVARDIMLEN(3)
      IF (ZVALU3D(J1,J2,J3)/=PMISSVALUE) PVALU3D(J1,J2,J3)=ZVALU3D(J1,J2,J3)
    ENDDO
  ENDDO
ENDDO
!
...
elseif (NINDEPTH<0) then
  ...
  PFIELD(:)=XUNDEF
  WHERE (ZVAL(:)/=ZUNDEF) PFIELD(:)=ZVAL(:)+XTT
...
!
  ALLOCATE(PFIELD(NILENGTH))
  PFIELD(:)=XUNDEF
  WHERE (ZVALUE(:,1)/=ZUNDEF) PFIELD(:)=ZVALUE(:,1)+XTT
  CINTERP_TYPE='HORIBL' !!interpolation from gaussian, legendre or regular grid
!                       !!CINGRID_TYPE='GAUSS  ' ou ='AROME '
!                       !!CINGRID_TYPE='LATLON '
endif
