...
ELSE
  ...
  IF (NPROC>1 .AND. YDIR=='H') THEN
    ...
    ENDIF
#ifdef SFX_MPI
    IF (LSFX_MPI) CALL MPI_BCAST(NINDEX,SIZE(NINDEX)*KIND(NINDEX)/4,MPI_INTEGER,NPIO,NCOMM,INFOMPI)
#endif
  ENDIF
  !
  IPIO_SAVE = NPIO
  ...
  DO JP = 1,IPAS
    ...
    IF (JCOVER<=IL2) THEN
      ...
      IF (HPROGRAM=='AROME ') THEN
        CALL YRSURFEX_HOST%READ_COVER (YREC,IL1,PFIELD(:,JCOVER),KRESP,YCOMMENT,YDIR)
      ELSE
        !
        ...
        ELSEIF (YDIR=='A' .OR. YDIR=='E') THEN
          ...
          IF (NRANK/=NPIO) THEN
            IDX = IDX + 1 
#ifdef SFX_MPI
        IF (LSFX_MPI) CALL MPI_SEND(ZFIELD,SIZE(ZFIELD)*KIND(ZFIELD)/4,MPI_REAL,NPIO,IDX,NCOMM,INFOMPI)
#endif
          ELSE
