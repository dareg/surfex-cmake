...
IF (NPROC>1) THEN
#ifdef SFX_MPI
  IF (LSFX_MPI) CALL MPI_BCAST(NID_FILE,KIND(NID_FILE)/4,MPI_INTEGER,NPIO,NCOMM,INFOMPI)
#endif
ENDIF
!
...
IF (NPROC>1) THEN
#ifdef SFX_MPI
  IF (LSFX_MPI) CALL MPI_BCAST(PVAR,KIND(PVAR)*SIZE(PVAR)/4,MPI_REAL,NPIO,NCOMM,INFOMPI)
#endif
ENDIF
!
