...
!              ---------------------------------
!
  ZFOES(JI,JJ) = EXP( XALPW - XBETAW/PT(JI,JJ) - XGAMW*LOG(PT(JI,JJ))  )
!
!*       2.    COMPUTE SATURATION HUMIDITY
!              ---------------------------
!
  PQSAT(JI,JJ) = XRD/XRV*ZFOES(JI,JJ)/PP(JI,JJ)   &
                     / (1.+(XRD/XRV-1.)*ZFOES(JI,JJ)/PP(JI,JJ))  
!
  ENDDO
ENDDO
