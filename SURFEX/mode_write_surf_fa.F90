...
!
USE MODD_IO_SURF_FA,        ONLY : NUNIT_FA, CMASK
!
USE MODE_FASURFEX
!
...
USE MODE_FASURFEX
!
USE MODD_IO_SURF_FA,        ONLY : NUNIT_FA, NMASK, CMASK
!
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK
USE PARKIND1  ,ONLY : JPRB
!
...
REAL                   :: ZMEAN, ZCOUNT
LOGICAL                :: GKNOWN
INTEGER                :: I,KNGRIB,KNBPDG,KNBCSP,KSTRON,KPUILA,KDMOPL
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!-------------------------------------------------------------------------------
...
IF(LFANOCOMPACT)THEN
  CALL FAVEUR(KRESP,NUNIT_FA,KNGRIB,KNBPDG,KNBCSP,KSTRON,KPUILA,KDMOPL)
  ! -- Pour ecrire sans compactage
  CALL FAGOTE(KRESP,NUNIT_FA,-1,KNBPDG,KNBCSP,KSTRON,KPUILA,KDMOPL)
  CALL FAIENC(KRESP,NUNIT_FA,'S1D_',0,HREC,ZWORK,.FALSE.)
  IF (KRESP/=0) THEN
    CALL ERROR_WRITE_SURF_FA(HREC,KRESP)
  ENDIF  
  ! On remet la valeur par defaut 
  CALL FAGOTE(KRESP,NUNIT_FA,KNGRIB,KNBPDG,KNBCSP,KSTRON,KPUILA,KDMOPL)
ELSE
  ZMEAN =0.0
  ZCOUNT=0.0
  DO I=1,NFULL
...
USE MODI_ERROR_WRITE_SURF_FA
!
USE MODD_IO_SURF_FA,        ONLY : CMASK, NUNIT_FA
!
USE MODE_FASURFEX
!
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK
