...
NAMELIST/NAM_DIAG_SURFn/N2M, L2M_MIN_ZS, LSURF_BUDGET, LRAD_BUDGET, LSURF_BUDGETC,  &
                        LRESET_BUDGETC, LCOEF, LSURF_VARS, LHU2M_QSAT
!                        
NAMELIST/NAM_WRITE_DIAG_SURFn/LPROVAR_TO_DIAG,LSNOWDIMNC,LRESETCUMUL,LRESETMINMAX, &
                              LSELECT,CSELECT
!
CONTAINS
!
...
!
  TYPE(DIAG_OPTIONS_t), INTENT(INOUT) :: DGO
  INTEGER :: ICOUNT,JI,JJ,IEXTRA
  CHARACTER(LEN=12),DIMENSION(7) :: EXTRA_VARS,TEST_VARS
  LOGICAL :: LFOUND
  REAL(KIND=JPHOOK) :: ZHOOK_HANDLE
  ...
  DGO%LRESETMINMAX = LRESETMINMAX  
!
  DGO%LSELECT = LSELECT
  IF (LSELECT) THEN
    TEST_VARS(1)='VERSION     '
    TEST_VARS(2)='BUG         '
    TEST_VARS(3)='STORAGETYPE '
    TEST_VARS(4)='DIM_FULL    '
    TEST_VARS(5)='WRITE_EXT   '
    TEST_VARS(6)='SPLIT_PATCH '
    TEST_VARS(7)='DTCUR       '
    EXTRA_VARS='            '

    ICOUNT = COUNT(CSELECT /= '            ')
    IEXTRA=0
    DO JJ=1,SIZE(TEST_VARS)
      LFOUND=.FALSE.
      SEARCH: DO JI=1,ICOUNT
        IF ( TRIM(CSELECT(JI)) == TRIM (TEST_VARS(JJ))) THEN
          LFOUND=.TRUE.
          EXIT SEARCH
        ENDIF
      ENDDO SEARCH
      IF ( .NOT. LFOUND ) THEN
        IEXTRA=IEXTRA+1
        EXTRA_VARS(IEXTRA)=TEST_VARS(JJ)
      ENDIF
    ENDDO
 
    IF(.NOT.ASSOCIATED(DGO%CSELECT))THEN
      ALLOCATE(DGO%CSELECT(ICOUNT+IEXTRA+1))
      DGO%CSELECT(:) = '            '
    ENDIF
    DGO%CSELECT(1:ICOUNT) = CSELECT(1:ICOUNT)
    DO JI=1,IEXTRA 
      DGO%CSELECT(ICOUNT+JI) = EXTRA_VARS(JI)
    ENDDO
  ELSE
