...
USE MODD_ISBA_n,      ONLY : NGROUND_LAYER,  NNBIOMASS, NNLITTER, NNSOILCARB,       &
                               NNLITTLEVS, CISBA, CDIF, CPHOTO, CSCOND, CRESPSL,    &
                               LGLACIER, XZ0_O_Z0H, XEMIS, XZ0, XWRMAX_CF, XGAMMA,  &
                               XCV, XRGL, XVEGTYPE_PATCH, XDG, XRUNOFFD, XRUNOFFB,  &
                               XWDRAIN, XTAUICE, XZ0REL, XGAMMAT,                   &
                               XC1SAT, XC2REF, XC3, XC4B, XC4REF, XACOEF, XPCOEF,   &
                               XWFC, XWWILT, XWSAT, XBCOEF, XWR, XTG, XWG, XWGI,    &
                               XLAI, XRESA, XVEG, XTDEEP, TSNOW, XROOTFRAC,         &
                               ...
                               CRUNOFF, CRAIN, CHORT, XMUF, XFSAT, XKSAT_ICE,       &
                               ...
                               XICE_STO, XFFROZEN, XCONDSAT_EXP, XEXP_DIF, XALPHA,  &
                               XNVG, XMVG, XLVG, XWRES  
...
USE MODD_PACK_ISBA,  ONLY :    NSIZE_LSIMPLE, NSIZE_L0, NSIZE_TSIMPLE,  NSIZE_T0, NSIZE_SIMPLE,  &
                               ...
                               XP_LE, XP_ALBNIR, XP_ALBVIS, XP_ALBUV,                            &
                               ...
                               XP_SCA_ALB_WITH_SNOW, XP_ICE_STO, XP_FFROZEN, XP_CONDSAT_EXP,     &
                               XP_EXP_DIF, XP_ALPHA, XP_NVG, XP_MVG, XP_LVG, XP_WRES  
!
...
!
IF(CDIF=='BC')THEN 
  !Note that if ISBA/=DIF, CDIF alway = 'BC'
  ISIZE_GROUND = ISIZE_GROUND + 1
...
ELSE
  ISIZE_01 = ISIZE_01 + 1
  XP_MPOTSAT    => XBLOCK_01(:,:,ISIZE_01)
  ISIZE_01 = ISIZE_01 + 1
  XP_BCOEF      => XBLOCK_01(:,:,ISIZE_01)
ENDIF  
!
IF (CISBA=='DIF') THEN
  ISIZE_GROUND = ISIZE_GROUND + 1
   XP_CONDSAT_EXP=> XBLOCK_GROUND(:,:,ISIZE_GROUND)
  ISIZE_GROUND = ISIZE_GROUND + 1
   XP_EXP_DIF    => XBLOCK_GROUND(:,:,ISIZE_GROUND)   
  ISIZE_GROUND = ISIZE_GROUND + 1
   XP_ROOTFRAC   => XBLOCK_GROUND(:,:,ISIZE_GROUND)  
  IF(CDIF=='VG')THEN
    ISIZE_GROUND = ISIZE_GROUND + 1
     XP_ALPHA    => XBLOCK_GROUND(:,:,ISIZE_GROUND)
    ISIZE_GROUND = ISIZE_GROUND + 1 
     XP_NVG      => XBLOCK_GROUND(:,:,ISIZE_GROUND)
    ISIZE_GROUND = ISIZE_GROUND + 1  
     XP_MVG      => XBLOCK_GROUND(:,:,ISIZE_GROUND)  
    ISIZE_GROUND = ISIZE_GROUND + 1
     XP_LVG      => XBLOCK_GROUND(:,:,ISIZE_GROUND) 
    ISIZE_GROUND = ISIZE_GROUND + 1
     XP_WRES     => XBLOCK_GROUND(:,:,ISIZE_GROUND) 
  ELSE
    ISIZE_01 = ISIZE_01 + 1
    XP_ALPHA    => XBLOCK_01(:,:,ISIZE_01)
    ISIZE_01 = ISIZE_01 + 1
    XP_NVG      => XBLOCK_01(:,:,ISIZE_01)
    ISIZE_01 = ISIZE_01 + 1
    XP_MVG      => XBLOCK_01(:,:,ISIZE_01)
    ISIZE_01 = ISIZE_01 + 1
    XP_LVG      => XBLOCK_01(:,:,ISIZE_01)
    ISIZE_01 = ISIZE_01 + 1
    XP_WRES     => XBLOCK_01(:,:,ISIZE_01)
  ENDIF      
ELSE
  ISIZE_01 = ISIZE_01 + 1
  XP_CONDSAT_EXP=> XBLOCK_01(:,:,ISIZE_01)
  ISIZE_01 = ISIZE_01 + 1
  XP_EXP_DIF    => XBLOCK_01(:,:,ISIZE_01) 
  ISIZE_01 = ISIZE_01 + 1
...
IF (NPATCH==1) THEN
  ...
  !
  IF (CISBA=='DIF') THEN
    XP_CONDSAT_EXP(:, :) =    XCONDSAT_EXP(:, :, 1)
    XP_EXP_DIF    (:, :) =    XEXP_DIF    (:, :, 1)    
...
!
  ...
  IF (CISBA=='DIF') THEN
    DO JK=1,SIZE(XROOTFRAC,2)
      DO JJ=1,KSIZE
        JI                     =    KMASK(JJ)
        XP_CONDSAT_EXP(JJ, JK) =    XCONDSAT_EXP(JI, JK, KPATCH)
        XP_EXP_DIF    (JJ, JK) =    XEXP_DIF    (JI, JK, KPATCH)        
        XP_ROOTFRAC   (JJ, JK) =    XROOTFRAC   (JI, JK, KPATCH)
      ENDDO
    ENDDO
    IF(CDIF=='VG')THEN
      DO JK=1,SIZE(XALPHA,2)
         DO JJ=1,KSIZE      
            JI                  =    KMASK(JJ)
            XP_ALPHA   (JJ, JK) =    XALPHA (JI, JK)
            XP_NVG     (JJ, JK) =    XNVG   (JI, JK)
            XP_MVG     (JJ, JK) =    XMVG   (JI, JK)
            XP_LVG     (JJ, JK) =    XLVG   (JI, JK)
            XP_WRES    (JJ, JK) =    XWRES  (JI, JK)
         ENDDO
      ENDDO           
    ENDIF     
  ENDIF
