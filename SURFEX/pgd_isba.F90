...
USE MODD_ISBA_n,         ONLY : NPATCH, NGROUND_LAYER, NNBIOMASS, CISBA, &
                                CDIF, CPEDOTF, XCOVER, LCOVER, XZS,      &
                                XZ0EFFJPDIR, CPHOTO,                     &
                                XCLAY, XSAND, XORGMAT, XDENSITY,         &
...
SELECT CASE (CISBA)
  ...
  CASE ('DIF')
    ...
    IF(CPEDOTF=='CH78'.OR.CPEDOTF=='CO84')CDIF='BC'
    IF(CPEDOTF=='CP88'.OR.CPEDOTF=='WO99')CDIF='VG'    
    WRITE(ILUOUT,*) '*****************************************'
    WRITE(ILUOUT,*) '* With option CISBA = ',CISBA,'               *'
    WRITE(ILUOUT,*) '* the number of soil layers is set to ', NGROUND_LAYER
    WRITE(ILUOUT,*) '* theta(psi) function is set to ',CDIF 
...
END DO
!
!*    9.      organic matter and density
!             --------------------------
!
IF(CDIF=='VG') THEN
!
  ALLOCATE(XORGMAT(ILU,NGROUND_LAYER))
!
  IF(LIMP_ORGMAT)THEN
!
    IF(YORGMATFILETYPE=='NETCDF')THEN
       CALL ABOR1_SFX('Use another format than netcdf for organic matter input file with LIMP_ORGMAT')
    ELSE
#ifdef ASC
       CFILEIN     = ADJUSTL(ADJUSTR(YSAND)//'.txt')
#endif
       ...
       CFILEIN_LFI = ADJUSTL(YSAND)
#endif
       CALL INIT_IO_SURF_n(YORGMATFILETYPE,'NATURE','ISBA  ','READ ')
    ENDIF     
!   
    CALL READ_SURF(YORGMATFILETYPE,'ORGMAT',XORGMAT(:,1),IRESP) 
!
    CALL END_IO_SURF_n(YORGMATFILETYPE)
!
  ELSE
    CALL PGD_FIELD(HPROGRAM,'organic matter','NAT',YORGMAT,YORGMATFILETYPE,XUNIF_ORGMAT,XORGMAT(:,1))
  ENDIF
!
  DO JLAYER=1,NGROUND_LAYER
    XORGMAT(:,JLAYER) = XORGMAT(:,1)
  END DO
!
  ALLOCATE(XDENSITY(ILU,NGROUND_LAYER))
!
  IF(LIMP_DENSITY)THEN
!
    IF(YDENSITYFILETYPE=='NETCDF')THEN
       CALL ABOR1_SFX('Use another format than netcdf for organic matter input file with LIMP_DENSITY')
    ELSE
#ifdef ASC
       CFILEIN     = ADJUSTL(ADJUSTR(YSAND)//'.txt')
#endif
       ...
       CFILEIN_LFI = ADJUSTL(YSAND)
#endif
       CALL INIT_IO_SURF_n(YDENSITYFILETYPE,'NATURE','ISBA  ','READ ')
    ENDIF     
!   
    CALL READ_SURF(YDENSITYFILETYPE,'DENSITY',XDENSITY(:,1),IRESP) 
!
    CALL END_IO_SURF_n(YDENSITYFILETYPE)
!
  ELSE
    CALL PGD_FIELD(HPROGRAM,'organic matter','NAT',YDENSITY,YDENSITYFILETYPE,XUNIF_DENSITY,XDENSITY(:,1))
  ENDIF
!
  DO JLAYER=1,NGROUND_LAYER
    XDENSITY(:,JLAYER) = XDENSITY(:,1)
  END DO
!
ELSE
!
  ALLOCATE(XORGMAT (0,1))
  ALLOCATE(XDENSITY(0,1))
!
ENDIF
!
!-------------------------------------------------------------------------------
