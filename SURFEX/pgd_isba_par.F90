...
USE MODD_ISBA_n,         ONLY : LECOCLIMAP, CISBA, CPHOTO, NGROUND_LAYER
...
NAMELIST/NAM_DATA_ISBA/NTIME, XUNIF_VEGTYPE, XUNIF_DG, XUNIF_ROOTFRAC, XUNIF_DICE,                  &
                         XUNIF_LAI, XUNIF_VEG, XUNIF_Z0, XUNIF_EMIS,                                &
...
ALLOCATE(XPAR_LAI         (NDIM,36,NVEGTYPE))
ALLOCATE(XPAR_VEG         (NDIM,36,NVEGTYPE))
ALLOCATE(XPAR_Z0          (NDIM,36,NVEGTYPE))
ALLOCATE(XPAR_EMIS        (NDIM,36,NVEGTYPE))
...
ALLOCATE(XPAR_IRRIG       (NDIM,36,NVEGTYPE))
ALLOCATE(XPAR_WATSUP      (NDIM,36,NVEGTYPE))
...
IF (.NOT. LDATA_DICE) DEALLOCATE(XPAR_DICE)        
!
...
!
CALL INI_VAR_FROM_DATA(HPROGRAM,'ARI','CE_NITRO: leaf area ratio sensitivity to nitrogen ccion','NAT',CFNAM_CE_NITRO,   &
       CFTYP_CE_NITRO,XUNIF_CE_NITRO,XPAR_CE_NITRO,LDATA_CE_NITRO)  
IF (.NOT. LDATA_CE_NITRO) DEALLOCATE(XPAR_CE_NITRO)
!
CALL INI_VAR_FROM_DATA(HPROGRAM,'ARI','CF_NITRO: lethal minimum value of leaf area ratio','NAT',CFNAM_CF_NITRO,   &
       CFTYP_CF_NITRO,XUNIF_CF_NITRO,XPAR_CF_NITRO,LDATA_CF_NITRO)
IF (.NOT. LDATA_CF_NITRO) DEALLOCATE(XPAR_CF_NITRO)
!
CALL INI_VAR_FROM_DATA(HPROGRAM,'ARI','CNA_NITRO: nitrogen ccion of active biomass','NAT',CFNAM_CNA_NITRO,   &
       CFTYP_CNA_NITRO,XUNIF_CNA_NITRO,XPAR_CNA_NITRO,LDATA_CNA_NITRO)
...
IF (.NOT. LDATA_H_TREE) DEALLOCATE(XPAR_H_TREE)
