!     #########
SUBROUTINE PREP_HOR_ISBA_FIELD(HPROGRAM,HSURF,HATMFILE,HATMFILETYPE)
!     #################################################################################
!
...
!
CALL READ_PREP_ISBA_CONF(HPROGRAM,HSURF,YFILE,YFILETYPE,HATMFILE,HATMFILETYPE,ILUOUT,GUNIF)
!
CMASK = 'NATURE'
!
!-------------------------------------------------------------------------------------
...
ELSE IF (YFILETYPE=='MESONH' .OR. YFILETYPE=='ASCII ' .OR. YFILETYPE=='LFI   ') THEN
   CALL PREP_ISBA_EXTERN(HPROGRAM,HSURF,YFILE,YFILETYPE,ILUOUT,ZFIELDIN)
...
!
ALLOCATE(ZW (SIZE(ZFIELDOUT,1),SIZE(ZFIELDOUT,2),NPATCH))
!
ZW = 0.
...
SELECT CASE (HSURF)
 ...
 CASE('ZS     ') 
  ...
  !
 CASE('WG     ') 
  ALLOCATE(ZF (SIZE(ZFIELDOUT,1),NGROUND_LAYER,NPATCH))
  !
  ...
  ALLOCATE(XWG(SIZE(ZFIELDOUT,1),NGROUND_LAYER,NPATCH))
  DO JPATCH=1,NPATCH
    XWG(:,:,JPATCH) = XWWILT + ZF(:,:,JPATCH) * (XWFC-XWWILT)
    XWG(:,:,JPATCH) = MAX(MIN(XWG(:,:,JPATCH),XWSAT),XWGMIN)
  END DO
  !
 ...
 CASE('WGI    ')
  ...
  ALLOCATE(XWGI(SIZE(ZFIELDOUT,1),NGROUND_LAYER,NPATCH))
  DO JPATCH=1,NPATCH
    XWGI(:,:,JPATCH) = ZF(:,:,JPATCH) * XWSAT
    XWGI(:,:,JPATCH) = MAX(MIN(XWGI(:,:,JPATCH),XWSAT),0.)
  END DO
  !
 ...
 CASE('TG     ') 
  ...
  ELSE
    IWORK=NGROUND_LAYER
  ENDIF
  ALLOCATE(XTG(SIZE(ZFIELDOUT,1),IWORK,NPATCH))
  ALLOCATE(ZDG(SIZE(XDG,1),IWORK,SIZE(XDG,3)))
  IF (CISBA=='2-L'.OR.CISBA=='3-L') THEN
     IF(LTEMP_ARP)THEN
       DO JPATCH=1,NPATCH
          DO JJ=1,IWORK
             ZDG(:,JJ,JPATCH) = XSODELX(JJ)-XSODELX(1)
          ENDDO
       ENDDO
     ELSE
       DO JPATCH=1,NPATCH
  ...
  !
 CASE('WR     ') 
  ALLOCATE(XWR(SIZE(ZFIELDOUT,1),NPATCH))
  DO JPATCH=1,NPATCH
