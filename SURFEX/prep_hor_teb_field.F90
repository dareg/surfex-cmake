...
USE MODD_PREP,     ONLY : CINGRID_TYPE, CINTERP_TYPE, XZS_LS, XLAT_OUT, XLON_OUT, &
                          XX_OUT, XY_OUT, CMASK
USE MODD_PREP_TEB, ONLY : XGRID_ROOF, XGRID_ROAD, XGRID_WALL, LSNOW_IDEAL,    &
                          XWSNOW_ROOF, XRSNOW_ROOF, XTSNOW_ROOF, XASNOW_ROOF, &
                          XWSNOW_ROAD, XRSNOW_ROAD, XTSNOW_ROAD, XASNOW_ROAD
USE MODD_TEB_n,     ONLY : TTIME, XWS_ROAD, XWS_ROOF, XT_ROAD, XT_ROOF, XT_WALL, XTI_BLD, &
...
IF (HSURF=='SN_ROOF') THEN
  ...
  DEALLOCATE(XWSNOW_ROOF)
  DEALLOCATE(XRSNOW_ROOF)
  DEALLOCATE(XTSNOW_ROOF)
  DEALLOCATE(ZSG1SNOW)
  DEALLOCATE(ZSG2SNOW)
...
ELSE IF (HSURF=='SN_ROAD') THEN
  ...
  CALL PREP_HOR_SNOW_FIELDS(HPROGRAM,HSURF,              &
                            ...
                            LSNOW_IDEAL, ZSG1SNOW,       &
                            ZSG2SNOW, ZHISTSNOW, ZAGESNOW )
  DEALLOCATE(XWSNOW_ROAD)
  DEALLOCATE(XRSNOW_ROAD)
  DEALLOCATE(XTSNOW_ROAD)
  DEALLOCATE(ZSG1SNOW)
  DEALLOCATE(ZSG2SNOW)
  DEALLOCATE(ZHISTSNOW)
  DEALLOCATE(ZAGESNOW)                               
...
ELSE IF (YFILETYPE=='GRIB  ') THEN
 ...
 ELSE IF (YFILETYPE=='MESONH' .OR. YFILETYPE=='ASCII ' .OR. YFILETYPE=='LFI   ') THEN
  CALL PREP_TEB_EXTERN(HPROGRAM,HSURF,YFILE,YFILETYPE,ILUOUT,ZFIELDIN)
