!     #########
SUBROUTINE PREP_TEB_EXTERN(HPROGRAM,HSURF,HFILE,HFILETYPE,KLUOUT,PFIELD)
!     #################################################################################
!
...
CALL TOWN_PRESENCE(HFILETYPE,GTEB)
!
!---------------------------------------------------------------------------------------
!
!*     3.      Orography
...
!
  CASE('T_ROAD','T_ROOF','T_WALL')
    ...
    CALL READ_SURF(HFILETYPE,YRECFM,ILAYER,IRESP)
    !
    ...
    CALL OLD_NAME(HFILETYPE,'COVER_LIST      ',YRECFM)
    CALL READ_SURF(HFILETYPE,YRECFM,GCOVER(:),IRESP,HDIR='-')
    !
    ALLOCATE(ZCOVER(INI,JPCOVER))
    YRECFM='COVER'
    CALL READ_SURF(HFILETYPE,YRECFM,ZCOVER(:,:),GCOVER,IRESP,HDIR='A')
    !
 ...
 ELSE
  ...
  SELECT CASE(HSURF)
   ...
   CASE('T_ROAD','T_ROOF','T_WALL')
    ...
    CALL READ_SURF(HFILETYPE,'PATCH_NUMBER',IPATCH,IRESP)
