!     #########
      SUBROUTINE READ_DIRECT_GAUSS(HPROGRAM,HSCHEME,HSUBROUTINE,HFILENAME,HFIELD)
!     #########################################################
!
...
!
USE MODD_GET_MESH_INDEX_GAUSS, ONLY : IMASK_GAUSS
!
USE MODI_GET_LUOUT
...
INTEGER                           :: IWORK, IMASK              ! index of these data
...
IF(YTYPE=='INTEGER')THEN
  IF(HFIELD=='CTI'.OR.HFIELD=='sand fraction'.OR.HFIELD=='clay fraction')THEN
    ZFACT=100.0
  ELSE
    ZFACT=1.0
  ENDIF
...
!            ------------------------
!
ALLOCATE(ZLAT_WORK  (INBCOL*INBLINE))
ALLOCATE(ZLON_WORK  (INBCOL*INBLINE))
ALLOCATE(ZVALUE_WORK(INBCOL*INBLINE))
ALLOCATE(IMASK_GAUSS(INBCOL*INBLINE))
ZLAT_WORK   = XUNDEF
ZLON_WORK   = XUNDEF
ZVALUE_WORK = XUNDEF
!
...
!
IF(IWORK>0)THEN
  CALL PT_BY_PT_TREATMENT(ILUOUT,ZLAT_WORK(1:IWORK),ZLON_WORK(1:IWORK),ZVALUE_WORK(1:IWORK),HSUBROUTINE)
ENDIF
!
DEALLOCATE(ZLAT_WORK  )
DEALLOCATE(ZLON_WORK  )
DEALLOCATE(ZVALUE_WORK)
DEALLOCATE(IMASK_GAUSS)
IF (LHOOK) CALL DR_HOOK('READ_DIRECT_GAUSS',1,ZHOOK_HANDLE)
!
!-------------------------------------------------------------------------------
!
