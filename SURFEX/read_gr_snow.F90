!     #########
      SUBROUTINE READ_GR_SNOW(HPROGRAM,HSURFTYPE,       &
...
!-------------------------------------------------------------------------------
!
!*      0.  Tests for urban snow if urban scheme exists
!           -------------------------------------------
!
IF (HSURFTYPE(1:2)=='RO' .OR. HSURFTYPE(1:2)=='GA') THEN
...
IF (IVERSION <=2 .OR. (IVERSION==3 .AND. IBUGFIX<=4)) THEN
  ...
  WRITE(YRECFM,YFMT) 'SNOW_',HSURFTYPE,'_TYPE'
ELSE
  WRITE(YFMT,'(A5,I1,A4)')     '(A3,A',ISURFTYPE_LEN,',A5)'
  WRITE(YRECFM,YFMT) 'SN_',HSURFTYPE,'_TYPE'
END IF
!
CALL READ_SURF(HPROGRAM,YRECFM,TPSNOW%SCHEME,IRESP)
!
!*       2.    Snow levels
!              -----------
!
...
IF (IVERSION <=2 .OR. (IVERSION==3 .AND. IBUGFIX<=4)) THEN
  ...
  WRITE(YRECFM,YFMT) 'SNOW_',HSURFTYPE,'_LAYER'
ELSE
  WRITE(YFMT,'(A5,I1,A4)')     '(A3,A',ISURFTYPE_LEN,',A2)'
  WRITE(YRECFM,YFMT) 'SN_',HSURFTYPE,'_N'
END IF
!
CALL READ_SURF(HPROGRAM,YRECFM,TPSNOW%NLAYER,IRESP)
!
