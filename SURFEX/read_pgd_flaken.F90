...
USE MODD_FLAKE_n,      ONLY : XCOVER, XZS,TTIME, LCOVER, &
                            XWATER_DEPTH  , &
                            XWATER_FETCH  , &
                            XT_BS         , &
                            XDEPTH_BS     , &
...
!
! The following variables are not in pgd files. 
! They may not be initialized here ... To see later ...
ALLOCATE(XCOR           (NDIM))
ALLOCATE(XALB_WATER     (NDIM))
ALLOCATE(XALB_ICE       (NDIM))
ALLOCATE(XALB_SNOW      (NDIM))
ALLOCATE(XEXTCOEF_ICE   (NDIM))
ALLOCATE(XEXTCOEF_SNOW  (NDIM))
