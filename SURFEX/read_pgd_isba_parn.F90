!     #########
      SUBROUTINE READ_PGD_ISBA_PAR_n(HPROGRAM,OLAND_USE)
!     ################################################
!
!!****  *READ_PGD_ISBA_PAR_n* - reads ISBA physiographic fields                     
!!
!!    PURPOSE
!!    -------
!!
!!**  METHOD
!!    ------
!!
!!    EXTERNAL
!!    --------
!!
!!
!!    IMPLICIT ARGUMENTS
!!    ------------------
!!
!!    REFERENCE
!!    ---------
!!
!!
!!    AUTHOR
!!    ------
!!	V. Masson   *Meteo France*	
!!
!!    MODIFICATIONS
!!    -------------
!!      Original    01/2003 
!!      P. Le Moigne 12/2004 : add type of photosynthesis 
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
!
USE MODD_DATA_COVER_PAR, ONLY : NVEGTYPE
USE MODD_ISBA_GRID_n,    ONLY : NDIM
USE MODD_ISBA_n,         ONLY : LECOCLIMAP, NGROUND_LAYER
USE MODD_DATA_ISBA_n,    ONLY : NTIME, XPAR_VEG, XPAR_LAI,XPAR_RSMIN,XPAR_GAMMA,XPAR_WRMAX_CF, &
                                XPAR_RGL,XPAR_CV,XPAR_DG,XPAR_Z0,XPAR_Z0_O_Z0H,         &
                                XPAR_ALBNIR_VEG,XPAR_ALBVIS_VEG, XPAR_ALBUV_VEG,          &
                                XPAR_ALBNIR_SOIL,XPAR_ALBVIS_SOIL, XPAR_ALBUV_SOIL,       &
                                XPAR_EMIS, XPAR_DICE,                                      &
                                XPAR_VEGTYPE,XPAR_ROOTFRAC,                                &
                                XPAR_GMES,XPAR_BSLAI,XPAR_LAIMIN,XPAR_SEFOLD,XPAR_GC,   &
                                XPAR_DMAX, XPAR_F2I, LPAR_STRESS, XPAR_H_TREE,XPAR_RE25,&
                                XPAR_CE_NITRO,XPAR_CF_NITRO,XPAR_CNA_NITRO, &
                                LPAR_STRESS, XPAR_IRRIG, XPAR_WATSUP, &
                                LDATA_VEGTYPE, LDATA_LAI, LDATA_H_TREE, LDATA_DG, LDATA_ROOTFRAC,&  
                                LDATA_VEG, LDATA_Z0, LDATA_EMIS, LDATA_DICE, &
                                LDATA_RSMIN, LDATA_GAMMA, LDATA_WRMAX_CF, LDATA_RGL, &
                                LDATA_CV, LDATA_Z0_O_Z0H, &
                                LDATA_ALBNIR_VEG, LDATA_ALBVIS_VEG, LDATA_ALBUV_VEG, &
                                LDATA_ALBVIS_SOIL, LDATA_ALBNIR_SOIL, LDATA_ALBUV_SOIL, &
                                LDATA_GMES, LDATA_BSLAI, LDATA_SEFOLD, LDATA_GC, LDATA_DMAX, &
                                LDATA_RE25, LDATA_LAIMIN, LDATA_F2I, &
                                LDATA_CE_NITRO,LDATA_CF_NITRO, LDATA_CNA_NITRO,&
                                LDATA_STRESS, LDATA_IRRIG, LDATA_WATSUP  
!
USE MODI_READ_SURF
USE MODI_READ_SURF_ISBA_PAR_n
USE MODI_PUT_IN_TIME
!
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK
USE PARKIND1  ,ONLY : JPRB
!
IMPLICIT NONE
!
!*       0.1   Declarations of arguments
!              -------------------------
!
CHARACTER(LEN=6),  INTENT(IN)  :: HPROGRAM ! program calling
LOGICAL,           INTENT(IN)  :: OLAND_USE ! 
!
!*       0.2   Declarations of local variables
!              -------------------------------
!
INTEGER           :: ITIME
INTEGER           :: IRESP          ! IRESP  : return-code if a problem appears
CHARACTER(LEN=16) :: YRECFM         ! Name of the article to be read
CHARACTER(LEN=100):: YCOMMENT       ! Comment string
INTEGER           :: JTIME          ! loop index
INTEGER           :: JLAYER         ! loop index
INTEGER           :: JPATCH         ! loop index
INTEGER           :: IVERSION       ! surface version
REAL,    DIMENSION(:,:), ALLOCATABLE :: ZWORK
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!
!
!-------------------------------------------------------------------------------
!
IF (LHOOK) CALL DR_HOOK('READ_PGD_ISBA_PAR_N',0,ZHOOK_HANDLE)
!
YRECFM='VERSION'
CALL READ_SURF(HPROGRAM,YRECFM,IVERSION,IRESP)
!
IF (IVERSION<7 .AND. .NOT.LECOCLIMAP) THEN
  !
  LDATA_VEGTYPE=.TRUE.
  LDATA_VEG=.TRUE.
  LDATA_LAI=.TRUE.
  LDATA_Z0=.TRUE.
  LDATA_EMIS=.TRUE.
  LDATA_RSMIN=.TRUE.
  LDATA_GAMMA=.TRUE.
  LDATA_WRMAX_CF=.TRUE.
  LDATA_RGL=.TRUE.
  LDATA_CV=.TRUE.
  LDATA_Z0_O_Z0H=.TRUE.
  LDATA_DG=.TRUE.
  LDATA_ROOTFRAC=.TRUE.
  LDATA_DICE=.FALSE.
  LDATA_ALBNIR_VEG=.TRUE.
  LDATA_ALBVIS_VEG=.TRUE.
  LDATA_ALBUV_VEG=.TRUE.
  LDATA_ALBNIR_SOIL=.TRUE.
  LDATA_ALBVIS_SOIL=.TRUE.
  LDATA_ALBUV_SOIL=.TRUE.
  LDATA_GMES=.TRUE.
  LDATA_BSLAI=.TRUE.
  LDATA_LAIMIN=.TRUE.
  LDATA_SEFOLD=.TRUE.
  LDATA_GC=.TRUE.
  LDATA_DMAX=.TRUE.
  LDATA_F2I=.TRUE.
  LDATA_STRESS=.TRUE.
  LDATA_H_TREE=.TRUE.
  LDATA_RE25=.TRUE.
  LDATA_CE_NITRO=.TRUE.
  LDATA_CF_NITRO=.TRUE.
  LDATA_CNA_NITRO=.TRUE.
  LDATA_IRRIG=.FALSE.
  LDATA_WATSUP=.FALSE.
  !
ENDIF
!
IF (.NOT.OLAND_USE) THEN
  !
  IF (IVERSION>=7) THEN
  !        
    YRECFM='L_VEGTYPE'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_VEGTYPE,IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='L_VEG'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_VEG,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_LAI'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_LAI,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_Z0'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_Z0,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_EMIS'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_EMIS,IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='L_RSMIN'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_RSMIN,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_GAMMA'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_GAMMA,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_WRMAX_CF'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_WRMAX_CF,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_RGL'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_RGL,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_CV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_CV,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_Z0_O_Z0H'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_Z0_O_Z0H,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_DG'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_DG,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_ROOTFRAC'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ROOTFRAC,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_DICE'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_DICE,IRESP,HCOMMENT=YCOMMENT)    
    YRECFM='L_ALBNIR_VEG'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ALBNIR_VEG,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_ALBVIS_VEG'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ALBVIS_VEG,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_ALBUV_VEG'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ALBUV_VEG,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_ALBNIR_SOI'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ALBNIR_SOIL,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_ALBVIS_SOI'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ALBVIS_SOIL,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_ALBUV_SOI'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ALBUV_SOIL,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_GMES'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_GMES,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_BSLAI'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_BSLAI,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_LAIMIN'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_LAIMIN,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_SEFOLD'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_SEFOLD,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_GC'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_GC,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_DMAX'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_DMAX,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_F2I'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_F2I,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_STRESS'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_STRESS,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_H_TREE'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_H_TREE,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_RE25'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_RE25,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_CE_NITRO'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_CE_NITRO,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_CF_NITRO'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_CF_NITRO,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_CNA_NITRO'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_CNA_NITRO,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_IRRIG'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_IRRIG,IRESP,HCOMMENT=YCOMMENT)
    YRECFM='L_WATSUP'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_WATSUP,IRESP,HCOMMENT=YCOMMENT)
    !
  ENDIF
  !
  !
  IF (LDATA_VEGTYPE) THEN
    ALLOCATE(XPAR_VEGTYPE   (NDIM,NVEGTYPE))
    YRECFM='D_VEGTYPE'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_VEGTYPE(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_LAI .OR. LDATA_VEG .OR. LDATA_Z0 .OR. LDATA_EMIS) THEN
    YRECFM='NDATA_TIME'
    CALL READ_SURF(HPROGRAM,YRECFM,NTIME,IRESP,HCOMMENT=YCOMMENT)
    ITIME = NTIME
  ENDIF
!  
  IF (LDATA_VEG) THEN
    ALLOCATE(XPAR_VEG(NDIM,36,NVEGTYPE))
    DO JTIME=1,ITIME
      WRITE(YRECFM,FMT='(A7,I2.2)') 'D_VEG_T',JTIME
        CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_VEG(:,JTIME,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
    END DO
    CALL PUT_IN_TIME(ITIME,36,XPAR_VEG(:,:,:))
  ENDIF
!
  IF (LDATA_LAI) THEN
    ALLOCATE(XPAR_LAI(NDIM,36,NVEGTYPE))
    DO JTIME=1,ITIME
      WRITE(YRECFM,FMT='(A7,I2.2)') 'D_LAI_T',JTIME
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_LAI(:,JTIME,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
    END DO
    CALL PUT_IN_TIME(ITIME,36,XPAR_LAI(:,:,:))
  ENDIF
!
  IF (LDATA_Z0) THEN
    ALLOCATE(XPAR_Z0        (NDIM,36,NVEGTYPE))
    DO JTIME=1,ITIME
      WRITE(YRECFM,FMT='(A6,I2.2)') 'D_Z0_T',JTIME
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_Z0(:,JTIME,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
    END DO
    CALL PUT_IN_TIME(ITIME,36,XPAR_Z0(:,:,:))
  ENDIF
!
  IF (LDATA_EMIS) THEN
    ALLOCATE(XPAR_EMIS      (NDIM,36,NVEGTYPE))
    DO JTIME=1,ITIME
      WRITE(YRECFM,FMT='(A8,I2.2)') 'D_EMIS_T',JTIME
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_EMIS(:,JTIME,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
    END DO
    CALL PUT_IN_TIME(ITIME,36,XPAR_EMIS(:,:,:))
  ENDIF
!
  IF (LDATA_RSMIN) THEN
    ALLOCATE(XPAR_RSMIN     (NDIM,NVEGTYPE))
    YRECFM='D_RSMIN'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_RSMIN(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_GAMMA) THEN
    ALLOCATE(XPAR_GAMMA     (NDIM,NVEGTYPE))
    YRECFM='D_GAMMA'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_GAMMA(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_WRMAX_CF) THEN
    ALLOCATE(XPAR_WRMAX_CF  (NDIM,NVEGTYPE))
    YRECFM='D_WRMAX_CF'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_WRMAX_CF(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_RGL) THEN
    ALLOCATE(XPAR_RGL       (NDIM,NVEGTYPE))
    YRECFM='D_RGL'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_RGL(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_CV) THEN
    ALLOCATE(XPAR_CV        (NDIM,NVEGTYPE))
    YRECFM='D_CV'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_CV(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_Z0_O_Z0H) THEN
    ALLOCATE(XPAR_Z0_O_Z0H  (NDIM,NVEGTYPE))
    YRECFM='D_Z0_O_Z0H'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_Z0_O_Z0H(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_DG) THEN
    ALLOCATE(XPAR_DG        (NDIM,NGROUND_LAYER,NVEGTYPE))
    ALLOCATE(ZWORK(SIZE(XPAR_DG,1),SIZE(XPAR_DG,3)))
    DO JLAYER=1,SIZE(XPAR_DG,2)
      IF (JLAYER<10)  WRITE(YRECFM,FMT='(A4,I1.1)') 'D_DG',JLAYER
      IF (JLAYER>=10) WRITE(YRECFM,FMT='(A4,I2.2)') 'D_DG',JLAYER
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ZWORK,IRESP,IVERSION,HCOMMENT=YCOMMENT)
      DO JPATCH=1,SIZE(XPAR_DG,3)
        XPAR_DG(:,JLAYER,JPATCH) = ZWORK(:,JPATCH)
      END DO
    END DO
    DEALLOCATE(ZWORK)
  ENDIF
!
  IF (LDATA_ROOTFRAC) THEN
    ALLOCATE(XPAR_ROOTFRAC  (NDIM,NGROUND_LAYER,NVEGTYPE))
    ALLOCATE(ZWORK(SIZE(XPAR_ROOTFRAC,1),SIZE(XPAR_ROOTFRAC,3)))
    DO JLAYER=1,SIZE(XPAR_ROOTFRAC,2)
      IF (JLAYER<10)  WRITE(YRECFM,FMT='(A10,I1.1)') 'D_ROOTFRAC',JLAYER
      IF (JLAYER>=10) WRITE(YRECFM,FMT='(A10,I2.2)') 'D_ROOTFRAC',JLAYER
        CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ZWORK,IRESP,IVERSION,HCOMMENT=YCOMMENT)
      DO JPATCH=1,SIZE(XPAR_ROOTFRAC,3)
        XPAR_ROOTFRAC(:,JLAYER,JPATCH) = ZWORK(:,JPATCH)
      END DO
    END DO
    DEALLOCATE(ZWORK)
  ENDIF
!
  IF (LDATA_DICE) THEN
    ALLOCATE(XPAR_DICE      (NDIM,NVEGTYPE))
    YRECFM='D_DICE'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_DICE(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBNIR_VEG) THEN
    ALLOCATE(XPAR_ALBNIR_VEG(NDIM,NVEGTYPE))
    YRECFM='D_ALBNIR_VEG'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_ALBNIR_VEG(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBVIS_VEG) THEN
    ALLOCATE(XPAR_ALBVIS_VEG(NDIM,NVEGTYPE))
    YRECFM='D_ALBVIS_VEG'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_ALBVIS_VEG(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBUV_VEG) THEN
    ALLOCATE(XPAR_ALBUV_VEG (NDIM,NVEGTYPE))
    YRECFM='D_ALBUV_VEG'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_ALBUV_VEG(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBNIR_SOIL) THEN
    ALLOCATE(XPAR_ALBNIR_SOIL(NDIM,NVEGTYPE))
    YRECFM='D_ALBNIR_SOI'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_ALBNIR_SOIL(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBVIS_SOIL) THEN
    ALLOCATE(XPAR_ALBVIS_SOIL(NDIM,NVEGTYPE))
    YRECFM='D_ALBVIS_SOI'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_ALBVIS_SOIL(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBUV_SOIL) THEN
    ALLOCATE(XPAR_ALBUV_SOIL (NDIM,NVEGTYPE))
    YRECFM='D_ALBUV_SOI'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_ALBUV_SOIL(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_GMES) THEN
    ALLOCATE(XPAR_GMES      (NDIM,NVEGTYPE))
    YRECFM='D_GMES'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_GMES(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_BSLAI) THEN
    ALLOCATE(XPAR_BSLAI     (NDIM,NVEGTYPE))
    YRECFM='D_BSLAI'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_BSLAI(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_LAIMIN) THEN
    ALLOCATE(XPAR_LAIMIN    (NDIM,NVEGTYPE))
    YRECFM='D_LAIMIN'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_LAIMIN(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_SEFOLD) THEN
    ALLOCATE(XPAR_SEFOLD    (NDIM,NVEGTYPE))
    YRECFM='D_SEFOLD'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_SEFOLD(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_GC) THEN
    ALLOCATE(XPAR_GC        (NDIM,NVEGTYPE))
    YRECFM='D_GC'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_GC(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_DMAX) THEN
    ALLOCATE(XPAR_DMAX      (NDIM,NVEGTYPE))
    YRECFM='D_DMAX'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_DMAX(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_F2I) THEN
    ALLOCATE(XPAR_F2I       (NDIM,NVEGTYPE))
    YRECFM='D_F2I'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_F2I(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_STRESS) THEN
    ALLOCATE(LPAR_STRESS   (NDIM,NVEGTYPE))
    ALLOCATE(ZWORK(SIZE(LPAR_STRESS,1),SIZE(LPAR_STRESS,2)))
    YRECFM='D_STRESS'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ZWORK(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
    LPAR_STRESS = .FALSE.
    WHERE(ZWORK==1.) LPAR_STRESS = .TRUE.
    DEALLOCATE(ZWORK)
  ENDIF
!
  IF (LDATA_H_TREE) THEN
    ALLOCATE(XPAR_H_TREE    (NDIM,NVEGTYPE))
    YRECFM='D_H_TREE'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_H_TREE(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_RE25) THEN
    ALLOCATE(XPAR_RE25      (NDIM,NVEGTYPE))
    YRECFM='D_RE25'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_RE25(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_CE_NITRO) THEN
    ALLOCATE(XPAR_CE_NITRO  (NDIM,NVEGTYPE))
    YRECFM='D_CE_NITRO'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_CE_NITRO(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_CF_NITRO) THEN
    ALLOCATE(XPAR_CF_NITRO  (NDIM,NVEGTYPE))
    YRECFM='D_CF_NITRO'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_CF_NITRO(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_CNA_NITRO) THEN
    ALLOCATE(XPAR_CNA_NITRO (NDIM,NVEGTYPE))
    YRECFM='D_CNA_NITRO'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_CNA_NITRO(:,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_IRRIG) THEN
    ALLOCATE(XPAR_IRRIG     (NDIM,36,NVEGTYPE))
    DO JTIME=1,ITIME
      WRITE(YRECFM,FMT='(A9,I2.2)') 'D_IRRIG_T',JTIME
        CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_IRRIG(:,JTIME,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
    END DO
    CALL PUT_IN_TIME(ITIME,36,XPAR_IRRIG(:,:,:))
  ENDIF
!
  IF (LDATA_WATSUP) THEN
    ALLOCATE(XPAR_WATSUP     (NDIM,36,NVEGTYPE))
    DO JTIME=1,ITIME
      WRITE(YRECFM,FMT='(A10,I2.2)') 'D_WATSUP_T',JTIME
        CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,XPAR_WATSUP(:,JTIME,:),IRESP,IVERSION,HCOMMENT=YCOMMENT)
    END DO
    CALL PUT_IN_TIME(ITIME,36,XPAR_WATSUP(:,:,:))
  ENDIF
!
ENDIF
!
IF (LHOOK) CALL DR_HOOK('READ_PGD_ISBA_PAR_N',1,ZHOOK_HANDLE)
!
!-------------------------------------------------------------------------------
!
END SUBROUTINE READ_PGD_ISBA_PAR_n
