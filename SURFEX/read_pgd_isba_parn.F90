  ...
  IF (LDATA_VEG) THEN
    ...
    DO JTIME=1,36
      WRITE(YRECFM,FMT='(A7,I2.2)') 'D_VEG_T',JTIME
        CALL READ_SURF(HPROGRAM,YRECFM,XPAR_VEG(:,JTIME,:),IRESP,HCOMMENT=YCOMMENT)
    END DO
  ENDIF
!
  IF (LDATA_LAI) THEN
    ...
    DO JTIME=1,36
      WRITE(YRECFM,FMT='(A7,I2.2)') 'D_LAI_T',JTIME
        CALL READ_SURF(HPROGRAM,YRECFM,XPAR_LAI(:,JTIME,:),IRESP,HCOMMENT=YCOMMENT)
    END DO
  ENDIF
!
  IF (LDATA_Z0) THEN
    ...
    DO JTIME=1,36
      WRITE(YRECFM,FMT='(A6,I2.2)') 'D_Z0_T',JTIME
        CALL READ_SURF(HPROGRAM,YRECFM,XPAR_Z0(:,JTIME,:),IRESP,HCOMMENT=YCOMMENT)
    END DO
  ENDIF
!
  IF (LDATA_EMIS) THEN
    ...
    DO JTIME=1,36
      WRITE(YRECFM,FMT='(A8,I2.2)') 'D_EMIS_T',JTIME
        CALL READ_SURF(HPROGRAM,YRECFM,XPAR_EMIS(:,JTIME,:),IRESP,HCOMMENT=YCOMMENT)
    END DO
  ENDIF
!
  IF (LDATA_RSMIN) THEN
    ...
    YRECFM='D_RSMIN'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_RSMIN(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_GAMMA) THEN
    ALLOCATE(XPAR_GAMMA     (NDIM,NVEGTYPE))
    YRECFM='D_GAMMA'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_GAMMA(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_WRMAX_CF) THEN
    ALLOCATE(XPAR_WRMAX_CF  (NDIM,NVEGTYPE))
    YRECFM='D_WRMAX_CF'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_WRMAX_CF(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_RGL) THEN
    ALLOCATE(XPAR_RGL       (NDIM,NVEGTYPE))
    YRECFM='D_RGL'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_RGL(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_CV) THEN
    ALLOCATE(XPAR_CV        (NDIM,NVEGTYPE))
    YRECFM='D_CV'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_CV(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_Z0_O_Z0H) THEN
    ALLOCATE(XPAR_Z0_O_Z0H  (NDIM,NVEGTYPE))
    YRECFM='D_Z0_O_Z0H'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_Z0_O_Z0H(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_DG) THEN
    ...
    DO JLAYER=1,SIZE(XPAR_DG,2)
      ...
      IF (JLAYER>=10) WRITE(YRECFM,FMT='(A4,I2.2)') 'D_DG',JLAYER
        CALL READ_SURF(HPROGRAM,YRECFM,ZWORK,IRESP,HCOMMENT=YCOMMENT)
...
!
  IF (LDATA_DICE) THEN
    ...
    YRECFM='D_DICE'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_DICE(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBNIR_VEG) THEN
    ALLOCATE(XPAR_ALBNIR_VEG(NDIM,NVEGTYPE))
    YRECFM='D_ALBNIR_VEG'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_ALBNIR_VEG(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBVIS_VEG) THEN
    ALLOCATE(XPAR_ALBVIS_VEG(NDIM,NVEGTYPE))
    YRECFM='D_ALBVIS_VEG'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_ALBVIS_VEG(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBUV_VEG) THEN
    ALLOCATE(XPAR_ALBUV_VEG (NDIM,NVEGTYPE))
    YRECFM='D_ALBUV_VEG'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_ALBUV_VEG(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBNIR_SOIL) THEN
    ALLOCATE(XPAR_ALBNIR_SOIL(NDIM,NVEGTYPE))
    YRECFM='D_ALBNIR_SOI'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_ALBNIR_SOIL(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBVIS_SOIL) THEN
    ALLOCATE(XPAR_ALBVIS_SOIL(NDIM,NVEGTYPE))
    YRECFM='D_ALBVIS_SOI'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_ALBVIS_SOIL(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_ALBUV_SOIL) THEN
    ALLOCATE(XPAR_ALBUV_SOIL (NDIM,NVEGTYPE))
    YRECFM='D_ALBUV_SOI'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_ALBUV_SOIL(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_GMES) THEN
    ALLOCATE(XPAR_GMES      (NDIM,NVEGTYPE))
    YRECFM='D_GMES'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_GMES(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_BSLAI) THEN
    ALLOCATE(XPAR_BSLAI     (NDIM,NVEGTYPE))
    YRECFM='D_BSLAI'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_BSLAI(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_LAIMIN) THEN
    ALLOCATE(XPAR_LAIMIN    (NDIM,NVEGTYPE))
    YRECFM='D_LAIMIN'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_LAIMIN(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_SEFOLD) THEN
    ALLOCATE(XPAR_SEFOLD    (NDIM,NVEGTYPE))
    YRECFM='D_SEFOLD'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_SEFOLD(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_GC) THEN
    ALLOCATE(XPAR_GC        (NDIM,NVEGTYPE))
    YRECFM='D_GC'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_GC(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_DMAX) THEN
    ALLOCATE(XPAR_DMAX      (NDIM,NVEGTYPE))
    YRECFM='D_DMAX'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_DMAX(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_F2I) THEN
    ALLOCATE(XPAR_F2I       (NDIM,NVEGTYPE))
    YRECFM='D_F2I'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_F2I(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
...
!
  IF (LDATA_H_TREE) THEN
    ...
    YRECFM='D_H_TREE'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_H_TREE(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_RE25) THEN
    ALLOCATE(XPAR_RE25      (NDIM,NVEGTYPE))
    YRECFM='D_RE25'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_RE25(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_CE_NITRO) THEN
    ALLOCATE(XPAR_CE_NITRO  (NDIM,NVEGTYPE))
    YRECFM='D_CE_NITRO'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_CE_NITRO(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_CF_NITRO) THEN
    ALLOCATE(XPAR_CF_NITRO  (NDIM,NVEGTYPE))
    YRECFM='D_CF_NITRO'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_CF_NITRO(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_CNA_NITRO) THEN
    ALLOCATE(XPAR_CNA_NITRO (NDIM,NVEGTYPE))
    YRECFM='D_CNA_NITRO'
    CALL READ_SURF(HPROGRAM,YRECFM,XPAR_CNA_NITRO(:,:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
  IF (LDATA_IRRIG) THEN
    ...
    DO JTIME=1,36
      WRITE(YRECFM,FMT='(A9,I2.2)') 'D_IRRIG_T',JTIME
        CALL READ_SURF(HPROGRAM,YRECFM,XPAR_IRRIG(:,JTIME,:),IRESP,HCOMMENT=YCOMMENT)
    END DO
  ENDIF
!
  IF (LDATA_WATSUP) THEN
    ...
    DO JTIME=1,36
      WRITE(YRECFM,FMT='(A10,I2.2)') 'D_WATSUP_T',JTIME
        CALL READ_SURF(HPROGRAM,YRECFM,XPAR_WATSUP(:,JTIME,:),IRESP,HCOMMENT=YCOMMENT)
    END DO
  ENDIF
!
ENDIF
