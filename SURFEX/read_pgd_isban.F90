...
USE MODD_ISBA_n, ONLY : NPATCH, TTIME, XCOVER, XZS, CISBA, CDIF,     &
                          CPEDOTF, CPHOTO, CRUNOFF, XCLAY, XSAND,    &
                          XORGMAT, XDENSITY,                                                   &
...
IF(CDIF=='VG')THEN
!  
  ALLOCATE(XORGMAT (NDIM,NGROUND_LAYER))
  ALLOCATE(XDENSITY(NDIM,NGROUND_LAYER))
!
  YRECFM='ORGMAT'
  CALL READ_SURF(HPROGRAM,YRECFM,XORGMAT(:,1),IRESP)
!
  YRECFM='DENSITY'
  CALL READ_SURF(HPROGRAM,YRECFM,XDENSITY(:,1),IRESP)
!
  DO JLAYER=2,NGROUND_LAYER
...
!
CALL READ_PGD_ISBA_PAR_n(HPROGRAM,OLAND_USE)
