!     #########
     SUBROUTINE SLT_DEP (PSVT, PFSVT,  PUSTAR, &
                      PRESA, PTA, PRHODREF)  
!###########################################################
  !
  !!                   
  ...
  !!    -------
  !!      
  !!    Compute dry deposition velocity for sea salt species 
  !!
  !!    AUTHOR
  !!    ------
  !!      P.Tulet      * CNRM *
...
!
  ...
  !
  REAL , DIMENSION(SIZE(PSVT,1), JPMODE_SLT*3) :: ZRD ! surface  resistance
  REAL , DIMENSION(SIZE(PSVT,1), JPMODE_SLT*3) :: ZVD ! [m/s] dry deposition velocity 
  REAL , DIMENSION(SIZE(PSVT,1), JPMODE_SLT*3) :: Stn ! Stockes number
  REAL , DIMENSION(SIZE(PSVT,1), JPMODE_SLT*3) :: Sc  ! Schmidt number
  REAL , DIMENSION(SIZE(PSVT,1))      :: ZUSTAR, ZRESA
  REAL , DIMENSION(SIZE(PSVT,1), JPMODE_SLT*3):: ZWORK
  REAL, DIMENSION(SIZE(PSVT,1)) :: ZNU
  REAL, DIMENSION(SIZE(PSVT,1),JPMODE_SLT*3)     :: Dg,zvs,zvsg, zdsg
  REAL, DIMENSION(SIZE(PSVT,1))        :: ZMU
  REAL, DIMENSION(SIZE(PSVT,1),JPMODE_SLT*3)   :: ZVGK, ZDPK
  REAL, DIMENSION(SIZE(PSVT,1),JPMODE_SLT) :: ZSIG, ZRG
  REAL, DIMENSION(SIZE(PSVT,1),JPMODE_SLT) :: ZVG, ZDG
  REAL, DIMENSION(SIZE(PSVT,1), SIZE(PSVT,2)) :: ZSVT
  INTEGER :: JJ, JSV, JN
  REAL :: ZDEN2MOL, ZG, ZMD, ZAVOGADRO,ZMI, ZFAC, ZPI, ZRHOP
  INTEGER, DIMENSION(JPMODE_SLT*3) :: NM0, NM3, NM6
...
IF (LHOOK) CALL DR_HOOK('SLT_DEP',0,ZHOOK_HANDLE)
ZRHOP = XDENSITY_SALT
ZPI   = 2.*ASIN(1.)
ZFAC  = (4./3.)*ZPI*ZRHOP*1.e-9
ZMI   = XMOLARWEIGHT_SALT*1E3 ! molecular mass in g/mol
...
CALL SALTMOMENT2SIZE(ZSVT, PRHODREF, PSIG1D=ZSIG, PRG1D=ZRG ) !don't need ZN, PN1D=ZN)

CALL SLT_VELGRAV1D(ZSIG, ZRG, PTA, PRHODREF, ZRHOP, ZMU, ZVGK,ZDPK, ZVG, ZDG)
...
Stn(:,:) =0.
  DO  JN=1,JPMODE_SLT*3
   ...
   ! The gravitation term as been computed by MesoNH (see sedim_salt.f90)
...
! Only M3 flux (mass) has been used ; flux for over moment has been made after
  ...
  ENDDO
IF (LHOOK) CALL DR_HOOK('SLT_DEP',1,ZHOOK_HANDLE)

!
!							       	       
!---------------------------------------------------------------------
!
END SUBROUTINE SLT_DEP
