...
!* default value
!
  GMASK (:) = (XZ0REL/=XUNDEF .AND. XZ0REL/=0. )
  ZZ0EFF(:) = XUNDEF
!
...
!
IF (CROUGH=="Z04D") THEN
  WHERE(GMASK(:)) ZALFA(:) = ZDIR(:) - XZ0EFFJPDIR(:) * XPI/180.
  WHERE(GMASK(:) .AND. ZALFA(:)<=-XPI) ZALFA = ZALFA + 2.*XPI
  WHERE(GMASK(:) .AND. ZALFA(:)>  XPI) ZALFA = ZALFA - 2.*XPI

  WHERE(GMASK(:) .AND. ZALFA(:)>=0. .AND. ZALFA(:)<XPI/2.)
    ZZ0EFF(:)=XZ0EFFIP(:)*SIN(ZALFA(:))**2 + XZ0EFFJP(:)*COS(ZALFA(:))**2
  END WHERE
  WHERE(GMASK(:) .AND. ZALFA(:)>=XPI/2. .AND. ZALFA(:)<=XPI)
    ZZ0EFF(:)=XZ0EFFIP(:)*SIN(ZALFA(:))**2 + XZ0EFFJM(:)*COS(ZALFA(:))**2
  END WHERE
  WHERE (GMASK(:) .AND. ZALFA(:)>=-XPI/2 .AND. ZALFA(:)<0.)
    ZZ0EFF(:)=XZ0EFFIM(:)*SIN(ZALFA(:))**2 + XZ0EFFJP(:)*COS(ZALFA(:))**2
  END WHERE
  WHERE (GMASK(:) .AND. ZALFA(:)>=-XPI .AND. ZALFA(:)<-XPI/2.)
    ZZ0EFF(:)=XZ0EFFIM(:)*SIN(ZALFA(:))**2 + XZ0EFFJM(:)*COS(ZALFA(:))**2
  END WHERE
END IF
!
