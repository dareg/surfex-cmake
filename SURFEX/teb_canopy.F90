...
REAL, DIMENSION(KI)      :: ZHOR_DRAG ! horizontal drag force at canopy top
...
DO JLAYER=2,KLVL-1
  WHERE(PZF(:,JLAYER)<PBLD_HEIGHT(:) .AND. PZF(:,JLAYER+1)>=PBLD_HEIGHT(:)) &
  ZSH(:,JLAYER) = PBLD(:)
END DO
WHERE(PZF(:,KLVL)<PBLD_HEIGHT(:)) ZSH(:,KLVL) = PBLD(:)
!
!*      1.3    Proportion of vertical   surfaces for each canopy level
!
!
ZSV(:,:) = 0.
DO JLAYER=1,KLVL-1
  WHERE(PZF(:,JLAYER)<PBLD_HEIGHT(:)) &
  ZSV(:,JLAYER) = PDZ(:,JLAYER) / PBLD_HEIGHT(:) * PWALL_O_HOR(:)
  WHERE(PZF(:,JLAYER)<PBLD_HEIGHT(:) .AND. PZF(:,JLAYER+1)>PBLD_HEIGHT(:)) &
  ZSV(:,JLAYER) = (PBLD_HEIGHT(:)-PZF(:,JLAYER)) / PBLD_HEIGHT(:) * PWALL_O_HOR(:)
END DO
!
...
!
PFORC_U(:,:) = PFORC_U    -      ZCDRAG(:,:)*PU(:,:)**2 * ZSV(:,:)/ZAIRVOL(:,:)/PDZ(:,:)
PDFORC_UDU   = PDFORC_UDU - 2. * ZCDRAG(:,:)*PU(:,:)    * ZSV(:,:)/ZAIRVOL(:,:)/PDZ(:,:)
!
!
...
!
DO JLAYER=2,KLVL
  ZHOR_DRAG(:)         = PUW_ROOF(:) * ZSH(:,JLAYER)/ZAIRVOL(:,JLAYER)/PDZ(:,JLAYER)
  PFORC_U(:,JLAYER)    = PFORC_U(:,JLAYER) + ZHOR_DRAG(:)
  PDFORC_UDU(:,JLAYER) = PDFORC_UDU(:,JLAYER) + PDUWDU_ROOF(:) * ZSH(:,JLAYER)/ZAIRVOL(:,JLAYER)/PDZ(:,JLAYER)
END DO
!
...
!
!
!*      3.1    Creation of TKE by wall surfaces
!              --------------------------------
!
...
! and  Sv/Vair = (Sv/Stot) * Stot/Vair = (Sv/Stot) / (Vair/Vtot) / Dz
!
PFORC_E    = PFORC_E    + ZCDRAG(:,:)*PU(:,:)**3 * ZSV(:,:)/ZAIRVOL(:,:)/PDZ(:,:)
...
!
DO JLAYER=1,KLVL
  PFORC_T(:,JLAYER) = PFORC_T(:,JLAYER) + PH_WALL / XCPD / PRHOA * ZSV(:,JLAYER)/ZAIRVOL(:,JLAYER)/PDZ(:,JLAYER)
...
!
DO JLAYER=2,KLVL
  PFORC_T(:,JLAYER)    = PFORC_T(:,JLAYER) + PH_ROOF / XCPD / PRHOA * ZSH(:,JLAYER)/ZAIRVOL(:,JLAYER)/PDZ(:,JLAYER)
  PDFORC_TDT(:,JLAYER) = PDFORC_TDT(:,JLAYER) + 0.
END DO
!
!-------------------------------------------------------------------------------------
