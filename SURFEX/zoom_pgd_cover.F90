...
IF (.NOT. OECOCLIMAP) THEN
  WRITE(ILUOUT,*) 'ERROR :'
  WRITE(ILUOUT,*) 'The zoom of PGD files without Ecoclimap is not implemented yet'
  WRITE(ILUOUT,*) 'Routine zoom_pgd_cover.f90 must be updated'
  WRITE(ILUOUT,*) 'to interpolate all COVER physiographic fields'
  CALL ABOR1_SFX('ZOOM_PGD_COVER: ZOOM OF PGD FILES WITHOUT ECOCLIMAP NOT IMPLEMENTED YET')
END IF
!
!
!------------------------------------------------------------------------------
