...
USE MODD_ISBA_n,           ONLY : XCLAY, XSAND, XRUNOFFB, XWDRAIN, &
                                    LECOCLIMAP, NGROUND_LAYER  
USE MODD_ISBA_GRID_n,      ONLY : XLAT, XLON, CGRID, XGRID_PAR
...
IF (.NOT. LECOCLIMAP) THEN
  ...
  WRITE(ILUOUT,*) 'Routine zoom_pgd_isba_full.f90 must be updated'
  WRITE(ILUOUT,*) 'to interpolate all ISBA physiographic fields'
  CALL ABOR1_SFX('ZOOM_PGD_ISBA_FULL: ECOCLIMAP NOT USED, ROUTINE MUST BE UPDATED')
END IF
!
