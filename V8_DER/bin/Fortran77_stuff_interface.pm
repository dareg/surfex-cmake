...
# Also put into the hash the linenumber of first line of statement as key 'first_line'
  ...
  foreach $line (@$lines) {
    ...
    if(/^\ \ \ \ \ ./ && !/^\ \ \ \ \ \ /) {
      s/^(\ \ \ \ \ )(.)(.+)$/$1\ $3/s;
      $first_line=$line_number-1 unless($prev);
      if($prev ne 1) {
        $statm=$save.$_;
        $prev=1;
      }
...
#    print "exec $href->{exec} \n";
    ...
    #mise en mémoire de l'indentation de la dernière instruction qui n'est pas un commentaire
    if ($cont ne 'comment') {
      $remember3=$cont;
      $indent=$href->{indent};
      }
...
#      }
#      els
#      s'il y a un implicit none, c'est simple, on place les instructions justes avant
	...
	$hook_status=1;
      ...
      #s'il y a un contains, on les met aussi
     elsif ($cont eq 'CONTAINS') {
       $$statements[$remember2+$nb_comment]->{post_insert}= $indent."USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK\n".
	  $indent."USE PARKIND1  ,ONLY : JPRB\n"."!\n";
       $hook_status=1;
       $parkind1_inserted=1;
      }
...
#      $href->{pre_insert}=$indent."USE PARKIND1  ,ONLY : JPRB\n";
#      $parkind1_inserted=1;
#    }
...
#   before first executable statement in program unit ($exec == 2)
    if($hook_status == 1 && ($exec == 2 or $exec == 23)) {
      if($remember) {
	$$statements[$remember]->{post_insert}=$indent3."REAL(KIND=JPRB) :: ZHOOK_HANDLE\n";
	$href->{pre_insert}=$indent."IF (LHOOK) CALL DR_HOOK(\'${unit_name}\',0,ZHOOK_HANDLE)\n";
      }
      else {
...
#   ($exec == 3)
	...
	$href->{post_insert}=$indent."IF (LHOOK) CALL DR_HOOK(\'${unit_name}\',1,ZHOOK_HANDLE)\n";
	$hook_status=3;
      }
      elsif($cont eq 'RETURN') {
	$href->{pre_insert}=$href->{pre_insert}.$indent."IF (LHOOK) CALL DR_HOOK(\'${unit_name}\',1,ZHOOK_HANDLE)\n";
      }
      elsif($cont eq 'IF') {
	      #print "data $href->{statement} \n";
	if($href->{content2} eq 'RETURN') {
	  $_=$href->{statement};
	  s/(\s*IF\s*$nest_par).*\n/$1/i;
	  s/\)$/ .AND. LHOOK\)/;
...
#      print '1',$myhref->{statement};
    ...
    if($content eq 'END SUBROUTINE' or $content eq 'END FUNCTION') { # Add END statement to interface block
