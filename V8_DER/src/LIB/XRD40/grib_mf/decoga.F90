     ...
     &              KJLENV,KJLENF,KCPACK,KSCALP,KERR,PMIN,PMAX,LDARPE)
...
!
      ...
      INTEGER(KIND=JPIM) :: KJLENV
      INTEGER(KIND=JPIM) :: KJLENF
...
!
      REAL (KIND=JPDBLR) :: PFDATA(KLENF)
      REAL (KIND=JPDBLR) :: PVERT(KLENV)
!
      LOGICAL :: LDARPE
!
      INTEGER(KIND=JPIM) :: IMAX, IOFF, IBYTE, INVAL, ISNEW
      INTEGER(KIND=JPIM) :: J, ITEMP, INC, ILBLK
      INTEGER(KIND=JPIM) :: ILNIL, IEXP, IMANT, ILEN
      INTEGER(KIND=JPIM) :: IPW, IPB, ILBIN, IFLAG, IREP
      INTEGER(KIND=JPIM) :: ISCALX, ISCALE, IMISS, ISPDA
      INTEGER(KIND=JPIM) :: IPREMC, ISSUIV, IL, ILBVAL
...
!
!
      INTEGER(KIND=JPIM) :: IBLOCK(24), ILAT(2)
...
!
        ZSCALE= ( PMAX - PMIN ) / REAL(2**KBITS-1,KIND=JPDBLR)
!
...
!
        ...
        ZSCALE = 2.0_JPDBLR**ISCALE
        PMAX=PMIN+REAL(2**(KBITS+1)-3,KIND=JPDBLR) *                    &
     &                 2.0_JPDBLR ** (ISCALE-1)
!
      ENDIF
!
...
!
 ...
 9003        FORMAT (1H ,'NUMBER OF VALUES TO BE DECODED IS - ',I7,     &
     &                    ', ARRAY SIZE - ',I7)
         ...
         ENDIF
!
      IF (IMISS.EQ.0.AND.ZSCALE.GT.0.0_JPDBLR) THEN
