...
IF ( CASSIM_ISBA /= 'OI   ' ) THEN
  ...
  IF (LAESNM) THEN
    ...
    CALL ASSIM_ISBA_UPDATE_SNOW(I,HPROGRAM,KI,ZSWE,ZSWE_ORIG,.TRUE.,.TRUE.,HTEST)
  ELSE
...
ELSEIF ( CASSIM_ISBA == 'OI   ' ) THEN
  ...
  IF (LAESNM) THEN
    IF (NRANK==NPIO) WRITE(*,*) 'UPDATE SNOW FROM ANALYSED VALUES'
    CALL ASSIM_ISBA_UPDATE_SNOW(I,HPROGRAM,KI,ZSWE,ZSWE_ORIG,.TRUE.,.FALSE.,HTEST)
  ELSE
    IF (NRANK==NPIO) WRITE(*,*) 'SNOW IS NOT UPDATED FROM ANALYSED VALUES'
  ENDIF
  !
  ! Run OI for soil
  CALL ASSIM_NATURE_ISBA_OI(I,HPROGRAM, KI,                                &
                            PCON_RAIN, PSTRAT_RAIN, PCON_SNOW, PSTRAT_SNOW,&
                            PCLOUDS,   PLSM,        PEVAPTR,   PEVAP,      &
                            PSWEC,     PTSC,        PUCLS,     PVCLS,      &
                            PTS,       PT2M,        PHU2M,     ZSWE,       &
  ...
  IF (LAESNM) THEN
    ...
    CALL ASSIM_ISBA_UPDATE_SNOW(I,HPROGRAM,KI,ZSWE,ZSWE_ORIG,.FALSE.,.TRUE.,HTEST)
  ELSE
...
IF ( LEXTRAP_NATURE ) THEN
  ...
  ZTS_EP  = I%R%XTG(:,1,JP)
  ZTP_EP  = I%R%XTG(:,2,JP)
  ZTL_EP  = I%R%XWGI(:,2,JP)
  ZSWE_EP = I%R%TSNOW%WSNOW(:,JL,JP)
  ZSNR_EP = I%R%TSNOW%RHO  (:,JL,JP)
  ZSNA_EP = I%R%TSNOW%ALB  (:,   JP)
  !
  ...
  ZTS_EP0(:) = ZTS_EP(:)
  CALL OI_HOR_EXTRAPOL_SURF(KI,IG%XLAT,IG%XLON,ZTS_EP0,IG%XLAT,IG%XLON,ZTS_EP,GINTERP_NATURE,I%P%XZS)
  ZTP_EP0(:) = ZTP_EP(:)
  CALL OI_HOR_EXTRAPOL_SURF(KI,IG%XLAT,IG%XLON,ZTP_EP0,IG%XLAT,IG%XLON,ZTP_EP,GINTERP_NATURE,I%P%XZS)
  ZTL_EP0(:) = ZTL_EP(:)
...
IF (LAESNM) THEN
  ...
  WHERE( I%R%TSNOW%WSNOW(:,JL,JP) < 1.0E-10 ) I%R%TSNOW%WSNOW(:,JL,JP) = 0.0

  ! No SNOW
  WHERE ( I%R%TSNOW%WSNOW(:,JL,JP) == 0.0 )
    I%R%TSNOW%RHO(:,JL,JP) = XUNDEF
    I%R%TSNOW%ALB(:,JP)    = XUNDEF
  END WHERE
  !
ENDIF
!
!to be improved later - needed for surfex course
 CALL AVERAGE_DIAG_MISC_ISBA_n(DGMI, DGMIP, I%O, I%IP, I%M%X, I%M%T%XLAI, I%R)
 !
