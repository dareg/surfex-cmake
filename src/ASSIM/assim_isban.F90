!     ###############################################################################
SUBROUTINE ASSIM_ISBA_n (DMI, DMIP, G, IO, S, K, NP, NPE, U, &
                         HPROGRAM,KI,                                   &
...
IF ( CASSIM_ISBA /= 'OI   ' ) THEN
  ...
  IF (LAESNM) THEN
    ...
    CALL ASSIM_ISBA_UPDATE_SNOW(IO, NP, NPE,HPROGRAM,KI,ZSWE,ZSWE_ORIG,.TRUE.,.TRUE.,HTEST)
  ELSE
...
IF ( CASSIM_ISBA == 'EKF  ' ) THEN
  !
  ! Run EKF for soil
  CALL ASSIM_NATURE_ISBA_EKF(IO, S, K, NP, NPE, &
                             HPROGRAM, KI, PT2M, PHU2M, HTEST)
  !
ELSEIF ( CASSIM_ISBA == 'ENKF ') THEN
  !
  CALL ASSIM_NATURE_ISBA_ENKF(IO, S, K, NP, NPE, &
                              HPROGRAM, KI, PT2M, PHU2M, HTEST)
  !  
ELSEIF ( CASSIM_ISBA == 'OI   ' ) THEN
  ...
  IF (LAESNM) THEN
    IF (NRANK==NPIO) WRITE(*,*) 'UPDATE SNOW FROM ANALYSED VALUES'
    CALL ASSIM_ISBA_UPDATE_SNOW(IO,NP, NPE,HPROGRAM,KI,ZSWE,ZSWE_ORIG,.TRUE.,.FALSE.,HTEST)
  ELSE
    IF (NRANK==NPIO) WRITE(*,*) 'SNOW IS NOT UPDATED FROM ANALYSED VALUES'
  ENDIF
  !
  ! Run OI for soil
  CALL ASSIM_NATURE_ISBA_OI(IO, S, K, NP, NPE,HPROGRAM, KI,                &
                            PCON_RAIN, PSTRAT_RAIN, PCON_SNOW, PSTRAT_SNOW,&
                            PCLOUDS,   PLSM,        PEVAPTR,   PEVAP,      &
                            PSWEC,     PTSC,        PUCLS,     PVCLS,      &
                            PTS,       PT2M,        PHU2M,     ZSWE,       &
  ...
  IF (LAESNM) THEN
    ...
    CALL ASSIM_ISBA_UPDATE_SNOW(IO,NP, NPE,HPROGRAM,KI,ZSWE,ZSWE_ORIG,.FALSE.,.TRUE.,HTEST)
  ELSE
...
IF ( LEXTRAP_NATURE ) THEN
  ...
  DO JI = 1,NP%AL(JP)%NSIZE_P
    ...
    ZWS_EP(IMASK)  = NPE%AL(JP)%XWG(JI,1)
    ZWP_EP(IMASK)  = NPE%AL(JP)%XWG(JI,2)
    ZTS_EP(IMASK)  = NPE%AL(JP)%XTG(JI,1)
    ZTP_EP(IMASK)  = NPE%AL(JP)%XTG(JI,2)
    ZTL_EP(IMASK)  = NPE%AL(JP)%XWGI(JI,2)
    ZSWE_EP(IMASK) = NPE%AL(JP)%TSNOW%WSNOW(JI,JL)
    ZSNR_EP(IMASK) = NPE%AL(JP)%TSNOW%RHO  (JI,JL)
    ZSNA_EP(IMASK) = NPE%AL(JP)%TSNOW%ALB  (JI)
  ENDDO
  !
  ...
  !
  ZWS_EP0(:) = ZWS_EP(:)
  CALL OI_HOR_EXTRAPOL_SURF(KI,G%XLAT,G%XLON,ZWS_EP0,G%XLAT,G%XLON,ZWS_EP,GINTERP_NATURE)
  ZWP_EP0(:) = ZWP_EP(:)
  CALL OI_HOR_EXTRAPOL_SURF(KI,G%XLAT,G%XLON,ZWP_EP0,G%XLAT,G%XLON,ZWP_EP,GINTERP_NATURE)
  ZTS_EP0(:) = ZTS_EP(:)
  CALL OI_HOR_EXTRAPOL_SURF(KI,G%XLAT,G%XLON,ZTS_EP0,G%XLAT,G%XLON,ZTS_EP,GINTERP_NATURE,S%XZS)
  ZTP_EP0(:) = ZTP_EP(:)
  CALL OI_HOR_EXTRAPOL_SURF(KI,G%XLAT,G%XLON,ZTP_EP0,G%XLAT,G%XLON,ZTP_EP,GINTERP_NATURE,S%XZS)
  ZTL_EP0(:) = ZTL_EP(:)
  CALL OI_HOR_EXTRAPOL_SURF(KI,G%XLAT,G%XLON,ZTL_EP0,G%XLAT,G%XLON,ZTL_EP,GINTERP_NATURE)
  ZSWE_EP0(:) = ZSWE_EP(:)
  CALL OI_HOR_EXTRAPOL_SURF(KI,G%XLAT,G%XLON,ZSWE_EP0,G%XLAT,G%XLON,ZSWE_EP,GINTERP_SN)
  ZSNR_EP0(:) = ZSNR_EP(:)
  CALL OI_HOR_EXTRAPOL_SURF(KI,G%XLAT,G%XLON,ZSNR_EP0,G%XLAT,G%XLON,ZSNR_EP,GINTERP_SN)
  ZSNA_EP0(:) = ZSNA_EP(:)
  CALL OI_HOR_EXTRAPOL_SURF(KI,G%XLAT,G%XLON,ZSNA_EP0,G%XLAT,G%XLON,ZSNA_EP,GINTERP_SN)  
  !
  ...
  DO JI = 1,NP%AL(JP)%NSIZE_P
    ...
    NPE%AL(JP)%XWG (JI,1) = ZWS_EP(IMASK)
    NPE%AL(JP)%XWG (JI,2) = ZWP_EP(IMASK)
    NPE%AL(JP)%XTG (JI,1) = ZTS_EP(IMASK)
    NPE%AL(JP)%XTG (JI,2) = ZTP_EP(IMASK)
    NPE%AL(JP)%XWGI(JI,2) = ZTL_EP(IMASK)
    NPE%AL(JP)%TSNOW%WSNOW(JI,JL) = ZSWE_EP(IMASK)
    NPE%AL(JP)%TSNOW%RHO  (JI,JL) = ZSNR_EP(IMASK)
    NPE%AL(JP)%TSNOW%ALB  (JI) = ZSNA_EP(IMASK)
  ENDDO
  !
...
IF (LAESNM) THEN
  ...
  WHERE( NPE%AL(JP)%TSNOW%WSNOW(:,JL) < 1.0E-10 ) NPE%AL(JP)%TSNOW%WSNOW(:,JL) = 0.0

  ! No SNOW
  WHERE ( NPE%AL(JP)%TSNOW%WSNOW(:,JL) == 0.0 )
    NPE%AL(JP)%TSNOW%RHO(:,JL) = XUNDEF
    NPE%AL(JP)%TSNOW%ALB(:)    = XUNDEF
  END WHERE
  !
ENDIF
!
!to be improved later - needed for surfex course
 CALL AVERAGE_DIAG_MISC_ISBA_n(DMI, DMIP, IO, NP, NPE)
 !
