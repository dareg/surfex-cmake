     ...
     DO I=1,NSIZE_NATURE
       DO J=1,PATCH_NUMBER
           ...
           IF (TRIM(XVAR(L)) == 'WG1') THEN
             Q(I,J+PATCH_NUMBER*(L-1),J+PATCH_NUMBER*(L-1)) = SCALE_Q*SCALE_Q*XSIGMA(L)*XSIGMA(L)*COFSWI(I)*COFSWI(I)
           ENDIF
           IF (TRIM(XVAR(L)) == 'TG2') Q(I,J+PATCH_NUMBER*(L-1),J+PATCH_NUMBER*(L-1)) = SCALE_Q*SCALE_Q*XSIGMA(L)*XSIGMA(L)
           IF (TRIM(XVAR(L)) == 'TG1') Q(I,J+PATCH_NUMBER*(L-1),J+PATCH_NUMBER*(L-1)) = SCALE_Q*SCALE_Q*XSIGMA(L)*XSIGMA(L)
       ENDDO
     ENDDO
   ENDDO
