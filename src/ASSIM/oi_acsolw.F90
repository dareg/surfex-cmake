...
!
!  
USE MODD_ASSIM, ONLY : NTVGLA, G1WSAT, G2WSAT, EWFC, GWFC, EWWILT, GWWILT, RD1, REPS2
...
DO JROF=KST,KNBPT
  IF ( PWS(JROF) == XUNDEF ) THEN
    ...
    PWFC  (JROF) = 1.0
  ELSE
    ZARG = MAX(REPS2,PARG(JROF))
    PWWILT(JROF) = GWWILT*(ZARG**EWWILT)
    PWFC  (JROF) = GWFC*(ZARG**EWFC)
  ENDIF
ENDDO
!
IF (LDHMT .AND. LHOOK) CALL DR_HOOK('OI_ACSOLW',1,ZHOOK_HANDLE)
IF (LDHMT) RETURN
!
!     ------------------------------------------------------------------
...
!
  ...
  ELSE
    ZSAB = MAX(REPS2,PSAB(JROF))
    PWSAT(JROF) = G1WSAT*ZSAB+G2WSAT
  ENDIF
!
! **  WSMX , WPMX **
!
  PWPMX(JROF) = PWSAT(JROF)*PD2(JROF)*XRHOLW
  PWSMX(JROF) = PWSAT(JROF)*RD1*XRHOLW
!
ENDDO
