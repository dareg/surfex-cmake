...
USE MODD_ASSIM,      ONLY : LAESNM,NECHGU,LAROME,XRSCALDW,NITRAD,NPRINTLEV, &
                            ...
                            XSIGWGB,XRD1,NECHGXFU,LAESIC,LAESIT,LAESST
USE MODD_OL_FILEID
...
USE MODD_IO_SURF_ARO,ONLY : NGPTOT, NGPTOT_CAP, NPROMA, NINDX1, NINDX2, NBLOCK, NKPROMA, &
                                                             YSURFEX_CACHE_OUT,   &
                            SURFEX_FIELD_BUF_PREALLOC, SURFEX_FIELD_BUF_SET_RECORD,SURFEX_FIELD_BUF_EXIST,YSURFEX_CACHE_IN
...
REAL(KIND=JPRB), OPTIONAL, DIMENSION (:,:) ::  P__TCMO
REAL(KIND=JPRB), OPTIONAL, DIMENSION (:,:) ::  P__SCMO
...
REAL, DIMENSION (:,:), ALLOCATABLE :: PWS, PWP, PTS, PTP, PTL, PSNS, PRSMIN, PD2, PLAI, PVEG,    &
                                      PRSN, PASN, PCMO_T, PCMO_S, PCMO_T_O, PCMO_S_O
...
INTEGER :: ILUNAM, INBNIV
LOGICAL :: GFOUND, LCMO, LSPLIT_PATCH
INTEGER :: IPATCH, JP, JL
...
IPATCH=1
#ifdef SFX_ARO
IF (SURFEX_FIELD_BUF_EXIST (YSURFEX_CACHE_IN, 'TG1P1')) THEN
...
! is CMO activated ?
LCMO=.FALSE.
IF (SURFEX_FIELD_BUF_EXIST (YSURFEX_CACHE_IN, 'TEMP_OC1')) LCMO=.TRUE.
! if CMO activated , read fields to be assimilated
IF (LCMO) THEN
  CALL READ_SURF(YPROGRAM,'SEA_NBLEVEL',INBNIV,IRESP)
  ALLOCATE (PCMO_T(ISIZE,INBNIV))
  ALLOCATE (PCMO_S(ISIZE,INBNIV))
  ALLOCATE (PCMO_T_O(ISIZE,INBNIV))
  ALLOCATE (PCMO_S_O(ISIZE,INBNIV))
  DO JL=1,INBNIV
    YVAR='TEMP_OC'
    WRITE(YPAT,'(I2)') JL
    YVAR=TRIM(YVAR)//ADJUSTL(YPAT(:LEN_TRIM(YPAT)))
    CALL READ_SURF(YPROGRAM,YVAR,PCMO_T(:,JL),IRESP)
  ENDDO
  DO JL=1,INBNIV
    YVAR='SALT_OC'
    WRITE(YPAT,'(I2)') JL
    YVAR=TRIM(YVAR)//ADJUSTL(YPAT(:LEN_TRIM(YPAT)))
    CALL READ_SURF(YPROGRAM,YVAR,PCMO_S(:,JL),IRESP)
  ENDDO
ENDIF
...
PCMO_T_O(:,:) = P__TCMO          (1:ISIZE,1:INBNIV)
PCMO_S_O(:,:) = P__SCMO          (1:ISIZE,1:INBNIV)
...
PRINT *,'Mean SN increments   ',SUM(ZSNINC)/YSC%U%NSIZE_NATURE
PRINT *,'           '
...
DO JJ = 1, ISIZE
  IF (YSC%U%XSEA(JJ)>0.0_JPRB) THEN
    IF (LCMO) THEN
      ! restore toward mercator fields
      DO JL=1,INBNIV
        IF (PCMO_T_O(JJ,JL) /= PUNDEF .AND. PCMO_T_O(JJ,JL) > XTT+ZTF) THEN
          PCMO_T(JJ,JL) = (1.0_JPRB-PCLISST)*PCMO_T(JJ,JL)+PCLISST*(PCMO_T_O(JJ,JL)-XTT)
        ENDIF
        PCMO_T(JJ,JL) = MAX(PCMO_T(JJ,JL),ZTF)        
        IF (PCMO_S_O(JJ,JL) /= PUNDEF) PCMO_S(JJ,JL) = (1.0_JPRB-PCLISST)*PCMO_S(JJ,JL)+PCLISST*PCMO_S_O(JJ,JL)
      ENDDO
      ! integrate buoys  
      IF ((PTS_O(JJ) >= XTT+ZTF ) .AND. (PITM(JJ) <= 0.5_JPRB) .AND. PSIC(JJ) <= 0.0_JPRB .AND. YSC%U%XNATURE(jj)==0._JPRB) THEN
        ZTSINC(JJ) = PTS_O(JJ)-PSST(JJ)
      ELSE
        ZTSINC(JJ) = 0.0_JPRB
      ENDIF
      DO JL=1,5
        IF (PCMO_T(JJ,JL) /= PUNDEF .AND. LAESST) THEN
          PCMO_T(JJ,JL) = PCMO_T(JJ,JL)+ZTSINC(JJ)*(1.0_JPRB/(2.0_JPRB**(FLOAT(JL)-1.0_JPRB)))
        ENDIF
      ENDDO
      PSST(JJ) = PCMO_T(JJ,1)+XTT
    ELSE
    ZTSINC(JJ) = PTS_O(JJ) - PSST(JJ)
    IF (PITM(JJ) <= 0.5_JPRB .AND. PSST_O(JJ) /= PUNDEF) THEN
      PSST(JJ) = PTS_O(JJ)
      ENDIF
      IF (PSST_O(JJ) /= PUNDEF) THEN 
  ...
  IF (YSC%U%XWATER(JJ)>0.0_JPRB) THEN
    ! do not use SST product when too close to XTT
    IF (PSST_O(JJ) /= PUNDEF .AND. PSST_O(JJ)>XTT+0.5) THEN ! 0.5 K is an arbitrary threshold here
      PLST(JJ) =PSST_O(JJ)
    ELSE    !corresponds to : PSST_O(JJ) == PUNDEF 
      IF (YSC%U%XNATURE(JJ)>0.0_JPRB) THEN
        PLST(JJ) = PTP(JJ,1)
    ...
    IF (YSC%U%XSEA(JJ)>0.0_JPRB) THEN
      IF (PSIC(JJ)<=1.1 .AND. PSIC(JJ)>=0.) THEN
        ...
        IF(LAESIT .AND. PSIT_O(JJ)<10._JPRB .AND. PSIT_O(JJ)>0._JPRB) PICEHSI(JJ)  &
          &= PCLIICE*PSIT_O(JJ)+(1.0_JPRB-PCLIICE)*PICEHSI(JJ)
...
!write CMO fields if necessary
IF (LCMO) THEN
  DO JL=1,INBNIV
    YVAR='TEMP_OC'
    WRITE(YPAT,'(I2)') JL
    YVAR=TRIM(YVAR)//ADJUSTL(YPAT(:LEN_TRIM(YPAT)))
    CALL DD (YVAR, PCMO_T (:,JL))
    YPREFIX='                          '
    CALL WRITE_SURF(YSC%DUO%CSELECT, YPROGRAM,YVAR,PCMO_T(:,JL),IRESP,HCOMMENT=YPREFIX)
  ENDDO
  DO JL=1,INBNIV
    YVAR='SALT_OC'
    WRITE(YPAT,'(I2)') JL
    YVAR=TRIM(YVAR)//ADJUSTL(YPAT(:LEN_TRIM(YPAT)))
    CALL DD (YVAR, PCMO_S (:,JL))
    YPREFIX='                          '
    CALL WRITE_SURF(YSC%DUO%CSELECT, YPROGRAM,YVAR,PCMO_S(:,JL),IRESP,HCOMMENT=YPREFIX)
  ENDDO
