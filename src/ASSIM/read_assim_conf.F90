...
USE MODD_ASSIM,    ONLY : NVAR,NOBSTYPE,TPRT,TPRT_M,XSIGMA,&
                        & XSIGMA_M,XVAR,XVAR_M,XOBS,INCO,&
                        & NVARMAX,INCV,LASSIM,CASSIM_ISBA,LPRT,&
                        & NOBSMAX
...
IF ( (CASSIM_ISBA == "EKF") .AND. ((LASSIM) .OR. (LPRT))) THEN
  IF (.NOT.ALLOCATED(TPRT))   ALLOCATE (TPRT(NVAR))
  ...
  IF (SUM(INCO) /= NOBSTYPE) THEN
    WRITE(*,*) 'INCONSISTENCY in set-up of OBSERVATIONS',SUM(INCO),NOBSTYPE
    CALL ABOR1_SFX('INCONSISTENCY in set-up ofOBSERVATIONS')
  ENDIF

  IF (.NOT.ALLOCATED(XOBS)) ALLOCATE (XOBS(NOBSTYPE))
  J = 1
  DO I = 1,NOBSMAX
    IF (INCO(I) == 1 .AND. J <= NOBSTYPE ) THEN
      SELECT CASE (I)
        CASE (1)   
          XOBS(J) = 'T2M'
        CASE (2)
