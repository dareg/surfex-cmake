...
CALL TEST_NAM_VAR_SURF(ILUOUT,'CASSIM_SEA',CASSIM_SEA,'INPUT','NONE ')
CALL TEST_NAM_VAR_SURF(ILUOUT,'CASSIM_WATER',CASSIM_WATER,'INPUT','NONE')
CALL TEST_NAM_VAR_SURF(ILUOUT,'CASSIM_ISBA',CASSIM_ISBA,'OI   ','EKF  ','ENKF ','NONE ')
CALL TEST_NAM_VAR_SURF(ILUOUT,'CASSIM_TEB',CASSIM_TEB,'ROADT','NONE ')
!
!* close namelist file
...
IF ( ( CASSIM_ISBA == "EKF" .AND. ( LASSIM.OR.LPRT ) ) .OR. &
  ...
  IF (SUM(NNCO) /= NOBSTYPE) THEN
    ...
    CALL ABOR1_SFX('INCONSISTENCY in set-up of OBSERVATIONS')
  ENDIF
  !
