...
!
 CALL READ_SURF(YPROGRAM,'SEA   ',U%CSEA   ,IRESP)
 CALL READ_SURF(YPROGRAM,'WATER ',U%CWATER ,IRESP)
 CALL READ_SURF(YPROGRAM,'NATURE',U%CNATURE,IRESP)
 CALL READ_SURF(YPROGRAM,'TOWN  ',U%CTOWN  ,IRESP)

!
 CALL READ_SURF(YPROGRAM,'DIM_FULL  ',U%NDIM_FULL,  IRESP)
 CALL READ_SURF(YPROGRAM,'DIM_SEA   ',U%NDIM_SEA,   IRESP)
 CALL READ_SURF(YPROGRAM,'DIM_NATURE',U%NDIM_NATURE,IRESP)
 CALL READ_SURF(YPROGRAM,'DIM_WATER ',U%NDIM_WATER, IRESP)
 CALL READ_SURF(YPROGRAM,'DIM_TOWN  ',U%NDIM_TOWN,  IRESP)
...
!
 CALL GET_SIZE_FULL_n(YPROGRAM,U%NDIM_FULL,U%NSIZE_FULL)
 CALL READ_COVER_n(YPROGRAM)
 CALL END_IO_SURF_n(YPROGRAM)
!
!   Perform masks (only nature used)
!
 ALLOCATE(U%XSEA(U%NDIM_FULL))
 ALLOCATE(U%XNATURE(U%NDIM_FULL))
 ALLOCATE(U%XWATER(U%NDIM_FULL))
 ALLOCATE(U%XTOWN(U%NDIM_FULL))
!
...
!
 ALLOCATE(ZSAND(U%NSIZE_NATURE))
 ALLOCATE(ZCLAY(U%NSIZE_NATURE))
!
...
!
 ...
 ENDDO
!
 ALLOCATE(XI(U%NSIZE_NATURE,PATCH_NUMBER,NVAR))
 ALLOCATE(VECT(U%NSIZE_NATURE,PATCH_NUMBER))
 ALLOCATE(LAI_PASS(U%NSIZE_NATURE,PATCH_NUMBER))
 ALLOCATE(ZBIO_PASS(U%NSIZE_NATURE,PATCH_NUMBER))
 ALLOCATE(ZBIO_OUT(U%NSIZE_NATURE,PATCH_NUMBER))
!
...
!
ALLOCATE(XPATCH(U%NSIZE_NATURE,PATCH_NUMBER))
ALLOCATE(LPATCH(U%NSIZE_NATURE,PATCH_NUMBER))
!
...
!
   ...
   ENDDO
!
 ALLOCATE(COFSWI(U%NSIZE_NATURE))
 ALLOCATE(WILT(U%NSIZE_NATURE))
...
!
   ...
   DO K = 1,NOBSTYPE
     ...
     IF( XOBS(K) == 'SWI2' ) THEN
       ...
       ALLOCATE(MWG2(U%NSIZE_NATURE,PATCH_NUMBER))
...
!
   ...
   ALLOCATE(LTM(U%NSIZE_NATURE,PATCH_NUMBER*NVAR,PATCH_NUMBER*NVAR))
   ALLOCATE(ZEPS(U%NSIZE_NATURE,PATCH_NUMBER,NVAR))
   ALLOCATE(XF(U%NSIZE_NATURE,PATCH_NUMBER,NVAR+1,NVAR))
...
!
 ALLOCATE(YF(U%NSIZE_NATURE,NVAR+1,NOBS))
 ALLOCATE(YF_PATCH(U%NSIZE_NATURE,PATCH_NUMBER,NVAR+1,NOBS))
 ALLOCATE(ZEPS(U%NSIZE_NATURE,PATCH_NUMBER,NVAR))
 ALLOCATE(XF(U%NSIZE_NATURE,PATCH_NUMBER,NVAR+1,NVAR))
!
! Read fraction of each patch (=> LPGD=T in OPTIONS.nam)
...
!   Observations
!
 ALLOCATE(YO(U%NSIZE_NATURE,NOBS))
 ALLOCATE(YOWR(U%NDIM_FULL,NOBS))
!
!   Temporary vectors used by the EKF approach
!
 ALLOCATE(HO(U%NSIZE_NATURE,NOBS,PATCH_NUMBER*NVAR))
 ALLOCATE(HOWR(U%NSIZE_NATURE,NOBS,PATCH_NUMBER*NVAR))
 ALLOCATE(HOT(U%NSIZE_NATURE,PATCH_NUMBER*NVAR,NOBS))
 ALLOCATE(R(U%NSIZE_NATURE,NOBS,NOBS))
 ALLOCATE(IDENT(PATCH_NUMBER*NVAR,PATCH_NUMBER*NVAR))
 ALLOCATE(GAIN(U%NSIZE_NATURE,PATCH_NUMBER*NVAR,NOBS))
 ALLOCATE(ZB(U%NSIZE_NATURE,NOBS))
 ALLOCATE(ZX(U%NSIZE_NATURE,NOBS))
 ALLOCATE(ZP(U%NSIZE_NATURE,NOBS))
 ALLOCATE(XINCR(U%NSIZE_NATURE,PATCH_NUMBER*NVAR))
 ALLOCATE(K1(U%NSIZE_NATURE,NOBS,NOBS))
 ALLOCATE(KH(U%NSIZE_NATURE,PATCH_NUMBER*NVAR,PATCH_NUMBER*NVAR))
 ALLOCATE(KRK(U%NSIZE_NATURE,PATCH_NUMBER*NVAR,PATCH_NUMBER*NVAR))
 ALLOCATE(IdKH(U%NSIZE_NATURE,PATCH_NUMBER*NVAR,PATCH_NUMBER*NVAR))
 ALLOCATE(FROZ(U%NSIZE_NATURE,PATCH_NUMBER))
!
!   Initialisations
!
