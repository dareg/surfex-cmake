!===================================================================================

SUBROUTINE MY_FORC_SAFRAN(HEXPER,HYEAR,HYEAR2,KNI,KNPTS,           &
                   KMONTH,KDAY,PTIME,                              &
                   PLON, PLAT, PZS, PZREF, PUREF,                  &
                   PTA, PQA, PPS, PWINDSPEED, PWINDDIR,            &
                   PDIR_SW, PSCA_SW, PLW, PRAIN, PSNOW, PCO2       )

!----------------------------
!!
!!    PURPOSE
!!    -------
!!   This subroutine allows the user to build atm. forcing of his(her) run.
!!
!!    AUTHOR
!!    ------
!!
!!    V. Masson and P. Lemoigne                 Meteo-France
!!
!!    MODIFICATION
!!    ------------
!!
!!    Original     06/06
!!
!
!----------------------------------------------------------------------------
!      
!*    0.     Declaration of dummy arguments
!            ------------------------------
!
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK
USE PARKIND1  ,ONLY : JPRB
!
IMPLICIT NONE

!----------------------------------------------------------------------------

CHARACTER(LEN=12), INTENT(IN) :: HEXPER    ! experiment name
CHARACTER(LEN=*), INTENT(IN)  :: HYEAR
CHARACTER(LEN=*), INTENT(IN)  :: HYEAR2
INTEGER, INTENT(IN)          :: KNI       ! number of grid cells
INTEGER, INTENT(IN)          :: KNPTS     ! number of forcing instants
INTEGER, INTENT(OUT)         :: KMONTH    ! month of simulation begining
INTEGER, INTENT(OUT)         :: KDAY      ! day   of simulation begining
REAL,    INTENT(OUT)         :: PTIME     ! time  of simulation begining (s)
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PCO2      ! CO2 concentration (kg/m3) 
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PDIR_SW   ! Solar direct   radiation (W/m2)
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PSCA_SW   ! Solar diffused radiation (W/m2)
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PLW       ! Longwave radiation (W/m2)
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PWINDSPEED! Wind speed (m/s)
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PWINDDIR  ! Wind dir. (deg. from N, clockwise)
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PRAIN     ! rain rate (kg/m2/s)
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PSNOW     ! snow rate (kg/m2/s)
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PTA       ! temperature (K)
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PQA       ! humidity (kg/kg)
REAL*4, DIMENSION(KNPTS,KNI), INTENT(OUT) :: PPS       ! pressure (Pa)
REAL*4, DIMENSION(KNI),       INTENT(OUT) :: PZREF     ! height of temperature forcing (m)
REAL*4, DIMENSION(KNI),       INTENT(OUT) :: PUREF     ! height of wind forcing (m)
REAL*4, DIMENSION(KNI),       INTENT(OUT) :: PZS       ! orography (m)
REAL, DIMENSION(KNI),       INTENT(OUT) :: PLON      ! longitude (degrees)
REAL, DIMENSION(KNI),       INTENT(OUT) :: PLAT      ! latitude  (degrees)
!
!*    1.     Declaration of user local variables
!            -----------------------------------
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
! Input file:
!
CHARACTER(LEN=*), PARAMETER       :: YFILE_DIR= '/sxmosa1/data1/martin/NO_SAVE/SAFRAN/CLIM-SIM/'
!
!----------------------------------------------------------------------------
! Declare variables to translate forcing from obs to surfex atmospheric forcing
!
REAL*4, DIMENSION(KNPTS,KNI) :: ZPS, ZRG
!
INTEGER :: I ! loop counters
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!----------------------------------------------------------------------------
!
!*    2.     Initialization of date (UTC)
!            ------------------------------------
!
IF (LHOOK) CALL DR_HOOK('MY_FORC_SAFRAN',0,ZHOOK_HANDLE)

KDAY    = 01          ! starting day 

KMONTH  = 08          ! starting month

PTIME   =    0.       ! starting time (s)
!
!-----------------------------------------------------------------------------
!      
!      3.    grid definition
!            ---------------
!
PLON(:)   = 1.300
PLAT(:)   = 43.484
!
!----------------------------------------------------------------------------
!      
!        4.    orography definition
!               --------------------
!      
PZS(:)   = 113.
!      
!-----------------------------------------------------------------------------
!      
!      5.    Forcing height
!            --------------
!
PZREF(:)   = 2.
PUREF(:)   = 10.
!
!----------------------------------------------------------------------------
!      
!*      6.   Initialization of forcing variables
!            -----------------------------------
!
!----------------------------------------------------------------------------
!      
!        3.1    reading forcing file
!               --------------------
!
!
print*, 'YFILE_DIR = ', YFILE_DIR,', YEAR = ',HYEAR
OPEN(UNIT=11, FILE=YFILE_DIR//'ForcGLO.DAT_france_'//HYEAR, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=12, FILE=YFILE_DIR//'ForcPRCP.DAT_france_'//HYEAR, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=13, FILE=YFILE_DIR//'ForcQ.DAT_france_'//HYEAR, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=14, FILE=YFILE_DIR//'ForcRAT.DAT_france_'//HYEAR, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=15, FILE=YFILE_DIR//'ForcSNOW.DAT_france_'//HYEAR, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=16, FILE=YFILE_DIR//'ForcT.DAT_france_'//HYEAR, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=17, FILE=YFILE_DIR//'ForcVu.DAT_france_'//HYEAR, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
!
OPEN(UNIT=18,FILE= YFILE_DIR//'ForcPRES.DAT_france', FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
!
DO I=1,KNPTS-1
  READ(11) ZRG(I,:)
  READ(12) PRAIN(I,:)
  READ(13) PQA(I,:)
  READ(14) PLW(I,:)
  READ(15) PSNOW(I,:)
  READ(16) PTA(I,:)
  READ(17) PWINDSPEED(I,:)
ENDDO
!
READ(18) PPS(1,:)
IF (KNI>1) THEN
  DO I=2,KNPTS
    PPS(I,:) = PPS(1,:)
  ENDDO
ENDIF
!
CLOSE(UNIT=11)
CLOSE(UNIT=12)
CLOSE(UNIT=13)
CLOSE(UNIT=14)
CLOSE(UNIT=15)
CLOSE(UNIT=16)
CLOSE(UNIT=17)
!
CLOSE(UNIT=18)
!
OPEN(UNIT=11, FILE=YFILE_DIR//'ForcGLO.DAT_france_'//HYEAR2, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=12, FILE=YFILE_DIR//'ForcPRCP.DAT_france_'//HYEAR2, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=13, FILE=YFILE_DIR//'ForcQ.DAT_france_'//HYEAR2, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=14, FILE=YFILE_DIR//'ForcRAT.DAT_france_'//HYEAR2, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=15, FILE=YFILE_DIR//'ForcSNOW.DAT_france_'//HYEAR2, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=16, FILE=YFILE_DIR//'ForcT.DAT_france_'//HYEAR2, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
OPEN(UNIT=17, FILE=YFILE_DIR//'ForcVu.DAT_france_'//HYEAR2, FORM='UNFORMATTED', ACCESS='SEQUENTIAL')
!
READ(11) ZRG(KNPTS,:)
READ(12) PRAIN(KNPTS,:)
READ(13) PQA(KNPTS,:)
READ(14) PLW(KNPTS,:)
READ(15) PSNOW(KNPTS,:)
READ(16) PTA(KNPTS,:)
READ(17) PWINDSPEED(KNPTS,:)
!
CLOSE(UNIT=11)
CLOSE(UNIT=12)
CLOSE(UNIT=13)
CLOSE(UNIT=14)
CLOSE(UNIT=15)
CLOSE(UNIT=16)
CLOSE(UNIT=17)
!
!!-----------------------------------------------------------------------------
!!
!!        6. Fills Surfex forcing variables
!!           ------------------------------
!!
!PCO2(:,:)    = 0.000620   ! (kg/m3, equivalent to 350 ppm) 
!!
!PDIR_SW(:,:) = 0.8*ZRG(:,:)
!PSCA_SW(:,:) = 0.2*ZRG(:,:)
!!
!PWINDDIR  (:,:) = 0.
!!
IF (LHOOK) CALL DR_HOOK('MY_FORC_SAFRAN',1,ZHOOK_HANDLE)

!----------------------------------------------------------------------------
END SUBROUTINE MY_FORC_SAFRAN
!==============================================================================
