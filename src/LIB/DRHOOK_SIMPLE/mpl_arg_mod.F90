...
IF (N_ARGS == -1) THEN
  ...
  IF (INUMPROC > 1) THEN
    ...
    CALL MPI_BCAST(IARGS,1,INT(MPI_INTEGER),IROOT,ICOMM,IERROR)
    ICOUNT = JP_ARGLEN
    IF (IRANK > 1) ALLOCATE(CL_ARGS(0:IARGS))
    IF (IRANK > 1) CALL PUTARG_INFO(IARGS, TRIM(CL_TERMINATE))
    DO J=0,IARGS
     ! The following broadcast does not use "mailbox" nor attached buffer, both potentially yet to be allocated
      CALL MPI_BCAST(CL_ARGS(J),ICOUNT,INT(MPI_BYTE),IROOT,ICOMM,IERROR)
...
SUBROUTINE MPL_GETARG(KARG, CDARG, KCOMM)
...
FUNCTION MPL_IARGC(KCOMM) RESULT(IRET)
INTEGER(KIND=JPIM),INTENT(IN),OPTIONAL :: KCOMM
INTEGER(KIND=JPIM) :: IRET, ICOMM
IF (PRESENT(KCOMM)) THEN
  ICOMM = KCOMM
ELSE
  ICOMM = INT(MPI_COMM_WORLD)
ENDIF
IF (N_ARGS == -1) CALL INIT_ARGS(ICOMM)
IRET = N_ARGS
