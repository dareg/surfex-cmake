...
!             ------------
!
USE MODD_FMDECLAR, ONLY: JPXKRK, JPXFIE, CNAMFI, JPFINL, JPNXLU
...
IF (LHOOK) CALL DR_HOOK('FM_READ',0,ZHOOK_HANDLE)
...
!ELSE
  IF (KLENG > DATASIZE) THEN
    ...
    GOTO 1000
  END IF
  KFIELD(1:KLENG)=IWORK(KLENCH+3:KLENCH+2+KLENG)
!END IF
!
SELECT CASE (KLENCH)
CASE(-10:-1)
...
IF (LHOOK) CALL DR_HOOK('FM_READ',1,ZHOOK_HANDLE)
