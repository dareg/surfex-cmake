!
!     ##########################
      SUBROUTINE AVG_PATCH_WG(KI,PWG,PWGI,PDG)
!     ##########################
!
!!
!!    PURPOSE
...
!
          ...
          PWG(JJ,1)  = PWG(JJ,1)  + XPATCH(JJ,JPATCH) * XWG(JJ,1,JPATCH)  * XDG (JJ,1,JPATCH) 
          PWG(JJ,2)  = PWG(JJ,2)  + XPATCH(JJ,JPATCH) * XWG(JJ,2,JPATCH)  * XDG (JJ,2,JPATCH) 
          PWG(JJ,3)  = PWG(JJ,3)  + XPATCH(JJ,JPATCH) * XWG(JJ,3,JPATCH)  * ZWORK
          PWGI(JJ,1) = PWGI(JJ,1) + XPATCH(JJ,JPATCH) * XWGI(JJ,1,JPATCH) * XDG (JJ,1,JPATCH) 
          PWGI(JJ,2) = PWGI(JJ,2) + XPATCH(JJ,JPATCH) * XWGI(JJ,2,JPATCH) * XDG (JJ,2,JPATCH) 
          PWGI(JJ,3) = PWGI(JJ,3) + XPATCH(JJ,JPATCH) * XWGI(JJ,3,JPATCH) * ZWORK
          ! 
          PDG(JJ,1) = PDG(JJ,1) + XPATCH(JJ,JPATCH) * XDG(JJ,1,JPATCH)
          PDG(JJ,2) = PDG(JJ,2) + XPATCH(JJ,JPATCH) * XDG (JJ,2,JPATCH)
          PDG(JJ,3) = PDG(JJ,3) + XPATCH(JJ,JPATCH) * XDG (JJ,3,JPATCH)
          !
!          
       ENDIF
     ENDDO
