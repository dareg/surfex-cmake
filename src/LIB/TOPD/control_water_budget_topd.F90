...
USE MODD_ISBA_n,           ONLY : NGROUND_LAYER,  NPATCH, XPATCH
...
REAL,    DIMENSION(SIZE(XPATCH,1)) :: ZWG_CORR, ZAVG_WGM, ZAVG_WG, ZAVG_DG
REAL,    DIMENSION(SIZE(XPATCH,1)) :: ZTOTBV_IN_MESH
LOGICAL, DIMENSION(SIZE(XPATCH,1)) :: LMODIF
!
...
!
          ZAVG_WGM(JJ)  = ZAVG_WGM(JJ)  + XPATCH(JJ,JPATCH) * PWGM(JJ,JPATCH)  * PDG (JJ,JPATCH) 
          ZAVG_WG(JJ)  = ZAVG_WG(JJ)  + XPATCH(JJ,JPATCH) * PWG(JJ,JPATCH)  * PDG (JJ,JPATCH) 
          ZAVG_DG(JJ) = ZAVG_DG(JJ) + XPATCH(JJ,JPATCH) * PDG (JJ,JPATCH)
!          
       ENDIF
     ENDDO
  ENDDO     
...
!
 ...
 CALL PACK_SAME_RANK(NR_NATURE,XTOTBV_IN_MESH,ZTOTBV_IN_MESH)
 IF (ZTMP/=0.) THEN
...
DO JPATCH=1,NPATCH
 WHERE ((PWG(:,JPATCH)/=XUNDEF).AND.(XPATCH(:,JPATCH)>0.)&
         .AND.(XPATCH(:,JPATCH)/=XUNDEF).AND.(ZTOTBV_IN_MESH(:)/=0.0))
