...
REAL,    DIMENSION(SIZE(I%IP%XPATCH,1)) :: ZWG_CORR, ZAVG_WGM, ZAVG_WG, ZAVG_DG
REAL,    DIMENSION(SIZE(I%IP%XPATCH,1)) :: ZTOTBV_IN_MESH
LOGICAL, DIMENSION(SIZE(I%IP%XPATCH,1)) :: LMODIF
!
...
!
          ZAVG_WGM(JJ)  = ZAVG_WGM(JJ)  + I%IP%XPATCH(JJ,JPATCH) * PWGM(JJ,JPATCH)  * PDG (JJ,JPATCH) 
          ZAVG_WG(JJ)  = ZAVG_WG(JJ)  + I%IP%XPATCH(JJ,JPATCH) * PWG(JJ,JPATCH)  * PDG (JJ,JPATCH) 
          ZAVG_DG(JJ) = ZAVG_DG(JJ) + I%IP%XPATCH(JJ,JPATCH) * PDG (JJ,JPATCH)
!          
       ENDIF
     ENDDO
  ENDDO     
...
DO JPATCH=1,I%O%NPATCH
 WHERE ((PWG(:,JPATCH)/=XUNDEF).AND.(I%IP%XPATCH(:,JPATCH)>0.)&
         .AND.(I%IP%XPATCH(:,JPATCH)/=XUNDEF).AND.(ZTOTBV_IN_MESH(:)/=0.0))
