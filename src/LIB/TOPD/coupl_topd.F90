!-----------------------------------------------------------------
!     #####################
      SUBROUTINE COUPL_TOPD (DGEIC, DGIC, DGMI, PMESH_SIZE, IO, IP, INI, IMX, IR, &
...
IR%XWG(:,:,:)     = MAX(IR%XWG(:,:,:),XWGMIN)
ZWG_TMP(:,:,:) = IR%XWG(:,:,:)
IF (.NOT.ALLOCATED(XWSUPSAT)) ALLOCATE(XWSUPSAT(KI))
!
DO JJ=1,U%NSIZE_NATURE
 INB_ACTIVE_PATCH(JJ)=COUNT(IP%XPATCH(JJ,:)/=0.)
ENDDO
!               ---------------
IF (IO%CISBA=='DIF') THEN
 CALL DG_DFTO3L(IO, IMX, IP, SIZE(IR%XWG,1), ZDG_3L)
...
ELSEIF (IO%CISBA=='3-L') THEN
 CALL AVG_PATCH_WG(IMX, IP, IR, SIZE(IR%XWG,1), ZWG_3L, ZWGI_3L, ZDG_3L)
ENDIF
!
...
!
IF (KSTEP==1) THEN 
  IF (LBUDGET_TOPD) CALL INIT_BUDGET_COUPL_ROUT(DGEIC, DGIC, DGMI, PMESH_SIZE, &
                                                IO, IP, IMX, IR, U, KI)
...
!               -----------------------------------------
!
CALL TOPD_TO_ISBA(IP, UG, U, KI,KSTEP,GTOPD)!=modif of XWG_FULL from XWTOPT
...
!
IF (IO%CISBA=='DIF') THEN
 CALL TOPD_TO_DF(IO, IP, IMX, IR, SIZE(ZWG_3L,1), ZWG_3L)
ELSEIF (IO%CISBA=='3-L') THEN
 CALL DISPATCH_WG(IP, IMX, IR, SIZE(ZWG_3L,1), ZWG_3L, ZWGI_3L, ZDG_3L)
ENDIF
!
WHERE(IR%XWG(:,:,:)>ZWSATMAX.AND.IR%XWG(:,:,:)/=XUNDEF)
 IR%XWG(:,:,:)=ZWSATMAX
ENDWHERE
WHERE(IR%XWG(:,:,:)<XWGMIN)
 IR%XWG(:,:,:)=XWGMIN
...
DO JLAYER=2,IO%NGROUND_LAYER
 IF (ALL(IR%XWG(:,JLAYER,:)==XUNDEF))THEN
...
CALL CONTROL_WATER_BUDGET_TOPD(IO, IP, U, ZWG_TMP(:,2,:), IR%XWG(:,2,:),IMX%XDG(:,2,:),&
                               ZMESH_SIZE,ZAVG_MESH_SIZE,ZWSAT(:))
DO JLAYER = 3,IACT_GROUND_LAYER
 CALL CONTROL_WATER_BUDGET_TOPD(IO, IP, U, ZWG_TMP(:,JLAYER,:), IR%XWG(:,JLAYER,:),&
                                IMX%XDG(:,JLAYER,:)-IMX%XDG(:,JLAYER-1,:),&
                                ZMESH_SIZE,ZAVG_MESH_SIZE,ZWSAT(:))
ENDDO
!
WHERE(IR%XWG(:,:,:)>ZWSATMAX.AND.IR%XWG(:,:,:)/=XUNDEF)
...
CALL UNPACK_SAME_RANK(U%NR_NATURE,DGEIC%XRUNOFF,ZRUNOFFC_FULL)
...
CALL UNPACK_SAME_RANK(U%NR_NATURE,DGEIC%XDRAIN*XATOP,ZDRAINC_FULL)
