!-----------------------------------------------------------------
!     #####################
      SUBROUTINE COUPL_TOPD (DGEIC, DGIC, DGMI, IG, I, UG, U, &
...
REAL, DIMENSION(U%NSIZE_NATURE,I%O%NGROUND_LAYER,I%O%NPATCH) :: ZWG_TMP
...
I%R%XWG(:,:,:)     = MAX(I%R%XWG(:,:,:),XWGMIN)
ZWG_TMP(:,:,:) = I%R%XWG(:,:,:)
IF (.NOT.ALLOCATED(XWSUPSAT)) ALLOCATE(XWSUPSAT(KI))
!
DO JJ=1,U%NSIZE_NATURE
 INB_ACTIVE_PATCH(JJ)=COUNT(I%IP%XPATCH(JJ,:)/=0.)
ENDDO
!               ---------------
...
!
IF (KSTEP==1) THEN 
  IF (LBUDGET_TOPD) CALL INIT_BUDGET_COUPL_ROUT(DGEIC, DGIC, DGMI, IG, I, U, &
                                                KI)
...
!
!actualisation M
IF( I%O%CKSAT=='EXP' .OR. I%O%CKSAT=='SGH' ) THEN
  !ludo test
  XF_PARAM(:) = I%I%XF_PARAM(:)
...
!               -----------------------------------------
!
CALL TOPD_TO_ISBA(I, UG, U, &
                  KI,KSTEP,GTOPD)!=modif of XWG_FULL from XWTOPT
...
IF (I%O%CISBA=='DIF') THEN
 CALL TOPD_TO_DF(I, &
                 SIZE(ZWG_3L,1),ZWG_3L)
ELSEIF (I%O%CISBA=='3-L') THEN
 CALL DISPATCH_WG(I, &
                  SIZE(ZWG_3L,1),ZWG_3L,ZWGI_3L,ZDG_3L)
ENDIF
!
WHERE(I%R%XWG(:,:,:)>ZWSATMAX.AND.I%R%XWG(:,:,:)/=XUNDEF)
 I%R%XWG(:,:,:)=ZWSATMAX
ENDWHERE
WHERE(I%R%XWG(:,:,:)<XWGMIN)
 I%R%XWG(:,:,:)=XWGMIN
ENDWHERE
!
IACT_GROUND_LAYER=3
...
DO JLAYER=2,I%O%NGROUND_LAYER
 IF (ALL(I%R%XWG(:,JLAYER,:)==XUNDEF))THEN
...
CALL CONTROL_WATER_BUDGET_TOPD(I, U, &
                               ZWG_TMP(:,2,:), I%R%XWG(:,2,:),I%M%X%XDG(:,2,:),&
           ZMESH_SIZE,ZAVG_MESH_SIZE,ZWSAT(:))
DO JLAYER = 3,IACT_GROUND_LAYER
 CALL CONTROL_WATER_BUDGET_TOPD(I, U, &
                               ZWG_TMP(:,JLAYER,:), I%R%XWG(:,JLAYER,:),&
              I%M%X%XDG(:,JLAYER,:)-I%M%X%XDG(:,JLAYER-1,:),&
              ZMESH_SIZE,ZAVG_MESH_SIZE,ZWSAT(:))
ENDDO
!
WHERE(I%R%XWG(:,:,:)>ZWSATMAX.AND.I%R%XWG(:,:,:)/=XUNDEF)
...
!
CALL DIAG_ISBA_TO_ROUT(UG%G%XMESH_SIZE, &
