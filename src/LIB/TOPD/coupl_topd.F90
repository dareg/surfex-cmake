...
REAL, DIMENSION(U%NSIZE_NATURE,I%NGROUND_LAYER,I%NPATCH) :: ZWG_TMP
...
!
!actualisation M
IF( I%CKSAT=='EXP' .OR. I%CKSAT=='SGH' ) THEN
  !ludo test
  XF_PARAM(:) = I%XF_PARAM(:)
...
ENDIF
!
WHERE(I%XWG(:,:,:)>ZWSATMAX.AND.I%XWG(:,:,:)/=XUNDEF)
 I%XWG(:,:,:)=ZWSATMAX
ENDWHERE
WHERE(I%XWG(:,:,:)<XWGMIN)
 I%XWG(:,:,:)=XWGMIN
...
DO JLAYER=2,I%NGROUND_LAYER
 IF (ALL(I%XWG(:,JLAYER,:)==XUNDEF))THEN
...
CALL CONTROL_WATER_BUDGET_TOPD(I, U, &
                               ZWG_TMP(:,2,:), I%XWG(:,2,:),I%XDG(:,2,:),&
           ZMESH_SIZE,ZAVG_MESH_SIZE,ZWSAT(:))
DO JLAYER = 3,IACT_GROUND_LAYER
 CALL CONTROL_WATER_BUDGET_TOPD(I, U, &
                               ZWG_TMP(:,JLAYER,:), I%XWG(:,JLAYER,:),&
              I%XDG(:,JLAYER,:)-I%XDG(:,JLAYER-1,:),&
              ZMESH_SIZE,ZAVG_MESH_SIZE,ZWSAT(:))
ENDDO
!
WHERE(I%XWG(:,:,:)>ZWSATMAX.AND.I%XWG(:,:,:)/=XUNDEF)
