...
USE MODD_COUPLING_TOPD,  ONLY   : XWG_FULL, XDTOPI, XKAC_PRE, XDTOPT, XWTOPT, XWSTOPT, XAS_NATURE,&
                                  ...
                                  !
USE MODD_ISBA_n,           ONLY : NGROUND_LAYER, XWG, XDG, XWGI ,CISBA,&
                                  XF_PARAM_i=>XF_PARAM, CKSAT, NPATCH, XPATCH,&
                                  XDG, XDZG, XDG2, XWSAT,NWG_LAYER ,NSIZE_NATURE_P

USE MODD_CSTS,             ONLY : XRHOLW, XRHOLI
USE MODD_DIAG_EVAP_ISBA_n, ONLY : XAVG_RUNOFFC, XAVG_DRAINC,XAVG_DWG,XAVG_DWGI
...
!
REAL, DIMENSION(NSIZE_NATURE,3) :: ZWG_3L,ZWGI_3L,ZDG_3L          
 REAL, DIMENSION(NSIZE_NATURE)  :: ZMESH_SIZE, ZWSAT         
REAL, DIMENSION(NSIZE_NATURE,NGROUND_LAYER,NPATCH) :: ZWG_TMP
REAL, DIMENSION(KI)            :: ZF_PARAM_FULL
REAL, DIMENSION(NNCAT,NMESHT)  :: ZF_PARAMT
!
! Taking several patches into account
INTEGER, DIMENSION(NSIZE_NATURE)         :: INB_ACTIVE_PATCH
REAL, DIMENSION(NSIZE_NATURE):: ZSUMFRD2, ZSUMFRD3
REAL, DIMENSION(NSIZE_NATURE,3) :: ZWG_CTL
 !
...
DO JJ=1,NSIZE_NATURE
 INB_ACTIVE_PATCH(JJ)=COUNT(XPATCH(JJ,:)/=0.)
ENDDO
!               ---------------
IF (CISBA=='DIF') THEN
 ...
 ZWG_3L(:,2)=XFRD2_TWG(:)
 ZWG_3L(:,3)=XFRD3_TWG(:)
 ZWGI_3L(:,2)=XFRD2_TWGI(:)
 ZWGI_3L(:,3)=XFRD3_TWGI(:)
...
END WHERE
!
CALL UNPACK_SAME_RANK(NR_NATURE,ZWG_3L(:,2),ZWG2_FULL)
CALL UNPACK_SAME_RANK(NR_NATURE,ZWG_3L(:,3),ZWG3_FULL)
!
...
CALL UNPACK_SAME_RANK(NR_NATURE,ZWGI_3L(:,2),ZWGI_FULL)
...
END WHERE
!
CALL UNPACK_SAME_RANK(NR_NATURE,XAVG_DWGI(:),ZRI_WGI)
!
WHERE ( XDTOPI==XUNDEF ) 
  ZRI_WGI = 0.0
END WHERE
...
!
CALL UNPACK_SAME_RANK(NR_NATURE,XAVG_DWG(:),ZRI)
WHERE ( XDTOPI/=XUNDEF )
...
!               -----------------------------------------
!
CALL TOPD_TO_ISBA(KI,KSTEP,GTOPD)!=modif of XWG_FULL from XWTOPT
!
CALL PACK_SAME_RANK(NR_NATURE, (1-XFRAC_D2)*ZWG2_FULL + XFRAC_D2*XWG_FULL, ZWG_3L(:,2))
CALL PACK_SAME_RANK(NR_NATURE, (1-XFRAC_D3)*ZWG3_FULL + XFRAC_D3*XWG_FULL, ZWG_3L(:,3))
!
!*       4.2    Budget correction
!  -----------------------------------------
!
 CALL PACK_SAME_RANK(NR_NATURE,XMESH_SIZE,ZMESH_SIZE)
 ZAVG_MESH_SIZE = SUM(ZMESH_SIZE(:),MASK=ZMESH_SIZE(:)/=XUNDEF) / COUNT(ZMESH_SIZE(:)/=XUNDEF)
!
IF (CISBA=='DIF') THEN
 CALL TOPD_TO_DF(SIZE(ZWG_3L,1),ZWG_3L)
ELSEIF (CISBA=='3-L') THEN
 CALL DISPATCH_WG(SIZE(ZWG_3L,1),ZWG_3L,ZWGI_3L,ZDG_3L)
ENDIF
!
WHERE(XWG(:,:,:)>ZWSATMAX.AND.XWG(:,:,:)/=XUNDEF)
 XWG(:,:,:)=ZWSATMAX
ENDWHERE
WHERE(XWG(:,:,:)<XWGMIN)
 XWG(:,:,:)=XWGMIN
...
DO JLAYER=2,NGROUND_LAYER
 IF (ALL(XWG(:,JLAYER,:)==XUNDEF))THEN
         IACT_GROUND_LAYER=JLAYER-1
...
ENDIF
!
 CALL PACK_SAME_RANK(NR_NATURE,Z_WSTOPI,ZWSAT)
CALL CONTROL_WATER_BUDGET_TOPD(ZWG_TMP(:,2,:), XWG(:,2,:),XDG(:,2,:),&
           ZMESH_SIZE,ZAVG_MESH_SIZE,ZWSAT(:))
DO JLAYER = 3,IACT_GROUND_LAYER
 CALL CONTROL_WATER_BUDGET_TOPD(ZWG_TMP(:,JLAYER,:), XWG(:,JLAYER,:),&
              XDG(:,JLAYER,:)-XDG(:,JLAYER-1,:),&
              ZMESH_SIZE,ZAVG_MESH_SIZE,ZWSAT(:))
ENDDO
!
WHERE(XWG(:,:,:)>ZWSATMAX.AND.XWG(:,:,:)/=XUNDEF)
