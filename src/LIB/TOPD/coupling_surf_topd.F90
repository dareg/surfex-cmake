!###################################################################
SUBROUTINE COUPLING_SURF_TOPD (DGEI, DGEIC, DGIC, DGMI, IG, I, UG, U, &
                                HPROGRAM,KI)
!###################################################################
!
!!****  *COUPLING_SURF_TOPD*  
...
IF ( LTOPD_STEP ) THEN
  ...
  IF (I%O%CRUNOFF=='TOPD') THEN
    CALL COUPL_TOPD(DGEIC, DGIC, DGMI, IG, I, UG, U, &
                    HPROGRAM,YSTEP,KI,NTOPD_STEP)
  ELSE
    CALL ROUT_DATA_ISBA(DGEIC, DGIC, DGMI, IG, I, UG, U, &
                        HPROGRAM,KI,NTOPD_STEP)
  ENDIF
  !
  IF (LBUDGET_TOPD) CALL BUDGET_COUPL_ROUT(DGEI, DGEIC, DGIC, DGMI, I, U, &
                                           KI,NTOPD_STEP)
  !
