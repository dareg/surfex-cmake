...
!*      0.2    declarations of local variables
 ...
 REAL, DIMENSION(SIZE(I%IP%XPATCH,1),SIZE(I%IP%XPATCH,2)) :: ZFRAC_PATCH2
 REAL, DIMENSION(SIZE(I%IP%XPATCH,1),SIZE(I%IP%XPATCH,2)) :: ZFRAC_PATCH3
...
IF (INP/=1)THEN
 DO JPATCH=1,INP
   DO JJ=1,INI
    IF ((I%IP%XPATCH(JJ,JPATCH)/=XUNDEF).AND.(I%IP%XPATCH(JJ,JPATCH)/=0.)&
        .AND.(XATOP(JJ)==1.)) THEN
     WHERE (I%R%XWG(JJ,:,JPATCH)/=XUNDEF)
      I%R%XWG(JJ,:,JPATCH) = PWG(JJ,:) 
      I%R%XWGI(JJ,:,JPATCH)= PWGI(JJ,:) 
      I%M%X%XDG(JJ,:,JPATCH) = PDG (JJ,:)
     ENDWHERE
    ENDIF
   ENDDO
 ENDDO
