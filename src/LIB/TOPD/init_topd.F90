...
!               -----------------------------
DO JCAT=1,NNCAT
  CALL READ_TOPD_HEADER_DTM("ASCII ",YFILETOP(JCAT),'FORMATTED',&
...
DO JCAT = 1,NNCAT
  !
  CALL READ_TOPD_FILE("ASCII ",YFILETOP(JCAT),'FORMATTED',NNPT(JCAT),ZTOPD_READ)
  DO JJ = 1,NNPT(JCAT)
...
!
DO JCAT=1,NNCAT
  CALL READ_TOPD_HEADER_CONNEX("ASCII ",YFILECON(JCAT),'FORMATTED',NNMC(JCAT))
ENDDO
!
NMESHT=MAXVAL(NNMC(:))
!
...
IF (HPROGRAM/='PGD') THEN
  ...
  !
  DO JCAT=1,NNCAT
    CALL READ_SLOPE_FILE("ASCII ",YFILESLO(JCAT),'FORMATTED',NNMC(JCAT),&
  ...
  !
  DO JCAT=1,NNCAT
    CALL  READ_TOPD_FILE("ASCII ",YFILEDR(JCAT),'FORMATTED',NNPT(JCAT),ZTOPD_READ)
    DO JJ=1,NPMAX
  ...
  !
  DO JCAT=1,NNCAT
    CALL  READ_TOPD_FILE("ASCII ",YFILEDH(JCAT),'FORMATTED',NNPT(JCAT),ZTOPD_READ)
    DO JJ=1,NPMAX
      IF ( NLINE(JCAT,JJ)/=0. .AND. NLINE(JCAT,JJ)/=XUNDEF ) &
