...
DO JJ=1,NNCAT
  DO JI=1,NNMC(JJ)
    IF (NMASKT(JJ,JI)/=NUNDEF) ZRUNOFF_TOPD(JJ,JI) = ZRUNOFF_TOPD(JJ,JI) / NNPIX(NMASKT(JJ,JI))
  ENDDO
ENDDO
!
!    Drainage treatment
...
DO JJ=1,NNCAT
  DO JI=1,NNMC(JJ)
    IF (NMASKT(JJ,JI)/=NUNDEF) ZDRAIN_TOPD(JJ,JI) = ZDRAIN_TOPD(JJ,JI) / NNPIX(NMASKT(JJ,JI))
  ENDDO
ENDDO
