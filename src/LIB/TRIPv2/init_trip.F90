...
ENDIF
!
IF(MOD(PTSTEP_DIAG,PTSTEP_RUN)*MOD(PTSTEP_RUN,PTSTEP_DIAG)/=0.)THEN
  WRITE(NLISTING,*)'! PTSTEP_DIAG and XTSTEP_RUN are not good !!!'  
  WRITE(NLISTING,*)'! XTSTEP_DIAG =', PTSTEP_DIAG    
  WRITE(NLISTING,*)'! XTSTEP_RUN  =', PTSTEP_RUN 
  CALL ABORT_TRIP('INIT_TRIP: PTSTEP_DIAG and XTSTEP_RUN are not good !!!')
ENDIF
!
...
!  
  ...
  WHERE(TPG%NSEQ(:,:)==0.OR.TP%XWIDTH(:,:)>=XUNDEF-1.0)
        TP%XSLOPEBED(:,:)= 0.0      
        TP%XWIDTH   (:,:)= 0.0      
        TP%XN       (:,:)= 0.0
        TPG%GMASK_VEL(:,:)=.FALSE.
