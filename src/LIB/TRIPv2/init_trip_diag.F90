...
IF(CGROUNDW=='CST') INDIAG = INDIAG + 1
IF(CGROUNDW/='DEF') INDIAG = INDIAG + 1
IF(CGROUNDW=='DIF')THEN 
  INDIAG = INDIAG + 3
  IF(LDIAG_MISC)INDIAG = INDIAG + 4
ENDIF
IF(LFLOOD)THEN
  INDIAG = INDIAG + 3
...
!
    INUM = INUM + 1
    YVNAME (INUM) = 'WTDELEV                   '
    YVLNAME(INUM) = 'WTD to mean grid elev     '
    YUNIT  (INUM) = 'm                         '
!
    INUM = INUM + 1
    YVNAME (INUM) = 'WTDRIV                    '
    YVLNAME(INUM) = 'WTD to river elev         '
    YUNIT  (INUM) = 'm                         '
!
    INUM = INUM + 1
    YVNAME (INUM) = 'QGCELL                    ' 
...
!  
  IF(LDIAG_MISC)THEN
    ALLOCATE(TPDIAG%XWTDRIV (KLON,KLAT))
    ALLOCATE(TPDIAG%XWTDELEV(KLON,KLAT))
    ...
    TPDIAG%XHGHS   (:,:) = 0.0
  ELSE
    ALLOCATE(TPDIAG%XWTDRIV (0,0))
    ALLOCATE(TPDIAG%XWTDELEV(0,0))
