...
!
WHERE(TGRID%GMASK(:,:).AND..NOT.TGRID%GMASK_GRE(:,:).AND..NOT.TGRID%GMASK_ANT(:,:))
...
IF(LFLOOD)THEN 
  ...
  WHERE(TTRIP%XFFLOOD(:,:)==1.0.AND.ZSRC_FLOOD(:,:)>0.0)
...
IF(CGROUNDW=='DIF')THEN
  CALL GWF(KLON,KLAT,OPRINT,PTSTEP_RUN,XTSTEP,          &
           TGRID%GMASK_GW,TTRIP%XNUM_AQUI,ZDRAIN,       &
           TGRID%XLEN,TTRIP%XWIDTH,TTRIP%XHC_BED,       &
           TTRIP%XTOPO_RIV,TTRIP%XTAUG,TGRID%XAREA,     &
           TTRIP%XELEV,TTRIP%XTRANS,TTRIP%XWEFF,        &
           TTRIP%XTABGW_F,TTRIP%XTABGW_H,               &
           TTRIP%XSURF_STO,TTRIP%XHGROUND,ZHG_OLD,      &
...
!
  CALL TRIP(KLISTING,CGROUNDW,LFLOOD,OPRINT,XTSTEP,               &
            TGRID%NGRCN,TGRID%NSEQ,TGRID%NNEXTX,TGRID%NNEXTY,     &
            TGRID%NSEQMAX,TGRID%XAREA,TGRID%XLEN,                 &
            TGRID%GMASK_GW,TGRID%GMASK_VEL,TGRID%GMASK_FLD,       &
            TTRIP%XTAUG,TTRIP%XFLOOD_LEN,TTRIP%XSLOPEBED,         &
            TTRIP%XWIDTH,TTRIP%XN,TTRIP%XN_FLOOD,TTRIP%XHC_BED,   &
            TTRIP%XWFLOOD,TTRIP%XTAB_F,TTRIP%XTAB_H,              &
            TTRIP%XTAB_VF,ZDRAIN,ZRUNOFF,ZSRC_FLOOD,              &
            TTRIP%XGROUND_STO,TTRIP%XSURF_STO,TTRIP%XFLOOD_STO,   &
            ZSOUT,ZGOUT,ZHS,TTRIP%XHFLOOD,ZVEL,TTRIP%XFFLOOD,     &
            ZQFR,ZQRF,ZVFIN,ZVFOUT,ZHSF,ZSIN,KNB_TSTEP_RUN,       &
            JTSTEP,ITSTEP,ZGSTO_ALL,ZGSTO2_ALL,ZGIN_ALL,ZGOUT_ALL,&
            TTRIP%XHGROUND,TTRIP%XWEFF)
!
...
!
   ...
   ENDIF
!
   CALL TRIP_DIAG(XTSTEP,ZSOUT,ZSIN,ZVEL,ZHS,ZGOUT,ZGNEG,    &
                  ZWTD,ZFWTD,ZWTDRIV,ZWTDELEV,ZQGCELL,ZHGHS, &
                  ZQFR,ZQRF,ZVFIN,ZVFOUT,ZHSF,ZSRC_FLOOD,    &
...
!
   IF (MOD(PTIME,PTSTEP_DIAG) == 0. ) THEN
      ...
      CALL TRIP_DIAG_WRITE(KLISTING,KLON,KLAT,KNB_TSTEP_DIAG,PTSTEP_DIAG)
   ENDIF
!
!  * end 
!
...
!               --------------------------------------
!
CALL TRIP_DIAG_CPL_ESM(PTSTEP_RUN,ZDISCHARGE,PCALVING,ZWTD,ZFWTD)
!
