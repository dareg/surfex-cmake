...
!               ---------------------------
!
! TRIP is for now a monoprocess model: the entire field will be exchanged by the process. 
!
...
!
  IF(LCPL_FLOOD)THEN
!
!   Iutput Flood precip interception
    CALL OASIS_DEF_VAR(NPFLOOD_ID,CPFLOOD,IPART_ID,IVAR_NODIMS,OASIS_IN,IVAR_SHAPE,OASIS_DOUBLE,IERR)  
    IF(IERR/=OASIS_OK) CALL ABORT_TRIP('TRIP_OASIS_DEFINE: OASIS def var problem for Flood precip interception')
!
!   Iutput Flood evaporation
    CALL OASIS_DEF_VAR(NEFLOOD_ID,CEFLOOD,IPART_ID,IVAR_NODIMS,OASIS_IN,IVAR_SHAPE,OASIS_DOUBLE,IERR)  
    IF(IERR/=OASIS_OK) CALL ABORT_TRIP('TRIP_OASIS_DEFINE: OASIS def var problem for Flood evaporation')
!
!   Output Flood infiltration
    CALL OASIS_DEF_VAR(NIFLOOD_ID,CIFLOOD,IPART_ID,IVAR_NODIMS,OASIS_IN,IVAR_SHAPE,OASIS_DOUBLE,IERR)  
    IF(IERR/=OASIS_OK) CALL ABORT_TRIP('TRIP_OASIS_DEFINE: OASIS def var problem for Flood infiltration')
!          
