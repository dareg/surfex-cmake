...
!          
    ...
    CALL OASIS_PUT(NWTD_ID,IDATE,TTRIP%XCPL_WTD(:,:),IERR)
    CALL CHECK_TRIP_SEND(YCOMMENT)
!          
    YCOMMENT='Grid-cell fraction of WTD to rise'
    CALL OASIS_PUT(NFWTD_ID,IDATE,TTRIP%XCPL_FWTD(:,:),IERR)
    CALL CHECK_TRIP_SEND(YCOMMENT)
!
  ENDIF
!
...
!
    ...
    CALL MASK_TRIP(TTRIP%XCPL_FFLOOD(:,:),ZWRITE(:,:),LMASK(:,:))
    CALL OASIS_PUT(NFFLOOD_ID,IDATE,ZWRITE(:,:),IERR)
    CALL CHECK_TRIP_SEND(YCOMMENT)
!          
    YCOMMENT='Flood potential infiltration'
    CALL MASK_TRIP(TTRIP%XCPL_PIFLOOD(:,:),ZWRITE(:,:),LMASK(:,:))
...
!
  LMASK(:,:) = (TGRID%NGRCN(:,:)==9.OR.TGRID%NGRCN(:,:)==12)
!
  YCOMMENT='Discharge to ocean'
  CALL MASK_TRIP(TTRIP%XCPL_RIVDIS(:,:),ZWRITE(:,:),LMASK(:,:))
...
!
    LMASK(:,:) = TGRID%GMASK_GRE(:,:)
!
    YCOMMENT='Calving flux over greenland'
    CALL MASK_TRIP(TTRIP%XCPL_CALVGRE(:,:),ZWRITE(:,:),LMASK(:,:))
    CALL OASIS_PUT(NCALVGRE_ID,IDATE,ZWRITE(:,:),IERR)
    CALL CHECK_TRIP_SEND(YCOMMENT)
!
    LMASK(:,:) = TGRID%GMASK_ANT(:,:)
!
    YCOMMENT='Calving flux over antarctica'
    CALL MASK_TRIP(TTRIP%XCPL_CALVANT(:,:),ZWRITE(:,:),LMASK(:,:))
