...
!
    ...
    ILDIMEN=IDIMEN
    CALL LFIECR_MT_BB                                             &
&                  (FA%LFI,IREP,KNUMER,FA%CPCADI,ILDIMEN,FA%JPCADI)
    IDIMEN=ILDIMEN
    IF (IREP.NE.0) GOTO 1001
!
    CALL LFIECR_MT_BB                                            &
&                  (FA%LFI,IREP,KNUMER,FA%CPCAFS,ZCHMID,FA%JPCAFS)
    IF (IREP.NE.0) GOTO 1001
!
   IF (.NOT.LLMLAM) THEN
!
       ...
       ILRDPOL=IRDPOL
       CALL LFIECR_MT_BB                                          &
&                     (FA%LFI,IREP,KNUMER,FA%CPCARP,ILRDPOL,ILONGA)
       IRDPOL=ILRDPOL
       IF (IREP.NE.0) GOTO 1001
!
       ILONGA=INPAHE
       CALL LFIECR_MT_BB                                         &
&                     (FA%LFI,IREP,KNUMER,FA%CPCASL,ZSINLA,ILONGA)
       IF (IREP.NE.0) GOTO 1001
!
    ELSE
!
...
!           
    ILONGA=1+(1+INIVER)*2
    CALL LFIECR_MT_BB                                           &
&                   (FA%LFI, IREP,KNUMER,FA%CPCACH,ZHYBRI,ILONGA)
    IF (IREP.NE.0) GOTO 1001
!
    ILPNVER=FA%JPNVER
    CALL LFIECR_MT_BB                                 &
&                   (FA%LFI, IREP,KNUMER,               &
&                   CLNOMA(1:ILNOMC),ILPNVER,1_JPLIKB )
    IF (IREP.NE.0) GOTO 1001
!
    LLRLFI=.FALSE.
    GOTO 300
...
!
CALL LFINFO_MT_BB                                           &
&               (FA%LFI, IREP,KNUMER,FA%CPCAFS,ILONGA,IPOSEX)
!
IF (IREP.NE.0) THEN
  LLRLFI=.TRUE.
  GOTO 1001
...
ILDATEF=IDATEF
CALL LFILEC_MT_BB                                               &
&               (FA%LFI, IREP,KNUMER,FA%CPDATE,ILDATEF,FA%JPLDAT)
IDATEF=ILDATEF
!
IF (IREP.NE.0) THEN
  LLRLFI=.TRUE.
...
CALL NEW_FICHIER (FA%FICHIER(IRANG), FA%JPLDAT, ITRONC, ITYPTR)
LLNEWF = .TRUE.
