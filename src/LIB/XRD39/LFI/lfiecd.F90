...
SUBROUTINE LFIECD_MT_BB                              &
&                     (LFI, KREP, KRANG, KTAB, KLONG,  &
&                      KPOSEC, KRETIN )
...
!     ECRITURE DES DONNEES PROPREMENT DITES, UNE FOIS L'ARTICLE LOGIQUE
...
IF (LHOOK) CALL DR_HOOK('LFIECD_MT',0,ZHOOK_HANDLE)
CLACTI=''
IRETOU=0
!
IF (KRANG.LE.0.OR.KRANG.GT.LFI%JPNXFI.OR.KPOSEC.EQ.0) &
&     THEN
  KREP=-16
  GOTO 1001
ENDIF
...
KREP=0
!**
!     2.  -   PARTIE ECRITURE DES DONNEES .
!-----------------------------------------------------------------------
!*
!     2.1 - UTILISATION DES ARTICLES PHYSIQUES PRESENTS EN MEMOIRE,
!           ET QUE L'ON S'APPRETE A ECRIRE *EN ENTIER*.
!-----------------------------------------------------------------------
!
IPODEB=KPOSEC
...
!
IF (LFI%LMISOP) THEN
 WRITE (UNIT=LFI%NULOUT,FMT=*)'KPOSEC= ',KPOSEC, &
&                            ', IPODEB= ',IPODEB, &
&                            ', IPOFIN= ',IPOFIN
 WRITE (UNIT=LFI%NULOUT,FMT=*)'IARDEB= ',IARDEB, &
&                            ', IARFIN= ',IARFIN, &
&                            ', IDCDEB= ',IDCDEB
 WRITE (UNIT=LFI%NULOUT,FMT=*)'IDCFIN= ',IDCFIN, &
&                            ', ICPLTI= ',ICPLTI, &
&                            ', ICPLTF= ',ICPLTF
 WRITE (UNIT=LFI%NULOUT,FMT=*)'ICPTTN= ',ICPTTN, &
&                            ', ICPTTX= ',ICPTTX
ENDIF
!
...
!
    DO JD=1,ILARPH
    LFI%MTAMPD(IXT(JD,J,KRANG))=KTAB(IDECDE+JD)
    ENDDO
!
    LFI%NLONPD(J,KRANG)=ILARPH
    LFI%LECRPD(J,KRANG)=.TRUE.
...
!           CETTE PAGE EST CONSERVEE... DES FOIS QUE LE PROCHAIN ACCES
!           AU FICHIER SOIT POUR L'ARTICLE LOGIQUE IMMEDIATEMENT DEVANT
!           ( CAS DE BALAYAGE INVERSE DU FICHIER, PAR EXEMPLE ) .
!-----------------------------------------------------------------------
!
IDECDE=0
!
IF (IDCDEB.EQ.0) THEN
  ...
  ITAMLI=LFI%JPNPDF
ELSE
  IPAREC=MIN (ILARPH*IARDEB,IPOFIN)-KPOSEC+1
  ITAMLI=LFI%JPNPDF-1
!
!           L'ARTICLE DE DONNEES A ECRIRE NE COMMENCE PAS AU DEBUT D'UN
...
!
    IF (LFI%NLONPD(INUMPJ,KRANG).LT.IDCDEB                     &
&        .AND.IARDEB.LE.LFI%MDES1D(IXM(LFI%JPAXPD,KRANG))) THEN
!
!           LA PAGE DE DONNEES EST INSUFFISAMMENT REMPLIE, ET A
!       UN ARTICLE ASSOCIE SUR FICHIER; IL FAUT DONC LIRE CET ARTICLE,
...
223 CONTINUE
!
!         COMPLEMENT DE LA PAGE DE DONNEES ASSOCIEE AU PREMIER ARTICLE
!       PHYSIQUE OU DOIVENT ETRE STOCKEES LES DONNEES A ECRIRE.
!
  DO JD=1,IPAREC
  LFI%MTAMPD(IXT(IDCDEB+JD,INUMPD,KRANG))=KTAB(JD)
  ENDDO
!
  LFI%LECRPD(INUMPD,KRANG)=.TRUE.
  LFI%NLONPD(INUMPD,KRANG)=MAX (LFI%NLONPD(INUMPD,KRANG), &
&                                 IDCDEB+IPAREC)
  LFI%NDERPD(KRANG)=INUMPD
ENDIF
!*
!     2.3 - ECRITURE DES ARTICLES PHYSIQUES COMPLETS NE TENANT PAS OU NE
!           DEVANT PAS ETRE STOCKES DANS LES PAGES DE DONNEES "TAMPON".
!           S'IL Y EN A, LES EVENTUELLES PAGES DE DONNEES COPIEES AUX
!           PARAGRAPHES PRECEDENTS (2.1 ET 2.2) PRECEDANT CES ARTICLES
!           DANS LE FICHIER SERONT ALORS ELLES AUSSI ECRITES SUR DISQUE.
!-----------------------------------------------------------------------
!
IF (.NOT.LFI%LTAMPE(KRANG)) THEN
  ITAMLI=0
ELSEIF (LLDERN) THEN
  ITAMLI=ITAMLI-1
ENDIF
...
INAPHY=0
CALL LFIECX_MT_BB                                     &
&               (LFI, KREP,KRANG,IARTIC,KTAB(IDECDE+1), &
&                LLADON,IRETIN)
!
IF (IRETIN.EQ.1) THEN
  GOTO 903
...
ENDDO
!*
!     2.4 - ECRITURE DES ARTICLES PHYSIQUES COMPLETS QUE L'ON PEUT STOC-
...
!
DO JD=1,ILARPH
LFI%MTAMPD(IXT(JD,INUMPD,KRANG))=KTAB(IDECDE+JD)
ENDDO
!
LFI%LECRPD(INUMPD,KRANG)=.TRUE.
...
!           PREMIERE ET ELLE COMMENCE JUSTE EN DEBUT D'ARTICLE PHYSIQUE.
!           CETTE PAGE EST CONSERVEE...EN ESPERANT QUE LE PROCHAIN ACCES
!           AU FICHIER SERA POUR L'ARTICLE LOGIQUE IMMEDIATEMENT
!           DERRIERE ( CAS DE CREATION DU FICHIER, PAR EXEMPLE ) .
!-----------------------------------------------------------------------
!
IF (LLDERN) THEN
!
...
!
  LFI%LECRPD(INUMPD,KRANG)=.TRUE.
  LFI%NLONPD(INUMPD,KRANG)=MAX (LFI%NLONPD(INUMPD,KRANG),IDCFIN)
  LFI%NDERPD(KRANG)=INUMPD
ENDIF
!
GOTO 1001
...
&  '', KLONG='',I7,'', KPOSEC='',I8,'', KRETIN='',I2)')   &
&   KREP, KRANG, KLONG, KPOSEC, KRETIN
  CALL LFIEMS_MT_BB                               &
&                 (LFI, INUMER,INIMES,KREP,.FALSE., &
&                  CLMESS,CLNSPR,CLACTI)
ENDIF
!
IF (LHOOK) CALL DR_HOOK('LFIECD_MT',1,ZHOOK_HANDLE)
...
! Oct-2012 P. Marguinaud 64b LFI
SUBROUTINE LFIECD_BB                                 &
&           (KREP, KRANG, KTAB, KLONG, KPOSEC, KRETIN)
...
INTEGER (KIND=JPLIKB)  KPOSEC                                 ! IN   
INTEGER (KIND=JPLIKB)  KRETIN                                 !   OUT

IF (.NOT. LFICOM_DEFAULT_INIT) CALL NEW_LFI_DEFAULT ()

CALL LFIECD_MT_BB                                         &
&           (LFI, KREP, KRANG, KTAB, KLONG, KPOSEC, KRETIN)

END SUBROUTINE

SUBROUTINE LFIECD_MM                                 &
&           (KREP, KRANG, KTAB, KLONG, KPOSEC, KRETIN)
USE LFIMOD, ONLY : LFI => LFICOM_DEFAULT, &
&                   LFICOM_DEFAULT_INIT,   &
&                   NEW_LFI_DEFAULT
...
INTEGER (KIND=JPLIKB)  IPOSEC                                 ! IN   
INTEGER (KIND=JPLIKB)  IRETIN                                 !   OUT
! Convert arguments

IRANG      = INT (     KRANG, JPLIKB)
ILONG      = INT (     KLONG, JPLIKB)
IPOSEC     = INT (    KPOSEC, JPLIKB)

CALL LFIECD_MT_BB                                         &
&           (LFI, IREP, IRANG, KTAB, ILONG, IPOSEC, IRETIN)
