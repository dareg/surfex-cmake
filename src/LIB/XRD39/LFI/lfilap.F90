...
IF (LHOOK) CALL DR_HOOK('LFILAP_MT',0,ZHOOK_HANDLE)
CLACTI=''
CALL LFINUM_MT_BB                 &
&               (LFI, KNUMER,IRANG)
LLVERF=.FALSE.
...
IF (LLFATA.OR.IXNIMS (IRANG).EQ.2) THEN
  INIMES=2
ELSE
  IF (LHOOK) CALL DR_HOOK('LFILAP_MT',1,ZHOOK_HANDLE)
  RETURN
ENDIF
!
CLNSPR='LFILAP'
