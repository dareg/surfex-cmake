...
IMPLICIT NONE
!****
!        SOUS-PROGRAMME DE LECTURE DE L'ARTICLE (DE DONNEES) *SUIVANT*
...
!     *LFICAS*, L'EXISTENCE D'UN ARTICLE LOGIQUE DE DONNEES "SUIVANT".
!     SINON, IL FAUT PREVOIR DE GERER L'ERREUR DE CODE (-23) ...
!     ENTRE AUTRES.
!
!
TYPE(LFICOM) :: LFI
...
IF (LHOOK) CALL DR_HOOK('LFILAS_MT',0,ZHOOK_HANDLE)
CLACTI=''
CALL LFINUM_MT_BB                 &
&               (LFI, KNUMER,IRANG)
LLVERF=.FALSE.
...
IF (LLFATA.OR.IXNIMS (IRANG).EQ.2) THEN
  INIMES=2
ELSE
  IF (LHOOK) CALL DR_HOOK('LFILAS_MT',1,ZHOOK_HANDLE)
  RETURN
ENDIF
!
CLNSPR='LFILAS'
...
INTEGER (KIND=JPDBLE)  KTAB       (KLONG)                     ! IN   
...
!INTF KNUMER        IN                                                                 
!INTF CDNOMA          OUT                                                              
!INTF KTAB          IN    DIMS=KLONG                     KIND=JPDBLE                   
!INTF KLONG         IN                                                                 
