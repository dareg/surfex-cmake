...
!      EN-TETE POUR GRIBEX (SECTIONS 1 ET 2) 
      ...
      TYPE FAFICH
      INTEGER (KIND=JPLIKB)          :: NULOGI      = NUNDEF
      ...
      INTEGER (KIND=JPLIKB)          :: NRASHO      = NUNDEF
      INTEGER (KIND=JPLIKB)          :: NRASVE      = NUNDEF
      ...
      LOGICAL                        :: LIFLAP      = LUNDEF
      LOGICAL                        :: LNOMME      = LUNDEF
      INTEGER (KIND=JPLIKB), POINTER :: NSEC1 (:)   => NULL ()
      INTEGER (KIND=JPLIKB), POINTER :: NSC2ALF(:)  => NULL ()
      LOGICAL                        :: LISEC1      = LUNDEF
      LOGICAL                        :: LISC2F      = LUNDEF
      INTEGER (KIND=JPLIKB), POINTER :: NCOGRIF(:)  => NULL ()
...
!  3 - PARAMETRES DEFINISSANT LE CODAGE GRIBEX
      ...
      INTEGER (KIND=JPLIKB), POINTER :: NCODGRI(:)   => NULL ()
      INTEGER (KIND=JPLIKB) JPXPAR
      INTEGER (KIND=JPLIKB) NBPARC
      CHARACTER(LEN=JPXPRF), POINTER :: CIPREF(:)    => NULL ()
      CHARACTER(LEN=JPXSUF), POINTER :: CISUFF(:)    => NULL ()
      INTEGER (KIND=JPLIKB), POINTER :: NCODPA(:,:)  => NULL ()

!  4 - TABLEAU POUR CODER LES COEF SPECTRAUX ALADIN AVEC GRIBEX

      INTEGER (KIND=JPDBLE), POINTER :: ICHAMP(:) => NULL ()
      INTEGER (KIND=JPDBLE), POINTER :: ICHAUX(:) => NULL ()
      ...
      INTEGER (KIND=JPLIKB) :: JPLSPX = JPLSPX
     ...
     &  CA%XSEC2   (10+2*(KPXNIV+1)))
      ...
      SUBROUTINE NEW_FICHIER (FI, KPLDAT, KPXTRO, KTYPTR)
     ...
     &  FI%NCOGRIF(10))
      ...
      IF (ASSOCIATED (FI%NSEC1  )) DEALLOCATE (FI%NSEC1  )
      IF (ASSOCIATED (FI%NSC2ALF)) DEALLOCATE (FI%NSC2ALF)
      IF (ASSOCIATED (FI%NCOGRIF)) DEALLOCATE (FI%NCOGRIF)
...
!
     ...
     &  FA%NULIND (FA%JPNXFA),        FA%NCAIND (FA%JPNXFA),           &
     &  STAT = KERR )
      IF (KERR /= 0) GOTO 999
