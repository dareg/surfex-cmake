...
!
!     FA%NCODGRI(1) = type de codage (0->option HOPER='C'; 1->option HOPER='K')
!     FA%NCODGRI(2) = KSEC4(6), indicateur de la presence de flags
!                               additionnels (0->non; 16->oui)
!     FA%NCODGRI(3) = KSEC4(7)
!     FA%NCODGRI(4) = KSEC4(9), indicateur de la presence de bitmaps
!                             secondaires (0->non; 32->oui)
!     FA%NCODGRI(5) = KSEC4(10), indicateur pour le nb de bits des
!                        groupes de pts de grille (0->const.; 16->different)
!     FA%NCODGRI(6) = KSEC4(11), nb de bits pour les groupes de pts de
!                      grille, quand il est constant.
!                        Si negatif, le logiciel calcule un nb optimal a partir
!                        de -KSEC4(11).
!     FA%NCODGRI(7) = KSEC4(12), indicateur pour les extensions generales de la
!                        compression (0->non; 8->oui)
!     FA%NCODGRI(8) = KSEC4(13), indicateur pour le rearrangement boustrophedo
!                        nique (0->non; 4->oui)
!     FA%NCODGRI(9) = KSEC4(14) (valeurs possibles: -1, 0 et 2)
!     FA%NCODGRI(10) = KSEC4(15) (valeurs possibles: -1, 0 et 1), sert avec
...
!
  IF (CDCLEF=='CLEFS'.OR.CDCLEF=='clefs') THEN
    ...
    WRITE (UNIT=FA%NULOUT,FMT=*)         &
&    'Mots clef disponibles pour FAREGI:'
    WRITE (UNIT=FA%NULOUT,FMT=*)
...
&    '       ligne a ligne pour les pts de grille'
    WRITE (UNIT=FA%NULOUT,FMT=*)        &
&    'PACK2: BASIC avec une compression'
    WRITE (UNIT=FA%NULOUT,FMT=*)                       &
&    '       ou le nb de bits est cst pour les groupes'
    WRITE (UNIT=FA%NULOUT,FMT=*)        &
&    'PACK3: BASIC avec une compression'
    WRITE (UNIT=FA%NULOUT,FMT=*)                   &
&    '       OMM general pour les points de grille'
    WRITE (UNIT=FA%NULOUT,FMT=*)   &
...
&    '       de grille dans le cas PACK2'
    WRITE (UNIT=FA%NULOUT,FMT=*)                        &
&    'GEXTE: les extensions generales de la compression'
    WRITE (UNIT=FA%NULOUT,FMT=*)                          &
&    '       sont activees (KVAL=1) ou desactiv. (KVAL=0)'
    WRITE (UNIT=FA%NULOUT,FMT=*)                    &
