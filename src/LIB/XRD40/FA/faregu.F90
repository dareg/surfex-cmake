...
!*
!     Signification des divers elements du tableau FA%NCOGRIF
!
!     FA%NCOGRIF(1,IRANG) = type de codage (0->option HOPER='C'
!                                        1->option HOPER='K')
!     FA%NCOGRIF(2,IRANG) = KSEC4(6), indicateur de la presence de flags
!                        additionnels (0->non; 16->oui)
!     FA%NCOGRIF(3,IRANG) = KSEC4(7)
!     FA%NCOGRIF(4,IRANG) = KSEC4(9), indicateur de la presence de bitmaps
!                        secondaires (0->non; 32->oui)
!     FA%NCOGRIF(5,IRANG) = KSEC4(10), indicateur pour le nb de bits des
!                        groupes de pts de grille (0->const.; 16->different)
!     FA%NCOGRIF(6,IRANG) = KSEC4(11), nb de bits pour les groupes de pts de grille
!                        quand il est constant.
!                        Si negatif, le logiciel calcule un nb optimal a partir
!                        de -KSEC4(11).
!     FA%NCOGRIF(7,IRANG) = KSEC4(12), indicateur pour les extensions generales de
!                        la compression (0->non; 8->oui)
!     FA%NCOGRIF(8,IRANG) = KSEC4(13), indicateur pour le rearrangement boustrophedo
!                        nique (0->non; 4->oui)
!     FA%NCOGRIF(9,IRANG) = KSEC4(14) (valeurs possibles: -1, 0 et 2)
!     FA%NCOGRIF(10,IRANG) = KSEC4(15) (valeurs possibles: -1, 0 et 1), sert avec
...
&      CDCLEF=='HELP' .OR. CDCLEF=='help') THEN
    ...
    WRITE (UNIT=FA%NULOUT,FMT=*)
    WRITE (UNIT=FA%NULOUT,FMT=*)         &
&    'Mots clef disponibles pour FAREGU:'
    WRITE (UNIT=FA%NULOUT,FMT=*)
    WRITE (UNIT=FA%NULOUT,FMT=*)'BASIC: pas de compression'
    WRITE (UNIT=FA%NULOUT,FMT=*)        &
&    'PACK1: BASIC avec une compression'
    WRITE (UNIT=FA%NULOUT,FMT=*)                  &
&    '       ligne a ligne pour les pts de grille'
    WRITE (UNIT=FA%NULOUT,FMT=*)             &
&    'PACK2: BASIC avec une compression avec'
    WRITE (UNIT=FA%NULOUT,FMT=*)             &
&    '       nb de bits cst pour les groupes'
    WRITE (UNIT=FA%NULOUT,FMT=*)                 &
&    'PACK3: BASIC avec une compression generale'
    WRITE (UNIT=FA%NULOUT,FMT=*)           &
&    '       OMM pour les points de grille'
    WRITE (UNIT=FA%NULOUT,FMT=*)    &
&    'APAC1: compression agressive:'
    WRITE (UNIT=FA%NULOUT,FMT=*)        &
&    '       BASIC et PACK1 sont testes'
    WRITE (UNIT=FA%NULOUT,FMT=*)    &
&    'APAC2: compression agressive:'
    WRITE (UNIT=FA%NULOUT,FMT=*)               &
&    '       BASIC, PACK1 et PACK2 sont testes'
    WRITE (UNIT=FA%NULOUT,FMT=*)    &
&    'APAC3: compression agressive:'
    WRITE (UNIT=FA%NULOUT,FMT=*)               &
&    '       BASIC, PACK1 et PACK3 sont testes'
    WRITE (UNIT=FA%NULOUT,FMT=*)    &
&    'APAC4: compression agressive:'
    WRITE (UNIT=FA%NULOUT,FMT=*)                 &
&    '       BASIC, PACK1, PACK2 et PACK3 testes'
    WRITE (UNIT=FA%NULOUT,FMT=*)            &
&    'WIDPA: lecture/ecriture du nb de bits'
    WRITE (UNIT=FA%NULOUT,FMT=*)                   &
&    '       a utiliser pour les groupes de points'
    WRITE (UNIT=FA%NULOUT,FMT=*)         &
&    '       de grille dans le cas PACK2'
    WRITE (UNIT=FA%NULOUT,FMT=*)                      &
&    'GEXTE: la compression avec extensions generales'
    WRITE (UNIT=FA%NULOUT,FMT=*)                       &
&    '       activees (KVAL=1) ou desactivees (KVAL=0)'
    WRITE (UNIT=FA%NULOUT,FMT=*)                    &
&    'BOUST: le rearrangement boustrophedonique est'
    WRITE (UNIT=FA%NULOUT,FMT=*)                   &
&    '       active (KVAL=1) ou desactive (KVAL=0)'
    WRITE (UNIT=FA%NULOUT,FMT=*)             &
&    'DIFFE: la differenciation spatiale est'
    WRITE (UNIT=FA%NULOUT,FMT=*)                    &
&    '       activee (KVAL=ordre de differ. (1 a 3)'
    WRITE (UNIT=FA%NULOUT,FMT=*)                   &
&    '       ou -1 (calcul dyn)) ou desactivee (0)'
    WRITE (UNIT=FA%NULOUT,FMT=*)                   &
&    'IDCEN: lect/ecriture de l''identificateur du'
    WRITE (UNIT=FA%NULOUT,FMT=*) &
&    '       centre meteo'
    WRITE (UNIT=FA%NULOUT,FMT=*)                   &
&    'IDMOD: lect/ecriture de l''identificateur du'
    WRITE (UNIT=FA%NULOUT,FMT=*) &
&    '       modele'
    WRITE (UNIT=FA%NULOUT,FMT=*)
!
