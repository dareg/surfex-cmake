...
!
      IMPLICIT NONE
!
      INTEGER(KIND=JPIM) :: KEXP
      INTEGER(KIND=JPIM) :: KMANT
!
      REAL (KIND=JPDBLR) :: PFVAL
      REAL (KIND=JPDBLR) :: PNFVAL
!
      INTEGER(KIND=JPIM) :: IAUXIL, IEXP
!
...
!
      SAVE ZEPS, ZC16
      DATA ZEPS / 1.E-12_JPDBLR /
      DATA ZC16 / 16.0_JPDBLR /
!
!
!
!           Elimination of sign.
...
!     EXPONENT
!
      IF (ZREF.EQ.0.0_JPDBLR) THEN
        KEXP = 0
      ELSE
        IEXP = INT ((LOG(ZREF)/LOG(16.0_JPDBLR) + 65.0_JPDBLR) +ZEPS)
