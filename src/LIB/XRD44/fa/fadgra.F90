...
!
LOGICAL LDCOSP, LDUNDF, LLUNDF, LLLTLN 
!
...
! Restore encoding parameters if needed
  ...
  ENDIF
  CALL IGRIB_SET_VALUE (IGRIBH, 'faFieldName', TRIM (CLNOMU))
ENDIF
...
CALL IGRIB_GET_VALUE (IGRIBH, 'FMULTM', IMULTM)
CALL IGRIB_GET_VALUE (IGRIBH, 'FMULTE', IMULTE)
