...
!     (Tableau)  KDATEF (Entree/Sortie) ==> Date elle-meme (FA%JPLDAT*2 mots).
!*
...
IF (LHOOK) CALL DR_HOOK('FANDAX_MT',0,ZHOOK_HANDLE)
CLACTI=''
LLVERF=.FALSE.
LLRLFI=.FALSE.
LLMODA=.FALSE.
...
IF (.NOT.LLFATA.AND.INIMES.NE.2)  THEN
  IF (LHOOK) CALL DR_HOOK('FANDAX_MT',1,ZHOOK_HANDLE)
  RETURN
ENDIF
!
CLNSPR='FANDAX'
...
INTEGER (KIND=JPLIKB)  KDATEF     (*)               ! INOUT
...
INTEGER (KIND=JPLIKM)  KDATEF     (*)               ! INOUT
...
END SUBROUTINE FANDAX

SUBROUTINE FANDAX_MT                 &
&           (FA, KREP, KNUMER, KDATEF)
USE FA_MOD, ONLY : FA_COM
USE LFI_PRECISION
IMPLICIT NONE
...
INTEGER (KIND=JPLIKM)  KREP                                   !   OUT
INTEGER (KIND=JPLIKM)  KNUMER                                 ! IN   
INTEGER (KIND=JPLIKM)  KDATEF     (FA%JPLDAT*2)               ! INOUT
! Local integers
INTEGER (KIND=JPLIKB)  IREP                                   !   OUT
INTEGER (KIND=JPLIKB)  INUMER                                 ! IN   
INTEGER (KIND=JPLIKB)  IDATEF     (FA%JPLDAT*2)               ! INOUT
! Convert arguments
...
END SUBROUTINE FANDAX_MT

!INTF KREP            OUT                               
!INTF KNUMER        IN                                  
!INTF KDATEF        INOUT DIMS=FA%JPLDAT*2            
