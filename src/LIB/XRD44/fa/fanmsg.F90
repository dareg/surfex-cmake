! Oct-2012 P. Marguinaud 64b LFI
! Jan-2011 P. Marguinaud Thread-safe FA
! R. El Khatib 30-Mar-2012 KULOUT
SUBROUTINE FANMSG_FORT                    &
&                     (FA,  KNIVAU, KULOUT)
USE FA_MOD, ONLY : FA_COM, JPNIIL
USE PARKIND1, ONLY : JPRB
USE YOMHOOK , ONLY : LHOOK, DR_HOOK
USE LFI_PRECISION
IMPLICIT NONE
!****
!        Ce sous-programme se charge de mettre le Niveau Global d'
!     impression des Messages du logiciel de Fichiers ARPEGE (*FA%NIMSGA*)
!     a la valeur KNIVAU, de meme que la variable correspondante du
!     du logiciel LFI. Par defaut, FA%NIMSGA vaut 1.
!**
!        Argument : KNIVAU (Entree) ==> Niveau Global d'Impression
!                                       des Messages.
!                                       Valeurs possibles:
!
!     0 : N'emettre que les messages d'erreurs reellement importants .
!     1 : N'emettre qu'un minimum de messages "globaux", et les messages
!         lies a un fichier ouvert qui sont de niveau au plus egal au
!         niveau de la messagerie pour ce fichier (Mode par defaut) .
!     2 : Emettre tous les messages possibles, meme s'ils ne correspon-
!         dent pas a un fichier ouvert avec le niveau de Messagerie 2 .
!
!                   KULOUT : logical unit number for printing
!
!
!
TYPE(FA_COM) :: FA
INTEGER (KIND=JPLIKB) KNIVAU
INTEGER (KIND=JPLIKB) KULOUT
!
INTEGER (KIND=JPLIKB) IREP, INIMES, INUMER
CHARACTER(LEN=FA%JPXNOM) CLACTI 
CHARACTER(LEN=FA%JPLMES) CLMESS 
CHARACTER(LEN=FA%JPLSPX) CLNSPR
LOGICAL                  LLFATA

!
!
REAL(KIND=JPRB) :: ZHOOK_HANDLE

IF (LHOOK) CALL DR_HOOK('FANMSG_MT',0,ZHOOK_HANDLE)
CLACTI=''

IF (FA%FANMSG_LLPREA) THEN
  CALL FARINE_FORT              &
&                 (FA, 2_JPLIKB )
  FA%FANMSG_LLPREA=.FALSE.
ENDIF
!
IF (KNIVAU.GE.0.AND.KNIVAU.LE.2) THEN
  INIMES=MAX (FA%NIMSGA,KNIVAU)
  FA%NIMSGA=KNIVAU
  CALL LFINMG_FORT                       &
&                 (FA%LFI, KNIVAU,KULOUT)
  IREP=0
ELSE
  INIMES=FA%NIMSGA
  IREP=-52
ENDIF
!
LLFATA=IREP.NE.0.AND.FA%NRFAGA.NE.2
!
IF (LLFATA) THEN
  INIMES=2
ELSEIF (IREP.NE.0) THEN
  INIMES=0
ELSEIF (INIMES.NE.2) THEN
  IF (LHOOK) CALL DR_HOOK('FANMSG_MT',1,ZHOOK_HANDLE)
  RETURN
ENDIF
!
INUMER=JPNIIL
CLNSPR='FANMSG'
!
IF (MAX (INIMES,FA%NIMSGA).EQ.2) THEN
  WRITE (UNIT=CLMESS,                                  &
&         FMT='(''KNIVAU='',I5,'', CODE INTERNE='',I4)' &
&         ) KNIVAU,IREP
  IF (INIMES.NE.2) CALL FAIPAR_FORT                          &
&                                  (FA, INUMER,FA%NIMSGA,IREP, &
&                                .FALSE.,CLMESS,               &
&                                CLNSPR,CLACTI,.FALSE.)
ENDIF
!
CALL FAIPAR_FORT                                     &
&               (FA, INUMER,INIMES,IREP,LLFATA,CLMESS, &
&             CLNSPR,CLACTI,                           &
&             .FALSE.)
!
IF (LHOOK) CALL DR_HOOK('FANMSG_MT',1,ZHOOK_HANDLE)
END SUBROUTINE FANMSG_FORT


! Oct-2012 P. Marguinaud 64b LFI
SUBROUTINE FANMSG64           &
&           (KNIVAU, KULOUT)
USE FA_MOD, ONLY : FA => FA_COM_DEFAULT, &
&                   FA_COM_DEFAULT_INIT,  &
&                   NEW_FA_DEFAULT
USE LFI_PRECISION
IMPLICIT NONE
! Arguments
INTEGER (KIND=JPLIKB)  KNIVAU                                 ! IN   
INTEGER (KIND=JPLIKB)  KULOUT                                 ! IN   

IF (.NOT. FA_COM_DEFAULT_INIT) CALL NEW_FA_DEFAULT ()

CALL FANMSG_FORT               &
&           (FA, KNIVAU, KULOUT)

END SUBROUTINE FANMSG64

SUBROUTINE FANMSG             &
&           (KNIVAU, KULOUT)
USE FA_MOD, ONLY : FA => FA_COM_DEFAULT, &
&                   FA_COM_DEFAULT_INIT,  &
&                   NEW_FA_DEFAULT
USE LFI_PRECISION
IMPLICIT NONE
! Arguments
INTEGER (KIND=JPLIKM)  KNIVAU                                 ! IN   
INTEGER (KIND=JPLIKM)  KULOUT                                 ! IN   

IF (.NOT. FA_COM_DEFAULT_INIT) CALL NEW_FA_DEFAULT ()

CALL FANMSG_MT                 &
&           (FA, KNIVAU, KULOUT)

END SUBROUTINE FANMSG

SUBROUTINE FANMSG_MT             &
&           (FA, KNIVAU, KULOUT)
USE FA_MOD, ONLY : FA_COM
USE LFI_PRECISION
IMPLICIT NONE
! Arguments
TYPE (FA_COM)          FA                                     ! INOUT
INTEGER (KIND=JPLIKM)  KNIVAU                                 ! IN   
INTEGER (KIND=JPLIKM)  KULOUT                                 ! IN   
! Local integers
INTEGER (KIND=JPLIKB)  INIVAU                                 ! IN   
INTEGER (KIND=JPLIKB)  IULOUT                                 ! IN   
! Convert arguments

INIVAU     = INT (    KNIVAU, JPLIKB)
IULOUT     = INT (    KULOUT, JPLIKB)

CALL FANMSG_FORT               &
&           (FA, INIVAU, IULOUT)


END SUBROUTINE FANMSG_MT

!INTF KNIVAU        IN    
!INTF KULOUT        IN    
