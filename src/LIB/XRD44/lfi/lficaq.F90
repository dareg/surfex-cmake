...
!     RECHERCHE DE L'ARTICLE LOGIQUE *DE DONNEES* PRECEDENT, DANS UNE
...
INTEGER (KIND=JPLIKB) INIMES, INUMER, KRETIN
...
IF (LHOOK) CALL DR_HOOK('LFICAQ_FORT',0,ZHOOK_HANDLE)
CLACTI=''
IRETOU=0
!
...
!        RECHERCHER CET ARTICLE ( A PRIORI, VIA *LFICAP* ) .
!
...
!
!        PLUS D'ARTICLE LOGIQUE "PRECEDENT" A LIRE .
!
  GOTO 1001
ELSEIF (LFI%NDERGF(IRANG).EQ.LFI%JPNIL.OR.           &
&        LFI%NPRECF(IRANG).LT.LFI%NDERGF(IRANG)) THEN
...
!     LA P.A.I. PRECEDENTE, EN RANG DANS LE FICHIER.
!
IF (IRGPIF.GT.1) THEN
  IRGPIF=IRGPIF-1
...
ENDIF
!
!    SI ON ARRIVE ICI, C'EST QUE LE PREMIER ARTICLE LOGIQUE EST UN TROU
!
LFI%NPRECF(IRANG)=0
GOTO 1001
!*
...
ENDIF
!
IF (LHOOK) CALL DR_HOOK('LFICAQ_FORT',1,ZHOOK_HANDLE)
