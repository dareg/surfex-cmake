...
IF (LHOOK) CALL DR_HOOK('LFIECC_FORT',0,ZHOOK_HANDLE)
...
ELSE
  ...
  CALL FI_FWRITE (IREP, CDTAB, 1_JPLIKB, ISIZE, YDFIC%N_C_FPDESC)
  IF (IREP /= ISIZE) THEN
    KREP = 1
    GOTO 901
  ENDIF
...
1001 CONTINUE
!
IF (LHOOK) CALL DR_HOOK('LFIECC_FORT',1,ZHOOK_HANDLE)
END SUBROUTINE LFIECC_FORT
