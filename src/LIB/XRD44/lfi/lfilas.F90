...
IF (LHOOK) CALL DR_HOOK('LFILAS_FORT',0,ZHOOK_HANDLE)
CLACTI=''
CALL LFINUM_FORT                    &
&               (LFI, KNUMER,IRANG)
LLVERF=.FALSE.
...
IF (LLFATA.OR.IXNIMS (IRANG).EQ.2) THEN
  INIMES=2
ELSE
  IF (LHOOK) CALL DR_HOOK('LFILAS_FORT',1,ZHOOK_HANDLE)
  RETURN
ENDIF
!
CLNSPR='LFILAS'
...
END SUBROUTINE LFILAS

SUBROUTINE LFILAS_MT                               &
&           (LFI, KREP, KNUMER, CDNOMA, KTAB, KLONG)
USE LFIMOD, ONLY : LFICOM
USE LFI_PRECISION
IMPLICIT NONE
