...
SUBROUTINE LFILCC_FORT                                    &
&                     (LFI, KREP, KNUMER, KREC, CDTAB,  &
&                      KNBLEC, KFACTM, YDFIC, KRETIN)
...
IF (LHOOK) CALL DR_HOOK('LFILCC_FORT',0,ZHOOK_HANDLE)
...
ELSE
  ...
  IF (KREP /= 0) GOTO 901
  ISIZE = INT (SIZE (CDTAB) * LEN (CDTAB), JPLIKB)
  CALL FI_FREAD (IREP, CDTAB, 1_JPLIKB, ISIZE, YDFIC%N_C_FPDESC)
  IF (IREP /= ISIZE) THEN
    KREP = 1
    GOTO 901
  ENDIF
...
1001 CONTINUE
!
IF (LHOOK) CALL DR_HOOK('LFILCC_FORT',1,ZHOOK_HANDLE)
END SUBROUTINE LFILCC_FORT
