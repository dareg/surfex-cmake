...
#include "swapbytes_ifsaux.h"

PUBLIC void
swap1bytes_ifsaux_(void *v, const int *vlen)
{
  /* does nothing; for convenience */
}

PUBLIC void
swap2bytes_ifsaux_(void *v, const int *vlen)
{
...
PUBLIC void
swap2bytes_ifsaux_via_copy_(void *v, const void *vin, const int *vlen)
{
  unsigned short int *u = v;
...
PUBLIC void
swap4bytes_ifsaux_(void *v, const int *vlen)
{
  unsigned int *u = v;
  int j,ulen = vlen ? *vlen : 0;
  for (j=0; j<ulen; j++) {
...
PUBLIC void
swap4bytes_ifsaux_via_copy_(void *v, const void *vin, const int *vlen)
{
  unsigned int *u = v;
...
PUBLIC void
swap8bytes_ifsaux_(void *v, const int *vlen)
{
  u_ll_t *u = v;
  int j,ulen = vlen ? *vlen : 0;
  for (j=0; j<ulen; j++) {
...
PUBLIC void
swap8bytes_ifsaux_via_copy_(void *v, const void *vin, const int *vlen)
{
  u_ll_t *u = v;
...
PUBLIC void
swapANYbytes_ifsaux_(void *v, int elsize, int n, unsigned int dtype, 
...
#ifdef DEBUG
  fprintf(stderr,
	  "swapANYbytes_ifsaux(%s:%s): elsize=%d, n=%d, dtype=%u (0x%x)\n",
...
#endif
    if (bits == 16 && elsize == 2) {
      swap2bytes_ifsaux_(v, &n);
    }
    else if (bits == 32 && elsize == 4) {
      swap4bytes_ifsaux_(v, &n);
    }
    else if (bits == 64 && elsize == 8) {
      swap8bytes_ifsaux_(v, &n);
    }
    else {
      fprintf(stderr,
	      " ***Error in swapANYbytes_ifsaux() [elsize=%d]: Can't swap bytes for dtype=0x%x (type=%s, var='%s')\n",
