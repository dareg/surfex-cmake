...
iret = NF_CREATE(YFILE, 0, KFILE_ID) 

IF (IRET.NE.NF_NOERR) CALL HANDLE_ERR(IRET,'CREATE_FILE')

! 2.0  define dimensions
!-----------------------
INBDIM=SIZE(KDIMS)
DO JNBDIM=1,INBDIM
   iret=NF_DEF_DIM(KFILE_ID,HNAME_DIM(JNBDIM),KDIMS(JNBDIM),KDIM_ID(JNBDIM))
ENDDO
