...
IF ( NRANK==NPIO ) THEN
  ...
  !
  CALL READ_SURF(IOB, &
                 HPROGRAM,'DIM_FULL  ',U%NDIM_FULL,IRESP,HDIR='A')
  NINDX2SFX = U%NDIM_FULL
  !
  CALL READ_ARRANGE_COVER(IOB, &
                          HPROGRAM,U%LWATER_TO_NATURE,U%LTOWN_TO_ROCK,'A')
  CALL READ_COVER_GARDEN(IOB, &
                         HPROGRAM,U%LGARDEN,'A')
  !
  ...
  !
  CALL READ_SURF(IOB, &
                 HPROGRAM,'GRID_TYPE',UG%CGRID,IRESP,HDIR='A')
  !
...
!
  ...
  ZPATCH(:,:)=0.0
!  
  IF (IVERSION<8) CALL READ_SURF(IOB, &
