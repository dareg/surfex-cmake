...
!*     3. Points by task
!
IF (NRANK==NPIO .OR. HINIT=='PGD' .OR. (HINIT=='OFF' .AND. LLAND_USE) .OR. LXIOS) THEN
...
ENDIF
!
IF (HINIT=='PGD'.OR.(HINIT=="OFF".AND.LLAND_USE).OR.LXIOS) THEN
...
ENDIF
!
IF (HINIT/='PGD'.AND..NOT.LLAND_USE) UG%XGRID_FULL_PAR=>NULL()
!
...
LOGICAL :: GDATA_VEGTYPE, GECOSG, GDIM
!
...
!
IF (GDIM) THEN
  CALL READ_SURF(HPROGRAM,'ECOSG',GECOSG,IRESP,HDIR='-')
ELSE
  GECOSG = .FALSE.
ENDIF
!
...
IF (YNATURE/='NONE' .AND. IDIM_NATURE>0) THEN
  ...
  IF (GDATA_VEGTYPE) THEN
    ...
    ELSE
      YRECFM='D_VEGTYPE'
    ENDIF          
    CALL MAKE_CHOICE_ARRAY(HPROGRAM, NVEGTYPE, GDIM, YRECFM, ZVEGTYPE(:,:),HDIR='A')
  ELSE
    IF (LWR_VEGTYPE) THEN
