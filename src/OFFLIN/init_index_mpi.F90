...
USE MODD_SURFEX_OMP, ONLY : NINDX2, NWORK, XWORK, XWORK2, XWORK3, NWORK_FULL, XWORK_FULL, XWORK2_FULL, NINDX1
!
...
IF (NRANK==NPIO) THEN
  ...
  IF (HALG=='LIN ') THEN
    !
    CALL SET_NB_POINTS_LIN(NPROC,NPROC-1,NDIM_FULL,NINDEX)
    !
  ELSEIF (HALG=='ADJ ' .OR. HALG=='TILA' .OR. HALG=='TILL') THEN
    ...
    !
    IF (HALG=='ADJ ') THEN
      CALL SET_NB_POINTS_ADJ(NPROC,NDIM_FULL,NDIM_FULL,ILEFT,IRIGHT,ITOP,IBOTTOM,NINDEX)
    ELSEIF (HALG=='TILA' .OR. HALG=='TILL') THEN
      CALL INI_DATA_COVER
      CALL SET_NB_POINTS_TIL(HPROGRAM,HALG,NPROC,NDIM_FULL,ILEFT,IRIGHT,ITOP,IBOTTOM,NINDEX)
    ENDIF
    !
    DEALLOCATE(ILEFT  )
    DEALLOCATE(IRIGHT )
...
IF (NPROC>1) THEN
  ...
  CALL MPI_BCAST(NINDX2,KIND(NINDX2)/4,MPI_INTEGER,NPIO,NCOMM,INFOMPI)
#endif
ENDIF
!
...
!***************************************************************
!
SUBROUTINE SET_NB_POINTS_LIN(KPROC,KPROCMIN,KSIZE,KINDEX)
!
IMPLICIT NONE
!
...
!
 CALL GET_SIZES_PARALLEL(KPROC,KSIZE,KPROCMIN,ISIZE_TASK)
!
IPROC1 = KPROCMIN
!
CPT   = 0
...
!**************************************************************************
!
SUBROUTINE SET_NB_POINTS_ADJ(KPROC,KSIZE,KSIZE_FULL,KLEFT,KRIGHT,KTOP,KBOTTOM,KINDEX)
!
IMPLICIT NONE
!
INTEGER, INTENT(IN) :: KPROC
...
!
 CALL GET_SIZES_PARALLEL(KPROC,KSIZE,NPIO,ISIZE_TASK)
!
DO I=1,KPROC-1
  !
  CPT_INTER = 0
...
!*****************************************************************
!
SUBROUTINE SET_NB_POINTS_TIL(HPROGRAM,HALG,KPROC,KSIZE_FULL,KLEFT,KRIGHT,KTOP,KBOTTOM,KINDEX)
!
...
DO I = 1,15
  ...
  DO J = 1,40
    ...
    IF (IFULL.NE.0) THEN
      ...
      !
      IF (HALG=='TILL') THEN
        CALL SET_NB_POINTS_LIN(KPROC,IPROCMIN(1),IFULL,IINDEX(:))
      ELSEIF (HALG=='TILA') THEN
        CALL SET_NB_POINTS_ADJ(KPROC,IFULL,KSIZE_FULL,KLEFT,KRIGHT,KTOP,KBOTTOM,IINDEX(:))
      ENDIF
      !
      DO K=1,KSIZE_FULL
