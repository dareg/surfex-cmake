...
IF ( NRANK==NPIO ) THEN
  ...
  CALL INIT_IO_SURF_n(HPROGRAM,'FULL  ','SURF  ','READ ')  
  !
  CALL READ_SURF(HPROGRAM,'GRID_TYPE',UG%CGRID,IRESP,HDIR='A')
  !
...
IF (NRANK==NPIO) THEN
  ...
  IF (HALG=='LIN ') THEN
    !
    CALL SET_NB_POINTS_LIN(NPROC,NPROC-1,U%NDIM_FULL,NINDEX,OSHADOWS)
    !
  ELSEIF (HALG=='ADJ ' .OR. HALG=='TILA' .OR. HALG=='TILL') THEN
    ...
    IF (HALG=='ADJ ' .OR. HALG=='TILA') THEN
      ...
      CALL GET_ADJACENT_MESHES(UG%CGRID,UG%NGRID_PAR,U%NDIM_FULL,UG%XGRID_FULL_PAR,ILEFT,IRIGHT,ITOP,IBOTTOM)
    ELSE
      ALLOCATE(ILEFT  (0))
      ALLOCATE(IRIGHT (0))
      ALLOCATE(IBOTTOM(0))
    ...
    !
    IF (HALG=='ADJ ') THEN
      CALL SET_NB_POINTS_ADJ(NPROC,U%NDIM_FULL,U%NDIM_FULL,ILEFT,IRIGHT,ITOP,IBOTTOM,NINDEX,OSHADOWS)
...
NSIZE_TASK(:) = 0
!
 CALL GET_SIZE_FULL_n(HPROGRAM,U%NDIM_FULL,NSIZE_TASK(NRANK))
...
!
  ...
  ELSE
    ...
    DO JVEGTYPE=1,NVEGTYPE
      CALL AV_PGD (ZVEGTYPE(:,JVEGTYPE),ZCOVER,DTCO%XDATA_VEGTYPE(:,JVEGTYPE),'NAT','ARI',GCOVER)
    END DO
  ENDIF
!
  CALL END_IO_SURF_n(HPROGRAM)
