...
USE MODD_SURFEX_MPI, ONLY : NRANK, NPIO, NPROC, NCOMM, NINDEX, NSIZE_TASK, NSIZE, WLOG_MPI, &
                            NREQ, NNUM, NDIM_FULL_INIT, LSFX_MPI
!
...
IF (NPROC>1) THEN
#ifdef SFX_MPI
  IF( LSFX_MPI) CALL MPI_BCAST(NSIZE_TASK,SIZE(NSIZE_TASK)*KIND(NSIZE_TASK)/4,MPI_INTEGER,NPIO,NCOMM,INFOMPI)
#endif
...
!
 CALL END_IO_SURF_n(HPROGRAM)
!
 CALL INI_DATA_COVER(HPROGRAM, DTCO, U)
!
! Algo
!
IF (NRANK==NPIO) THEN
