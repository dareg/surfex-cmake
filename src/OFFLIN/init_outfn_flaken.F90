...
IF (N2M>0) THEN
  ...
  CALL DEF_VAR_NETCDF(IFILE_ID,'W10M_WAT','10m_Wind_Strength     '   ,IDDIM,YATT_TITLE,YATT)
  CALL DEF_VAR_NETCDF(IFILE_ID,'W10MMAX_WAT','Maximum_10m_Wind_Strength     '   ,IDDIM,YATT_TITLE,YATT)  
ENDIF
!
IF (N2M>0) THEN
  YATT='-'
...
IF(LSURF_BUDGET) THEN
  ...
  YATT = 'kg/m2s'
  CALL DEF_VAR_NETCDF(IFILE_ID,'EVAP_WAT'   ,'Averaged_Total_Evaporation'                ,IDDIM,YATT_TITLE,YATT)
  CALL DEF_VAR_NETCDF(IFILE_ID,'SUBL_WAT'   ,'Averaged_Sublimation_of_ice/snow'          ,IDDIM,YATT_TITLE,YATT)  
ENDIF
!
IF (LSURF_BUDGET.OR.LSURF_BUDGETC)  THEN
   YATT='-'
   CALL DEF_VAR_NETCDF(IFILE_ID,'TALB_WAT'  , 'Flake_total_albedo         '               ,IDDIM,YATT_TITLE,YATT)
   YATT='kg/m2'
   CALL DEF_VAR_NETCDF(IFILE_ID,'WSN_WAT'   , 'Flake_snow_water_equivalent'               ,IDDIM,YATT_TITLE,YATT)
ENDIF
!
...
IF (LSURF_BUDGETC) THEN
  ...
  YATT='kg/m2'  
  CALL DEF_VAR_NETCDF(IFILE_ID,'EVAPC_WAT'  ,'Cumulated_Averaged_Total_Evaporation'      ,IDDIM,YATT_TITLE,YATT)
  CALL DEF_VAR_NETCDF(IFILE_ID,'SUBLC_WAT'  ,'Cumulated_Averaged_Sublimation_of_ice/snow',IDDIM,YATT_TITLE,YATT)  
  !     
  CALL OL_WRITE_COORD(YFILE,IFILE_ID,IDDIM,YATT_TITLE,YNAME_DIM,YUNIT1,YUNIT2,IDIM1,YDATE,ZX,ZY)
  !
ENDIF
...
YATT='m/s'
CALL DEF_VAR_NETCDF(IFILE_ID,'USTAR_WATER','friction velocity           '       ,IDDIM,YATT_TITLE,YATT)
!
!
IF (LSBL) THEN
  DO JLAYER=1,NLVL
