...
USE MODD_ISBA_n,           ONLY : CISBA, CPHOTO, LTR_ML, CRUNOFF, CRAIN,        &
                                  ...
                                  NTEMPLAYER_ARP, TSNOW, TTIME, CHORT,          &
                                  NPATCH, NWG_LAYER
...
IF(.NOT.LPROVAR_TO_DIAG)THEN
  ...
  !
  IF(CISBA=='DIF')THEN
    IL=MAXVAL(NWG_LAYER(:,:),NWG_LAYER(:,:)/=NUNDEF)
  ELSE
    IL=INLVLD
  ENDIF
  DO JLAYER=1,IL
...
IF (LSURF_EVAP_BUDGET) THEN
  ...
  IF(LPATCH_BUDGET) THEN      
    ...
    CALL DEF_VAR_NETCDF(IFILE_ID,'LES_PATCH'    ,'Snow_Evaporation_Heat_Flux'                          ,IDDIM,YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(IFILE_ID,'LER_PATCH'    ,'Canopy_Water_Interception_Evaporation'               ,IDDIM,YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(IFILE_ID,'LETR_PATCH'   ,'Vegetation_Evapotranspiration'                       ,IDDIM,YATT_TITLE,YATT)
    YATT = 'kg/m2s'
...
IF (LSURF_MISC_BUDGET) THEN
  ...
  IF(CISBA=='DIF')THEN
    ...
    CALL DEF_VAR_NETCDF(IFILE_ID,'TSWI_D_ISBA'    ,'Total_swi_over_deep_soil   ',JDIM,YATT_TITLE,YATT)    
  ENDIF  
  !
  YATT = 'kg/m2'
  ...
  IF(CISBA=='DIF')THEN
    CALL DEF_VAR_NETCDF(IFILE_ID,'WGTOT_S_ISBA','Total_soil_water_reservoir_(liquid+solid)_over_surface  ',JDIM,YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(IFILE_ID,'WGTOT_R_ISBA','Total_soil_water_reservoir_(liquid+solid)_over_root_zone',JDIM,YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(IFILE_ID,'WGTOT_D_ISBA','Total_soil_water_reservoir_(liquid+solid)_over_deep_soil',JDIM,YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(IFILE_ID,'WGI_S_ISBA'  ,'Total_soil_ice_reservoir_over_surface'                   ,JDIM,YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(IFILE_ID,'WGI_R_ISBA'  ,'Total_soil_ice_reservoir_over_root_zone'                 ,JDIM,YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(IFILE_ID,'WGI_D_ISBA'  ,'Total_soil_ice_reservoir_over_deep_soil'                 ,JDIM,YATT_TITLE,YATT)
  ENDIF  
  !
  IF (TSNOW%SCHEME=='3-L' .OR. TSNOW%SCHEME=='CRO') THEN
    YATT = 'K'          
...
IF (LSURF_BUDGETC) THEN
  ...
  IF(LPATCH_BUDGET)THEN      
    ...
    CALL DEF_VAR_NETCDF(IFILE_ID,'LESC_PATCH'   ,'Cumulated_Snow_Evaporation_Heat_Flux'               ,IDDIM,YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(IFILE_ID,'LERC_PATCH'   ,'Cumulated_Canopy_Water_Interception_Evaporation'    ,IDDIM,YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(IFILE_ID,'LETRC_PATCH'  ,'Cumulated_Vegetation_Evapotranspiration'            ,IDDIM,YATT_TITLE,YATT)
    IF(LRAD_BUDGET)THEN
