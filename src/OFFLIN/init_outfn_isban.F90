...
!
!
IF (IM%ID%O%LPATCH_BUDGET.AND.LAGRIP .AND. (IM%O%CPHOTO=='NIT' .OR. IM%O%CPHOTO=='LAI' &
        .OR. IM%O%CPHOTO=='LST' .OR. IM%O%CPHOTO=='NCB')) THEN
...
IF (IM%CHI%SVI%NBEQ>0 .AND. IM%CHI%CCH_DRY_DEP=="WES89 ") THEN
  ...
  DO JSV = 1,SIZE(IM%CHI%CCH_NAMES,1)
    !
    YRECFM ='DV_NAT_'//TRIM(IM%CHI%CCH_NAMES(JSV))
...
ELSEIF(IM%ID%O%LPGD)THEN
  ...
  DO JP = 1,IM%O%NPATCH
    ...
    ENDIF
    !  
    IF (LAGRIP .AND. (IM%O%CPHOTO=='NIT' .OR. IM%O%CPHOTO=='LAI' .OR. IM%O%CPHOTO=='LST' .OR. IM%O%CPHOTO=='NCB') ) THEN
