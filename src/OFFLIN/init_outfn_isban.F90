...
IF ( OSNOWDIMNC ) INSNLAYER = ISNOW_LAYER
 CALL OL_DEFINE_DIM(UG, U%NSIZE_FULL, HPROGRAM, KLUOUT, INI, IDIM1, YUNIT1, YUNIT2,&
                    ZX, ZY, IDIMS, IDDIMALL, YNAME_DIM, KNSNLAYER=INSNLAYER)
...
IF (.NOT. IM%ID%DM%LPROSNOW) THEN
  ...
  CALL OL_WRITE_COORD(HSELECT,YFILE,IFILE_ID,IDDIMALL,YATT_TITLE,YNAME_DIM,YUNIT1,YUNIT2,IDIM1,YDATE,ZX,ZY)
  !
  !
