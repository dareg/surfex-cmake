...
!     #########
       SUBROUTINE INIT_OUTFN_ISBA_n (IM, UG, U, HSELECT, OPROVAR_TO_DIAG, HPROGRAM, KLUOUT)
!     ###############################
!
!
...
!
IF(IM%O%LTEMP_ARP)THEN
  IL=IM%O%NTEMPLAYER_ARP
ELSEIF(IM%O%CISBA=='DIF')THEN
  IL=INLVLD
ELSE
  IL=2
ENDIF
...
ENDIF
!
IF (IM%O%CPHOTO =='NIT' .OR. IM%O%CPHOTO =='NCB') THEN
...
!
!
IF (IM%ID%O%LPATCH_BUDGET.AND.LAGRIP .AND. (IM%O%CPHOTO=='NIT' .OR. IM%O%CPHOTO=='NCB')) THEN
  CALL DEF_VAR_NETCDF(HSELECT,IFILE_ID,'IRRISEUIL','Irrigation_Threshold',JDIM,YATT_TITLE,YATT)
ENDIF
!
IF (IM%ID%DE%LSURF_EVAP_BUDGET) THEN
  !
  YATT ='W/m2'
  ...
  ENDIF
  IF(IM%O%CRUNOFF=='SGH'.AND.IM%O%CISBA=='DIF')THEN  
  ...
  ENDIF
  IF(IM%ID%DE%LWATER_BUDGET)THEN 
    YATT ='kg/m2s'
...
IF (IM%ID%DM%LSURF_MISC_BUDGET) THEN
  ...
  IF(IM%O%CISBA=='DIF'.AND.IM%ID%DM%LSURF_MISC_DIF)THEN
  ...
  !
  IF(IM%O%CRUNOFF=='SGH'.OR.IM%O%CRUNOFF=='DT92')THEN
    YATT ='-'
...
ENDIF
!
IF (IM%CHI%SVI%NBEQ>0 .AND. IM%CHI%CCH_DRY_DEP=="WES89 ") THEN
  !
  YATT="(m/s)"
  !
  DO JSV = 1,SIZE(IM%CHI%CCH_NAMES,1)
    !
    YRECFM ='DVNT'//TRIM(IM%CHI%CCH_NAMES(JSV))
    ...
    DO JP = 1,IM%O%NPATCH
      ...
      YPAT = "P"//ADJUSTL(YPAT(:LEN_TRIM(YPAT)))
      !
      CALL DEF_VAR_NETCDF(HSELECT,IFILE_ID,TRIM(ADJUSTL(YRECFM))//YPAT,YCOMMENT,JDIM,YATT_TITLE,YATT)
      !
    ENDDO
    !
  ENDDO
...
IF (IM%CHI%SVI%NBEQ>0 .AND. IM%CHI%LCH_BIO_FLUX) THEN
  !
  IF (ASSOCIATED(IM%GB%XFISO)) THEN
    YRECFM='FISO'
...
IF (IM%CHI%LCH_NO_FLUX) THEN
  !
  IF (ASSOCIATED(IM%GB%XNOFLUX)) THEN
    YRECFM='NOFLUX'
...
ELSEIF(IM%ID%O%LPGD)THEN
  ...
  DO JVEG=1,NVEGTYPE
     ...
     CALL DEF_VAR_NETCDF(HSELECT,IFILE_ID,'VEGTYPE'//YLVLV,&
             'fraction_of_vegtype_in_the_grid_cell',JDIM(1:INDIMS-1),YATT_TITLE,(/'-'/))
  ENDDO
  !    
  ...
  DO JP = 1,IM%O%NPATCH
    ...
    CALL DEF_VAR_NETCDF(HSELECT,IFILE_ID,'ALBNIR_S'//YPAT,'Output_ALBNIR_SOIL'//YPAT,JDIM(1:INDIMS-1),YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(HSELECT,IFILE_ID,'ALBVIS_S'//YPAT,'Output_ALBVIS_SOIL'//YPAT,JDIM(1:INDIMS-1),YATT_TITLE,YATT)
    CALL DEF_VAR_NETCDF(HSELECT,IFILE_ID,'ALBUV_S'//YPAT,'soil_UV_albedo'//YPAT,JDIM(1:INDIMS-1),YATT_TITLE,YATT)
    ...
    ENDIF
    !  
    IF (LAGRIP .AND. (IM%O%CPHOTO=='NIT' .OR. IM%O%CPHOTO=='NCB') ) THEN
