...
!     #########
       SUBROUTINE INIT_OUTFN_TEB_n (TM, GDM, GRM, UG, U, HSELECT, HPROGRAM, KLUOUT, DGO)
!     ###############################
!
!
...
!
TYPE(DIAG_OPTIONS_t),OPTIONAL,  INTENT(INOUT) :: DGO
!
...
IF (LHOOK) CALL DR_HOOK('INIT_OUTFN_TEB_N',0,ZHOOK_HANDLE)
 ...
 CALL OL_DEFINE_DIM(UG, U%NSIZE_FULL, HPROGRAM, KLUOUT, INI, DGO%LSNOWDIMNC,&
		   						 GDM%NPE%AL(1)%TSNOW, IDIM1, YUNIT1, YUNIT2, &
                   ZX, ZY, IDIMS, IDDIM, YNAME_DIM,.FALSE.,PLAT=ZLAT,PLON=ZLON)
...
DEALLOCATE(IDIMS, IDDIM, YNAME_DIM)
 ...
 CALL OL_DEFINE_DIM(UG, U%NSIZE_FULL, HPROGRAM, KLUOUT, INI, DGO%LSNOWDIMNC, &
									  GDM%NPE%AL(1)%TSNOW,IDIM1, YUNIT1, YUNIT2, &
                    ZX, ZY, IDIMS, IDDIM, YNAME_DIM, .FALSE., KNSNLAYER=1)
  !
...
IF (TM%TOP%LGARDEN) THEN
  ...
  !
    CALL OL_DEFINE_DIM(UG, U%NSIZE_FULL, HPROGRAM, KLUOUT, INI, DGO%LSNOWDIMNC, &
											GDM%NPE%AL(1)%TSNOW, IDIM1, YUNIT1, YUNIT2, &
                      ZX, ZY, IDIMS, IDDIM, YNAME_DIM,.FALSE., &
											KNSNLAYER=GRM%NPE%AL(1)%TSNOW%NLAYER)
    !
