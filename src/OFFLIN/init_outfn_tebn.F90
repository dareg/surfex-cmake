!     #########
       SUBROUTINE INIT_OUTFN_TEB_n (BOP, CHT, DGMTO, DGU, DGT, DGUT, UG, U, TCP, TGD, TGDO, TGR, TGRO, T, TOP, &
                                    HPROGRAM,KLUOUT)
!     ###############################
!
!
...
IF (DGMTO%LSURF_MISC_BUDGET) THEN
  ...
  DO JPATCH=1,TOP%NTEB_PATCH
    ...
    IF (TOP%LGREENROOF) THEN
      ...
      YATT='(kg/m2/s)'
      CALL DEF_VAR_NETCDF(DGU, &
                      IFILE_ID,ADJUSTL(YPATCH//'RUNOFF_GR'),'GREENROOF soil surface runoff',IDDIM,YATT_TITLE,YATT)
      CALL DEF_VAR_NETCDF(DGU, &
...
DO JPATCH=1,TOP%NTEB_PATCH
  ...
  IF (TOP%LGREENROOF) THEN
    ...
    !
    IF (TGRO%CPHOTO/='NON' .AND. TGRO%CPHOTO/='AGS' .AND. TGRO%CPHOTO/='AST') THEN
      CALL DEF_VAR_NETCDF(DGU, &
                      IFILE_ID,ADJUSTL(YPATCH//'GR_LAI'), 'GR_LAI', IDDIM, YATT_TITLE, (/'(m2/m2)'/))
    ENDIF
    !
    IF (TGRO%CPHOTO=='NIT') THEN
      DO JNBIOMASS=1,TGRO%NNBIOMASS
        WRITE(YPAS,'(I3)') JNBIOMASS ; YLVL=ADJUSTL(YPAS(:LEN_TRIM(YPAS)))
        CALL DEF_VAR_NETCDF(DGU, &
                      IFILE_ID,ADJUSTL(YPATCH//'GR_BIOMA'), 'GR_BIOMA', IDDIM, YATT_TITLE, (/'(kg/m2)'/))
      ENDDO
      DO JNBIOMASS=2,TGRO%NNBIOMASS
        WRITE(YPAS,'(I3)') JNBIOMASS ; YLVL=ADJUSTL(YPAS(:LEN_TRIM(YPAS)))
        CALL DEF_VAR_NETCDF(DGU, &
