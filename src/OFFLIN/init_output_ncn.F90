  ...
  IF (.NOT.GEXIST .OR. .NOT.LCREATED .OR. CFILEOUT_NC/=CFILEOUT_NC_SAVE) THEN
    ...
    IF (CSOFTWARE=='PGD') THEN
      CALL OL_DEFINE_DIM(UG, U%NSIZE_FULL, 'NOTIME ', ILUOUT, IFULL, IDIM1, YUNIT1, YUNIT2, &
                         ZX, ZY, IDIMS, IDDIM, YNAME_DIM, KNVEGTYPE = NVEGTYPE)            
    ELSE
      CALL OL_DEFINE_DIM(UG, U%NSIZE_FULL, 'NOTIME ', ILUOUT, IFULL, IDIM1, YUNIT1, YUNIT2, &
    ...
    IF (UG%G%CGRID=='IGN') THEN
      ...
      YATT(1) = "longitude"
      CALL DEF_VAR_NETCDF(HSELECT,NID_NC,'XLON','XLON',IDDIM(1:1),YATT_TITLE,YATT,ID0,NF_DOUBLE)
      YATT(1) = "latitude"
      CALL DEF_VAR_NETCDF(HSELECT,NID_NC,'XLAT','XLAT',IDDIM(1:1),YATT_TITLE,YATT,ID0,NF_DOUBLE)
    ENDIF      
    CFILEOUT_NC_SAVE = CFILEOUT_NC
    LCREATED = .TRUE.
    IF (CSOFTWARE=='PGD') IRET = NF_DEF_DIM(NID_NC,"Nb_of_input_data",NVEGTYPE*IM%DTV%NTIME,IDIMID)
