...
IF (NRANK==NPIO) THEN
  ...
  IF (YSC%U%NDIM_NATURE>0) THEN
    IF (YSC%U%CNATURE=='ISBA  '.OR.YSC%U%CNATURE=='TSZ0  ') THEN 
      CALL INIT_OUTFN_ISBA_n(YSC%IM, YSC%UG, YSC%U, YSC%DUO%CSELECT, YSC%DUO%LPROVAR_TO_DIAG, "NC    ",ILUOUT)
  ...
  IF (YSC%U%NDIM_WATER>0) THEN
    ...
    IF (YSC%U%CWATER=='FLAKE ') CALL INIT_OUTFN_FLAKE_n(YSC%WM%CHW, YSC%FM, YSC%UG, YSC%U, YSC%DUO%CSELECT, "NC    ",ILUOUT)
