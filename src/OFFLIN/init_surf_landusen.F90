...
!
 CALL GET_TYPE_DIM_n(YSC%DTCO, YSC%U, &
                     'NATURE',ILU)
ALLOCATE(ZWORK(ILU,YSC%IM%I%O%NPATCH))
!
...
!       
ALLOCATE(YSC%IM%I%IP%XPATCH_OLD(ILU,YSC%IM%I%O%NPATCH))       
YRECFM = 'PATCH'
 CALL READ_SURF(&
                HPROGRAM,YRECFM,YSC%IM%I%IP%XPATCH_OLD(:,:),IRESP)
!
!* read old soil layer thicknesses (m)
!
ALLOCATE(YSC%IM%I%M%X%XDG_OLD(ILU,YSC%IM%I%O%NGROUND_LAYER,YSC%IM%I%O%NPATCH))
!
DO JLAYER=1,YSC%IM%I%O%NGROUND_LAYER
  WRITE(YLVL,'(I4)') JLAYER
  YRECFM='OLD_DG'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
  CALL READ_SURF(&
                HPROGRAM,YRECFM,ZWORK(:,:),IRESP)
  YSC%IM%I%M%X%XDG_OLD(:,JLAYER,:)=ZWORK
END DO
DEALLOCATE(ZWORK)
!
!* End of IO
