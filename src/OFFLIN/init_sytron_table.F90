SUBROUTINE INIT_SYTRON_TABLE(I,PZS,KI,PLAT,PLON)
...
GDISTRIB=.FALSE.
ZSLOPE_DEG=180./XPI*ATAN(I%XSSO_SLOPE(JI))
...
ZSLOPE_DEG2=180./XPI*ATAN(I%XSSO_SLOPE(JJ))
IF(PLAT(JJ)==PLAT(JI) .AND. PLON(JJ)==PLON(JI)) THEN ! Points JJ and JI belong to the same massif
    IF(PZS(JJ) == PZS(JI) .AND. NINT(ZSLOPE_DEG)==NINT(ZSLOPE_DEG2)) THEN   ! Points JJ and JI have the same elevation and the same slope
          ZASP_OPPOSITE = I%XSSO_DIR(JJ)+180.
          IF(ZASP_OPPOSITE > 360.) ZASP_OPPOSITE=ZASP_OPPOSITE-360.

          IF(ZASP_OPPOSITE==I%XSSO_DIR(JI)) THEN  ! Point JJ is the opposite point of JI
               NTAB_SYT(JI)=JJ
               NTAB_SYT(JJ)=JI
               EXIT
          ENDIF 
