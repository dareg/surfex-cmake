...
IF (NRANK==NPIO) THEN
  ...
  IF (NID_NC.NE.0) THEN
    ...
    !----------------------------
    IF (ITYPE==NF90_DOUBLE) THEN
      IRET(1)=NF90_GET_VAR(NID_NC,IVAR_ID,ZWORK)
    ELSEIF (ITYPE==NF90_FLOAT) THEN
      ALLOCATE(ZTAB_1D4(SIZE(ZWORK)))
      IRET(2)=NF90_GET_VAR(NID_NC,IVAR_ID,ZTAB_1D4)
      ZWORK(:) = ZTAB_1D4(:)
      DEALLOCATE(ZTAB_1D4)
    ENDIF
    !
...
IF (NRANK==NPIO) THEN
  ...
  IF (NID_NC.NE.0) THEN
    ...
    !
    IF (ITYPE==NF90_DOUBLE) THEN
      IRET(2)=NF90_GET_VAR(NID_NC,IVAR_ID,ZWORK2)
    ELSEIF (ITYPE==NF90_FLOAT) THEN
      ALLOCATE(ZTAB_2D4(SIZE(ZWORK2,1),SIZE(ZWORK2,2)))
      IRET(2)=NF90_GET_VAR(NID_NC,IVAR_ID,ZTAB_2D4)
      ZWORK2(:,:) = ZTAB_2D4(:,:)
      DEALLOCATE(ZTAB_2D4)
    ENDIF      
    !
...
IF (NRANK==NPIO) THEN
  ...
  IF (NID_NC.NE.0) THEN
    ...
    ! 2. Get variable
    !----------------------------
    IRET(1)=NF90_GET_VAR(NID_NC,IVAR_ID,YTAB_1D)
    !
