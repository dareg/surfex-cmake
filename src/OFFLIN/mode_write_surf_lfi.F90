...
!
!     #############################################################
      SUBROUTINE WRITE_SURFX0_LFI (IOB, &
...
!
 CALL IO_BUFF_n(IOB, &
                HREC,'W',LWORK0)
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX0_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFN0_LFI (IOB, &
...
!
 CALL IO_BUFF_n(IOB, &
                HREC,'W',LWORK0)
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFN0_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFL0_LFI (IOB, &
...
!
 CALL IO_BUFF_n(IOB, &
                HREC,'W',LWORK0)
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFL0_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFC0_LFI (IOB, &
...
!
 CALL IO_BUFF_n(IOB, &
                HREC,'W',LWORK0)
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFC0_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFX1_LFI (IOB, &
...
!$OMP END SINGLE
!
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
...
!$OMP SINGLE
  ...
  IF (HDIR=='H') THEN
    ...
    IF (.NOT. LMNH_COMPATIBLE) THEN
      CALL FMWRITX1(CFILEOUT_LFI,HREC,CLUOUT_LFI,NFULL,ZWORK,4,100,HCOMMENT,NWORKB)
    ...
    ELSE
      ...
      IF     (HREC=='DX              ' .OR. HREC=='XX              ') THEN
        YREC = 'XHAT'
        CALL WRITE_IN_LFI_X1_FOR_MNH(HREC,YREC,ZWORK2D(NIB:NIE,NJB),NWORKB,HCOMMENT,NIU,NIB,NIE)
      ELSEIF (HREC=='DY              ' .OR. HREC=='YY              ') THEN
        YREC = 'YHAT'
        CALL WRITE_IN_LFI_X1_FOR_MNH(HREC,YREC,ZWORK2D(NIB,NJB:NJE),NWORKB,HCOMMENT,NJU,NJB,NJE)
      ELSEIF (NJB==NJE) THEN
         YREC = HREC
        CALL WRITE_IN_LFI_X1_FOR_MNH(HREC,YREC,ZWORK2D(:,NJB),NWORKB,HCOMMENT,NIU,NIB,NIE)
      ELSEIF (NIB==NIE) THEN
        YREC = HREC
        CALL WRITE_IN_LFI_X1_FOR_MNH(HREC,YREC,ZWORK2D(NIB,:),NWORKB,HCOMMENT,NJU,NJB,NJE)
      ELSE
        CALL FMWRITX2(CFILEOUT_LFI,HREC,CLUOUT_LFI,SIZE(ZWORK2D),ZWORK2D,4,100,HCOMMENT,NWORKB)
        CALL ERROR_WRITE_SURF_LFI(HREC,NWORKB)
      ENDIF
      !
    END IF
    !
  ELSE
    CALL FMWRITX1(CFILEOUT_LFI,HREC,CLUOUT_LFI,SIZE(PFIELD),PFIELD,4,100,HCOMMENT,NWORKB)
    CALL ERROR_WRITE_SURF_LFI(HREC,NWORKB)
  END IF
  ! 
!$OMP END SINGLE
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFX2_LFI (IOB, &
                                   HREC,PFIELD,KRESP,HCOMMENT,HDIR)
!     #############################################################
!
!!****  * - routine to fill a write 2D array for the externalised surface 
...
!$OMP END SINGLE
!
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX2_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
IF (HDIR=='H') CALL GATHER_AND_WRITE_MPI(PFIELD,ZWORK,NMASK)
!
...
!$OMP SINGLE
  ...
  IF (HDIR=='H') THEN
    ...
    !
    IF (.NOT. LMNH_COMPATIBLE) THEN
      CALL FMWRITX2(CFILEOUT_LFI,HREC,CLUOUT_LFI,SIZE(ZWORK),ZWORK,4,100,HCOMMENT,NWORKB)
      CALL ERROR_WRITE_SURF_LFI(HREC,NWORKB)
    ELSE
      CALL WRITE_IN_LFI_X2_FOR_MNH(HREC,ZWORK,NWORKB,HCOMMENT)
    END IF
    !
  ELSE
    CALL FMWRITX2(CFILEOUT_LFI,HREC,CLUOUT_LFI,SIZE(PFIELD),PFIELD,4,100,HCOMMENT,NWORKB)
    CALL ERROR_WRITE_SURF_LFI(HREC,NWORKB)
  END IF
  !
!$OMP END SINGLE
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFN1_LFI (IOB, &
...
!$OMP END SINGLE
!
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFN1_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
...
!$OMP SINGLE
  !    
  IF (HDIR=='H') THEN
    CALL FMWRITN1(CFILEOUT_LFI,HREC,CLUOUT_LFI,NFULL,IWORK,4,100,HCOMMENT,NWORKB)
  ELSE
    CALL FMWRITN1(CFILEOUT_LFI,HREC,CLUOUT_LFI,SIZE(KFIELD),KFIELD,4,100,HCOMMENT,NWORKB)
  END IF
  !  
!$OMP END SINGLE 
  !
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFL1_LFI (IOB, &
...
!$OMP END SINGLE
!
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFL1_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
IF (NRANK==NPIO) THEN
  !
...
!$OMP SINGLE
    !
    CALL FMWRITL1(CFILEOUT_LFI,HREC,CLUOUT_LFI,SIZE(OFIELD),OFIELD,4,100,HCOMMENT,NWORKB)
    !
!$OMP END SINGLE
    !    
    CALL ERROR_WRITE_SURF_LFI(HREC,NWORKB)
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFT0_LFI (IOB, &
...
!
 CALL IO_BUFF_n(IOB, &
                HREC,'W',LWORK0)
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFT0_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
ITDATE(1) = KYEAR
ITDATE(2) = KMONTH
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFT1_LFI (IOB, &
                                   HREC,KYEAR,KMONTH,KDAY,PTIME,KRESP,HCOMMENT)
!     #############################################################
!
!!****  * - routine to write a date
...
!$OMP END SINGLE
!
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFT1_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
IF (NRANK==NPIO) THEN
  !
...
!$OMP SINGLE
  ...
  !
  YREC=TRIM(HREC)//'%TDATE'
  CALL FMWRITN2(CFILEOUT_LFI,YREC,CLUOUT_LFI,SIZE(ITDATE),ITDATE,4,100,HCOMMENT,NWORKB)
  !
  YREC=TRIM(HREC)//'%TIME'
  CALL FMWRITX1(CFILEOUT_LFI,YREC,CLUOUT_LFI,SIZE(PTIME),PTIME,4,100,HCOMMENT,NWORKB)
  !
!$OMP END SINGLE
  !   
  CALL ERROR_WRITE_SURF_LFI(HREC,NWORKB)
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFT2_LFI (IOB, &
                                   HREC,KYEAR,KMONTH,KDAY,PTIME,KRESP,HCOMMENT)
!     #############################################################
!
!!****  * - routine to write a date
...
!$OMP END SINGLE
!
IF (LWORK0 .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFT2_LFI',1,ZHOOK_HANDLE)
IF (LWORK0) RETURN
!
IF (NRANK==NPIO) THEN
  !
...
!$OMP SINGLE
  ...
  !
  YREC=TRIM(HREC)//'%YEAR'
  CALL FMWRITN2(CFILEOUT_LFI,YREC,CLUOUT_LFI,SIZE(KYEAR),KYEAR,4,100,HCOMMENT,NWORKB)
  !
  YREC=TRIM(HREC)//'%MONTH'
  CALL FMWRITN2(CFILEOUT_LFI,YREC,CLUOUT_LFI,SIZE(KMONTH),KMONTH,4,100,HCOMMENT,NWORKB)
  !
  YREC=TRIM(HREC)//'%DAY'
  CALL FMWRITN2(CFILEOUT_LFI,YREC,CLUOUT_LFI,SIZE(KDAY),KDAY,4,100,HCOMMENT,NWORKB)  
  !
  YREC=TRIM(HREC)//'%TIME'
  CALL FMWRITX2(CFILEOUT_LFI,YREC,CLUOUT_LFI,SIZE(PTIME),PTIME,4,100,HCOMMENT,NWORKB)
  !
!$OMP END SINGLE
  !   
  CALL ERROR_WRITE_SURF_LFI(HREC,NWORKB)
