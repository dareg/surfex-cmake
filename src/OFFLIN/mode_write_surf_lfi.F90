...
!
IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_1',0,ZHOOK_HANDLE)
!
KRESP=0
!
 CALL IO_BUFF(&
                HREC,'W',GFOUND)
!
IF (GFOUND .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_1',1,ZHOOK_HANDLE)
IF (GFOUND) RETURN
!
IF (HDIR=='H') CALL GATHER_AND_WRITE_MPI(PFIELD,ZWORK,NMASK)
!  
IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_1',1,ZHOOK_HANDLE)
!
IF (NRANK==NPIO) THEN
  !
  IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_2',0,ZHOOK_HANDLE)
  !
#ifdef SFX_MPI
  XTIME0 = MPI_WTIME()
#endif
  !   
  IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_2',1,ZHOOK_HANDLE)
  !
  IF (HDIR=='H') THEN
    !
    IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_3',0,ZHOOK_HANDLE)
    CALL GET_SURF_UNDEF(ZUNDEF)
    IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_3',1,ZHOOK_HANDLE)
    !
    IF (.NOT. LMNH_COMPATIBLE) THEN
      IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_4',0,ZHOOK_HANDLE)
      CALL FMWRITX1(CFILEOUT_LFI,HREC,CLUOUT_LFI,NFULL,ZWORK,4,100,HCOMMENT,KRESP)
      CALL ERROR_WRITE_SURF_LFI(HREC,KRESP)
      IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_4',1,ZHOOK_HANDLE)
    ELSE
      !
!$OMP PARALLEL PRIVATE(ZHOOK_HANDLE_OMP)  
IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_51',0,ZHOOK_HANDLE_OMP)
!$OMP DO PRIVATE(JJ,JI)
      ...
      END DO
!$OMP END DO
IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_51',1,ZHOOK_HANDLE_OMP)
!$OMP END PARALLEL      
      !
      IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_52',0,ZHOOK_HANDLE)      
      ...
      ENDIF
      !
      IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_52',1,ZHOOK_HANDLE)
      !
    END IF
    !
  ELSE
    IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_6',0,ZHOOK_HANDLE)
    CALL FMWRITX1(CFILEOUT_LFI,HREC,CLUOUT_LFI,SIZE(PFIELD),PFIELD,4,100,HCOMMENT,KRESP)
    CALL ERROR_WRITE_SURF_LFI(HREC,KRESP)
    IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_6',1,ZHOOK_HANDLE)
  END IF
  ! 
  IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_7',0,ZHOOK_HANDLE)
#ifdef SFX_MPI
  XTIME_NPIO_WRITE = XTIME_NPIO_WRITE + (MPI_WTIME() - XTIME0)
#endif
  IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_LFI:WRITE_SURFX1_LFI_7',1,ZHOOK_HANDLE)
  !  
ENDIF
!
