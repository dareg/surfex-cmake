...
IF (NRANK==NPIO) THEN 
  ...
  IF (NID_NC /= 0) THEN
    ...
    IRET(2) = NF_INQ_DIMID(NID_NC,TRIM(YNAM_DIM),IDIMIDS(1))
    IF (IRET(2)/=0) THEN
      ...
      ENDIF
      IRET0=NF_INQ_DIMLEN(NID_NC,IDIMIDS(2),IDIMLEN(2))
    ENDIF
    IRET0=NF_INQ_DIMLEN(NID_NC,IDIMIDS(1),IDIMLEN(1))
    !
    IRET(4)=NF_INQ_DIMNAME(NID_NC,IDIMIDS(1),YNAME)
    !
    DO JRET=1,4
      IF (IRET0==0 .OR. IRET(JRET).NE.NF_NOERR) KRESP=1
    ENDDO
    !
  ELSE
    INDIMS = 0
...
IF (YNAME.NE.'lon' .AND. YNAME.NE.'xx') THEN
  IF (LDEF) THEN
    IF (NRANK==NPIO) THEN
      CALL DEF_VAR_NETCDF( HSELECT, NID_NC, HREC, HREC, IDIMIDS(1:1), YATT_TITLE, YATT, IVAR_ID, NF_DOUBLE,1)
    ENDIF
  ELSE
...
ELSE
  IF (LDEF) THEN
    IF (NRANK==NPIO) THEN
      CALL DEF_VAR_NETCDF( HSELECT, NID_NC, HREC, HREC, IDIMIDS(1:2), YATT_TITLE, YATT, IVAR_ID, NF_DOUBLE)
    ENDIF 
  ELSE  
    JRET = NF_INQ_VARID(NID_NC,HREC,IVAR_ID)
...
IF (NRANK==NPIO) THEN
  ...
  IF (NID_NC /= 0) THEN 
    ...
    IF ( TRIM(YNAM_DIM) == "Nemis_snap" ) THEN
      IRET(2) = NF_INQ_DIMID(NID_NC,"Nsnap_temp",IDIMIDS(1))
    ELSE
      IRET(2) = NF_INQ_DIMID(NID_NC,"Number_of_points",IDIMIDS(1))
    ENDIF
    IF (IRET(2)==0) THEN
      IRET(3) = NF_INQ_DIMID(NID_NC,TRIM(YNAM_DIM),IDIMIDS(2))
    ELSE
      IRET(2) = NF_INQ_DIMID(NID_NC,'lon',IDIMIDS(1))
      IF (IRET(2)/=0) THEN
        IRET(2) = NF_INQ_DIMID(NID_NC,'xx',IDIMIDS(1))
      ...
      ELSE
        IRET(3) = NF_INQ_DIMID(NID_NC,'lat',IDIMIDS(2))
      ENDIF
      IRET(4) = NF_INQ_DIMID(NID_NC,TRIM(YNAM_DIM),IDIMIDS(3))
      IRET0=NF_INQ_DIMLEN(NID_NC,IDIMIDS(3),IDIMLEN(3))
    ENDIF           
    DO JDIM=1,2
      IRET0=NF_INQ_DIMLEN(NID_NC,IDIMIDS(JDIM),IDIMLEN(JDIM))
    ENDDO
    !
