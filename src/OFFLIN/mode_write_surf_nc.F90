...
IF (NID_NC /= 0) THEN        
  !
  CALL DEF_VAR_NETCDF(NID_NC, HREC, HREC, IDIMS, YATT_TITLE, YATT, IVAR_ID, NF_CHAR, 1)
  !
  ! 2. Put variable
  !----------------------------
  !
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFX1_NC(HREC,PFIELD,KRESP,HCOMMENT,HDIR,HNAM_DIM)
!     #############################################################
!
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFX2_NC(HREC,PFIELD,KRESP,HCOMMENT,HDIR,HNAM_DIM)
!     #############################################################
!
...
!     #############################################################
      SUBROUTINE WRITE_SURFN1_NC(HREC,KFIELD,KRESP,HCOMMENT,HDIR,HNAM_DIM)
!     #############################################################
!
!!****  *WRITEN0* - routine to read an integer
!
...
IF (YNAME.NE.'lon' .AND. YNAME.NE.'xx') THEN
  IF (NRANK==NPIO) THEN
!$OMP SINGLE
    CALL DEF_VAR_NETCDF(NID_NC, HREC, HREC, IDIMIDS(1:1), YATT_TITLE, YATT, NWORKB, NF_INT)
!$OMP END SINGLE
    IVAR_ID = NWORKB
  ENDIF
  CALL WRITE_DATAN1_NC(IDIMLEN(1),INDIMS)
ELSE
  IF (NRANK==NPIO) THEN
!$OMP SINGLE          
    CALL DEF_VAR_NETCDF(NID_NC, HREC, HREC, IDIMIDS(1:2), YATT_TITLE, YATT, NWORKB, NF_INT)
!$OMP END SINGLE
    IVAR_ID = NWORKB
  ENDIF      
  CALL WRITE_DATAN1_NC(IDIMLEN(1)*IDIMLEN(2),INDIMS)
ENDIF
!
...
CONTAINS
!
SUBROUTINE WRITE_DATAN1_NC(KDIM,KNDIMS)
!
...
!     #############################################################
      SUBROUTINE WRITE_SURFN2_NC(HREC,KFIELD,KRESP,HCOMMENT,HDIR,HNAM_DIM)
!     #############################################################
!
!!****  *WRITEN0* - routine to read an integer
!
...
ELSE
  IF (NRANK==NPIO) THEN
!$OMP SINGLE          
    CALL DEF_VAR_NETCDF(NID_NC, HREC, HREC, IDIMIDS(1:3), YATT_TITLE, YATT, NWORKB, NF_INT)
!$OMP END SINGLE
    IVAR_ID = NWORKB
  ENDIF      
...
IF(.NOT.ALLOCATED(NMASK_IGN))THEN
  IF (LMASK) THEN
    CALL GATHER_AND_WRITE_MPI(KFIELD,ITAB2D,NMASK)
  ELSE 
    CALL GATHER_AND_WRITE_MPI(KFIELD,ITAB2D)
  ENDIF
ELSE
  !ign grid 
  IF (LMASK) THEN
  ...
  ELSE 
    CALL GATHER_AND_WRITE_MPI(KFIELD,IWORK_IGN(1:SIZE(NMASK_IGN),:))
  ENDIF
  CALL UNPACK_SAME_RANK(NMASK_IGN,IWORK_IGN(1:SIZE(NMASK_IGN),:),ITAB2D)
ENDIF
!
IF (NRANK==NPIO) THEN
!$OMP SINGLE
...
!$OMP SINGLE
    ...
    !
    CALL DEF_VAR_NETCDF(NID_NC, HREC, HREC, IDIMIDS(1:1), YATT_TITLE, YATT, IVAR_ID, NF_CHAR, 1)
    !
...
DO JWRK=1,4
  ...
  IF (JRET==0) THEN
    IF (JWRK==1) THEN
      CALL WRITE_SURFN1_NC(YRECFM,KYEAR,IRET(JWRK),HCOMMENT,'-','Number_of_dates ')
    ELSEIF (JWRK==2) THEN
      CALL WRITE_SURFN1_NC(YRECFM,KMONTH,IRET(JWRK),HCOMMENT,'-','Number_of_dates ')
    ELSEIF (JWRK==3) THEN
      CALL WRITE_SURFN1_NC(YRECFM,KDAY,IRET(JWRK),HCOMMENT,'-','Number_of_dates ')
    ELSEIF (JWRK==4) THEN
      CALL WRITE_SURFX1_NC(YRECFM,PTIME,IRET(JWRK),HCOMMENT,'-','Number_of_dates ')
    ENDIF
  ELSE
    IF (JWRK==1) THEN
...
DO JWRK=1,4
  ...
  IF (JRET==0) THEN
    IF (JWRK==1) THEN
      CALL WRITE_SURFN2_NC(YRECFM,KYEAR,IRET(JWRK),HCOMMENT,'-','Number_of_dates ')
    ELSEIF (JWRK==2) THEN
      CALL WRITE_SURFN2_NC(YRECFM,KMONTH,IRET(JWRK),HCOMMENT,'-','Number_of_dates ')
    ELSEIF (JWRK==3) THEN
      CALL WRITE_SURFN2_NC(YRECFM,KDAY,IRET(JWRK),HCOMMENT,'-','Number_of_dates ')
    ELSEIF (JWRK==4) THEN
      CALL WRITE_SURFX2_NC(YRECFM,PTIME,IRET(JWRK),HCOMMENT,'-','Number_of_dates ')
    ENDIF
  ELSE
    IF (JWRK==1) THEN
