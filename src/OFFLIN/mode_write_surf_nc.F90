...
IF (NID_NC /= 0) THEN        
  !
  IF (LDEF) THEN
    CALL DEF_VAR_NETCDF(NID_NC, HREC, HREC, IDIMS, YATT_TITLE, YATT, IVAR_ID, NF_CHAR)
    !
  ELSE
    IRET = NF_INQ_VARID(NID_NC,HREC,IVAR_ID)
    ! 2. Put variable
...
IF (YNAME.NE.'lon' .AND. YNAME.NE.'xx') THEN
  IF (LDEF) THEN
    IF (NRANK==NPIO) THEN
!$OMP SINGLE
      CALL DEF_VAR_NETCDF(NID_NC, HREC, HREC, IDIMIDS(1:1), YATT_TITLE, YATT, NWORKB, NF_DOUBLE,1)
!$OMP END SINGLE
      IVAR_ID = NWORKB
    ENDIF
  ELSE
