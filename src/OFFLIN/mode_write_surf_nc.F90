...
IF (YNAME .EQ. 'Number_of_points') THEN
  IF (NRANK==NPIO) THEN
!$OMP SINGLE
    CALL DEF_VAR_NETCDF(NID_NC, HREC, HREC, IDIMIDS(1:1), YATT_TITLE, YATT, NWORKB, NF_DOUBLE)
!$OMP END SINGLE
    IVAR_ID = NWORKB
  ENDIF
...
ELSE
  IF (NRANK==NPIO) THEN
!$OMP SINGLE          
    CALL DEF_VAR_NETCDF(NID_NC, HREC, HREC, IDIMIDS(1:2), YATT_TITLE, YATT, NWORKB, NF_DOUBLE)
!$OMP END SINGLE
    IVAR_ID = NWORKB
  ENDIF      
...
ELSE
  IF (NRANK==NPIO) THEN
!$OMP SINGLE          
    CALL DEF_VAR_NETCDF(NID_NC, HREC, HREC, IDIMIDS(1:3), YATT_TITLE, YATT, NWORKB, NF_DOUBLE)
!$OMP END SINGLE
    IVAR_ID = NWORKB
  ENDIF      
