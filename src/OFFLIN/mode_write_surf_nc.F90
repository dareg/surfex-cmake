...
!
IF (NRANK==NPIO) THEN 
  IRET(5)=NF90_PUT_VAR(NID_NC,IVAR_ID,ZTAB1D)
...
!
IF (NRANK==NPIO) THEN
  IRET(5)=NF90_PUT_VAR(NID_NC,IVAR_ID,ZTAB2D)
  CALL HANDLE_ERR(IRET(5),HREC)  
ENDIF
!
...
IF (NRANK==NPIO) THEN
  ...
  IF (NID_NC /= 0) THEN 
    ...
    IRET(2) = NF90_INQ_DIMID(NID_NC,TRIM(YNAM_DIM),IDIMIDS(1))
    IF (IRET(2)==0) THEN
      IRET(3) = NF90_INQ_DIMID(NID_NC,'Number_of_Tile',IDIMIDS(2))
    ELSE
      IRET(2) = NF90_INQ_DIMID(NID_NC,'lon',IDIMIDS(1))
      IF (IRET(2)/=0) THEN
        IRET(2) = NF90_INQ_DIMID(NID_NC,'xx',IDIMIDS(1))
...
! 2. Put variable
!-----------------
IRET(3)=NF90_PUT_VAR(NID_NC,IVAR_ID,YTAB1D)
!
