...
IF (PTAB(1).NE.0 .AND. XTYPE.NE.1) THEN
  !
  JRET = NF90_INQ_VARID(PTAB(1),'time',IVAR_ID)
  IF (JRET.EQ.NF90_NOERR) THEN
    JRET = NF90_PUT_VAR(PTAB(1),IVAR_ID,PFIELD,ISTART)
...
DO JJ=2,SIZE(PTAB)
  ...
  IF (PTAB(JJ).NE.PTAB(JJ-1) .AND. PTAB(JJ).NE.0) THEN
    JRET = NF90_INQ_VARID(PTAB(JJ),'time',IVAR_ID)
    IF (JRET.EQ.NF90_NOERR) THEN
      JRET = NF90_PUT_VAR(PTAB(JJ),IVAR_ID,PFIELD,ISTART)
      CALL HANDLE_ERR(JRET,'time')
    ENDIF
  ENDIF
  ! 
...
DO JWRK=1,2
  ...
  IF (IFILE_ID /= 0) THEN
    ...
    ! 1. Find id of the variable
    !----------------------------
    IRET(1+JWRK*2)=NF90_INQ_VARID   (IFILE_ID,YRECFM,IVAR_ID)
    IF (JWRK == 1) THEN 
      ITDATE(1)=KYEAR
      ITDATE(2)=KMONTH
      ITDATE(3)=KDAY
      IRET(JWRK)=NF90_PUT_VAR(IFILE_ID,IVAR_ID,ITDATE)
    ELSE
      JLEN=1
      IRET(JWRK)=NF90_PUT_VAR(IFILE_ID,IVAR_ID,PTIME)
    ENDIF
  ENDIF
ENDDO
!
