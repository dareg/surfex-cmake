...
INTEGER, DIMENSION(:), ALLOCATABLE :: ISIZE_OMP, ISIZE_OMP0
...
CALL INIT_INDEX_MPI(YSURF_CUR, &
                    CSURF_FILETYPE, 'OFF', YALG_MPI, XIO_FRAC, GSHADOWS)
!
 CALL WLOG_MPI(' ')
...
!
 CALL GOTO_MODEL(1)
ALLOCATE(ISIZE_OMP0(0:NBLOCKTOT-1))
 CALL GET_SIZES_PARALLEL(YSURF_CUR%DTCO, YSURF_CUR%DGU, YSURF_CUR%UG, YSURF_CUR%U, &
                         NBLOCKTOT,INI,0,ISIZE_OMP0, GSHADOWS)
 CALL SUNPOS(ISIZE_OMP0, IYEAR, IMONTH, IDAY, ZTIME, ZLON, ZLAT, XTSUN, XZENITH, XAZIM)
!
...
!
ALLOCATE(ISIZE_OMP(0:IBLOCKTOT-1))
 CALL GET_SIZES_PARALLEL(YSURF_CUR%DTCO, YSURF_CUR%DGU, YSURF_CUR%UG, YSURF_CUR%U, &
                         IBLOCKTOT,INI,0,ISIZE_OMP, GSHADOWS)
DO J=0,IBLOCKTOT-1
...
!!$ NBLOCK = OMP_GET_THREAD_NUM()
!
IF (IBLOCK==IBLOCKTOT) THEN
  CALL INIT_DIM(ISIZE_OMP,0,INKPROMA,NINDX1SFX,NINDX2SFX)
  ELSE
  CALL INIT_DIM(ISIZE_OMP,IBLOCK,INKPROMA,NINDX1SFX,NINDX2SFX)
ENDIF
!
IF (IBLOCK==0) THEN
  CALL GOTO_MODEL(IBLOCKTOT)
ELSE
  CALL GOTO_MODEL(IBLOCK)
ENDIF
!
...
DO JFORC_STEP=1,INB_STEP_ATM
  ...
  DO JSURF_STEP=1,INB_ATM
    ...
    XTIME1 = MPI_WTIME()
#endif
    CALL SUNPOS(ISIZE_OMP0, IYEAR, IMONTH, IDAY, ZTIME, &
                ZLON, ZLAT, XTSUN, XZENITH, XAZIM)
    CALL SUNPOS(ISIZE_OMP0, IYEAR, IMONTH, IDAY, ZTIME+XTSTEP_SURF, &
    ...
    CALL OL_TIME_INTERP_ATM(JSURF_STEP,INB_ATM,ISIZE_OMP0,            &
