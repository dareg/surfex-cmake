...
USE MODD_CONST_ATM, ONLY : JPNBANDS_ATM, JPNLYR_CLEAR, PPZP_CUT, PPHUND
...
REAL, DIMENSION(:), ALLOCATABLE :: ZO3, ZAE, ZP_CLOUD, ZTCLOUD55, ZMU 
...
IF (.NOT.ALLOCATED(ZO3)) ALLOCATE(ZO3  (INI)       )! 
IF (.NOT.ALLOCATED(ZAE)) ALLOCATE(ZAE  (INI)       )! 
...
!!!    ! IF SNOWCRORAD=TARTES then spectral repartition of direct and diffuse radiation
   ...
   IF (GCRORAD) THEN 
    ...
    ZO3(NINDX1SFX:NINDX2SFX)= 0.3 ! integrated ozone (atm-cm)
    ZAE(NINDX1SFX:NINDX2SFX)= 0.1 ! aerosol optical depth
...
!write(*,*) IMONTH, IDAY, ZTIME/(3600.),ZMU(NINDX1SFX:NINDX2SFX), XZENITH(NINDX1SFX:NINDX2SFX)	  
