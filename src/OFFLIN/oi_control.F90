...
!
 ...
 REAL, DIMENSION (:),   ALLOCATABLE :: PSST, PSAB, PARG, PLAT, PLON,                                   &
                                      & PTSUN, PZENITH, PAZIMSOL, PTCLS, PHCLS, PRAIN, PSNOW,          &
                                      & PWIND, PSWD, PSWS, PUCLS, PVCLS, PEVAP, PEVAPTR,               &
                                      & PT2M_O, PHU2M_O, PTS_O, ZT2INC, ZH2INC,                        &
                                      & ZWS, ZWP, ZTL, ZTS, ZTP, ZTCLS, ZHCLS, ZUCLS, ZVCLS,           &
                                      & PSSTC, PWPINC1, PWPINC2, PWPINC3, PT2MBIAS, PH2MBIAS,          &
                                      & PRRCN, PRRCL, PRRSN, PRRSL,                                    &
                                      & PATMNEB, PITM, PALBF, PEMISF, PZ0F, PIVEG, PZ0H, PTSC,         &
                                      & PTPC, PWSC, PWPC, PSNC, ZEVAP, ZEVAPTR, PGELAT, PGELAM, PGEMU, &
                                      & ZWSINC, ZWPINC,   ZTSINC, ZTPINC, ZTLINC, ZSNINC, ZSNS,        &
                                      & ZPX, ZPY, PSM_O,  PSIG_SMO, PLSM_O, PWS_O,  ZWGINC ,           &
                                      & PLST, PTRD3, ZSST, ZLST, ZALT
 REAL, DIMENSION (:),   ALLOCATABLE :: ZSST1, ZLST1, PSST1, PLST1, PLAT1, PLON1, ZLAT1, ZALT1
!
 INTEGER                            :: I,J
...
ENDIF
!
IF (NSIZE_SEA>0 .AND. CSEA/='NONE') THEN
  CALL READ_SURF(YPROGRAM,'SST',       PSST,  IRESP)
ENDIF
!
IF (NSIZE_WATER>0 .AND. CWATER/='NONE') THEN
  CALL READ_SURF(YPROGRAM,'TS_WATER',  PLST,  IRESP)
ENDIF
!
IF (NSIZE_TOWN>0 .AND. CTOWN/='NONE' .AND. LAROME) THEN
...
!
 ...
 ENDIF
!
!WRITE(*,'(A)') 'TEMPORARY TEST ON READ FA FILE VALUES:'
!WRITE(*,'(A)') ' I PLSM PT2M PTS PTSC'
!DO I=1,NDIM_FULL
!  WRITE(*,'(I6,A1,4F12.5)') I,' ',PT2M_O(I), PTS_O(I)
!ENDDO
...
!
 ...
 IF (LOBSWG) THEN
   ...
   DO I=1,NDIM_FULL
     READ(111,*,END=990) PSM_O(I),PSIG_SMO(I),PLSM_O(I)
...
!
 IF (LDINLINE) THEN
   ...
   ELSE
     ...
     J = 1
     DO I = 1, ISIZE
       IF (.NOT. LD_MASKEXT (I)) THEN
         PSST1 (J) = PSST (I)
         PLST1 (J) = PLST (I)
         PLAT1 (J) = PLAT (I)
         PLON1 (J) = PLON (I)
         ZALT1 (J) = ZALT (I)
         OINTERP_LST1 (J) = OINTERP_LST (I)
         OINTERP_SST1 (J) = OINTERP_SST (I)
         J = J + 1
       ENDIF
     ENDDO
...
#endif
 ENDIF
 CALL FLAG_UPDATE(.FALSE.,.FALSE.,.TRUE.,.FALSE.)
