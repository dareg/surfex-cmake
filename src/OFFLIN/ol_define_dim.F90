...
!-------------------------------------------------------------------------------                         
!
USE MODD_SURF_ATM_GRID_n, ONLY : CGRID, NGRID_PAR, XGRID_FULL_PAR, XGRID_PAR
...
IF (.NOT.LWRITE_COORD) THEN
  ...
  .OR. CGRID.EQ.'LONLAT REG' .OR. CGRID.EQ.'IGN' ) THEN
    ...
    IF (ASSOCIATED(XGRID_FULL_PAR)) THEN
      CALL GET_GRID_DIM(CGRID,NGRID_PAR,XGRID_FULL_PAR,GRECT,KDIM1,IDIM2)
    ELSEIF (ASSOCIATED(XGRID_PAR)) THEN
...
IF (LWRITE_COORD) THEN
  !
  IF (ASSOCIATED(XGRID_FULL_PAR)) THEN
    CALL GET_GRID_COORD(KLUOUT,PX=PX,PY=PY,KL=KNI,HGRID=CGRID,PGRID_PAR=XGRID_FULL_PAR)
  ELSEIF (ASSOCIATED(XGRID_PAR)) THEN
    CALL GET_GRID_COORD(KLUOUT,PX=PX,PY=PY,KL=KNI,HGRID=CGRID,PGRID_PAR=XGRID_PAR)
  ENDIF
  !
ELSEIF ( CGRID.EQ.'CONF PROJ '.OR. CGRID.EQ.'CARTESIAN '.OR. &
  ...
  !
  IF (ASSOCIATED(XGRID_FULL_PAR)) THEN
    CALL GET_GRID_COORD(KLUOUT,PX=ZXX,PY=ZYY,KL=KNI,HGRID=CGRID,PGRID_PAR=XGRID_FULL_PAR)
  ELSEIF (ASSOCIATED(XGRID_PAR)) THEN
    CALL GET_GRID_COORD(KLUOUT,PX=ZXX,PY=ZYY,KL=KNI,HGRID=CGRID,PGRID_PAR=XGRID_PAR)
  ENDIF
  !
...
ELSEIF(CGRID.EQ.'IGN       ')THEN
  !
  IF (ASSOCIATED(XGRID_FULL_PAR)) THEN
    CALL GET_GRIDTYPE_IGN(XGRID_FULL_PAR,PX=ZXX,PY=ZYY,PXALL=PX,PYALL=PY)
  ELSEIF (ASSOCIATED(XGRID_PAR)) THEN
