...
IF (.NOT.LFIND_VAR) THEN
  ...
  JRET=NF_NOERR+1
  DO WHILE  ((JFILE<= SIZE(XID_IN)) .AND. (JRET /= NF_NOERR))
       IFILE_ID=XID_IN(JFILE)
       JRET=NF_INQ_VARID(IFILE_ID,HNAME_PAS,IVAR_ID)
       JFILE=JFILE+1
  ENDDO
  ...
  IF (JRET == NF_NOERR) THEN
    ...
    DEALLOCATE(HTEMP)
  ELSE
    CALL ABOR1_SFX("OL_FIND_FILE_READ: VARIABLE NOT FOUND IN INPUT NC FILES")
  ENDIF
