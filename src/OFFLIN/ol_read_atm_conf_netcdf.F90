...
SUBROUTINE OL_READ_ATM_CONF_NETCDF (YSC, &
                                    HSURF_FILETYPE,ODELAYEDSTART_NC, &
                                     KDATESTOP,&
...
IF (IFILE_ID.NE.0) THEN
  ...
  IF ((YSC%UG%CGRID.EQ.'LONLAT REG').AND.(INDIMS==1)) THEN
      OLONLAT1D=.TRUE.
      CALL OL_FIND_FILE_READ('LAT',IFILE_ID)
      IRET(5)=NF_INQ_VARID   (IFILE_ID,'LAT',IVAR_ID)
      IRET(6)=NF_INQ_VARNDIMS(IFILE_ID,IVAR_ID,INDIMS)
      IF (INDIMS==1) THEN
        IRET(7)=NF_INQ_VARDIMID(IFILE_ID,IVAR_ID,IDIMIDS(2))
        IRET(8)=NF_INQ_DIMLEN(IFILE_ID,IDIMIDS(2),IDIMLEN(2))
        KNLON1D=IDIMLEN(1)
        KNLAT1D=IDIMLEN(2)
      ELSE
        KRESP=1
