...
IF (IFILE_ID.NE.0) THEN
  ...
  SELECT CASE (ITYPE)
    ...
    CASE (NF_FLOAT)
      IRET(7)=REAL(NF_GET_VAR_REAL(IFILE_ID,IVAR_ID,ZTIMEFILE))
    CASE (NF_INT)
      IRET(7)=REAL(NF_GET_VAR_INT(IFILE_ID,IVAR_ID,ZTIMEFILE))
    CASE DEFAULT
      CALL ABOR1_SFX('OL_READ_ATM_CONF_NETCDF: TYPE OF TIME VARIABLE NOT KNOWN')
  END SELECT
  
  ! For now, we just remember the first date, but maybe it will change
