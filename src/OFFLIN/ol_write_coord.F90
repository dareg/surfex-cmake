...
IF ( LWRITE_COORD ) THEN
  CALL DEF_VAR_NETCDF(HSELECT,KFILE_ID,'xx','abscissas',KDDIM(1:1),HATT_TITLE,(/''/))
  CALL DEF_VAR_NETCDF(HSELECT,KFILE_ID,'yy','ordinates',KDDIM(1:1),HATT_TITLE,(/''/))
ELSEIF (KDIM1.NE.0) THEN
  CALL DEF_VAR_NETCDF(HSELECT,KFILE_ID,TRIM(HNAME_DIM(1)),'',KDDIM(1:1),HATT_TITLE,HUNIT1)
  CALL DEF_VAR_NETCDF(HSELECT,KFILE_ID,TRIM(HNAME_DIM(2)),'',KDDIM(2:2),HATT_TITLE,HUNIT2)
ENDIF
INDIMS = SIZE(KDDIM)
 CALL DEF_VAR_NETCDF(HSELECT,KFILE_ID,'time','',KDDIM(INDIMS:INDIMS),HATT_TITLE,HDATE)
!
JRET=NF90_ENDDEF(KFILE_ID)
!
!JRET=NF90_OPEN(HFILE, NF90_WRITE,KFILE_ID)
IF (KDIM1.NE.0) THEN
  JRET = NF90_INQ_VARID(KFILE_ID, HNAME_DIM(1), IVAR_ID)
  JRET = NF90_PUT_VAR  (KFILE_ID, IVAR_ID, PX)
  JRET = NF90_INQ_VARID(KFILE_ID, HNAME_DIM(2), IVAR_ID)
  JRET = NF90_PUT_VAR  (KFILE_ID, IVAR_ID, PY)
ELSEIF (LWRITE_COORD) THEN
  JRET = NF90_INQ_VARID(KFILE_ID, 'xx', IVAR_ID)
  JRET = NF90_PUT_VAR  (KFILE_ID, IVAR_ID, PX)
  JRET = NF90_INQ_VARID(KFILE_ID, 'yy', IVAR_ID)
  JRET = NF90_PUT_VAR  (KFILE_ID, IVAR_ID, PY)
ENDIF
!
ALLOCATE(ITEMP(XOUT))
