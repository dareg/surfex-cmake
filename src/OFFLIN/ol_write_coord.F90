SUBROUTINE OL_WRITE_COORD(HFILE, KFILE_ID, KDDIM, HATT_TITLE, HNAME_DIM, &
...
IF ( LWRITE_COORD ) THEN
  CALL DEF_VAR_NETCDF(KFILE_ID,'xx','abscissas',KDDIM(1:1),HATT_TITLE,(/''/))
  CALL DEF_VAR_NETCDF(KFILE_ID,'yy','ordinates',KDDIM(1:1),HATT_TITLE,(/''/))
ELSEIF (KDIM1.NE.0) THEN
  CALL DEF_VAR_NETCDF(KFILE_ID,TRIM(HNAME_DIM(1)),'',KDDIM(1:1),HATT_TITLE,HUNIT1)
  CALL DEF_VAR_NETCDF(KFILE_ID,TRIM(HNAME_DIM(2)),'',KDDIM(2:2),HATT_TITLE,HUNIT2)
ENDIF
INDIMS = SIZE(KDDIM)
 CALL DEF_VAR_NETCDF(KFILE_ID,'time','',KDDIM(INDIMS:INDIMS),HATT_TITLE,HDATE)
!
