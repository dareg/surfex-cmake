...
SELECT CASE (HVAR)
  CASE ('WG     ')
    ...
    IF (OUNIF .AND. (XHUG_SURF==XUNDEF)) THEN
       ...
       CALL ABOR1_SFX('READ_PREP_TEB_GREENROOF_CONF: XHUG_SURF MUST BE SET')
    END IF
    IF (OUNIF .AND. (XHUG_ROOT==XUNDEF)) THEN
       WRITE(KLUOUT,*)'ONE OF XHUG_SURF, XHUG_ROOT OR XHUG_DEEP IS GIVEN'
       CALL ABOR1_SFX('READ_PREP_TEB_GREENROOF_CONF: XHUG_ROOT MUST BE SET')
    END IF
    ...
    IF (OUNIF .AND. (XHUGI_SURF==XUNDEF)) THEN
       ...
       CALL ABOR1_SFX('READ_PREP_TEB_GREENROOF_CONF: XHUGI_SURF MUST BE SET')
    END IF
    IF (OUNIF .AND. (XHUGI_ROOT==XUNDEF)) THEN
       WRITE(KLUOUT,*)'ONE OF XHUGI_SURF, XHUGI_ROOT OR XHUGI_DEEP IS GIVEN'
       CALL ABOR1_SFX('READ_PREP_TEB_GREENROOF_CONF: XHUGI_ROOT MUST BE SET')
    END IF
    IF (OUNIF .AND. (XHUGI_DEEP==XUNDEF)) THEN
       WRITE(KLUOUT,*)'ONE OF XHUGI_SURF, XHUGI_ROOT OR XHUGI_DEEP IS GIVEN'
       CALL ABOR1_SFX('READ_PREP_TEB_GREENROOF_CONF: XHUGI_DEEP MUST BE SET')
    END IF
    !
  CASE ('TG     ')
    ...
    IF (OUNIF .AND. (XTG_SURF==XUNDEF)) THEN
       ...
       CALL ABOR1_SFX('READ_PREP_TEB_GREENROOF_CONF: XTG_SURF MUST BE SET')
    END IF
    IF (OUNIF .AND. (XTG_ROOT==XUNDEF)) THEN
       WRITE(KLUOUT,*)'ONE OF XTG_SURF, XTG_ROOT OR XTG_DEEP IS GIVEN'
       CALL ABOR1_SFX('READ_PREP_TEB_GREENROOF_CONF: XTG_ROOT MUST BE SET')
    END IF
    IF (OUNIF .AND. (XTG_DEEP==XUNDEF)) THEN
       WRITE(KLUOUT,*)'ONE OF XTG_SURF, XTG_ROOT OR XTG_DEEP IS GIVEN'
       CALL ABOR1_SFX('READ_PREP_TEB_GREENROOF_CONF: XTG_DEEP MUST BE SET')
    END IF
    !
END SELECT
!
