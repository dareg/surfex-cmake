...
!     #############################################################
      SUBROUTINE ALLOC_DIAG_SURF_ATM_n (DGO, D, DC, DP, DPC, &
...
DO JTILE = 1,NTILESFC
  !
  CALL ALLOC_DIAG(DP%AL(JTILE),.FALSE.,KSIZE_FULL,KSW)
  CALL INIT_DIAG(DP%AL(JTILE),.FALSE.,XUNDEF)
  !
ENDDO
!
...
IF (DGO%LSURF_BUDGETC) THEN
  ...
  DO JTILE = 1,NTILESFC
    !
    CALL ALLOC_DIAG(DPC%AL(JTILE),.TRUE.,KSIZE_FULL,0)
    CALL INIT_DIAG(DPC%AL(JTILE),.TRUE.,XUNDEF)
    !
  ENDDO
  !
...
ELSE
  !
  DO JTILE=1,NTILESFC
    CALL ALLOC_DIAG(DPC%AL(JTILE),.TRUE.,0,0)
  ENDDO
  !
