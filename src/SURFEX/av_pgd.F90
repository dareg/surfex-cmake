...
!
  SELECT CASE (HSFTYPE)
       ...
       CASE('TRE')
         PFIELD(:)=0.
         ZWEIGHT=XDATA_NATURE(JJ) * (  XDATA_VEGTYPE(JJ,NVT_TEBD) &
                                       + XDATA_VEGTYPE(JJ,NVT_TRBE) &
                                       + XDATA_VEGTYPE(JJ,NVT_TRBD) &
                                       + XDATA_VEGTYPE(JJ,NVT_TEBE) &
                                       + XDATA_VEGTYPE(JJ,NVT_TENE) &
                                       + XDATA_VEGTYPE(JJ,NVT_BOBD) &
                                       + XDATA_VEGTYPE(JJ,NVT_BOND) &
                                       + XDATA_VEGTYPE(JJ,NVT_SHRB) &
                                       + XDATA_VEGTYPE(JJ,NVT_BONE) )  

       CASE('GRT')
         PFIELD(:)=0.
         ZWEIGHT=XDATA_TOWN(JJ) * XDATA_GARDEN(JJ) &
                         * (  XDATA_VEGTYPE(JJ,NVT_TEBD) &
                            + XDATA_VEGTYPE(JJ,NVT_TRBE) &
                            + XDATA_VEGTYPE(JJ,NVT_TRBD) &
                            + XDATA_VEGTYPE(JJ,NVT_TEBE) &
                            + XDATA_VEGTYPE(JJ,NVT_TENE) &
...
USE MODD_DATA_COVER_n,   ONLY : XDATA_NATURE, XDATA_TOWN, XDATA_BLD, XDATA_GARDEN, &
                                XDATA_SEA, XDATA_WATER, XDATA_VEGTYPE, XDATA_WEIGHT
...
!
IF (.NOT.ASSOCIATED(XDATA_WEIGHT)) THEN
  !
  ...
  DO JJ=1,ICOVER
    ...
    DO JVEGTYPE=1,NVEGTYPE
      ...
      IF (JVEGTYPE==NVT_TEBD) THEN
        XDATA_WEIGHT(JCOVER,NVT_TEBD,11)= XDATA_WEIGHT(JCOVER,NVT_TEBD,1)
        XDATA_WEIGHT(JCOVER,NVT_TEBD,12)= XDATA_WEIGHT(JCOVER,NVT_TEBD,2)
      ENDIF
      IF (JVEGTYPE==NVT_BONE) THEN
        XDATA_WEIGHT(JCOVER,NVT_BONE,11)= XDATA_WEIGHT(JCOVER,NVT_BONE,1)
        XDATA_WEIGHT(JCOVER,NVT_BONE,12)= XDATA_WEIGHT(JCOVER,NVT_BONE,2)
      ENDIF
      IF (JVEGTYPE==NVT_TRBE) THEN
        XDATA_WEIGHT(JCOVER,NVT_TRBE,11)= XDATA_WEIGHT(JCOVER,NVT_TRBE,1)
        XDATA_WEIGHT(JCOVER,NVT_TRBE,12)= XDATA_WEIGHT(JCOVER,NVT_TRBE,2)
      ENDIF
      IF (JVEGTYPE==NVT_TRBD) THEN
        XDATA_WEIGHT(JCOVER,NVT_TRBD,11)= XDATA_WEIGHT(JCOVER,NVT_TRBD,1)
        XDATA_WEIGHT(JCOVER,NVT_TRBD,12)= XDATA_WEIGHT(JCOVER,NVT_TRBD,2)
      ENDIF
      IF (JVEGTYPE==NVT_TEBE) THEN
        XDATA_WEIGHT(JCOVER,NVT_TEBE,11)= XDATA_WEIGHT(JCOVER,NVT_TEBE,1)
        XDATA_WEIGHT(JCOVER,NVT_TEBE,12)= XDATA_WEIGHT(JCOVER,NVT_TEBE,2)
      ENDIF
      IF (JVEGTYPE==NVT_TENE) THEN
        XDATA_WEIGHT(JCOVER,NVT_TENE,11)= XDATA_WEIGHT(JCOVER,NVT_TENE,1)
        XDATA_WEIGHT(JCOVER,NVT_TENE,12)= XDATA_WEIGHT(JCOVER,NVT_TENE,2)
      ENDIF
      IF (JVEGTYPE==NVT_BOBD) THEN
        XDATA_WEIGHT(JCOVER,NVT_BOBD,11)= XDATA_WEIGHT(JCOVER,NVT_BOBD,1)
        XDATA_WEIGHT(JCOVER,NVT_BOBD,12)= XDATA_WEIGHT(JCOVER,NVT_BOBD,2)
      ENDIF
      IF (JVEGTYPE==NVT_BOND) THEN
        XDATA_WEIGHT(JCOVER,NVT_BOND,11)= XDATA_WEIGHT(JCOVER,NVT_BOND,1)
        XDATA_WEIGHT(JCOVER,NVT_BOND,12)= XDATA_WEIGHT(JCOVER,NVT_BOND,2)
      ENDIF
      IF (JVEGTYPE==NVT_SHRB) THEN
        XDATA_WEIGHT(JCOVER,NVT_SHRB,11)= XDATA_WEIGHT(JCOVER,NVT_SHRB,1)
        XDATA_WEIGHT(JCOVER,NVT_SHRB,12)= XDATA_WEIGHT(JCOVER,NVT_SHRB,2)
      ENDIF   
      !
    ENDDO      
    !
...
!
  SELECT CASE (HSFTYPE)
     ...
     CASE('GRL')
      ...
      CASE('TRE')
        ZWEIGHT(:)=0.
        IF (XDATA_VEGTYPE(JJ,NVT_TEBD)>0.) THEN
          ZWEIGHT(NVT_TEBD)=XDATA_NATURE(JJ) * XDATA_VEGTYPE(JJ,NVT_TEBD)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_BONE)>0.) THEN
          ZWEIGHT(NVT_BONE)=XDATA_NATURE(JJ) * XDATA_VEGTYPE(JJ,NVT_BONE)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_TRBE)>0.) THEN
          ZWEIGHT(NVT_TRBE)=XDATA_NATURE(JJ) * XDATA_VEGTYPE(JJ,NVT_TRBE)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_TRBD)>0.) THEN
          ZWEIGHT(NVT_TRBD)=XDATA_NATURE(JJ) * XDATA_VEGTYPE(JJ,NVT_TRBD)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_TEBE)>0.) THEN
          ZWEIGHT(NVT_TEBE)=XDATA_NATURE(JJ) * XDATA_VEGTYPE(JJ,NVT_TEBE)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_TENE)>0.) THEN
          ZWEIGHT(NVT_TENE)=XDATA_NATURE(JJ) * XDATA_VEGTYPE(JJ,NVT_TENE)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_BOBD)>0.) THEN
          ZWEIGHT(NVT_BOBD)=XDATA_NATURE(JJ) * XDATA_VEGTYPE(JJ,NVT_BOBD)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_BOND)>0.) THEN
          ZWEIGHT(NVT_BOND)=XDATA_NATURE(JJ) * XDATA_VEGTYPE(JJ,NVT_BOND)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_SHRB)>0.) THEN
          ZWEIGHT(NVT_SHRB)=XDATA_NATURE(JJ) * XDATA_VEGTYPE(JJ,NVT_SHRB)
        END IF

      CASE('GRT')
        ZWEIGHT(:)=0.
        IF (XDATA_VEGTYPE(JJ,NVT_TEBD)>0.) THEN
          ZWEIGHT(NVT_TEBD)=XDATA_TOWN(JJ)*XDATA_GARDEN(JJ) * XDATA_VEGTYPE(JJ,NVT_TEBD)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_BONE)>0.) THEN
          ZWEIGHT(NVT_BONE)=XDATA_TOWN(JJ)*XDATA_GARDEN(JJ) * XDATA_VEGTYPE(JJ,NVT_BONE)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_TRBE)>0.) THEN
          ZWEIGHT(NVT_TRBE)=XDATA_TOWN(JJ)*XDATA_GARDEN(JJ) * XDATA_VEGTYPE(JJ,NVT_TRBE)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_TRBD)>0.) THEN
          ZWEIGHT(NVT_TRBD)=XDATA_TOWN(JJ)*XDATA_GARDEN(JJ) * XDATA_VEGTYPE(JJ,NVT_TRBD)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_TEBE)>0.) THEN
          ZWEIGHT(NVT_TEBE)=XDATA_TOWN(JJ)*XDATA_GARDEN(JJ) * XDATA_VEGTYPE(JJ,NVT_TEBE)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_TENE)>0.) THEN
          ZWEIGHT(NVT_TENE)=XDATA_TOWN(JJ)*XDATA_GARDEN(JJ) * XDATA_VEGTYPE(JJ,NVT_TENE)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_BOBD)>0.) THEN
          ZWEIGHT(NVT_BOBD)=XDATA_TOWN(JJ)*XDATA_GARDEN(JJ) * XDATA_VEGTYPE(JJ,NVT_BOBD)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_BOND)>0.) THEN
          ZWEIGHT(NVT_BOND)=XDATA_TOWN(JJ)*XDATA_GARDEN(JJ) * XDATA_VEGTYPE(JJ,NVT_BOND)
        END IF
        IF (XDATA_VEGTYPE(JJ,NVT_SHRB)>0.) THEN
          ZWEIGHT(NVT_SHRB)=XDATA_TOWN(JJ)*XDATA_GARDEN(JJ) * XDATA_VEGTYPE(JJ,NVT_SHRB)
        END IF
