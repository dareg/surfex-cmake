...
!            -----------
!
IF (LHOOK) CALL DR_HOOK('MODI_AV_PGD:AV_PGD_1D_1',1,ZHOOK_HANDLE)
!
IF (HATYPE=='ARI' .OR. HATYPE=='INV' .OR. HATYPE=='CDN') THEN
  !
  IF (LHOOK) CALL DR_HOOK('MODI_AV_PGD:AV_PGD_1D_2',0,ZHOOK_HANDLE_OMP)

!$OMP PARALLEL PRIVATE(JCOVER,JI,ZVAL,ZCOVER_WEIGHT)
  DO JCOVER=1,ICOVER
    IF (ZWEIGHT(JCOVER)/=0.) THEN
      !
      IF (HATYPE=='ARI') THEN
        ZVAL = PDATA(IMASK(JCOVER))
!$OMP DO
        DO JI = 1,SIZE(PCOVER,1)
          IF (PCOVER(JI,JCOVER)/=0.) THEN
            ...
            ZWORK(JI) = ZWORK(JI) + ZVAL * ZCOVER_WEIGHT
          ENDIF
        ENDDO
!$OMP END DO
      ELSEIF (HATYPE=='INV') THEN
        ZVAL = 1./PDATA(IMASK(JCOVER))
!$OMP DO
        DO JI = 1,SIZE(PCOVER,1)
          IF (PCOVER(JI,JCOVER)/=0.) THEN
            ZCOVER_WEIGHT = PCOVER(JI,JCOVER) * ZWEIGHT(JCOVER)
...
!$OMP DO
        DO JI = 1,SIZE(PCOVER,1)
          IF (PCOVER(JI,JCOVER)/=0.) THEN
            ZVAL = 1./(XZ0_OFFSET + LOG(ZDZ(JI)/PDATA(IMASK(JCOVER))))**2 
            ZCOVER_WEIGHT = PCOVER(JI,JCOVER) * ZWEIGHT(JCOVER)
            ZSUM_COVER_WEIGHT(JI) = ZSUM_COVER_WEIGHT(JI) + ZCOVER_WEIGHT
            ZWORK(JI) = ZWORK(JI) + ZVAL * ZCOVER_WEIGHT
          ENDIF
...
!$OMP END PARALLEL
  ...
  IF (LHOOK) CALL DR_HOOK('MODI_AV_PGD:AV_PGD_1D_2',1,ZHOOK_HANDLE_OMP)
  !
...
!
!$OMP PARALLEL PRIVATE(JVEGTYPE,JPATCH,JCOVER,JI,ZVAL,ZCOVER_WEIGHT,ZHOOK_HANDLE_OMP)
...
DO JVEGTYPE=1,NVEGTYPE
  ...
  DO JCOVER=1,ICOVER
    ...
    IF (ZWEIGHT(JCOVER,JVEGTYPE)/=0.) THEN
      !
      IF (HATYPE=='ARI') THEN
        ZVAL = PDATA(IMASK0(JCOVER),JVEGTYPE)
!$OMP DO
        DO JI=1,SIZE(PCOVER,1)
          IF (PCOVER(JI,JCOVER)/=0.) THEN
            ...
            ZWORK(JI,JPATCH) = ZWORK(JI,JPATCH) + ZVAL * ZCOVER_WEIGHT
          ENDIF
        ENDDO
!$OMP END DO
      ELSEIF (HATYPE=='INV') THEN
        ZVAL = 1. / PDATA(IMASK0(JCOVER),JVEGTYPE)
!$OMP DO
        DO JI=1,SIZE(PCOVER,1)
          IF (PCOVER(JI,JCOVER)/=0.) THEN
            ZCOVER_WEIGHT =  PCOVER(JI,JCOVER) * ZWEIGHT(JCOVER,JVEGTYPE)      
...
!$OMP DO
        DO JI=1,SIZE(PCOVER,1)
          IF (PCOVER(JI,JCOVER)/=0.) THEN
            ZVAL = 1./(XZ0_OFFSET + LOG(ZDZ(JI,JPATCH)/PDATA(IMASK0(JCOVER),JVEGTYPE)))**2 
            ZCOVER_WEIGHT =  PCOVER(JI,JCOVER) * ZWEIGHT(JCOVER,JVEGTYPE)      
            ZSUM_COVER_WEIGHT_PATCH(JI,JPATCH) = ZSUM_COVER_WEIGHT_PATCH(JI,JPATCH) + ZCOVER_WEIGHT
            ZWORK(JI,JPATCH) = ZWORK(JI,JPATCH) + ZVAL * ZCOVER_WEIGHT
          ENDIF
