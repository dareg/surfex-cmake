...
!-------------------------------------------------------------------------------
  ...
  !            -----------------------------------
  !
!$OMP PARALLEL PRIVATE(JCOVER,JVEG,ZVAL,JI,ZCOVER_WEIGHT)
DO JCOVER=1,ICOVER
  !
  DO JVEG=1,NVEGTYPE
    !
    IF (PATCH_LIST(JVEG)==KPATCH) THEN
    !
    IF (ZWEIGHT(JCOVER,JVEG)/=0.) THEN
      !
      IF (HATYPE=='ARI') THEN
        ZVAL = PDATA(IMASK0(JCOVER),JVEG)
!$OMP DO
        DO JI=1,SIZE(PFIELD)
          IF (PCOVER(KMASK(JI),JCOVER)/=0.) THEN
            ZCOVER_WEIGHT =  PCOVER(KMASK(JI),JCOVER) * ZWEIGHT(JCOVER,JVEG)      
...
!$OMP END DO
      ELSEIF (HATYPE=='INV') THEN
        ZVAL = 1. / PDATA(IMASK0(JCOVER),JVEG)
!$OMP DO
        DO JI=1,SIZE(PFIELD)
          IF (PCOVER(KMASK(JI),JCOVER)/=0.) THEN
            ZCOVER_WEIGHT =  PCOVER(KMASK(JI),JCOVER) * ZWEIGHT(JCOVER,JVEG)      
...
!$OMP DO
        DO JI=1,SIZE(PFIELD)
          IF (PCOVER(KMASK(JI),JCOVER)/=0.) THEN
            ZVAL = 1./(LOG(ZDZ(JI)/PDATA(IMASK0(JCOVER),JVEG)))**2 
            ZCOVER_WEIGHT =  PCOVER(KMASK(JI),JCOVER) * ZWEIGHT(JCOVER,JVEG)      
            ZSUM_COVER_WEIGHT_PATCH(JI) = ZSUM_COVER_WEIGHT_PATCH(JI) + ZCOVER_WEIGHT
            ZWORK(JI) = ZWORK(JI) + ZVAL * ZCOVER_WEIGHT
          ENDIF
