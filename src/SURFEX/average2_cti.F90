...
DO JI = 1,SIZE(XMEAN_WORK,1)
  ...
  IF (XMEAN_WORK(JI)/=XUNDEF) THEN
    ...
    ZINT = FLOAT(AINT(XMEAN_WORK(JI),KIND=16))
    IF (XMEAN_WORK(JI)/=ZINT) &
      XMEAN_WORK(JI) = ZINT + NINT((XMEAN_WORK(JI)-ZINT)*100000000.)/100000000.

    ZINT = FLOAT(AINT(XMIN_WORK(JI),KIND=16))
    IF (XMIN_WORK(JI)/=ZINT) &
      XMIN_WORK(JI) = ZINT + NINT((XMIN_WORK(JI)-ZINT)*100000000.)/100000000.

    ZINT = FLOAT(AINT(XMAX_WORK(JI),KIND=16))
    IF (XMAX_WORK(JI)/=ZINT) &      
      XMAX_WORK(JI) = ZINT + NINT((XMAX_WORK(JI)-ZINT)*100000000.)/100000000.

    ZINT = FLOAT(AINT(XSTD_WORK(JI),KIND=16))
    IF (XSTD_WORK(JI)/=ZINT) &      
    XSTD_WORK(JI) = ZINT + NINT((XSTD_WORK(JI)-ZINT)*100000000.)/100000000.

    ZINT = FLOAT(AINT(XSKEW_WORK(JI),KIND=16))
    IF (XSKEW_WORK(JI)/=ZINT) &      
    XSKEW_WORK(JI) = ZINT + NINT((XSKEW_WORK(JI)-ZINT)*100000000.)/100000000.
!
  ENDIF
ENDDO
!
