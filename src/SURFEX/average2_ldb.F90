...
!
!
DO JI = 1,SIZE(XSUMVAL2,1)
  !
  DO JGRAD = 1,SIZE(XSUMVAL2,2)
    IF (NSIZE(JI).NE.0) XSUMVAL2(JI,JGRAD) = XSUMVAL2(JI,JGRAD)/NSIZE(JI)
  ENDDO
  !
  !2 because first centre is for values lower than 0
  ZFRAC = SUM(XSUMVAL2(JI,2:SIZE(XSUMVAL2,2)))
  !
  ZMAX = XSMALL_DUMMY
  IGRAD_MODE = 2
  !
  IF (KSTAT.EQ.1) THEN
    !
    DO JGRAD = 2, SIZE(XSUMVAL2,2)
      ZPDF = XSUMVAL2(JI,JGRAD) / (ZBOUND(JGRAD)-ZBOUND(JGRAD-1))
  ...
  ELSEIF (KSTAT.EQ.2) THEN
    ...
    ZAVE = 0.
    DO JGRAD = 2, SIZE(XSUMVAL2,2)
      ZAVE = ZAVE + ZCENTR(JGRAD) * XSUMVAL2(JI,JGRAD)
    ENDDO
    !
