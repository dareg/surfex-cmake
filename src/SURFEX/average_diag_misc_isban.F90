...
REAL                            :: ZWORK
INTEGER                         :: INI,INP,IDEPTH,IWORK, JB
!
...
IF (DGMI%LPROSNOW) THEN
  DGMI%XAVG_SNOWDEPTH_1DAYS(:) = 0.
  DGMI%XAVG_SNOWDEPTH_3DAYS(:) = 0.
  DGMI%XAVG_SNOWDEPTH_5DAYS(:) = 0.
  DGMI%XAVG_SNOWDEPTH_7DAYS(:) = 0.
  DGMI%XAVG_SNOWSWE_1DAYS(:) = 0.
  DGMI%XAVG_SNOWSWE_3DAYS(:) = 0.
  DGMI%XAVG_SNOWSWE_5DAYS(:) = 0.
  DGMI%XAVG_SNOWSWE_7DAYS(:) = 0.
  DGMI%XAVG_SNOWRAM_SONDE(:) = 0.
 
 IF (DGMI%LPROBANDS) THEN
  DGMI%XAVG_SPEC_ALB(:,:)=0.
  DGMI%XAVG_DIFF_RATIO(:,:)=0.
 ENDIF 
  DGMI%XAVG_SNOW_WETTHICKNESS(:) = 0.
  DGMI%XAVG_SNOW_REFROZENTHICKNESS(:) = 0.
ENDIF 
!
DGMI%XAVG_LAI   (:) = 0.
!   
...
!      
      IF (DGMI%XTWSNOW(JJ,JPATCH)>0.0) THEN
         ...
         IF (DGMI%LPROSNOW) THEN
           DGMI%XAVG_SNOWDEPTH_1DAYS(JJ) = DGMI%XAVG_SNOWDEPTH_1DAYS(JJ) + I%XPATCH(JJ,JPATCH) * DGMI%XSNOWDEPTH_1DAYS(JJ,JPATCH)
           DGMI%XAVG_SNOWDEPTH_3DAYS(JJ) = DGMI%XAVG_SNOWDEPTH_3DAYS(JJ) + I%XPATCH(JJ,JPATCH) * DGMI%XSNOWDEPTH_3DAYS(JJ,JPATCH)
           DGMI%XAVG_SNOWDEPTH_5DAYS(JJ) = DGMI%XAVG_SNOWDEPTH_5DAYS(JJ) + I%XPATCH(JJ,JPATCH) * DGMI%XSNOWDEPTH_5DAYS(JJ,JPATCH)
           DGMI%XAVG_SNOWDEPTH_7DAYS(JJ) = DGMI%XAVG_SNOWDEPTH_7DAYS(JJ) + I%XPATCH(JJ,JPATCH) * DGMI%XSNOWDEPTH_7DAYS(JJ,JPATCH)
           DGMI%XAVG_SNOWSWE_1DAYS(JJ) = DGMI%XAVG_SNOWSWE_1DAYS(JJ) + I%XPATCH(JJ,JPATCH) * DGMI%XSNOWSWE_1DAYS(JJ,JPATCH)
           DGMI%XAVG_SNOWSWE_3DAYS(JJ) = DGMI%XAVG_SNOWSWE_3DAYS(JJ) + I%XPATCH(JJ,JPATCH) * DGMI%XSNOWSWE_3DAYS(JJ,JPATCH)
           DGMI%XAVG_SNOWSWE_5DAYS(JJ) = DGMI%XAVG_SNOWSWE_5DAYS(JJ) + I%XPATCH(JJ,JPATCH) * DGMI%XSNOWSWE_5DAYS(JJ,JPATCH)
           DGMI%XAVG_SNOWSWE_7DAYS(JJ) = DGMI%XAVG_SNOWSWE_7DAYS(JJ) + I%XPATCH(JJ,JPATCH) * DGMI%XSNOWSWE_7DAYS(JJ,JPATCH)
           DGMI%XAVG_SNOWRAM_SONDE(JJ) = DGMI%XAVG_SNOWRAM_SONDE(JJ) + I%XPATCH(JJ,JPATCH) * DGMI%XSNOWRAM_SONDE(JJ,JPATCH)
           DGMI%XAVG_SNOW_WETTHICKNESS(JJ) = DGMI%XAVG_SNOW_WETTHICKNESS(JJ) + &
           I%XPATCH(JJ,JPATCH) * DGMI%XSNOW_WETTHICKNESS(JJ,JPATCH)
           DGMI%XAVG_SNOW_REFROZENTHICKNESS(JJ) = DGMI%XAVG_SNOW_REFROZENTHICKNESS(JJ) + &
           I%XPATCH(JJ,JPATCH) * DGMI%XSNOW_REFROZENTHICKNESS(JJ,JPATCH)
         ENDIF
      ENDIF
!
    ENDIF
...
DO JB=1, size(DGMI%XAVG_SPEC_ALB,2)
DO JPATCH=1,INP
DO JJ=1,INI
!
    IF (ZSUMPATCH(JJ) > 0.) THEN
...
!      
      IF (DGMI%XTWSNOW(JJ,JPATCH)>0.0) THEN
         ...
         IF (DGMI%LPROBANDS) THEN
           ...
           DGMI%XAVG_SPEC_ALB(JJ,JB) = DGMI%XAVG_SPEC_ALB(JJ,JB) + I%XPATCH(JJ,JPATCH)*DGMI%XSPEC_ALB(JJ,JB,JPATCH)
           DGMI%XAVG_DIFF_RATIO(JJ,JB) =DGMI%XAVG_DIFF_RATIO(JJ,JB) + I%XPATCH(JJ,JPATCH)*DGMI%XDIFF_RATIO(JJ,JB,JPATCH)
