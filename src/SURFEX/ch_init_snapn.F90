...
! Read the temporal profiles of all snaps
  ...
  ELSE
    CALL READ_SURF(HPROGRAM,YRECFM,CHN%XSNAP_MONTHLY(:,:,JSPEC),IRESP,YCOMMENT,HDIR='-')
  ENDIF
  ...
  ELSE
    CALL READ_SURF(HPROGRAM,YRECFM,CHN%XSNAP_DAILY(:,:,JSPEC),IRESP,YCOMMENT,HDIR='-')
  ENDIF
  ...
  ELSE
    CALL READ_SURF(HPROGRAM,YRECFM,CHN%XSNAP_HOURLY(:,:,JSPEC),IRESP,YCOMMENT,HDIR='-')
  ENDIF
END DO
!
