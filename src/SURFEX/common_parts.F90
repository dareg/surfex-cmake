...
!#############################################################
!
!!****  *COMMON_PARTS_n* - routine to initialize ISBA
!!
!!    PURPOSE
!!    -------
!!
...
!
IF (LHOOK) CALL DR_HOOK('COMMON_PARTS',0,ZHOOK_HANDLE)
!
...
IF(HPHOTO /= 'NON' .AND. HINIT == 'ALL') THEN
  ...
  CALL CO2_INIT_n(KSIZE_NATURE_P, KR_NATURE_P, PVEGTYPE_PATCH, &
...
ENDIF
!
IF (LHOOK) CALL DR_HOOK('COMMON_PARTS',1,ZHOOK_HANDLE)
!
END SUBROUTINE COMMON_PARTS
