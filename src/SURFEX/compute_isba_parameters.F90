...
INTEGER           :: JI, JL, JK     ! loop increment
...
!
! Load randomly perturbed fields. 
! Perturbation ratios are saved in case fields are reset later.
!
!
  CALL READ_SURF(HPROGRAM,'VERSION',IVERSION,IRESP)
  CALL READ_SURF(HPROGRAM,'BUG',IBUGFIX,IRESP)
...
!
  ALLOCATE(ZWORK(KI,IO%NPATCH))
!
  CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, 'PERTVEG', ZWORK, KRESP=IRESP)
!
  IF ( IRESP == 0 ) THEN
    ALLOCATE(S%XPERTVEG(KI,IO%NPATCH))
    S%XPERTVEG(:,:)=ZWORK(:,:)
  ENDIF
!
  CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, 'PERTLAI', ZWORK, KRESP=IRESP)
  IF ( IRESP == 0 ) THEN
    ALLOCATE(S%XPERTLAI(KI,IO%NPATCH))
    S%XPERTLAI(:,:)=ZWORK(:,:)
  ENDIF
!
  CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, 'PERTCV', ZWORK, KRESP=IRESP)
  IF ( IRESP == 0 ) THEN
    ALLOCATE(S%XPERTCV(KI,IO%NPATCH))
    S%XPERTCV(:,:)=ZWORK(:,:)
  ENDIF
!
  CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, 'PERTALB', ZWORK, KRESP=IRESP)
  IF ( IRESP == 0 ) THEN
    ALLOCATE(S%XPERTALB(KI,IO%NPATCH))
    S%XPERTALB(:,:)=ZWORK(:,:)
  ENDIF
!
  CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, 'PERTZ0LAND', ZWORK, KRESP=IRESP)
  IF ( IRESP == 0 ) THEN
    ALLOCATE(S%XPERTZ0(KI,IO%NPATCH))
    S%XPERTZ0(:,:)=ZWORK(:,:)
  ENDIF
!
  DEALLOCATE(ZWORK)
!
  CALL APPLY_SURF_PERT (ISS, NISS, IO, S, NP, NPE)
!
ENDIF
!
!-------------------------------------------------------------------------------
