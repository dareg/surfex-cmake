...
IF( IO%CRUNOFF=='SGH '.AND. HINIT/='PRE' .AND. .NOT.LASSIM ) THEN 
  ...
  DO JP = 1,IO%NPATCH
    ...
    IF(IO%CISBA=='DIF') THEN
      ALLOCATE(PK%XTOPQS(KI,IO%NGROUND_LAYER))
      PK%XTOPQS(:,:) = 0.0
    ELSE
...
!
 CALL DIAG_ISBA_INIT_n(CHI, ID%DE, ID%DEC, ID%DEP, ID%DEPC, ID%O, &
                       ID%D, ID%DC, ID%DP, ID%DPC, ID%DM, ID%DMP, &
