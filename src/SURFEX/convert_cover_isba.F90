...
IF (PRESENT(PRE25)) THEN
  IF (SIZE(PRE25)>0) &
    CALL AV_PGD (PRE25  ,PCOVER ,XDATA_RE25  (:,:),YNAT,'ARI',OCOVER)  
END IF
...
IF(HISBA/='DIF')THEN
  ...
  !
  DO JLAYER=1,KGROUND
     CALL AV_PGD (PDG(:,JLAYER,:),PCOVER,ZDATA_DG(:,JLAYER,:),YNAT,'ARI',OCOVER)
  ENDDO
  !
ELSE
!
  CALL AV_PGD (ZDTOT (:,:),PCOVER,XDATA_GROUND_DEPTH,YNAT,'ARI',OCOVER)
!  
! CALCULATION OF GROUND_DEPTH over Permafrost area
  ...
  ENDIF
!  
  IF (LDG2) CALL AV_PGD (PDG2  (:,:),PCOVER,XDATA_ROOT_DEPTH,YNAT,'ARI',OCOVER)
  IF (LDROOT .OR. LROOTFRAC) THEN
    CALL AV_PGD (ZDROOT(:,:),PCOVER,XDATA_ROOT_DEPTH,YDIF,'ARI',OCOVER)
...
!
  IF (LROOTFRAC) THEN
!      
    CALL AV_PGD (ZROOT_EXT(:,:),PCOVER,XDATA_ROOT_EXTINCTION(:,:),YDIF,'ARI',OCOVER)
    CALL AV_PGD (ZROOT_LIN(:,:),PCOVER,XDATA_ROOT_LIN(:,:),YDIF,'ARI',OCOVER)
