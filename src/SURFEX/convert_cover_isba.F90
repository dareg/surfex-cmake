...
IF (SIZE(OCOVER)/=SIZE(XDATA_GROUND_DEPTH,1)) THEN
  ...
  IF (GTOWN_TO_ROCK) THEN
    WHERE (YDTCO%XDATA_TOWN(:)>0..OR.YDTCO%XDATA_GARDEN(:)>0.)
      YDTCO%XDATA_NATURE(:) = YDTCO%XDATA_NATURE(:) + YDTCO%XDATA_GARDEN(:)* YDTCO%XDATA_TOWN(:)
      YDTCO%XDATA_TOWN  (:) = YDTCO%XDATA_TOWN(:) * (1. - YDTCO%XDATA_GARDEN(:))
      YDTCO%XDATA_VEGTYPE(:,NVT_ROCK) = MAX(YDTCO%XDATA_TOWN(:),YDTCO%XDATA_VEGTYPE(:,NVT_ROCK))
      YDTCO%XDATA_NATURE(:) = YDTCO%XDATA_NATURE(:) + YDTCO%XDATA_TOWN(:)
      ZDATA_ROOT_DEPTH  (:,NVT_ROCK) = 0.2
      ZDATA_GROUND_DEPTH(:,NVT_ROCK) = 0.2
    END WHERE
  ELSEIF (.NOT.GGARDEN) THEN
    YDTCO%XDATA_NATURE(:) = YDTCO%XDATA_NATURE(:) + YDTCO%XDATA_GARDEN(:) + YDTCO%XDATA_TOWN(:)
    YDTCO%XDATA_GARDEN(:) = 0.
  ENDIF

  CALL INI_DATA_PARAM(YDTCO%XDATA_VEGTYPE,PSURF=YDTCO%XDATA_NATURE,PSURF2=YDTCO%XDATA_GARDEN,&
                      PROOT_LIN=ZDATA_ROOT_LIN,PROOT_EXTINCTION=ZDATA_ROOT_EXTINCTION)
