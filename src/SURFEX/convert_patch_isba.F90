...
!
  IF (IO%CISBA/='DIF') THEN
    IF (GDATA .AND. DTV%LDATA_DICE) THEN
  ...
  ENDIF
!
  IF (GDATA .AND. DTV%LDATA_Z0_O_Z0H) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!
  IF (IO%CPHOTO/='NON'.OR.LTREEDRAG) THEN
    IF (GDATA .AND. DTV%LDATA_H_TREE) THEN
      CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!
  IF (IO%CPHOTO/='NON') THEN
    ...
    IF (SIZE(PK%XRE25)>0) THEN
      IF (GDATA .AND. DTV%LDATA_RE25) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
    ...
    IF (SIZE(PK%XDMAX)>0) THEN
      IF (GDATA .AND. DTV%LDATA_DMAX) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!   VEG
!   ----
  IF (GDATA .AND. DTV%LDATA_ISBA) THEN
...
!   LAI
!   ----
  IF (GDATA .AND. DTV%LDATA_LAI) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!emis needs VEG by vegtypes is changed at this step
  IF (GDATA .AND. DTV%LDATA_EMIS) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!    Z0V
!    ----
  IF (GDATA .AND. DTV%LDATA_Z0) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!
 ENDIF
  ...
  IF (GDATA .AND. DTV%LDATA_ALBNIR_VEG) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PEK%XALBNIR_VEG,DTV%XPAR_VEGTYPE,DTV%XPAR_ALBNIR_VEG,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ...
  ENDIF
!
  IF (GDATA .AND. DTV%LDATA_ALBVIS_VEG) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PEK%XALBVIS_VEG,DTV%XPAR_VEGTYPE,DTV%XPAR_ALBVIS_VEG,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSEIF (IO%CALBEDO=='CM13') THEN
...
!
  ...
  ELSEIF (GDATA .AND. DTV%LDATA_ALBUV_VEG) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PEK%XALBUV_VEG,DTV%XPAR_VEGTYPE,DTV%XPAR_ALBUV_VEG,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSE
...
!        ----------------
  IF( SIZE(PEK%XRSMIN)>0) THEN
    IF (GDATA .AND. DTV%LDATA_RSMIN) THEN
      CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
  ...
  ENDIF
!
  IF (GDATA .AND. DTV%LDATA_GAMMA) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
  ...
  ENDIF
!
  IF (GDATA .AND. DTV%LDATA_WRMAX_CF) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
  ...
  ENDIF
!
  IF (GDATA .AND. DTV%LDATA_RGL) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
  ...
  ENDIF
!
  IF (GDATA .AND. DTV%LDATA_CV) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!
  IF (ISIZE_LMEB_PATCH>0 .OR. IO%CPHOTO/='NON') THEN
    ...
    IF( SIZE(PEK%XBSLAI)>0) THEN
      IF (GDATA .AND. DTV%LDATA_BSLAI) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!
  ...
  !
    IF (SIZE(PEK%XLAIMIN)>0) THEN
      IF (GDATA .AND. DTV%LDATA_LAIMIN) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
  ...
  !
    IF (SIZE(PEK%XSEFOLD)>0) THEN
      IF (GDATA .AND. DTV%LDATA_SEFOLD) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
    ...
    IF ( SIZE(PEK%XGMES)>0) THEN
      IF (GDATA .AND. DTV%LDATA_GMES) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
    ...
    IF ( SIZE(PEK%XGC)>0) THEN
      IF (GDATA .AND. DTV%LDATA_GC) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
    ...
    !
      IF (SIZE(PEK%XF2I)>0) THEN
        IF (GDATA .AND. DTV%LDATA_F2I) THEN
          CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
      ...
      !
        IF (SIZE(PEK%XCE_NITRO)>0) THEN
          IF (GDATA .AND. DTV%LDATA_CE_NITRO) THEN
            CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
        ...
        IF (SIZE(PEK%XCF_NITRO)>0) THEN
          IF (GDATA .AND. DTV%LDATA_CF_NITRO) THEN
            CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
        ...
        IF (SIZE(PEK%XCNA_NITRO)>0) THEN
          IF (GDATA .AND. DTV%LDATA_CNA_NITRO) THEN
            CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!   GNDLITTER
!   ---------
  IF (GDATA .AND. DTV%LDATA_GNDLITTER) THEN
...
!        H_VEG
!        -----
  IF (GDATA .AND. DTV%LDATA_H_VEG) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!    Z0LITTER
!    --------
  IF (GDATA .AND. DTV%LDATA_Z0LITTER) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!
  ...
  !
    IF (SIZE(PEK%XIRRIG)>0) THEN
      IF (GDATA .AND. DTV%LDATA_IRRIG) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!
    IF (SIZE(PEK%XWATSUP)>0) THEN
      IF (GDATA .AND. DTV%LDATA_WATSUP) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
IF (OALB) THEN
!
  IF (GDATA .AND. DTV%LDATA_ALBNIR_SOIL) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PEK%XALBNIR_SOIL,DTV%XPAR_VEGTYPE,DTV%XPAR_ALBNIR_SOIL,YBAR,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSEIF (IO%CALBEDO=='CM13') THEN
  ...
  ENDIF
!
  IF (GDATA .AND. DTV%LDATA_ALBVIS_SOIL) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PEK%XALBVIS_SOIL,DTV%XPAR_VEGTYPE,DTV%XPAR_ALBVIS_SOIL,YBAR,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSEIF (IO%CALBEDO=='CM13') THEN
...
!
  ...
  ELSEIF (GDATA .AND. DTV%LDATA_ALBUV_SOIL) THEN
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PEK%XALBUV_SOIL,DTV%XPAR_VEGTYPE,DTV%XPAR_ALBUV_SOIL,YNAT,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSE
...
!
IF (GDATA .AND. DTV%LDATA_STRESS) THEN
  ZSTRESS(:,:)=0.
  DO JVEGTYPE=1,NVEGTYPE
    WHERE (DTV%LPAR_STRESS(:,JVEGTYPE)) ZSTRESS(:,JVEGTYPE)=1.
  ENDDO
  CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
...
!
GDATA_DG           = GDATA .AND. DTV%LDATA_DG
GDATA_GROUND_DEPTH = GDATA .AND. DTV%LDATA_GROUND_DEPTH
GDATA_ROOT_DEPTH   = GDATA .AND. DTV%LDATA_ROOT_DEPTH
GDATA_ROOTFRAC     = GDATA .AND. DTV%LDATA_ROOTFRAC
!
!####################################################################################
!
...
IF (IO%CISBA=='DIF' .OR. .NOT.GDATA_DG) THEN
  ...
  IF (GNOECO) THEN
    ...
    !CONSISTENCY CHECKS
    IF (DTV%LDATA_DG) ZDG2(:) = MIN(ZDG2(:),PK%XDG(:,KGROUND))
  ...
  IF (IO%CISBA=='DIF' .AND. .NOT.DTV%LDATA_ROOTFRAC) THEN
    !
      DO JJ=1,KNI
...
IF (IO%CISBA=='DIF') THEN
  ...
  ELSE
    !
    !DEPENDS ON DROOT
    IF (GDATA .AND. DTV%LDATA_ROOT_LIN) THEN
      CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
    ...
    ENDIF
    !
    IF (GDATA .AND. DTV%LDATA_ROOT_EXTINCTION) THEN
      CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
