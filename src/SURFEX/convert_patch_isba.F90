!     #########
      SUBROUTINE CONVERT_PATCH_ISBA (DTCO, DTI, IO, &
...
!
REAL, DIMENSION(:),   OPTIONAL, INTENT(IN)   :: PALBVIS_DRY
REAL, DIMENSION(:),   OPTIONAL, INTENT(IN)   :: PALBNIR_DRY
REAL, DIMENSION(:),   OPTIONAL, INTENT(IN)   :: PALBUV_DRY
REAL, DIMENSION(:),   OPTIONAL, INTENT(IN)   :: PALBVIS_WET
REAL, DIMENSION(:),   OPTIONAL, INTENT(IN)   :: PALBNIR_WET
REAL, DIMENSION(:),   OPTIONAL, INTENT(IN)   :: PALBUV_WET
!
...
IF (PRESENT(PALBUV_VEG)) THEN
  IF ((IO%CALBEDO=='CM13'.OR.OTR_ML).AND.PRESENT(PALBVIS_VEG)) THEN
...
IF (PRESENT(PALBNIR_SOIL)) THEN
  ...
  ELSEIF (IO%CALBEDO=='CM13') THEN
    CALL AV_PGD(DTCO, &
        ...
        OCOVER,KDECADE=KDECADE)
  ELSE
    CALL SOIL_ALBEDO (IO%CALBEDO, PWSAT(:,1),PWG1, PALBVIS_DRY,PALBNIR_DRY,PALBUV_DRY,     &
                      PALBVIS_WET,PALBNIR_WET,PALBUV_WET, PALBNIR_SOIL=PALBNIR_SOIL )
  ENDIF
ENDIF
!
IF (PRESENT(PALBVIS_SOIL)) THEN
  ...
  ELSEIF (IO%CALBEDO=='CM13') THEN
    CALL AV_PGD(DTCO, &
        ...
        OCOVER,KDECADE=KDECADE)    
  ELSE
   CALL SOIL_ALBEDO (IO%CALBEDO, PWSAT(:,1),PWG1, PALBVIS_DRY,PALBVIS_DRY,PALBUV_DRY,     &
                     PALBVIS_WET,PALBNIR_WET,PALBUV_WET, PALBVIS_SOIL=PALBVIS_SOIL )
  ENDIF
ENDIF
!
IF (PRESENT(PALBUV_SOIL)) THEN
  IF ((IO%CALBEDO=='CM13'.OR.OTR_ML).AND.PRESENT(PALBVIS_SOIL)) THEN
  ...
  ELSE
    ...
    ELSE
      CALL SOIL_ALBEDO (IO%CALBEDO, PWSAT(:,1),PWG1, PALBVIS_DRY,PALBUV_DRY,PALBUV_DRY,     &
                        PALBVIS_WET,PALBNIR_WET,PALBUV_WET,PALBUV_SOIL=PALBUV_SOIL )
    ENDIF
  ENDIF
ENDIF
!
       ...
       DO JPATCH=1,KPATCH
          DO JJ=1,KNI
             IF(OPERM.AND.IWG_LAYER(JJ,JPATCH)/=NUNDEF)THEN
               IF(PPERM(JJ)<XPERMFRAC) ZDG2(JJ,JPATCH)=PDG(JJ,IWG_LAYER(JJ,JPATCH),JPATCH)
