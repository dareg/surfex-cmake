...
! 
USE MODD_ISBA_n,         ONLY : CALBEDO, LMEB_PATCH,                      &
                                XALBNIR_DRY, XALBVIS_DRY, XALBUV_DRY,     &
                                XALBNIR_WET, XALBVIS_WET, XALBUV_WET,     &
                                XWSAT, XPERM
!
USE MODD_DATA_COVER_n,   ONLY : XDATA_VEGTYPE, XDATA_NATURE, XDATA_WEIGHT
...
IF (PRESENT(PVEGTYPE)) THEN
  IF (GDATA .AND. LDATA_VEGTYPE) THEN
    PVEGTYPE=XPAR_VEGTYPE
  ELSE
    !classical ecoclimap case
    DO JVEGTYPE=1,NVEGTYPE
      CALL AV_PGD (PVEGTYPE(:,JVEGTYPE),PCOVER ,XDATA_VEGTYPE(:,JVEGTYPE),YNAT,'ARI',OCOVER)
    END DO
  ENDIF
ENDIF
!
...
IF (PRESENT(PLAIGV)) THEN
  IF (GDATA .AND. LDATA_LAIGV) THEN
    CALL AV_PGD_PARAM(PLAIGV,XPAR_VEGTYPE,XPAR_LAIGV(:,KDECADE2,:),YVEG,'ARI',KDECADE=KDECADE2)
  ELSE
...
IF (PRESENT(PH_VEG)) THEN
  IF (GDATA .AND. LDATA_H_VEG) THEN
    CALL AV_PGD_PARAM(PH_VEG,XPAR_VEGTYPE,XPAR_H_VEG(:,KDECADE2,:),YVEG,'ARI',KDECADE=KDECADE2)
  ELSE
...
! In case of MEB, force 0<PH_VEG<XUNDEF for those patches where LMEB_PATCH=.T.
  IF (ISIZE_LMEB_PATCH>0)THEN
    ...
    DO JJ=1,SIZE(LMEB_PATCH)
      IF(LMEB_PATCH(JJ))THEN
        ZH_VEG=PH_VEG(:,JJ)
...
IF (PRESENT(PRSMINGV)) THEN
  IF( SIZE(PRSMINGV)>0) THEN
    IF (GDATA .AND. LDATA_RSMINGV) THEN
      CALL AV_PGD_PARAM(PRSMINGV,XPAR_VEGTYPE,XPAR_RSMINGV,YLAI,'INV',KDECADE=KDECADE2)
    ELSE
...
IF (PRESENT(PGAMMAGV)) THEN
  IF (GDATA .AND. LDATA_GAMMAGV) THEN
    CALL AV_PGD_PARAM(PGAMMAGV,XPAR_VEGTYPE,XPAR_GAMMAGV,YVEG,'ARI',KDECADE=KDECADE2)
  ELSE
...
IF (PRESENT(PWRMAX_CF)) THEN
  IF (GDATA .AND. LDATA_WRMAX_CF) THEN
...
IF (PRESENT(PWRMAX_CFGV)) THEN
  IF (GDATA .AND. LDATA_WRMAX_CFGV) THEN
    CALL AV_PGD_PARAM(PWRMAX_CFGV,XPAR_VEGTYPE,XPAR_WRMAX_CFGV,YVEG,'ARI',KDECADE=KDECADE2)
  ELSE
...
IF (PRESENT(PRGLGV)) THEN
  IF (GDATA .AND. LDATA_RGLGV) THEN
    CALL AV_PGD_PARAM(PRGLGV,XPAR_VEGTYPE,XPAR_RGLGV,YVEG,'ARI',KDECADE=KDECADE2)
  ELSE
...
IF (PRESENT(PZF_TALLVEG)) THEN
  IF (GDATA .AND. LDATA_ZF_TALLVEG) THEN
    CALL AV_PGD_PARAM(PZF_TALLVEG,XPAR_VEGTYPE,XPAR_ZF_TALLVEG,YVEG,'ARI',KDECADE=KDECADE2)
  ELSE
...
IF (PRESENT(PZ0_O_Z0H)) THEN
  IF (GDATA .AND. LDATA_Z0_O_Z0H) THEN
...
IF (PRESENT(PALBNIR_VEG)) THEN
  IF (GDATA .AND. LDATA_ALBNIR_VEG) THEN
...
IF (PRESENT(PALBVIS_VEG)) THEN
  IF (GDATA .AND. LDATA_ALBVIS_VEG) THEN
...
IF (HPHOTO/='NON') THEN
  ...
  IF (PRESENT(PH_TREE)) THEN
    IF (GDATA .AND. LDATA_H_TREE) THEN
  ...
  IF (PRESENT(PLAIMIN)) THEN
    IF (SIZE(PLAIMIN)>0) THEN
      IF (GDATA .AND. LDATA_LAIMIN) THEN
  ...
  IF (PRESENT(PSEFOLD)) THEN
    IF (SIZE(PSEFOLD)>0) THEN
      IF (GDATA .AND. LDATA_SEFOLD) THEN
    ...
    IF (HPHOTO=='NIT' .OR. HPHOTO=='NCB') THEN
      ...
      IF (PRESENT(PCE_NITRO)) THEN
        IF (SIZE(PCE_NITRO)>0) THEN
          IF (GDATA .AND. LDATA_CE_NITRO) THEN
      ...
      IF (PRESENT(PCF_NITRO)) THEN
        IF (SIZE(PCF_NITRO)>0) THEN
          IF (GDATA .AND. LDATA_CF_NITRO) THEN
      ...
      IF (PRESENT(PCNA_NITRO)) THEN
        IF (SIZE(PCNA_NITRO)>0) THEN
          IF (GDATA .AND. LDATA_CNA_NITRO) THEN
...
IF ((HPHOTO == 'LAI' .OR. HPHOTO == 'LST' .OR. HPHOTO == 'NIT' .OR. HPHOTO=='NCB') .AND. OAGRIP)  THEN
  ...
  IF (PRESENT(PWATSUP)) THEN  
    IF (SIZE(PWATSUP)>0) THEN
      IF (GDATA .AND. LDATA_WATSUP) THEN
...
IF (PRESENT(PALBNIR_SOIL)) THEN
  IF (GDATA .AND. LDATA_ALBNIR_SOIL) THEN
...
IF (PRESENT(PALBVIS_SOIL)) THEN
  IF (GDATA .AND. LDATA_ALBVIS_SOIL) THEN
...
!
IF (GDATA .AND. LDATA_STRESS) THEN
  ZSTRESS(:,:)=0.
  DO JVEGTYPE=1,NVEGTYPE
...
GDATA_ROOTFRACGV   = GDATA .AND. LDATA_ROOTFRACGV
!
!####################################################################################
!
       ...
       DO JPATCH=1,KPATCH
          DO JJ=1,KNI
             IF(OPERM.AND.IWG_LAYER(JJ,JPATCH)/=NUNDEF)THEN
               IF(XPERM(JJ)<XPERMFRAC) ZDG2(JJ,JPATCH)=PDG(JJ,IWG_LAYER(JJ,JPATCH),JPATCH)
...
IF (HISBA=='DIF') THEN
  ...
  IF ( (GDATA_ROOTFRAC .OR. GDATA_ROOTFRACGV) .AND. (LDG2 .OR. LDROOT .OR. LROOTFRAC .OR. LROOTFRACGV)) THEN
    ...
    IF(LROOTFRACGV)THEN
      DO JL=1,KGROUND
        CALL AV_PGD_PARAM(PROOTFRACGV(:,JL,:),XPAR_VEGTYPE,XPAR_ROOTFRACGV(:,JL,:),YNAT,'ARI')
      ENDDO
    ENDIF
    !
    !ROOTFRAC IN NAMELIST
    DO JL=1,KGROUND
      CALL AV_PGD_PARAM(ZROOTFRAC(:,JL,:),XPAR_VEGTYPE,XPAR_ROOTFRAC(:,JL,:),YNAT,'ARI',KDECADE=KDECADE)
    ENDDO
  ...
  ELSEIF (LROOTFRAC .OR. LROOTFRACGV) THEN
    ...
    ENDIF
    IF(LROOTFRACGV)THEN
      IF (GDATA .AND. LDATA_ROOT_EXTINCTIONGV) THEN
        CALL AV_PGD_PARAM(ZROOT_EXT(:,:),XPAR_VEGTYPE,XPAR_ROOT_EXTINCTIONGV(:,:),YDIF,'ARI')
      ELSE
