...
IF (IO%CISBA/='2-L') THEN 
  ...
  IF (GDATA_GROUND_DEPTH .AND. (IO%CISBA=='DIF' .OR. .NOT.GDATA_DG)) THEN
    ...
    WHERE(ZDTOT(:)/=XUNDEF) ZDTOT(:)=NINT(ZDTOT(:)*ZPREC)/ZPREC
    !CONSISTENCY CHECK
...
IF (IO%CISBA=='DIF' .OR. .NOT.GDATA_DG) THEN
  ...
  IF (GNOECO) THEN
    ...
    CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
            ...
            PK%NR_P,IO%NPATCH,KPATCH)
    !Error Due to machine precision
    WHERE(ZDG2(:)/=XUNDEF) ZDG2(:)=NINT(ZDG2(:)*ZPREC)/ZPREC  
    !CONSISTENCY CHECKS
    ...
    IF (IO%CISBA=='DIF') THEN
     ...
     !Error Due to machine precision
      WHERE(PK%XDROOT(:)/=XUNDEF)
          PK%XDROOT(:)=NINT(PK%XDROOT(:)*ZPREC)/ZPREC
      ENDWHERE 
       IF(CDGDIF=='ROOT')THEN
...
IF (IO%CISBA=='DIF') THEN
  ...
  ENDIF
  !
  WHERE(PK%XROOTFRAC(:,:)/=XUNDEF) PK%XROOTFRAC(:,:)=NINT(PK%XROOTFRAC(:,:)*ZPREC)/ZPREC
  !
  PK%XDG2(:) = ZDG2(:)
  !
ENDIF
