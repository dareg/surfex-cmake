!     #########
      SUBROUTINE CONVERT_PATCH_ISBA_1P (DTCO, DTI, IO, KDEC, KDEC2, PCOVER, OCOVER,&
...
!             ---------------
!
IF (LHOOK) CALL DR_HOOK('CONVERT_PATCH_ISBA_1P',0,ZHOOK_HANDLE)
!
...
!
  IF (IO%CISBA/='DIF') THEN
    IF (GDATA .AND. DTI%LDATA_DICE) THEN
      CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                        PK%XD_ICE,DTI%XPAR_VEGTYPE,DTI%XPAR_DICE,YNAT,'ARI',PK%NR_P,IO%NPATCH,KPATCH)
    ELSE
...
!
  IF (GDATA .AND. DTI%LDATA_Z0_O_Z0H) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PK%XZ0_O_Z0H,DTI%XPAR_VEGTYPE,DTI%XPAR_Z0_O_Z0H,YNAT,'ARI',PK%NR_P,IO%NPATCH,KPATCH)
  ELSE
...
!
  IF (IO%CPHOTO/='NON'.OR.LTREEDRAG) THEN
    IF (GDATA .AND. DTI%LDATA_H_TREE) THEN
      CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PK%XH_TREE,DTI%XPAR_VEGTYPE,DTI%XPAR_H_TREE,YTREE,'ARI',PK%NR_P,IO%NPATCH,KPATCH)
    ELSE
  ...
  IF (IO%CPHOTO/='NON') THEN
    ...
    IF (SIZE(PK%XRE25)>0) THEN
      IF (GDATA .AND. DTI%LDATA_RE25) THEN
        CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PK%XRE25,DTI%XPAR_VEGTYPE,DTI%XPAR_RE25,YNAT,'ARI',PK%NR_P,IO%NPATCH,KPATCH)      
      ELSE
    ...
    IF (SIZE(PK%XDMAX)>0) THEN
      IF (GDATA .AND. DTI%LDATA_DMAX) THEN
        CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PK%XDMAX,DTI%XPAR_VEGTYPE,DTI%XPAR_DMAX,YTREE,'ARI',PK%NR_P,IO%NPATCH,KPATCH)
      ELSEIF (IO%CPHOTO == 'AST' .OR. IO%CPHOTO == 'LST' .OR. IO%CPHOTO == 'NIT' .OR. IO%CPHOTO == 'NCB') THEN
        CALL AV_PGD_1P(DTCO, PK%XDMAX,PCOVER,XDATA_DMAX_ST,YTREE,'ARI',OCOVER,&
                PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)  
      ELSE
        CALL AV_PGD_1P(DTCO, PK%XDMAX,PCOVER,XDATA_DMAX,YTREE,'ARI',OCOVER,&
                PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)
      ENDIF
    ENDIF
    !
...
!   ----
  IF (GDATA .AND. DTI%LDATA_VEG) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, PEK%XVEG,DTI%XPAR_VEGTYPE,DTI%XPAR_VEG(:,KDEC2,:),&
...
!   ----
  IF (GDATA .AND. DTI%LDATA_LAI) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XLAI,DTI%XPAR_VEGTYPE,DTI%XPAR_LAI(:,KDEC2,:),YVEG,'ARI',&
...
!emis needs VEG by vegtypes is changed at this step
  IF (GDATA .AND. DTI%LDATA_EMIS) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XEMIS ,DTI%XPAR_VEGTYPE,DTI%XPAR_EMIS(:,KDEC2,:),YNAT,'ARI',&
...
!    ----
  IF (GDATA .AND. DTI%LDATA_Z0) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XZ0,DTI%XPAR_VEGTYPE,DTI%XPAR_Z0(:,KDEC2,:),YNAT,'CDN',&
                      PK%NR_P,IO%NPATCH,KPATCH)
  ELSE
...
!
 ENDIF
  IF (GDATA .AND. DTI%LDATA_ALBNIR_VEG) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XALBNIR_VEG,DTI%XPAR_VEGTYPE,DTI%XPAR_ALBNIR_VEG,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
...
!
  IF (GDATA .AND. DTI%LDATA_ALBVIS_VEG) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XALBVIS_VEG,DTI%XPAR_VEGTYPE,DTI%XPAR_ALBVIS_VEG,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSEIF (IO%CALBEDO=='CM13') THEN
...
!
  ...
  ELSEIF (GDATA .AND. DTI%LDATA_ALBUV_VEG) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XALBUV_VEG,DTI%XPAR_VEGTYPE,DTI%XPAR_ALBUV_VEG,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSE
...
!        ----------------
  IF( SIZE(PEK%XRSMIN)>0) THEN
    IF (GDATA .AND. DTI%LDATA_RSMIN) THEN
      CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XRSMIN,DTI%XPAR_VEGTYPE,DTI%XPAR_RSMIN,YLAI,'INV',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
    ELSE
...
!
  IF (GDATA .AND. DTI%LDATA_GAMMA) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XGAMMA,DTI%XPAR_VEGTYPE,DTI%XPAR_GAMMA,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSE
...
!
  IF (GDATA .AND. DTI%LDATA_WRMAX_CF) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XWRMAX_CF,DTI%XPAR_VEGTYPE,DTI%XPAR_WRMAX_CF,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSE
...
!
  IF (GDATA .AND. DTI%LDATA_RGL) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XRGL,DTI%XPAR_VEGTYPE,DTI%XPAR_RGL,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSE
...
!
  IF (GDATA .AND. DTI%LDATA_CV) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XCV,DTI%XPAR_VEGTYPE,DTI%XPAR_CV,YVEG,'INV',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSE
  ...
  IF (ISIZE_LMEB_PATCH>0 .OR. IO%CPHOTO/='NON') THEN
    ...
    IF( SIZE(PEK%XBSLAI)>0) THEN
      IF (GDATA .AND. DTI%LDATA_BSLAI) THEN
        CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XBSLAI,DTI%XPAR_VEGTYPE,DTI%XPAR_BSLAI,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
      ELSEIF (IO%CPHOTO == 'AST' .OR. IO%CPHOTO == 'LST' .OR. IO%CPHOTO == 'NIT' .OR. IO%CPHOTO == 'NCB') THEN
        CALL AV_PGD_1P(DTCO, PEK%XBSLAI,PCOVER,XDATA_BSLAI_ST,YVEG,'ARI',OCOVER,&
                PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)  
      ELSE
        CALL AV_PGD_1P(DTCO, PEK%XBSLAI,PCOVER,XDATA_BSLAI,YVEG,'ARI',OCOVER,&
                PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)
      ENDIF
    ENDIF
  ENDIF
  ...
  !
    IF (SIZE(PEK%XLAIMIN)>0) THEN
      IF (GDATA .AND. DTI%LDATA_LAIMIN) THEN
        CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XLAIMIN,DTI%XPAR_VEGTYPE,DTI%XPAR_LAIMIN,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
      ELSE
  ...
  !
    IF (SIZE(PEK%XSEFOLD)>0) THEN
      IF (GDATA .AND. DTI%LDATA_SEFOLD) THEN
        CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XSEFOLD,DTI%XPAR_VEGTYPE,DTI%XPAR_SEFOLD,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
      ELSEIF (IO%CPHOTO == 'AST' .OR. IO%CPHOTO == 'LST' .OR. IO%CPHOTO == 'NIT' .OR. IO%CPHOTO == 'NCB') THEN
        CALL AV_PGD_1P(DTCO, PEK%XSEFOLD,PCOVER,XDATA_SEFOLD_ST,YVEG,'ARI',OCOVER,&
                PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)  
      ELSE
        CALL AV_PGD_1P(DTCO, PEK%XSEFOLD,PCOVER,XDATA_SEFOLD,YVEG,'ARI',OCOVER,&
                PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)
      ENDIF
    ENDIF
    !
    IF ( SIZE(PEK%XGMES)>0) THEN
      IF (GDATA .AND. DTI%LDATA_GMES) THEN
        CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XGMES,DTI%XPAR_VEGTYPE,DTI%XPAR_GMES,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
      ELSEIF (IO%CPHOTO == 'AST' .OR. IO%CPHOTO == 'LST' .OR. IO%CPHOTO == 'NIT' .OR. IO%CPHOTO == 'NCB') THEN
        CALL AV_PGD_1P(DTCO, PEK%XGMES,PCOVER,XDATA_GMES_ST,YVEG,'ARI',OCOVER,&
                PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)  
      ELSE
        CALL AV_PGD_1P(DTCO, PEK%XGMES,PCOVER,XDATA_GMES,YVEG,'ARI',OCOVER,&
                PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)
      ENDIF
    ENDIF
    !
    IF ( SIZE(PEK%XGC)>0) THEN
      IF (GDATA .AND. DTI%LDATA_GC) THEN
        CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                          PEK%XGC,DTI%XPAR_VEGTYPE,DTI%XPAR_GC,YVEG,'ARI',&
                          PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
      ELSEIF (IO%CPHOTO == 'AST' .OR. IO%CPHOTO == 'LST' .OR. IO%CPHOTO == 'NIT' .OR. IO%CPHOTO == 'NCB') THEN
        CALL AV_PGD_1P(DTCO, PEK%XGC,PCOVER,XDATA_GC_ST,YVEG,'ARI',OCOVER,&
                PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)  
      ELSE
        CALL AV_PGD_1P(DTCO, PEK%XGC,PCOVER,XDATA_GC,YVEG,'ARI',OCOVER,&
                PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)
      ENDIF
    ENDIF
    !
    ...
    !
      IF (SIZE(PEK%XF2I)>0) THEN
        IF (GDATA .AND. DTI%LDATA_F2I) THEN
          CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XF2I,DTI%XPAR_VEGTYPE,DTI%XPAR_F2I,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
        ELSE
      ...
      !
        IF (SIZE(PEK%XCE_NITRO)>0) THEN
          IF (GDATA .AND. DTI%LDATA_CE_NITRO) THEN
            CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XCE_NITRO,DTI%XPAR_VEGTYPE,DTI%XPAR_CE_NITRO,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
          ELSE
        ...
        IF (SIZE(PEK%XCF_NITRO)>0) THEN
          IF (GDATA .AND. DTI%LDATA_CF_NITRO) THEN
            CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XCF_NITRO,DTI%XPAR_VEGTYPE,DTI%XPAR_CF_NITRO,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
          ELSE
        ...
        IF (SIZE(PEK%XCNA_NITRO)>0) THEN
          IF (GDATA .AND. DTI%LDATA_CNA_NITRO) THEN
            CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XCNA_NITRO,DTI%XPAR_VEGTYPE,DTI%XPAR_CNA_NITRO,YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
          ELSE
...
!       --------
  IF (SIZE(PEK%LSTRESS)>0) THEN
    CALL SET_STRESS(SIZE(PEK%LSTRESS))      
  ENDIF
!
 ENDIF
!
...
!   ---------
  IF (GDATA .AND. DTI%LDATA_GNDLITTER) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, PEK%XGNDLITTER,DTI%XPAR_VEGTYPE,&
                        DTI%XPAR_GNDLITTER(:,KDEC2,:),YNAT,'ARI',PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSE
...
!    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
!                      PEK%XLAIGV,DTI%XPAR_VEGTYPE,DTI%XPAR_LAIGV(:,KDEC2,:),YVEG,'ARI',&
!                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
!  ELSE
!    CALL AV_PGD_1P(DTCO, PEK%XLAIGV,PCOVER,XDATA_LAIGV(:,KDEC,:),YVEG,'ARI',OCOVER,&
!            PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)
!  ENDIF
!
!        H_VEG
!        -----
  IF (GDATA .AND. DTI%LDATA_H_VEG) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XH_VEG,DTI%XPAR_VEGTYPE,DTI%XPAR_H_VEG(:,KDEC2,:),YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSE
...
!    --------
  IF (GDATA .AND. DTI%LDATA_Z0LITTER) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XZ0LITTER,DTI%XPAR_VEGTYPE,DTI%XPAR_Z0LITTER(:,KDEC2,:),YNAT,'CDN',&
                      PK%NR_P,IO%NPATCH,KPATCH)
  ELSE
  ...
  ENDIF
!
!  IF( SIZE(PEK%XRSMINGV)>0) THEN
!    IF (GDATA .AND. DTI%LDATA_RSMINGV) THEN
!      CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
!                      PEK%XRSMINGV,DTI%XPAR_VEGTYPE,DTI%XPAR_RSMINGV,YLAI,'INV',&
!                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
!    ELSE
!      CALL AV_PGD_1P(DTCO, PEK%XRSMINGV,PCOVER,XDATA_RSMINGV,YLAI,'INV',OCOVER,&
!              PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)  
!    ENDIF
!  ENDIF
!
...
!    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
!                      PEK%XGAMMAGV,DTI%XPAR_VEGTYPE,DTI%XPAR_GAMMAGV,YVEG,'ARI',&
!                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
!  ELSE
!    CALL AV_PGD_1P(DTCO, PEK%XGAMMAGV,PCOVER,XDATA_GAMMAGV,YVEG,'ARI',OCOVER,&
!            PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)  
!  ENDIF
!
!  IF (GDATA .AND. DTI%LDATA_WRMAX_CFGV) THEN
!    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
!                      PEK%XWRMAX_CFGV,DTI%XPAR_VEGTYPE,DTI%XPAR_WRMAX_CFGV,YVEG,'ARI',&
!                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
!  ELSE
!    CALL AV_PGD_1P(DTCO, PEK%XWRMAX_CFGV,PCOVER,XDATA_WRMAX_CFGV,YVEG,'ARI',OCOVER,&
!            PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)  
!  ENDIF
!
!  IF (GDATA .AND. DTI%LDATA_RGLGV) THEN
!    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
!                      PEK%XRGLGV,DTI%XPAR_VEGTYPE,DTI%XPAR_RGLGV,YVEG,'ARI',&
!                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
!  ELSE
!    CALL AV_PGD_1P(DTCO, PEK%XRGLGV,PCOVER,XDATA_RGLGV,YVEG,'ARI',OCOVER,&
!            PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)  
!  ENDIF
!
ENDIF
  ...
  !
    IF (SIZE(PEK%XIRRIG)>0) THEN
      IF (GDATA .AND. DTI%LDATA_IRRIG) THEN
        CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XIRRIG,DTI%XPAR_VEGTYPE,DTI%XPAR_IRRIG(:,KDEC2,:),YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
      ELSE
...
!
    IF (SIZE(PEK%XWATSUP)>0) THEN
      IF (GDATA .AND. DTI%LDATA_WATSUP) THEN
        CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XWATSUP,DTI%XPAR_VEGTYPE,DTI%XPAR_WATSUP(:,KDEC2,:),YVEG,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
      ELSE
...
!
  IF (GDATA .AND. DTI%LDATA_ALBNIR_SOIL) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XALBNIR_SOIL,DTI%XPAR_VEGTYPE,DTI%XPAR_ALBNIR_SOIL,YBAR,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSEIF (IO%CALBEDO=='CM13') THEN
...
!
  IF (GDATA .AND. DTI%LDATA_ALBVIS_SOIL) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XALBVIS_SOIL,DTI%XPAR_VEGTYPE,DTI%XPAR_ALBVIS_SOIL,YBAR,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSEIF (IO%CALBEDO=='CM13') THEN
...
!
  ...
  ELSEIF (GDATA .AND. DTI%LDATA_ALBUV_SOIL) THEN
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PEK%XALBUV_SOIL,DTI%XPAR_VEGTYPE,DTI%XPAR_ALBUV_SOIL,YNAT,'ARI',&
                      PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
  ELSE
...
IF (ASSOCIATED(DTCO%XDATA_WEIGHT)) DEALLOCATE(DTCO%XDATA_WEIGHT)
!
IF (LHOOK) CALL DR_HOOK('CONVERT_PATCH_ISBA_1P',1,ZHOOK_HANDLE)
!
!-------------------------------------------------------------------------------
CONTAINS
!-------------------------------------------------------------------------------
!
SUBROUTINE SET_STRESS(KSIZE1)
!
IMPLICIT NONE
!
INTEGER, INTENT(IN) :: KSIZE1
!
REAL, DIMENSION(KSIZE1)   :: ZWORK
REAL, DIMENSION(KSIZE1,NVEGTYPE) :: ZSTRESS
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!
IF (LHOOK) CALL DR_HOOK('CONVERT_PATCH_ISBA_1P:SET_STRESS',0,ZHOOK_HANDLE)
!
IF (GDATA .AND. DTI%LDATA_STRESS) THEN
  ...
  ENDDO
  CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      ZWORK,DTI%XPAR_VEGTYPE,ZSTRESS,YVEG,'ARI',PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC2)
ELSE
...
END WHERE
!
IF (LHOOK) CALL DR_HOOK('CONVERT_PATCH_ISBA_1P:SET_STRESS',1,ZHOOK_HANDLE)
...
!
USE MODI_INI_DATA_ROOTFRAC1
USE MODI_INI_DATA_SOIL1
...
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!
IF (LHOOK) CALL DR_HOOK('CONVERT_PATCH_ISBA_1P:SET_GRID_PARAM',0,ZHOOK_HANDLE)
!
IF(IO%CISBA=='DIF')THEN
  IF(.NOT.OFIX) CALL ABOR1_SFX('CONVERT_PATCH_ISBA_1P: SET_GRID_PARAM: KWG_LAYER, PDROOT and PGD2 must be present with DIF')
...
IF (GDATA_DG) THEN
  ...
  DO JLAYER=1,KGROUND
    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PK%XDG(:,JLAYER),DTI%XPAR_VEGTYPE,DTI%XPAR_DG(:,JLAYER,:),YNAT,CDGAVG,&
                      PK%NR_P,IO%NPATCH,KPATCH)
  ENDDO
  !
ENDIF
...
IF (IO%CISBA=='DIF' .OR. .NOT.GDATA_DG) THEN
  ...
  IF (GNOECO) THEN
    ...
    !CONSISTENCY CHECKS
    IF (DTI%LDATA_DG) ZDG2(:) = MIN(ZDG2(:),PK%XDG(:,KGROUND))
    ZDTOT(:) = MAX(ZDG2(:),ZDTOT(:))
    IF (IO%CISBA=='DIF') THEN
      CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PK%XDROOT(:),DTI%XPAR_VEGTYPE,DTI%XPAR_ROOT_DEPTH(:,:),YDIF,CDGAVG,&
            PK%NR_P,IO%NPATCH,KPATCH)
     !Error Due to machine precision
      WHERE(PK%XDROOT(:)/=XUNDEF)
          PK%XDROOT(:)=INT(PK%XDROOT(:)*ZPREC)/ZPREC
      ENDWHERE 
       IF(CDGDIF=='ROOT')THEN
  ...
  IF (.NOT.GDATA_DG) THEN
    ...
    CALL INI_DATA_SOIL1(IO%CISBA, PK%XDG,PROOTDEPTH=ZDG2, PSOILDEPTH=ZDTOT,&
...
IF (IO%CISBA=='DIF') THEN
  ...
  IF (GDATA_ROOTFRAC .OR. GDATA_ROOTFRACGV) THEN
    ...
    !  DO JL=1,KGROUND
    !    CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
    !                  PEK%XROOTFRACGV(:,JL),DTI%XPAR_VEGTYPE,DTI%XPAR_ROOTFRACGV(:,JL,:),YNAT,'ARI',&
    !        PK%NR_P,IO%NPATCH,KPATCH)
    !  ENDDO
    ENDIF
    !
    ...
    DO JL=1,KGROUND
      CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
                      PK%XROOTFRAC(:,JL),DTI%XPAR_VEGTYPE,DTI%XPAR_ROOTFRAC(:,JL,:),YNAT,'ARI',&
            PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)
    ENDDO
    !    
    ZDG2  (:)=0.0
  ...
  ELSE
    ...
    ENDIF
    !
    CALL INI_DATA_ROOTFRAC1(PK%XDG,PK%XDROOT,ZROOT_EXT,ZROOT_LIN,PK%XROOTFRAC)
  ENDIF
  IF(GMEB)THEN
    IF (GDATA .AND. DTI%LDATA_ROOT_EXTINCTIONGV) THEN
      CALL AV_PGD_PARAM_1P(DTI%XPAR_LAI, DTI%XPAR_VEG, &
            ...
            PK%NR_P,IO%NPATCH,KPATCH)
    ELSE
      CALL AV_PGD_1P(DTCO, ZROOT_EXT(:),PCOVER,XDATA_ROOT_EXTINCTIONGV(:,:),YDIF,'ARI',OCOVER,&
            PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)
    ENDIF
    !
    !CALL INI_DATA_ROOTFRAC1(PK%XDG,PK%XDROOT,ZROOT_EXT,ZROOT_LIN,  &
    !                       PEK%XROOTFRACGV,OGV=GMEB)
    !
  ENDIF
  !
  PK%XDG2(:) = ZDG2(:)
...
ENDIF
!
IF (LHOOK) CALL DR_HOOK('CONVERT_PATCH_ISBA_1P:SET_GRID_PARAM',1,ZHOOK_HANDLE)
!
END SUBROUTINE SET_GRID_PARAM
!-------------------------------------------------------------------------------
END SUBROUTINE CONVERT_PATCH_ISBA_1P
