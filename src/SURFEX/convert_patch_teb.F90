...
USE MODD_DATA_BEM_n
!
USE MODD_TEB_OPTION_n,      ONLY : LGARDEN, CBLD_ATYPE
USE MODD_BLD_DESCRIPTION_n, ONLY : NDESC_ROOF_LAYER, NDESC_WALL_LAYER, &
...
IF (PRESENT(PGARDEN)) THEN
  IF (LDATA_GARDEN) THEN
    PGARDEN=XPAR_GARDEN
  ELSE
    CALL AV_PGD (PGARDEN, PCOVER, XDATA_GARDEN(:),'TWN','ARI',OCOVER)
  ENDIF
ENDIF
!
!
...
IF (PRESENT(PBLD)) THEN
  IF (LDATA_BLD) THEN
    PBLD=XPAR_BLD  
  ELSE
    CALL AV_PGD (PBLD, PCOVER, XDATA_BLD(:),'TWN','ARI',OCOVER)
  ENDIF
ENDIF
!
!
...
IF (PRESENT(PWALL_O_HOR)) THEN
  ...
  ELSE
    CALL AV_PGD (PWALL_O_HOR ,PCOVER ,XDATA_WALL_O_HOR (:),'BLD','ARI',OCOVER)  
  ENDIF  
ENDIF
!
!      ALB_ROOF
...
IF (PRESENT(PEMIS_PANEL)) THEN
  IF (LDATA_EMIS_PANEL) THEN
    PEMIS_PANEL=XPAR_EMIS_PANEL
  ELSEIF (LDATA_BLDTYPE) THEN
    CALL INI_DATA_PARAM_TEB(NPAR_BLDCODE,PEMIS_PANEL=PEMIS_PANEL)
  ELSE
...
IF (PRESENT(PALB_PANEL)) THEN
  IF (LDATA_ALB_PANEL) THEN
    PALB_PANEL=XPAR_ALB_PANEL
  ELSEIF (LDATA_BLDTYPE) THEN
    CALL INI_DATA_PARAM_TEB(NPAR_BLDCODE,PALB_PANEL=PALB_PANEL)
  ELSE
...
IF (PRESENT(PEFF_PANEL)) THEN
  IF (LDATA_EFF_PANEL) THEN
    PEFF_PANEL=XPAR_EFF_PANEL
  ELSEIF (LDATA_BLDTYPE) THEN
    CALL INI_DATA_PARAM_TEB(NPAR_BLDCODE,PEFF_PANEL=PEFF_PANEL)
  ELSE
...
IF (PRESENT(PFRAC_PANEL)) THEN
  IF (LDATA_FRAC_PANEL) THEN
    PFRAC_PANEL=XPAR_FRAC_PANEL
  ELSEIF (LDATA_BLDTYPE) THEN
    CALL INI_DATA_PARAM_TEB(NPAR_BLDCODE,PFRAC_PANEL=PFRAC_PANEL)
  ELSE
...
IF (PRESENT(PRESIDENTIAL)) THEN 
  IF (LDATA_RESIDENTIAL) THEN
    PRESIDENTIAL=XPAR_RESIDENTIAL
  ELSEIF (LDATA_USETYPE) THEN
    CALL INI_DATA_PARAM_TEB(NPAR_USETYPE,PRESIDENTIAL=PRESIDENTIAL)
  ELSE
