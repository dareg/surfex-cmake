!     ###############################################################################
SUBROUTINE COUPLING_FLAKE_OROGRAPHY_n (CHF, CHS, CHW, DTS, DGMF, DGO, DGS, DGSI, DGW, DST, O, OR, SG, S, SLT, W, &
                                        DGF, F, FSB, &
...
!
  ...
  ZSNOW(:) = XUNDEF
!        
   CALL FORCING_VERT_SHIFT(PZS,F%XZS,PTA,PQA,PPA,PRHOA,PLW,PRAIN,PSNOW,&
...
!
 CALL COUPLING_SEAWAT_SBL_n(CHF, CHS, CHW, DTS, DGF, DGMF, DGO, DGS, DGSI, DGW, DST, F, O, OR, SG, S, SLT, W, &
                            HPROGRAM, HCOUPLING, PTIMEC, 'F', &
               PTSTEP, KYEAR, KMONTH, KDAY, PTIME, &
               KI, KSV, KSW, &
               PTSUN, PZENITH, PZENITH2, PAZIM, &
               PZREF, PUREF, F%XZS, PU, PV, ZQA, ZTA, ZRHOA, PSV, PCO2, HSV, &
               ZRAIN, ZSNOW, ZLW, PDIR_SW, PSCA_SW, PSW_BANDS, ZPS, ZPA, &
               PSFTQ, PSFTH, PSFTS, PSFCO2, PSFU, PSFV, F%LSBL, F%XTS, F%XZ0, &
               FSB%XZ, FSB%XU, FSB%NLVL, FSB%XTKE, FSB%XT, FSB%XQ, FSB%XLMO, FSB%XZF, FSB%XDZ, FSB%XDZF, FSB%XP, &
               DGF%N2M, DGF%XT2M, DGF%XQ2M, DGF%XHU2M, DGF%XZON10M, DGF%XMER10M, DGF%XWIND10M, DGF%XWIND10M_MAX, &
               DGF%XT2M_MIN, DGF%XT2M_MAX, DGF%XHU2M_MIN, DGF%XHU2M_MAX, &
               PTRAD, PDIR_ALB, PSCA_ALB, PEMIS, PTSURF, PZ0, PZ0H, PQSURF, &
               PPEW_A_COEF, PPEW_B_COEF, &
