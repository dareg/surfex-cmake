!     ###############################################################################
SUBROUTINE COUPLING_TSZ0_n (DTCO, UG, U, USS, IM, DTZ, DTGD, DTGR, DST, SLT,  &
                            HPROGRAM, HCOUPLING,                                              &
...
!
!
REAL, DIMENSION(KI,IM%I%O%NGROUND_LAYER,IM%I%O%NPATCH) :: ZTG   ! soil temperature
REAL, DIMENSION(KI,IM%I%O%NGROUND_LAYER,IM%I%O%NPATCH) :: ZWG   ! soil water content
REAL, DIMENSION(KI,IM%I%O%NGROUND_LAYER,IM%I%O%NPATCH) :: ZWGI  ! soil ice content
REAL, DIMENSION(KI,IM%I%O%NPATCH) :: ZWR   ! interception reservoir
REAL, DIMENSION(KI,IM%I%O%NPATCH) :: ZRESA ! aerodynamical resistance
REAL, DIMENSION(KI,IM%I%R%TSNOW%NLAYER,IM%I%O%NPATCH) :: ZWSNOW! snow reservoir
REAL, DIMENSION(KI,IM%I%R%TSNOW%NLAYER,IM%I%O%NPATCH) :: ZRHOSN! snow density
REAL, DIMENSION(KI,IM%I%R%TSNOW%NLAYER,IM%I%O%NPATCH) :: ZHEASN! snow heat content
REAL, DIMENSION(KI,IM%I%O%NPATCH) :: ZALBSN! snow albedo
REAL, DIMENSION(KI,IM%I%O%NPATCH) :: ZEMISN! snow emissivity
!
...
!
 CALL TSZ0(DTZ, &
           PTIME, PTSTEP, IM%I%IP%XWFC, IM%I%R%XTG, IM%I%R%XWG)
!
!
...
ZWR  (:,:)   = IM%I%R%XWR        (:,:)
ZRESA(:,:)   = IM%I%R%XRESA      (:,:)
ZWSNOW(:,:,:)= IM%I%R%TSNOW%WSNOW(:,:,:)
ZRHOSN(:,:,:)= IM%I%R%TSNOW%RHO  (:,:,:)
ZALBSN(:,:)  = IM%I%R%TSNOW%ALB  (:,:)
IF (IM%I%R%TSNOW%SCHEME=='3-L' .OR. IM%I%R%TSNOW%SCHEME=='CRO') THEN
  ZHEASN(:,:,:)= IM%I%R%TSNOW%HEAT (:,:,:)
  ZEMISN(:,:)  = IM%I%R%TSNOW%EMIS (:,:)
END IF
!
!
...
IM%I%R%XWR  (:,:)   = ZWR
IM%I%R%XRESA(:,:)   = ZRESA
IM%I%R%TSNOW%WSNOW(:,:,:) = ZWSNOW
IM%I%R%TSNOW%RHO  (:,:,:) = ZRHOSN
IM%I%R%TSNOW%ALB  (:,:)   = ZALBSN
IF (IM%I%R%TSNOW%SCHEME=='3-L' .OR. IM%I%R%TSNOW%SCHEME=='CRO') THEN
  IM%I%R%TSNOW%HEAT (:,:,:) = ZHEASN
  IM%I%R%TSNOW%EMIS (:,:)   = ZEMISN
END IF
!
