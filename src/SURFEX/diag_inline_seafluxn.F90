...
IF (DGMSI%LDIAG_MISC_SEAICE) THEN
   IF (TRIM(S%CSEAICE_SCHEME) == 'GELATO') THEN 
      S%GLTPARAM%NX=SIZE(PTA)
      DGMSI%XSIT  = RESHAPE(glt_avhicem(S%TGLT%dom,S%TGLT%sit,S%GLTPARAM%nt,S%GLTPARAM%nx,S%GLTPARAM%ny),(/S%GLTPARAM%NX/))
      DGMSI%XSND  = RESHAPE(glt_avhsnwm(S%TGLT%dom,S%TGLT%sit,S%GLTPARAM%nt,S%GLTPARAM%nx,S%GLTPARAM%ny),(/S%GLTPARAM%NX/))
