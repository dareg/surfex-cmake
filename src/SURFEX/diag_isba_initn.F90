...
IF ( OPROSNOW .AND. HSNOW_SCHEME=="CRO" ) THEN
  ...
  IF (DMA%LPROBANDS) THEN
		ALLOCATE(DMA%XSPEC_ALB(KLUA,KSW))
		ALLOCATE(DMA%XDIFF_RATIO(KLUA,KSW))
	ELSE
		ALLOCATE(DMA%XSPEC_ALB(KLUA,KSW))
		ALLOCATE(DMA%XDIFF_RATIO(KLUA,KSW))
  !AVOIR RAFIFE (les dimensions était toujours a 0 pour le 2eme CALL ALLOC_MISC_BUD 
  !a la ligne 1079 car DMA%LPROBANDS est false pour NDM%AL)
