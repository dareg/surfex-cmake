  ...
  ELSE
    ...
    IF(DGO%LPATCH_BUDGET .AND. IO%NPATCH>1)THEN
      ...
      IF (ISIZE_LMEB_PATCH>0) THEN
        ...
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XLEVCV(:))
        ENDDO
        !        
        YREC='LESCC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XLESC(:))
        ENDDO   
        !       
        YREC='LETRCVC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XLETRCV(:))
        ENDDO   
        !       
        YREC='LERCVC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XLERCV(:))
        ENDDO
        !        
        YREC='LE_C_AC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XLE_C_A(:))
        ENDDO   
        !       
        YREC='LE_V_CC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XLE_V_C(:))
        ENDDO
        !        
        YREC='LE_G_CC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XLE_G_C(:))
        ENDDO 
        !       
        YREC='LE_N_CC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XLE_N_C(:))
        ENDDO          
        !
        YREC='SWNT_VC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        ...
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XH_V_C(:))
        ENDDO 
        !        
        YREC='H_G_CC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XH_G_C(:))
        ENDDO  
        !        
        YREC='H_C_AC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XH_C_A(:))
        ENDDO       
        !      
        YREC='H_N_CC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XH_N_C(:))
        ENDDO  
        !       
        YREC='SR_GNC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        ...
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XMELTCV(:))
        ENDDO
        !        
        YREC='FRZCVC_'//YREC2
        CALL MAKE_CHOICE_ARRAY(HPROGRAM, IO%NPATCH, GDIM, YREC, ZWORK)
        DO JP = 1,IO%NPATCH
          CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XFRZCV(:))
        ENDDO
        !        
      ENDIF
      !
...
IF (YDM%LSURF_MISC_BUDGET) THEN
  !
  CALL ALLOC_MISC_BUD(DM,KLU,0,IO%NGROUND_LAYER,0)
  DO JP=1,IO%NPATCH
    CALL ALLOC_MISC_BUD(NDM%AL(JP),NP%AL(JP)%NSIZE_P,&
                        NP%AL(JP)%NSIZE_P,IO%NGROUND_LAYER,KSNOW_NLAYER)
  ENDDO
  !
ELSE
  !
  CALL ALLOC_MISC_BUD(DM,0,0,0,0)
  DO JP=1,IO%NPATCH
    CALL ALLOC_MISC_BUD(NDM%AL(JP),0,0,0,0)
  ENDDO  
  !
END IF
!
...
!
!
SUBROUTINE ALLOC_MISC_BUD(DMA,KLUA,KLUAP,KNLAYER,KSNLAYER)
!
