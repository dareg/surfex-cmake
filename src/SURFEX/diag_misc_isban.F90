...
!
REAL, DIMENSION(SIZE(PPSN))    :: ZCORR_SNOWLIQ
...
IF (DGMI%LSURF_MISC_BUDGET) THEN
  ...
  IF (DGMI%LPROSNOW) THEN
    DO JJ = 1,SIZE(PWSNOW,1)
      !this variable is used further for snowliq
      ZCORR_SNOWLIQ(JJ)=1./PSLOPECOS(JJ)
    END DO
    DO JI = 1,SIZE(PWSNOW,2)
      DO JJ = 1,SIZE(PWSNOW,1)
        ZWSNOW(JJ,JI)=PWSNOW(JJ,JI)*ZCORR_SNOWLIQ(JJ)
      ENDDO
    ENDDO
  ELSE
    ZCORR_SNOWLIQ(:)=1.
...
!cdir nodep
      DO JJ=1,KSIZE
        ...
        IF (DGMI%LVOLUMETRIC_SNOWLIQ) THEN
          ...
          ELSE
            DGMI%XSNOWLIQ (JI,JK,KPATCH)  =  PKDI%XP_SNOWLIQ(JJ,JK)*ZCORR_SNOWLIQ(JJ)*XRHOLW/PKDI%XP_SNOWDZ(JJ,JK)
          ENDIF
        ELSE
          DGMI%XSNOWLIQ (JI,JK,KPATCH)  =  PKDI%XP_SNOWLIQ(JJ,JK)*ZCORR_SNOWLIQ(JJ)
        ENDIF
        DGMI%XSNOWTEMP(JI,JK,KPATCH)  =  PKDI%XP_SNOWTEMP   (JJ,JK)
        
        IF (DGMI%LPROSNOW) THEN
          IF (DGMI%XTWSNOW  (JI, KPATCH)>0.) THEN
            DGMI%XSNOWDZ(JI,JK,KPATCH)=PKDI%XP_SNOWDZ(JJ,JK)!*ZCORR_SLOPE_2D(JJ,JK) A rajouter plus tard
          ELSE
