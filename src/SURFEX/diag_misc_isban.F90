...
!
REAL, DIMENSION(SIZE(PPSN))    :: ZCORR_SLOPE
REAL, DIMENSION(SIZE(PWSNOW,1),SIZE(PWSNOW,2)) :: ZCORR_SLOPE_2D
...
IF (DGMI%LSURF_MISC_BUDGET) THEN
  ...
  IF (DGMI%LPROSNOW) THEN
    ...
    DO JI = 1,SIZE(PWSNOW,2)
      DO JJ = 1,SIZE(PWSNOW,1)
        IF (PWSNOW(JJ,JI)>0) THEN
          ZCORR_SLOPE_2D(JJ,JI)=ZCORR_SLOPE(JJ)
        ELSE
          ZCORR_SLOPE_2D(JJ,JI)=1.
        ENDIF
        ZWSNOW(JJ,JI)=PWSNOW(JJ,JI)*ZCORR_SLOPE_2D(JJ,JI)
      ENDDO
    ENDDO
  ELSE
    ZCORR_SLOPE(:)=1.
...
!cdir nodep 
    DO JJ = 1,SIZE(PWSNOW,1)
      ZWORK(JJ,JI)  = ZWSNOW(JJ,JI) / PRSNOW(JJ,JI)
    ENDDO
  ENDDO
  !
  PKDI%XP_TWSNOW=0.
...
!cdir nodep 
    DO JJ = 1,SIZE(PWSNOW,1)
      PKDI%XP_TWSNOW(JJ) = PKDI%XP_TWSNOW(JJ) + ZWSNOW(JJ,JI)      
...
!cdir nodep
  DO JJ=1,KSIZE
     ...
     IF ((DGMI%LPROSNOW).AND.(HSNOW=='CRO')) THEN
     
       IF (DGMI%XTWSNOW  (JI, KPATCH)>0.) THEN
        DGMI%XSNOWDEPTH_1DAYS(JI, KPATCH)= PKDI%XP_SNOWDEPTH_1DAYS(JJ)*ZCORR_SLOPE(JJ)
        DGMI%XSNOWDEPTH_3DAYS(JI, KPATCH)= PKDI%XP_SNOWDEPTH_3DAYS(JJ)*ZCORR_SLOPE(JJ)
        DGMI%XSNOWDEPTH_5DAYS(JI, KPATCH)= PKDI%XP_SNOWDEPTH_5DAYS(JJ)*ZCORR_SLOPE(JJ)
        DGMI%XSNOWDEPTH_7DAYS(JI, KPATCH)= PKDI%XP_SNOWDEPTH_7DAYS(JJ) *ZCORR_SLOPE(JJ)   
        DGMI%XSNOWSWE_1DAYS(JI, KPATCH)= PKDI%XP_SNOWSWE_1DAYS(JJ)*ZCORR_SLOPE(JJ)
        DGMI%XSNOWSWE_3DAYS(JI, KPATCH)= PKDI%XP_SNOWSWE_3DAYS(JJ)*ZCORR_SLOPE(JJ)
        DGMI%XSNOWSWE_5DAYS(JI, KPATCH)= PKDI%XP_SNOWSWE_5DAYS(JJ)*ZCORR_SLOPE(JJ)
        DGMI%XSNOWSWE_7DAYS(JI, KPATCH)= PKDI%XP_SNOWSWE_7DAYS(JJ)*ZCORR_SLOPE(JJ)
        DGMI%XSNOWRAM_SONDE(JI, KPATCH)= PKDI%XP_SNOWRAM_SONDE(JJ)*ZCORR_SLOPE(JJ)
        DGMI%XSNOW_REFROZENTHICKNESS(JI, KPATCH)= PKDI%XP_SNOW_REFROZENTHICKNESS(JJ)*ZCORR_SLOPE(JJ) 
        DGMI%XSNOW_WETTHICKNESS(JI, KPATCH)= PKDI%XP_SNOW_WETTHICKNESS(JJ)*ZCORR_SLOPE(JJ)
       ELSE
	 DGMI%XSNOWDEPTH_1DAYS(JI, KPATCH)= XUNDEF
	 DGMI%XSNOWDEPTH_3DAYS(JI, KPATCH)= XUNDEF
	 DGMI%XSNOWDEPTH_5DAYS(JI, KPATCH)= XUNDEF
	 DGMI%XSNOWDEPTH_7DAYS(JI, KPATCH)= XUNDEF
	 DGMI%XSNOWSWE_1DAYS(JI, KPATCH)= XUNDEF
	 DGMI%XSNOWSWE_3DAYS(JI, KPATCH)= XUNDEF
	 DGMI%XSNOWSWE_5DAYS(JI, KPATCH)= XUNDEF
	 DGMI%XSNOWSWE_7DAYS(JI, KPATCH)= XUNDEF
	 DGMI%XSNOWRAM_SONDE(JI, KPATCH)= XUNDEF
	 DGMI%XSNOW_REFROZENTHICKNESS(JI, KPATCH)= XUNDEF   
	 DGMI%XSNOW_WETTHICKNESS(JI, KPATCH)= XUNDEF
       ENDIF
     ENDIF
     !
  END DO
...
!cdir nodep
      DO JJ=1,KSIZE
        ...
        DGMI%XSNOWLIQ (JI,JK,KPATCH)  =  PKDI%XP_SNOWLIQ    (JJ,JK)
          ELSE
            DGMI%XSNOWLIQ (JI,JK,KPATCH)  =  PKDI%XP_SNOWLIQ(JJ,JK)*ZCORR_SLOPE_2D(JJ,JK)*XRHOLW/PKDI%XP_SNOWDZ(JJ,JK)
          ENDIF
        ELSE
          DGMI%XSNOWLIQ (JI,JK,KPATCH)  =  PKDI%XP_SNOWLIQ(JJ,JK)*ZCORR_SLOPE_2D(JJ,JK)
        ENDIF
        DGMI%XSNOWTEMP(JI,JK,KPATCH)  =  PKDI%XP_SNOWTEMP   (JJ,JK)
        
        IF (DGMI%LPROSNOW) THEN
          IF (DGMI%XTWSNOW  (JI, KPATCH)>0.) THEN
            DGMI%XSNOWDZ(JI,JK,KPATCH)=PKDI%XP_SNOWDZ(JJ,JK)*ZCORR_SLOPE_2D(JJ,JK)
          ELSE
            DGMI%XSNOWDZ(JI,JK,KPATCH)=XUNDEF
          ENDIF
          IF (HSNOW=='CRO') THEN
	  ...
	  ELSE
	    DGMI%XSNOWDZ(JI,JK,KPATCH)=XUNDEF
	    DGMI%XSNOWDEND(JI,JK,KPATCH)=XUNDEF
	    DGMI%XSNOWSPHER(JI,JK,KPATCH)=XUNDEF
	    DGMI%XSNOWSIZE(JI,JK,KPATCH)=XUNDEF
	    DGMI%XSNOWSSA(JI,JK,KPATCH)=XUNDEF
	    DGMI%XSNOWTYPEMEPRA(JI,JK,KPATCH)=XUNDEF
	    DGMI%XSNOWRAM(JI,JK,KPATCH)=XUNDEF
	    DGMI%XSNOWSHEAR(JI,JK,KPATCH)=XUNDEF
	  ENDIF
        ENDIF
        ENDIF
  ...
  IF (HSNOW=='CRO') THEN
    DO JJ=1,KSIZE
      JI                      =  KMASK         (JJ)
      !
      DGMI%XSYTMASS (JI,KPATCH)  =  PKDI%XP_SYTMASS    (JJ)*ZCORR_SLOPE(JJ)
    ...
    IF (DGMI%LPROSNOW) THEN
      DO JIMP=1,NIMPUR
        DO JK=1,SIZE(PKDI%XP_SNOWLIQ,2)
          DO JJ=1,KSIZE
            JI                      =  KMASK         (JJ)
            DGMI%XIMPUR(JI,JK,JIMP,KPATCH)  =  PKDI%XP_SNOWIMPUR(JJ,JK,JIMP)
          ENDDO
        ENDDO
      ENDDO
    ENDIF
