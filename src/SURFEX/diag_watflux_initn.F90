!     #########
      SUBROUTINE DIAG_WATFLUX_INIT_n (OREAD_BUDGETC, DWO, DGW, DGWC, W, HPROGRAM,KLU,KSW)
!     #####################
!
...
!
#ifdef SFX_OL
IF (DWO%LSURF_BUDGETC .OR. (LRESTART .AND. .NOT.DWO%LRESET_BUDGETC)) THEN
#else
IF (DWO%LSURF_BUDGETC .OR. .NOT.DWO%LRESET_BUDGETC) THEN
#endif
!        
...
!
  IF (.NOT. OREAD_BUDGETC) THEN        
     ...
     DGWC%XFMU  = 0.0
     DGWC%XFMV  = 0.0
  ELSEIF (OREAD_BUDGETC.AND.DWO%LRESET_BUDGETC) THEN
     DGWC%XRN   = 0.0
     DGWC%XH    = 0.0
     DGWC%XLE   = 0.0
     DGWC%XLEI  = 0.0
  ...
  ELSE
     YREC='RNC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XRN,IRESP)
     YREC='HC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XH,IRESP)
     YREC='LEC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XLE,IRESP)
     YREC='LEIC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XLEI,IRESP)     
     YREC='GFLUXC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XGFLUX,IRESP)
     YREC='SWDC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XSWD,IRESP)
     YREC='SWUC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XSWU,IRESP)
     YREC='LWDC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XLWD,IRESP)
     YREC='LWUC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XLWU,IRESP)
     YREC='FMUC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XFMU,IRESP)
     YREC='FMVC_WAT'
     CALL READ_SURF(HPROGRAM,YREC,DGWC%XFMV,IRESP)
!
     CALL READ_SURF(&
      ...
      ELSE
         YREC='EVAPC_WAT'
         CALL READ_SURF(HPROGRAM,YREC,DGWC%XEVAP,IRESP)
         YREC='SUBLC_WAT'
         CALL READ_SURF(HPROGRAM,YREC,DGWC%XSUBL,IRESP)              
      ENDIF
!
  ENDIF   
ELSE
