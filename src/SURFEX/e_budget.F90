!     #########
      SUBROUTINE E_BUDGET(IO, IP, INI, IMT, IR, DGIP, DGMI, HSNOW_ISBA, HIMPLICIT_WIND,  &
...
!
!
REAL, DIMENSION(SIZE(IR%XSNOWFREE_ALB(:,1))) :: ZRORA,                        &
!                                             rhoa / ra
!
                                               ZA,ZB,ZC  
...
! ISBA-DF:
!
REAL, DIMENSION(SIZE(IR%XSNOWFREE_ALB(:,1))) ::  ZCONDAVG, ZCOND1, ZCOND2, ZTERM2, ZTERM1
!
! implicit atmospheric coupling coefficients: (modified-form)
!
REAL, DIMENSION(SIZE(IR%XSNOWFREE_ALB(:,1))) :: ZPET_A_COEF, ZPEQ_A_COEF, ZPET_B_COEF,      &
...
!
REAL, DIMENSION(SIZE(IR%XSNOWFREE_ALB(:,1))) :: ZUSTAR2, ZVMOD
!                              ZUSTAR2 = friction     (m2/s2)
!                              ZVMOD   = wind modulus (m/s)
REAL, DIMENSION(SIZE(IR%XSNOWFREE_ALB(:,1))) :: ZXCPV_XCL_AVG
REAL, DIMENSION(SIZE(IR%XSNOWFREE_ALB(:,1))) :: ZCNHUMA, ZPEQA2, ZDPQB, ZCDQSAT, ZINCR, ZTRAD, &
...
!
PDQSAT(:) = DQSAT(IR%XTG(:,1,1),PPS(:),PQSAT(:))
!                                              function zrsra
!
! Modify flux-form implicit coupling coefficients:
! - wind components:
!
ZTEMP  (:) = DGIP%XCD(:)*PVMOD(:)
!
...
!
!print*,'RHOA ',PRHOA(1)
!print*,'RESA ',IR%XRESA(1,1)
ZRORA(:)    = PRHOA(:) / IR%XRESA(:,1)
!
...
!
!print*,'PET_B_COEF ',PPET_B_COEF(1)
!print*,'PET_A_COEF ',PPET_A_COEF(1)
!print*,'EXNA ',PEXNA(1)
!print*,'EXNS ',PEXNS(1)
...
!
ZFV(:) = IMT%XVEG(:,1)     * (1-IR%XPSNV(:,1)-INI%XFFV(:,1))
ZFG(:) = (1.-IMT%XVEG(:,1))*(1.-IR%XPSNG(:,1)-INI%XFFG(:,1))
ZFNFRZ(:) = (1.-INI%XFFROZEN(:,1))*INI%XFF(:,1) + ZFV + ZFG(:)*(1.-PFROZEN1(:))
ZFFRZ(:)  = INI%XFFROZEN(:,1)     *INI%XFF(:,1) +       ZFG(:)*PFROZEN1(:)      + IR%XPSN(:,1)
!
ZSNOW      = 1.
ZFNSNOW(:) = 1.
ZCPS(:)    = IP%XPCPS(:,1)
...
IF (LCPL_ARP) THEN
  ...
  ZLAVG(:)        = IP%XPLVTT(:,1)*ZFNFRZ(:) + IP%XPLSTT(:,1)*ZFFRZ(:)
...
!
ZHUMA(:) = ZLVTT(:)/ZLAVG(:) * ((1.-INI%XFFROZEN(:,1))*INI%XFF(:,1) + ZFV(:)*DGMI%XHV(:) + ZFGNFRZ(:))   +  &
           ZLSTT(:)/ZLAVG(:) * (INI%XFFROZEN(:,1)     *INI%XFF(:,1) + ZFGFRZ(:) + ZSNOW*IR%XPSN(:,1) ) 
!
ZHUMS(:) = ZLVTT(:)/ZLAVG(:) * ((1.-INI%XFFROZEN(:,1))*INI%XFF(:,1) + ZFV(:)*DGMI%XHV(:) + ZFGNFRZ(:)*DGIP%XHUG(:)) +   &
           ZLSTT(:)/ZLAVG(:) * (INI%XFFROZEN(:,1)     *INI%XFF(:,1) + ZFGFRZ(:)*PHUI(:)  + ZSNOW*IR%XPSN(:,1) )  
!
IF(HSNOW_ISBA == '3-L' .OR. HSNOW_ISBA == 'CRO' .OR. IO%CISBA == 'DIF')THEN
!
! humidity considering no snow (done elsewhere) and flooded zones:
!
  ZHUMAD(:) = INI%XFF(:,1) + ZFV(:)*DGMI%XHV(:) + ZFGNFRZ(:) + ZFGFRZ(:)  
  ZHUMSD(:) = INI%XFF(:,1) + ZFV(:)*DGMI%XHV(:) + ZFGNFRZ(:)*DGIP%XHUG(:) + ZFGFRZ(:)*PHUI(:)          
ELSE
  ZHUMAD(:) = ZHUMA(:)
  ZHUMSD(:) = ZHUMS(:)
ENDIF
...
ZPEQ_B_COEF(:) = ( PPEQ_B_COEF(:) - ZTEMP(:)*ZHUMSD(:)* &
                  (PQSAT(:) - PDQSAT(:)*IR%XTG(:,1,1)) )/Z_CCOEF(:)  
!
!-------------------------------------------------------------------------------
!
...
!          
     PALBT (:) = IR%XSNOWFREE_ALB(:,1)
     PEMIST(:) = IMT%XEMIS(:,1)
!     
  ELSE
!
! Taking into account the floodplains with snow grid fractions :
!     PFF    1.-PFF-IR%XPSN(:,1)   IR%XPSN(:,1)
! |------------|----|---------------|
!
  WHERE(IR%XPSN(:,1)<1.0)          
     PALBT (:) = ((1.-INI%XFF(:,1)-IR%XPSN(:,1))*IR%XSNOWFREE_ALB(:,1)  + INI%XFF(:,1)*INI%XALBF(:,1) )/(1.-IR%XPSN(:,1))
     PEMIST(:) = ((1.-INI%XFF(:,1)-IR%XPSN(:,1))*IMT%XEMIS(:,1)         + INI%XFF(:,1)*INI%XEMISF(:,1))/(1.-IR%XPSN(:,1))
  ELSEWHERE
     PALBT (:) = IR%XSNOWFREE_ALB(:,1)
     PEMIST(:) = IMT%XEMIS(:,1)
  ENDWHERE
...
!
      PALBT(:)  = (1-IMT%XVEG(:,1)) * (IR%XSNOWFREE_ALB_SOIL(:,1)*(1-IR%XPSNG(:,1)) + IR%TSNOW%ALB(:,1)*IR%XPSNG(:,1)) + &
                     IMT%XVEG(:,1)  * (IR%XSNOWFREE_ALB_VEG (:,1)*(1-IR%XPSNV_A(:,1)) +             &
                                                IR%TSNOW%ALB(:,1)*IR%XPSNV_A(:,1))  
!
      PEMIST(:) = IMT%XEMIS(:,1)-IR%XPSN(:,1)*(IMT%XEMIS(:,1)-XEMCRIN)
!      
   ELSE
!
      PALBT (:) = ( 1.-IR%XPSN(:,1)-INI%XFF(:,1))* IR%XSNOWFREE_ALB(:,1) + &
                       IR%XPSN(:,1)              * IR%TSNOW%ALB(:,1) + INI%XFF(:,1)*INI%XALBF(:,1)     
!
      PEMIST(:) = ( 1.-IR%XPSN(:,1)-INI%XFF(:,1))* IMT%XEMIS(:,1) + &
                       IR%XPSN(:,1)              * XEMISSN + INI%XFF(:,1)*INI%XEMISF(:,1)
!
   ENDIF
...
!       ------------
!
ZTRAD(:) =  PEMIST(:) * XSTEFAN * (IR%XTG(:,1,1)**3)
...
!
!print*,'CT ',DGMI%XCT(1)
!print*,'FNSNOW ',ZFNSNOW(1)
!print*,'TRAD ',ZTRAD(1)
!print*,'RORA ',ZRORA(1)
!print*,'CPS ',ZCPS(1)
!print*,'PETA2 ',ZPETA2(1)
!print*,'CHUMS ',ZCHUMS(1)
!print*,'DQSAT ',PDQSAT(1)
!print*,'CHUMA ',ZCHUMA(1)
!print*,'PEQ_A_COEF ',ZPEQ_A_COEF(1)
!print*,'TG2 ',IR%XTG(1,2,1)
!print*,'PETB2 ',ZPETB2(1)
!print*,'SW_RAD ',PSW_RAD(1)
!print*,'ALBT ',PALBT(1)
!print*,'LW_RAD ',PLW_RAD(1)
!print*,'EMIST ',PEMIST(1)
!print*,'QSAT ',PQSAT(1)
!print*,'PEQ_B_COEF ',ZPEQ_B_COEF(1)
ZA(:) = 1. / PTSTEP + DGMI%XCT(:) *                            &
         ((ZFNSNOW(:) *                                    &
           ( 4.*ZTRAD(:) + ZRORA(:)*ZCPS(:)*ZPETA2(:) ))   &
         + ZCHUMS(:)*PDQSAT(:) - ZCHUMA(:)*ZPEQ_A_COEF(:)) &
         + 2. * XPI / XDAY        
!
ZB(:) = 1. / PTSTEP + DGMI%XCT(:) * ( ZFNSNOW(:)* 3.*ZTRAD(:) + ZCHUMS(:)*PDQSAT(:) ) 
!
ZC(:) = 2. * XPI * IR%XTG(:,2,1) / XDAY + DGMI%XCT(:) *              &
       ( ZFNSNOW(:) *                                       &
       ( ZRORA(:)*ZCPS(:)*ZPETB2(:)                         &
...
!       -------------------------------------------------
!
   ZC(:) = ZC(:) + DGMI%XCT(:)*(IR%XPSN(:,1)*PGRNDFLUX(:)+PFLUX_COR(:,1))
!
...
ZINCR(:)= DGMI%XCT(:) * ZRORA(:) * &
          (ZCDQSAT(:) * ( ZPETA2(:)*IR%XTG(:,1,1) - ZPETB2(:)) + &
          ZXCPV_XCL_AVG(:) * &
          (ZHUMS(:)*PQSAT(:) - ZHUMA(:) * (ZPEQ_B_COEF(:) + ZPEQ_A_COEF(:) * IR%XTG(:,1,1)))) 
...
!
    ...
    ZPEQA2(:)=ZCNHUMA(:)*ZPEQ_A_COEF(:)*ZPETA2(:)*IR%XTG(:,1,1)
    ZDPQB(:)=ZPEQ_B_COEF(:)-PQA(:)

    ZA(:) = ZA(:) + DGMI%XCT(:) * ZRORA(:) * &
             (2.* ZPEQA2(:) + &
             ZCNHUMA(:) * (ZDPQB(:)*ZPETA2(:) - ZPEQ_A_COEF(:)*ZPETB2(:) )) 

    ZB(:) = ZB(:) + DGMI%XCT(:) * ZRORA(:) * ZPEQA2(:)         
  
    ZC(:) = ZC(:) + DGMI%XCT(:)*ZRORA(:)*ZCNHUMA(:) *ZDPQB(:)*ZPETB2(:) 
...
!
   ZCOND1(:) = IP%XDZG(:,1,1)/((IP%XDZG(:,1,1)+IP%XDZG(:,2,1))*PSOILCONDZ(:,1))
   ZCOND2(:) = IP%XDZG(:,2,1)/((IP%XDZG(:,1,1)+IP%XDZG(:,2,1))*PSOILCONDZ(:,2))
!
   ZCONDAVG(:) = 1.0/(ZCOND1(:)+ZCOND2(:))
!   
   ZA(:)       = ZA(:) - (2. * XPI / XDAY) + ZCONDAVG(:)*DGMI%XCG(:)/IP%XDZDIF(:,1,1)
   ZTERM2(:)   = ZCONDAVG(:)*DGMI%XCG(:)/(ZA(:)*IP%XDZDIF(:,1,1))
   ZTERM1(:)   = (IR%XTG(:,1,1)*ZB(:) + (ZC(:) - (2. * XPI * IR%XTG(:,2,1) / XDAY)) )/ZA(:)  
!
! Determine the soil temperatures:
!
   CALL SOIL_HEATDIF(PTSTEP,IP%XDZG(:,:,1),IP%XDZDIF(:,:,1),PSOILCONDZ,  &
                     PSOILHCAPZ,DGMI%XCG,ZTERM1,ZTERM2, PTDEEP_A,        &
                     IP%XTDEEP, IR%XTG(:,:,1), PDEEP_FLUX, PFLUX_COR   )
!
!
! "Restore" flux here is actually the heat flux between the surface
! and sub-surface layers (W m-2):
!
   PRESTORE(:) = ZCONDAVG(:)*(IR%XTG(:,1,1)-IR%XTG(:,2,1))/IP%XDZDIF(:,1,1)
!
ELSE
!
  IF(IO%LTEMP_ARP)THEN
!
    CALL SOIL_TEMP_ARP(PTSTEP,ZA,ZB,ZC,IP%XGAMMAT,IP%XTDEEP,IO%XSODELX,IR%XTG(:,:,1))
!
!     "Restore" flux between surface and deep layer(W m-2):
    PRESTORE(:) = 2.0 * XPI * (IR%XTG(:,1,1)-IR%XTG(:,2,1)) / &
                         ( DGMI%XCT(:)*XDAY*IO%XSODELX(1)*(IO%XSODELX(1)+IO%XSODELX(2)) )
!      
  ELSE
!
    IR%XTG(:,1,1) = ( IR%XTG(:,1,1)*ZB(:) + ZC(:) ) / ZA(:)
!
    WHERE(IP%XTDEEP(:) /= XUNDEF .AND. IP%XGAMMAT(:) /= XUNDEF)
      IR%XTG(:,2,1) = (IR%XTG(:,2,1) + (PTSTEP/XDAY)*(IR%XTG(:,1,1) + IP%XGAMMAT(:)*IP%XTDEEP(:))) / &
                         (1.+(PTSTEP/XDAY)*(1.0+IP%XGAMMAT(:)))  
    ELSEWHERE
      IR%XTG(:,2,1) = (IR%XTG(:,2,1) + (PTSTEP/XDAY)*IR%XTG(:,1,1)) /        &
                         (1.+(PTSTEP/XDAY) )  
    END WHERE
!
!     "Restore" flux between surface and deep layer(W m-2):
    PRESTORE(:) = 2.0*XPI*(IR%XTG(:,1,1)-PT2M(:))/(DGMI%XCT(:)*XDAY)  
!
  ENDIF
!
ENDIF
...
!
PQA_IC(:) =  ZPEQ_A_COEF(:)*IR%XTG(:,1,1)   + ZPEQ_B_COEF(:)
!
PTA_IC(:) =  ZPET_A_COEF(:)*IR%XTG(:,1,1)   + ZPET_B_COEF(:)
!
PUSTAR2_IC(:) =  ZUSTAR2(:)
!
!--------------------------------------------------------------------------------------
...
IF (LCPL_ARP) THEN
  ...
  IF (.NOT.LQVNPLUS) THEN
    IP%XPCPS(:,1) =  IP%XPCPS(:,1) + (XCPV-XCPD) *ZHUMS(:)*PDQSAT(:)*(IR%XTG(:,1,1)-PTSM(:))
  ENDIF
  ...
  IP%XPLSTT(:,1) = IP%XPLSTT(:,1) + (XCPV-XCI)*(IR%XTG(:,1,1)-PTSM(:))

  IP%XPLVTT(:,1) = IP%XPLVTT(:,1) + (XCPV-XCL)*(IR%XTG(:,1,1)-PTSM(:))
