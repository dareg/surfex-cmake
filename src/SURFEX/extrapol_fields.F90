SUBROUTINE EXTRAPOL_FIELDS (DTCO, DTI, KDIM, IO, IP, UG, U, HPROGRAM,KLUOUT)
!!
!!    PURPOSE
!!    -------
...
!   ECOCLIMAP spatial distribution field
    ...
    CALL AV_PGD(DTCO, &
                ZWORK(:,JTIME,:),IP%XCOVER,XDATA_LAI(:,JTIME,:),YVEG,'ARI',IP%LCOVER,KDECADE=JTIME)
    DO JVEGTYPE=1,3
...
!   H_TREE
!   ------
IF (.NOT.DTI%LDATA_H_TREE .AND. (IO%CPHOTO/='NON' .OR. .NOT.DTI%LDATA_Z0)) THEN
!
...
! ECOCLIMAP spatial distribution field  
  IF (ASSOCIATED(DTCO%XDATA_WEIGHT)) DEALLOCATE(DTCO%XDATA_WEIGHT)
  CALL AV_PGD(DTCO, &
                DTI%XPAR_H_TREE,IP%XCOVER,XDATA_H_TREE,YTREE,'ARI',IP%LCOVER,KDECADE=1)
!
! Extrapolation toward new vegtype distribution field from updated land-use map or user  
  CALL INI_VAR_FROM_VEGTYPE_DATA(DTCO, DTI, UG, U, &
...
IF (.NOT.DTI%LDATA_DG .AND. .NOT.DTI%LDATA_ROOT_DEPTH) THEN
  ...
  IF (ASSOCIATED(DTCO%XDATA_WEIGHT)) DEALLOCATE(DTCO%XDATA_WEIGHT)        
  CALL AV_PGD(DTCO, &
                DTI%XPAR_ROOT_DEPTH(:,:),IP%XCOVER,XDATA_ROOT_DEPTH(:,:),YNAT,CDGAVG,IP%LCOVER,KDECADE=1)
...
IF (.NOT.DTI%LDATA_DG .AND. .NOT.DTI%LDATA_ROOT_DEPTHGV) THEN
  ...
  IF (ASSOCIATED(DTCO%XDATA_WEIGHT)) DEALLOCATE(DTCO%XDATA_WEIGHT)        
  CALL AV_PGD(DTCO, &
                DTI%XPAR_ROOT_DEPTHGV(:,:),IP%XCOVER,XDATA_ROOT_DEPTHGV(:,:),YNAT,CDGAVG,IP%LCOVER,KDECADE=1)
...
IF (.NOT.DTI%LDATA_DG .AND. IO%CISBA/='2-L' .AND. .NOT.DTI%LDATA_GROUND_DEPTH) THEN
  ...
  IF (ASSOCIATED(DTCO%XDATA_WEIGHT)) DEALLOCATE(DTCO%XDATA_WEIGHT)      
  CALL AV_PGD(DTCO, &
                DTI%XPAR_GROUND_DEPTH(:,:),IP%XCOVER,ZDATA_GROUND_DEPTH(:,:),YNAT,CDGAVG,IP%LCOVER,KDECADE=1)
...
!   ECOCLIMAP spatial distribution field      
    IF (ASSOCIATED(DTCO%XDATA_WEIGHT)) DEALLOCATE(DTCO%XDATA_WEIGHT)
    CALL AV_PGD(DTCO, &
                ZWORK(:,JTIME,:),IP%XCOVER,XDATA_IRRIG,YVEG,'ARI',IP%LCOVER,KDECADE=JTIME)
!   Extrapolation toward new vegtype distribution field from updated land-use map or user  
    CALL INI_VAR_FROM_VEGTYPE_DATA(DTCO, DTI, UG, U, &
...
!   ECOCLIMAP spatial distribution field    
    IF (ASSOCIATED(DTCO%XDATA_WEIGHT)) DEALLOCATE(DTCO%XDATA_WEIGHT)
    CALL AV_PGD(DTCO, &
                ZWORK(:,JTIME,:),IP%XCOVER,XDATA_WATSUP,YVEG,'ARI',IP%LCOVER,KDECADE=JTIME)  
!   Extrapolation toward new vegtype distribution field from updated land-use map or user  
    CALL INI_VAR_FROM_VEGTYPE_DATA(DTCO, DTI, UG, U, &
