...
IF (.NOT.LDATA_DG .AND. .NOT.LDATA_ROOT_DEPTH) THEN
  YROOT=YNAT
  IF(CISBA=='DIF')YROOT=YDIF
  IF (ASSOCIATED(XDATA_WEIGHT)) DEALLOCATE(XDATA_WEIGHT)        
  CALL AV_PGD (XPAR_ROOT_DEPTH(:,:),XCOVER,XDATA_ROOT_DEPTH(:,:),YNAT,CDGAVG,LCOVER,KDECADE=1)
...
IF (.NOT.LDATA_DG .AND. CISBA/='2-L' .AND. .NOT.LDATA_GROUND_DEPTH) THEN
  ...
  IF(CISBA=='DIF'.AND.CDGDIF=='ROOT')THEN
  ...
  ENDIF
  IF (ASSOCIATED(XDATA_WEIGHT)) DEALLOCATE(XDATA_WEIGHT)      
  CALL AV_PGD (XPAR_GROUND_DEPTH(:,:),XCOVER,XDATA_GROUND_DEPTH(:,:),YNAT,CDGAVG,LCOVER,KDECADE=1)
