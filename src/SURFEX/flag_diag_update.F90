!     #########
      SUBROUTINE FLAG_DIAG_UPDATE(OFRAC, ODIAG_GRID, K2M, OSURF_BUDGET, ORAD_BUDGET, OCOEF,  &
                                  ...
                                  OINTERPOL_SST, OINTERPOL_SSS, OINTERPOL_SIC, OINTERPOL_SIT,&
...
LOGICAL, INTENT(IN) :: OINTERPOL_SSS
LOGICAL, INTENT(IN) :: OINTERPOL_SIC
LOGICAL, INTENT(IN) :: OINTERPOL_SIT
...
!
IF (CSEA   =='SEAFLX') CALL FLAG_SEA_UPDATE(KBEQ, ODIAG_OCEAN, ODIAG_SEAICE, &
                                            OINTERPOL_SST, OINTERPOL_SSS,    &
                                            OINTERPOL_SIC, OINTERPOL_SIT,    &
                                            LSURF_BUDGETC, K2M)
IF (CWATER =='FLAKE ') CALL FLAG_FLAKE_UPDATE(KBEQ, OWATER_PROFILE, LSURF_BUDGETC ,K2M)
IF (CWATER =='WATFLX') CALL FLAG_WATER_UPDATE(KBEQ, OINTERPOL_TS, LSURF_BUDGETC ,K2M)
IF (CNATURE=='ISBA  ') CALL FLAG_ISBA_UPDATE(KBEQ, KDSTEQ, OSURF_EVAP_BUDGET, OFLOOD, &
                                ...
                                LSURF_BUDGETC, K2M)
IF (CTOWN  =='TEB   ') CALL FLAG_TEB_UPDATE(KBEQ, OPGD_TEB, OSURF_MISC_BUDGET_TEB, K2M)
!
IF (LHOOK) CALL DR_HOOK('FLAG_DIAG_UPDATE',1,ZHOOK_HANDLE)
!-------------------------------------------------------------------------------
CONTAINS
!
SUBROUTINE FLAG_SEA_UPDATE(KBEQ, ODIAG_OCEAN, ODIAG_SEAICE, OINTERPOL_SST, &
                           OINTERPOL_SSS, OINTERPOL_SIC, OINTERPOL_SIT,    &
                           OSURF_BUDGETC, K2M)
!
USE MODD_CH_SEAFLUX_n,   ONLY : NBEQ
USE MODD_DIAG_OCEAN_n,   ONLY : LDIAG_OCEAN
USE MODD_DIAG_SEAICE_n,  ONLY : LDIAG_SEAICE
USE MODD_SEAFLUX_n,      ONLY : LINTERPOL_SST, LINTERPOL_SSS, LINTERPOL_SIC, LINTERPOL_SIT
USE MODD_DIAG_SEAFLUX_n, ONLY : LSURF_BUDGETC, N2M
!
IMPLICIT NONE
!
INTEGER, INTENT(IN) :: KBEQ
...
LDIAG_SEAICE = ODIAG_SEAICE
LINTERPOL_SST = OINTERPOL_SST
LINTERPOL_SSS = OINTERPOL_SSS
LINTERPOL_SIC = OINTERPOL_SIC
LINTERPOL_SIC = OINTERPOL_SIT
LSURF_BUDGETC = OSURF_BUDGETC
N2M = K2M
IF (LHOOK) CALL DR_HOOK('FLAG_DIAG_UPDATE:FLAG_SEA_UPDATE',1,ZHOOK_HANDLE)
!
END SUBROUTINE FLAG_SEA_UPDATE
!
SUBROUTINE FLAG_WATER_UPDATE(KBEQ, OINTERPOL_TS, OSURF_BUDGETC, K2M)
!
USE MODD_CH_WATFLUX_n, ONLY : NBEQ
USE MODD_WATFLUX_n,ONLY : LINTERPOL_TS
USE MODD_DIAG_WATFLUX_n, ONLY : LSURF_BUDGETC, N2M
!
IMPLICIT NONE
!
INTEGER, INTENT(IN) :: KBEQ
...
!
USE MODD_CH_FLAKE_n, ONLY : NBEQ
USE MODD_DIAG_MISC_FLAKE_n,ONLY : LWATER_PROFILE
USE MODD_DIAG_FLAKE_n, ONLY : LSURF_BUDGETC, N2M
!
IMPLICIT NONE
!
INTEGER, INTENT(IN) :: KBEQ
...
USE MODD_DIAG_ISBA_n, ONLY : LPGD, LPATCH_BUDGET, N2M
...
USE MODD_DIAG_TEB_n, ONLY : LPGD, N2M
