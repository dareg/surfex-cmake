...
USE MODD_SURFEX_MPI, ONLY : NINDEX, NPROC, NRANK, NCOMM, NPIO, NSIZE, &
                            XTIME_CALC_WRITE, XTIME_COMM_WRITE, &
                            XTIME_OMP_BARR, IDX_W, WLOG_MPI
USE MODD_SURFEX_OMP, ONLY : NINDX1, NINDX2, NWORK2, NWORK2_FULL
!
USE MODI_UNPACK_SAME_RANK
!
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK
...
IF (SIZE(KWORK,1)>0) THEN
  IF (PRESENT(KMASK)) THEN
    CALL UNPACK_SAME_RANK(KMASK,KWORK,NWORK2(NINDX1:NINDX2,1:IP2))
  ELSE
    NWORK2(NINDX1:NINDX2,1:IP2) = KWORK(:,:)
  ENDIF
ENDIF
!
#ifndef NOMPI
