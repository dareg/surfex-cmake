...
!
SUBROUTINE GATHER_AND_WRITE_MPI_X1D0(PWORK,PWORK2,KMASK,PMISS)
!
...
!
SUBROUTINE GATHER_AND_WRITE_MPI_X2D0(PWORK,PWORK2,KMASK,PMISS)
!
REAL, DIMENSION(:,:), INTENT(IN) :: PWORK
REAL, DIMENSION(:,:), INTENT(OUT) :: PWORK2
!
...
!
SUBROUTINE GATHER_AND_WRITE_MPI_X3D0(PWORK,PWORK2,KMASK,PMISS)
!
REAL, DIMENSION(:,:,:), INTENT(IN) :: PWORK
REAL, DIMENSION(:,:,:), INTENT(OUT) :: PWORK2
!
...
IF (SIZE(PWORK)>0) THEN
  IF (PRESENT(KMASK)) THEN
    IF (PRESENT(PMISS)) THEN
      CALL UNPACK_SAME_RANK(KMASK,PWORK,ZWORK(:),PMISS)
    ELSE
...
ELSE
  ...
  DO I=0,NPROC-1  
    ...
    IF (I/=NPIO) THEN
#ifdef SFX_MPI
      IF (LSFX_MPI)  CALL MPI_RECV(ZINTER,SIZE(ZINTER)*KIND(ZINTER)/4,MPI_REAL,I,IDX_W,NCOMM,ISTATUS,INFOMPI)
#endif
    ELSE
      ZINTER(:) = ZWORK(:)
    ENDIF
...
USE MODD_SURFEX_MPI, ONLY : NINDEX, NPROC, NRANK, NCOMM, NPIO, NSIZE, &
                            XTIME_COMM_WRITE,  &
                            XTIME_CALC_WRITE, IDX_W, WLOG_MPI, LSFX_MPI
!
USE MODI_UNPACK_SAME_RANK
!
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK, JPHOOK
