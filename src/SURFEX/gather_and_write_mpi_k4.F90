...
USE MODD_SURFEX_MPI, ONLY : NINDEX, NPROC, NRANK, NCOMM, NPIO, NSIZE, &
                            XTIME_COMM_WRITE, XTIME_OMP_BARR,  &
                            XTIME_CALC_WRITE, IDX_W, WLOG_MPI
USE MODD_SURFEX_OMP, ONLY : NINDX1SFX, NINDX2SFX, XWORK3, XWORK3_FULL, NBLOCK
!
USE MODI_UNPACK_SAME_RANK
!
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK
...
INTEGER, DIMENSION(MPI_STATUS_SIZE) :: ISTATUS
#endif
INTEGER :: ICPT, IX2, IX3, IS1, IS2, IS3, IP1, IP2, IP3
INTEGER :: I,J
INTEGER :: INFOMPI
!
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!
!
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X3D0',0,ZHOOK_HANDLE)
!
IP2 = SIZE(PWORK,2)
IX2 = SIZE(XWORK3,2)
IP3 = SIZE(PWORK,3)
...
!$OMP SINGLE
!
IF ((IP2>IX2).OR.(IP3>IX3)) THEN
  DEALLOCATE(XWORK3)
  ALLOCATE(XWORK3(NSIZE,IP2,IP3))
ENDIF
!
!$OMP END SINGLE
!
XWORK3(NINDX1SFX:NINDX2SFX,1:IP2,1:IP3) = XUNDEF
!
#ifdef SFX_MPI
XTIME0 = MPI_WTIME()
#endif
...
IF (SIZE(PWORK,1)>0) THEN
  IF (PRESENT(KMASK)) THEN
    CALL UNPACK_SAME_RANK(KMASK,PWORK,XWORK3(NINDX1SFX:NINDX2SFX,1:IP2,1:IP3))
  ELSE
    XWORK3(NINDX1SFX:NINDX2SFX,1:IP2,1:IP3) = PWORK(:,:,:)
  ENDIF
ENDIF
!
#ifdef SFX_MPI
...
#ifdef SFX_MPI
  XTIME0 = MPI_WTIME()
  CALL MPI_SEND(XWORK3(:,1:IP2,1:IP3),NSIZE*IP2*IP3*KIND(XWORK3)/4,MPI_REAL,NPIO,IDX_W,NCOMM,INFOMPI)
  XTIME_COMM_WRITE = XTIME_COMM_WRITE + (MPI_WTIME() - XTIME0)
#endif
  !
!$OMP END SINGLE
...
!$OMP SINGLE
  !
  IS1 = SIZE(XWORK3_FULL,1)
  IS2 = SIZE(XWORK3_FULL,2)
  IS3 = SIZE(XWORK3_FULL,3)
!
  IF (IP1>IS1 .OR. IP2>IS2 .OR. IP3>IS3) THEN
    IF (ASSOCIATED(XWORK3_FULL)) DEALLOCATE(XWORK3_FULL)
    ALLOCATE(XWORK3_FULL(MAX(IP1,IS1),MAX(IP2,IS2),MAX(IP3,IS3)))
  ENDIF
  !
  XWORK3_FULL(1:IP1,1:IP2,1:IP3) = 0.
  !
  IDX_W = IDX_W + 1
  !
  DO I=1,NPROC
...
!    !
    ...
    DO J=1,SIZE(NINDEX)
      ...
      IF ( NINDEX(J)==MOD(I,NPROC) ) THEN
        ICPT = ICPT + 1
        XWORK3_FULL(J,1:IP2,1:IP3) = ZINTER(ICPT,:,:)
      ENDIF
      !
    ENDDO
     !
...
!$OMP END SINGLE
  !
  PWORK2(:,:,:) = XWORK3_FULL(1:IP1,1:IP2,1:IP3)
  !
ENDIF
!
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X3D0',1,ZHOOK_HANDLE)
!
!
END SUBROUTINE GATHER_AND_WRITE_MPI_X3D0
!
