...
IF (SIZE(PWORK)>0) THEN
  IF (PRESENT(KMASK)) THEN
    CALL UNPACK_SAME_RANK(KMASK,PWORK,ZWORK(NINDX1SFX:NINDX2SFX))
  ELSE
    ZWORK(NINDX1SFX:NINDX2SFX) = PWORK(:)
  ENDIF
ENDIF
!
#ifdef SFX_MPI
...
ELSE
  ...
  IDX_W = IDX_W + 1
  !
!$OMP PARALLEL DO PRIVATE(I,ZINTER,J,ICPT,ISTATUS,INFOMPI,ZHOOK_HANDLE_OMP)
  DO I=0,NPROC-1  
    !
#ifdef SFX_MPI
    XTIME0 = MPI_WTIME()
...
USE MODD_SURFEX_OMP, ONLY : NINDX1SFX, NINDX2SFX, XWORK2
!
USE MODI_UNPACK_SAME_RANK
!
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK
...
!
ZWORK(NINDX1SFX:NINDX2SFX,:) = XUNDEF
!
#ifdef SFX_MPI
XTIME0 = MPI_WTIME()
#endif
...
!
!
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_1',0,ZHOOK_HANDLE)
!
ZWORK(:,:,:) = XUNDEF
!
#ifndef NOMPI
...
#endif
!
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_1',1,ZHOOK_HANDLE)
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_2',0,ZHOOK_HANDLE)
...
#endif
  !
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_2',1,ZHOOK_HANDLE)
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_5',0,ZHOOK_HANDLE)
  !
ELSEIF (NPROC>1) THEN
  !
  PWORK2(:,:,:) = 0.
...
!$OMP PARALLEL DO PRIVATE(I,ZINTER,ICPT,J,ISTATUS,INFOMPI,ZHOOK_HANDLE_OMP)
  DO I=0,NPROC-1
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_3',0,ZHOOK_HANDLE_OMP)
    !
#ifndef NOMPI   
    XTIME0 = MPI_WTIME()
#endif    
...
#endif     
    !    
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_3',1,ZHOOK_HANDLE_OMP)
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_4',0,ZHOOK_HANDLE_OMP)
...
#endif      
     ! 
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_4',1,ZHOOK_HANDLE_OMP)
  ENDDO
!$OMP END PARALLEL DO
  !
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_5',0,ZHOOK_HANDLE)
  !
ELSE
  PWORK2(:,:,:) = ZWORK(NINDX1SFX:NINDX2SFX,:,:)
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_2',1,ZHOOK_HANDLE)
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_5',0,ZHOOK_HANDLE)

ENDIF
!
IF (LHOOK) CALL DR_HOOK('GATHER_AND_WRITE_MPI_X2D0_5',1,ZHOOK_HANDLE)
!
!
END SUBROUTINE GATHER_AND_WRITE_MPI_X3D0
!
