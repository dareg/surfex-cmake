...
!     ##############################################################
!
!!**** *GET_ADJ_MES_CART* get the near grid mesh indices
!!
!!    PURPOSE
!!    -------
!!
...
IF (LHOOK) CALL DR_HOOK('GET_ADJ_MES_CART',0,ZHOOK_HANDLE)
 CALL GET_GRIDTYPE_CARTESIAN(PGRID_PAR,KIMAX=IIMAX,KJMAX=IJMAX,PX=ZX,PY=ZY)
!
KLEFT  (:) = 0
KRIGHT (:) = 0
KTOP   (:) = 0
...
IF (IIMAX*IJMAX==KL) THEN
  ...
  END DO
END IF
IF (LHOOK) CALL DR_HOOK('GET_ADJ_MES_CART',1,ZHOOK_HANDLE)
!
!-------------------------------------------------------------------------------
!
