...
!     
  CASE("CONF PROJ ","LONLAT REG","GAUSS     ","IGN      ","LONLATVAL ","LONLAT ROT")
    ...
    IF (UG%CGRID=="CONF PROJ ") THEN
      CALL GET_MESH_INDEX_CONF_PROJ(UG%NGRID_PAR,ISSO,UG%XGRID_FULL_PAR,PLAT,PLON,KINDEX,IISSOX,IISSOY)  
    ENDIF
    IF (UG%CGRID=="LONLAT REG") THEN
      IF (PRESENT(PVALUE) .AND. PRESENT(PNODATA)) THEN
        CALL GET_MESH_INDEX_LONLAT_REG(UG%NGRID_PAR,ISSO,UG%XGRID_FULL_PAR,PLAT,PLON,KINDEX,IISSOX,IISSOY, &
                                     PVALUE,PNODATA)
      ELSE            
        CALL GET_MESH_INDEX_LONLAT_REG(UG%NGRID_PAR,ISSO,UG%XGRID_FULL_PAR,PLAT,PLON,KINDEX,IISSOX,IISSOY)  
      ENDIF
    ...
    IF (UG%CGRID=="GAUSS     ") THEN
      IF (PRESENT(PVALUE) .AND. PRESENT(PNODATA)) THEN
        CALL GET_MESH_INDEX_GAUSS(KNBLINES,UG%NGRID_PAR,ISSO,UG%XGRID_FULL_PAR,PLAT,PLON,KINDEX,IISSOX,IISSOY, &
                                     PVALUE,PNODATA)
      ELSE
        CALL GET_MESH_INDEX_GAUSS(KNBLINES,UG%NGRID_PAR,ISSO,UG%XGRID_FULL_PAR,PLAT,PLON,KINDEX,IISSOX,IISSOY)
      ENDIF              
    ...
    IF (UG%CGRID=="IGN       ") THEN
      IF (PRESENT(PVALUE) .AND. PRESENT(PNODATA)) THEN
        CALL GET_MESH_INDEX_IGN(UG%NGRID_PAR,ISSO,UG%XGRID_FULL_PAR,PLAT,PLON,KINDEX,IISSOX,IISSOY, &
                                     PVALUE,PNODATA)
      ELSE       
        CALL GET_MESH_INDEX_IGN(UG%NGRID_PAR,ISSO,UG%XGRID_FULL_PAR,PLAT,PLON,KINDEX,IISSOX,IISSOY)
      ENDIF  
    ENDIF
    IF (UG%CGRID=="LONLATVAL ") &
      CALL GET_MESH_INDEX_LONLATVAL(UG%NGRID_PAR,ISSO,UG%XGRID_FULL_PAR,PLAT,PLON,KINDEX,IISSOX,IISSOY)  
    IF (UG%CGRID=="LONLAT ROT") THEN
      CALL GET_MESH_INDEX_LONLAT_ROT(UG%NGRID_PAR,SIZE(PLAT),UG%XGRID_FULL_PAR,PLAT,PLON,KINDEX,ISSO,IISSOX,IISSOY)  
    ENDIF
    !
    IF (PRESENT(KSSO) .AND. PRESENT(KISSOX) .AND. PRESENT(KISSOY)) THEN
      KISSOX = IISSOX
