...
!$OMP SINGLE
  IF (NIX==0) THEN
    ...
    CALL READ_SURF(CSURF_FILETYPE,'GRID_TYPE',UG%CGRID,IRESP,HDIR='A')
    CALL READ_GRIDTYPE(CSURF_FILETYPE,UG%CGRID,UG%NGRID_PAR,U%NDIM_FULL,.FALSE.,HDIR="A")
    ALLOCATE(UG%XGRID_FULL_PAR(UG%NGRID_PAR))
    CALL READ_GRIDTYPE(CSURF_FILETYPE,UG%CGRID,UG%NGRID_PAR,U%NDIM_FULL,.TRUE.,UG%XGRID_FULL_PAR,IRESP,HDIR="A")  
