...
!$OMP SINGLE
  IF (NIX==0) THEN
    ...
    CALL READ_SURF(CSURF_FILETYPE,'GRID_TYPE',CGRID,IRESP,HDIR='A')
    CALL READ_GRIDTYPE(CSURF_FILETYPE,CGRID,NGRID_PAR,NDIM_FULL,.FALSE.,HDIR="A")
    ALLOCATE(XGRID_FULL_PAR(NGRID_PAR))
    CALL READ_GRIDTYPE(CSURF_FILETYPE,CGRID,NGRID_PAR,NDIM_FULL,.TRUE.,XGRID_FULL_PAR,IRESP,HDIR="A")  
    CALL END_IO_SURF_n(CSURF_FILETYPE)
    CALL GET_GRID_DIM(CGRID,SIZE(XGRID_FULL_PAR),XGRID_FULL_PAR,GRECT,NIX,NIY)  
    XGRID_FULL_PAR=>NULL()
  ENDIF
!$OMP END SINGLE  
