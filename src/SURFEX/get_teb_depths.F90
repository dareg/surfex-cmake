!     #########
      SUBROUTINE GET_TEB_DEPTHS(HFILEPGD, HFILEPGDTYPE, PD_ROOF, PD_ROAD, PD_WALL, PD_FLOOR)
!     ##############################################################
!
!!**** *CONVERT_COVER* 
!!
...
!* depths are deduced from the cover types
  ...
  DO JLAYER=1,IDATA_LAYER
    CALL AV_PGD (ZPAR_D(:,JLAYER), ZCOVER, ZDATA(:,JLAYER),YAREA,'ARI',GCOVER)
  END DO
  DEALLOCATE(ZCOVER)
  CALL CLOSE_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE)
ENDIF
