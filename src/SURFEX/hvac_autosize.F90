!     #############################################################
SUBROUTINE HVAC_AUTOSIZE (B, BOP, TG, T, TOP, KI,KLUOUT)
!     #############################################################
!!
!!    PURPOSE
!!    -------
...
USE MODD_DIAG_CUMUL_TEB_n, ONLY : DIAG_CUMUL_TEB_1P_t, DIAG_CUMUL_TEB_1P_INIT
...
TYPE(DIAG_CUMUL_TEB_1P_t) :: YDGCT
TYPE(DIAG_MISC_TEB_1P_t) :: YDGMT
TYPE(DIAG_MISC_TEB_OPTIONS_t) :: YDGMTO
!
! Options TOP
!
LOGICAL :: GCANOPY
...
T%XWS_ROAD = 0.0
!
ALLOCATE(T%TSNOW_ROOF%WSNOW(KI,1,1))
ALLOCATE(T%TSNOW_ROOF%T(KI,1,1))
ALLOCATE(T%TSNOW_ROOF%ALB(KI,1))
ALLOCATE(T%TSNOW_ROOF%RHO(KI,1,1))
ALLOCATE(T%TSNOW_ROOF%TS(KI,1))
ALLOCATE(T%TSNOW_ROOF%EMIS(KI,1))
!
...
!
ALLOCATE(T%TSNOW_ROAD%WSNOW(KI,1,1))
ALLOCATE(T%TSNOW_ROAD%T(KI,1,1))
ALLOCATE(T%TSNOW_ROAD%ALB(KI,1))
ALLOCATE(T%TSNOW_ROAD%RHO(KI,1,1))
ALLOCATE(T%TSNOW_ROAD%TS(KI,1))
ALLOCATE(T%TSNOW_ROAD%EMIS(KI,1))
!
...
!----------------------------------------------
!
 CALL DIAG_MISC_TEB_1P_INIT(YDGMT)
!
ZUW_GREENROOF    (:) = 0.
!
ZAC_GARDEN    (:) = 0.
...
ZE_SHADING(:) = 0.
!
 CALL DIAG_MISC_TEB_OPTIONS_INIT(YDGMTO)
 CALL DIAG_CUMUL_TEB_1P_INIT(YDGCT)
!
 YDGMTO%LSURF_MISC_BUDGET = .TRUE.
 CALL DIAG_MISC_TEB_INIT_n(YDGCT, YDGMT, YDGMTO, TOP, KI, 0)
!
! greenroofs   are not taken into account in the building's HVAC equipment sizing process
YDGMT%XRN_GREENROOF    (:) = 0.
YDGMT%XH_GREENROOF     (:) = 0.
YDGMT%XLE_GREENROOF    (:) = 0.
YDGMT%XGFLUX_GREENROOF (:) = 0.
YDGMT%XRUNOFF_GREENROOF(:) = 0.
YDGMT%XDRAIN_GREENROOF (:) = 0.
!* one supposes zero conduction heat flux between the greenroof and the nnroof.
YDGMT%XG_GREENROOF_ROOF(:) = 0.
!
YDGMT%XQIN(:) = B%XQIN(:)
!
YDGMT%XT_SYS(:) = B%XTI_BLD(:)
YDGMT%XQ_SYS(:) = B%XQI_BLD(:)
!
YDGMT%XTHEAT_TARGET(:) = B%XTHEAT_TARGET(:)
YDGMT%XTCOOL_TARGET(:) = B%XTCOOL_TARGET(:)
!
...
!               ---------------
!
  CALL SUNPOS(JPYEAR, IMONTH, IDAY, ZTIME, TG%XLON, TG%XLAT, ZTSUN, ZZENITH, ZAZIM)
!
...
! solar panels are not taken into account in the building's HVAC equipment sizing process
  CALL URBAN_SOLAR_ABS(TOP, T, B, YDGMT, ZDIR_SW, ZSCA_SW, ZZENITH, ZAZIM,&
...
!               -------------
!
  CALL TEB  (TOP, T, BOP, B, YIR, YDGMT, YIMPLICIT_WIND, ZTSUN,        &
...
!
B%XQIN(:) = YDGMT%XQIN(:)
B%XTHEAT_TARGET(:) = YDGMT%XTHEAT_TARGET(:)
B%XTCOOL_TARGET(:) = YDGMT%XTCOOL_TARGET(:)
!
