...
! 	 
    IF (PRESENT(PVEGGV)) THEN
      PVEGGV (JLOOP,:,:) = 0.
      IF(PTYPE(JLOOP,NVT_TREE)>0. )  PVEGGV (JLOOP,:,NVT_TREE) = 0.95
      IF(PTYPE(JLOOP,NVT_CONI)>0. )  PVEGGV (JLOOP,:,NVT_CONI) = 0.95
      IF(PTYPE(JLOOP,NVT_EVER)>0. )  PVEGGV (JLOOP,:,NVT_EVER) = 0.99
...
!
    IF (PRESENT(PZF_TALLVEG)) THEN
      PZF_TALLVEG (JLOOP,:) = 0.
      IF(PTYPE(JLOOP,NVT_TREE)>0. )  PZF_TALLVEG (JLOOP,NVT_TREE) = 1.0
      IF(PTYPE(JLOOP,NVT_CONI)>0. )  PZF_TALLVEG (JLOOP,NVT_CONI) = 1.0
      IF(PTYPE(JLOOP,NVT_EVER)>0. )  PZF_TALLVEG (JLOOP,NVT_EVER) = 1.0
    ENDIF   
!
!-------------------------------------------------------------------------------
!
...
!            -------------------
      ZLAIFRGV (JLOOP,:) = 0.
      IF(PTYPE(JLOOP,NVT_TREE)>0. )  ZLAIFRGV (JLOOP,NVT_TREE) = 0.1
      IF(PTYPE(JLOOP,NVT_CONI)>0. )  ZLAIFRGV (JLOOP,NVT_CONI) = 0.1
      IF(PTYPE(JLOOP,NVT_EVER)>0. )  ZLAIFRGV (JLOOP,NVT_EVER) = 0.1
