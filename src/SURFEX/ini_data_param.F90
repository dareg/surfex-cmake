...
!            ------
    IF (PRESENT(PALBNIR_VEG)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PALBNIR_VEG(JLOOP,NVT_FLTR) = 0.25
      ENDIF
    ENDIF
!-------------------------------------------------------------------------------
!*    7.6    albvis (veg only)
!            ------
    IF (PRESENT(PALBVIS_VEG)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PALBVIS_VEG(JLOOP,NVT_FLTR) = 0.05
      ENDIF
    ENDIF        
!-------------------------------------------------------------------------------
!*    7.6    albUV (veg only)
!            -----
    IF (PRESENT(PALBUV_VEG)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PALBUV_VEG(JLOOP,NVT_FLTR) = 0.0525
      ENDIF
      !
      ...
      IF(GAGRI_TO_GRASS)THEN
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PALBUV_VEG(JLOOP,NVT_C3W )= 0.08
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PALBUV_VEG(JLOOP,NVT_C3S )= 0.08
        ENDIF
      ...
      ELSEIF (NVT_FLGR/=0) THEN
        IF(PTYPE(JLOOP,NVT_FLGR)>0. )  PALBUV_VEG(JLOOP,NVT_FLGR)= 0.08
      ENDIF
      !
    ENDIF        
!------------------------------------------------------------------------------
...
!            -----
    IF (PRESENT(PRSMIN)) THEN
      ...
      !
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PRSMIN(JLOOP,NVT_FLTR) = 150.
      ENDIF
      !
...
!            -----
    IF (PRESENT(PGAMMA)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PGAMMA(JLOOP,NVT_FLTR) = 0.04
      ENDIF
    ENDIF
!-------------------------------------------------------------------------------
!*    7.8    Wrmax_cf
!            --------
    IF (PRESENT(PWRMAX_CF)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PWRMAX_CF(JLOOP,NVT_FLTR) = 0.1
      ENDIF
    ENDIF
!-------------------------------------------------------------------------------
!*    7.9    Rgl
!            ---
    IF (PRESENT(PRGL)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PRGL(JLOOP,NVT_FLTR) = 30.
      ENDIF
    ENDIF    
!-------------------------------------------------------------------------------
!*    7.10   Cv
!            --
    IF (PRESENT(PCV)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PCV(JLOOP,NVT_FLTR) = 1.E-5
      ENDIF
    ENDIF    
!-------------------------------------------------------------------------------
...
!            --------------------------------------------------------------------
    IF (PRESENT(PGMES)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PGMES(JLOOP,NVT_FLTR) = 0.001
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PGMES(JLOOP,NVT_C3W )= 0.020
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PGMES(JLOOP,NVT_C3S )= 0.020
        ENDIF
      ...
      ELSE
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PGMES(JLOOP,NVT_C3W )= 0.003
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PGMES(JLOOP,NVT_C3S )= 0.003
        ENDIF
...
!
    IF (PRESENT(PGMES_ST)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PGMES_ST(JLOOP,NVT_FLTR) = 0.0018
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PGMES_ST(JLOOP,NVT_C3W )= 0.001
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PGMES_ST(JLOOP,NVT_C3S )= 0.001
        ENDIF
      ...
      ELSE
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PGMES_ST(JLOOP,NVT_C3W )= 0.00175
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PGMES_ST(JLOOP,NVT_C3S )= 0.00175
        ENDIF
      ...
      ELSEIF (NVT_FLGR/=0) THEN
        IF(PTYPE(JLOOP,NVT_FLGR)>0. )  PGMES_ST(JLOOP,NVT_FLGR)= 0.001
      ENDIF
      !
    ENDIF    
!-------------------------------------------------------------------------------
...
!            -----------------------------------
    IF (PRESENT(PRE25)) THEN
      ...
      IF(GAGRI_TO_GRASS)THEN
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PRE25(JLOOP,NVT_C3W )= 3.6E-7
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PRE25(JLOOP,NVT_C3S )= 3.6E-7
        ENDIF
...
!            -----------------------------
    IF (PRESENT(PGC)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PGC(JLOOP,NVT_FLTR) = 0.00015
      ENDIF  
    ENDIF
!
    IF (PRESENT(PGC_ST)) THEN
      ...
      ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
        IF(PTYPE(JLOOP,NVT_C3W )>0. )  PGC_ST(JLOOP,NVT_C3W )= 0.00025
        IF(PTYPE(JLOOP,NVT_C3S )>0. )  PGC_ST(JLOOP,NVT_C3S )= 0.00025
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
      ...
      ELSEIF (NVT_FLGR/=0) THEN
        IF(PTYPE(JLOOP,NVT_FLGR)>0. )  PGC_ST(JLOOP,NVT_FLGR)= 0.001
      ENDIF 
      !
    ENDIF    
!-------------------------------------------------------------------------------
...
!            -----------------------
    IF (PRESENT(PBSLAI)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PBSLAI(JLOOP,NVT_FLTR) = 0.25
      ENDIF 
      !
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PBSLAI(JLOOP,NVT_C3W )= 0.36
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PBSLAI(JLOOP,NVT_C3S )= 0.36
        ENDIF
      ...
      ELSE
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PBSLAI(JLOOP,NVT_C3W )= 0.06
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PBSLAI(JLOOP,NVT_C3S )= 0.06
        ENDIF
...
!    
    IF (PRESENT(PBSLAI_ST)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PBSLAI_ST(JLOOP,NVT_FLTR) = 0.125
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PBSLAI_ST(JLOOP,NVT_C3W )= 0.08
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PBSLAI_ST(JLOOP,NVT_C3S )= 0.08
        ENDIF
      ...
      ELSE
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PBSLAI_ST(JLOOP,NVT_C3W )= 0.06
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PBSLAI_ST(JLOOP,NVT_C3S )= 0.06
        ENDIF
...
!            -----------------------------------------------------
    IF (PRESENT(PDMAX)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PDMAX(JLOOP,NVT_FLTR) = 0.01
      ENDIF
    ENDIF    
!
    IF (PRESENT(PDMAX_ST)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PDMAX_ST(JLOOP,NVT_FLTR) = 0.109
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
        !
...
!            ----------------------------
    IF (PRESENT(PSTRESS)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PSTRESS(JLOOP,NVT_FLTR) = 0.
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PSTRESS(JLOOP,NVT_C3W )= 0.
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PSTRESS(JLOOP,NVT_C3S )= 0.
        ENDIF
        IF (NVT_IRR/=0) THEN
      ...
      ELSEIF (NVT_FLGR/=0) THEN
        IF(PTYPE(JLOOP,NVT_FLGR)>0. )  PSTRESS(JLOOP,NVT_FLGR)= 0.
      ENDIF 
      !
    ENDIF    
!-------------------------------------------------------------------------------
...
! parameters use in case HPHOTO == 'NONE'
    IF (PRESENT(PSEFOLD)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PSEFOLD(JLOOP,NVT_FLTR) = 365.* XDAY
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PSEFOLD(JLOOP,NVT_C3W )= 90.* XDAY
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PSEFOLD(JLOOP,NVT_C3S )= 90.* XDAY
        ENDIF
        !
      ...
      ELSE
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PSEFOLD(JLOOP,NVT_C3W )= 60.* XDAY
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PSEFOLD(JLOOP,NVT_C3S )= 60.* XDAY
        ENDIF
        !
...
! parameters use in case HPHOTO == 'AST', 'NIT', 'NCB'
    ...
    IF (PRESENT(PSEFOLD_ST)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PSEFOLD_ST(JLOOP,NVT_FLTR) = 365.* XDAY
      ENDIF
    ENDIF    
!-------------------------------------------------------------------------------
...
!
    IF (PRESENT(PCE_NITRO)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PCE_NITRO(JLOOP,NVT_FLTR) = 4.83
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PCE_NITRO(JLOOP,NVT_C3W )= 5.56
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PCE_NITRO(JLOOP,NVT_C3S )= 5.56
        ENDIF
      ...
      ELSE
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PCE_NITRO(JLOOP,NVT_C3W )= 3.79
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PCE_NITRO(JLOOP,NVT_C3S )= 3.79
        ENDIF
      ...
      ELSEIF (NVT_FLGR/=0) THEN
        IF(PTYPE(JLOOP,NVT_FLGR)>0. )  PCE_NITRO(JLOOP,NVT_FLGR)= 5.56
      ENDIF   
      ! 
    ENDIF
!
...
!            ------------------------------------------------------
    IF (PRESENT(PCF_NITRO)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PCF_NITRO(JLOOP,NVT_FLTR) = 5.11
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PCF_NITRO(JLOOP,NVT_C3W )= 6.73
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PCF_NITRO(JLOOP,NVT_C3S )= 6.73
        ENDIF
      ...
      ELSE
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PCF_NITRO(JLOOP,NVT_C3W )= 9.84
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PCF_NITRO(JLOOP,NVT_C3S )= 9.84
        ENDIF
      ...
      ELSEIF (NVT_FLGR/=0) THEN
        IF(PTYPE(JLOOP,NVT_FLGR)>0. )  PCF_NITRO(JLOOP,NVT_FLGR)= 6.73
      ENDIF  
      ! 
    ENDIF    
!-------------------------------------------------------------------------------
...
!            --------------------------------------------------
    IF (PRESENT(PCNA_NITRO)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PCNA_NITRO(JLOOP,NVT_FLTR) = 2.13
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
      ...
      ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
        IF(PTYPE(JLOOP,NVT_C3W )>0. )  PCNA_NITRO(JLOOP,NVT_C3W )= 1.3
        IF(PTYPE(JLOOP,NVT_C3S )>0. )  PCNA_NITRO(JLOOP,NVT_C3S )= 1.3
      ENDIF
      !
      ...
      ELSEIF (NVT_FLGR/=0) THEN
        IF(PTYPE(JLOOP,NVT_FLGR)>0. )  PCNA_NITRO(JLOOP,NVT_FLGR)= 1.3
      ENDIF  
      !  
    ENDIF    
!-------------------------------------------------------------------------------
...
!            -------------------------------------------------------
    IF (PRESENT(PROOT_EXTINCTION)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PROOT_EXTINCTION(JLOOP,NVT_FLTR) = 0.966
      ENDIF
      !
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PROOT_EXTINCTION(JLOOP,NVT_C3W )= 0.943
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PROOT_EXTINCTION(JLOOP,NVT_C3S )= 0.943
        ENDIF
      ...
      ELSE
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PROOT_EXTINCTION(JLOOP,NVT_C3W )= 0.961
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PROOT_EXTINCTION(JLOOP,NVT_C3S )= 0.961
        ENDIF
...
!            -------------------------------------------
    IF (PRESENT(PSOILRC_SO2)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PSOILRC_SO2(JLOOP,NVT_FLTR) = 500.
      ENDIF  
      ! 
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PSOILRC_SO2(JLOOP,NVT_C3W )= 350.
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PSOILRC_SO2(JLOOP,NVT_C3S )= 350.
        ENDIF
      ...
      ELSE
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PSOILRC_SO2(JLOOP,NVT_C3W )= 150.
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PSOILRC_SO2(JLOOP,NVT_C3S )= 150.
        ENDIF
      ...
      ELSEIF (NVT_FLGR/=0) THEN
        IF(PTYPE(JLOOP,NVT_FLGR)>0. )  PSOILRC_SO2(JLOOP,NVT_FLGR)= 350.
      ENDIF  
      !
...
!            ------------------------------------------
    IF (PRESENT(PSOILRC_O3)) THEN
      ...
      IF (NVT_FLTR>0) THEN
        IF (PTYPE(JLOOP,NVT_FLTR)>0. ) PSOILRC_O3(JLOOP,NVT_FLTR) = 200.
      ENDIF 
      !
      IF(GAGRI_TO_GRASS)THEN
        !
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PSOILRC_O3(JLOOP,NVT_C3W )= 200.
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PSOILRC_O3(JLOOP,NVT_C3S )= 200.
        ENDIF
      ...
      ELSE
        ...
        ELSEIF (NVT_C3W/=0 .AND. NVT_C3S/=0) THEN
          IF(PTYPE(JLOOP,NVT_C3W )>0. )  PSOILRC_O3(JLOOP,NVT_C3W )= 150.
          IF(PTYPE(JLOOP,NVT_C3S )>0. )  PSOILRC_O3(JLOOP,NVT_C3S )= 150.
        ENDIF
      ...
      ELSEIF (NVT_FLGR/=0) THEN
        IF(PTYPE(JLOOP,NVT_FLGR)>0. )  PSOILRC_O3(JLOOP,NVT_FLGR)= 200.
      ENDIF   
      !
