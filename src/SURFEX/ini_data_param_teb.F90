...
!
!
ALLOCATE(ILIST(MAXVAL(NDESC_CODE_LIST)))
ILIST(:) = 0
DO JLIST=1,NDESC_CODE
  ILIST(NDESC_CODE_LIST(JLIST)) = JLIST
END DO
!
IINDEX = 0
!
...
!
    ...
    IF (PRESENT(PALB_ROAD)) THEN
      PALB_ROAD(JLOOP)=XDESC_ALB_ROAD(IINDEX)
    ENDIF
    IF (PRESENT(PEMIS_ROAD)) THEN
      PEMIS_ROAD(JLOOP)=XDESC_EMIS_ROAD(IINDEX)
    ENDIF
    IF (PRESENT(PHC_ROAD)) THEN
      PHC_ROAD(JLOOP,:)= XDESC_HC_ROAD(IINDEX,:)
    ENDIF
    IF (PRESENT(PTC_ROAD)) THEN
      PTC_ROAD(JLOOP,:)= XDESC_TC_ROAD(IINDEX,:)
    ENDIF
    IF (PRESENT(PD_ROAD)) THEN
      PD_ROAD(JLOOP,:)= XDESC_D_ROAD(IINDEX,:)
    ENDIF
!
!-------------------------------------------------------------------------------
!
...
!
    IF (PRESENT(PALB_ROOF)) THEN
      PALB_ROOF(JLOOP)=XDESC_ALB_ROOF(IINDEX)
    ENDIF
    IF (PRESENT(PALB_WALL)) THEN
      PALB_WALL(JLOOP)=XDESC_ALB_WALL(IINDEX)
    ENDIF

    IF (PRESENT(PEMIS_ROOF)) THEN
      PEMIS_ROOF(JLOOP)=XDESC_EMIS_ROOF(IINDEX)
    ENDIF
    IF (PRESENT(PEMIS_WALL)) THEN
      PEMIS_WALL(JLOOP)=XDESC_EMIS_WALL(IINDEX)
    ENDIF

    IF (PRESENT(PHC_ROOF)) THEN
      PHC_ROOF(JLOOP,:)= XDESC_HC_ROOF(IINDEX,:)
    ENDIF
    IF (PRESENT(PHC_WALL)) THEN
      PHC_WALL(JLOOP,:)= XDESC_HC_WALL(IINDEX,:)
    ENDIF
 
    IF (PRESENT(PTC_ROOF)) THEN
      PTC_ROOF(JLOOP,:)= XDESC_TC_ROOF(IINDEX,:)
    ENDIF
    IF (PRESENT(PTC_WALL)) THEN
      PTC_WALL(JLOOP,:)= XDESC_TC_WALL(IINDEX,:)
    ENDIF

    IF (PRESENT(PD_ROOF)) THEN
      PD_ROOF(JLOOP,:)= XDESC_D_ROOF(IINDEX,:)
    ENDIF
    IF (PRESENT(PD_WALL)) THEN
      PD_WALL(JLOOP,:)= XDESC_D_WALL(IINDEX,:)
    ENDIF


    IF (PRESENT(PHC_FLOOR)) THEN
      PHC_FLOOR(JLOOP,:)= XDESC_HC_FLOOR(IINDEX,:)
    ENDIF

    IF (PRESENT(PTC_FLOOR)) THEN
      PTC_FLOOR(JLOOP,:)= XDESC_TC_FLOOR(IINDEX,:)
    ENDIF

    IF (PRESENT(PD_FLOOR)) THEN
      PD_FLOOR(JLOOP,:)= XDESC_D_FLOOR(IINDEX,:)
    ENDIF
!
    IF (PRESENT(PEFF_HEAT)) THEN
      PEFF_HEAT(JLOOP) = XDESC_EFF_HEAT(IINDEX)
    ENDIF
!
    IF (PRESENT(PSHGC)) THEN
      PSHGC(JLOOP) = XDESC_SHGC(IINDEX)
    ENDIF
!
    IF (PRESENT(PU_WIN)) THEN
      PU_WIN(JLOOP) = XDESC_U_WIN(IINDEX)
    ENDIF
!
    IF (PRESENT(PGR)) THEN
      PGR(JLOOP) = XDESC_GR(IINDEX)
    ENDIF

    IF (PRESENT(PCOP_RAT)) THEN
      PCOP_RAT(JLOOP) = XDESC_COP_RAT(IINDEX)
    ENDIF
!
    IF (PRESENT(PF_WATER_COND)) THEN
      PF_WATER_COND(JLOOP) = XDESC_F_WATER_COND(IINDEX)
    ENDIF
!
    IF (PRESENT(PF_WASTE_CAN)) THEN
      PF_WASTE_CAN(JLOOP) = XDESC_F_WASTE_CAN(IINDEX)
    ENDIF
!
    IF (PRESENT(PINF)) THEN
      PINF(JLOOP) = XDESC_INF(IINDEX)
    ENDIF
!
...
!      -----------------------------------------------
!
    IF (KTYPE(JLOOP)<=SIZE(NDESC_USE_LIST) .AND. KTYPE(JLOOP)>0) &
    IINDEX = NDESC_USE_LIST(KTYPE(JLOOP))
!
    IF (PRESENT(PTCOOL_TARGET)) THEN
      PTCOOL_TARGET(JLOOP) = XDESC_TCOOL_TARGET(IINDEX)
    ENDIF
!  
    IF (PRESENT(PTHEAT_TARGET)) THEN
      PTHEAT_TARGET(JLOOP) = XDESC_THEAT_TARGET(IINDEX)
    ENDIF
!
    IF (PRESENT(PQIN)) THEN
      PQIN(JLOOP) = XDESC_QIN(IINDEX)
    ENDIF
!
    IF (PRESENT(PQIN_FLAT)) THEN
      PQIN_FLAT(JLOOP) = XDESC_QIN_FLAT(IINDEX)
    ENDIF
!
    IF (PRESENT(PSHGC_SH)) THEN
      PSHGC_SH(JLOOP) = XDESC_SHGC_SH(IINDEX)
    ENDIF    
    !
    IF (PRESENT(PSHADE)) THEN
      PSHADE(JLOOP) = XDESC_SHADE(IINDEX)
    ENDIF
!
