...
!
SUBROUTINE INI_VAR_FROM_DATA_1D (DTCO, UG, U, USS, PPAR_VEGTYPE, &
...
!
!
      SUBROUTINE INI_VAR_FROM_DATA_2D (DTCO, UG, U, USS, PPAR_VEGTYPE, &
...
IF (ANY(LPRESENT(:))) THEN
  ...
  IF (TRIM(HTYPE)=="NAT" .AND. &
      SIZE(PFIELD,2)==NVEGTYPE .AND. YNAME(1:7).NE.'VEGTYPE') THEN
    ...
    DO JI=1,SIZE(PFIELD,2)
      IF (LPRESENT(JI)) THEN
        IF (SIZE(PPAR_VEGTYPE)>0) WHERE (PPAR_VEGTYPE(:,JI).EQ.0.) PFIELD(:,JI)=0.0
      ELSE
        PFIELD(:,JI)=0.
      ENDIF
    ENDDO
...
DO JTIME=1,SIZE(PFIELD_TIME,2)
  ...
  IF (ANY(LPRESENT(:))) THEN
    ...
    IF (TRIM(HTYPE)=="NAT" .AND. SIZE(PFIELD_TIME,3)==NVEGTYPE) THEN
      ...
      DO JI=1,SIZE(PFIELD_TIME,3)
        IF (LPRESENT(JI)) THEN
          IF (SIZE(PPAR_VEGTYPE)>0) &
                  WHERE (PPAR_VEGTYPE(:,JI).EQ.0.) PFIELD_TIME(:,JTIME,JI)=0.0
        ELSE
