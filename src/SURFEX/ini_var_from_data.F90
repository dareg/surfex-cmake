...
ELSE
  ...
  DO JV=1,SIZE(PFIELD,2)
    ...
    IF (HNAME(1:6)=='H_TREE') THEN
      IF (JV<=3.OR.(JV>=7.AND.JV<=12).OR.JV>=18) CYCLE
    ENDIF
...
!
  ...
  ELSE
    ...
    IF (.NOT.ALL(LEN_TRIM(HFNAM(:,JTIME))/=0) .AND. &
        .NOT.ALL(LEN_TRIM(HFNAM(2:,JTIME))==0)) THEN
    ...
    DO JV=1,ISIZE_V
      ...
      IF (HNAME(1:3)=='LAI'.OR.HNAME(1:10)=='ALBNIR_VEG'.OR.HNAME(1:10)=='ALBVIS_VEG') THEN
        IF (JV<=3) CYCLE
      ENDIF
