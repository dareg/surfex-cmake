      SUBROUTINE INI_VAR_FROM_PATCH (DTCO, UG, U, PPATCH, PPATCH_OLD, PLAI, &
...
!            ------------------------------
!
LOGICAL, DIMENSION(SIZE(PLAI,1),SIZE(PLAI,2)) :: GVEG
...
!                 
  ...
  IF (TRIM(HNAME)=='WR')THEN
    ...
    DO JPATCH=1,IPATCH
      WHERE(PPATCH(:,JPATCH) /=0. .AND. PPATCH_OLD(:,JPATCH) ==0..AND.PLAI(:,JPATCH)==0.)
          PFIELD(:,JPATCH) = 0.  
          GVEG  (:,JPATCH) = .FALSE.
      ENDWHERE
    END DO
