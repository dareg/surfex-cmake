...
!
 CALL GET_SURF_MASK_n('NATURE',INI,IMASK,U%NSIZE_FULL,ILUOUT)
!
DO JVEGTYPE=1,IVEGTYPE
  ...
  IF(PRESENT(PDEF))THEN
    CALL INTERPOL_FIELD(HPROGRAM,ILUOUT,NSIZE_TOT,ZFIELD_TOT,HNAME,PDEF=PDEF(JVEGTYPE))
  ELSE
    CALL INTERPOL_FIELD(HPROGRAM,ILUOUT,NSIZE_TOT,ZFIELD_TOT,HNAME)
  ENDIF 
