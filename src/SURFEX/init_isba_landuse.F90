!#############################################################
SUBROUTINE INIT_ISBA_LANDUSE (DTCO, UG, U, IO, NK, NP, NPE, PMESH_SIZE, &
                              HPROGRAM)  
!#############################################################
!
...
!
INTEGER :: ILUOUT, ICOUNT, ISIZE, JP
...
!-------------------------------------------------------------------------------
!
 CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH,  &
                         HPROGRAM,ILUOUT,'WR      ', 0)

IF (IO%LGLACIER) CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'ICE_STO ', 0)
!
DO JLAYER=1,SIZE(NPE%AL(1)%XTG,2)
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'TEMP GRO', 0, JLAYER)
END DO
!
!
 CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'ALBSNOW ', 0)
!
IF (NPE%AL(1)%TSNOW%SCHEME=='1-L'  .OR. NPE%AL(1)%TSNOW%SCHEME=='3-L' &
        .OR. NPE%AL(1)%TSNOW%SCHEME=='CRO') THEN
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'EMISSNOW', 0)    
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'TSSNOW  ', 0)
ENDIF
!
DO JLAYER=1,NPE%AL(1)%TSNOW%NLAYER
   !
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'WSNOW   ', 0, JLAYER)
   !
   IF (NPE%AL(1)%TSNOW%SCHEME=='3-L' .OR. NPE%AL(1)%TSNOW%SCHEME=='CRO') THEN            
      CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'TEMPSNOW',0, JLAYER)
      CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'HEATSNOW', 0, JLAYER)     
      CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'AGESNOW ', 0, JLAYER)
   ENDIF
   !
   IF (NPE%AL(1)%TSNOW%SCHEME=='1-L') THEN
      CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'TSNOW   ', 0, JLAYER)
   ENDIF
   !
   IF(NPE%AL(1)%TSNOW%SCHEME=='CRO') THEN
      CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'GR1SNOW', 0, JLAYER)
      CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'GR2SNOW', 0, JLAYER)
      CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'HISTSNOW', 0, JLAYER)
   ENDIF
   !
ENDDO
!
...
DO JLAYER=1,IO%NGROUND_LAYER
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'WG      ', 0, JLAYER)
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'WGI     ', 0, JLAYER)
ENDDO
!
!
 CALL CONSERV_GLOBAL_MASS(DTCO, U, NP, NPE, PMESH_SIZE,  &
                          IO%NPATCH,ILUOUT,ZZDG,ZZDG_OLD,'WG ', ZWG_OLD )
 CALL CONSERV_GLOBAL_MASS(DTCO, U, NP, NPE, PMESH_SIZE,  &
                          IO%NPATCH,ILUOUT,ZZDG,ZZDG_OLD,'WGI',ZWGI_OLD)
!
...
!
 CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'RESA    ', 3, JLAYER)
!
DO JLAYER=1,NPE%AL(1)%TSNOW%NLAYER
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'RHOSNOW ', 3, JLAYER)
ENDDO
!
IF (IO%CPHOTO/='NON') THEN
   !
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'AN      ', 3)
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'ANDAY   ', 3)   
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'ANFM    ', 3)
   CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'LE      ', 3)
   !
   DO JNBIOMASS=1,IO%NNBIOMASS
      CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'RESPBIOM', 3,JNBIOMASS)
      CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'BIOMASS ', 3,JNBIOMASS)
   ENDDO
   !
   IF (IO%CRESPSL=='CNT') THEN
      ...
      DO JNLITTLEVS=1,IO%NNLITTLEVS
         CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'LIGNINST',3,JNLITTLEVS)
         DO JNLITTER=1,IO%NNLITTER
            CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'LITTER  ',3,JNLITTER,JNLITTLEVS)
         ENDDO
      ENDDO
      !
      DO JNSOILCARB=1,IO%NNSOILCARB
         CALL INI_VAR_FROM_PATCH(DTCO, UG, U, NP, NPE, IO%NPATCH, &
                         HPROGRAM,ILUOUT,'SOILCARB',3,JNSOILCARB)
      ENDDO
      !
   ENDIF
   !
