...
IF (ANY(DTV%LDATA_VEG)) THEN
  ...
  IF (.NOT.ANY(DTV%LDATA_GAMMA)) &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PGAMMA=DTV%XPAR_GAMMA)
  IF (.NOT.ANY(DTV%LDATA_WRMAX_CF)) &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PWRMAX_CF=DTV%XPAR_WRMAX_CF)
  IF (.NOT.ANY(DTV%LDATA_RGL))      &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PRGL=DTV%XPAR_RGL)
  IF (.NOT.ANY(DTV%LDATA_CV))       &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PCV=DTV%XPAR_CV)
  IF (.NOT.ANY(DTV%LDATA_ALBNIR_VEG)) THEN
    CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PALBNIR_VEG=DTV%XPAR_ALBNIR_VEG(:,1,:))
    DO JT = 2,DTV%NTIME 
      DTV%XPAR_ALBNIR_VEG(:,JT,:) = DTV%XPAR_ALBNIR_VEG(:,1,:)
    ENDDO
  ENDIF
  IF (.NOT.ANY(DTV%LDATA_ALBVIS_VEG)) THEN
    CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PALBVIS_VEG=DTV%XPAR_ALBVIS_VEG(:,1,:))
    DO JT = 2,DTV%NTIME 
      DTV%XPAR_ALBVIS_VEG(:,JT,:) = DTV%XPAR_ALBVIS_VEG(:,1,:)
    ENDDO
  ENDIF    
  IF (.NOT.ANY(DTV%LDATA_ALBUV_VEG)) THEN
    CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PALBUV_VEG=DTV%XPAR_ALBUV_VEG(:,1,:))
    DO JT = 2,DTV%NTIME 
      DTV%XPAR_ALBUV_VEG(:,JT,:) = DTV%XPAR_ALBUV_VEG(:,1,:)
    ENDDO
  ENDIF      
  IF (ISIZE_LMEB_PATCH>0)  THEN
    IF (.NOT.ANY(DTV%LDATA_BSLAI)) CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,PBSLAI=DTV%XPAR_BSLAI)
  ENDIF
  IF (IO%CPHOTO /= 'NON')  THEN
    IF (.NOT.ANY(DTV%LDATA_GMES))   &
            CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PGMES_ST=DTV%XPAR_GMES)
    IF (.NOT.ANY(DTV%LDATA_BSLAI))  &
            CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PBSLAI_ST=DTV%XPAR_BSLAI)
    IF (.NOT.ANY(DTV%LDATA_SEFOLD)) &
            CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PSEFOLD_ST=DTV%XPAR_SEFOLD)
    IF (.NOT.ANY(DTV%LDATA_GC))     &
            CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PGC_ST=DTV%XPAR_GC)
  ELSE
    IF (.NOT.ANY(DTV%LDATA_GMES))   &
            CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PGMES=DTV%XPAR_GMES)
    IF (.NOT.ANY(DTV%LDATA_BSLAI))  &
            CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PBSLAI=DTV%XPAR_BSLAI)
    IF (.NOT.ANY(DTV%LDATA_SEFOLD)) &
            CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PSEFOLD=DTV%XPAR_SEFOLD)
    IF (.NOT.ANY(DTV%LDATA_GC))     &
            CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PGC=DTV%XPAR_GC)
  ENDIF
  IF (.NOT.ANY(DTV%LDATA_LAIMIN))    &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PLAIMIN_OUT=DTV%XPAR_LAIMIN)
  IF (.NOT.ANY(DTV%LDATA_F2I))       &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PF2I=DTV%XPAR_F2I)
  IF (.NOT.ANY(DTV%LDATA_CE_NITRO))  &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PCE_NITRO=DTV%XPAR_CE_NITRO)
  IF (.NOT.ANY(DTV%LDATA_CF_NITRO))  &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PCF_NITRO=DTV%XPAR_CF_NITRO)
  IF (.NOT.ANY(DTV%LDATA_CNA_NITRO)) &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PCNA_NITRO=DTV%XPAR_CNA_NITRO)
  !
  ...
  IF (.NOT.ANY(DTV%LDATA_IRRIG)) THEN
    ...
    CALL AV_PGD(DTCO, DTV%XPAR_IRRIG(:,KDECADE2,:),PCOVER,XDATA_IRRIG(:,:),YVEG,'ARI',OCOVER,KDECADE=KDECADE2)
    DTV%LDATA_IRRIG(:)=.TRUE.
  ENDIF
  !
  IF (.NOT.ANY(DTV%LDATA_WATSUP)) THEN
    ALLOCATE(DTV%XPAR_WATSUP     (KDIM,DTV%NTIME,NVEGTYPE))
    CALL AV_PGD(DTCO, DTV%XPAR_WATSUP(:,KDECADE2,:),PCOVER,XDATA_WATSUP(:,:),YVEG,'ARI',OCOVER,KDECADE=KDECADE2)
  ...
  IF (.NOT.ANY(DTV%LDATA_SEED_M).OR..NOT.ANY(DTV%LDATA_SEED_D)) THEN
    ...
    CALL AV_PGD(TPWORK(:,:),PCOVER,TDATA_SEED(:,:),YVEG,'MAJ',OCOVER,KDECADE=KDECADE2)
  ...
  IF (.NOT.ANY(DTV%LDATA_REAP_M).OR..NOT.ANY(DTV%LDATA_REAP_D)) THEN
    ...
    CALL AV_PGD(TPWORK(:,:),PCOVER,TDATA_REAP(:,:),YVEG,'MAJ',OCOVER,KDECADE=KDECADE2)
...
IF (DTV%LDATA_VEGTYPE) THEN
  ...
  IF (.NOT.ANY(DTV%LDATA_Z0_O_Z0H)) &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PZ0_O_Z0H=DTV%XPAR_Z0_O_Z0H)
  IF (IO%CPHOTO /= 'NON')  THEN
    IF (.NOT.ANY(DTV%LDATA_DMAX)) &
            CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PDMAX_ST=DTV%XPAR_DMAX)
  ELSE
    IF (.NOT.ANY(DTV%LDATA_DMAX)) &
            CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PDMAX=DTV%XPAR_DMAX)
  ENDIF
  IF (.NOT.ANY(DTV%LDATA_RE25)) &
          CALL INI_DATA_PARAM(DTV%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PRE25=DTV%XPAR_RE25)
  !
