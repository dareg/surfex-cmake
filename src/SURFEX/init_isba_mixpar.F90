!     #########
      SUBROUTINE INIT_ISBA_MIXPAR (DTCO, DTI, KDIM, IO, &
...
IF (.NOT.DTI%LDATA_VEG) THEN
  ...
  IF (DTI%LDATA_LAI) THEN
    CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PLAI=DTI%XPAR_LAI,PVEG_OUT=DTI%XPAR_VEG)
    DTI%LDATA_VEG=.TRUE.
  ELSE
    CALL AV_PGD(DTCO, &
...
IF (.NOT.DTI%LDATA_ROOTFRAC .AND. IO%CISBA=='DIF' .AND. DTI%LDATA_ROOT_DEPTH) THEN
  !
  IF (.NOT.DTI%LDATA_ROOT_EXTINCTION) THEN
    ALLOCATE(DTI%XPAR_ROOT_EXTINCTION(KDIM,NVEGTYPE))
    CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PROOT_EXTINCTION=DTI%XPAR_ROOT_EXTINCTION)
    DTI%LDATA_ROOT_EXTINCTION = .TRUE.
  ENDIF
  !
  IF (.NOT.DTI%LDATA_ROOT_LIN) THEN
    ALLOCATE(DTI%XPAR_ROOT_LIN(KDIM,NVEGTYPE))
    CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PROOT_LIN=DTI%XPAR_ROOT_LIN)
...
!
! MEB roots
IF (.NOT.DTI%LDATA_ROOTFRACGV .AND. IO%CISBA=='DIF' .AND. DTI%LDATA_ROOT_DEPTHGV) THEN
  !
...
IF (.NOT.DTI%LDATA_EMIS .AND. DTI%LDATA_VEG) THEN
  ...
  CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PVEG_IN=DTI%XPAR_VEG,PEMIS_ECO=DTI%XPAR_EMIS)
  DTI%LDATA_EMIS=.TRUE.
ENDIF
!z0
IF (.NOT.DTI%LDATA_Z0 .AND. (DTI%LDATA_LAI .OR.DTI%LDATA_H_TREE)) THEN
  ALLOCATE(DTI%XPAR_Z0(KDIM,DTI%NTIME,NVEGTYPE))
  CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PLAI=DTI%XPAR_LAI,PH_TREE=DTI%XPAR_H_TREE,PZ0=DTI%XPAR_Z0)
  DTI%LDATA_Z0=.TRUE.
ENDIF
...
IF (DTI%LDATA_LAI) THEN
  IF (.NOT.DTI%LDATA_RSMIN) THEN
    ...
    CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PRSMIN=DTI%XPAR_RSMIN)
  ENDIF
...
IF (DTI%LDATA_VEG) THEN
  !
  IF (.NOT.DTI%LDATA_GAMMA) ALLOCATE(DTI%XPAR_GAMMA     (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_WRMAX_CF) ALLOCATE(DTI%XPAR_WRMAX_CF  (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_RGL) ALLOCATE(DTI%XPAR_RGL       (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_CV) ALLOCATE(DTI%XPAR_CV        (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_ALBNIR_VEG) ALLOCATE(DTI%XPAR_ALBNIR_VEG(KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_ALBVIS_VEG) ALLOCATE(DTI%XPAR_ALBVIS_VEG(KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_ALBUV_VEG) ALLOCATE(DTI%XPAR_ALBUV_VEG (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_GMES) ALLOCATE(DTI%XPAR_GMES      (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_BSLAI) ALLOCATE(DTI%XPAR_BSLAI     (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_SEFOLD) ALLOCATE(DTI%XPAR_SEFOLD    (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_GC) ALLOCATE(DTI%XPAR_GC        (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_LAIMIN) ALLOCATE(DTI%XPAR_LAIMIN    (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_F2I) ALLOCATE(DTI%XPAR_F2I       (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_CE_NITRO) ALLOCATE(DTI%XPAR_CE_NITRO  (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_CF_NITRO) ALLOCATE(DTI%XPAR_CF_NITRO  (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_CNA_NITRO) ALLOCATE(DTI%XPAR_CNA_NITRO (KDIM,NVEGTYPE))
  !
  IF (.NOT.DTI%LDATA_GAMMA) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PGAMMA=DTI%XPAR_GAMMA)
  IF (.NOT.DTI%LDATA_WRMAX_CF) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PWRMAX_CF=DTI%XPAR_WRMAX_CF)
  IF (.NOT.DTI%LDATA_RGL) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PRGL=DTI%XPAR_RGL)
  IF (.NOT.DTI%LDATA_CV) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PCV=DTI%XPAR_CV)
  IF (.NOT.DTI%LDATA_ALBNIR_VEG) &
     CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PALBNIR_VEG=DTI%XPAR_ALBNIR_VEG)
  IF (.NOT.DTI%LDATA_ALBVIS_VEG) &
     CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PALBVIS_VEG=DTI%XPAR_ALBVIS_VEG)
  IF (.NOT.DTI%LDATA_ALBUV_VEG) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PALBUV_VEG=DTI%XPAR_ALBUV_VEG)
  ...
  IF (IO%CPHOTO == 'AST' .OR. IO%CPHOTO == 'LST' .OR. IO%CPHOTO == 'NIT' .OR. IO%CPHOTO=='NCB')  THEN
    IF (.NOT.DTI%LDATA_GMES) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PGMES_ST=DTI%XPAR_GMES)
    IF (.NOT.DTI%LDATA_BSLAI) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PBSLAI_ST=DTI%XPAR_BSLAI)
    IF (.NOT.DTI%LDATA_SEFOLD) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PSEFOLD_ST=DTI%XPAR_SEFOLD)
    IF (.NOT.DTI%LDATA_GC) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PGC_ST=DTI%XPAR_GC)
  ELSE
    IF (.NOT.DTI%LDATA_GMES) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PGMES=DTI%XPAR_GMES)
    IF (.NOT.DTI%LDATA_BSLAI) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PBSLAI=DTI%XPAR_BSLAI)
    IF (.NOT.DTI%LDATA_SEFOLD) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PSEFOLD=DTI%XPAR_SEFOLD)
    IF (.NOT.DTI%LDATA_GC) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PGC=DTI%XPAR_GC)
  ENDIF
  IF (.NOT.DTI%LDATA_LAIMIN) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PLAIMIN_OUT=DTI%XPAR_LAIMIN)
  IF (.NOT.DTI%LDATA_F2I) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PF2I=DTI%XPAR_F2I)
  IF (.NOT.DTI%LDATA_CE_NITRO) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PCE_NITRO=DTI%XPAR_CE_NITRO)
  IF (.NOT.DTI%LDATA_CF_NITRO) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PCF_NITRO=DTI%XPAR_CF_NITRO)
  IF (.NOT.DTI%LDATA_CNA_NITRO) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PCNA_NITRO=DTI%XPAR_CNA_NITRO)
  !
...
! MEB stuff
!
  IF (.NOT.DTI%LDATA_GAMMAGV) ALLOCATE(DTI%XPAR_GAMMAGV     (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_WRMAX_CFGV) ALLOCATE(DTI%XPAR_WRMAX_CFGV  (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_RGLGV) ALLOCATE(DTI%XPAR_RGLGV       (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_RSMINGV) ALLOCATE(DTI%XPAR_RSMINGV     (KDIM,NVEGTYPE))
  !
...
!LAIGV
  IF (.NOT.DTI%LDATA_LAIGV) THEN
    ALLOCATE(DTI%XPAR_LAIGV(KDIM,DTI%NTIME,NVEGTYPE))
...
IF (DTI%LDATA_VEGTYPE) THEN
  IF (.NOT.DTI%LDATA_Z0_O_Z0H) ALLOCATE(DTI%XPAR_Z0_O_Z0H  (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_DMAX) ALLOCATE(DTI%XPAR_DMAX      (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_RE25) ALLOCATE(DTI%XPAR_RE25      (KDIM,NVEGTYPE))
  IF (.NOT.DTI%LDATA_Z0_O_Z0H) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PZ0_O_Z0H=DTI%XPAR_Z0_O_Z0H)
  IF (IO%CPHOTO == 'AST' .OR. IO%CPHOTO == 'LST' .OR. IO%CPHOTO == 'NIT' .OR. IO%CPHOTO=='NCB')  THEN
    IF (.NOT.DTI%LDATA_DMAX) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PDMAX_ST=DTI%XPAR_DMAX)
  ELSE
    IF (.NOT.DTI%LDATA_DMAX) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PDMAX=DTI%XPAR_DMAX)
  ENDIF
  IF (.NOT.DTI%LDATA_RE25) CALL INI_DATA_PARAM(DTI%XPAR_VEGTYPE,OAGRI_TO_GRASS=IO%LAGRI_TO_GRASS,PRE25=DTI%XPAR_RE25)
  !
