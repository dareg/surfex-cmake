...
IF (.NOT.LDATA_LAI) THEN
  ...
  CALL AV_PGD(XPAR_LAI(:,KDECADE2,:),PCOVER,XDATA_LAI(:,KDECADE,:),YVEG,'ARI',KDECADE=JTIME)
ENDIF
!veg
...
IF (LDATA_VEG) THEN
  ...
  IF (.NOT.LDATA_IRRIG) THEN
    ...
    CALL AV_PGD(XPAR_IRRIG(:,KDECADE2,:),PCOVER,XDATA_IRRIG(:,:),YVEG,'ARI',KDECADE=JTIME)
    LDATA_IRRIG=.TRUE.
  ENDIF
  !
  IF (.NOT.LDATA_WATSUP) THEN
    ALLOCATE(XPAR_WATSUP     (NDIM,NTIME,NVEGTYPE))
    CALL AV_PGD(XPAR_WATSUP(:,KDECADE2,:),PCOVER,XDATA_WATSUP(:,:),YVEG,'ARI',KDECADE=JTIME)
    LDATA_WATSUP=.TRUE.
  ENDIF
  !
ENDIF
