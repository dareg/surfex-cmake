...
!               --------
 ...
 !        0.1. Hard defaults
 !      
 CALL DEFAULT_ISBA(IM%I%O%XTSTEP, IM%I%O%XOUT_TSTEP,                           &
                     IM%I%O%CROUGH,IM%I%O%CRUNOFF,IM%I%O%CALBEDO,IM%I%O%CSCOND,              &
                     IM%I%O%CC1DRY, IM%I%O%CSOILFRZ, IM%I%O%CDIFSFCOND, IM%I%O%CSNOWRES,     &
                     IM%I%O%CCPSURF, IM%I%O%XCGMAX, IM%I%O%XCDRAG, IM%I%O%CKSAT, IM%I%O%LSOC,       &
                     IM%I%O%CRAIN, IM%I%O%CHORT, IM%I%O%LGLACIER, IM%I%O%LCANOPY_DRAG,       &
                     IM%I%O%LVEGUPD, IM%I%O%LSPINUPCARBS, IM%I%O%LSPINUPCARBW,        &
                     IM%I%O%XSPINMAXS, IM%I%O%XSPINMAXW, IM%I%O%XCO2_START, IM%I%O%XCO2_END, &
                     IM%I%O%NNBYEARSPINS, IM%I%O%NNBYEARSPINW, IM%I%O%LNITRO_DILU     )
 !                  
 ...
 CALL DEFAULT_DIAG_ISBA(IM%DGI%N2M,IM%DGI%LSURF_BUDGET,IM%DGI%L2M_MIN_ZS,IM%DGI%LRAD_BUDGET,   &
                        ...
                        IM%DGI%XDIAG_TSTEP                                )  
 !
 CALL DEFAULT_CROCUS(IM%I%O%LSNOWDRIFT,IM%I%O%LSNOWDRIFT_SUBLIM,IM%I%O%LSNOW_ABS_ZENITH,&
                     IM%I%O%CSNOWMETAMO,IM%I%O%CSNOWRAD)
 ! 
ENDIF
!
...
!
IM%I%O%NNBYEARSOLD = 1
IM%I%O%NSPINS      = 1
IM%I%O%NSPINW      = 1
!
IF (HINIT=='PRE') THEN 
  CALL READ_PREP_ISBA_SNOW(HPROGRAM,IM%I%R%TSNOW%SCHEME,IM%I%R%TSNOW%NLAYER) 
!
!* initialization of soil carbon scheme
!
  CALL READ_PREP_ISBA_CARBON(HPROGRAM,IM%I%O%CRESPSL)
!
  IF (IM%I%O%CRESPSL=='CNT') THEN
    IM%I%O%NNLITTER = 2
    IM%I%O%NNLITTLEVS = 2
    IM%I%O%NNSOILCARB = 3
  ELSE
...
!
  ...
  ELSE  
    CALL READ_SURF(&
                HPROGRAM,'RESPSL',IM%I%O%CRESPSL,IRESP)
    CALL READ_SURF(&
                HPROGRAM,'NLITTER',IM%I%O%NNLITTER,IRESP)
    CALL READ_SURF(&
                HPROGRAM,'NLITTLEVS',IM%I%O%NNLITTLEVS,IRESP)
    CALL READ_SURF(&
                HPROGRAM,'NSOILCARB',IM%I%O%NNSOILCARB,IRESP)
    IF(IVERSION>=7.AND.(IM%I%O%LSPINUPCARBS.OR.IM%I%O%LSPINUPCARBW))THEN
      CALL READ_SURF(&
                HPROGRAM,'NBYEARSOLD',IM%I%O%NNBYEARSOLD,IRESP)
    ELSE
      IM%I%O%NNBYEARSOLD=NUNDEF
    ENDIF
  ENDIF
!
  CALL END_IO_SURF_n(HPROGRAM)
...
SELECT CASE (HINIT)
  CASE ('PGD')
    IM%I%I%TTIME%TDATE%YEAR = NUNDEF
    IM%I%I%TTIME%TDATE%MONTH= NUNDEF
    IM%I%I%TTIME%TDATE%DAY  = NUNDEF
    IM%I%I%TTIME%TIME       = XUNDEF
  ...
  CASE ('PRE')
    ...
    CALL READ_ISBA_DATE(&
                        HPROGRAM,HINIT,ILUOUT,HATMFILE,HATMFILETYPE,KYEAR,KMONTH,KDAY,PTIME,IM%I%I%TTIME)

  CASE DEFAULT
CALL INIT_IO_SURF_n(DTCO, DGU, U, &
                     HPROGRAM,'NATURE','ISBA  ','READ ')
    CALL READ_SURF(&
                HPROGRAM,'DTCUR',IM%I%I%TTIME,IRESP)
    CALL END_IO_SURF_n(HPROGRAM)
END SELECT
!
!-----------------------------------------------------------------------------------------------------
...
!               ------
!
IF ( IM%I%O%CPHOTO/='NON' .AND. IM%I%O%NPATCH/=12 .AND. IM%I%O%NPATCH/=19 )THEN
  CALL ABOR1_SFX('INIT_ISBAN: INCONSISTENCY BETWEEN CPHOTO AND NPATCH')
ENDIF
!
IF (HINIT=='PRE' .AND. IM%I%R%TSNOW%SCHEME.NE.'3-L' .AND. IM%I%R%TSNOW%SCHEME.NE.'CRO' .AND. IM%I%O%CISBA=='DIF') THEN
    CALL ABOR1_SFX("INIT_ISBAN: WITH CISBA = DIF, CSNOW MUST BE 3-L OR CRO")
ENDIF
IF ( IM%I%O%CPHOTO/='LAI' .AND. IM%I%O%CPHOTO/='LST' .AND. IM%I%O%CPHOTO/='NIT' .AND. IM%I%O%CPHOTO/='NCB' .AND. LAGRIP) THEN
  CALL ABOR1_SFX('INIT_ISBAN: INCONSISTENCY BETWEEN CPHOTO AND LAGRIP')
ENDIF
IF ( IM%I%O%CPHOTO/='NCB' .AND. IM%I%O%CRESPSL=='CNT') THEN
  CALL ABOR1_SFX('INIT_ISBAN: INCONSISTENCY BETWEEN CPHOTO AND CRESPSL')
ENDIF
IF (HINIT=='PRE' .AND. ISIZE_LMEB_PATCH>0 .AND. IM%I%R%TSNOW%SCHEME.NE.'3-L' .AND. IM%I%R%TSNOW%SCHEME.NE.'CRO') THEN
    CALL ABOR1_SFX("INIT_ISBAN: WITH LMEB_PATCH = TRUE, CSNOW MUST BE 3-L OR CRO")
ENDIF
IF(IM%I%O%CPHOTO/='NCB'.AND.IM%I%O%LSPINUPCARBW)THEN
  CALL ABOR1_SFX('INIT_ISBAN: INCONSISTENCY BETWEEN CPHOTO AND LSPINUPCARBW (if not NCB must be false)')
ENDIF
IF(IM%I%O%CRESPSL/='CNT'.AND.IM%I%O%LSPINUPCARBS)THEN
  CALL ABOR1_SFX('INIT_ISBAN: INCONSISTENCY BETWEEN CRESPSL AND LSPINUPCARBS (if not CNT must be false)')
ENDIF
IF(IM%I%O%LSPINUPCARBW.AND.REAL(IM%I%O%NNBYEARSPINW)>REAL(IM%I%O%NNBYEARSPINS)*0.5)THEN
  WRITE(ILUOUT,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
...
ENDIF
IF(IM%I%O%LSPINUPCARBS.AND.(IM%I%O%XCO2_START==XUNDEF.OR.IM%I%O%XCO2_END==XUNDEF))THEN
  WRITE(ILUOUT,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
...
!
   CALL INI_DATA_PARAM(DTCO%XDATA_VEGTYPE, PSURF=DTCO%XDATA_NATURE, PH_TREE=XDATA_H_TREE,PLAI=XDATA_LAI,                   &
                                  ...
                                  PSEFOLD_ST=XDATA_SEFOLD_ST, PDMAX_ST=XDATA_DMAX_ST, OAGRI_TO_GRASS=IM%I%O%LAGRI_TO_GRASS)
!
   ZCO2(:) = PRHOA(:) * IM%I%O%XCO2_START * 1.E-6 * XMCO2 / XMD
!
ELSEIF(IM%I%O%LSPINUPCARBS .AND. (IM%I%O%NNBYEARSOLD > ISPINEND) .AND. (IM%I%O%NNBYEARSOLD <= IM%I%O%NNBYEARSPINS) )THEN
!
   ZSPINCO2 = IM%I%O%XCO2_START + (IM%I%O%XCO2_END-IM%I%O%XCO2_START) * REAL(IM%I%O%NNBYEARSOLD - ISPINEND) / &
                REAL(IM%I%O%NNBYEARSPINS - ISPINEND)
!
...
CALL COMPUTE_ISBA_PARAMETERS(DTCO, DGU, UG, U, IM, DST, SLT, SV, &
                             ...
                             HTEST                                )
!
IF ( IM%I%O%CSNOWMETAMO/="B92" ) THEN
  CALL READ_FZ06('drdt_bst_fit_60.nc')
ENDIF
!
IF ( IM%I%O%CSNOWRAD=="TAR" .OR. IM%I%O%CSNOWRAD=="TA1" .OR.  IM%I%O%CSNOWRAD=="TA2" ) THEN
  CALL INIT_TARTES()
END IF
!
IF (LHOOK) CALL DR_HOOK('INIT_ISBA_N',1,ZHOOK_HANDLE)
