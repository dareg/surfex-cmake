...
!
  IF (IVERSION<6) THEN
    I%CRESPSL='DEF'
  ELSE  
    CALL READ_SURF(HPROGRAM,'RESPSL',I%CRESPSL,IRESP)
    CALL READ_SURF(HPROGRAM,'NLITTER',I%NNLITTER,IRESP)
    CALL READ_SURF(HPROGRAM,'NLITTLEVS',I%NNLITTLEVS,IRESP)
    CALL READ_SURF(HPROGRAM,'NSOILCARB',I%NNSOILCARB,IRESP)
    IF(IVERSION>=7.AND.(I%LSPINUPCARBS.OR.I%LSPINUPCARBW))THEN
      CALL READ_SURF(HPROGRAM,'NBYEARSOLD',I%NNBYEARSOLD,IRESP)
    ELSE
...
SELECT CASE (HINIT)
  ...
  CASE ('PRE')
    ...
    CALL READ_ISBA_DATE(HPROGRAM,HINIT,ILUOUT,HATMFILE,HATMFILETYPE,KYEAR,KMONTH,KDAY,PTIME,I%TTIME)

  CASE DEFAULT
    CALL INIT_IO_SURF_n(HPROGRAM,'NATURE','ISBA  ','READ ')
    CALL READ_SURF(HPROGRAM,'DTCUR',I%TTIME,IRESP)
    CALL END_IO_SURF_n(HPROGRAM)
END SELECT
!
!-----------------------------------------------------------------------------------------------------
