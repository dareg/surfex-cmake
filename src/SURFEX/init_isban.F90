!#############################################################
SUBROUTINE INIT_ISBA_n (DTCO, OREAD_BUDGETC, UG, U, USS, AG, CHI, DTI, DGI, GB, &
                        ICP, ISS, IG, I, DTZ, DST, SLT, SV, &
...
!               --------
 ...
 !        0.1. Hard defaults
 !      
 CALL DEFAULT_ISBA(I%O%XTSTEP, I%O%XOUT_TSTEP,                                 &
                   I%O%CROUGH,I%O%CRUNOFF,I%O%CALBEDO,I%O%CSCOND,              &
                   I%O%CC1DRY, I%O%CSOILFRZ, I%O%CDIFSFCOND, I%O%CSNOWRES,     &
                   I%O%CCPSURF, I%O%XCGMAX, I%O%XCDRAG, I%O%CKSAT, I%O%LSOC,   &
                   I%O%CRAIN, I%O%CHORT, I%O%LGLACIER, I%O%LCANOPY_DRAG,       &
                   I%O%LVEGUPD, I%O%LSPINUPCARBS, I%O%LSPINUPCARBW,            &
                   I%O%XSPINMAXS, I%O%XSPINMAXW, I%O%XCO2_START, I%O%XCO2_END, &
                   I%O%NNBYEARSPINS, I%O%NNBYEARSPINW, I%O%LNITRO_DILU     )
 !                  
 CALL DEFAULT_CH_DEP(CHI%CCH_DRY_DEP)
 CALL DEFAULT_CH_BIO_FLUX(CHI%LCH_BIO_FLUX)                  
 CALL DEFAULT_DIAG_ISBA(DGI%O%N2M,DGI%O%LSURF_BUDGET,DGI%O%L2M_MIN_ZS,DGI%O%LRAD_BUDGET,  &
                        DGI%O%LCOEF,DGI%O%LSURF_VARS,DGI%DE%LSURF_EVAP_BUDGET,            &
                        DGI%DM%LSURF_MISC_BUDGET,DGI%DM%LSURF_DIAG_ALBEDO,                &
                        DGI%O%LSURF_BUDGETC,DGI%DM%LSURF_MISC_DIF,DGI%O%LPATCH_BUDGET,   &
                        DGI%O%LPGD,DGI%O%LRESET_BUDGETC,DGI%DE%LWATER_BUDGET,            &
                        DGI%O%XDIAG_TSTEP                                )  
 !
 CALL DEFAULT_CROCUS(I%O%LSNOWDRIFT,I%O%LSNOWDRIFT_SUBLIM,I%O%LSNOW_ABS_ZENITH,&
                     I%O%CSNOWMETAMO,I%O%CSNOWRAD)
 ! 
ENDIF
!
!        0.2. Defaults from file header
!    
 CALL READ_DEFAULT_ISBA_n(CHI, DGI%DE, DGI%O, DGI%DM, I%O, HPROGRAM)
!
 CALL READ_ISBA_CONF_n(CHI, DGI%DE, DGI%O, DGI%DM, I%O, HPROGRAM)
!
...
!
IF (HINIT=='PRE') THEN 
  CALL READ_PREP_ISBA_SNOW(HPROGRAM,I%R%TSNOW%SCHEME,I%R%TSNOW%NLAYER) 
!
!* initialization of soil carbon scheme
!
  CALL READ_PREP_ISBA_CARBON(HPROGRAM,I%O%CRESPSL)
!
...
!
  IF (IVERSION<6) THEN
    I%O%CRESPSL='DEF'
  ELSE  
    CALL READ_SURF(HPROGRAM,'RESPSL',I%O%CRESPSL,IRESP)
    CALL READ_SURF(HPROGRAM,'NLITTER',I%O%NNLITTER,IRESP)
    CALL READ_SURF(HPROGRAM,'NLITTLEVS',I%O%NNLITTLEVS,IRESP)
    CALL READ_SURF(HPROGRAM,'NSOILCARB',I%O%NNSOILCARB,IRESP)
    IF(IVERSION>=7.AND.(I%O%LSPINUPCARBS.OR.I%O%LSPINUPCARBW))THEN
      CALL READ_SURF(HPROGRAM,'NBYEARSOLD',I%O%NNBYEARSOLD,IRESP)
    ELSE
      I%O%NNBYEARSOLD=NUNDEF
    ENDIF
  ENDIF
!
  CALL END_IO_SURF_n(HPROGRAM)
...
SELECT CASE (HINIT)
  CASE ('PGD')
    I%I%TTIME%TDATE%YEAR = NUNDEF
    I%I%TTIME%TDATE%MONTH= NUNDEF
    I%I%TTIME%TDATE%DAY  = NUNDEF
    I%I%TTIME%TIME       = XUNDEF

  CASE ('PRE')
    CALL PREP_CTRL_ISBA(DGI%O,DGI%DE%LSURF_EVAP_BUDGET,DGI%DM%LSURF_MISC_BUDGET,DGI%DM%LSURF_MISC_DIF,ILUOUT)    
    IF (LNAM_READ) CALL READ_NAM_PREP_ISBA_n(HPROGRAM)                        
    CALL READ_ISBA_DATE(&
                        HPROGRAM,HINIT,ILUOUT,HATMFILE,HATMFILETYPE,KYEAR,KMONTH,KDAY,PTIME,I%I%TTIME)

  CASE DEFAULT
CALL INIT_IO_SURF_n(DTCO, U, HPROGRAM,'NATURE','ISBA  ','READ ')
    CALL READ_SURF(HPROGRAM,'DTCUR',I%I%TTIME,IRESP)
    CALL END_IO_SURF_n(HPROGRAM)
END SELECT
!
!-----------------------------------------------------------------------------------------------------
...
!               ---------------------------------
!
 CALL READ_PGD_ISBA_n(CHI, DTCO, DTI, DTZ, GB, IG, ISS, I%O, I%P, &
...
ENDIF
!
IF (HINIT=='PRE' .AND. I%R%TSNOW%SCHEME.NE.'3-L' .AND. I%R%TSNOW%SCHEME.NE.'CRO' .AND. I%O%CISBA=='DIF') THEN
    CALL ABOR1_SFX("INIT_ISBAN: WITH CISBA = DIF, CSNOW MUST BE 3-L OR CRO")
ENDIF
IF ( I%O%CPHOTO/='LAI' .AND. I%O%CPHOTO/='LST' .AND. I%O%CPHOTO/='NIT' .AND. I%O%CPHOTO/='NCB' .AND. LAGRIP) THEN
  CALL ABOR1_SFX('INIT_ISBAN: INCONSISTENCY BETWEEN CPHOTO AND LAGRIP')
ENDIF
IF ( I%O%CPHOTO/='NCB' .AND. I%O%CRESPSL=='CNT') THEN
  CALL ABOR1_SFX('INIT_ISBAN: INCONSISTENCY BETWEEN CPHOTO AND CRESPSL')
ENDIF
IF (HINIT=='PRE' .AND. ISIZE_LMEB_PATCH>0 .AND. I%R%TSNOW%SCHEME.NE.'3-L' .AND. I%R%TSNOW%SCHEME.NE.'CRO') THEN
    CALL ABOR1_SFX("INIT_ISBAN: WITH LMEB_PATCH = TRUE, CSNOW MUST BE 3-L OR CRO")
ENDIF
IF(I%O%CPHOTO/='NCB'.AND.I%O%LSPINUPCARBW)THEN
  CALL ABOR1_SFX('INIT_ISBAN: INCONSISTENCY BETWEEN CPHOTO AND LSPINUPCARBW (if not NCB must be false)')
ENDIF
IF(I%O%CRESPSL/='CNT'.AND.I%O%LSPINUPCARBS)THEN
  CALL ABOR1_SFX('INIT_ISBAN: INCONSISTENCY BETWEEN CRESPSL AND LSPINUPCARBS (if not CNT must be false)')
ENDIF
IF(I%O%LSPINUPCARBW.AND.REAL(I%O%NNBYEARSPINW)>REAL(I%O%NNBYEARSPINS)*0.5)THEN
  WRITE(ILUOUT,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
...
ENDIF
IF(I%O%LSPINUPCARBS.AND.(I%O%XCO2_START==XUNDEF.OR.I%O%XCO2_END==XUNDEF))THEN
  WRITE(ILUOUT,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
...
!
ISPINEND=I%O%NNBYEARSPINS-NINT(I%O%NNBYEARSPINS*XSPIN_CO2)
!
I%O%LAGRI_TO_GRASS = .FALSE.
!
IF ( I%O%LSPINUPCARBS .AND. (I%O%NNBYEARSOLD <= ISPINEND) ) THEN
!
   I%O%LAGRI_TO_GRASS = .TRUE.
!
   CALL INI_DATA_PARAM(DTCO%XDATA_VEGTYPE, PSURF=DTCO%XDATA_NATURE, PH_TREE=XDATA_H_TREE,PLAI=XDATA_LAI, &
                       ...
                       PSEFOLD_ST=XDATA_SEFOLD_ST, PDMAX_ST=XDATA_DMAX_ST, OAGRI_TO_GRASS=I%O%LAGRI_TO_GRASS)
!
   ZCO2(:) = PRHOA(:) * I%O%XCO2_START * 1.E-6 * XMCO2 / XMD
!
ELSEIF(I%O%LSPINUPCARBS .AND. (I%O%NNBYEARSOLD > ISPINEND) .AND. (I%O%NNBYEARSOLD <= I%O%NNBYEARSPINS) )THEN
!
   ZSPINCO2 = I%O%XCO2_START + (I%O%XCO2_END-I%O%XCO2_START) * REAL(I%O%NNBYEARSOLD - ISPINEND) / &
                REAL(I%O%NNBYEARSPINS - ISPINEND)
!
...
END IF
!
CALL COMPUTE_ISBA_PARAMETERS(DTCO, OREAD_BUDGETC, UG, U, AG, CHI, DTI, DGI, GB,   &
                             ICP, ISS, IG, I, DST, SLT, SV,             &
                             HPROGRAM,HINIT,OLAND_USE,                  &
                             KI,KSV,KSW, HSV,ZCO2,PRHOA,                &
                             PZENITH,PSW_BANDS,PDIR_ALB,PSCA_ALB,       &
                             PEMIS,PTSRAD,PTSURF,HTEST                  )
!
IF ( I%O%CSNOWMETAMO/="B92" ) THEN
  CALL READ_FZ06('drdt_bst_fit_60.nc')
ENDIF
!
IF ( I%O%CSNOWRAD=="TAR" .OR. I%O%CSNOWRAD=="TA1" .OR.  I%O%CSNOWRAD=="TA2" ) THEN
  CALL INIT_TARTES()
END IF
!
IF (LHOOK) CALL DR_HOOK('INIT_ISBA_N',1,ZHOOK_HANDLE)
