!#############################################################
SUBROUTINE INIT_ISBA_n (DTCO, OREAD_BUDGETC, UG, U, USS, IM, DTZ, NDST, SLT, SV, &
...
!               --------
 ...
 CALL DEFAULT_ISBA(IM%O%XTSTEP, IM%O%XOUT_TSTEP,                                 &
                   IM%O%CROUGH,IM%O%CRUNOFF,IM%O%CALBEDO,IM%O%CSCOND,              &
...
SELECT CASE (HINIT)
  ...
  CASE ('PRE')
    CALL PREP_CTRL_ISBA(IM%ID%O,IM%ID%DE%LSURF_EVAP_BUDGET,IM%ID%DM%LSURF_MISC_BUDGET,IM%ID%DM%LSURF_MISC_DIF,ILUOUT)    
...
!               ---------------------------------
!
 CALL READ_PGD_ISBA_n(IM%CHI, DTCO, IM%DTI, DTZ, IM%GB, IM%G, IM%ISS, IM%O, IM%S, IM%K, &
...
ENDIF
!
IF (HINIT=='PRE' .AND. IM%NPE%AL(1)%TSNOW%SCHEME.NE.'3-L' .AND. IM%NPE%AL(1)%TSNOW%SCHEME.NE.'CRO' .AND. IM%O%CISBA=='DIF') THEN
    CALL ABOR1_SFX("INIT_ISBAN: WITH CISBA = DIF, CSNOW MUST BE 3-L OR CRO")
ENDIF
IF ( IM%O%CPHOTO/='LAI' .AND. IM%O%CPHOTO/='LST' .AND. IM%O%CPHOTO/='NIT' .AND. IM%O%CPHOTO/='NCB' .AND. LAGRIP) THEN
...
ENDIF
IF (HINIT=='PRE' .AND. ISIZE_LMEB_PATCH>0 .AND. IM%NPE%AL(1)%TSNOW%SCHEME.NE.'3-L' .AND. IM%NPE%AL(1)%TSNOW%SCHEME.NE.'CRO') THEN
...
END IF
!
CALL COMPUTE_ISBA_PARAMETERS(DTCO, OREAD_BUDGETC, UG, U, IM, NDST, SLT, SV, &
