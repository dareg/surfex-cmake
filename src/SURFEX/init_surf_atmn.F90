...
SELECT CASE (HINIT)
  ...
  CASE DEFAULT
    ...
    CALL READ_SURF(HPROGRAM,'DTCUR',U%TTIME,IRESP)
...
!         reading
!
 CALL READ_SURF(HPROGRAM,'SEA   ',U%CSEA   ,IRESP)
 CALL READ_SURF(HPROGRAM,'WATER ',U%CWATER ,IRESP)
 CALL READ_SURF(HPROGRAM,'NATURE',U%CNATURE,IRESP)
 CALL READ_SURF(HPROGRAM,'TOWN  ',U%CTOWN  ,IRESP)
!
 CALL READ_SURF(HPROGRAM,'DIM_FULL  ',U%NDIM_FULL,  IRESP)
IF (HINIT=='PRE') THEN
  ...
  ENDIF
ENDIF  
 CALL READ_SURF(HPROGRAM,'DIM_SEA   ',U%NDIM_SEA,   IRESP)
 CALL READ_SURF(HPROGRAM,'DIM_NATURE',U%NDIM_NATURE,IRESP)
 CALL READ_SURF(HPROGRAM,'DIM_WATER ',U%NDIM_WATER, IRESP)
 CALL READ_SURF(HPROGRAM,'DIM_TOWN  ',U%NDIM_TOWN,  IRESP)
...
IF (HPROGRAM/='AROME '.AND.NRANK==NPIO) THEN
  ...
  IF (.NOT.ASSOCIATED(UG%XGRID_FULL_PAR)) THEN
    CALL READ_GRIDTYPE(HPROGRAM,UG%CGRID,UG%NGRID_PAR,U%NDIM_FULL,.FALSE.,HDIR='A')
    ALLOCATE(UG%XGRID_FULL_PAR(UG%NGRID_PAR))
    CALL READ_GRIDTYPE(HPROGRAM,UG%CGRID,UG%NGRID_PAR,U%NDIM_FULL,.TRUE.,&
...
!
 CALL READ_SURF(HPROGRAM,'CH_EMIS',CHU%LCH_EMIS,IRESP)
!
IF (CHU%LCH_EMIS) THEN
  ...
  IF ( IVERSION<7 .OR. IVERSION==7 .AND. IBUGFIX<3 ) THEN
    CHU%CCH_EMIS='AGGR'
  ELSE
    CALL READ_SURF(HPROGRAM,'CH_EMIS_OPT',CHU%CCH_EMIS,IRESP)
  END IF
  !
  IF (SV%NBEQ > 0) THEN
    !
...
! initialization
IF (U%NDIM_TOWN>0) &
  CALL INIT_TOWN_n(HPROGRAM,HINIT,U%NSIZE_TOWN,KSV,KSW,                 &
                   HSV,ZP_CO2,ZP_RHOA,                                &
                   ZP_ZENITH,ZP_AZIM,PSW_BANDS,ZP_DIR_ALB,ZP_SCA_ALB, &
                   ZP_EMIS,ZP_TSRAD,ZP_TSURF,                         &
                   KYEAR,KMONTH,KDAY,PTIME, HATMFILE,HATMFILETYPE,    &
