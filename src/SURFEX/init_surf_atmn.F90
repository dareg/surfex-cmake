...
IF (LNAM_READ) THEN
 ...
 CALL DEFAULT_DIAG_SURF_ATM(YSC%DUO%N2M, YSC%DUO%LT2MMW, YSC%DUO%LSURF_BUDGET,&
                            YSC%DUO%L2M_MIN_ZS, YSC%DUO%LHU2M_QSAT, &
                            YSC%DUO%LRAD_BUDGET, YSC%DUO%LCOEF,&
...
!
#ifdef SFX_MPI
IF (LSFX_MPI) XTIME_INIT_TOWN = XTIME_INIT_TOWN + (MPI_WTIME() - XTIME0)*100./MAX(1,YSC%U%NSIZE_TOWN)
#endif
!
!
