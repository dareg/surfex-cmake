!#############################################################
SUBROUTINE INIT_TEB_GARDEN_n (DTCO, UG, U, DMTO, TOP, IO, DTI, K, P, PEK, &
                              GDDG, HPROGRAM, HINIT, KI, KSW, PSW_BANDS, KPATCH)
!#############################################################
!
...
!
 CALL AVG_ALBEDO_EMIS_TEB_VEG(PEK, IO%CALBEDO,  ZTG1, PSW_BANDS, &
                              ZDIR_ALB, ZSCA_ALB, ZEMIS,ZTSRAD      )  
!
 CALL DIAG_TEB_VEG_INIT_n(GDDG%D, GDDG%DP, GDDG%DE, GDDG%DEP, GDDG%DM, KI, PEK%TSNOW%NLAYER)
!
!-------------------------------------------------------------------------------
!
IF (LHOOK) CALL DR_HOOK('INIT_TEB_GARDEN_N',1,ZHOOK_HANDLE)
