...
!
 ...
 !
P%NSIZE_P = KI
 CALL INIT_VEG_n(IO, K, P, PEK, DMTO%LSURF_DIAG_ALBEDO, ZDIR_ALB, ZSCA_ALB, ZEMIS, ZTSRAD )
!
ZWG1(:) = PEK%XWG(:,1)
ZTG1(:) = PEK%XTG(:,1)
!
...
ELSE
  ...
  ELSE
    IDECADE = 1
  END IF
  CALL INIT_FROM_DATA_TEB_VEG_n(DTV, K, P, PEK, IDECADE,IO%CPHOTO, .FALSE., .FALSE., .FALSE., .TRUE. )   
END IF
!
