!#############################################################
SUBROUTINE INIT_TEB_GREENROOF_n (DTCO, U, DMTO, TOP, IO, DTI, K, P, PEK, &
                                 GRDG, HPROGRAM,HINIT,KI,KSW,PSW_BANDS,KPATCH)
!#############################################################
!
...
!
   IF (PEK%TSNOW%SCHEME.NE.'3-L' .AND. PEK%TSNOW%SCHEME.NE.'CRO' &
           .AND. IO%CISBA=='DIF') THEN
...
!
  CALL SET_ROUGH(TOP%LCANOPY,IO%CROUGH)
 !
  P%NSIZE_P = KI
  CALL INIT_VEG_n(IO, K, P, PEK, DMTO%LSURF_DIAG_ALBEDO, &
...
ELSE
  ...
  ELSE
    IDECADE = 1
  END IF
  CALL INIT_FROM_DATA_TEB_VEG_1P_n(DTI, K, P, PEK, IDECADE,IO%CPHOTO, .FALSE., .FALSE., .FALSE., .TRUE. )   
END IF
!
...
ENDWHERE  
!
 CALL AVG_ALBEDO_EMIS_TEB_VEG(PEK, IO%CALBEDO,  ZTG1, PSW_BANDS, &
                              ZDIR_ALB, ZSCA_ALB, ZEMIS,ZTSRAD      )  
!
 CALL DIAG_TEB_VEG_INIT_n(GRDG%D, GRDG%DP, GRDG%DE, GRDG%DEP, GRDG%DM, &
                                KI, PEK%TSNOW%NLAYER)
!-------------------------------------------------------------------------------
!
IF (LHOOK) CALL DR_HOOK('INIT_TEB_GREENROOF_N',1,ZHOOK_HANDLE)
