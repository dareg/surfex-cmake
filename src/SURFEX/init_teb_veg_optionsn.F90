!#############################################################
SUBROUTINE INIT_TEB_VEG_OPTIONS_n (CHT, OSURF_DIAG_ALBEDO, TVO, HPROGRAM)
!#############################################################
!
...
!    
 CALL READ_DEFAULT_TEB_VEG_n(CHT, TVO, HPROGRAM)
!
 CALL READ_TEB_VEG_CONF_n(CHT, TVO, HPROGRAM)
!
!-------------------------------------------------------------------------------
TVO%NPATCH = 1
TVO%CRESPSL = 'DEF'
...
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) YRECFM='GD_ISBA'
 CALL READ_SURF(HPROGRAM,YRECFM,TVO%CISBA,IRESP)
!
IF (IVERSION>=7) THEN
  !
  YRECFM='TWN_PEDOTF'
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) YRECFM='GD_PEDOTF'
  CALL READ_SURF(HPROGRAM,YRECFM,TVO%CPEDOTF,IRESP)
  !
ELSE
  TVO%CPEDOTF = 'CH78'
ENDIF
...
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) YRECFM='GD_PHOTO'
 CALL READ_SURF(HPROGRAM,YRECFM,TVO%CPHOTO,IRESP)
!
YRECFM='TWN_LAYER'
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) YRECFM='GD_LAYER'
 CALL READ_SURF(HPROGRAM,YRECFM,TVO%NGROUND_LAYER,IRESP)
!
...
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=2) THEN
  ...
  CALL READ_SURF(HPROGRAM,YRECFM,TVO%LTR_ML,IRESP)
  !
ELSE 
  TVO%LTR_ML = .FALSE.
ENDIF
...
IF(TVO%CISBA=='DIF') THEN
  ...
  IF (IVERSION>=8) THEN
     DO JLAYER=1,TVO%NGROUND_LAYER
        ...
        CALL READ_SURF(HPROGRAM,YRECFM,TVO%XSOILGRID(JLAYER),IRESP)
     ENDDO
  ELSEIF (IVERSION==7 .AND. IBUGFIX>=2) THEN
    YRECFM='TWN_SOILGRID'
    IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) YRECFM='GD_SOILGRID'
    CALL READ_SURF(HPROGRAM,YRECFM,TVO%XSOILGRID,IRESP,HDIR='-')
  ELSE
