...
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) YRECFM='GD_ISBA'
 CALL READ_SURF(HPROGRAM,YRECFM,TVG%CISBA,IRESP)
!
IF (IVERSION>=7) THEN
  !
  YRECFM='TWN_PEDOTF'
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) YRECFM='GD_PEDOTF'
  CALL READ_SURF(HPROGRAM,YRECFM,TVG%CPEDOTF,IRESP)
  !
ELSE
  TVG%CPEDOTF = 'CH78'
ENDIF
...
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) YRECFM='GD_PHOTO'
 CALL READ_SURF(HPROGRAM,YRECFM,TVG%CPHOTO,IRESP)
!
YRECFM='TWN_LAYER'
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) YRECFM='GD_LAYER'
 CALL READ_SURF(HPROGRAM,YRECFM,TGDO%NGROUND_LAYER,IRESP)
!
...
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=2) THEN
  ...
  CALL READ_SURF(HPROGRAM,YRECFM,TVG%LTR_ML,IRESP)
  !
ELSE 
  TVG%LTR_ML = .FALSE.
ENDIF
...
IF(TVG%CISBA=='DIF') THEN
  ...
  IF (IVERSION>=8) THEN
     DO JLAYER=1,TGDO%NGROUND_LAYER
        ...
        CALL READ_SURF(HPROGRAM,YRECFM,TGDO%XSOILGRID(JLAYER),IRESP)
     ENDDO
  ELSEIF (IVERSION==7 .AND. IBUGFIX>=2) THEN
    YRECFM='TWN_SOILGRID'
    IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) YRECFM='GD_SOILGRID'
    CALL READ_SURF(HPROGRAM,YRECFM,TGDO%XSOILGRID,IRESP,HDIR='-')
  ELSE
