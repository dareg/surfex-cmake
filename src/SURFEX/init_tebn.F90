...
IF (LNAM_READ) THEN
 ...
 !  
  DO JP=1,TOP%NTEB_PATCH
    CALL GOTO_WRAPPER_TEB_PATCH(JP, NT=NT) 
    CALL DEFAULT_TEB(TOP%CZ0H, TOP%XTSTEP, TOP%XOUT_TSTEP, TOP%CCH_BEM, &
                     NT%CUR%XDT_RES, NT%CUR%XDT_OFF)
  ENDDO
...
!    
 CALL READ_DEFAULT_TEB_n(CHT, TD%MTO, TD%O, TD%DUT, GRM%O, NT, TOP, HPROGRAM)
!
!*       1.     Reading of configuration:
!               -------------------------
!
 CALL READ_TEB_CONF_n(CHT, TD%MTO, TD%O, TD%DUT, NT, TOP, HPROGRAM)
!
...
IF (HINIT=='PRE') THEN
  DO JP=1,TOP%NTEB_PATCH
    CALL GOTO_WRAPPER_TEB_PATCH(JP, NT=NT)
    CALL READ_PREP_TEB_SNOW(HPROGRAM, NT%CUR%TSNOW_ROOF%SCHEME, NT%CUR%TSNOW_ROOF%NLAYER, &
                                      NT%CUR%TSNOW_ROAD%SCHEME, NT%CUR%TSNOW_ROAD%NLAYER)
  END DO
ENDIF
!
...
DO JP=1,TOP%NTEB_PATCH
  !
  CALL GOTO_WRAPPER_TEB_PATCH(JP, B=B, NT=NT)
  !-----------------------------------------------------------------------------------
  !
  !*       3.     Physiographic data fields from land cover:
  !               -----------------------------------------
  !
  ALLOCATE(NT%CUR%XZ0_TOWN     (ILU))
  ALLOCATE(NT%CUR%XALB_ROOF    (ILU))
  ALLOCATE(NT%CUR%XEMIS_ROOF   (ILU))
  ALLOCATE(NT%CUR%XALB_ROAD    (ILU))
  ALLOCATE(NT%CUR%XEMIS_ROAD   (ILU))
  ALLOCATE(NT%CUR%XALB_WALL    (ILU))
  ALLOCATE(NT%CUR%XEMIS_WALL   (ILU))
  ALLOCATE(NT%CUR%XBLD         (ILU))
  ALLOCATE(NT%CUR%XROAD_DIR    (ILU))
  ALLOCATE(NT%CUR%XROAD        (ILU))
  ALLOCATE(NT%CUR%XBLD_HEIGHT  (ILU))
  ALLOCATE(NT%CUR%XWALL_O_HOR  (ILU))
  ALLOCATE(NT%CUR%XCAN_HW_RATIO(ILU))
  ALLOCATE(NT%CUR%XROAD_O_GRND (ILU))
  ALLOCATE(NT%CUR%XGARDEN_O_GRND(ILU))
  ALLOCATE(NT%CUR%XWALL_O_GRND (ILU))
  ALLOCATE(NT%CUR%XWALL_O_BLD(  ILU))
  ALLOCATE(NT%CUR%XH_TRAFFIC   (ILU))
  ALLOCATE(NT%CUR%XLE_TRAFFIC  (ILU))
  ALLOCATE(NT%CUR%XH_INDUSTRY  (ILU))
  ALLOCATE(NT%CUR%XLE_INDUSTRY (ILU))
  ALLOCATE(NT%CUR%XHC_ROOF     (ILU,TOP%NROOF_LAYER))
  ALLOCATE(NT%CUR%XTC_ROOF     (ILU,TOP%NROOF_LAYER))
  ALLOCATE(NT%CUR%XD_ROOF      (ILU,TOP%NROOF_LAYER))
  ALLOCATE(NT%CUR%XHC_ROAD     (ILU,TOP%NROAD_LAYER))
  ALLOCATE(NT%CUR%XTC_ROAD     (ILU,TOP%NROAD_LAYER))
  ALLOCATE(NT%CUR%XD_ROAD      (ILU,TOP%NROAD_LAYER))
  ALLOCATE(NT%CUR%XHC_WALL     (ILU,TOP%NWALL_LAYER))
  ALLOCATE(NT%CUR%XTC_WALL     (ILU,TOP%NWALL_LAYER))
  ALLOCATE(NT%CUR%XD_WALL      (ILU,TOP%NWALL_LAYER))
  ALLOCATE(NT%CUR%XROUGH_ROOF  (ILU))
  ALLOCATE(NT%CUR%XROUGH_WALL  (ILU))
  ALLOCATE(NT%CUR%XRESIDENTIAL (ILU))
  ALLOCATE(NT%CUR%XGREENROOF   (ILU))
  ALLOCATE(NT%CUR%XGARDEN      (ILU))
  ...
  END IF
  !
  CALL CONVERT_PATCH_TEB(BDD, DTB, DTCO, DTT, TOP, ZDEF_ROAD_DIR, T=NT%CUR, TPN=TPN  )
  !
  IF (.NOT. TOP%LGREENROOF .AND. MAXVAL(NT%CUR%XGREENROOF)>0. ) THEN !<== A paralleliser pour un stop propre
  ...
  !               ----------------
  !
  ALLOCATE(NT%CUR%XSVF_ROAD  (ILU))
  ALLOCATE(NT%CUR%XSVF_GARDEN(ILU))
  ALLOCATE(NT%CUR%XSVF_WALL  (ILU))
  !
  ...
  !
  !
  CALL TEB_MORPHO(HPROGRAM, NT%CUR  )
                !
  !-------------------------------------------------------------------------------
  !
  ...
  !               ---------------------
  !
  CALL INIT_BEM_n(DTCO, TOP, BOP, DTT, DTB, BDD, TG, NT%CUR, B%CUR, ILUOUT)
  !
  !-------------------------------------------------------------------------------
  !
  ...
  !
    ...
    CALL INIT_TEB_GARDEN_PGD_n(DTCO, U, CHT%LCH_BIO_FLUX, TG, NT%CUR%XGARDEN, TOP, &
    ...
    IF (TOP%LGREENROOF) THEN
      CALL INIT_TEB_GREENROOF_PGD_n(DTCO, U, CHT%LCH_BIO_FLUX, TG, NT%CUR%XGREENROOF, TOP, &
...
!
!* TEB fields
  CALL READ_TEB_n(B%CUR, BOP, DTCO, U, NT%CUR, TOP, TPN, HPROGRAM,JP)
!
  ALLOCATE(NT%CUR%XAC_ROOF    (ILU))
  ALLOCATE(NT%CUR%XAC_ROAD    (ILU))
  ALLOCATE(NT%CUR%XAC_WALL    (ILU))
  ALLOCATE(NT%CUR%XAC_TOP     (ILU))
  ALLOCATE(NT%CUR%XAC_ROOF_WAT(ILU))
  ALLOCATE(NT%CUR%XAC_ROAD_WAT(ILU))
  ALLOCATE(NT%CUR%XQSAT_ROOF  (ILU))
  ALLOCATE(NT%CUR%XQSAT_ROAD  (ILU))
  ALLOCATE(NT%CUR%XDELT_ROOF  (ILU))
  ALLOCATE(NT%CUR%XDELT_ROAD  (ILU))
!
...
!
  CALL INIT_SNOW_LW(XEMISSN,NT%CUR%TSNOW_ROOF)
  CALL INIT_SNOW_LW(XEMISSN,NT%CUR%TSNOW_ROAD)
!
  IF (TOP%LGARDEN) THEN
    ZDIR_SW=0. ! night as first guess for albedo computation
    ZSCA_SW=0. !
    CALL TEB_VEG_PROPERTIES(NT%CUR%XGARDEN, GDM%O, GDM%NPE%AL(JP), &
  ...
  IF (TOP%LGREENROOF) THEN
    ZDIR_SW=0. ! night as first guess for albedo computation
    ZSCA_SW=0. !
    CALL TEB_VEG_PROPERTIES(NT%CUR%XGREENROOF, GRM%O, GRM%NPE%AL(JP),         & 
                              ZDIR_SW, ZSCA_SW, PSW_BANDS, KSW,              &
...
!
  CALL AVERAGED_TSRAD_TEB(NT%CUR, B%CUR, ZEMIS_GARDEN, ZTS_GARDEN,  &
...
!
  CALL AVERAGED_ALBEDO_TEB(TOP,NT%CUR,TPN,B%CUR,PZENITH,PAZIM,   &
...
!
DO JP=1,TOP%NTEB_PATCH
    CALL GOTO_WRAPPER_TEB_PATCH(JP, DMTC=TD%DMTC, DMT=TD%DMT)
  CALL DIAG_MISC_TEB_INIT_n(TD%DMTC%CUR, TD%DMT%CUR, TD%MTO, TOP, ILU, ISWB)
END DO ! end of loop on patches
!
!-------------------------------------------------------------------------------
!
