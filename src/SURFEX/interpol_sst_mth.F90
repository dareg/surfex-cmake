...
!              ------------
!
USE MODD_SEAFLUX_n,  ONLY : XSST_MTH, XSSS_MTH, XSIC_MTH, XSIT_MTH, &
                CINTERPOL_SST, CINTERPOL_SSS, CINTERPOL_SIC, CINTERPOL_SIT
!
...
!
IF (((HFLAG=='T').AND.(TRIM(CINTERPOL_SST)=='MONTH')) .OR. &
    ((HFLAG=='S').AND.(TRIM(CINTERPOL_SSS)=='MONTH')) .OR. &
    ((HFLAG=='H').AND.(TRIM(CINTERPOL_SIT)=='MONTH')) .OR. &
    ((HFLAG=='C').AND.(TRIM(CINTERPOL_SIC)=='MONTH'))) THEN
   IMTH1=1
   IMTH2=2
   IMTH3=3
ELSE
...
IF (HFLAG .EQ. 'T' ) THEN 
   CALL INTERPOL_QUADRA(ZDAT,ZNDAT,XSST_MTH(:,IMTH1),XSST_MTH(:,IMTH2),XSST_MTH(:,IMTH3),ZFIELD)
   POUT(:) = ZFIELD(:)
ELSEIF (HFLAG .EQ. 'S' ) THEN 
   CALL INTERPOL_QUADRA(ZDAT,ZNDAT,XSSS_MTH(:,IMTH1),XSSS_MTH(:,IMTH2),XSSS_MTH(:,IMTH3),ZFIELD)
   POUT(:) = MAX(0.0,ZFIELD(:))
ELSEIF (HFLAG .EQ. 'H' ) THEN 
   CALL INTERPOL_QUADRA(ZDAT,ZNDAT,XSIT_MTH(:,IMTH1),XSIT_MTH(:,IMTH2),XSIT_MTH(:,IMTH3),ZFIELD)
   POUT(:) = MAX(0.0,ZFIELD(:))
ELSE ! SIC
   CALL INTERPOL_QUADRA(ZDAT,ZNDAT,XSIC_MTH(:,IMTH1),XSIC_MTH(:,IMTH2),XSIC_MTH(:,IMTH3),ZFIELD)
