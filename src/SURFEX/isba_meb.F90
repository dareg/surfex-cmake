...
!  
 CALL SNOW3L_ISBA(IO, G, PK, PEK, DK, DEK, DMK, OMEB, HIMPLICIT_WIND,       &
                  ...
                  PPET_B_COEF, PPEQ_B_COEF, PSNOW_THRUFAL, PGRNDFLUX,       &
...
INLL = INL
IF(IO%LMEB_LITTER .AND. MINVAL(PEK%XGNDLITTER(:))>0.0)INLL = INL + 1
...
IF (OMEB_LITTER .AND. MINVAL(PEK%XGNDLITTER(:))>0.0)THEN
