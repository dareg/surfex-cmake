...
!
    DO JPATCH=1,I%O%NPATCH
      ...
      DO JL=1,I%O%NLAYER_DUN
        DO JJ=1,INI
          ZD_TOP    (JJ) = ZD_TOP    (JJ) + I%IP%XPATCH(JJ,JPATCH)*I%IP%XSOILWGHT(JJ,JL,JPATCH)
          ZWSAT_AVG (JJ) = ZWSAT_AVG (JJ) + I%IP%XPATCH(JJ,JPATCH)*I%IP%XSOILWGHT(JJ,JL,JPATCH)*I%IP%XWSAT(JJ,JL)
          ZWWILT_AVG(JJ) = ZWWILT_AVG(JJ) + I%IP%XPATCH(JJ,JPATCH)*I%IP%XSOILWGHT(JJ,JL,JPATCH)*I%IP%XWD0 (JJ,JL)
          ZW_TOP    (JJ) = ZW_TOP    (JJ) + I%IP%XPATCH(JJ,JPATCH)*I%IP%XSOILWGHT(JJ,JL,JPATCH)*I%R%XWG(JJ,JL,JPATCH)
        ENDDO
      ENDDO
      ENDIF
    ENDDO
...
!    
    DO JPATCH=1,I%O%NPATCH
      IF (I%IP%NSIZE_NATURE_P(JPATCH)>0 )THEN
        DO JJ=1,INI
          ZD_TOP(JJ) = ZD_TOP(JJ)+I%IP%XRUNOFFD(JJ,JPATCH)*I%IP%XPATCH(JJ,JPATCH)
          ZW_TOP(JJ) = ZW_TOP(JJ)+I%IP%XRUNOFFD(JJ,JPATCH)*I%IP%XPATCH(JJ,JPATCH)*I%R%XWG(JJ,2,JPATCH)
        ENDDO
      ENDIF
    ENDDO
!  
    ...
    ENDWHERE
!      
    ZWSAT_AVG (:) = I%IP%XWSAT(:,1)
    ZWWILT_AVG(:) = I%IP%XWD0 (:,1)
!
  ENDIF
!
! Find the boundary
...
!
  ...
  ICOUNT=0
  DO JJ=1,INI  
     IF((I%P%XTI_MEAN(JJ)/=XUNDEF.AND.ZW_TOP(JJ)<ZWSAT_AVG(JJ).AND.ZW_TOP(JJ)>ZWWILT_AVG(JJ)))THEN     
       ICOUNT=ICOUNT+1
       NMASK(ICOUNT)=JJ       
     ENDIF
...
!
  DO JTAB=1,NDIMTAB
     DO JJ=1,ICOUNT
        JI = NMASK(JJ)    
        IF(I%I%XTAB_WTOP(JI,JTAB)>ZW_TOP(JI))THEN
          IUP(JJ)=JTAB
          IDOWN(JJ)=JTAB+1
        ELSEIF(I%I%XTAB_WTOP(JI,JTAB)==ZW_TOP(JI))THEN
          IUP(JJ)=JTAB
          IDOWN(JJ)=JTAB
        ENDIF
     ENDDO    
...
!     
!    new range
     ZF_UP   = I%I%XTAB_FSAT(JI,IUP  (JJ))
     ZF_DOWN = I%I%XTAB_FSAT(JI,IDOWN(JJ))
     ZQ_UP   = I%I%XTAB_QTOP(JI,IUP  (JJ))
     ZQ_DOWN = I%I%XTAB_QTOP(JI,IDOWN(JJ))     
     ZW_UP   = I%I%XTAB_WTOP(JI,IUP  (JJ))
     ZW_DOWN = I%I%XTAB_WTOP(JI,IDOWN(JJ))
!     
!    Calculate new FSAT
     ZSLOPEF = 0.0
     ZSLOPEQ = 0.0
...
!
    DO JPATCH=1,I%O%NPATCH
      IF(I%IP%NSIZE_NATURE_P(JPATCH)>0)THEN
        DO JL=1,I%O%NLAYER_DUN
           DO JJ=1,INI
            PTOPQS(JJ,JL,JPATCH)=I%IP%XKANISO(JJ,JL)*I%IP%XCONDSAT(JJ,1,JPATCH)*ZQTOP(JJ)*&
                    I%IP%XSOILWGHT(JJ,JL,JPATCH)/I%IP%XRUNOFFD(JJ,JPATCH)
           ENDDO
        ENDDO
      ENDIF
    ENDDO
