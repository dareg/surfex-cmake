!     #########################
SUBROUTINE ISBA_SOC_PARAMETERS (HRUNOFF,PDG,PSOC,IP,PFRACSOC     )
!     ########################################################################
!
!!****  *ISBA_SOC_PARAMETERS*  
!!
...
DO JP=1,INP
  DO JI=1,INI
     ZMASK(JI)=ZMASK(JI)+IP%XPATCH(JI,JP)
  ENDDO
ENDDO
!  
DO JL=1,INL
   DO JI=1,INI
     IF(ZMASK(JI)>0.0)THEN
       ZDG_SOIL (JI,JL)=SUM(PDG(JI,JL,:)*IP%XPATCH(JI,:),IP%XPATCH(JI,:)>0.0) &
                       /SUM(IP%XPATCH(JI,:),IP%XPATCH(JI,:)>0.0)
     ENDIF
  ENDDO
ENDDO
!
...
DO JP=1,INP
  DO JL=1,INL
     DO JI=1,INI
        IF(IP%XPATCH(JI,JP)/=XUNDEF)THEN
...
!      New soil thermal properties      
       IP%XHCAPSOIL(JI,JL  ) = (1.0-PFRACSOC(JI,JL))*IP%XHCAPSOIL(JI,JL) + PFRACSOC(JI,JL)*XOMRHO*XOMSPH
       IP%XCONDDRY (JI,JL  ) = (IP%XCONDDRY(JI,JL)**(1.0-PFRACSOC(JI,JL))) * (XOMCONDDRY**PFRACSOC(JI,JL))
       IP%XCONDSLD (JI,JL  ) = (IP%XCONDSLD(JI,JL)**(1.0-PFRACSOC(JI,JL))) * (XOMCONDSLD**PFRACSOC(JI,JL))
!      New soil hydraulic properties
       IP%XBCOEF   (JI,JL  ) = (1.0-PFRACSOC(JI,JL))*IP%XBCOEF   (JI,JL) + PFRACSOC(JI,JL)*ZPEAT_BCOEF  (JI,JL)
       IP%XMPOTSAT (JI,JL  ) = (1.0-PFRACSOC(JI,JL))*IP%XMPOTSAT (JI,JL) + PFRACSOC(JI,JL)*ZPEAT_MPOTSAT(JI,JL)
       IP%XWSAT    (JI,JL  ) = (1.0-PFRACSOC(JI,JL))*IP%XWSAT    (JI,JL) + PFRACSOC(JI,JL)*ZPEAT_WSAT   (JI,JL)  
       IP%XWFC     (JI,JL  ) = (1.0-PFRACSOC(JI,JL))*IP%XWFC     (JI,JL) + PFRACSOC(JI,JL)*ZPEAT_WFC    (JI,JL)
       IP%XWWILT   (JI,JL  ) = (1.0-PFRACSOC(JI,JL))*IP%XWWILT   (JI,JL) + PFRACSOC(JI,JL)*ZPEAT_WWILT  (JI,JL)
       DO JP=1,INP
          IF(IP%XPATCH(JI,JP)/=XUNDEF)THEN
            IP%XCONDSAT (JI,JL,JP) = IP%XCONDSAT(JI,JL,JP)**(1.0-PFRACSOC(JI,JL))*ZPEAT_CONDSAT(JI,JL,JP)**PFRACSOC(JI,JL)
          ENDIF
       ENDDO
     ENDIF
   ENDDO   
...
IF(HRUNOFF=='SGH')THEN
  DO JL=1,INL
     DO JI=1,INI
       IF(ZMASK(JI)>0.0)THEN
         IP%XWD0  (JI,JL) = (1.0-PFRACSOC(JI,JL))*IP%XWD0  (JI,JL) + PFRACSOC(JI,JL)*ZPEAT_WD0  (JI,JL)
         IP%XKANISO(JI,JL) = (1.0-PFRACSOC(JI,JL))*IP%XKANISO(JI,JL) + PFRACSOC(JI,JL)*ZPEAT_ANISO(JI,JL)
       ENDIF
     ENDDO   
  ENDDO
ENDIF
