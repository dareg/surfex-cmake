...
IF (ODIM) THEN
  DO JP=1,KNPATCH
    ...
    !YS Comment next line, pb with patches and optional arguments
    !IF (IPATCH/=0) YRECFM=TRIM(YRECFM)//'P'//ADJUSTL(YPAT(:LEN_TRIM(YPAT)))
    PRINT*,'sous make_choice_array YRECFM=',YRECFM, KNPATCH,YPAT
   ! CALL READ_SURF(HPROGRAM,YRECFM,PWORK(:,JP),IRESP,HDIR=YDIR)
    CALL READ_SURF(HPROGRAM,YRECFM,PWORK(:,:),IRESP,HDIR=YDIR)
  ENDDO
ELSE
