...
IF (LKFROM) THEN
CH_FLAKE_MODEL(KFROM)%XDEP=>XDEP
CH_FLAKE_MODEL(KFROM)%CSV=>CSV
CH_FLAKE_MODEL(KFROM)%CCH_NAMES=>CCH_NAMES
CH_FLAKE_MODEL(KFROM)%CDSTNAMES=>CDSTNAMES
CH_FLAKE_MODEL(KFROM)%CSLTNAMES=>CSLTNAMES
CH_FLAKE_MODEL(KFROM)%CAER_NAMES=>CAER_NAMES
ENDIF

!
! Current model is set to model KTO
IF (LHOOK) CALL DR_HOOK('MODD_CH_FLAKE_N:CH_FLAKE_GOTO_MODEL',0,ZHOOK_HANDLE)
CCH_DRY_DEP=>CH_FLAKE_MODEL(KTO)%CCH_DRY_DEP
XDEP=>CH_FLAKE_MODEL(KTO)%XDEP
CSV=>CH_FLAKE_MODEL(KTO)%CSV
NSV_CHSBEG=>CH_FLAKE_MODEL(KTO)%NSV_CHSBEG
NSV_CHSEND=>CH_FLAKE_MODEL(KTO)%NSV_CHSEND
NBEQ=>CH_FLAKE_MODEL(KTO)%NBEQ
CCH_NAMES=>CH_FLAKE_MODEL(KTO)%CCH_NAMES
NSV_DSTBEG=>CH_FLAKE_MODEL(KTO)%NSV_DSTBEG
NSV_DSTEND=>CH_FLAKE_MODEL(KTO)%NSV_DSTEND
NDSTEQ=>CH_FLAKE_MODEL(KTO)%NDSTEQ
CDSTNAMES=>CH_FLAKE_MODEL(KTO)%CDSTNAMES
NSV_SLTBEG=>CH_FLAKE_MODEL(KTO)%NSV_SLTBEG
NSV_SLTEND=>CH_FLAKE_MODEL(KTO)%NSV_SLTEND
NSLTEQ=>CH_FLAKE_MODEL(KTO)%NSLTEQ
CSLTNAMES=>CH_FLAKE_MODEL(KTO)%CSLTNAMES
NSV_AERBEG=>CH_FLAKE_MODEL(KTO)%NSV_AERBEG
NSV_AEREND=>CH_FLAKE_MODEL(KTO)%NSV_AEREND
NAEREQ=>CH_FLAKE_MODEL(KTO)%NAEREQ
CAER_NAMES=>CH_FLAKE_MODEL(KTO)%CAER_NAMES
