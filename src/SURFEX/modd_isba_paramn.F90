...
!
TYPE ISBA_PARAM_PATCH_t
!
TYPE(ISBA_PARAM_t), ALLOCATABLE :: AL(:)
!
END TYPE ISBA_PARAM_PATCH_t
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!TYPE TEB_VEG_PARAM_TIME_t
!  !
!  TYPE(ISBA_PARAM_TIME_t), POINTER :: ALP(:) => NULL()
!  TYPE(ISBA_PARAM_TIME_t), POINTER :: CUR => NULL()
!  !
...
CONTAINS
!
SUBROUTINE ISBA_PARAM_PATCH_INIT(YISBA_PARAM_PATCH,KPATCH)
TYPE(ISBA_PARAM_PATCH_t), INTENT(INOUT) :: YISBA_PARAM_PATCH 
INTEGER, INTENT(IN) :: KPATCH
INTEGER :: JP
REAL(KIND=JPRB) :: ZHOOK_HANDLE
IF (LHOOK) CALL DR_HOOK("MODD_ISBA_PARAM_N:ISBA_PARAM_PATCH_INIT",0,ZHOOK_HANDLE)
 ALLOCATE(YISBA_PARAM_PATCH%AL(KPATCH))
DO JP=1,KPATCH
  CALL ISBA_PARAM_TIME_INIT(YISBA_PARAM_PATCH%AL(JP)%T)
  CALL ISBA_PARAM_INIT(YISBA_PARAM_PATCH%AL(JP)%X,YISBA_PARAM_PATCH%AL(JP)%M,&
                       YISBA_PARAM_PATCH%AL(JP)%A,YISBA_PARAM_PATCH%AL(JP)%I)
ENDDO         
IF (LHOOK) CALL DR_HOOK("MODD_ISBA_PARAM_N:ISBA_PARAM_PATCH_INIT",1,ZHOOK_HANDLE)
END SUBROUTINE ISBA_PARAM_PATCH_INIT
!
