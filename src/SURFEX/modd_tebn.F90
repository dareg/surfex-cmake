...
TYPE TEB_NP_t
  !
  TYPE(TEB_t), POINTER :: ALP(:) => NULL()
  TYPE(TEB_t), POINTER :: CUR => NULL()
  !
END TYPE TEB_NP_t
!
CONTAINS
!----------------------------------------------------------------------------
!
SUBROUTINE TEB_GOTO_PATCH(YNTEB,KTO_PATCH)
...
IF (.NOT.ASSOCIATED(YNTEB%ALP)) THEN
  ALLOCATE(YNTEB%ALP(KPATCH))
  YNTEB%CUR => YNTEB%ALP(1)
  DO JP=1,KPATCH
    CALL TEB_INIT(YNTEB%ALP(JP))
  ENDDO
ELSE
  DO JP=1,KPATCH
    CALL TEB_INIT(YNTEB%ALP(JP))
