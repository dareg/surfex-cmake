...
!
!     ###############################################################################
SUBROUTINE INIT_2M_10M( SB, DGS, PU, PV, PWIND, PRHOA )
!
USE MODD_CANOPY_n, ONLY : CANOPY_t
...
!
DGS%XT2M(:) = SB%XT(:,2)
DGS%XT2M_MIN(:) = MIN(DGS%XT2M(:),DGS%XT2M_MIN(:))
DGS%XT2M_MAX(:) = MAX(DGS%XT2M(:),DGS%XT2M_MAX(:))
DGS%XQ2M(:) = SB%XQ(:,2) / PRHOA(:)
DGS%XHU2M(:)= MIN( DGS%XQ2M(:) / QSAT(DGS%XT2M(:),SB%XP(:,2)) , 1.)
DGS%XHU2M_MIN(:) = MIN(DGS%XHU2M(:),DGS%XHU2M_MIN(:))
DGS%XHU2M_MAX(:) = MAX(DGS%XHU2M(:),DGS%XHU2M_MAX(:))
 CALL INTERPOL_SBL(SB%XZ(:,:),SB%XU(:,:),10.,ZU10(:))
DO JJ=1,SIZE(SB%XT(:,2))
  IF (ZU10(JJ)/=XUNDEF) THEN
    IF (PWIND(JJ)>0.) THEN
      DGS%XZON10M(JJ) = ZU10(JJ) * PU(JJ)/PWIND(JJ)
      DGS%XMER10M(JJ) = ZU10(JJ) * PV(JJ)/PWIND(JJ)
    ELSE
      DGS%XZON10M(JJ) = 0.
      DGS%XMER10M(JJ) = 0.
    END IF
     DGS%XWIND10M(JJ) = SQRT(DGS%XZON10M(JJ)**2+DGS%XMER10M(JJ)**2)
     DGS%XWIND10M_MAX(JJ) = MAX(DGS%XWIND10M(JJ),DGS%XWIND10M_MAX(JJ))
  ELSE
    DGS%XZON10M(JJ) = XUNDEF
    DGS%XMER10M(JJ) = XUNDEF
    DGS%XWIND10M(JJ) = XUNDEF
    DGS%XWIND10M_MAX(JJ) = XUNDEF
  END IF
END DO
!
