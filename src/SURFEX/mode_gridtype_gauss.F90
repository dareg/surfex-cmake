...
SUBROUTINE XY_GAUSS(PCODIL,KSIZE_DLAT,KSIZE_LON,PNODATA,PVALUE,PLAT_XY,PLON_XY)  
  ...
  REAL, DIMENSION(:), CONTIGUOUS, INTENT(IN) :: PVALUE  ! value of the point to add
  REAL, DIMENSION(:), CONTIGUOUS, INTENT(OUT):: PLAT_XY,PLON_XY 
  !
  !*     0.2    Declarations of local variables
  ! 
  REAL :: ZCOS1, ZV1, ZINVS, ZLAT1, ZCOS2, ZLON1, ZINVC, ZSINN
  REAL :: ZV2,  ZSINT, ZCOST, ZV3, ZLAT2, ZM, ZLON2, ZD
  !
  INTEGER :: JLAT, JLON, IOFF
  !
  REAL(KIND=JPHOOK) :: ZHOOK_HANDLE
  !--------------------------------------------------------------------------------
  !
  ...
  !
  !
!$OMP PARALLEL PRIVATE(IOFF, JLON, ZCOS1, ZV1, ZLAT1, ZCOS2, ZLON1, ZINVC, &
!$OMP ZSINN, ZV2, ZINVS, ZSINT, ZCOST, ZV3, ZLAT2, ZM, ZLON2, ZD,ZHOOK_HANDLE)
IF (LHOOK) CALL DR_HOOK('MODE_GRIDTYPE_GAUSS:XY_GAUSS',0,ZHOOK_HANDLE)
!$OMP DO
DO JLAT=1,CEILING(1.*SIZE(PVALUE)/KSIZE_DLAT)

  IOFF=(JLAT-1)*KSIZE_LON

  DO JLON=1,KSIZE_LON

  IF (PVALUE(JLON+IOFF)/=PNODATA) THEN
    !
    ZCOS1 = XCOSN(JLON)*XCOST(JLAT)
    !ZCOS1 = COS(XLAT(JLAT))*COS(XLON(JLON)-XLONP)
    ZV1 = MIN(1.,MAX(-1.,XSINTS(JLAT)+XCOSP*ZCOS1))
    !ZV1 = MIN(1.,MAX(-1.,SIN(XLATP)*SIN(XLAT(JLAT))+COS(XLATP)*ZCOS1))
    ZLAT1 = ASIN(ZV1)
    !
    ZCOS2 = COS(ZLAT1)
    !
    ...
    IF (ZCOS2 /= 0.0) THEN
      !ZINVC = 1./ZCOS2
      !ZSINN = - XCOST(JLAT)*XSINN(JLON)*ZINVC
      !ZV2 = MIN(1.,MAX(-1.,(XSINTC(JLAT)-XSINP*ZCOS1)*ZINVC))
      ZSINN = - XCOST(JLAT)*XSINN(JLON)/ZCOS2
      !ZSINN = - COS(XLAT(JLAT))*SIN(XLON(JLON)-XLONP)/ZCOS2
      ZV2 = MIN(1.,MAX(-1.,(XSINTC(JLAT)-XSINP*ZCOS1)/ZCOS2))     
      !ZV2 = MIN(1.,MAX(-1.,(COS(XLATP)*SIN(XLAT(JLAT))-SIN(XLATP)*ZCOS1)/ZCOS2))
    ...
    !              ----
    !
    PLAT_XY(JLON+IOFF) = ZLAT2 / XDR
    PLON_XY(JLON+IOFF) = ZLON2 / XDR
    !
    IF (ABS(PLON_XY(JLON+IOFF)-360.)<1.E-4) PLON_XY(JLON+IOFF) = 0.
    !
