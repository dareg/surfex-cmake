...
IF (PRESENT(PX)) THEN
  IL = NINT(PGRID_PAR(2))
  PX(:) = PGRID_PAR(2+1:2+IL)
END IF
!
IF (PRESENT(PY)) THEN
  IL = NINT(PGRID_PAR(2))
  PY(:) = PGRID_PAR(2+IL+1:2+2*IL)
END IF
!
IF (PRESENT(PDX)) THEN
  IL = NINT(PGRID_PAR(2))
  PDX(:)= PGRID_PAR(2+2*IL+1:2+3*IL)
END IF
!
IF (PRESENT(PDY)) THEN
  IL = NINT(PGRID_PAR(2))
  PDY(:)= PGRID_PAR(2+3*IL+1:2+4*IL)
END IF
!
IF (PRESENT(KDIMX)) THEN
  IL = NINT(PGRID_PAR(2))
  KDIMX= NINT(PGRID_PAR(3+4*IL))
END IF
!
IF (PRESENT(KDIMY)) THEN
  IL = NINT(PGRID_PAR(2))
  KDIMY= NINT(PGRID_PAR(4+4*IL))
END IF
!
