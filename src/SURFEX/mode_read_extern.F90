...
IF (HNAT=='NAT' .AND. (IVERSION>=7 .OR. .NOT.GECOCLIMAP)) THEN
  ...
  IF (GDATA_DG) THEN
    ...
    DO JL=1,KLAYER
      ...
      CALL READ_SURF_ISBA_PAR_n(DTCO, U, KPATCH, &
                                HFILEPGDTYPE,YRECFM,KLUOUT,KNI,PDEPTH(:,JL,:),IRESP,IVERSION,HDIR='E')
    END DO
    GREAD_OK = .TRUE.
    !
  ENDIF
...
IF (GECOCLIMAP .AND. .NOT.GREAD_OK ) THEN
  ...
  IF (GREAD_EXT) THEN
    ...
    DO JL=1,KLAYER
      ...
      CALL MAKE_CHOICE_ARRAY(HFILETYPE, KPATCH, GDIM, YRECFM, PDEPTH(:,JL,:),HDIR='E')   
    END DO     
    CALL CLOSE_AUX_IO_SURF(HFILE,HFILETYPE)
    !
  ELSE
...
IF (HFIELD=='WG    ' .OR. HFIELD=='WGI   ') THEN
  ...
  IF(GSOC_DATA)THEN
    ...
    YRECFM='PATCH'
    CALL MAKE_CHOICE_ARRAY(HFILETYPE, IPATCH, GDIM, YRECFM, ZWORK,HDIR='E')
    ...
    IF(GSOC)THEN
      ...
      DO JP = 1,IPATCH
        PK => YNP%AL(JP)
        ISIZE = COUNT(ZWORK(:,JP)/=0.)
        ALLOCATE(PK%XPATCH(ISIZE))
        CALL PACK_SAME_RANK(PK%NR_P,ZWORK(:,JP),PK%XPATCH)
        ALLOCATE(PK%XDG(ISIZE,ILAYER))
        CALL PACK_SAME_RANK(PK%NR_P,PDEPTH(:,:,JP),PK%XDG)
        ALLOCATE(PK%XCONDSAT(ISIZE,ILAYER))
        PK%XCONDSAT(:,:) = 0.0
      ENDDO
