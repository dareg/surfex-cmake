...
IF (HNAT=='NAT' .AND. (IVERSION>=7 .OR. .NOT.GECOCLIMAP)) THEN
  ...
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=2) THEN
    ...
    YCOMMENT=YRECFM2
    CALL READ_SURF(HFILEPGDTYPE,YRECFM2,GDATA_GROUND_DEPTH,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
    !
...
IF(YISBA=='DIF') THEN
  ...
  IF (IVERSION>=8) THEN
     DO JLAYER=1,ILAYER
        ...
        ENDIF
        CALL READ_SURF(HFILEPGDTYPE,YRECFM,ZSOILGRID(JLAYER),IRESP,HDIR='A')
     ENDDO
...
DO JLAYER=1,IWORK
  ...
  CALL READ_SURF(HFILETYPE,YRECFM,PFIELD(:,JLAYER,:),IRESP,HDIR='E')
  DO JPATCH=1,IPATCH
...
IF (HFIELD=='WG    ' .OR. HFIELD=='WGI   ') THEN
  ...
  IF(GSOC_DATA)THEN
    !
    ALLOCATE(YIP%XPATCH(KNI,IPATCH))
    !
    CALL OPEN_AUX_IO_SURF(HFILE,HFILETYPE,'NATURE')
    YRECFM='SOC'
    CALL READ_SURF(HFILETYPE,YRECFM,GSOC,IRESP,HDIR='-')
    YRECFM='PATCH'
    CALL READ_SURF(HFILETYPE,YRECFM,YIP%XPATCH(:,:),IRESP,HDIR='E')
    WHERE(YIP%XPATCH(:,:)==XUNDEF)YIP%XPATCH(:,:)=0.0
    CALL CLOSE_AUX_IO_SURF(HFILE,HFILETYPE)    
    !
    IF(GSOC)THEN
      ALLOCATE(YIP%XBCOEF   (KNI,ILAYER))
      ALLOCATE(YIP%XMPOTSAT (KNI,ILAYER))
      ALLOCATE(YIP%XHCAPSOIL(KNI,ILAYER))
      ALLOCATE(YIP%XCONDDRY (KNI,ILAYER))
      ALLOCATE(YIP%XCONDSLD (KNI,ILAYER))
      ALLOCATE(YIP%XWD0     (KNI,ILAYER))
      ALLOCATE(YIP%XKANISO   (KNI,ILAYER))
      ALLOCATE(ZFRACSOC (KNI,ILAYER))
      ALLOCATE(YIP%XCONDSAT (KNI,ILAYER,IPATCH))
      YIP%XBCOEF   (:,:)=0.0
      YIP%XMPOTSAT (:,:)=0.0
      YIP%XHCAPSOIL(:,:)=0.0
      YIP%XCONDDRY (:,:)=XUNDEF
      ...
      ZFRACSOC (:,:)=0.0
      YIP%XCONDSAT (:,:,:)=0.0
      CALL ISBA_SOC_PARAMETERS ('NONE' ,PDEPTH,ZSOC,YIP,ZFRACSOC )
      CALL ISBA_INIT_PGD_INIT(YIP)
    ENDIF
    !
    DEALLOCATE(YIP%XPATCH)
    !
  ENDIF
  !
  IF(YISBA=='DIF')THEN
