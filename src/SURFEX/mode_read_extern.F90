...
IF (HNAT=='NAT' .AND. (IVERSION>=7 .OR. .NOT.GECOCLIMAP)) THEN
  ...
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=2) THEN
    ...
    YRECFM='L_GROUND_DPT'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,GDATA_GROUND_DEPTH,IRESP,HCOMMENT=YCOMMENT)
    !
    IF (GDATA_GROUND_DEPTH) THEN
      !
      YRECFM='D_GROUND_DPT'
      ALLOCATE(ZGROUND_DEPTH(KNI,KPATCH))
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,KLUOUT,KNI,ZGROUND_DEPTH(:,:),IRESP,IVERSION,HDIR='A')
      !
      DO JPATCH=1,KPATCH
        DO JJ=1,KNI
          DO JLAYER=1,KLAYER
...
!
IF (HFIELD=='WG    ' .OR. HFIELD=='WGI   ' .OR. HFIELD=='TWN_WG  ' .OR. HFIELD=='TWN_WGI ') THEN
  KLAYER = IHYDRO_LAYER
ENDIF
!
!-------------------------------------------------------------------
