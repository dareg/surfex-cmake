...
IF (GECOCLIMAP) THEN
  ...
  !
  CALL CONVERT_COVER_ISBA(HISBA,NUNDEF,ZCOVER,GCOVER,'   ',HNAT,PSOILGRID=PSOILGRID,PDG=ZDG,KWG_LAYER=IWG_LAYER)
...
ALLOCATE(PDEPTH    (KNI,ILAYER,KPATCH))
PDEPTH = XUNDEF
DO JPATCH=1,KPATCH
  WHERE(ZD(:,1,JPATCH)/=XUNDEF) &
    PDEPTH    (:,1,JPATCH)=ZD(:,1,JPATCH)/2.  
  DO JLAYER=2,ILAYER
    WHERE(ZD(:,1,JPATCH)/=XUNDEF) &
      PDEPTH    (:,JLAYER,JPATCH) = (ZD(:,JLAYER-1,JPATCH) + ZD(:,JLAYER,JPATCH))/2.  
  END DO
END DO
DEALLOCATE(ZD)
!-------------------------------------------------------------------
...
IF (HFIELD=='WG    ' .OR. HFIELD=='WGI   ') THEN
    ...
    DO JPATCH=1,IPATCH
      DO JLAYER=1,ILAYER
        WHERE(ZVAR(:,JLAYER,JPATCH)/=XUNDEF)
          ...
          !
          PFIELD(:,JLAYER,JPATCH) = (ZVAR(:,JLAYER,JPATCH) - ZWWILT(:)) / (ZWFC(:) - ZWWILT(:))
        END WHERE
      END DO
    END DO
  ELSE IF (HFIELD=='WGI   ') THEN
    DO JPATCH=1,IPATCH
      DO JLAYER=1,ILAYER
        WHERE(ZVAR(:,JLAYER,JPATCH)/=XUNDEF) &
          PFIELD(:,JLAYER,JPATCH) = ZVAR(:,JLAYER,JPATCH) / ZWSAT(:)  
      END DO
    END DO
  END IF
!
