...
CONTAINS
      ...
      INTEGER, PARAMETER              :: MAX_LATS=10000
...
ELSE
  CALL LFILEC (KRESP, NUNIT_FA, CPREFIX1D//TRIM(HREC), ZFIELD8, 1)
  IF (KRESP/=0) CALL ERROR_READ_SURF_FA(HREC,KRESP)
  PFIELD=ZFIELD8
ENDIF
!
...
ELSE
  ...
  !ENDIF  
ENDIF
WHERE (PFIELD(:)>XUNDEF) PFIELD=XUNDEF
!
DEALLOCATE(ZWORK)
!
...
!
DEALLOCATE(IWORK)
IF (ALLOCATED(IWORK8)) DEALLOCATE(IWORK8)
!
...
REAL(KIND=JPHOOK) :: ZHOOK_HANDLE
!
IF (LHOOK) CALL DR_HOOK('MODE_READ_SURF_FA:READ_SURFT1_FA',0,ZHOOK_HANDLE)
