...
END INTERFACE
INTERFACE WD0_FUNC
  MODULE PROCEDURE WD0_FUNC_2D
  MODULE PROCEDURE WD0_FUNC_1D
END INTERFACE
...
!
   ...
   !
   !
   FUNCTION WD0_FUNC_2D(PCLAY,PSAND,HPEDOTF)
   USE MODD_SURF_PAR,   ONLY : XUNDEF
   IMPLICIT NONE
   REAL, DIMENSION(:,:), INTENT(IN)     :: PCLAY
   REAL, DIMENSION(:,:), INTENT(IN)     :: PSAND
   CHARACTER(LEN=4),     INTENT(IN)     :: HPEDOTF   
   REAL, DIMENSION(SIZE(PCLAY,1),SIZE(PCLAY,2)) :: WD0_FUNC_2D
   REAL(KIND=JPRB) :: ZHOOK_HANDLE
   !
   IF (LHOOK) CALL DR_HOOK('MODE_SOIL:WD0_FUNC_2D',0,ZHOOK_HANDLE)
   SELECT CASE (HPEDOTF)
     CASE ('CH78')
       WHERE(PCLAY/=XUNDEF) &
         WD0_FUNC_2D = 0.37528*PCLAY**0.32803
     CASE ('CO84')
       WHERE(PCLAY/=XUNDEF) &
         WD0_FUNC_2D = 0.123225-0.091362*PSAND+0.237614*PCLAY**(1./3.)
   END SELECT
   
   WHERE (PCLAY==XUNDEF) WD0_FUNC_2D = XUNDEF
   IF (LHOOK) CALL DR_HOOK('MODE_SOIL:WD0_FUNC_2D',1,ZHOOK_HANDLE)
   !
   END FUNCTION WD0_FUNC_2D
   !
   !
   !
...
!
   ...
   !
   !
   FUNCTION WD0_FUNC_1D(PCLAY,PSAND,HPEDOTF)
   USE MODD_SURF_PAR,   ONLY : XUNDEF
   IMPLICIT NONE
   REAL, DIMENSION(:), INTENT(IN)     :: PCLAY
   REAL, DIMENSION(:), INTENT(IN)     :: PSAND
   CHARACTER(LEN=4),     INTENT(IN)   :: HPEDOTF   
   REAL, DIMENSION(SIZE(PCLAY))       :: WD0_FUNC_1D
   REAL(KIND=JPRB) :: ZHOOK_HANDLE
   !
   IF (LHOOK) CALL DR_HOOK('MODE_SOIL:WD0_FUNC_1D',0,ZHOOK_HANDLE)
   SELECT CASE (HPEDOTF)
     CASE ('CH78')
       WHERE(PCLAY/=XUNDEF) &
         WD0_FUNC_1D = 0.37528*PCLAY**0.32803
     CASE ('CO84')
       WHERE(PCLAY/=XUNDEF) &
         WD0_FUNC_1D = 0.123225-0.091362*PSAND+0.237614*PCLAY**(1./3.)
   END SELECT
   
   WHERE (PCLAY==XUNDEF) WD0_FUNC_1D = XUNDEF
   IF (LHOOK) CALL DR_HOOK('MODE_SOIL:WD0_FUNC_1D',1,ZHOOK_HANDLE)
   !
   END FUNCTION WD0_FUNC_1D   
   !
   !
   !
