...
INTEGER, PARAMETER :: NFLDMAX = 4000
INTEGER :: NFLDCUR = 0
...
ELSE
  ...
  IWORK8 = KFIELD
  CALL LFIECR (KRESP, NUNIT_FA, CPREFIX1D//TRIM(HREC), IWORK8, 1)
  IF (KRESP/=0) CALL ERROR_WRITE_SURF_FA(HREC,KRESP)
ENDIF
!
...
REAL(KIND=JPRB)   :: ZHOOK_HANDLE
!
IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_FA:WRITE_SURFX3_FA',0,ZHOOK_HANDLE)
!
KRESP=0
!
CALL IO_BUFF(HREC,'W',GFOUND)
...
IF (NRANK==NPIO) THEN
  ...
  IF (.NOT. LFAGMAP) THEN    
    ...
    ENDIF
  ELSE
    !This cannot possibly work, but is it used??
!    DO JP=1,SIZE(ZWORK,3)
!      DO JL=1,SIZE(ZWORK,2)
!        CALL SETNBITS (CPREFIX2D, JL, HREC)
!        CALL FAIENC_NOEXTZ (KRESP, NUNIT_FA, CPREFIX2D, JL, HREC, ZWORK(:,JL,JP), .FALSE., ILCHAM) 
!        IF (KRESP/=0) CALL ERROR_WRITE_SURF_FA(HREC,KRESP)
!      ENDDO
!      CALL FAIDX_ADD (HREC, 'X2', CMASK, ILCHAM, SIZE (ZWORK, 2))
!    ENDDO
    CALL ABOR1('WRITE_SURFX3_FA not coded for LFAGMAP=T')
  ENDIF
  !
#ifdef SFX_MPI
  XTIME_NPIO_WRITE = XTIME_NPIO_WRITE + (MPI_WTIME() - XTIME0)
...
IF (NRANK==NPIO) THEN
  ...
  ELSE
    ...
    ALLOCATE (IWORK8(KL))
    IWORK8 = IWORK (1:KL)
    CALL LFIECR (KRESP, NUNIT_FA, CPREFIX1D//TRIM(HREC), IWORK8, SIZE (IWORK8))
    IF (KRESP/=0) CALL ERROR_WRITE_SURF_FA(HREC,KRESP)
    DEALLOCATE(IWORK8)
  ENDIF
  !
...
ELSE
  ...
  IWORK8(2) = KMONTH
  IWORK8(3) = KDAY
  IWORK8(4) = TRANSFER (REAL(PTIME,JPRD), IWORK8(4))
  CALL LFIECR (KRESP, NUNIT_FA, TRIM(CPREFIX1D)//HREC, IWORK8, SIZE(IWORK8))
ENDIF
!
