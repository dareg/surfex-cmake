...
INTERFACE WRITE_SURFT_FA
        ...
        MODULE PROCEDURE WRITE_SURFT1_FA
        MODULE PROCEDURE WRITE_SURFT2_FA
END INTERFACE

INTEGER, PARAMETER :: NFLDMAX = 10000
INTEGER :: NFLDCUR = 0
...
!
IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_FA:WRITE_SURFX0_FA',0,ZHOOK_HANDLE)
!
KRESP=0
!
...
ELSE
  IF (LQDNORMS) THEN
    CALL GET_LUOUT('FA    ',ILUOUT)
    WRITE(ILUOUT,'(''X0 : '',A16,E20.14)') TRIM(CPREFIX1D)//HREC, PFIELD
  ENDIF
  CALL FAIDX_ADD (HREC, 'X0', '', -1, -1)
  ZFIELD8=PFIELD
  CALL LFIECR (KRESP, NUNIT_FA, TRIM(CPREFIX1D)//HREC, ZFIELD8, 1)
...
ELSE
  IF (LQDNORMS) THEN
    CALL GET_LUOUT('FA    ',ILUOUT)
    WRITE(ILUOUT,'(''N0 : '',A16,I8)') CPREFIX1D//TRIM(HREC), KFIELD
  ENDIF
...
ELSE
  IF (LQDNORMS) THEN
    CALL GET_LUOUT('FA    ',ILUOUT)
    WRITE(ILUOUT,'(''L0 : '',A16,L2)') CPREFIX1D//TRIM(HREC), OFIELD
  ENDIF
...
ELSE
  IF (LQDNORMS) THEN
    CALL GET_LUOUT('FA    ',ILUOUT)
    WRITE(ILUOUT,'(''C0 : '',A16,A40)') TRIM(CPREFIX1D)//HREC, HFIELD
  ENDIF
...
IF (NRANK==NPIO) THEN
  ...
  ELSE
    IF (LQDNORMS) THEN
      CALL GET_LUOUT('FA    ',ILUOUT)
      IF (COUNT(ZWORK/=XUNDEF) /= 0) THEN
        WRITE(ILUOUT,'(''X1 : '',A16,I8,3(1X,E20.14))') TRIM(CPREFIX1D)//HREC, COUNT(ZWORK/=XUNDEF), &
         & MINVAL(ZWORK,MASK=(ZWORK/=XUNDEF)), MAXVAL(ZWORK,MASK=(ZWORK/=XUNDEF)), SUM(ZWORK,MASK=(ZWORK/=XUNDEF))/COUNT(ZWORK/=XUNDEF)
      ELSE
        WRITE(ILUOUT,'(''X1 : '',A16,I8,1X,3(E20.14))') TRIM(CPREFIX1D)//HREC, COUNT(ZWORK/=XUNDEF)
      ENDIF
    ENDIF
...
IF (NRANK==NPIO) THEN
  ...
  ELSE
    IF (LQDNORMS) THEN
      CALL GET_LUOUT('FA    ',ILUOUT)
      IF (COUNT(ZWORK/=XUNDEF) /= 0) THEN
        WRITE(ILUOUT,'(''X2 : '',A16,I8,3(1X,E20.14))') TRIM(CPREFIX2D)//HREC, COUNT(ZWORK/=XUNDEF), &
         & MINVAL(ZWORK,MASK=(ZWORK/=XUNDEF)), MAXVAL(ZWORK,MASK=(ZWORK/=XUNDEF)), SUM(ZWORK,MASK=(ZWORK/=XUNDEF))/COUNT(ZWORK/=XUNDEF)
      ELSE
        WRITE(ILUOUT,'(''X2 : '',A16,I8,1X,3(E20.14))') TRIM(CPREFIX2D)//HREC, COUNT(ZWORK/=XUNDEF)
      ENDIF
    ENDIF
    DO JL = 1, SIZE (ZWORK, 2)
...
IF (NRANK==NPIO) THEN
  ...
  ELSE
    IF (LQDNORMS) THEN
      CALL GET_LUOUT('FA    ',ILUOUT)
      WRITE(ILUOUT,'(''N1 : '',A16,2I8,E20.14)') CPREFIX1D//TRIM(HREC), MINVAL(JWORK(1:KL)), MAXVAL(JWORK(1:KL)), REAL(SUM(JWORK(1:KL)))/REAL(KL)
    ENDIF
...
IF (NRANK==NPIO) THEN
  ...
  ELSE
    ...
    IF (LQDNORMS) THEN
      CALL GET_LUOUT('FA    ',ILUOUT)
      WRITE(ILUOUT,'(''L1 : '',A16,3(I8))') TRIM(CPREFIX1D)//HREC, MINVAL(IWORK),MAXVAL(IWORK),COUNT(OFIELD(1:KL))
    ENDIF
    CALL LFIECR (KRESP, NUNIT_FA, CPREFIX1D//TRIM(HREC), IWORK, SIZE (IWORK))
    IF (KRESP/=0) CALL ERROR_WRITE_SURF_FA(HREC,KRESP)
  ENDIF
...
ELSE
  ...
  IF (LQDNORMS) THEN
    CALL GET_LUOUT('FA    ',ILUOUT)
    WRITE(ILUOUT,'(''T0 : '',A16,1X,I4,1X,I2,1X,I2,1X,E20.14)') TRIM(CPREFIX1D)//HREC, KYEAR, KMONTH, KDAY, PTIME 
  ENDIF
ENDIF
!
...
!
!     #############################################################
      SUBROUTINE WRITE_SURFT1_FA (&
                                  HREC,KYEAR,KMONTH,KDAY,PTIME,KRESP,HCOMMENT)
!     #############################################################
!
!!****  * - routine to write a 1D array of dates
!
!
!
!
...
REAL(KIND=JPHOOK)        :: ZHOOK_HANDLE
!
IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_FA:WRITE_SURFT1_FA',0,ZHOOK_HANDLE)
!
KRESP=0
!
 CALL IO_BUFF(HREC,'W',GFOUND)
!
IF (GFOUND .AND. LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_FA:WRITE_SURFT1_FA',1,ZHOOK_HANDLE)
IF (GFOUND) RETURN
!
!In other multi-dimensional routines, there is a 'HDIR' argument but not here...
!IF(HDIR=='H')THEN
!  CALL GATHER_AND_WRITE_MPI(PFIELD,ZWORK,NMASK)
!ELSE !no horizontal dim. case (not masked)
!
...
IF (NRANK==NPIO) THEN
  ...
  !
  IF (.NOT. LFAGMAP) THEN
    WRITE(NLUOUT,*) ' WRITE_SURFT1_FA : time in 1 dimension with LFAGMAP=.F. not yet implemented'
    CALL ABOR1_SFX('MODE_WRITE_SURF_FA:WRITE_SURFT1_FA: time in 1 dimension with LFAGMAP=.F. not yet implemented')
  ELSE
    CALL FAIDX_ADD (HREC, 'T1', '', SIZE(KYEAR), -1)
    ALLOCATE (IWORK (SIZE(KYEAR)*4))
    !Same encoding as in modd_io_surf_aro
    DO J = 1, SIZE(KYEAR)
      IWORK(1+(J-1)*4) = KYEAR(J)
      IWORK(2+(J-1)*4) = KMONTH(J)
      IWORK(3+(J-1)*4) = KDAY(J)
      IWORK(4+(J-1)*4) = TRANSFER(PTIME(J), IWORK(4+(J-1)*4))
    ENDDO
    IF (LQDNORMS) THEN
      CALL GET_LUOUT('FA    ',ILUOUT)
      WRITE(ILUOUT,'(''T1 : '',A16,1X,I3,1X,10(I4,1X,I2,1X,I2,1X,E20.14))')  CPREFIX1D//TRIM(HREC),  SIZE(KYEAR), &
       & (KYEAR(J),KMONTH(J),KDAY(J),PTIME(J),J=1,SIZE(KYEAR))
    ENDIF
    CALL LFIECR (KRESP, NUNIT_FA, CPREFIX1D//TRIM(HREC), IWORK, SIZE (IWORK))
    IF (KRESP/=0) CALL ERROR_WRITE_SURF_FA(HREC,KRESP)
  ENDIF
...
ENDIF
!
IF (LHOOK) CALL DR_HOOK('MODE_WRITE_SURF_FA:WRITE_SURFT1_FA',1,ZHOOK_HANDLE)
!
END SUBROUTINE WRITE_SURFT1_FA
!
!     #############################################################
