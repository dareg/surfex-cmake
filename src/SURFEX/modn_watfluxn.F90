...
!
SUBROUTINE INIT_NAM_DIAG_SURFn (DWO)
!
...
!
  ...
  XDIAG_TSTEP = DWO%XDIAG_TSTEP  
  N2M = DWO%N2M
  L2M_MIN_ZS = DWO%L2M_MIN_ZS
  LSURF_BUDGET = DWO%LSURF_BUDGET
  LRAD_BUDGET = DWO%LRAD_BUDGET
  LSURF_BUDGETC = DWO%LSURF_BUDGETC
  LRESET_BUDGETC = DWO%LRESET_BUDGETC  
  LCOEF = DWO%LCOEF
  LSURF_VARS = DWO%LSURF_VARS
IF (LHOOK) CALL DR_HOOK('MODN_WATFLUX_N:INIT_NAM_DIAG_SURFN',1,ZHOOK_HANDLE)
END SUBROUTINE INIT_NAM_DIAG_SURFn

SUBROUTINE UPDATE_NAM_DIAG_SURFn (DWO)
!
  USE MODD_DIAG_n, ONLY : DIAG_OPTIONS_t
!
  IMPLICIT NONE
...
!
  ...
  DWO%XDIAG_TSTEP = XDIAG_TSTEP  
  DWO%N2M = N2M
  DWO%L2M_MIN_ZS = L2M_MIN_ZS
  DWO%LSURF_BUDGET = LSURF_BUDGET
  DWO%LRAD_BUDGET = LRAD_BUDGET 
  DWO%LSURF_BUDGETC = LSURF_BUDGETC
  DWO%LRESET_BUDGETC = LRESET_BUDGETC  
  DWO%LCOEF = LCOEF
  DWO%LSURF_VARS = LSURF_VARS
