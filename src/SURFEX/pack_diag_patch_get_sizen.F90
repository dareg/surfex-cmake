...
IF(I%TSNOW%SCHEME=='CRO') THEN
  ...
  IF (DGMI%LPROSNOW) THEN
    ...
    PKDI%NSIZE_IMPUR=PKDI%NSIZE_IMPUR+1 !BC merge
    IF (DGMI%LPROBANDS) THEN
       PKDI%NSIZE_KSW=PKDI%NSIZE_KSW+2
ELSE
       PKDI%NSIZE_00=PKDI%NSIZE_00+2
    ENDIF
  ELSE
    PKDI%NSIZE_0=PKDI%NSIZE_0+11
    PKDI%NSIZE_00=PKDI%NSIZE_00+7  
    PKDI%NSIZE_00=PKDI%NSIZE_00+2 ! pour diag lprobands
    PKDI%NSIZE_000=PKDI%NSIZE_000+1  ! pour les impuretés
  ENDIF  
ELSE
! pour sytmass
...
! pour diags lprosnow
  ...
  ! pour diag lprobands
  PKDI%NSIZE_00=PKDI%NSIZE_00+2
  ! pour les impuretés BC merge
  PKDI%NSIZE_000=PKDI%NSIZE_000+1 
ENDIF
!
IF (I%LTR_ML) THEN
