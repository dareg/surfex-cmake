...
INTEGER :: ISIZE_LSIMPLE, ISIZE_L0, ISIZE_TSIMPLE, ISIZE_T0, ISIZE_SIMPLE,  &
           ...
           ISIZE_LITTER,ISIZE_IMPUR, ISIZE_0, ISIZE_00,ISIZE_000, ISIZE_01,             &
...
ALLOCATE(PKI%XBLOCK_IMPUR(KSIZE,I%TSNOW%NLAYER,NIMPUR,PKI%NSIZE_IMPUR))
...
PKI%NP_TAB_SYT      => PKI%NBLOCK_SIMPLE(:,ISIZE_NSIMPLE)
ISIZE_SIMPLE = ISIZE_SIMPLE + 1
...
IF(I%TSNOW%SCHEME=='CRO') THEN
  ...
  ISIZE_SNOW = ISIZE_SNOW + 1
   PKI%XP_SNOWIMPUR   => PKI%XBLOCK_SNOW(:,:,ISIZE_SNOW)
  ISIZE_IMPUR = ISIZE_IMPUR + 1
   PKI%XP_SNOWIMPURV2   => PKI%XBLOCK_IMPUR(:,:,:,ISIZE_IMPUR)
ELSE      
  ISIZE_00 = ISIZE_00 + 1
  ...
  ISIZE_00 = ISIZE_00 + 1
  PKI%XP_SNOWIMPUR   => PKI%XBLOCK_00(:,:,ISIZE_00)
  ISIZE_000 = ISIZE_000 + 1
  PKI%XP_SNOWIMPURV2   => PKI%XBLOCK_000(:,:,:,ISIZE_000)  
END IF
!
IF(I%LGLACIER)THEN  
  ISIZE_SIMPLE = ISIZE_SIMPLE + 1  
...
IF (I%NPATCH==1) THEN
  ...
  IF(I%TSNOW%SCHEME=='CRO') THEN
    ...
    PKI%XP_SNOWIMPURV2 (:,:,:)=   I%TSNOW%IMPURV2(:,:,:,1)
  END IF
  !
  IF(I%LGLACIER)THEN
  ...
  IF (I%TSNOW%SCHEME=='CRO') THEN
    DO JK=1,SIZE(PKI%XP_SNOWGRAN1,2)
      DO JJ=1,KSIZE
        ...
        PKI%XP_SNOWIMPUR (JJ,JK)   =    I%TSNOW%IMPUR   (JI, JK, KPATCH)
      ENDDO
    END DO
  END IF
  ...
  IF (I%TSNOW%SCHEME=='CRO') THEN
    DO JIMP=1,NIMPUR
      DO JK=1,SIZE(PKI%XP_SNOWGRAN1,2)
        DO JJ=1,KSIZE   
              JI                     =    KMASK(JJ)    
              PKI%XP_SNOWIMPURV2 (JJ,JK,JIMP)=    I%TSNOW%IMPURV2   (JI, JK,JIMP, KPATCH)
        ENDDO
      ENDDO
    ENDDO
  END IF
