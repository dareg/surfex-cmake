!     #########
      SUBROUTINE PGD_GRID (UG, U, &
                           HPROGRAM,HFILE,HFILETYPE,OGRID,HGRID,KGRID_PAR,PGRID_PAR,HDIR)
!     ##########################################################
!!
!!    PURPOSE
!!    -------
...
IF (LEN_TRIM(YINIFILETYPE)>0 .AND. LEN_TRIM(YINIFILE)>0 ) THEN
  IF (YINIFILETYPE=='MESONH' .OR. YINIFILETYPE=='LFI   ' .OR. YINIFILETYPE=='ASCII ' .OR. YINIFILETYPE=='NC    ') THEN
    CALL GRID_FROM_FILE(UG,U,HPROGRAM,YINIFILE,YINIFILETYPE,OGRID,CGRID,NGRID_PAR,XGRID_PAR,NL,YDIR)
  ELSE
...
!
  ELSE
!
    CALL READ_NAM_GRIDTYPE(UG,U,HPROGRAM,CGRID,NGRID_PAR,XGRID_PAR,NL,YDIR)
!
  END IF
