...
USE MODD_DATA_ISBA_n,    ONLY : XPAR_VEGTYPE,  XPAR_LAI, XPAR_H_TREE, XPAR_DG, &
                                ...
                                XPAR_GNDLITTER, XPAR_ZF_TALLVEG , XPAR_RGLGV, XPAR_GAMMAGV, XPAR_RSMINGV,   &
                                XPAR_ROOTFRACGV, XPAR_WRMAX_CFGV, XPAR_LAIGV, XPAR_Z0LITTER, XPAR_H_VEG,    &
                                ...
                                LDATA_GNDLITTER, LDATA_ZF_TALLVEG , LDATA_RGLGV, LDATA_GAMMAGV, LDATA_RSMINGV,   &
                                LDATA_ROOTFRACGV, LDATA_WRMAX_CFGV, LDATA_LAIGV, LDATA_Z0LITTER, LDATA_H_VEG,  &
...
ELSEIF (LDATA_VEGTYPE) THEN
  ...
  WHERE (XPAR_VEGTYPE(:,:)/=0.) XPAR_VEGTYPE(:,:) = XPAR_VEGTYPE(:,:) / &
                                                    SPREAD(SUM(XPAR_VEGTYPE(:,:),2),2,NVEGTYPE)
  !  
ENDIF
!
!--------------------------------temporal fields-----------------------------------
...
ALLOCATE(XPAR_VEG      (NDIM,NTIME,NVEGTYPE))
 CALL INI_VAR_FROM_DATA(HPROGRAM,'ARI','VEG: vegetation fraction','NAT',CFNAM_VEG,CFTYP_VEG,XUNIF_VEG, &
                        XPAR_VEG,LDATA_VEG)
IF (.NOT. LDATA_VEG) DEALLOCATE(XPAR_VEG)
!
ALLOCATE(XPAR_LAI      (NDIM,NTIME,NVEGTYPE))
 CALL INI_VAR_FROM_DATA(HPROGRAM,'ARI','LAI: leaf area index','NAT',CFNAM_LAI,CFTYP_LAI,XUNIF_LAI, &
                        XPAR_LAI,LDATA_LAI) 
IF (.NOT. LDATA_VEGTYPE .AND. .NOT. LDATA_LAI) DEALLOCATE(XPAR_LAI)
!
ALLOCATE(XPAR_H_VEG       (NDIM,NTIME,NVEGTYPE))
 CALL INI_VAR_FROM_DATA(HPROGRAM,'ARI','H_VEG: vegetation height','NAT',CFNAM_H_VEG,CFTYP_H_VEG,XUNIF_H_VEG, &
                        XPAR_H_VEG,LDATA_H_VEG) 
IF (.NOT. LDATA_H_VEG) DEALLOCATE(XPAR_H_VEG)
!
ALLOCATE(XPAR_Z0       (NDIM,NTIME,NVEGTYPE))
 CALL INI_VAR_FROM_DATA(HPROGRAM,'CDN','Z0: roughness length','NAT',CFNAM_Z0,CFTYP_Z0,XUNIF_Z0, &
                        XPAR_Z0,LDATA_Z0)
IF (.NOT. LDATA_Z0) DEALLOCATE(XPAR_Z0)
!
ALLOCATE(XPAR_EMIS     (NDIM,NTIME,NVEGTYPE))
 CALL INI_VAR_FROM_DATA(HPROGRAM,'ARI','EMIS: emissivity','NAT',CFNAM_EMIS,CFTYP_EMIS,XUNIF_EMIS, &
                        XPAR_EMIS,LDATA_EMIS)
...
IF(ISIZE_LMEB_PATCH>0) THEN
  ...
  CALL INI_VAR_FROM_DATA(HPROGRAM,'ARI','GNDLITTER: ground litter fraction','NAT',&
                         CFNAM_GNDLITTER,CFTYP_GNDLITTER,XUNIF_GNDLITTER,XPAR_GNDLITTER,LDATA_GNDLITTER)
  IF (.NOT. LDATA_GNDLITTER) DEALLOCATE(XPAR_GNDLITTER)
  !
  ALLOCATE(XPAR_Z0LITTER        (NDIM,NTIME,NVEGTYPE))
  CALL INI_VAR_FROM_DATA(HPROGRAM,'CDN','Z0LITTER: ground litter roughness length','NAT', &
                         CFNAM_Z0LITTER,CFTYP_Z0LITTER,XUNIF_Z0LITTER,XPAR_Z0LITTER,LDATA_Z0LITTER)
  IF (.NOT. LDATA_Z0LITTER) DEALLOCATE(XPAR_Z0LITTER)
  !
ENDIF
! ------------ End MEB parameters ---------------------
!
