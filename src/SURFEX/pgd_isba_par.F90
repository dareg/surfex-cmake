...
IF (.NOT. DTV%LDATA_VEG) DEALLOCATE(DTV%XPAR_VEG)
!
...
!
IF (.NOT.IO%LECOCLIMAP .AND. .NOT.(DTV%LDATA_VEG .AND. DTV%LDATA_LAI .AND. DTV%LDATA_Z0 .AND. DTV%LDATA_EMIS)) THEN
  !
  WRITE(ILUOUT,*) ' '
  WRITE(ILUOUT,*) '***********************************************************'
  WRITE(ILUOUT,*) '* Error in PGD field preparation of temporal fields       *'
  WRITE(ILUOUT,*) '* There is no prescribed value and no input file :        *'
  IF (.NOT.DTV%LDATA_VEG ) WRITE(ILUOUT,*) '* for VEG                            *'
...
!--------------------------------------AGS parameters----------------------------
!
IF (IO%CPHOTO/='NON' .OR. (.NOT.DTV%LDATA_Z0.AND.(DTV%LDATA_LAI.OR.DTV%LDATA_VEGTYPE)) .OR. ISIZE_LMEB_PATCH>0) THEN
  !
