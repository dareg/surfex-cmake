...
END IF
!-------------------------------------------------------------------------------
IF (NGROUND_MAX < I%O%NGROUND_LAYER) THEN
  WRITE(ILUOUT,*) '------------------------------------'
  WRITE(ILUOUT,*) 'Please update pgd_isba_par.f90 routine : '
  WRITE(ILUOUT,*) 'The maximum number of soil layers  '
  WRITE(ILUOUT,*) 'in the declaration of the namelist variables '
  WRITE(ILUOUT,*) 'must be increased to : ', I%O%NGROUND_LAYER
  WRITE(ILUOUT,*) '------------------------------------'
...
IF (.NOT. DTI%LDATA_VEGTYPE ) DEALLOCATE(DTI%XPAR_VEGTYPE)
!
IF (.NOT.I%O%LECOCLIMAP .AND. .NOT.DTI%LDATA_VEGTYPE) THEN
  !
  WRITE(ILUOUT,*) ' '
  WRITE(ILUOUT,*) '***********************************************************'
...
!
IF (.NOT.I%O%LECOCLIMAP .AND. .NOT.(DTI%LDATA_VEG .AND. DTI%LDATA_LAI .AND. DTI%LDATA_Z0 .AND. DTI%LDATA_EMIS)) THEN
  !
  WRITE(ILUOUT,*) ' '
  WRITE(ILUOUT,*) '***********************************************************'
...
!--------------------------------depths fields-----------------------------------
!
ALLOCATE(DTI%XPAR_DG          (IG%NDIM,I%O%NGROUND_LAYER,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, DGU, UG, U, USS, DTI, &
...
IF(I%O%CISBA=='DIF')THEN 
  ! 
  ALLOCATE(DTI%XPAR_ROOTFRAC    (IG%NDIM,I%O%NGROUND_LAYER,NVEGTYPE))  
CALL INI_VAR_FROM_DATA(DTCO, DGU, UG, U, USS, DTI, &
...
CALL INI_VAR_FROM_DATA(DTCO, DGU, UG, U, USS, DTI, &
  ...
  IF(ISIZE_LMEB_PATCH>0) THEN
    !
    ALLOCATE(DTI%XPAR_ROOTFRACGV  (IG%NDIM,I%O%NGROUND_LAYER,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, DGU, UG, U, USS, DTI, &
    ...
    IF (.NOT. DTI%LDATA_ROOTFRACGV .AND. DTI%LDATA_ROOTFRAC)THEN
      ALLOCATE(DTI%XPAR_ROOTFRACGV  (IG%NDIM,I%O%NGROUND_LAYER,NVEGTYPE))
      IHGROUND_LAYER=INT(CEILING(REAL(I%O%NGROUND_LAYER)/2.0))
...
CALL INI_VAR_FROM_DATA(DTCO, DGU, UG, U, USS, DTI, &
  ...
  IF (.NOT.I%O%LECOCLIMAP) THEN
    ...
    ELSEIF( .NOT.ALL(I%O%XSOILGRID(:)==XUNDEF) .AND. &
      ...
      CALL ABOR1_SFX("PGD_ISBA_PAR: PROBLEM IN SOIL GRID COMPUTATION") 
    ENDIF
    IF(.NOT.DTI%LDATA_DG .AND.ALL(I%O%XSOILGRID(:)==XUNDEF))THEN
      WRITE(ILUOUT,*) ' '
      WRITE(ILUOUT,*) '****************************************************************************'
      WRITE(ILUOUT,*) '* Without ECOCLIMAP, Error in PGD field preparation for ISBA-DIF           *'
...
ELSE   
  !
  IF ( .NOT.I%O%LECOCLIMAP .AND. .NOT.DTI%LDATA_DG .AND. &
...
!
IF (.NOT.I%O%LECOCLIMAP.AND..NOT. DTI%LDATA_DICE) THEN
  IF(I%O%CISBA/='DIF' .AND. (DTI%LDATA_DG.OR.DTI%LDATA_ROOT_DEPTH)) THEN   
...
!
IF (.NOT.I%O%LECOCLIMAP .AND. .NOT.(DTI%LDATA_RSMIN.AND.DTI%LDATA_GAMMA.AND.DTI%LDATA_WRMAX_CF.AND.DTI%LDATA_RGL &
...
!--------------------------------------AGS parameters----------------------------
!
IF (I%O%CPHOTO/='NON' .OR. (.NOT.DTI%LDATA_Z0.AND.(DTI%LDATA_LAI.OR.DTI%LDATA_VEGTYPE)) .OR. ISIZE_LMEB_PATCH>0) THEN
  !
...
IF (I%O%CPHOTO/='NON' .OR. ISIZE_LMEB_PATCH>0) THEN
...
ENDIF
!
IF (.NOT.I%O%LECOCLIMAP .AND. ISIZE_LMEB_PATCH>0 .AND. .NOT.(DTI%LDATA_H_TREE      &
...
CALL INI_VAR_FROM_DATA(DTCO, DGU, UG, U, USS, DTI, &
  ...
  !
  IF (.NOT.I%O%LECOCLIMAP .AND. .NOT.(DTI%LDATA_H_TREE.AND.DTI%LDATA_RE25.AND.DTI%LDATA_LAIMIN.AND.&
  ...
  !  
  IF (I%O%CPHOTO/='AGS' .AND. I%O%CPHOTO/='LAI') THEN
    !
...
CALL INI_VAR_FROM_DATA(DTCO, DGU, UG, U, USS, DTI, &
    ...
    !
    IF (.NOT.I%O%LECOCLIMAP .AND. .NOT.(DTI%LDATA_F2I.AND.DTI%LDATA_DMAX)) THEN
      !
      WRITE(ILUOUT,*) ' '
      WRITE(ILUOUT,*) '***********************************************************'
...
CALL INI_VAR_FROM_DATA(DTCO, DGU, UG, U, USS, DTI, &
      ...
      !
      IF (.NOT.I%O%LECOCLIMAP .AND. .NOT.(DTI%LDATA_CE_NITRO.AND.DTI%LDATA_CF_NITRO.AND.DTI%LDATA_CNA_NITRO)) THEN
        !
        WRITE(ILUOUT,*) ' '
        WRITE(ILUOUT,*) '***********************************************************'
...
!----------------------------------------------------------------------------------------
!
IF (I%O%LECOCLIMAP .AND. DTI%LDATA_VEGTYPE) THEN
  !  
