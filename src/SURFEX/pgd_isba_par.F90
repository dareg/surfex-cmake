!     #########
      SUBROUTINE PGD_ISBA_PAR (DTCO, UG, U, USS, DTI, IO, IP, KDIM, &
                               HPROGRAM)
!     ##############################################################
!
...
!            ------------------------------
!
REAL, DIMENSION(KDIM,NVEGTYPE)     :: ZROOTFRACGV,TEST,TEST2,TEST3
...
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
IF (.NOT. DTI%LDATA_VEGTYPE ) THEN
  DEALLOCATE(DTI%XPAR_VEGTYPE)
  ALLOCATE(DTI%XPAR_VEGTYPE(0,0))
ENDIF
!
IF (.NOT.IO%LECOCLIMAP .AND. .NOT.DTI%LDATA_VEGTYPE) THEN
  !
  WRITE(ILUOUT,*) ' '
  WRITE(ILUOUT,*) '***********************************************************'
...
!--------------------------------temporal fields-----------------------------------
!
ALLOCATE(DTI%XPAR_VEG      (KDIM,NTIME,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
ALLOCATE(DTI%XPAR_LAI      (KDIM,NTIME,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
ALLOCATE(DTI%XPAR_H_VEG       (KDIM,NTIME,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
ALLOCATE(DTI%XPAR_Z0       (KDIM,NTIME,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
ALLOCATE(DTI%XPAR_EMIS     (KDIM,NTIME,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
IF (.NOT.IO%LECOCLIMAP .AND. .NOT.(DTI%LDATA_VEG .AND. DTI%LDATA_LAI .AND. DTI%LDATA_Z0 .AND. DTI%LDATA_EMIS)) THEN
  !
  WRITE(ILUOUT,*) ' '
  WRITE(ILUOUT,*) '***********************************************************'
...
IF(ISIZE_LMEB_PATCH>0) THEN
  !
  ALLOCATE(DTI%XPAR_LAIGV       (KDIM,NTIME,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
  ...
  !
  ALLOCATE(DTI%XPAR_GNDLITTER   (KDIM,NTIME,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
  ...
  !
  ALLOCATE(DTI%XPAR_Z0LITTER        (KDIM,NTIME,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
  ...
  IF(ISIZE_LMEB_PATCH>0) THEN
    !
    ALLOCATE(DTI%XPAR_ROOTFRACGV  (KDIM,IO%NGROUND_LAYER,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
    ...
    IF (.NOT. DTI%LDATA_ROOTFRACGV .AND. DTI%LDATA_ROOTFRAC)THEN
      ALLOCATE(DTI%XPAR_ROOTFRACGV  (KDIM,IO%NGROUND_LAYER,NVEGTYPE))
      IHGROUND_LAYER=INT(CEILING(REAL(IO%NGROUND_LAYER)/2.0))
      ...
      DO IIH=1,IHGROUND_LAYER
        TEST=RESHAPE(DTI%XPAR_ROOTFRACGV(:,IIH,:),(/KDIM,NVEGTYPE/))
        TEST2=1.
    ...
    ENDIF
    !                                     '                                '
    ALLOCATE(DTI%XPAR_ROOT_EXTINCTIONGV(KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
  ...
  IF (.NOT.IO%LECOCLIMAP) THEN
    ...
    ELSEIF( .NOT.ALL(IO%XSOILGRID(:)==XUNDEF) .AND. &
      ...
      CALL ABOR1_SFX("PGD_ISBA_PAR: PROBLEM IN SOIL GRID COMPUTATION") 
    ENDIF
    IF(.NOT.DTI%LDATA_DG .AND.ALL(IO%XSOILGRID(:)==XUNDEF))THEN
      WRITE(ILUOUT,*) ' '
      WRITE(ILUOUT,*) '****************************************************************************'
      WRITE(ILUOUT,*) '* Without ECOCLIMAP, Error in PGD field preparation for ISBA-DIF           *'
...
ELSE   
  !
  IF ( .NOT.IO%LECOCLIMAP .AND. .NOT.DTI%LDATA_DG .AND. &
...
!
IF (.NOT.IO%LECOCLIMAP.AND..NOT. DTI%LDATA_DICE) THEN
  IF(IO%CISBA/='DIF' .AND. (DTI%LDATA_DG.OR.DTI%LDATA_ROOT_DEPTH)) THEN   
  ...
  ENDIF
ENDIF  
IF (.NOT.DTI%LDATA_DICE) DEALLOCATE(DTI%XPAR_DICE)
!
!---------------------classical fields---------------------------------------------
!
...
!
ALLOCATE(DTI%XPAR_ALBNIR_VEG  (KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
ALLOCATE(DTI%XPAR_ALBVIS_VEG  (KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
ALLOCATE(DTI%XPAR_ALBUV_VEG   (KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
ALLOCATE(DTI%XPAR_ALBNIR_SOIL (KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
ALLOCATE(DTI%XPAR_ALBVIS_SOIL (KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
ALLOCATE(DTI%XPAR_ALBUV_SOIL  (KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!
IF (.NOT.IO%LECOCLIMAP .AND. .NOT.(DTI%LDATA_RSMIN.AND.DTI%LDATA_GAMMA.AND.DTI%LDATA_WRMAX_CF.AND.DTI%LDATA_RGL &
...
IF(ISIZE_LMEB_PATCH>0) THEN
  !
  ALLOCATE(DTI%XPAR_RSMINGV     (KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
  ...
  !                                     '                            '
  ALLOCATE(DTI%XPAR_GAMMAGV     (KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
  ...
  !                                     '                            '
  ALLOCATE(DTI%XPAR_WRMAX_CFGV  (KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
  ...
  !                                     '                            '
  ALLOCATE(DTI%XPAR_RGLGV       (KDIM,NVEGTYPE))
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
...
!--------------------------------------AGS parameters----------------------------
!
IF (IO%CPHOTO/='NON' .OR. (.NOT.DTI%LDATA_Z0.AND.(DTI%LDATA_LAI.OR.DTI%LDATA_VEGTYPE)) .OR. ISIZE_LMEB_PATCH>0) THEN
  !
...
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
  ...
  IF (.NOT. DTI%LDATA_H_TREE) DEALLOCATE(DTI%XPAR_H_TREE)
  !
ENDIF
...
ENDIF
!
IF (.NOT.IO%LECOCLIMAP .AND. ISIZE_LMEB_PATCH>0 .AND. .NOT.(DTI%LDATA_H_TREE      &
...
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
  ...
  !
  IF (.NOT.IO%LECOCLIMAP .AND. .NOT.(DTI%LDATA_H_TREE.AND.DTI%LDATA_RE25.AND.DTI%LDATA_LAIMIN.AND.&
...
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
    ...
    !
    IF (.NOT.IO%LECOCLIMAP .AND. .NOT.(DTI%LDATA_F2I.AND.DTI%LDATA_DMAX)) THEN
      !
      WRITE(ILUOUT,*) ' '
      WRITE(ILUOUT,*) '***********************************************************'
...
CALL INI_VAR_FROM_DATA(DTCO, UG, U, USS, DTI%XPAR_VEGTYPE, &
      ...
      !
      IF (.NOT.IO%LECOCLIMAP .AND. .NOT.(DTI%LDATA_CE_NITRO.AND.DTI%LDATA_CF_NITRO.AND.DTI%LDATA_CNA_NITRO)) THEN
        !
        WRITE(ILUOUT,*) ' '
        WRITE(ILUOUT,*) '***********************************************************'
...
!----------------------------------------------------------------------------------------
!
IF (IO%LECOCLIMAP .AND. DTI%LDATA_VEGTYPE) THEN
  !  
  CALL EXTRAPOL_FIELDS(DTCO, DTI, KDIM, IO, IP, UG, U, HPROGRAM,ILUOUT)
  !
ENDIF
!
!----------------------------------------------------------------------------------------
