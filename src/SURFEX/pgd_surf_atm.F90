...
USE MODD_PGD_GRID,        ONLY : LLATLONMASK, NGRID_PAR
!
...
IF (.NOT.ASSOCIATED(YSC%UG%XGRID_FULL_PAR)) THEN
  ALLOCATE(YSC%UG%XGRID_FULL_PAR(SIZE(YSC%UG%XGRID_PAR)))
  YSC%UG%XGRID_FULL_PAR(:) = YSC%UG%XGRID_PAR(:)
  YSC%UG%NGRID_FULL_PAR = NGRID_PAR
ENDIF
!
!*    2.4     mask to limit the number of input data to read
 CALL LATLONMASK      (YSC%UG%CGRID,YSC%UG%NGRID_FULL_PAR,YSC%UG%XGRID_FULL_PAR,LLATLONMASK)
!
!-------------------------------------------------------------------------------
!
!*    3.      surface cover
