...
END IF
!
IF (DTT%LDATA_BLDTYPE .OR. DTT%LDATA_BLD_AGE .OR. DTT%LDATA_USETYPE)  CALL READ_CSVDATA_TEB(HPROGRAM,CCSVDATAFILE)
!
!* building's code
IF (ASSOCIATED(DTT%NPAR_BLDTYPE)) DTT%NPAR_BLDCODE(:) = BLDCODE(DTT%NPAR_BLDTYPE,DTT%NPAR_BLD_AGE)
!
!
!* other building parameters
 CALL INI_VAR_FROM_DATA_0D(HPROGRAM,'ARI','BLD        ','TWN', CFNAM_BLD,CFTYP_BLD,XUNIF_BLD,DTT%XPAR_BLD,DTT%LDATA_BLD )
