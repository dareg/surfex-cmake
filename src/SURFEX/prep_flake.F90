!     #########
SUBROUTINE PREP_FLAKE (DTCO, USS, FG, F, FSB, UG, U, &
...
!
 CALL PREP_HOR_FLAKE_FIELD(DTCO, UG, U, USS, ISIZE, F, &
...
IF (.NOT.LCLIM_LAKE) THEN
  ...
  !
  IF (.NOT.GNOVALUE) CALL PREP_HOR_FLAKE_FIELD(DTCO, UG, U, USS, ISIZE, F, &
...
!
F%LSBL = LWAT_SBL
IF (F%LSBL) CALL PREP_FLAKE_SBL(ISIZE, FSB)
!
