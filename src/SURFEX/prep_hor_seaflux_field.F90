!     #########
SUBROUTINE PREP_HOR_SEAFLUX_FIELD(HPROGRAM,HSURF,HATMFILE,HATMFILETYPE,HPGDFILE,HPGDFILETYPE)
!     #################################################################################
!
...
USE MODD_PREP,           ONLY : CINGRID_TYPE, CINTERP_TYPE, XZS_LS, XLAT_OUT, XLON_OUT, &
                                XX_OUT, XY_OUT, CMASK
USE MODD_SEAFLUX_n,      ONLY : XSST, XSSS, XSIC
...
!
 CALL READ_PREP_SEAFLUX_CONF(HPROGRAM,HSURF,YFILE,YFILETYPE,YFILEPGD,YFILEPGDTYPE,&
...
ELSE IF (YFILETYPE=='MESONH' .OR. YFILETYPE=='ASCII ' .OR. YFILETYPE=='LFI   '.OR. YFILETYPE=='FA    ') THEN        
   CALL PREP_SEAFLUX_EXTERN(HPROGRAM,HSURF,YFILE,YFILETYPE,YFILEPGD,YFILEPGDTYPE,ILUOUT,ZFIELDIN)
...
SELECT CASE (HSURF)
 CASE('ZS     ') 
  ...
  XZS_LS(:) = ZFIELDOUT(:,1)
 CASE('SST    ')
  ALLOCATE(XSST(SIZE(ZFIELDOUT,1)))
  XSST(:) = ZFIELDOUT(:,1)
  IF (LSST_DATA) THEN
     ! XSST is derived from array XDATA_SST from MODD_DATA_SEAFLUX, with time interpolation
     CALL PREP_SST_INIT(XSST)
  END IF
  IF (LMERCATOR) THEN
    ! Preparing input for ocean 1D model
    CALL PREP_HOR_OCEAN_FIELDS(HPROGRAM,HSURF,YFILE,YFILETYPE,ILUOUT,GUNIF)
  ENDIF
 CASE('SSS    ')
  ALLOCATE(XSSS(SIZE(ZFIELDOUT,1)))
  XSSS(:) = ZFIELDOUT(:,1)
 CASE('SIC    ')
  ALLOCATE(XSIC(SIZE(ZFIELDOUT,1)))
  XSIC(:) = ZFIELDOUT(:,1)
END SELECT
!
!-------------------------------------------------------------------------------------
