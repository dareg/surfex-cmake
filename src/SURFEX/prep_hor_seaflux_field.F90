...
SUBROUTINE PREP_HOR_SEAFLUX_FIELD (DTCO, UG, U, GCP, DTS, O, OR, KLAT, S, &
                                   HPROGRAM,HSURF,HATMFILE,HATMFILETYPE,HPGDFILE,HPGDFILETYPE,INIV)
!     #################################################################################
!
...
!
INTEGER, INTENT(IN),OPTIONAL :: INIV
!
...
!
IF ((HSURF=='TEMP_OCXX').OR.(HSURF=='SALT_OCXX')) THEN
 NBNIV=NOCKMAX
 WRITE(CNIV,'(I6)')INIV
 CHSURF=trim(HSURF(1:7))//trim(adjustl(CNIV))
ELSE
 CHSURF=HSURF
ENDIF
!
 CALL READ_PREP_SEAFLUX_CONF(O%LMERCATOR, &
                             HPROGRAM,CHSURF,YFILE,YFILETYPE,YFILEPGD,YFILEPGDTYPE,&
...
!
IF (GUNIF) THEN
  CALL PREP_SEAFLUX_UNIF(ILUOUT,CHSURF,ZFIELDIN)
ELSE IF (YFILETYPE=='GRIB  ') THEN
  CALL PREP_GRIB_GRID(YFILE,ILUOUT,CINMODEL,CINGRID_TYPE,CINTERP_TYPE,TZTIME_GRIB)
  IF (NRANK==NPIO) CALL PREP_SEAFLUX_GRIB(HPROGRAM,CHSURF,YFILE,ILUOUT,ZFIELDIN)        
ELSE IF (YFILETYPE=='MESONH' .OR. YFILETYPE=='ASCII ' .OR. YFILETYPE=='LFI   '&
        .OR. YFILETYPE=='FA    '.OR. YFILETYPE=='AROME '.OR.YFILETYPE=='NC    ') THEN        
  CALL PREP_SEAFLUX_EXTERN(GCP,HPROGRAM,HSURF,YFILE,YFILETYPE,YFILEPGD,YFILEPGDTYPE,ILUOUT,ZFIELDIN,CNIV)
ELSE IF (YFILETYPE=='BUFFER') THEN
  CALL PREP_SEAFLUX_BUFFER(HPROGRAM,CHSURF,ILUOUT,ZFIELDIN)
ELSE IF (YFILETYPE=='NETCDF') THEN
  CALL PREP_SEAFLUX_NETCDF(HPROGRAM,CHSURF,YFILE,ILUOUT,ZFIELDIN)
ELSE
...
CASE('SST      ')
  ...
  IF (O%LMERCATOR) THEN
    ...
    CALL PREP_HOR_OCEAN_FIELDS(DTCO, UG, U, GCP, O, OR, KLAT, S%XSEABATHY, &
                               HPROGRAM,CHSURF,YFILE,YFILETYPE,ILUOUT,GUNIF)
  ENDIF
  CASE('TEMP_OCXX')
    IF (.NOT. S%LALLOC_XSEAT) THEN
     ALLOCATE(O%XSEAT(SIZE(ZFIELDOUT,1),NBNIV))
     S%LALLOC_XSEAT=.TRUE.
    ENDIF
    O%XSEAT(:,INIV) = ZFIELDOUT(:,1)
  CASE('SALT_OCXX')
    IF (.NOT. S%LALLOC_XSEAS) THEN
     ALLOCATE(O%XSEAS(SIZE(ZFIELDOUT,1),NBNIV))
     S%LALLOC_XSEAS=.TRUE.
    ENDIF
    O%XSEAS(:,INIV) = ZFIELDOUT(:,1)
CASE('SSS      ')
