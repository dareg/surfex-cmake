...
IF (HSURF=='SN_ROOF') THEN
  ...
  CALL PREP_HOR_SNOW_FIELDS(DTCO, TG, U, &
                            ...
                            SIZE(TG%XLAT),T%TSNOW_ROOF, TOP%TTIME,&
...
ELSE IF (HSURF=='SN_ROAD') THEN
  ...
  CALL PREP_HOR_SNOW_FIELDS(DTCO, TG, U, &
                            ...
                            YFILEPGD, YFILEPGDTYPE,      &                            
                            ILUOUT,GUNIF,1,KPATCH,       &
                            SIZE(TG%XLAT),T%TSNOW_ROAD, TOP%TTIME,&
...
ELSE IF (YFILETYPE=='GRIB  ') THEN
 ...
 ELSE IF (YFILETYPE=='MESONH' .OR. YFILETYPE=='ASCII ' .OR. YFILETYPE=='LFI   '.OR. YFILETYPE=='FA    ') THEN
  CALL PREP_TEB_EXTERN(DTCO, HPROGRAM,HSURF,YFILE,YFILETYPE,YFILEPGD,YFILEPGDTYPE,ILUOUT,KPATCH,ZFIELDIN)
...
#endif
ENDIF
ALLOCATE(ZFIELDOUT(SIZE(TG%XLAT),INL))
!
 CALL HOR_INTERPOL(DTCO, U, ILUOUT,ZFIELDIN,ZFIELDOUT)
!
DEALLOCATE(ZFIELDIN )
!
