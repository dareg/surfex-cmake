...
!
  SELECT CASE (HSURF)
    ...
    CASE('T_ROAD ') 
      ...
      IF (SIZE(ZFIELDOUT,2)/=TOP%NROAD_LAYER) THEN       
    ...
    CASE('T_WALLB')
      ...
      ELSE
        ...
        ELSE
          T%XT_WALL_B(:,:) = ZFIELDOUT(:,:)
        ENDIF        
      END IF  
    CASE('T_ROOF ') 
      ALLOCATE(T%XT_ROOF(SIZE(ZFIELDOUT,1),TOP%NROOF_LAYER))
      IF (SIZE(ZFIELDOUT,2)/=TOP%NROOF_LAYER) THEN 
