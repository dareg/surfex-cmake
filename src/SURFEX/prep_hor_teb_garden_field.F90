!     #########
SUBROUTINE PREP_HOR_TEB_GARDEN_FIELD (DTCO, UG, U, USS, GDR, GDO, GDMX, PLAI, GDIP, TG, TOP,  &
...
IF (HSURF=='SN_VEG ') THEN
  CALL READ_PREP_GARDEN_SNOW(HPROGRAM,GDR%TSNOW%SCHEME,GDR%TSNOW%NLAYER,YFILE_SNOW,&
  ...
  ALLOCATE(ZPATCH(SIZE(GDMX%XVEGTYPE,1),1))
  ALLOCATE(ZVEGTYPE_PATCH (SIZE(GDMX%XVEGTYPE,1),SIZE(GDMX%XVEGTYPE,2),1))
  !
  ZPATCH=1.
  ZVEGTYPE_PATCH(:,:,1) = GDMX%XVEGTYPE(:,:)
  CALL PREP_HOR_SNOW_FIELDS(DTCO, TG, U, &
                            HPROGRAM,HSURF,                 &
                            YFILE,YFILETYPE,                &
                            YFILEPGD, YFILEPGDTYPE,         &
                            ILUOUT,GUNIF_SNOW,1,KPATCH,     &
                            INI,GDR%TSNOW, TOP%TTIME,   &
                            XWSNOW_GD, XRSNOW_GD, XTSNOW_GD,&
                            XLWCSNOW_GD, XASNOW_GD,         &
                            LSNOW_IDEAL_GD, ZSG1SNOW,       &
                            ZSG2SNOW, ZHISTSNOW, XAGESNOW_GD,  &
                            GDMX%XVEGTYPE,ZVEGTYPE_PATCH, ZPATCH )
  DEALLOCATE(ZSG1SNOW)
  DEALLOCATE(ZSG2SNOW)
...
ELSE IF (YFILETYPE=='MESONH' .OR. YFILETYPE=='ASCII ' .OR. YFILETYPE=='LFI   '.OR.YFILETYPE=='FA    ') THEN
   CALL PREP_TEB_GARDEN_EXTERN(DTCO, GDO, U, &
...
!
DO JPATCH = 1, INP
  IF (INP==NVEGTYPE) LINTERP = (GDMX%XVEGTYPE(:,JPATCH) > 0.)
...
IF (1/=INP) THEN
  ...
  DO JLAYER=1,SIZE(ZW,2)
    ZSUM(:) = SUM(GDMX%XVEGTYPE(:,:),2,ZFIELDOUTV(:,JLAYER,:)/=XUNDEF)
    DO JVEGTYPE=1,NVEGTYPE
      WHERE (ZFIELDOUTV(:,JLAYER,JVEGTYPE)/=XUNDEF) 
        ZW(:,JLAYER) = ZW(:,JLAYER) + GDMX%XVEGTYPE(:,JVEGTYPE) * ZFIELDOUTV(:,JLAYER,JVEGTYPE) / ZSUM(:)
      END WHERE
    END DO
    DO JI=1,SIZE(ZW,1)
...
SELECT CASE (HSURF)
  ...
  !
 CASE('WG     ') 
  ALLOCATE(ZF (INI,GDO%NGROUND_LAYER))
  !
  !* interpolates on output levels
  CALL INIT_FROM_REF_GRID(XGRID_SOIL,ZW,GDMX%XDG(:,:,1),ZF)
  !
  !* retrieves soil water content from soil relative humidity
  ALLOCATE(GDR%XWG(INI,GDO%NGROUND_LAYER,1))
  GDR%XWG(:,:,1) = GDIP%XWWILT + ZF(:,:) * (GDIP%XWFC-GDIP%XWWILT)
  GDR%XWG(:,:,1) = MAX(MIN(GDR%XWG(:,:,1),GDIP%XWSAT),XWGMIN)
  !
  WHERE(ZF(:,:)==XUNDEF)GDR%XWG(:,:,1)=XUNDEF
  !
  DEALLOCATE(ZF)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 CASE('WGI    ')
  ...
  ALLOCATE(GDR%XWGI(INI,GDO%NGROUND_LAYER,1))
  GDR%XWGI(:,:,1) = ZF(:,:) * GDIP%XWSAT
  GDR%XWGI(:,:,1) = MAX(MIN(GDR%XWGI(:,:,1),GDIP%XWSAT),0.)
  !
  WHERE(ZF(:,:)==XUNDEF)GDR%XWGI(:,:,1)=XUNDEF
  !
  DEALLOCATE(ZF)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  !
 CASE('TG     ') 
  IWORK=GDO%NGROUND_LAYER
  ALLOCATE(GDR%XTG(INI,IWORK,1))
  ALLOCATE(ZDG(SIZE(GDMX%XDG,1),IWORK))
  IF (GDO%CISBA=='2-L'.OR.GDO%CISBA=='3-L') THEN
    ZDG(:,1) = 0.01
    ZDG(:,2) = 0.40   ! deep temperature for force-restore taken at 20cm
    IF(GDO%CISBA=='3-L') ZDG(:,3) = 5.00   ! climatological temperature, usually not used
  ELSE
    !* diffusion method, the soil grid is the same as for humidity
    ZDG(:,:) = GDMX%XDG(:,:,1)
  END IF
  CALL INIT_FROM_REF_GRID(XGRID_SOIL,ZW,ZDG,GDR%XTG(:,:,1))
  DEALLOCATE(ZDG)
  !
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  !
 CASE('WR     ') 
  ALLOCATE(GDR%XWR(INI,1))
  GDR%XWR(:,1) = ZW(:,1)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  !
...
!
  IF (GDO%CISBA=='2-L' .OR. GDO%CISBA=='3-L') THEN
    ...
    ENDIF
!    
  ELSEIF(GDO%CISBA=='DIF')THEN
       !surface layer (generally 0.01m imposed)
       PT2(:,1) = PT1(:,1) 
       !deep layers
       DO JL=2,GDO%NGROUND_LAYER
          PT2(:,JL) = PT1(:,3)
       END DO
       !if root layers
       DO JI=1,SIZE(PT1,1)
          DO JL=2,GDO%NGROUND_LAYER
             IF(GDMX%XROOTFRAC(JI,JL,1)<=1.0)THEN 
                PT2(JI,JL) = PT1(JI,2)
                EXIT
             ENDIF
          END DO
