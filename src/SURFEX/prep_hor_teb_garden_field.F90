...
IF (HSURF=='SN_VEG ') THEN
  CALL READ_PREP_GARDEN_SNOW(HPROGRAM,TGDR%CUR%TSNOW%SCHEME,TGDR%CUR%TSNOW%NLAYER,YFILE_SNOW,&
  ...
  ALLOCATE(ZPATCH(SIZE(TGDM%X%XVEGTYPE,1),1))
  ALLOCATE(ZVEGTYPE_PATCH (SIZE(TGDM%X%XVEGTYPE,1),SIZE(TGDM%X%XVEGTYPE,2),1))
  !
  ZPATCH=1.
...
ELSE IF (YFILETYPE=='MESONH' .OR. YFILETYPE=='ASCII ' .OR. YFILETYPE=='LFI   '.OR.YFILETYPE=='FA    ') THEN
   CALL PREP_TEB_GARDEN_EXTERN(DTCO, TGDO, U, &
...
!
DO JPATCH = 1, INP
  IF (INP==NVEGTYPE) LINTERP = (TGDM%X%XVEGTYPE(:,JPATCH) > 0.)
...
IF (1/=INP) THEN
  ...
  DO JLAYER=1,SIZE(ZW,2)
    ZSUM(:) = SUM(TGDM%X%XVEGTYPE(:,:),2,ZFIELDOUTV(:,JLAYER,:)/=XUNDEF)
    DO JVEGTYPE=1,NVEGTYPE
      WHERE (ZFIELDOUTV(:,JLAYER,JVEGTYPE)/=XUNDEF) 
        ZW(:,JLAYER) = ZW(:,JLAYER) + TGDM%X%XVEGTYPE(:,JVEGTYPE) * ZFIELDOUTV(:,JLAYER,JVEGTYPE) / ZSUM(:)
      END WHERE
    END DO
    DO JI=1,SIZE(ZW,1)
...
SELECT CASE (HSURF)
  ...
  !
 CASE('WG     ') 
  ALLOCATE(ZF (INI,TGDO%NGROUND_LAYER))
  !
  !* interpolates on output levels
  CALL INIT_FROM_REF_GRID(XGRID_SOIL,ZW,TGDM%X%XDG(:,:,1),ZF)
  !
  !* retrieves soil water content from soil relative humidity
  ALLOCATE(TGDR%CUR%XWG(INI,TGDO%NGROUND_LAYER,1))
  TGDR%CUR%XWG(:,:,1) = TGDIP%XWWILT + ZF(:,:) * (TGDIP%XWFC-TGDIP%XWWILT)
  TGDR%CUR%XWG(:,:,1) = MAX(MIN(TGDR%CUR%XWG(:,:,1),TGDIP%XWSAT),XWGMIN)
  !
  WHERE(ZF(:,:)==XUNDEF)TGDR%CUR%XWG(:,:,1)=XUNDEF
  !
  DEALLOCATE(ZF)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 CASE('WGI    ')
  ...
  ALLOCATE(TGDR%CUR%XWGI(INI,TGDO%NGROUND_LAYER,1))
  TGDR%CUR%XWGI(:,:,1) = ZF(:,:) * TGDIP%XWSAT
  TGDR%CUR%XWGI(:,:,1) = MAX(MIN(TGDR%CUR%XWGI(:,:,1),TGDIP%XWSAT),0.)
  !
  WHERE(ZF(:,:)==XUNDEF)TGDR%CUR%XWGI(:,:,1)=XUNDEF
  !
  DEALLOCATE(ZF)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 CASE('TG     ') 
  ...
  ALLOCATE(TGDR%CUR%XTG(INI,IWORK,1))
  ALLOCATE(ZDG(SIZE(TGDM%X%XDG,1),IWORK))
  IF (TGDO%CISBA=='2-L'.OR.TGDO%CISBA=='3-L') THEN
    ZDG(:,1) = 0.01
    ZDG(:,2) = 0.40   ! deep temperature for force-restore taken at 20cm
    IF(TGDO%CISBA=='3-L') ZDG(:,3) = 5.00   ! climatological temperature, usually not used
  ELSE
    !* diffusion method, the soil grid is the same as for humidity
    ZDG(:,:) = TGDM%X%XDG(:,:,1)
  END IF
  CALL INIT_FROM_REF_GRID(XGRID_SOIL,ZW,ZDG,TGDR%CUR%XTG(:,:,1))
  DEALLOCATE(ZDG)
  !
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  !
 CASE('WR     ') 
  ALLOCATE(TGDR%CUR%XWR(INI,1))
  TGDR%CUR%XWR(:,1) = ZW(:,1)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  !
 CASE('LAI    ') 
  !* LAI is updated only if present and pertinent (evolutive LAI) in input file
   ...
   WHERE (ZW(:,1)/=XUNDEF) TGDM%T%CUR%XLAI(:,1) = ZW(:,1)
  !
END SELECT
!
DEALLOCATE(ZW)
...
!    
  ELSEIF(TGDO%CISBA=='DIF')THEN
       ...
       DO JI=1,SIZE(PT1,1)
          DO JL=2,TGDO%NGROUND_LAYER
             IF(TGDM%X%XROOTFRAC(JI,JL,1)<=1.0)THEN 
                PT2(JI,JL) = PT1(JI,2)
                EXIT
             ENDIF
          END DO
