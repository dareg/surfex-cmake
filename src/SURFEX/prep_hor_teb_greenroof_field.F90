!     #########
SUBROUTINE PREP_HOR_TEB_GREENROOF_FIELD (DTCO, IG, UG, U, USS, &
                                         TGRR, TGRO, TGRM, TGRIP, TG, TOP,  &
...
IF (HSURF=='SN_VEG ') THEN
  CALL READ_PREP_GREENROOF_SNOW(HPROGRAM,TGRR%CUR%TSNOW%SCHEME,TGRR%CUR%TSNOW%NLAYER,YFILE_SNOW,&
  ...
  ALLOCATE(ZPATCH(SIZE(TGRM%X%XVEGTYPE,1),1))
  ALLOCATE(ZVEGTYPE_PATCH (SIZE(TGRM%X%XVEGTYPE,1),SIZE(TGRM%X%XVEGTYPE,2),1))
  !
  ZPATCH=1.
  ZVEGTYPE_PATCH(:,:,1) = TGRM%X%XVEGTYPE(:,:)
...
ELSE IF (YFILETYPE=='MESONH' .OR. YFILETYPE=='ASCII ' .OR. YFILETYPE=='LFI   '.OR. YFILETYPE=='FA    ') THEN
   CALL PREP_TEB_GREENROOF_EXTERN(DTCO, TGRO, U, &
...
!
DO JPATCH = 1, INP
  IF (INP==NVEGTYPE) LINTERP = (TGRM%X%XVEGTYPE(:,JPATCH) > 0.)
...
IF (1/=INP) THEN
  ...
  DO JVEGTYPE=1,NVEGTYPE
    DO JLAYER=1,SIZE(ZW,2)
      ZW(:,JLAYER) = ZW(:,JLAYER) + TGRM%X%XVEGTYPE(:,JVEGTYPE) * ZFIELDOUTV(:,JLAYER,JVEGTYPE)
    END DO
  END DO
...
SELECT CASE (HSURF)
  ...
  !
 CASE('WG     ') 
  ALLOCATE(ZF (INI,TGRO%NGROUND_LAYER))
  !
  !* interpolates on output levels
  CALL INIT_FROM_REF_GRID(XGRID_SOIL,ZW,TGRM%X%XDG(:,:,1),ZF)
  !
  !* retrieves soil water content from soil relative humidity
  ALLOCATE(TGRR%CUR%XWG(INI,TGRO%NGROUND_LAYER,1))
  TGRR%CUR%XWG(:,:,1) = TGRIP%XWWILT + ZF(:,:) * (TGRIP%XWFC-TGRIP%XWWILT)
  TGRR%CUR%XWG(:,:,1) = MAX(MIN(TGRR%CUR%XWG(:,:,1),TGRIP%XWSAT),XWGMIN)
  !
  WHERE(ZF(:,:)==XUNDEF)TGRR%CUR%XWG(:,:,1)=XUNDEF
  !
  DEALLOCATE(ZF)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 CASE('WGI    ')
  ...
  ALLOCATE(TGRR%CUR%XWGI(INI,TGRO%NGROUND_LAYER,1))
  TGRR%CUR%XWGI(:,:,1) = ZF(:,:) * TGRIP%XWSAT
  TGRR%CUR%XWGI(:,:,1) = MAX(MIN(TGRR%CUR%XWGI(:,:,1),TGRIP%XWSAT),0.)
  !
  WHERE(ZF(:,:)==XUNDEF)TGRR%CUR%XWGI(:,:,1)=XUNDEF
  !
  DEALLOCATE(ZF)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  !
 CASE('TG     ') 
  IWORK=TGRO%NGROUND_LAYER
  ALLOCATE(TGRR%CUR%XTG(INI,IWORK,1))
  ALLOCATE(ZDG(SIZE(TGRM%X%XDG,1),IWORK))
  !* diffusion method, the soil grid is the same as for humidity
  ZDG(:,:) = TGRM%X%XDG(:,:,1)
  CALL INIT_FROM_REF_GRID(XGRID_SOIL,ZW,ZDG,TGRR%CUR%XTG(:,:,1))
  DEALLOCATE(ZDG)
  !
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  !
 CASE('WR     ') 
  ALLOCATE(TGRR%CUR%XWR(INI,1))
  TGRR%CUR%XWR(:,1) = ZW(:,1)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  !
 CASE('LAI    ') 
  ...
  WHERE (ZW(:,1)/=XUNDEF) TGRM%T%CUR%XLAI(:,1) = ZW(:,1)
  !
END SELECT
!
DEALLOCATE(ZW)
...
!
       ...
       PT2(:,1) = PT1(:,1) 
       !deep layers
       DO JL=2,TGRO%NGROUND_LAYER
          PT2(:,JL) = PT1(:,3)
       END DO
       !if root layers
       DO JI=1,SIZE(PT1,1)
          DO JL=2,TGRO%NGROUND_LAYER
             IF(TGRM%X%XROOTFRAC(JI,JL,1)<=1.0)THEN 
                PT2(JI,JL) = PT1(JI,2)
                EXIT
             ENDIF
          END DO
