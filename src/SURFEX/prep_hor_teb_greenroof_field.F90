!     #########
SUBROUTINE PREP_HOR_TEB_GREENROOF_FIELD (DTCO, UG, U, USS, GRR, GRO, GRMX, PLAI, GRIP, TG, TOP,  &
...
IF (HSURF=='SN_VEG ') THEN
  CALL READ_PREP_GREENROOF_SNOW(HPROGRAM,GRR%TSNOW%SCHEME,GRR%TSNOW%NLAYER,YFILE_SNOW,&
  ...
  ALLOCATE(ZPATCH(SIZE(GRMX%XVEGTYPE,1),1))
  ALLOCATE(ZVEGTYPE_PATCH (SIZE(GRMX%XVEGTYPE,1),SIZE(GRMX%XVEGTYPE,2),1))
  !
  ZPATCH=1.
...
ELSE IF (YFILETYPE=='MESONH' .OR. YFILETYPE=='ASCII ' .OR. YFILETYPE=='LFI   '.OR. YFILETYPE=='FA    ') THEN
   CALL PREP_TEB_GREENROOF_EXTERN(DTCO, GRO, U, &
...
!
DO JPATCH = 1, INP
  IF (INP==NVEGTYPE) LINTERP = (GRMX%XVEGTYPE(:,JPATCH) > 0.)
...
IF (1/=INP) THEN
  ...
  DO JVEGTYPE=1,NVEGTYPE
    DO JLAYER=1,SIZE(ZW,2)
      ZW(:,JLAYER) = ZW(:,JLAYER) + GRMX%XVEGTYPE(:,JVEGTYPE) * ZFIELDOUTV(:,JLAYER,JVEGTYPE)
    END DO
  END DO
...
SELECT CASE (HSURF)
  ...
  !
 CASE('WG     ') 
  ALLOCATE(ZF (INI,GRO%NGROUND_LAYER))
  !
  !* interpolates on output levels
  CALL INIT_FROM_REF_GRID(XGRID_SOIL,ZW,GRMX%XDG(:,:,1),ZF)
  !
  !* retrieves soil water content from soil relative humidity
  ALLOCATE(GRR%XWG(INI,GRO%NGROUND_LAYER,1))
  GRR%XWG(:,:,1) = GRIP%XWWILT + ZF(:,:) * (GRIP%XWFC-GRIP%XWWILT)
  GRR%XWG(:,:,1) = MAX(MIN(GRR%XWG(:,:,1),GRIP%XWSAT),XWGMIN)
  !
  WHERE(ZF(:,:)==XUNDEF)GRR%XWG(:,:,1)=XUNDEF
  !
  DEALLOCATE(ZF)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 CASE('WGI    ')
  ...
  ALLOCATE(GRR%XWGI(INI,GRO%NGROUND_LAYER,1))
  GRR%XWGI(:,:,1) = ZF(:,:) * GRIP%XWSAT
  GRR%XWGI(:,:,1) = MAX(MIN(GRR%XWGI(:,:,1),GRIP%XWSAT),0.)
  !
  WHERE(ZF(:,:)==XUNDEF)GRR%XWGI(:,:,1)=XUNDEF
  !
  DEALLOCATE(ZF)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 CASE('TG     ') 
  ...
  ALLOCATE(GRR%XTG(INI,IWORK,1))
  ALLOCATE(ZDG(SIZE(GRMX%XDG,1),IWORK))
  !* diffusion method, the soil grid is the same as for humidity
  ZDG(:,:) = GRMX%XDG(:,:,1)
  CALL INIT_FROM_REF_GRID(XGRID_SOIL,ZW,ZDG,GRR%XTG(:,:,1))
  DEALLOCATE(ZDG)
  !
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  !
 CASE('WR     ') 
  ALLOCATE(GRR%XWR(INI,1))
  GRR%XWR(:,1) = ZW(:,1)
  !
  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  !
...
!
       ...
       PT2(:,1) = PT1(:,1) 
       !deep layers
       DO JL=2,GRO%NGROUND_LAYER
          PT2(:,JL) = PT1(:,3)
       END DO
       !if root layers
       DO JI=1,SIZE(PT1,1)
          DO JL=2,GRO%NGROUND_LAYER
             IF(GRMX%XROOTFRAC(JI,JL,1)<=1.0)THEN 
                PT2(JI,JL) = PT1(JI,2)
                EXIT
             ENDIF
          END DO
