!     #########
SUBROUTINE PREP_HOR_TEB_GREENROOF_FIELD (DTCO, UG, U, USS, IO, S, K, P, PEK, TG, TOP,  &
...
IF (HSURF=='SN_VEG ') THEN
  ...
  CALL PREP_HOR_SNOW_FIELDS(DTCO, TG, U, HPROGRAM,HSURF,    &
                            YFILE,YFILETYPE,                &
...
ELSE IF (YFILETYPE=='MESONH' .OR. YFILETYPE=='ASCII ' .OR. YFILETYPE=='LFI   '.OR. YFILETYPE=='FA    ') THEN
   CALL PREP_TEB_GREENROOF_EXTERN(DTCO, IO, U, &
