...
#define MPI_SFX_REAL MPI_DOUBLE_PRECISION
   ...
   DO JP = 1,IO%NPATCH
    IF (NPROC>1) THEN
#ifdef SFX_MPI
      CALL MPI_ALLREDUCE(MAXVAL(SUM(NPE%AL(JP)%TSNOW%WSNOW,DIM=2)), ZSMAX, 1, MPI_SFX_REAL, MPI_MAX, NCOMM, INFOMPI)
#endif
    ELSE
    ...
    ELSE
      ...
      ELSE
        DO JL=1,NPE%AL(JP)%TSNOW%NLAYER
          IF (NPROC>1) THEN
#ifdef SFX_MPI
            CALL MPI_ALLREDUCE(MAXVAL(NPE%AL(JP)%TSNOW%WSNOW(:,JL)), ZSMAXJL, 1, MPI_SFX_REAL, MPI_MAX, NCOMM, INFOMPI)
#endif
          ELSE
