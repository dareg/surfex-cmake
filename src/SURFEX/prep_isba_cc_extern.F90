...
YRECFM='PHOTO'
CALL READ_SURF(HFILEPGDTYPE,YRECFM,YPHOTO,IRESP)  
YRECFM='PATCH_NUMBER'
CALL READ_SURF(HFILEPGDTYPE,YRECFM,IPATCH,IRESP)
CALL CLOSE_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE)
!
IF(IVERSION<7.OR.(IVERSION==7.AND.IBUGFIX<=3).OR.(YPHOTO/='NIT'.AND.YPHOTO/='NCB'))THEN
  OPREP_AGS = .FALSE.
...
!
  CASE('BIOMASS')
     CALL OPEN_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE,'NATURE')
     YRECFM='NBIOMASS'
     CALL READ_SURF(HFILEPGDTYPE,YRECFM,INBIOMASS,IRESP)
     CALL CLOSE_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE)
     IF (YPHOTO=='NIT' .OR. YPHOTO=='NCB') THEN
       ...
       DO JNBIOMASS=1,INBIOMASS
          ...
          CALL READ_SURF(HFILETYPE,YRECFM,PFIELD(:,JNBIOMASS,:),IRESP,HDIR='A')
       ENDDO
...
!
  CASE('LITTER')
     CALL OPEN_AUX_IO_SURF(HFILE,HFILETYPE,'NATURE')
     YRECFM='RESPSL'
     CALL READ_SURF(HFILETYPE,YRECFM,YRESPSL,IRESP)
     IF(YRESPSL=='CNT')THEN
       YRECFM='NLITTER'
       CALL READ_SURF(HFILETYPE,YRECFM,INLITTER,IRESP)
       YRECFM='NLITTLEVS'
       CALL READ_SURF(HFILETYPE,YRECFM,INLITTLEVS,IRESP)
       ...
       DO JNLITTER=1,INLITTER
          DO JNLITTLEVS=1,INLITTLEVS
             ...
             CALL READ_SURF(HFILETYPE,YRECFM,PFIELD(:,IWORK,:),IRESP,HDIR='A')
          ENDDO
       ENDDO
     ELSE
       OPREP_AGS = .FALSE.
...
!
  CASE('SOILCARB')
     ...
     IF(YRESPSL=='CNT')THEN
       YRECFM='NSOILCARB'
       CALL READ_SURF(HFILETYPE,YRECFM,INSOILCARB,IRESP)
       ...
       DO JNSOILCARB=1,INSOILCARB
          ...
          CALL READ_SURF(HFILETYPE,YRECFM,PFIELD(:,JNSOILCARB,:),IRESP,HDIR='A')
       ENDDO
     ELSE
       OPREP_AGS = .FALSE.
     ENDIF    
  ...
  CASE('LIGNIN')
     ...
     IF(YRESPSL=='CNT')THEN
       ...
       DO JNLITTLEVS=1,INLITTLEVS
          ...
          CALL READ_SURF(HFILETYPE,YRECFM,PFIELD(:,JNLITTLEVS,:),IRESP,HDIR='A')
       ENDDO
     ELSE
       OPREP_AGS = .FALSE.
     ENDIF    
