...
YRECFM='PHOTO'
CALL READ_SURF(HFILETYPE,YRECFM,YPHOTO,IRESP)  
YRECFM='PATCH_NUMBER'
...
!
  CASE('BIOMASS')
     CALL OPEN_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE,'NATURE')
     YRECFM='NBIOMASS'
     CALL READ_SURF(HPROGRAM,YRECFM,INBIOMASS,IRESP)
     CALL CLOSE_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE)
...
!
  CASE('LITTER')
     CALL OPEN_AUX_IO_SURF(HFILE,HFILETYPE,'NATURE')
     YRECFM='RESPSL'
     CALL READ_SURF(HPROGRAM,YRECFM,YRESPSL,IRESP)
     IF(YRESPSL=='CNT')THEN
       YRECFM='NLITTER'
       CALL READ_SURF(HPROGRAM,YRECFM,INLITTER,IRESP)
       YRECFM='NLITTLEVS'
       CALL READ_SURF(HPROGRAM,YRECFM,INLITTLEVS,IRESP)
       ...
       DO JNLITTER=1,INLITTER
          DO JNLITTLEVS=1,INLITTLEVS
             ...
             CALL READ_SURF(HPROGRAM,YRECFM,PFIELD(:,IWORK,:),IRESP,HDIR='A')
          ENDDO
       ENDDO
     ELSE
       OPREP_AGS = .FALSE.
...
!
  CASE('SOILCARB')
     ...
     IF(YRESPSL=='CNT')THEN
       YRECFM='NSOILCARB'
       CALL READ_SURF(HPROGRAM,YRECFM,INSOILCARB,IRESP)
       ...
       DO JNSOILCARB=1,INSOILCARB
          ...
          CALL READ_SURF(HPROGRAM,YRECFM,PFIELD(:,JNSOILCARB,:),IRESP,HDIR='A')
       ENDDO
     ELSE
       OPREP_AGS = .FALSE.
     ENDIF    
  ...
  CASE('LIGNIN')
     ...
     IF(YRESPSL=='CNT')THEN
       ...
       DO JNLITTLEVS=1,INLITTLEVS
          ...
          CALL READ_SURF(HPROGRAM,YRECFM,PFIELD(:,JNLITTLEVS,:),IRESP,HDIR='A')
       ENDDO
     ELSE
       OPREP_AGS = .FALSE.
     ENDIF    
