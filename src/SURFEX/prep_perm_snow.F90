...
!
  IF(I%O%LGLACIER.AND.TPSNOW%NLAYER>=6)THEN
    WHERE(GWORK(:,1))
...
!
  DO JLAYER=1,IWORK
     WHERE(PPERM_SNOW_FRAC(:,NVT_SNOW)>0.0)
           I%R%XWGI(:,JLAYER,KSNOW) = MAX(I%R%XWGI(:,JLAYER,KSNOW),ZWAT(:,JLAYER)*ZPSN(:))
           I%R%XWG (:,JLAYER,KSNOW) = MIN(I%R%XWG (:,JLAYER,KSNOW),MAX(I%IP%XWSAT(:,JLAYER)-I%R%XWGI(:,JLAYER,KSNOW),XWGMIN))
     END WHERE
     WHERE(I%R%XWG(:,JLAYER,KSNOW) /= XUNDEF .AND. (I%R%XWG(:,JLAYER,KSNOW) + I%R%XWGI(:,JLAYER,KSNOW)) > I%IP%XWSAT(:,JLAYER) )
           I%R%XWGI(:,JLAYER,KSNOW) = I%IP%XWSAT(:,JLAYER)-I%R%XWG (:,JLAYER,KSNOW) !WGT<=WSAT
     END WHERE
  ENDDO
!
ENDIF
