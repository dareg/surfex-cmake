  ...
  CASE('ZS       ')
    SELECT CASE (CINMODEL)
      CASE ('ECMWF ','ARPEGE','ALADIN','MOCAGE','HIRLAM','RACMO ')
        ...
        DEALLOCATE(ZFIELD)
      CASE DEFAULT
        CALL ABOR1_SFX('PREP_SEAFLUX_GRIB (ZS): UNSUPPORTED MODEL ' // TRIM(CINMODEL))
    END SELECT
  ...
  CASE('SST      ')
    SELECT CASE (CINMODEL)
      CASE ('ECMWF ','ARPEGE','ALADIN','MOCAGE','HIRLAM','RACMO ')
        ...
        DEALLOCATE(ZFIELD)
      CASE DEFAULT
        CALL ABOR1_SFX('PREP_SEAFLUX_GRIB (SST): UNSUPPORTED MODEL ' // TRIM(CINMODEL))
    END SELECT
!
!* 3.  Sea ice fraction
!      -------------------------------------
  ...
  CASE('SIC    ')
    SELECT CASE (CINMODEL)
      CASE ('ECMWF ','ARPEGE','ALADIN','MOCAGE','HIRLAM','RACMO ')
        ...
        DEALLOCATE(ZFIELD)
      CASE DEFAULT
        CALL ABOR1_SFX('PREP_SEAFLUX_GRIB (SIC): UNSUPPORTED MODEL ' // TRIM(CINMODEL))
    END SELECT
!
