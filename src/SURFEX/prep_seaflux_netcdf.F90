...
IF (NPROC>1) THEN
#ifdef SFX_MPI
  IF (LSFX_MPI) CALL MPI_BCAST(CINTERP_TYPE,LEN(CINTERP_TYPE),MPI_CHARACTER,NPIO,NCOMM,INFOMPI)
#endif
  IF (TRIM(CINTERP_TYPE)=="UNIF") THEN
    IF (NRANK/=NPIO) ALLOCATE(PFIELD(1,1))
#ifdef SFX_MPI
    IF (LSFX_MPI) CALL MPI_BCAST(PFIELD(1:1,1:1),KIND(PFIELD)/4,MPI_REAL,NPIO,NCOMM,INFOMPI)
#endif
