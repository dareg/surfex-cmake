!     #########
SUBROUTINE PREP_TEB_GARDEN_EXTERN (DTCO, I, U, &
...
CALL READ_SURF(&
               HFILEPGDTYPE,'VERSION',IVERSION,IRESP,HDIR='-')
CALL READ_SURF(&
               HFILEPGDTYPE,'BUG',IBUGFIX,IRESP,HDIR='-')
...
CALL TOWN_PRESENCE(&
                   HFILEPGDTYPE,GTEB,HDIR='-')
!
IF (GTEB) THEN
  CALL CLOSE_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE)
  CALL OPEN_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE,'TOWN  ')
  CALL READ_TEB_PATCH(&
                      HFILEPGD,HFILEPGDTYPE,IVERSION,IBUGFIX,ITEB_PATCH,HDIR='-')
  YPATCH='   '
...
!* choice if one reads garden fields (if present) or ISBA fields
    ...
    IF (GTEB) CALL READ_SURF(&
               HFILEPGDTYPE,'GARDEN',GGARDEN,IRESP,HDIR='A')
    CALL CLOSE_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE)
    IF (GGARDEN) THEN
      YSURF = 'GD_'//HSURF(1:3)
...
!
  CASE('WR     ')
     ...
     GGARDEN = .FALSE.
     IF (GTEB) CALL READ_SURF(&
               HFILEPGDTYPE,'GARDEN',GGARDEN,IRESP,HDIR='-')
     CALL CLOSE_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE)
     IF (GGARDEN) THEN
       IPATCH = 1    
       YRECFM = 'GD_WR'
