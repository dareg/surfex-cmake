  ...
  CASE('ZS     ')
    SELECT CASE (CINMODEL)
      CASE ('ECMWF ','ARPEGE','ALADIN','MOCAGE','HIRLAM','RACMO ')
        ...
        DEALLOCATE(ZFIELD1D)
      CASE DEFAULT
        CALL ABOR1_SFX('PREP_TEB_GRIB (ZS): UNSUPPORTED MODEL ' // TRIM(CINMODEL))
    END SELECT
!
...
!
  CASE('T_ROAD')
     ...
     SELECT CASE(CINMODEL)
       ...
       CASE DEFAULT
         CALL ABOR1_SFX('PREP_TEB_GRIB (T_ROAD): UNSUPPORTED MODEL ' // TRIM(CINMODEL))
     END SELECT
...
!              --------------------------------
  ...
  CASE('T_FLOOR')    
     ...
     SELECT CASE(CINMODEL)
       ...
       CASE DEFAULT
         CALL ABOR1_SFX('PREP_TEB_GRIB (T_FLOOR): UNSUPPORTED MODEL ' // TRIM(CINMODEL))
     END SELECT
     !* if deep road temperature is prescribed
     IF (XTI_ROAD/=XUNDEF) THEN
       ZFIELD(:,2:) = XTI_ROAD 
  ...
  CASE('T_WIN1')
    SELECT CASE (CINMODEL)
      CASE ('ECMWF ','ARPEGE','ALADIN','MOCAGE','HIRLAM','RACMO ')
        ...
        DEALLOCATE(ZFIELD1D)
      CASE DEFAULT
        CALL ABOR1_SFX('PREP_TEB_GRIB (T_WIN1): UNSUPPORTED MODEL ' // TRIM(CINMODEL))
    END SELECT
!
  ...
  CASE('T_CAN  ')
    SELECT CASE (CINMODEL)
      CASE ('ECMWF ','ARPEGE','ALADIN','MOCAGE','HIRLAM','RACMO ')
        ...
        DEALLOCATE(ZFIELD1D)
      CASE DEFAULT
        CALL ABOR1_SFX('PREP_TEB_GRIB (T_CAN): UNSUPPORTED MODEL ' // TRIM(CINMODEL))
    END SELECT
!
  ...
  CASE('Q_CAN  ')
    SELECT CASE (CINMODEL)
      CASE ('ECMWF ','ARPEGE','ALADIN','MOCAGE','HIRLAM','RACMO ')
        ...
        PFIELD(:,1) = 0.01
      CASE DEFAULT
        CALL ABOR1_SFX('PREP_TEB_GRIB (Q_CAN): UNSUPPORTED MODEL ' // TRIM(CINMODEL))
    END SELECT
