...
T%XTI_ROAD = T%XTI_ROAD  + XT_CLIM_GRAD  * (TOP%XZS - XZS_LS)
!
!*      1.4    Road Temperature profile
!
DO JL=1,SIZE(T%XT_ROAD,2)
  T%XT_ROAD(:,JL) = T%XT_ROAD(:,JL) + XT_CLIM_GRAD  * (TOP%XZS - XZS_LS)
END DO
!
...
DO JL=1,SIZE(T%XT_WALL_A,2)
  T%XT_WALL_A(:,JL) = T%XT_WALL_A(:,JL) + XT_CLIM_GRAD  * (TOP%XZS - XZS_LS) &
                                     * MAX(1.-2.*ZGRID(:,JL)/ZD(:),0.)  
  T%XT_WALL_B(:,JL) = T%XT_WALL_B(:,JL) + XT_CLIM_GRAD  * (TOP%XZS - XZS_LS) &
                                     * MAX(1.-2.*ZGRID(:,JL)/ZD(:),0.)  
END DO
!
DEALLOCATE(ZD)
...
DO JL=1,SIZE(T%XT_ROOF,2)
  T%XT_ROOF(:,JL) = T%XT_ROOF(:,JL) + XT_CLIM_GRAD  * (TOP%XZS - XZS_LS) &
                                   * MAX(1.-2.*ZGRID(:,JL)/ZD(:),0.)  
END DO
!
DEALLOCATE(ZD)
...
IF (TOP%CBEM=='BEM') THEN
  ...
  DO JL=1,SIZE(B%XT_FLOOR,2)
    B%XT_FLOOR(:,JL) = B%XT_FLOOR(:,JL) + XT_CLIM_GRAD  * (TOP%XZS - XZS_LS) &
  ...
  DO JL=1,SIZE(B%XT_MASS,2)
    B%XT_MASS(:,JL) = B%XT_MASS(:,JL) + XT_CLIM_GRAD  * (TOP%XZS - XZS_LS) &
                                   * MAX(2.*ZGRID(:,JL)/ZD(:)-1.,0.)
  END DO
  !
  DEALLOCATE(ZD)
...
!*      1.7    Snow variables
!
 CALL PREP_VER_SNOW(T%TSNOW_ROOF,XZS_LS,TOP%XZS)
 CALL PREP_VER_SNOW(T%TSNOW_ROAD,XZS_LS,TOP%XZS)
!
!
...
ZT_LS = T%XT_CANYON
!
T%XT_CANYON = T%XT_CANYON  + XT_CLIM_GRAD  * (TOP%XZS - XZS_LS)
!
