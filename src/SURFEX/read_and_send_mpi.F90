...
!$OMP SINGLE
  ...
  DO I=1,NPROC
    ...
    IF (I<NPROC) THEN
      XTIME0 = MPI_WTIME()
      CALL MPI_SEND(XWORK2(:,1:IP2),SIZE(XWORK2,1)*IP2*KIND(XWORK2)/4,MPI_REAL,I,IDX_R,NCOMM,INFOMPI)
      XTIME_COMM_READ = XTIME_COMM_READ + (MPI_WTIME() - XTIME0)
    ENDIF
#endif
    !
...
!$OMP SINGLE
  ...
  !
  XTIME0 = MPI_WTIME()
  CALL MPI_RECV(XWORK2(:,1:IP2),SIZE(XWORK2,1)*IP2*KIND(XWORK2)/4,MPI_REAL,NPIO,IDX_R,NCOMM,ISTATUS,INFOMPI)
  XTIME_COMM_READ = XTIME_COMM_READ + (MPI_WTIME() - XTIME0)
#endif
  !
!$OMP END SINGLE
...
USE MODD_SURFEX_MPI, ONLY : NINDEX, NPROC, NRANK, NCOMM, NPIO, NSIZE, &
                            XTIME_NPIO_READ, XTIME_COMM_READ, IDX_R, WLOG_MPI
USE MODD_SURFEX_OMP, ONLY : NINDX1, NINDX2, XWORK3
!
USE MODI_PACK_SAME_RANK
!
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK
...
!$OMP SINGLE
!
IF (IP2>IW2 .OR. IP3>IW3) THEN
  DEALLOCATE(XWORK3)
  ALLOCATE(XWORK3(NSIZE,MAX(IP2,IW2),MAX(IP3,IW3)))
ENDIF
!
!$OMP END SINGLE
!
...
!$OMP SINGLE
  ...
  DO I=1,NPROC
    ...
    IF (I<NPROC) THEN
      XTIME0 = MPI_WTIME()
      CALL MPI_SEND(XWORK3(:,1:IP2,1:IP3),SIZE(XWORK3,1)*IP2*IP3*KIND(XWORK3)/4,MPI_REAL,I,IDX_R,NCOMM,INFOMPI)
      XTIME_COMM_READ = XTIME_COMM_READ + (MPI_WTIME() - XTIME0)
    ENDIF
#endif
    !
...
!$OMP SINGLE
  ...
  !
  XTIME0 = MPI_WTIME()  
  CALL MPI_RECV(XWORK3(:,1:IP2,1:IP3),SIZE(XWORK3,1)*IP2*IP3*KIND(XWORK3)/4,MPI_REAL,NPIO,IDX_R,NCOMM,ISTATUS,INFOMPI)
  XTIME_COMM_READ = XTIME_COMM_READ + (MPI_WTIME() - XTIME0)
#endif
  !
!$OMP END SINGLE
