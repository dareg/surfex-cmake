...
USE MODD_SURFEX_OMP, ONLY : NINDX1SFX, NINDX2SFX, NBLOCK
!
...
!$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(I,ICPT,J,IREQ,INFOMPI,ZHOOK_HANDLE_OMP)
  DO I=0,NPROC-1
    ...
    IF (I/=IPIO) THEN
      ...
      IF (PRESENT(KPIO)) THEN !.AND.PRESENT(KDX)) THEN
        IF (I<IPIO) THEN
          IREQ = I+1
        ELSE
          IREQ = I
        ENDIF
        CALL MPI_ISEND(IWORKS(:,I+1),SIZE(IWORKR)*KIND(IWORKS)/4,&
                           MPI_INTEGER,I,IDX,NCOMM,NREQ(IREQ),INFOMPI)
      ELSE    
        CALL MPI_SEND(IWORKS(:,I+1),SIZE(IWORKR)*KIND(IWORKR)/4,MPI_INTEGER,I,IDX,NCOMM,INFOMPI)
      ENDIF
...
!$OMP END PARALLEL DO
  ! 
  IF (PRESENT(KPIO).AND..NOT.PRESENT(KDX)) CALL MPI_WAITALL(NPROC-1,NREQ(1:NPROC-1),ISTATUS2,INFOMPI)
  !
ELSE
  ! 
  IDX = IDX + 1
...
!
  ...
  !IF (PRESENT(KPIO).AND..NOT.PRESENT(KDX)) CALL MPI_WAITALL(NPROC-1,NREQ(1:NPROC-1),ISTATUS,INFOMPI)
IF (PRESENT(KMASK)) THEN
  CALL PACK_SAME_RANK(KMASK,IWORKR(NINDX1SFX:NINDX2SFX),KWORK2)
ELSE
  KWORK2(:) = IWORKR(NINDX1SFX:NINDX2SFX)
ENDIF
!
...
!$OMP PARALLEL DO PRIVATE(I,ICPT,J,INFOMPI,ZHOOK_HANDLE_OMP)  
  DO I=0,NPROC-1
...
!IF (PRESENT(KPIO).AND..NOT.PRESENT(KDX)) CALL MPI_WAITALL(NPROC-1,NREQ(1:NPROC-1),ISTATUS,INFOMPI)
IF (PRESENT(KMASK)) THEN
  CALL PACK_SAME_RANK(KMASK,IWORK3(NINDX1SFX:NINDX2SFX,:,:),KWORK2)
ELSE
  KWORK2(:,:,:) = IWORK3(NINDX1SFX:NINDX2SFX,:,:)
ENDIF
!
IF (.NOT.PRESENT(KDX)) IDX_R = IDX_R + 1
!
    ...
    IF (I/=IPIO) THEN
      ...
      IF (PRESENT(KPIO).AND.PRESENT(KDX)) THEN
        ...
        ELSE
          IREQ = I
        ENDIF
        CALL MPI_ISEND(ZWORKS(:,I+1),SIZE(ZWORKS(:,I+1))*KIND(ZWORKS)/4,MPI_REAL,I,IDX,NCOMM,NREQ(IREQ),INFOMPI)
      ELSE
        CALL MPI_SEND(ZWORKS(:,I+1),SIZE(ZWORKS(:,I+1))*KIND(ZWORKS)/4,MPI_REAL,I,IDX,NCOMM,INFOMPI)
      ENDIF
      XTIME_COMM_READ = XTIME_COMM_READ + (MPI_WTIME() - XTIME0)
    ELSE
      ZWORKR(:) = ZWORKS(:,I+1)
...
!IF (PRESENT(KPIO).AND..NOT.PRESENT(KDX)) CALL MPI_WAITALL(NPROC-1,NREQ(1:NPROC-1),ISTATUS,INFOMPI)
IF (PRESENT(KMASK)) THEN
  CALL PACK_SAME_RANK(KMASK,ZWORKR(NINDX1SFX:NINDX2SFX),PWORK2(:))
ELSE
  PWORK2(:) = ZWORKR(NINDX1SFX:NINDX2SFX)
ENDIF
...
!IF (PRESENT(KPIO).AND..NOT.PRESENT(KDX)) CALL MPI_WAITALL(NPROC-1,NREQ(1:NPROC-1),ISTATUS,INFOMPI)
IF (PRESENT(KMASK)) THEN
  CALL PACK_SAME_RANK(KMASK,ZWORK2(NINDX1SFX:NINDX2SFX,:),PWORK2(:,:))
ELSE
  PWORK2(:,:) = ZWORK2(NINDX1SFX:NINDX2SFX,:)
ENDIF
!
IF (.NOT.PRESENT(KDX)) IDX_R = IDX_R + 1
!
...
!
IF (PRESENT(KMASK)) THEN
  CALL PACK_SAME_RANK(KMASK,ZWORK3(NINDX1SFX:NINDX2SFX,:,:),PWORK2)
ELSE
  PWORK2(:,:,:) = ZWORK3(NINDX1SFX:NINDX2SFX,:,:)
ENDIF
!
IF (.NOT.PRESENT(KDX)) IDX_R = IDX_R + 1
!
