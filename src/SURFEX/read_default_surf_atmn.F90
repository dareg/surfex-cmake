!     #########
      SUBROUTINE READ_DEFAULT_SURF_ATM_n (CHU, DUO, USS, HPROGRAM)
!     #######################################################
!
...
IF (IMI.NE.-1 .AND. LNAM_READ) THEN
 ...
 CALL INIT_NAM_DIAG_SURF_ATMn(DUO)
 CALL INIT_NAM_DIAG_SURFn(DUO)
 CALL INIT_NAM_WRITE_DIAG_SURFn(DUO)
ENDIF
! 
IF (LNAM_READ) THEN
 !
...
IF (IMI.NE.-1) THEN
 ...
 CALL UPDATE_NAM_DIAG_SURF_ATMn(DUO)
 CALL UPDATE_NAM_DIAG_SURFn(DUO)
 CALL UPDATE_NAM_WRITE_DIAG_SURFn(DUO)
ENDIF
