!     #########
      SUBROUTINE READ_DEFAULT_TEB_VEG_n (CHT, TVO, HPROGRAM)
!     #######################################################
!
...
IF (IMI.NE.-1 .AND. LNAM_READ) THEN
 CALL INIT_NAM_TEB_VEGn(TVO)
 CALL INIT_NAM_TEB_VEG_AGSn(TVO)
 CALL INIT_NAM_CH_CONTROLn(CHT)
 CALL INIT_NAM_CH_TEB_VEGn(CHT)
 CALL INIT_NAM_SGH_TEB_VEGn(TVO)
ENDIF
!
IF (LNAM_READ) THEN
 !
...
!
IF (IMI.NE.-1) THEN
 CALL UPDATE_NAM_TEB_VEGn(TVO)
 CALL UPDATE_NAM_TEB_VEG_AGSn(TVO)
 CALL UPDATE_NAM_CH_CONTROLn(CHT)
 CALL UPDATE_NAM_CH_TEB_VEGn(CHT)
 CALL UPDATE_NAM_SGH_TEB_VEGn(TVO)
ENDIF
