!     #########
      SUBROUTINE READ_DEFAULT_TEB_n (CHT, DGMTO, DTO, DGUT, TGRO, T, TOP, HPROGRAM)
!     #######################################################
!
...
IF (IMI.NE.-1 .AND. LNAM_READ) THEN
 ...
 CALL INIT_NAM_DIAG_SURFn(DTO, TOP)
 CALL INIT_NAM_DIAG_TEBn(DGMTO, DTO, DGUT)
...
IF (IMI.NE.-1) THEN
 ...
 CALL UPDATE_NAM_DIAG_SURFn(DTO)
 CALL UPDATE_NAM_DIAG_TEBn(DGMTO, DTO, DGUT)
