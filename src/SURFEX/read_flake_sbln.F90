!     #########
      SUBROUTINE READ_FLAKE_SBL_n (DTCO, U, OSBL, FSB, HPROGRAM)
!     #########################################
!
...
!
YRECFM='WAT_SBL_LVL'
 CALL READ_SURF(HPROGRAM,YRECFM,FSB%NLVL,IRESP)
!
!*       2.     Prognostic fields:
!               -----------------
!
...
DO JLAYER=1,FSB%NLVL
  ...
  CALL READ_SURF(HPROGRAM,YRECFM,FSB%XZ(:,JLAYER),IRESP)
END DO
!
...
ALLOCATE(FSB%XLMO(ILU,FSB%NLVL))     
...
IF(YREAD=='ALL') THEN
  ...
  DO JLAYER=1,FSB%NLVL
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,FSB%XU(:,JLAYER),IRESP)
  END DO
  !
  !* theta in SBL
  DO JLAYER=1,FSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'WAT_SBL_T',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,FSB%XT(:,JLAYER),IRESP)
  END DO
  !
  !* humidity in SBL
  DO JLAYER=1,FSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'WAT_SBL_Q',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,FSB%XQ(:,JLAYER),IRESP)
  END DO
  !
  !* Tke in SBL
  DO JLAYER=1,FSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'WAT_SBL_E',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,FSB%XTKE(:,JLAYER),IRESP)
  END DO
  !
  !* Monin-Obhukov length
  YRECFM='WAT_SBL_LMO     '
  CALL READ_SURF(HPROGRAM,YRECFM,FSB%XLMO(:,1),IRESP)
  DO JLAYER = 2,FSB%NLVL
    FSB%XLMO(:,JLAYER) = FSB%XLMO(:,1)
  ENDDO    
  !
  !* Pressure
  DO JLAYER=1,FSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'WAT_SBL_P',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,FSB%XP(:,JLAYER),IRESP)
  END DO
  !
ELSE
  FSB%XU  (:,:) = XUNDEF
