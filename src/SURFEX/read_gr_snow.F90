...
!
ALLOCATE(ZWORK(KLU))
IF (.NOT.GDIM .AND. INPATCH>1) ALLOCATE(ZWORK2(KLU,INPATCH))
!
IF (TPSNOW%SCHEME=='1-L' .OR. TPSNOW%SCHEME=='D95' .OR. TPSNOW%SCHEME=='EBA' &
  ...
  IF (GDIM .OR. INPATCH<=1) THEN
    CALL READ_SURF(HPROGRAM,YRECFM,ZWORK,IRESP,HDIR=YDIR)
    CALL PACK_SAME_RANK(KMASK_P,ZWORK,TPSNOW%ALB(:))
  ELSE
    CALL READ_SURF(HPROGRAM,YRECFM,ZWORK2,IRESP,HDIR=YDIR)
    CALL PACK_SAME_RANK(KMASK_P,ZWORK2(:,KPATCH),TPSNOW%ALB(:))
  ENDIF
  !
ENDIF
!
DEALLOCATE(ZWORK)
IF (.NOT.GDIM .AND. INPATCH>1) DEALLOCATE(ZWORK2)
!
IF (LHOOK) CALL DR_HOOK('READ_GR_SNOW_1',1,ZHOOK_HANDLE)
!
IF (TPSNOW%SCHEME=='1-L' .OR. TPSNOW%SCHEME=='D95' .OR. TPSNOW%SCHEME=='EBA' &
...
!$OMP DO PRIVATE(JI,JLAYER)
  DO JI = 1,SIZE(TPSNOW%WSNOW,1)
    ...
    IF (TPSNOW%WSNOW(JI,1) == 0.0 ) THEN
      ...
      DO JLAYER = 1,TPSNOW%NLAYER
        !
        TPSNOW%RHO(JI,JLAYER)=XUNDEF
        IF (TPSNOW%SCHEME=='1-L') THEN
          TPSNOW%T(JI,JLAYER) = XUNDEF
        ELSEIF (TPSNOW%SCHEME=='3-L' .OR. TPSNOW%SCHEME=='CRO') THEN
          TPSNOW%HEAT(JI,JLAYER) = XUNDEF
          IF (TPSNOW%SCHEME=='CRO') THEN
            TPSNOW%HIST (JI,JLAYER) = XUNDEF
            TPSNOW%GRAN1(JI,JLAYER) = XUNDEF
            TPSNOW%GRAN2(JI,JLAYER) = XUNDEF
            TPSNOW%AGE  (JI,JLAYER) = XUNDEF
          ENDIF
        ENDIF
        !
      ENDDO
...
YNLAYER='I1.1'
IF (KPATCH>=10) THEN
  WRITE(YPATCH,'(I2.2)') KPATCH
ELSEIF (KPATCH>0) THEN
  WRITE(YPATCH,'(I1.1)') KPATCH
ENDIF
!
DO JLAYER = 1,KNL
  !
...
!
  ...
  ELSE
    WRITE(YRECFM,YFMT) TRIM(HREC),'_',TRIM(HSURF),JLAYER
  ENDIF
  IF (KPATCH>0 .AND. GDIM) YRECFM = ADJUSTL(YRECFM(:LEN_TRIM(YRECFM)))//"P"//ADJUSTL(YPATCH(:LEN_TRIM(YPATCH)))
  IF (OVERSION) YRECFM=ADJUSTL(TRIM(HPREF)//YRECFM)
  !
  IF (GDIM .OR. INPATCH<=1) THEN
    CALL READ_SURF(HPROGRAM,YRECFM,ZWORK,IRESP,HDIR=HDIRIN)
    CALL PACK_SAME_RANK(KMASK_P,ZWORK,PTAB(:,JLAYER))
  ELSE
    CALL READ_SURF(HPROGRAM,YRECFM,ZWORK2,IRESP,HDIR=HDIRIN)
    CALL PACK_SAME_RANK(KMASK_P,ZWORK2(:,KPATCH),PTAB(:,JLAYER))
  ENDIF
  !
ENDDO
!
