...
IF (TPSNOW%SCHEME=='1-L' .OR. TPSNOW%SCHEME=='D95' .OR. TPSNOW%SCHEME=='EBA' &
  ...
  ELSE
    ...
    CALL PACK_SAME_RANK(KMASK_P,ZWORK(:,KPATCH),TPSNOW%ALB(:))
  ENDIF
  !
ENDIF
!
...
!
  ...
  !
  IF (GDIM) THEN
    CALL READ_SURF(HPROGRAM,YRECFM,ZWORK(:,1),IRESP,HDIR=HDIRIN)
    CALL PACK_SAME_RANK(KMASK_P,ZWORK(:,1),PTAB(:,JLAYER))
  ELSE
    CALL READ_SURF(HPROGRAM,YRECFM,ZWORK,IRESP,HDIR=HDIRIN)
    CALL PACK_SAME_RANK(KMASK_P,ZWORK(:,MAX(1,KPATCH)),PTAB(:,JLAYER))
  ENDIF
  !
ENDDO
!
