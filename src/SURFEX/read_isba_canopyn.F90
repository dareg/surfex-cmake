!     #########
      SUBROUTINE READ_ISBA_CANOPY_n (DTCO, ICP, OCANOPY, U, HPROGRAM)
!     #########################################
!
...
ELSE
  YRECFM='ISBA_CANOPY'
  CALL READ_SURF(HPROGRAM,YRECFM,OCANOPY,IRESP)
END IF
!
IF (.NOT.OCANOPY) THEN
  ALLOCATE(ICP%XZ  (0,0))
...
!
YRECFM='ISBA_CAN_LVL'
 CALL READ_SURF(HPROGRAM,YRECFM,ICP%NLVL,IRESP)
!
!*       2.     Prognostic fields:
!               -----------------
!
...
DO JLAYER=1,ICP%NLVL
  ...
  CALL READ_SURF(HPROGRAM,YRECFM,ICP%XZ(:,JLAYER),IRESP)
END DO
!
...
ALLOCATE(ICP%XLMO(ILU,ICP%NLVL))
...
IF(YREAD=='ALL') THEN
  ...
  DO JLAYER=1,ICP%NLVL
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,ICP%XU(:,JLAYER),IRESP)
  END DO
  !
  !* theta in SBL
  DO JLAYER=1,ICP%NLVL
    WRITE(YRECFM,'(A10,I2.2)') 'ISBA_CAN_T',JLAYER
    CALL READ_SURF(HPROGRAM,YRECFM,ICP%XT(:,JLAYER),IRESP)
  END DO
  !
  !* humidity in SBL
  DO JLAYER=1,ICP%NLVL
    WRITE(YRECFM,'(A10,I2.2)') 'ISBA_CAN_Q',JLAYER
    CALL READ_SURF(HPROGRAM,YRECFM,ICP%XQ(:,JLAYER),IRESP)
  END DO
  !
  !* Tke in SBL
  DO JLAYER=1,ICP%NLVL
    WRITE(YRECFM,'(A10,I2.2)') 'ISBA_CAN_E',JLAYER
    CALL READ_SURF(HPROGRAM,YRECFM,ICP%XTKE(:,JLAYER),IRESP)
  END DO
  !
  !* Monin-Obhukov length
  YRECFM='ISBA_CAN_LMO     '
  CALL READ_SURF(HPROGRAM,YRECFM,ICP%XLMO(:,1),IRESP)
  DO JLAYER = 2,ICP%NLVL
    ICP%XLMO(:,JLAYER) = ICP%XLMO(:,1)
  ENDDO  
  !
  !* Pressure
  DO JLAYER=1,ICP%NLVL
    WRITE(YRECFM,'(A10,I2.2)') 'ISBA_CAN_P',JLAYER
    CALL READ_SURF(HPROGRAM,YRECFM,ICP%XP(:,JLAYER),IRESP)
  END DO
  !
ELSE
  ICP%XU  (:,:) = XUNDEF
