...
IF (LNAM_READ) THEN
 ...
 CALL POSNAM(INAM,'NAM_CROCUSN',GFOUND,ILUOUT)
 IF (GFOUND) READ(UNIT=INAM,NML=NAM_CROCUSn) 
 !
...
IF (IMI.NE.-1) THEN
 ...
 CALL UPDATE_NAM_SPINUP_CARB_ISBAn
 CALL UPDATE_NAM_CROCUSn 
ENDIF
!
!-------------------------------------------------------------------------------
!
