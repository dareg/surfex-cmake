!     #########
      SUBROUTINE READ_ISBA_CONF_n (CHI, DGEI, DIO, DGMI, I, &
                                   HPROGRAM)
!     #######################################################
!
...
TYPE(ISBA_OPTIONS_t), INTENT(INOUT) :: I
!
...
IF (IMI.NE.-1 .AND. LNAM_READ) THEN
 ...
 CALL INIT_NAM_DIAG_ISBAn(DGEI, DIO, DGMI)
 CALL INIT_NAM_DIAG_SURFn(DIO)
...
IF (IMI.NE.-1) THEN
 ...
 CALL UPDATE_NAM_DIAG_ISBAn(DGEI, DIO, DGMI)
 CALL UPDATE_NAM_DIAG_SURFn(DIO)
