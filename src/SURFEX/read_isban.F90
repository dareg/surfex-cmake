...
!
!
USE MODD_SURFEX_MPI, ONLY : WLOG_MPI, NWG_SIZE
...
DO JL=1,IWORK
  ...
  CALL READ_SURF(HPROGRAM,YRECFM,XTG(:,JL,:),IRESP)
END DO
!
!
...
DO JL=1,IWORK
  ...
  YRECFM='WG'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
   CALL READ_SURF(HPROGRAM,YRECFM,XWG(:,JL,:),IRESP)
END DO
!
DO JL=1,IWORK
  WRITE(YLVL,'(I4)') JL
  YRECFM='WGI'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
  CALL READ_SURF(HPROGRAM,YRECFM,XWGI(:,JL,:),IRESP)
END DO
!
...
IF (CRESPSL=='CNT') THEN
  ...
  DO JNLITTER=1,NNLITTER
    DO JNLITTLEVS=1,NNLITTLEVS
      ...
      CALL READ_SURF(HPROGRAM,YRECFM,XLITTER(:,JNLITTER,JNLITTLEVS,:),IRESP)
    END DO
  END DO
  ...
  DO JNSOILCARB=1,NNSOILCARB
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,XSOILCARB(:,JNSOILCARB,:),IRESP)
  END DO
!
  XLIGNIN_STRUC(:,:,:) = 0.
  DO JNLITTLEVS=1,NNLITTLEVS
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,XLIGNIN_STRUC(:,JNLITTLEVS,:),IRESP)
  END DO
!
ENDIF
!
