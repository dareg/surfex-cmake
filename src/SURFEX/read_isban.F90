...
IF ( TRIM(CASSIM_ISBA)=="ENKF") THEN
  DO JP = 1,IO%NPATCH
    ALLOCATE(NP%AL(JP)%XRED_NOISE(NP%AL(JP)%NSIZE_P,NVAR))
    NP%AL(JP)%XRED_NOISE(:,:) = 0.
  ENDDO
...
!
DO JP = 1,IO%NPATCH
  ALLOCATE(NPE%AL(JP)%XTG(NP%AL(JP)%NSIZE_P,IWORK))
  NPE%AL(JP)%XTG(:,:)=XUNDEF
ENDDO
!
DO JL=1,IWORK
  WRITE(YLVL,'(I4)') JL
  ...
  !
  DO JP = 1,IO%NPATCH
    CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NPE%AL(JP)%XTG(:,JL))
  ENDDO
  !
ENDDO
!
...
IF ( TRIM(CASSIM_ISBA)=="EKF" .AND. LPRT ) THEN
  ...
  ! read in control variable
    IF ( (TRIM(CVAR(NIVAR))=="TG1" .AND. JL==1) .OR. &
      ...
      DO JP = 1,IO%NPATCH
        WHERE ( NPE%AL(JP)%XTG(:,JL)/=XUNDEF )
          NPE%AL(JP)%XTG(:,JL) = NPE%AL(JP)%XTG(:,JL) + XTPRT(NIVAR)*NPE%AL(JP)%XTG(:,JL)
        ENDWHERE
      ENDDO
    ENDIF
  END DO
...
!
DO JP = 1,IO%NPATCH
  ALLOCATE(NPE%AL(JP)%XWG (NP%AL(JP)%NSIZE_P,IO%NGROUND_LAYER))
  ALLOCATE(NPE%AL(JP)%XWGI(NP%AL(JP)%NSIZE_P,IO%NGROUND_LAYER))
  !
  NPE%AL(JP)%XWG (:,:)=XUNDEF
  NPE%AL(JP)%XWGI(:,:)=XUNDEF
  !
ENDDO
!
DO JL=1,IO%NGROUND_LAYER
  ...
  !
  DO JP = 1,IO%NPATCH
    CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NPE%AL(JP)%XWG(:,JL))
  ENDDO
  !
ENDDO
!
...
IF ( TRIM(CASSIM_ISBA)=="EKF" .AND. LPRT ) THEN
   ...
   DO JL=1,IO%NGROUND_LAYER
     ...
     IF ( (TRIM(CVAR(NIVAR))=="WG1" .AND. JL==1) .OR. & 
       ...
       DO JP = 1,IO%NPATCH
         WHERE ( NPE%AL(JP)%XWG(:,JL)/=XUNDEF )
           NPE%AL(JP)%XWG(:,JL) = NPE%AL(JP)%XWG(:,JL) + XTPRT(NIVAR)*NPE%AL(JP)%XWG(:,JL)
         ENDWHERE
       ENDDO
     ENDIF
     !
...
DO JL=1,IWORK
  ...
  !
  DO JP = 1,IO%NPATCH
    CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NPE%AL(JP)%XWGI(:,JL))
  ENDDO
  !
ENDDO
!
...
IF (IO%CPHOTO=='LAI' .OR. IO%CPHOTO=='LST' .OR. IO%CPHOTO=='NIT' .OR. IO%CPHOTO=='NCB') THEN
  ...
  IF ( TRIM(CASSIM_ISBA)=="EKF" .AND. LPRT ) THEN
    ...
    IF ( TRIM(CVAR(NIVAR))=="LAI" ) THEN
      DO JP = 1,IO%NPATCH
        WHERE ( NPE%AL(JP)%XLAI(:)/=XUNDEF ) 
           NPE%AL(JP)%XLAI(:) =  NPE%AL(JP)%XLAI(:) + XTPRT(NIVAR)* NPE%AL(JP)%XLAI(:)
        ENDWHERE
      ENDDO
    ENDIF
    !
...
DO JP = 1,IO%NPATCH
  ...
  ENDIF
  CALL READ_GR_SNOW(HPROGRAM,'VEG','     ',ILU,NP%AL(JP)%NSIZE_P,NP%AL(JP)%NR_P,JP,NPE%AL(JP)%TSNOW  )
ENDDO
!
IF(IO%LGLACIER)THEN
  DO JP = 1,IO%NPATCH
...
!
DO JP = 1,IO%NPATCH
  ALLOCATE(NPE%AL(JP)%XRESA(NP%AL(JP)%NSIZE_P))
  NPE%AL(JP)%XRESA(:) = 100.
  ALLOCATE(NPE%AL(JP)%XLE  (NP%AL(JP)%NSIZE_P))
  NPE%AL(JP)%XLE(:) = 0.      
  IF (IO%CPHOTO/='NON') THEN
    ALLOCATE(NPE%AL(JP)%XANFM  (NP%AL(JP)%NSIZE_P))
    ALLOCATE(NPE%AL(JP)%XAN    (NP%AL(JP)%NSIZE_P))
    ALLOCATE(NPE%AL(JP)%XANDAY (NP%AL(JP)%NSIZE_P))
    NPE%AL(JP)%XANFM (:) = XANFMINIT  
    NPE%AL(JP)%XAN   (:) = 0.
    NPE%AL(JP)%XANDAY(:) = 0.      
  END IF
  !
  IF(IO%CPHOTO/='NON') THEN
    ALLOCATE(NPE%AL(JP)%XBIOMASS         (NP%AL(JP)%NSIZE_P,IO%NNBIOMASS))
    ALLOCATE(NPE%AL(JP)%XRESP_BIOMASS    (NP%AL(JP)%NSIZE_P,IO%NNBIOMASS))
    NPE%AL(JP)%XBIOMASS(:,:) = 0.
    NPE%AL(JP)%XRESP_BIOMASS(:,:) = 0.    
  END IF
ENDDO
!
!
...
IF (IO%CPHOTO=='LAI' .OR. IO%CPHOTO=='LST') THEN
  !
  DO JP = 1,IO%NPATCH
    NPE%AL(JP)%XBIOMASS(:,1) = NPE%AL(JP)%XBSLAI(:) * NPE%AL(JP)%XLAI(:)
  ENDDO
  !
ELSEIF (IO%CPHOTO=='NIT'.OR.IO%CPHOTO=='NCB') THEN
  ...
  DO JNBIOMASS=1,IO%NNBIOMASS
    ...
    ENDIF     
    DO JP = 1,IO%NPATCH
      CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NPE%AL(JP)%XBIOMASS(:,JNBIOMASS))
    ENDDO 
  END DO
!
  IWORK=0
...
!
  DO JNBIOMASS=2,IO%NNBIOMASS-IWORK
    ...
    ENDIF  
    DO JP = 1,IO%NPATCH
      CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NPE%AL(JP)%XRESP_BIOMASS(:,JNBIOMASS))
    ENDDO
  END DO
  !
ENDIF
...
IF (IO%CRESPSL=='CNT') THEN
  !
  DO JP = 1,IO%NPATCH
    ALLOCATE(NPE%AL(JP)%XLITTER      (NP%AL(JP)%NSIZE_P,IO%NNLITTER,IO%NNLITTLEVS))
    ALLOCATE(NPE%AL(JP)%XSOILCARB    (NP%AL(JP)%NSIZE_P,IO%NNSOILCARB))
    ALLOCATE(NPE%AL(JP)%XLIGNIN_STRUC(NP%AL(JP)%NSIZE_P,IO%NNLITTLEVS))
    !
    NPE%AL(JP)%XLITTER(:,:,:) = 0.
    NPE%AL(JP)%XSOILCARB(:,:) = 0. 
    NPE%AL(JP)%XLIGNIN_STRUC(:,:) = 0.  
    !
  ENDDO
  !
...
!
DO JP = 1,IO%NPATCH
  ALLOCATE(NPE%AL(JP)%XSNOWFREE_ALB     (NP%AL(JP)%NSIZE_P))
  ALLOCATE(NPE%AL(JP)%XSNOWFREE_ALB_VEG (NP%AL(JP)%NSIZE_P))
  ALLOCATE(NPE%AL(JP)%XSNOWFREE_ALB_SOIL(NP%AL(JP)%NSIZE_P))
ENDDO
!
    ...
    IF (XADDINFL(IVAR)>0.) THEN
      ...
      IF (LASSIM .OR. (.NOT.LENS_GEN .AND. XADDTIMECORR(IVAR)>0.)) THEN
        ...
        IF (.NOT.LASSIM) THEN
          ...
          DO JP = 1,IO%NPATCH
            DO JI = 1,NP%AL(JP)%NSIZE_P
              ZWHITE_NOISE = XADDINFL(IVAR)*PCOFSWI(NP%AL(JP)%NR_P(JI))*RANDOM_NORMAL()
              CALL ADD_NOISE(XADDTIMECORR(IVAR),XASSIM_WINH,ZWHITE_NOISE,NP%AL(JP)%XRED_NOISE(JI,IVAR))
            ENDDO
          ENDDO
          !
      ...
      ELSE
        ...
        DO JP = 1,IO%NPATCH
          DO JI = 1,NP%AL(JP)%NSIZE_P
            NP%AL(JP)%XRED_NOISE(JI,IVAR) = XADDINFL(IVAR)*PCOFSWI(NP%AL(JP)%NR_P(JI))*RANDOM_NORMAL()
          ENDDO
        ENDDO
        !
        ZCOEF = 1. 
