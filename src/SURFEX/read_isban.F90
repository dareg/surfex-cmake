...
!
YRECFM = 'WR'
 CALL READ_SURF(HPROGRAM,YRECFM,I%XWR(:,:),IRESP)
!
!* Leaf Area Index
!
IF (I%CPHOTO=='LAI' .OR. I%CPHOTO=='LST' .OR. I%CPHOTO=='NIT' .OR. I%CPHOTO=='NCB') THEN
  YRECFM = 'LAI'
  CALL READ_SURF(HPROGRAM,YRECFM,I%XLAI(:,:),IRESP)
  IF ( LPRT ) THEN
    ! read in control variable
...
IF(I%LGLACIER)THEN
  ...
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=2) THEN
    YRECFM = 'ICE_STO'
    CALL READ_SURF(HPROGRAM,YRECFM,I%XICE_STO(:,:),IRESP)
  ELSE
    I%XICE_STO(:,:) = 0.0
  ENDIF
ELSE
...
!
  ...
  YRECFM = 'WRVN'
  CALL READ_SURF(HPROGRAM,YRECFM,I%XWRVN(:,:),IRESP)
!
...
!
  ...
  YRECFM = 'TC'
  CALL READ_SURF(HPROGRAM,YRECFM,I%XTC(:,:),IRESP)
!
!* vegetation canopy air specific humidity
!
  ALLOCATE(I%XQC(ILU,I%NPATCH))
  YRECFM = 'QC'
  CALL READ_SURF(HPROGRAM,YRECFM,I%XQC(:,:),IRESP)
!
ENDIF
!
!-------------------------------------------------------------------------------
...
YRECFM = 'RESA'
I%XRESA(:,:) = 100.
 CALL READ_SURF(HPROGRAM,YRECFM,I%XRESA(:,:),IRESP)
!
...
ELSE
  YRECFM='TSRAD_NAT'
  CALL READ_SURF(HPROGRAM,YRECFM,I%XTSRAD_NAT(:),IRESP)
ENDIF
!
I%XLE(:,:) = XUNDEF
!
...
IF (I%CPHOTO/='NON') THEN
  YRECFM = 'AN'
  I%XAN(:,:) = 0.
  CALL READ_SURF(HPROGRAM,YRECFM,I%XAN(:,:),IRESP)
  !
  YRECFM = 'ANDAY'
  I%XANDAY(:,:) = 0.
  CALL READ_SURF(HPROGRAM,YRECFM,I%XANDAY(:,:),IRESP)
  !
  YRECFM = 'ANFM'
  I%XANFM(:,:) = XANFMINIT
  CALL READ_SURF(HPROGRAM,YRECFM,I%XANFM(:,:),IRESP)
  !
  YRECFM = 'LE_AGS'
  I%XLE(:,:) = 0.
  CALL READ_SURF(HPROGRAM,YRECFM,I%XLE(:,:),IRESP)
END IF
!
