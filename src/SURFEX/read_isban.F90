...
IF ( LASSIM ) THEN
  ! Diagnostic fields for assimilation used as first guess
...
!       YRECFM='HU2M_ISBA'  ! Not yet compatible with CANARI first guess
    ...
    IF ( .NOT. ALLOCATED(XAZON10M_ISBA)) ALLOCATE(XAZON10M_ISBA(ILU,IO%NPATCH))
    XAZON10M_ISBA=XUNDEF
    YRECFM='ZON10M'
    IF (( .NOT. IO%LCANOPY) .AND. IO%NPATCH > 1 ) THEN
!       YRECFM='ZON10M_P'  ! Not yet compatible with CANARI first guess
      YRECFM='ZON10M'
      CALL READ_SURF(HPROGRAM,YRECFM,XAZON10M_ISBA(:,1),IRESP)
      DO JP=2,IO%NPATCH
        XAZON10M_ISBA(:,JP)=XAZON10M_ISBA(:,1)
      END DO
    ELSE
      YRECFM='ZON10M'
      CALL READ_SURF(HPROGRAM,YRECFM,XAZON10M_ISBA(:,1),IRESP)
    ...
    IF ( .NOT. ALLOCATED(XAMER10M_ISBA)) ALLOCATE(XAMER10M_ISBA(ILU,IO%NPATCH))
    XAMER10M_ISBA=XUNDEF
    IF (( .NOT. IO%LCANOPY) .AND. IO%NPATCH > 1 ) THEN
!       YRECFM='MER10M_P'  ! Not yet compatible with CANARI first guess
      YRECFM='MER10M'
      !CALL IO_BUFF(YRECFM,'R',GKNOWN)
      CALL READ_SURF(HPROGRAM,YRECFM,XAMER10M_ISBA(:,1),IRESP)
      DO JP=2,IO%NPATCH
        XAMER10M_ISBA(:,JP)=XAMER10M_ISBA(:,1)
      END DO
    ELSE
      YRECFM='MER10M'
      CALL READ_SURF(HPROGRAM,YRECFM,XAMER10M_ISBA(:,1),IRESP)
