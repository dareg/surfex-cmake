...
!
YRECFM='VERSION'
 CALL READ_SURF(HPROGRAM,YRECFM,IVERSION,IRESP,YDIR)
YRECFM='BUG'
 CALL READ_SURF(HPROGRAM,YRECFM,IBUGFIX,IRESP,YDIR)
!
IF (IVERSION<1 .OR. (IVERSION==1 .AND. IBUGFIX==0)) THEN
  OECOCLIMAP = .TRUE.
ELSE
  YRECFM='ECOCLIMAP'
  CALL READ_SURF(HPROGRAM,YRECFM,OECOCLIMAP,IRESP,YDIR)
END IF
!
...
ELSE
  YRECFM='ECOSG'
  CALL READ_SURF(HPROGRAM,YRECFM,OECOSG,IRESP,YDIR)
END IF
!
