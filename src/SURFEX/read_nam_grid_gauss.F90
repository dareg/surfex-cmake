!################################################################
SUBROUTINE READ_NAM_GRID_GAUSS(UG,U,HPROGRAM,KGRID_PAR,KL,PGRID_PAR,HDIR)
!################################################################
!
...
ELSE
  !
  ALLOCATE(ZLON0(U%NDIM_FULL),ZLAT0(U%NDIM_FULL),ZLON_XY0(U%NDIM_FULL),ZLAT_XY0(U%NDIM_FULL),&
           ZMESH_SIZE0(U%NDIM_FULL),ZLONINF0(U%NDIM_FULL),ZLATINF0(U%NDIM_FULL),&
           ZLONSUP0(U%NDIM_FULL),ZLATSUP0(U%NDIM_FULL))
  !
 CALL GET_GRIDTYPE_GAUSS(UG%XGRID_FULL_PAR,INLATI)
  !
  ALLOCATE(INLOPA(INLATI))  
  !
  CALL GET_GRIDTYPE_GAUSS(UG%XGRID_FULL_PAR,KNLATI=INLATI,PLAPO=ZLAPO,&
