...
YRECFM='SIZE_WATER'
 CALL GET_TYPE_DIM_n('WATER ',FG%NDIM)
!
!
!
...
ALLOCATE(F%LCOVER(JPCOVER))
 CALL READ_LCOVER(HPROGRAM,F%LCOVER)
!
ALLOCATE(F%XCOVER(FG%NDIM,JPCOVER))
 CALL READ_SURF_COV(HPROGRAM,'COVER',F%XCOVER(:,:),F%LCOVER,IRESP)
!
!* orography
!
ALLOCATE(F%XZS(FG%NDIM))
YRECFM='ZS'
 CALL READ_SURF(HPROGRAM,YRECFM,F%XZS(:),IRESP)
!
!* latitude, longitude 
!
...
ALLOCATE(FG%XMESH_SIZE(FG%NDIM))
 CALL READ_GRID(HPROGRAM,FG%CGRID,FG%XGRID_PAR,FG%XLAT,FG%XLON,FG%XMESH_SIZE,IRESP)
!
!* FLake parameters
!
