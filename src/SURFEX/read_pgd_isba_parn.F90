...
IF (.NOT.OLAND_USE) THEN
  ...
  IF (IVERSION>=7) THEN
    ...
    YRECFM='L_Z0GV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_Z0GV,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_Z0GV) LDATA_MIXPAR = .TRUE.
    YRECFM='L_EMIS'
    YCOMMENT=YRECFM
...
!
  IF (LDATA_Z0GV) THEN
    ...
    DO JTIME=1,ITIME
      WRITE(YRECFM,FMT='(A8,I2.2)') 'D_Z0GV_T',JTIME
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_Z0GV(:,JTIME,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
    END DO
  ENDIF
!
