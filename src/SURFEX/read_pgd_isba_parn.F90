...
IF (.NOT.OLAND_USE) THEN
  ...
  IF (IVERSION>=7) THEN
    ...
    YRECFM='L_VEGGV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_VEGGV,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_VEGGV) LDATA_MIXPAR = .TRUE.
    YRECFM='L_RSMIN'
    YCOMMENT=YRECFM
...
!
  IF (LDATA_VEGGV) THEN
    ...
    DO JTIME=1,ITIME
      WRITE(YRECFM,FMT='(A7,I2.2)') 'D_VEGGV',JTIME
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_VEGGV(:,JTIME,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
    END DO
  ENDIF
!
