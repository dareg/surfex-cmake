...
USE MODD_ISBA_n,         ONLY : LECOCLIMAP, NGROUND_LAYER, LMEB_PATCH
USE MODD_DATA_ISBA_n,    ONLY : NTIME, XPAR_VEG, XPAR_LAI,XPAR_RSMIN,XPAR_GAMMA,XPAR_WRMAX_CF, &
                                ...
                                XPAR_ROOT_EXTINCTIONGV, XPAR_ROOT_DEPTHGV,        &
...
ENDIF
!
IF ((IVERSION<8.AND..NOT.LECOCLIMAP).OR.ISIZE_LMEB_PATCH==0.0) THEN
  !
  LDATA_GNDLITTER=.FALSE.
  LDATA_ZF_TALLVEG=.FALSE.
  LDATA_RGLGV=.FALSE.
  LDATA_GAMMAGV=.FALSE.
  LDATA_RSMINGV=.FALSE.
  LDATA_ROOTFRACGV=.FALSE.
  LDATA_WRMAX_CFGV=.FALSE.
  LDATA_LAIGV=.FALSE.
  LDATA_Z0LITTER=.FALSE.
  LDATA_H_VEG=.FALSE.
  LDATA_ROOT_DEPTHGV=.FALSE.
  LDATA_ROOT_EXTINCTIONGV=.FALSE.
  !
ENDIF
!
IF (.NOT.OLAND_USE) THEN
  ...
  IF (IVERSION>=8.AND.ISIZE_LMEB_PATCH>0) THEN
    ...
    YRECFM='L_LAIGV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_LAIGV,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_LAIGV) LDATA_MIXPAR = .TRUE.
    YRECFM='L_H_VEG'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_H_VEG,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_H_VEG) LDATA_MIXPAR = .TRUE.
    YRECFM='L_Z0LITTER'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_Z0LITTER,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_Z0LITTER) LDATA_MIXPAR = .TRUE.
    YRECFM='L_GNDLITTER'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_GNDLITTER,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_GNDLITTER) LDATA_MIXPAR = .TRUE.
    YRECFM='L_RSMINGV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_RSMINGV,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_RSMINGV) LDATA_MIXPAR = .TRUE.
    YRECFM='L_GAMMAGV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_GAMMAGV,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_GAMMAGV) LDATA_MIXPAR = .TRUE.
    YRECFM='L_WRMAX_CFGV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_WRMAX_CFGV,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_WRMAX_CFGV) LDATA_MIXPAR = .TRUE.
    YRECFM='L_RGLGV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_RGLGV,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_RGLGV) LDATA_MIXPAR = .TRUE.
    YRECFM='L_ZF_TALLVEG'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ZF_TALLVEG,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_ZF_TALLVEG) LDATA_MIXPAR = .TRUE.
    YRECFM='L_RTFRACGV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ROOTFRACGV,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_ROOTFRACGV) LDATA_MIXPAR = .TRUE.
    YRECFM='L_RT_DEPTHGV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ROOT_DEPTHGV,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_ROOT_DEPTHGV) LDATA_MIXPAR = .TRUE.
    YRECFM='L_RT_EXTGV'
    YCOMMENT=YRECFM
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ROOT_EXTINCTIONGV,IRESP,HCOMMENT=YCOMMENT)
    IF (LDATA_ROOT_EXTINCTIONGV) LDATA_MIXPAR = .TRUE.    
    !
  ELSE
    !
    LDATA_GNDLITTER=.FALSE.
...
!
  IF (LDATA_LAIGV) THEN
    ...
    DO JTIME=1,ITIME
      ...
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_LAIGV(:,JTIME,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
    END DO
  ENDIF
!
  IF (LDATA_H_VEG) THEN
    ...
    DO JTIME=1,ITIME
      ...
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_H_VEG(:,JTIME,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
    END DO
  ENDIF
!
  IF (LDATA_GNDLITTER) THEN
    ALLOCATE(XPAR_GNDLITTER(NDIM,NTIME,NVEGTYPE))
    DO JTIME=1,ITIME
      WRITE(YRECFM,FMT='(A7,I2.2)') 'D_GNDLITTER',JTIME
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_GNDLITTER(:,JTIME,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
    END DO
  ENDIF
!
  ...
  ENDIF
!
  IF (LDATA_Z0LITTER) THEN
    ALLOCATE(XPAR_Z0LITTER        (NDIM,NTIME,NVEGTYPE))
    DO JTIME=1,ITIME
      WRITE(YRECFM,FMT='(A8,I2.2)') 'D_Z0LITTER_T',JTIME
      CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_Z0LITTER(:,JTIME,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
    END DO
  ENDIF
!
...
!
  ...
  ENDIF
!
  IF (LDATA_RSMINGV) THEN
    ALLOCATE(XPAR_RSMINGV     (NDIM,NVEGTYPE))
    YRECFM='D_RSMINGV'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_RSMINGV(:,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
  ENDIF
!
  IF (LDATA_GAMMA) THEN
  ...
  ENDIF
!
  IF (LDATA_GAMMAGV) THEN
    ALLOCATE(XPAR_GAMMAGV     (NDIM,NVEGTYPE))
    YRECFM='D_GAMMAGV'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_GAMMAGV(:,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
  ENDIF
!
  ...
  ENDIF
!
  IF (LDATA_WRMAX_CFGV) THEN
    ALLOCATE(XPAR_WRMAX_CFGV  (NDIM,NVEGTYPE))
    YRECFM='D_WRMAX_CFGV'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_WRMAX_CFGV(:,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
  ENDIF
!
  IF (LDATA_RGL) THEN
...
!
  IF (LDATA_RGLGV) THEN
    ...
    YRECFM='D_RGLGV'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_RGLGV(:,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
  ENDIF
!
  IF (LDATA_ZF_TALLVEG) THEN
    ALLOCATE(XPAR_ZF_TALLVEG       (NDIM,NVEGTYPE))
    YRECFM='D_ZF_TALLVEG'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_ZF_TALLVEG(:,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
  ENDIF
!
  IF (LDATA_CV) THEN
...
!
  ...
  ENDIF
!
  IF (LDATA_ROOTFRACGV) THEN
    ALLOCATE(XPAR_ROOTFRACGV  (NDIM,NGROUND_LAYER,NVEGTYPE))
    ALLOCATE(ZWORK(SIZE(XPAR_ROOTFRACGV,1),SIZE(XPAR_ROOTFRACGV,3)))
    DO JLAYER=1,SIZE(XPAR_ROOTFRACGV,2)
      IF (JLAYER<10)  WRITE(YRECFM,FMT='(A10,I1.1)') 'D_RTFRACGV',JLAYER
      IF (JLAYER>=10) WRITE(YRECFM,FMT='(A10,I2.2)') 'D_RTFRACGV',JLAYER
        CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,ZWORK,IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
      DO JPATCH=1,SIZE(XPAR_ROOTFRACGV,3)
        XPAR_ROOTFRACGV(:,JLAYER,JPATCH) = ZWORK(:,JPATCH)
      END DO
    END DO
    DEALLOCATE(ZWORK)
  ENDIF
...
!
  ...
  ENDIF
!
  IF (LDATA_ROOT_DEPTHGV) THEN
    ALLOCATE(XPAR_ROOT_DEPTHGV(NDIM,NVEGTYPE))
    YRECFM='D_RT_DEPTHGV'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_ROOT_DEPTHGV(:,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
  ENDIF
!
  ...
  ENDIF
!
  IF (LDATA_ROOT_EXTINCTIONGV) THEN
    ALLOCATE(XPAR_ROOT_EXTINCTIONGV(NDIM,NVEGTYPE))
    YRECFM='D_RT_EXTGV'
    CALL READ_SURF_ISBA_PAR_n(HPROGRAM,YRECFM,ILUOUT,KSIZE,XPAR_ROOT_EXTINCTIONGV(:,:),IRESP,IVERSION,&
                                HCOMMENT=YCOMMENT,HDIR=YDIR)
  ENDIF
