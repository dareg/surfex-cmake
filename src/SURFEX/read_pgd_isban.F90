...
IF(I%CISBA=='DIF') THEN
  ...
  IF (IVERSION>8 .OR. (IVERSION==8 .AND. IBUGFIX>=1)) THEN
    CALL READ_SURF(HPROGRAM,"L_SOILGRID",GSOILGRID,IRESP)
  ELSE
    GSOILGRID = .TRUE.
  ENDIF  
  IF (GSOILGRID) THEN
    IF (IVERSION>=8) THEN
      DO JLAYER=1,I%NGROUND_LAYER
        ...
        CALL READ_SURF(HPROGRAM,YRECFM,I%XSOILGRID(JLAYER),IRESP)
      ENDDO    
    ELSEIF (IVERSION==7 .AND. IBUGFIX>=2) THEN
      YRECFM='SOILGRID'
      CALL READ_SURF(HPROGRAM,YRECFM,I%XSOILGRID,IRESP,HDIR='-')
    ELSE
