!     #########
      SUBROUTINE READ_PGD_ISBA_n (CHI, DTCO, DTI, DTZ, GB, IG, ISS, IO, IP, &
...
!
ALLOCATE(IP%XCLAY(IG%NDIM,IO%NGROUND_LAYER))
YRECFM='CLAY'
 CALL READ_SURF(HPROGRAM,YRECFM,IP%XCLAY(:,1),IRESP)
...
!* sand fraction
!
ALLOCATE(IP%XSAND(IG%NDIM,IO%NGROUND_LAYER))
YRECFM='SAND'
 CALL READ_SURF(HPROGRAM,YRECFM,IP%XSAND(:,1),IRESP)
DO JLAYER=2,IO%NGROUND_LAYER
...
IF(IO%LSOCP)THEN
!  
  ALLOCATE(IP%XSOC (IG%NDIM,IO%NGROUND_LAYER))
!
  YRECFM='SOC_TOP'
  CALL READ_SURF(HPROGRAM,YRECFM,IP%XSOC(:,1),IRESP)
  YRECFM='SOC_SUB'
  CALL READ_SURF(HPROGRAM,YRECFM,IP%XSOC(:,2),IRESP)
!
  DO JLAYER=2,IO%NGROUND_LAYER
...
IF(IO%LPERM)THEN
!  
  ALLOCATE(IP%XPERM (IG%NDIM))
!
  YRECFM='PERM'
  CALL READ_SURF(HPROGRAM,YRECFM,IP%XPERM(:),IRESP)
!
ELSE
!  
  ALLOCATE(IP%XPERM (0))
...
IF(IO%LGW)THEN
!  
  ALLOCATE(IP%XGW (IG%NDIM))
!
  YRECFM='GWFRAC'
  CALL READ_SURF(HPROGRAM,YRECFM,IP%XGW(:),IRESP)
!
ELSE
!  
  ALLOCATE(IP%XGW (0))
...
IF (CHI%LCH_NO_FLUX) THEN
  ...
  IF (IO%LNOF) THEN
    !
    ALLOCATE(IP%XPH(IG%NDIM))
    YRECFM='PH'
    CALL READ_SURF(HPROGRAM,YRECFM,IP%XPH(:),IRESP)
    !
    ALLOCATE(IP%XFERT(IG%NDIM))
    YRECFM='FERT'
    CALL READ_SURF(HPROGRAM,YRECFM,IP%XFERT(:),IRESP)
    !
  ELSE
...
!
ALLOCATE(IP%XRUNOFFB(IG%NDIM))
YRECFM='RUNOFFB'
 CALL READ_SURF(HPROGRAM,YRECFM,IP%XRUNOFFB,IRESP)
!
!* subgrid drainage coefficient
!
ALLOCATE(IP%XWDRAIN(IG%NDIM))
IF (IVERSION<=3) THEN
  IP%XWDRAIN = 0.
ELSE
  YRECFM='WDRAIN'
  CALL READ_SURF(HPROGRAM,YRECFM,IP%XWDRAIN,IRESP)
ENDIF
!
...
!
  ...
  !
  ALLOCATE(IP%XTI_MIN(IG%NDIM))
  ALLOCATE(IP%XTI_MAX(IG%NDIM))
  ALLOCATE(IP%XTI_MEAN(IG%NDIM))
  ALLOCATE(IP%XTI_STD(IG%NDIM))
  ALLOCATE(IP%XTI_SKEW(IG%NDIM))
!
  YRECFM='TI_MIN'
  CALL READ_SURF(HPROGRAM,YRECFM,IP%XTI_MIN,IRESP)
!
  YRECFM='TI_MAX'
  CALL READ_SURF(HPROGRAM,YRECFM,IP%XTI_MAX,IRESP)
!
  YRECFM='TI_MEAN'
  CALL READ_SURF(HPROGRAM,YRECFM,IP%XTI_MEAN,IRESP)
!
  YRECFM='TI_STD'
  CALL READ_SURF(HPROGRAM,YRECFM,IP%XTI_STD,IRESP)
!
  YRECFM='TI_SKEW'
  CALL READ_SURF(HPROGRAM,YRECFM,IP%XTI_SKEW,IRESP)
!
ELSE
!
...
!
 CALL READ_PGD_ISBA_PAR_n(DTCO, U, DTI, IG%NDIM, IO, HPROGRAM,IG%NDIM,OLAND_USE)
