...
! Ground layers
 IF (IO%NGROUND_LAYER<=3) THEN
   ...
   IF(DTV%XPAR_VEGTYPE(JI,NVT_NO  )>0. )  ZDATA_DG(JI,2,1)= 0.5
   IF(DTV%XPAR_VEGTYPE(JI,NVT_ROCK)>0. )  ZDATA_DG(JI,2,1)= 0.5
   IF(DTV%XPAR_VEGTYPE(JI,NVT_SNOW)>0. )  ZDATA_DG(JI,2,1)= 0.5
   IF (IO%NGROUND_LAYER==3) THEN
      ZDATA_DG(JI,3,:) = 2.00
