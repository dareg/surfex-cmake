...
IF (IO%CTYPE_LVEG == 'C3  ') DTV%XPAR_VEGTYPE(:, NVT_C3  ) = DTV%XPAR_FRAC_LVEG(:)
IF (IO%CTYPE_LVEG == 'C4  ') DTV%XPAR_VEGTYPE(:, NVT_C4  ) = DTV%XPAR_FRAC_LVEG(:)
IF (IO%CTYPE_LVEG == 'IRR ') DTV%XPAR_VEGTYPE(:, NVT_IRR ) = DTV%XPAR_FRAC_LVEG(:)
...
IF (IO%CTYPE_LVEG == 'PARK') DTV%XPAR_VEGTYPE(:, NVT_PARK) = DTV%XPAR_FRAC_LVEG(:)
...
! Near-IR, visible, and UV albedo (vegetation only)
 ...
 IF (DTV%XPAR_FRAC_HVEG(JI)+DTV%XPAR_FRAC_LVEG(JI) .GT. 0.) THEN
  DTV%XPAR_ALBNIR_VEG(JI,1) =  ( ZDATA_ALBNIR_VEG(JI,1)*DTV%XPAR_FRAC_HVEG(JI)   &
                            + ZDATA_ALBNIR_VEG(JI,2)*DTV%XPAR_FRAC_LVEG(JI) ) &
                          / ( DTV%XPAR_FRAC_HVEG(JI)+DTV%XPAR_FRAC_LVEG(JI)    )  
!
  DTV%XPAR_ALBVIS_VEG(JI,1) =  ( ZDATA_ALBVIS_VEG(JI,1)*DTV%XPAR_FRAC_HVEG(JI)   &
                            + ZDATA_ALBVIS_VEG(JI,2)*DTV%XPAR_FRAC_LVEG(JI) ) &
                          / ( DTV%XPAR_FRAC_HVEG(JI)+DTV%XPAR_FRAC_LVEG(JI)    )  
! 
  DTV%XPAR_ALBUV_VEG (JI,1) =  ( ZDATA_ALBUV_VEG (JI,1)*DTV%XPAR_FRAC_HVEG(JI)   &
