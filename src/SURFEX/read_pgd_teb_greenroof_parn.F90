...
YRECFM='GR_NTIME'
 CALL READ_SURF(&
                HPROGRAM,YRECFM,DTGR%NTIME,IRESP)
!
YRECFM='GR_LAYER'
 CALL READ_SURF(&
                HPROGRAM,YRECFM,TGRO%NGROUND_LAYER,IRESP)
!
! Read type of green roof
YRECFM='D_TYPE_GR'
 CALL READ_SURF(&
                HPROGRAM,YRECFM,TGRO%CTYP_COV,IRESP)
!
! Read green roof OM fraction
ALLOCATE(TGRP%XSOC    (TG%NDIM,TGRO%NGROUND_LAYER))
DO JLAYER=1,TGRO%NGROUND_LAYER
  !WRITE(YRECFM,FMT='(A8,I1.1)') 'D_OM_GR0',JLAYER
  WRITE(YRECFM,FMT='(A7,I2.2)') 'D_OM_GR',JLAYER
  CALL READ_SURF(&
                HPROGRAM,YRECFM,TGRP%XSOC(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
! Read green roof SAND fraction
ALLOCATE(TGRP%XSAND   (TG%NDIM,TGRO%NGROUND_LAYER))
DO JLAYER=1,TGRO%NGROUND_LAYER
  !WRITE(YRECFM,FMT='(A10,I1.1)') 'D_SAND_GR0',JLAYER
  WRITE(YRECFM,FMT='(A9,I2.2)') 'D_SAND_GR',JLAYER
  CALL READ_SURF(&
                HPROGRAM,YRECFM,TGRP%XSAND(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
! Read green roof CLAY fraction
ALLOCATE(TGRP%XCLAY   (TG%NDIM,TGRO%NGROUND_LAYER))
DO JLAYER=1,TGRO%NGROUND_LAYER
  !WRITE(YRECFM,FMT='(A10,I1.1)') 'D_CLAY_GR0',JLAYER
  WRITE(YRECFM,FMT='(A9,I2.2)') 'D_CLAY_GR',JLAYER
  CALL READ_SURF(&
                HPROGRAM,YRECFM,TGRP%XCLAY(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
! Read green roof LAI
ALLOCATE(DTGR%XPAR_LAI    (TG%NDIM,DTGR%NTIME,1))
...
!              -----------------------------
!
ALLOCATE(DTGR%XPAR_VEG        (TG%NDIM,DTGR%NTIME,1))
ALLOCATE(DTGR%XPAR_RSMIN      (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_GAMMA      (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_WRMAX_CF   (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_RGL        (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_CV         (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_DG         (TG%NDIM,TGRO%NGROUND_LAYER,1))
ALLOCATE(DTGR%XPAR_ROOTFRAC   (TG%NDIM,TGRO%NGROUND_LAYER,1))
ALLOCATE(DTGR%XPAR_DICE       (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_Z0         (TG%NDIM,DTGR%NTIME,1))
ALLOCATE(DTGR%XPAR_Z0_O_Z0H   (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_ALBNIR_VEG (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_ALBVIS_VEG (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_ALBUV_VEG  (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_ALBNIR_SOIL(TG%NDIM,1))
ALLOCATE(DTGR%XPAR_ALBVIS_SOIL(TG%NDIM,1))
ALLOCATE(DTGR%XPAR_ALBUV_SOIL (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_EMIS       (TG%NDIM,DTGR%NTIME,1))
ALLOCATE(DTGR%XPAR_VEGTYPE    (TG%NDIM,NVEGTYPE))
ALLOCATE(DTGR%XPAR_GMES       (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_RE25       (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_BSLAI      (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_LAIMIN     (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_SEFOLD     (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_GC         (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_DMAX       (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_F2I        (TG%NDIM,1))
ALLOCATE(DTGR%LPAR_STRESS    (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_H_TREE     (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_CE_NITRO   (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_CF_NITRO   (TG%NDIM,1))
ALLOCATE(DTGR%XPAR_CNA_NITRO  (TG%NDIM,1))
!
...
!
DTGR%XPAR_VEGTYPE(:,:) = 0.
IF (TGRO%CTYP_COV == 'GRASS') DTGR%XPAR_VEGTYPE(:, NVT_GRAS) = 1.
IF (TGRO%CTYP_COV == 'SEDUM') DTGR%XPAR_VEGTYPE(:, NVT_TROG) = 1.
!--------------------------------------------------------------------------
!
! Dry/Wet soil albedos: (* Will need to account for XOM_GR eventually *)
!CALL DRY_WET_SOIL_ALBEDOS_1D(XSAND_GR(:,1),XCLAY_GR(:,1),    
ALLOCATE(TGRIP%XALBNIR_DRY (TG%NDIM))
ALLOCATE(TGRIP%XALBVIS_DRY (TG%NDIM))
ALLOCATE(TGRIP%XALBUV_DRY (TG%NDIM))
ALLOCATE(TGRIP%XALBNIR_WET (TG%NDIM))
ALLOCATE(TGRIP%XALBVIS_WET (TG%NDIM))
ALLOCATE(TGRIP%XALBUV_WET (TG%NDIM))
 CALL DRY_WET_SOIL_ALBEDOS_1D(TGRP%XSAND(:,1),TGRP%XCLAY(:,1),              &
                               DTGR%XPAR_VEGTYPE,                                   &
                               TGRIP%XALBNIR_DRY,TGRIP%XALBVIS_DRY,TGRIP%XALBUV_DRY, &
                               TGRIP%XALBNIR_WET,TGRIP%XALBVIS_WET,TGRIP%XALBUV_WET  ) 
!
...
! Soil albedo (* Will need to be refined for greenroofs - cf OM fraction *)
 ...
 CALL SOIL_ALBEDO('DRY',                                              &
                    ...
                    TGRIP%XALBVIS_DRY, TGRIP%XALBNIR_DRY, TGRIP%XALBUV_DRY, &
                    TGRIP%XALBVIS_WET, TGRIP%XALBNIR_WET, TGRIP%XALBUV_WET, &
