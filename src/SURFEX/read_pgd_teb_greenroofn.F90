!     #########
      SUBROUTINE READ_PGD_TEB_GREENROOF_n (CHT, DTCO, DTGR, GBGR, U, TGRO, TGRP, TGRIP, TG, &
                                           HPROGRAM,KVERSION)
!     #########################################
!
...
YRECFM='GR_ISBA'
 CALL READ_SURF(&
                HPROGRAM,YRECFM,TGRO%CISBA,IRESP)
!
YRECFM='GR_SCOND'
 CALL READ_SURF(&
                HPROGRAM,YRECFM,TGRO%CSCOND,IRESP)
!
...
!
ALLOCATE(TGRP%XRUNOFFB(TG%NDIM))
YRECFM='GR_RUNOFFB' 
 CALL READ_SURF(&
                HPROGRAM,YRECFM,TGRP%XRUNOFFB,IRESP)
!
!* subgrid drainage coefficient
!
ALLOCATE(TGRP%XWDRAIN(TG%NDIM))
IF (KVERSION<=6) THEN
  TGRP%XWDRAIN = 0.
ELSE
  YRECFM='GR_WDRAIN'
  CALL READ_SURF(&
                HPROGRAM,YRECFM,TGRP%XWDRAIN,IRESP)
ENDIF
!
!-------------------------------------------------------------------------------
...
TGRO%LPAR = .TRUE. 
!
 CALL READ_PGD_TEB_GREENROOF_PAR_n(DTGR, TGRO, TGRP, TGRIP, TG, &
                                                       HPROGRAM)
!
