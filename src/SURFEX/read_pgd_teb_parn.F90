...
ELSEIF (IVERSION>=7) THEN
  ...
  IF (IVERSION>7 .OR. IBUGFIX>=3) THEN
    ...
    IF (DTT%LDATA_BLDTYPE .OR. DTT%LDATA_BLD_AGE .OR. DTT%LDATA_USETYPE) CALL READ_BLD_DESCRIPTION_n(HPROGRAM)
  ENDIF
  YRECFM='L_Z0_TOWN'
...
IF (DTT%LDATA_BLDTYPE .AND. DTT%LDATA_BLDTYPE) THEN
  ...
  DTT%NPAR_BLDCODE=BLDCODE(DTT%NPAR_BLDTYPE,DTT%NPAR_BLD_AGE)
END IF
!
