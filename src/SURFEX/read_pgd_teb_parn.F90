...
ELSEIF (IVERSION>=7) THEN
  ...
  IF (IVERSION>7 .OR. IBUGFIX>=3) THEN
    ...
    IF (LDATA_BLDTYPE .OR. LDATA_BLD_AGE .OR. LDATA_USETYPE) CALL READ_BLD_DESCRIPTION_n(HPROGRAM)
  ENDIF
  YRECFM='L_Z0_TOWN'
  ...
  IF (IVERSION==7 .AND. IBUGFIX<=3) THEN
    LDATA_EMIS_PANEL = .FALSE.
    LDATA_ALB_PANEL  = .FALSE.
    LDATA_EFF_PANEL  = .FALSE.
    LDATA_FRAC_PANEL = .FALSE.
  ELSE
    YRECFM='L_EMIS_PANEL'
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_EMIS_PANEL,IRESP)
    YRECFM='L_ALB_PANEL'
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_ALB_PANEL,IRESP)
    YRECFM='L_EFF_PANEL'
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_EFF_PANEL,IRESP)
    YRECFM='L_FRAC_PANEL'
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_FRAC_PANEL,IRESP)
  END IF
  YRECFM='L_ALB_ROOF'
  ...
  IF (IVERSION>7) THEN
    YRECFM='L_F_RESIDENT'
    CALL READ_SURF(HPROGRAM,YRECFM,LDATA_RESIDENTIAL,IRESP)
  ENDIF
  !
  IF (CBEM=='BEM') THEN
    YRECFM='L_HC_FLOOR'
...
IF (LDATA_RESIDENTIAL) THEN
  ...
  YRECFM='D_F_RESIDENT'
  CALL READ_FIELD(YRECFM,XPAR_RESIDENTIAL,HDIRIN)
ENDIF
!
! solar panels
!
IF (LDATA_EMIS_PANEL) THEN
  ...
  YRECFM='D_EMIS_PANEL'
  CALL READ_FIELD(YRECFM,XPAR_EMIS_PANEL,HDIRIN)
ENDIF
!
IF (LDATA_ALB_PANEL) THEN
  ALLOCATE(XPAR_ALB_PANEL (NDIM))
  YRECFM='D_ALB_PANEL'
  CALL READ_FIELD(YRECFM,XPAR_ALB_PANEL,HDIRIN)
ENDIF
!
IF (LDATA_EFF_PANEL) THEN
  ALLOCATE(XPAR_EFF_PANEL (NDIM))
  YRECFM='D_EFF_PANEL'
  CALL READ_FIELD(YRECFM,XPAR_EFF_PANEL,HDIRIN)
ENDIF
!
IF (LDATA_FRAC_PANEL) THEN
  ALLOCATE(XPAR_FRAC_PANEL (NDIM))
  YRECFM='D_FRAC_PANEL'
  CALL READ_FIELD(YRECFM,XPAR_FRAC_PANEL,HDIRIN)
ENDIF
!
