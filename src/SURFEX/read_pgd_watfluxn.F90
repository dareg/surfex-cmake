...
YRECFM='SIZE_WATER'
 CALL GET_TYPE_DIM_n('WATER ',WG%NDIM)
!
!
!
...
ALLOCATE(W%LCOVER(JPCOVER))
 CALL READ_LCOVER(HPROGRAM,W%LCOVER)
!
ALLOCATE(W%XCOVER(WG%NDIM,JPCOVER))
 CALL READ_SURF_COV(HPROGRAM,'COVER',W%XCOVER(:,:),W%LCOVER,IRESP)
!
!* orography
!
ALLOCATE(W%XZS(WG%NDIM))
YRECFM='ZS'
 CALL READ_SURF(HPROGRAM,YRECFM,W%XZS(:),IRESP)
!
!* latitude, longitude 
!
...
ALLOCATE(WG%XMESH_SIZE(WG%NDIM))
 CALL READ_GRID(HPROGRAM,WG%CGRID,WG%XGRID_PAR,WG%XLAT,WG%XLON,WG%XMESH_SIZE,IRESP)
