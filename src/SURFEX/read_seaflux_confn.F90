...
CALL POSNAM(INAM,'NAM_DIAG_OCEANN',GFOUND,ILUOUT)
 ...
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CINTERPOL_SIC',CINTERPOL_SIC,'ANNUAL','MONTH ','NONE  ')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CINTERPOL_SIT',CINTERPOL_SIT,'ANNUAL','MONTH ','NONE  ')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CSEAICE_SCHEME',CSEAICE_SCHEME,'GELATO','NONE  ')
 ! IF (.NOT. LHANDLE_SIC) LDIAG_SEAICE=.FALSE.
 !
 !
 !* close namelist file
 !
