!     #########
      SUBROUTINE READ_SEAFLUX_SBL_n (DTCO, OSBL, SSB, U, HPROGRAM)
!     #########################################
!
...
!
YRECFM='SEA_SBL_LVL'
 CALL READ_SURF(HPROGRAM,YRECFM,SSB%NLVL,IRESP)
!
!*       2.     Prognostic fields:
!               -----------------
!
...
DO JLAYER=1,SSB%NLVL
  ...
  CALL READ_SURF(HPROGRAM,YRECFM,SSB%XZ(:,JLAYER),IRESP)
END DO
!
...
ALLOCATE(SSB%XLMO(ILU,SSB%NLVL))
...
IF(YREAD=='ALL') THEN
  ...
  DO JLAYER=1,SSB%NLVL
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,SSB%XU(:,JLAYER),IRESP)
  END DO
  !
  !* theta in SBL
  DO JLAYER=1,SSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'SEA_SBL_T',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,SSB%XT(:,JLAYER),IRESP)
  END DO
  !
  !* humidity in SBL
  DO JLAYER=1,SSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'SEA_SBL_Q',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,SSB%XQ(:,JLAYER),IRESP)
  END DO
  !
  !* Tke in SBL
  DO JLAYER=1,SSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'SEA_SBL_E',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,SSB%XTKE(:,JLAYER),IRESP)
  END DO
  !
  !* Monin-Obhukov length
  YRECFM='SEA_SBL_LMO '
  CALL READ_SURF(HPROGRAM,YRECFM,SSB%XLMO(:,1),IRESP)
  DO JLAYER = 2,SSB%NLVL
    SSB%XLMO(:,JLAYER) = SSB%XLMO(:,1)
  ENDDO            
  !
  !* Pressure
  DO JLAYER=1,SSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'SEA_SBL_P',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,SSB%XP(:,JLAYER),IRESP)
  END DO
  !
ELSE
  SSB%XU  (:,:) = XUNDEF
