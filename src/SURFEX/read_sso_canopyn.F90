...
!
IF (.NOT. GCANOPY) THEN
  CALL SET_SSO_LEVELS(SSCP, ILU)
ELSE
  !
  YRECFM='SSO_CAN_LVL'
  CALL READ_SURF(HPROGRAM,YRECFM,SSCP%NLVL,IRESP)
  !
  !
  ...
  DO JLAYER=1,SSCP%NLVL
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,SSCP%XZ(:,JLAYER),IRESP)
  END DO
  !    
  !* wind in canopy
  DO JLAYER=1,SSCP%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'SSO_CAN_U',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,SSCP%XU(:,JLAYER),IRESP)
  END DO
  !
  !* Tke in canopy
  DO JLAYER=1,SSCP%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'SSO_CAN_E',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,SSCP%XTKE(:,JLAYER),IRESP)
  END DO
  !
ENDIF
!
