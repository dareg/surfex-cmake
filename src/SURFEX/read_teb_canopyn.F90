...
USE MODD_TEB_CANOPY_n,   ONLY : NLVL, XZ, XU, XT, XQ, XTKE, XLMO, XLM,XLEPS, XDZ, XZF, XDZF, XP
!
USE MODI_READ_SURF
...
IF(YREAD=='ALL') THEN
  ...
  !* wind in SBL
  DO JLAYER=1,NLVL
    WRITE(YRECFM,'(A9,I2.2,A4)') 'TEB_CAN_U',JLAYER,'    '
    CALL READ_SURF(HPROGRAM,YRECFM,XU(:,JLAYER),IRESP)
  END DO
  !
  !* theta in SBL
  DO JLAYER=1,NLVL
    WRITE(YRECFM,'(A9,I2.2,A4)') 'TEB_CAN_T',JLAYER,'    '
    CALL READ_SURF(HPROGRAM,YRECFM,XT(:,JLAYER),IRESP)
  END DO
  !
  !* humidity in SBL
  DO JLAYER=1,NLVL
    WRITE(YRECFM,'(A9,I2.2,A4)') 'TEB_CAN_Q',JLAYER,'    '
    CALL READ_SURF(HPROGRAM,YRECFM,XQ(:,JLAYER),IRESP)
  END DO
  !
  !* Tke in SBL
  DO JLAYER=1,NLVL
    WRITE(YRECFM,'(A9,I2.2,A4)') 'TEB_CAN_E',JLAYER,'    '
  ...
  IF (IVERSION<7) THEN
    YRECFM='TEB_CAN_LMO     ' 
    CALL READ_SURF(HPROGRAM,YRECFM,XLMO(:,1),IRESP) 
    DO JLAYER = 2,NLVL
      XLMO(:,JLAYER) = XLMO(:,1)
    ENDDO    
  ELSE
  ...
  !* Pressure
  DO JLAYER=1,NLVL
    WRITE(YRECFM,'(A9,I2.2,A4)') 'TEB_CAN_P',JLAYER,'    '
