!     #########
      SUBROUTINE READ_TEB_CANOPY_n (DTCO, U, TCP, OCANOPY, HPROGRAM)
!     #########################################
!
...
!
YRECFM='TEB_CAN_LVL'
 CALL READ_SURF(HPROGRAM,YRECFM,TCP%NLVL,IRESP)
!
!*       2.     Prognostic fields:
!               -----------------
!
...
DO JLAYER=1,TCP%NLVL
  ...
  CALL READ_SURF(HPROGRAM,YRECFM,TCP%XZ(:,JLAYER),IRESP)
END DO
!
...
IF(YREAD=='ALL') THEN
  ...
  DO JLAYER=1,TCP%NLVL
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,TCP%XU(:,JLAYER),IRESP)
  END DO
  !
  !* theta in SBL
  DO JLAYER=1,TCP%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'TEB_CAN_T',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,TCP%XT(:,JLAYER),IRESP)
  END DO
  !
  !* humidity in SBL
  DO JLAYER=1,TCP%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'TEB_CAN_Q',JLAYER,' '
    CALL READ_SURF( HPROGRAM,YRECFM,TCP%XQ(:,JLAYER),IRESP)
  END DO
  !
  !* Tke in SBL
  DO JLAYER=1,TCP%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'TEB_CAN_E',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,TCP%XTKE(:,JLAYER),IRESP)
  END DO
  !
  !* Monin-Obhukov length
  IF (IVERSION<7) THEN
    YRECFM='TEB_CAN_LMO ' 
    CALL READ_SURF(HPROGRAM,YRECFM,TCP%XLMO(:,1),IRESP) 
  ...
  ELSE
    DO JLAYER=1,TCP%NLVL
      ...
      CALL READ_SURF(HPROGRAM,YRECFM,TCP%XLMO(:,JLAYER),IRESP)
    ENDDO
  ENDIF    
  !
  !* Pressure
  DO JLAYER=1,TCP%NLVL
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,TCP%XP(:,JLAYER),IRESP)
  END DO
  !
ELSE
  TCP%XU  (:,:) = XUNDEF
