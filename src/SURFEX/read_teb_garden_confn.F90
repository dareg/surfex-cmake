!     #########
      SUBROUTINE READ_TEB_GARDEN_CONF_n(HPROGRAM)
!     #######################################################
!
!!****  *READ_TEB_GARDEN_CONF* - routine to read the configuration for GARDEN
!!
!!    PURPOSE
!!    -------
!!
...
!
IF (LHOOK) CALL DR_HOOK('READ_TEB_GARDEN_CONF_N',0,ZHOOK_HANDLE)
...
IF (LNAM_READ) THEN
 ...
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CALBEDO',CALBEDO,'EVOL','DRY ','WET ','MEAN','USER')
 ...
 !
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CRUNOFF',CRUNOFF,'WSAT','DT92','SGH ')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CTOPREG',CTOPREG,'DEF','NON')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CKSAT',CKSAT,'DEF','SGH')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CSOM',CSOM,'DEF','SGH')
...
IF (IMI.NE.-1) THEN
 ...
 CALL UPDATE_NAM_SGH_GARDENn        
ENDIF
IF (LHOOK) CALL DR_HOOK('READ_TEB_GARDEN_CONF_N',1,ZHOOK_HANDLE)
!
!-------------------------------------------------------------------------------
!
...
!-------------------------------------------------------------------------------
!
END SUBROUTINE READ_TEB_GARDEN_CONF_n
