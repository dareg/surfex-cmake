...
ENDIF
YRECFM=ADJUSTL(YRECFM)
 CALL READ_SURF(HPROGRAM,YRECFM,TGD%CUR%XWR(:),IRESP)
!
...
IF (TVG%CPHOTO=='LAI' .OR. TVG%CPHOTO=='LST' .OR. TVG%CPHOTO=='NIT' .OR. TVG%CPHOTO=='NCB') THEN
  ...
  ENDIF        
  YRECFM=ADJUSTL(YRECFM)
  CALL READ_SURF(HPROGRAM,YRECFM,TGDPE%CUR%XLAI(:),IRESP)        
END IF
!
!* snow mantel
!
...
ELSE
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
    CALL READ_GR_SNOW(HPROGRAM,'GD',HPATCH,ILU,1,TGD%CUR%TSNOW  )
  ELSE
    CALL READ_GR_SNOW(HPROGRAM,'GARD',HPATCH,ILU,1,TGD%CUR%TSNOW  )
  ENDIF
ENDIF
!
!-------------------------------------------------------------------------------
...
YRECFM=ADJUSTL(YRECFM)
TGD%CUR%XRESA(:) = 100.
 CALL READ_SURF(HPROGRAM,YRECFM,TGD%CUR%XRESA(:),IRESP)
!
...
ELSEIF (TVG%CPHOTO=='NIT' .OR. TVG%CPHOTO=='NCB') THEN
  ...
  DO JNBIOMASS=1,TVG%NNBIOMASS
    ...
    ENDIF
    CALL READ_SURF(HPROGRAM,YRECFM,TGD%CUR%XBIOMASS(:,JNBIOMASS),IRESP)
  END DO
  ...
  DO JNBIOMASS=2,TVG%NNBIOMASS
    ...
    ENDIF    
    CALL READ_SURF(HPROGRAM,YRECFM,TGD%CUR%XRESP_BIOMASS(:,JNBIOMASS),IRESP)
  END DO
  !
ENDIF
!
