!     #########
      SUBROUTINE READ_TEB_GARDEN_n (DTCO, U, GDO, GDR, GDMT, HPROGRAM,HPATCH)
!     ##################################
!
...
DO JLAYER=1,IWORK
  ...
  GDR%XTG(:,JLAYER,1)=ZWORK
END DO
!
!
!* soil liquid water content
!
ALLOCATE(GDR%XWG(ILU,IWORK,1))
DO JLAYER=1,GDO%NGROUND_LAYER
  ...
  YRECFM=ADJUSTL(YRECFM)
  CALL READ_SURF(HPROGRAM,YRECFM,ZWORK(:),IRESP)
  GDR%XWG(:,JLAYER,1)=ZWORK
END DO
!
!* soil ice water content
!
ALLOCATE(GDR%XWGI(ILU,IWORK,1))
DO JLAYER=1,GDO%NGROUND_LAYER
  WRITE(YLVL,'(I2)') JLAYER
! ajouter ici un test pour lire les anciens fichiers
  ...
  YRECFM=ADJUSTL(YRECFM)  
  CALL READ_SURF(HPROGRAM,YRECFM,ZWORK(:),IRESP)
  GDR%XWGI(:,JLAYER,1)=ZWORK
END DO
!
!* water intercepted on leaves
!
ALLOCATE(GDR%XWR(ILU,1))
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
  YRECFM=HPATCH//'GD_WR'
ELSE
  YRECFM='TWN_WR'
ENDIF
YRECFM=ADJUSTL(YRECFM)
 CALL READ_SURF(HPROGRAM,YRECFM,GDR%XWR(:,1),IRESP)
!
!* Leaf Area Index (if prognostic)
!
IF (GDO%CPHOTO=='LAI' .OR. GDO%CPHOTO=='LST' .OR. &
                GDO%CPHOTO=='NIT' .OR. GDO%CPHOTO=='NCB') THEN
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
  ...
  ENDIF        
  YRECFM=ADJUSTL(YRECFM)
  CALL READ_SURF(HPROGRAM,YRECFM,GDMT%XLAI(:,1),IRESP)        
END IF
!
!* snow mantel
!
...
!
IF (.NOT. GTOWN) THEN
  GDR%TSNOW%SCHEME='1-L'
  CALL ALLOCATE_GR_SNOW(GDR%TSNOW,ILU,1)
ELSE
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
    CALL READ_GR_SNOW(HPROGRAM,'GD',HPATCH,ILU,1,GDR%TSNOW  )
  ELSE
    CALL READ_GR_SNOW(HPROGRAM,'GARD',HPATCH,ILU,1,GDR%TSNOW  )
  ENDIF
ENDIF
!
!-------------------------------------------------------------------------------
...
YRECFM=ADJUSTL(YRECFM)
GDR%XRESA(:,:) = 100.
 CALL READ_SURF(HPROGRAM,YRECFM,GDR%XRESA(:,1),IRESP)
!
ALLOCATE(GDR%XLE(ILU,1))
GDR%XLE(:,1) = XUNDEF
!
!* ISBA-AGS variables
!
IF (GDO%CPHOTO/='NON') THEN
  ALLOCATE(GDR%XAN   (ILU,1)) 
  ALLOCATE(GDR%XANDAY(ILU,1)) 
  ALLOCATE(GDR%XANFM (ILU,1))
  GDR%XAN(:,1)    = 0.
  GDR%XANDAY(:,1) = 0.
  GDR%XANFM(:,1)  = XANFMINIT
  GDR%XLE(:,1)    = 0.
ELSE
  ALLOCATE(GDR%XAN   (0,0)) 
  ALLOCATE(GDR%XANDAY(0,0)) 
  ALLOCATE(GDR%XANFM (0,0))
ENDIF
!
IF(GDO%CPHOTO/='NON') THEN
  ALLOCATE(GDR%XBIOMASS         (ILU,GDO%NNBIOMASS,1))
  ALLOCATE(GDR%XRESP_BIOMASS    (ILU,GDO%NNBIOMASS,1))
ELSE
  ALLOCATE(GDR%XBIOMASS         (0,0,0))
  ALLOCATE(GDR%XRESP_BIOMASS    (0,0,0))
END IF
!
...
ELSEIF (GDO%CPHOTO=='LAI' .OR. GDO%CPHOTO=='LST') THEN
  !
  GDR%XBIOMASS(:,1,:) = GDMT%XBSLAI(:,:) * GDMT%XLAI(:,:)
  GDR%XRESP_BIOMASS(:,:,:) = 0.
ELSEIF (GDO%CPHOTO=='NIT' .OR. GDO%CPHOTO=='NCB') THEN
  ...
  DO JNBIOMASS=1,GDO%NNBIOMASS
    ...
    ENDIF
    YRECFM=ADJUSTL(YRECFM)
    CALL READ_SURF(HPROGRAM,YRECFM,GDR%XBIOMASS(:,JNBIOMASS,1),IRESP)
  END DO
  ...
  DO JNBIOMASS=2,GDO%NNBIOMASS
    ...
    ENDIF    
    YRECFM=ADJUSTL(YRECFM)
    CALL READ_SURF(HPROGRAM,YRECFM,GDR%XRESP_BIOMASS(:,JNBIOMASS,1),IRESP)
  END DO
  !
ENDIF
!
