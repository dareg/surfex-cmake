...
DO JLAYER=1,IWORK
  ...
  TGD%XTG(:,JLAYER)=ZWORK
END DO
!
!
!* soil liquid water content
!
ALLOCATE(TGD%XWG(ILU,IWORK))
DO JLAYER=1,TGDO%NGROUND_LAYER
  ...
  YRECFM=ADJUSTL(YRECFM)
  CALL READ_SURF(HPROGRAM,YRECFM,ZWORK(:),IRESP)
  TGD%XWG(:,JLAYER)=ZWORK
END DO
!
!* soil ice water content
!
ALLOCATE(TGD%XWGI(ILU,IWORK))
DO JLAYER=1,TGDO%NGROUND_LAYER
  WRITE(YLVL,'(I2)') JLAYER
! ajouter ici un test pour lire les anciens fichiers
  ...
  YRECFM=ADJUSTL(YRECFM)  
  CALL READ_SURF(HPROGRAM,YRECFM,ZWORK(:),IRESP)
  TGD%XWGI(:,JLAYER)=ZWORK
END DO
!
!* water intercepted on leaves
!
ALLOCATE(TGD%XWR(ILU))
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
  YRECFM=HPATCH//'GD_WR'
ELSE
  YRECFM='TWN_WR'
ENDIF
YRECFM=ADJUSTL(YRECFM)
 CALL READ_SURF(HPROGRAM,YRECFM,TGD%XWR(:),IRESP)
!
...
IF (TVG%CPHOTO=='LAI' .OR. TVG%CPHOTO=='LST' .OR. TVG%CPHOTO=='NIT' .OR. TVG%CPHOTO=='NCB') THEN
  ...
  ENDIF        
  YRECFM=ADJUSTL(YRECFM)
  CALL READ_SURF(HPROGRAM,YRECFM,TGDPE%XLAI(:),IRESP)        
END IF
!
!* snow mantel
!
...
!
IF (.NOT. GTOWN) THEN
  TGD%TSNOW%SCHEME='1-L'
  CALL ALLOCATE_GR_SNOW(TGD%TSNOW,ILU,1)
ELSE
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
    CALL READ_GR_SNOW(HPROGRAM,'GD',HPATCH,ILU,1,TGD%TSNOW  )
  ELSE
    CALL READ_GR_SNOW(HPROGRAM,'GARD',HPATCH,ILU,1,TGD%TSNOW  )
  ENDIF
ENDIF
!
!-------------------------------------------------------------------------------
...
YRECFM=ADJUSTL(YRECFM)
TGD%XRESA(:) = 100.
 CALL READ_SURF(HPROGRAM,YRECFM,TGD%XRESA(:),IRESP)
!
ALLOCATE(TGD%XLE(ILU))
TGD%XLE(:) = XUNDEF
!
!* ISBA-AGS variables
!
IF (TVG%CPHOTO/='NON') THEN
  ALLOCATE(TGD%XAN   (ILU)) 
  ALLOCATE(TGD%XANDAY(ILU)) 
  ALLOCATE(TGD%XANFM (ILU))
...
ELSE
  ALLOCATE(TGD%XAN   (0)) 
  ALLOCATE(TGD%XANDAY(0)) 
  ALLOCATE(TGD%XANFM (0))
  ALLOCATE(TGDP%XANF  (0))
ENDIF
!
IF(TVG%CPHOTO/='NON') THEN
  ALLOCATE(TGD%XBIOMASS         (ILU,TVG%NNBIOMASS))
  ALLOCATE(TGD%XRESP_BIOMASS    (ILU,TVG%NNBIOMASS))
ELSE
  ALLOCATE(TGD%XBIOMASS         (0,0))
  ALLOCATE(TGD%XRESP_BIOMASS    (0,0))
END IF
!
...
ELSEIF (TVG%CPHOTO=='LAI' .OR. TVG%CPHOTO=='LST') THEN
  !
  TGD%XBIOMASS(:,1) = TGDP%XBSLAI(:) * TGDPE%XLAI(:)
  TGD%XRESP_BIOMASS(:,:) = 0.
ELSEIF (TVG%CPHOTO=='NIT' .OR. TVG%CPHOTO=='NCB') THEN
  ...
  DO JNBIOMASS=1,TVG%NNBIOMASS
    ...
    ENDIF
    CALL READ_SURF(HPROGRAM,YRECFM,TGD%XBIOMASS(:,JNBIOMASS),IRESP)
  END DO
  ...
  DO JNBIOMASS=2,TVG%NNBIOMASS
    ...
    ENDIF    
    CALL READ_SURF(HPROGRAM,YRECFM,TGD%XRESP_BIOMASS(:,JNBIOMASS),IRESP)
  END DO
  !
ENDIF
!
