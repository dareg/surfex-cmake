!     #########
      SUBROUTINE READ_TEB_GREENROOF_PAR_n(HPROGRAM)
!     ################################################
!
!!****  *READ_TEB_GREENROOF_PAR_n* - reads ISBA physiographic fields
!!                        
!!
!!    PURPOSE
!!    -------
...
!              --------------------
!
IF (LHOOK) CALL DR_HOOK('READ_TEB_GREENROOF_PAR_N',0,ZHOOK_HANDLE)
!
YRECFM='VERSION'
...
! Depth of greenroof ground layers
 XPAR_DG(JI, 1) = XGRID_SOIL(NGRID_LEVEL-17)
 XPAR_DG(JI, 2) = XGRID_SOIL(NGRID_LEVEL-16)
 XPAR_DG(JI, 3) = XGRID_SOIL(NGRID_LEVEL-15)
 XPAR_DG(JI, 4) = XGRID_SOIL(NGRID_LEVEL-14)
 XPAR_DG(JI, 5) = XGRID_SOIL(NGRID_LEVEL-13)
 XPAR_DG(JI, 6) = XGRID_SOIL(NGRID_LEVEL-12)
 XPAR_DG(JI, 7) = XGRID_SOIL(NGRID_LEVEL-11)
 XPAR_DG(JI, 8) = XGRID_SOIL(NGRID_LEVEL-10)
 XPAR_DG(JI, 9) = XGRID_SOIL(NGRID_LEVEL-9)
 XPAR_DG(JI,10) = XGRID_SOIL(NGRID_LEVEL-8)
 XPAR_DG(JI,11) = XGRID_SOIL(NGRID_LEVEL-7)
 XPAR_DG(JI,12) = XGRID_SOIL(NGRID_LEVEL-6)
 XPAR_DG(JI,13) = XGRID_SOIL(NGRID_LEVEL-5)
 XPAR_DG(JI,14) = XGRID_SOIL(NGRID_LEVEL-4)
 XPAR_DG(JI,15) = XGRID_SOIL(NGRID_LEVEL-3)
 XPAR_DG(JI,16) = XGRID_SOIL(NGRID_LEVEL-2)
 XPAR_DG(JI,17) = XGRID_SOIL(NGRID_LEVEL-1)
 XPAR_DG(JI,18) = XGRID_SOIL(NGRID_LEVEL-0)
!
! Root fractions
 XPAR_ROOTFRAC(JI, 1)  = 0.03
 XPAR_ROOTFRAC(JI, 2)  = 0.06
 XPAR_ROOTFRAC(JI, 3)  = 0.10
 XPAR_ROOTFRAC(JI, 4)  = 0.14
 XPAR_ROOTFRAC(JI, 5)  = 0.2
 XPAR_ROOTFRAC(JI, 6)  = 0.3
 XPAR_ROOTFRAC(JI, 7)  = 0.4
 XPAR_ROOTFRAC(JI, 8)  = 0.5
 XPAR_ROOTFRAC(JI, 9)  = 0.6
 XPAR_ROOTFRAC(JI,10) = 0.7
 XPAR_ROOTFRAC(JI,11) = 0.8
 XPAR_ROOTFRAC(JI,12) = 0.9
 XPAR_ROOTFRAC(JI,13) = 1.
 XPAR_ROOTFRAC(JI,14) = 1.
 XPAR_ROOTFRAC(JI,15) = 1.
 XPAR_ROOTFRAC(JI,16) = 1.
 XPAR_ROOTFRAC(JI,17) = 1.
 XPAR_ROOTFRAC(JI,18) = 1.
!
...
!* Will need to be refined for greenroofs *)
  !XPAR_VEG (JI,1,JTIME) = VEG_FROM_LAI (XPAR_LAI_GR(JI,1,JTIME),   &
  !                                       XPAR_VEGTYPE(JI,:))  
 IF(XPAR_VEGTYPE(JI,NVT_GRAS)>0. )   XPAR_VEG (JI,JTIME) = 0.9
 IF(XPAR_VEGTYPE(JI,NVT_TROG)>0. )   XPAR_VEG (JI,JTIME) = 0.9
...
! Emissivity
!* Will need to be refined for greenroofs *)
  !XPAR_EMIS (JI,JTIME) = EMIS_FROM_VEG (XPAR_VEG    (JI,JTIME),&
  !                                         XPAR_VEGTYPE(JI,:))  
...
ENDDO
!
IF (LHOOK) CALL DR_HOOK('READ_TEB_GREENROOF_PAR_N',1,ZHOOK_HANDLE)
!
!-------------------------------------------------------------------------------
!
END SUBROUTINE READ_TEB_GREENROOF_PAR_n
