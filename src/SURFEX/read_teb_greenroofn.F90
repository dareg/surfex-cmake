...
YRECFM=HPATCH//'GR_WR'
YRECFM=ADJUSTL(YRECFM)
 CALL READ_SURF(HPROGRAM,YRECFM,TGR%CUR%XWR(:),IRESP)
!
!* Leaf Area Index
!
IF (TVG%CPHOTO=='LAI' .OR. TVG%CPHOTO=='LST' .OR. TVG%CPHOTO=='NIT' .OR. TVG%CPHOTO=='NCB') THEN
  ...
  YRECFM=ADJUSTL(YRECFM)
  CALL READ_SURF(HPROGRAM,YRECFM,TGRPE%CUR%XLAI(:),IRESP)
END IF
!
!* snow mantel
!
...
YRECFM=ADJUSTL(YRECFM)
TGR%CUR%XRESA(:) = 100.
 CALL READ_SURF(HPROGRAM,YRECFM,TGR%CUR%XRESA(:),IRESP)
!
TGR%CUR%XLE(:) = XUNDEF
!
!* ISBA-AGS variables
...
ELSEIF (TVG%CPHOTO=='NIT') THEN
  ...
  DO JNBIOMASS=1,TVG%NNBIOMASS
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,TGR%CUR%XBIOMASS(:,JNBIOMASS),IRESP)
  END DO
  ...
  DO JNBIOMASS=2,TVG%NNBIOMASS
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,TGR%CUR%XRESP_BIOMASS(:,JNBIOMASS),IRESP)
  END DO
ENDIF
!
!
