...
DO JLAYER=1,NROOF_LAYER
  WRITE(YRECFM,'(A5,A5,I1.1,A4)') YPATCH,'TROOF',JLAYER,'    '
  YRECFM=ADJUSTL(YRECFM)
  IF (GOLD_NAME) WRITE(YRECFM,'(A6,I1.1,A4)') 'T_ROOF',JLAYER,'    '
...
DO JLAYER=1,NROAD_LAYER
  WRITE(YRECFM,'(A5,A5,I1.1,A4)') YPATCH,'TROAD',JLAYER,'    '
  YRECFM=ADJUSTL(YRECFM)
  IF (GOLD_NAME) WRITE(YRECFM,'(A6,I1.1,A4)') 'T_ROAD',JLAYER,'    '
...
DO JLAYER=1,NWALL_LAYER
  IF (CWALL_OPT=='UNIF' .OR. GOLD_NAME) THEN
    WRITE(YRECFM,'(A5,A5,I1.1,A4)') YPATCH,'TWALL',JLAYER,'    '
    YRECFM=ADJUSTL(YRECFM)
    IF (GOLD_NAME) WRITE(YRECFM,'(A6,I1.1,A4)') 'T_WALL',JLAYER,'    '
    CALL READ_SURF(HPROGRAM,YRECFM,XT_WALL_A(:,JLAYER),IRESP)
    !
    XT_WALL_B = XT_WALL_A
  ELSE
    WRITE(YRECFM,'(A5,A6,I1.1,A3)') YPATCH,'TWALLA',JLAYER,'   '
    YRECFM=ADJUSTL(YRECFM)
    CALL READ_SURF(HPROGRAM,YRECFM,XT_WALL_A(:,JLAYER),IRESP)
    !
    WRITE(YRECFM,'(A5,A6,I1.1,A3)') YPATCH,'TWALLB',JLAYER,'   '
    YRECFM=ADJUSTL(YRECFM)
...
IF (CBEM=='BEM') THEN
  ...
  DO JLAYER=1,NFLOOR_LAYER
    WRITE(YRECFM,'(A5,A5,I1.1,A4)') YPATCH,'TFLOO',JLAYER,'    '
    YRECFM=ADJUSTL(YRECFM)
  ...
  !
  DO JLAYER=1,NFLOOR_LAYER
    WRITE(YRECFM,'(A5,A5,I1.1,A4)') YPATCH,'TMASS',JLAYER,'    '
    YRECFM=ADJUSTL(YRECFM)
