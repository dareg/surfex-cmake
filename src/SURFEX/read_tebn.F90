...
DO JLAYER=1,NROOF_LAYER
  WRITE(YRECFM,'(A5,A5,I1.1,A1)') YPATCH,'TROOF',JLAYER,' '
...
DO JLAYER=1,NROAD_LAYER
  WRITE(YRECFM,'(A5,A5,I1.1,A1)') YPATCH,'TROAD',JLAYER,' '
  YRECFM=ADJUSTL(YRECFM)
...
DO JLAYER=1,NWALL_LAYER
  IF (CWALL_OPT=='UNIF' .OR. GOLD_NAME) THEN
    WRITE(YRECFM,'(A5,A5,I1.1,A1)') YPATCH,'TWALL',JLAYER,' '
    YRECFM=ADJUSTL(YRECFM)
    ...
    XT_WALL_B = XT_WALL_A
  ELSE
    WRITE(YRECFM,'(A5,A6,I1.1)') YPATCH,'TWALLA',JLAYER
    YRECFM=ADJUSTL(YRECFM)
    CALL READ_SURF(HPROGRAM,YRECFM,XT_WALL_A(:,JLAYER),IRESP)
    !
    WRITE(YRECFM,'(A5,A6,I1.1)') YPATCH,'TWALLB',JLAYER
    YRECFM=ADJUSTL(YRECFM)
...
IF (CBEM=='BEM') THEN
  ...
  DO JLAYER=1,NFLOOR_LAYER
    WRITE(YRECFM,'(A5,A5,I1.1,A1)') YPATCH,'TFLOO',JLAYER,' '
    YRECFM=ADJUSTL(YRECFM)
  ...
  !
  DO JLAYER=1,NFLOOR_LAYER
    WRITE(YRECFM,'(A5,A5,I1.1,A1)') YPATCH,'TMASS',JLAYER,' '
    YRECFM=ADJUSTL(YRECFM)
