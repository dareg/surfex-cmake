...
ELSE
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
    CALL READ_GR_SNOW(HPROGRAM,'RD',YPATCH,ILU,1,T%CUR%TSNOW_ROAD  )
    CALL READ_GR_SNOW(HPROGRAM,'RF',YPATCH,ILU,1,T%CUR%TSNOW_ROOF  )
  ELSE
    CALL READ_GR_SNOW(HPROGRAM,'ROAD',YPATCH,ILU,1,T%CUR%TSNOW_ROAD  )
    CALL READ_GR_SNOW(HPROGRAM,'ROOF',YPATCH,ILU,1,T%CUR%TSNOW_ROOF  )
  ENDIF    
END IF
!
!-------------------------------------------------------------------------------
