!     #########
      SUBROUTINE READ_WATFLUX_SBL_n (DTCO, U, OSBL, WSB, HPROGRAM)
!     #########################################
!
...
!
YRECFM='WAT_SBL_LVL'
 CALL READ_SURF(HPROGRAM,YRECFM,WSB%NLVL,IRESP)
!
!*       2.     Prognostic fields:
!               -----------------
!
...
DO JLAYER=1,WSB%NLVL
  ...
  CALL READ_SURF(HPROGRAM,YRECFM,WSB%XZ(:,JLAYER),IRESP)
END DO
!
...
ALLOCATE(WSB%XLMO(ILU,WSB%NLVL))
...
IF(YREAD=='ALL') THEN
  ...
  DO JLAYER=1,WSB%NLVL
    ...
    CALL READ_SURF(HPROGRAM,YRECFM,WSB%XU(:,JLAYER),IRESP)
  END DO
  !
  !* theta in SBL
  DO JLAYER=1,WSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'WAT_SBL_T',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,WSB%XT(:,JLAYER),IRESP)
  END DO
  !
  !* humidity in SBL
  DO JLAYER=1,WSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'WAT_SBL_Q',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,WSB%XQ(:,JLAYER),IRESP)
  END DO
  !
  !* Tke in SBL
  DO JLAYER=1,WSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'WAT_SBL_E',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,WSB%XTKE(:,JLAYER),IRESP)
  END DO
  !
  !* Monin-Obhukov length
  YRECFM='WAT_SBL_LMO '
  CALL READ_SURF(HPROGRAM,YRECFM,WSB%XLMO(:,1),IRESP)
  DO JLAYER = 2,WSB%NLVL
    WSB%XLMO(:,JLAYER) = WSB%XLMO(:,1)
  ENDDO           
  !
  !* Pressure
  DO JLAYER=1,WSB%NLVL
    WRITE(YRECFM,'(A9,I2.2,A1)') 'WAT_SBL_P',JLAYER,' '
    CALL READ_SURF(HPROGRAM,YRECFM,WSB%XP(:,JLAYER),IRESP)
  END DO
  !
ELSE
  WSB%XU  (:,:) = XUNDEF
