...
!     #########
      SUBROUTINE SEAICE_SICE_SNOW3L(HSNOWRES, TPTIME, HIMPLICIT_WIND,     &
...
!
USE MODE_SEAICE_SICE_SNOW3L, DEFAULT_SNOW3LALB => SNOW3LALB
...
!
REAL, DIMENSION(:), INTENT(OUT)     :: PSWNETSNOW, PLWNETSNOW, PSWNETSNOWS
...
!                                                  (W m-2) Imposed if MEB=T, diagnosed herein if MEB=F
!
REAL, DIMENSION(:), INTENT(OUT)     :: PUSTAR, PCDSNOW, PCHSNOW, PRI
...
!
CALL SNOW3LALB(ZWORK2, PTSTEP, PSNOWLIQ(:,1), PSNOWDZ(:,1),     &
               PSNOWRHO(:,1), GSFCMELT, PSR, PPERMSNOWFRAC)
...
!
 CALL SNOW3LRAD(XSNOWDZMIN,PSW_RAD,PSNOWALB,      &
...
!
 CALL SNOW3LSOLVT( PTSTEP,XSNOWDZMIN,PSNOWDZ,ZSCOND,ZSCAP,PTG,                 &
...
!! CALL SNOW3LALB(PSNOWALB,ZSPECTRALALBEDO,PSNOWRHO(:,1),PSNOWAGE(:,1),PPERMSNOWFRAC,PPS)
 CALL SNOW3LALB(PSNOWALB, PTSTEP, PSNOWLIQ(:,1), PSNOWDZ(:,1),     &
                PSNOWRHO(:,1), GSFCMELT, PSR, PPERMSNOWFRAC)
!
!
...
!####################################################################
!####################################################################
      SUBROUTINE SNOW3LRAD(PSNOWDZMIN, PSW_RAD, PSNOWALB,            &
...
!
USE MODE_SEAICE_SICE_SNOW3L,   ONLY : SNOW3LDOPT, SNOW3LRADABS_SFC
!
IMPLICIT NONE
!
!*      0.1    declarations of arguments
...
!
REAL, DIMENSION(SIZE(PSNOWRHO,1),SIZE(PSNOWRHO,2)) :: ZDSGRAIN, ZCOEF, ZSNOWDZ, ZAGE, ZSXNU, ZSNOW
...
DO JJ = 1, INLVLS
 ZCOEF(:,JJ)   = (1.0 - PSNOWALB(:)) * EXP(-ZSXNU(:,JJ)*ZSNOW(:,JJ))
ENDDO

PSWNETSNOW(:)        = PSW_RAD(:)*(1.0 - PSNOWALB(:))
...
USE MODD_SNOW_PAR, ONLY : XSNOWDMIN
USE MODD_SICE_SNOW_PAR, ONLY: XRHOSMIN_ES, XRHOSMAX_ES
!
IMPLICIT NONE
!
!*      0.1    declarations of arguments
...
END SUBROUTINE SEAICE_SICE_SNOW3L
