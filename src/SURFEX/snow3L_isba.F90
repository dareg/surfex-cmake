...
!
   ...
   DO JIMP=1,NIMPUR
     DO JWRK=1,INLVLS
        DO JJ=1,SIZE(PSNOWSWE,1)
           PSNOWIMPURV2 (JJ,JWRK,JIMP)=(1.0-ZSNOWABLAT_DELTA(JJ))*PSNOWIMPURV2(JJ,JWRK,JIMP) !N6K
        ENDDO
     ENDDO
   ENDDO
!  
...
REAL, DIMENSION(KSIZE1,KSIZE2,NIMPUR) :: ZPSNOWIMP_CONC !N6K
!
  ...
  GCOMPUTECRODIAG=(SIZE(PSNOWDEND)>1).AND.(MOD(TPTIME%TIME,XTSTEP_OUTPUT) == 0.)
   ...
   IF (GCOMPUTECRODIAG) THEN
     CALL SNOWCRO_DIAG(HSNOWMETAMO,&
                     ...
                     ZP_SNOWLIQ,ZP_DIRCOSZW,ZP_SNOWIMPURV2,ZP_SNOWDEND,ZP_SNOWSPHER,ZP_SNOWSIZE,ZP_SNOWSSA,ZP_SNOWTYPEMEPRA,     &
                     ...
                     ZP_SNOWSWE_7DAYS,ZP_SNOWRAM_SONDE,ZP_SNOW_WETTHICKNESS,ZP_SNOW_REFROZENTHICKNESS,ZPSNOWIMP_CONC)
   ENDIF
  ...
  ! This is equivalent to test the value of DGMI%LPROSNOW which does not enter in ISBA
    DO JWRK=1,KSIZE2
      DO JJ=1,KSIZE1
        ...
        DO JIMP=1,NIMPUR
          PSNOWIMP_CONC (JI,JWRK,JIMP) = ZPSNOWIMP_CONC (JJ,JWRK,JIMP)
        ENDDO
      ENDDO
    ENDDO
  ENDIF
