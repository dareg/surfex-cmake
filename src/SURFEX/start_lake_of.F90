...
!                                    instead of aborting
!                    03/2023, E. Kourzeneva : implementing the inner loop on grid points of the target grid
!                                    and optimization
!------------------------------------------------------------------------------------------------------------
!
...
INTEGER, INTENT(IN) :: KDAY,   & ! The day number
                       KMONTH    ! The month number
REAL, DIMENSION(:), INTENT(IN) :: PLON, PLAT   ! Longitude and latitude of the center of the atmospheric model grid box, 
                                 ! deg. dec. (-180.0 ... 180.0), (-90.0 ... 90.0)
REAL, DIMENSION(:), INTENT(IN) :: PDEPTH       ! The depth of the lakes, m
REAL, DIMENSION(:), INTENT(OUT) :: PT_SNOW, &  ! the snow temperature, K (no snow at present, so equal to the ice temperature) 
                     PT_ICE,  &  ! the ice temperature, K
                     PT_MNW,  &  ! the mean water temperature, K
                     PT_WML,  &  ! the mixed layer temperature, K
                     PT_BOT,  &  ! the bottom temperature, K
...
!    
!*      5.     Loop on grid-ponts of the target grid
!
DO JI=1,SIZE(PLAT)
!
!*      6.     Check coordinates
!
  IF(PLON(JI).LT.-180. .OR. PLON(JI).GT.180.) CALL ABOR1_SFX("START_LAKE_OF: WRONG LONGITUDE (!!!!)")
  IF(PLAT(JI).LT.-90.  .OR. PLAT(JI).GT.90. ) CALL ABOR1_SFX("START_LAKE_OF: WRONG LATITUDE (!!!!)")
!
!*      7.     Calculate lon-lat indexes
!
  ZWLON = PLON(JI)
  ZWLAT = MAX(PLAT(JI),XFIRSTLAT)
!
