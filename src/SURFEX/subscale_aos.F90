...
REAL, DIMENSION(:), ALLOCATABLE   :: ZDX, ZDY, ZSEA, ZAVG_ZS     ! grid mesh size in x direction
...
IF (NRANK==NPIO) THEN
  ...
  CALL GET_MESH_DIM(UG%CGRID,UG%NGRID_FULL_PAR,U%NDIM_FULL,UG%XGRID_FULL_PAR,ZDX,ZDY,UG%XMESH_SIZE)
  !
  !
  ...
  CALL GET_ADJACENT_MESHES(UG%CGRID,UG%NGRID_FULL_PAR,U%NDIM_FULL,UG%XGRID_FULL_PAR,&
