...
!  R. El Khatib 20-Mar-2024 Fix arithmetic overflow
!     Modifications:
!     08/2023 M. Dian: No USURIC correction for TOUCANS; added C3TKEFREE in PCH
!-----------------------------------------------------------------------
...
LOGICAL :: LLM123
INTEGER :: ITTYPE
REAL :: ZETKE_R(SIZE(PTA)),ZETKE_P(SIZE(PTA)),ZRITKE(SIZE(PTA)),&
&ZMRIFPP(SIZE(PTA)),ZCHI3TA(SIZE(PTA)),ZPHI3TA(SIZE(PTA)),ZFMTKE(SIZE(PTA)),&
&ZFTTKE(SIZE(PTA))
REAL :: ZXETKE_R,ZXETKE_RIFC,ZEPS8,ZSM0,ZLAMF_X,ZLAMBDA,ZSF,&
...
IF (LCOEFKTKE) THEN
  ZUSURIC=0.  ! no USURIC correction for TOUCANS
ELSE
...
IF (LCOEFKTKE) THEN
  ...
  ZSM0=NUPTKE**4/2.
  ZLAMF_X=SQRT(375.0*C3TKEFREE**2*(C3TKEFREE**2+2.*C3TKEFREE+1.)*ZSM0**2 &
   & -20.0*(C3TKEFREE**2+C3TKEFREE-1.)*ZSM0+1.0)
  ZLAMBDA=SQRT(5.0)*3.0/4.0*ZLAMF_X-30.0*C3TKEFREE &
   & *(C3TKEFREE+1.)*ZSM0+0.25
  ZSF=(10.0-75.0*C3TKEFREE*(C3TKEFREE+1.)*ZSM0)/(8.0*ZLAMBDA)

  ZXETKE_LAM1=4.0/15.0*ZLAMBDA
  ZXETKE_LAM3=(14.0*ZLAMBDA*ZSF+5.0*ZLAMBDA)/75.0
  ZXETKE_LAM2=(5.0*ZLAMBDA-6.0*ZLAMBDA*ZSF)/25.0
  ...
  ELSE IF (ITTYPE==2) THEN
   ...
   ZXETKE_R=1.0-2.0*ZLAMBDA*(44.0*ZSF**2+160.0*ZSF-25.0)/375.0
   ZXETKE_LAM4=0.0
   ZXETKE_RIFC=(15.0*ZSM0*C3TKEFREE**2*(C3TKEFREE+1.0))&
   & /(6.0*ETKE_OLAM+2.0*C3TKEFREE)
  ELSE IF ((ITTYPE==3).OR.(ITTYPE==4)) THEN
   ! Model II or QNSE


   ZXETKE_R=-4.0*C3TKEFREE/75.0*&
    &(88.0*ZLAMBDA*ZSF**2+320.0*ZLAMBDA*ZSF-50.0*ZLAMBDA-375.0)/&
    &(8.0*ZLAMBDA*C3TKEFREE*ZSF+10.0*ZLAMBDA*C3TKEFREE+20.0*C3TKEFREE+15.0)


   


   ZXETKE_LAM4=ZLAMBDA/4.0
   ZXETKE_RIFC=(30*C3TKEFREE**3*ZSM0)/&
    &(12.0*ETKE_OLAM+(-60.0*C3TKEFREE**2*ZXETKE_LAM3-20.0*C3TKEFREE**2&
    &*ZXETKE_LAM2+4.0*C3TKEFREE)*ZXETKE_LAM4-15.0*C3TKEFREE**2*ZXETKE_LAM1     &
    & +4.0*C3TKEFREE)
...
IF (LCOEFKTKE) THEN
  ...
  IF (LLM123) THEN
    ...
    DO JLON=1,SIZE(PTA)
      ...
      ZSIG=SIGN(0.5,ZRITKE(JLON))+0.5

      ZMRIFPP(JLON)=-(sqrt((ZETKE_R(JLON)*                            &
      & (ZRITKE(JLON)*C3TKEFREE+ZETKE_P(JLON)))**2                          &
      & -4.0*ZRITKE(JLON)*C3TKEFREE*ZETKE_R(JLON)*ZETKE_P(JLON)**2)    &
  ...
  IF (ITTYPE == 4) THEN
    ...
    DO JLON=1,SIZE(PTA)
      ...
      Z_ETKE_RU=(RQNSE_GU1*ZRITKE(JLON)+RQNSE_S0)/&
       &(RQNSE_GU2*ZRITKE(JLON)+1.0)
      !  STABLE
      Z_ETKE_R=(ZRITKE(JLON)*(RQNSE_GS1*ZRITKE(JLON)+&
       & RQNSE_GS2)+RQNSE_S0)/&
       &(ZRITKE(JLON)*(RQNSE_GS3*ZRITKE(JLON)+RQNSE_GS4)+1.0)
      ZETKE_R(JLON)=ZSIG*Z_ETKE_R+(1.0-ZSIG)*Z_ETKE_RU
      ZETKE_P(JLON)=ZXETKE_RIFC
  ...
  IF (ITTYPE == 5) THEN
    ...
    DO JLON=1,SIZE(PTA)
      ...
      ZS_CQ=-0.5*(2.0*ZS_CW**3-(ZS_AA2*ZS_CW-ZS_AA1)/ZS_AA4)
      ZS_CDIS=ZS_CQ**2+ZS_CP

      ZS_ZSOL = HUGE(1._JPRB)

      IF (ZS_CDIS < 0.0) THEN
      !         three real solutions!
      !         Confine the argument of ACOS to the interval [-1;1]!
        ZS_CPHI=ACOS(MIN(1.,MAX(-1.,ZS_CQ/SQRT(-ZS_CP))))
        ZS_CP=2.0*(-ZS_CP)**(0.5*ZS_CTHIRD)
        ZS_CU1=ZS_CP*COS((ZS_CPHI+2.0*XPI)*ZS_CTHIRD)-ZS_CW
        ZS_CU2=ZS_CP*COS((ZS_CPHI+4.0*XPI)*ZS_CTHIRD)-ZS_CW
        ZS_CU3=ZS_CP*COS((ZS_CPHI+6.0*XPI)*ZS_CTHIRD)-ZS_CW
      ...
      ZETKE_P(JLON)=ZETKE_R(JLON)&
       & /(ETKE_OLAM*ZETKE_R(JLON)/(EFB_AZ0*C3TKEFREE)+1.0)
  ...
  IF (ITTYPE == 6) THEN
    ...
    DO JLON=1,SIZE(PTA)
      ...
      Z_ETKE_RU=(RLOUIS_GU1*ZRITKE(JLON)+RLOUIS_S0)/&
       &(RLOUIS_GU2*ZRITKE(JLON)+1.0)
      !  STABLE
      Z_ETKE_R=(ZRITKE(JLON)*(RLOUIS_GS1*ZRITKE(JLON)+&
       & RLOUIS_GS2)+RLOUIS_S0)/&
       &(ZRITKE(JLON)*(RLOUIS_GS3*ZRITKE(JLON)+RLOUIS_GS4)+1.0)
      ZETKE_R(JLON)=ZSIG*Z_ETKE_R+(1.0-ZSIG)*Z_ETKE_RU

      !UNSTABLE
      Z_ETKE_PU=(PLOUIS_GU1*ZRITKE(JLON)+PLOUIS_S0)/&
       &(PLOUIS_GU2*ZRITKE(JLON)+1.0)
      !STABLE
      Z_ETKE_P=(ZRITKE(JLON)*(PLOUIS_GS1*ZRITKE(JLON)+&
       & PLOUIS_GS2)+PLOUIS_S0)/&
       &(ZRITKE(JLON)*(PLOUIS_GS3*ZRITKE(JLON)+PLOUIS_GS4)+1.0)
      ZETKE_P(JLON)=ZSIG*Z_ETKE_P+(1.0-ZSIG)*Z_ETKE_PU
  ...
  DO JLON=1,SIZE(PTA)
    ZCHI3TA(JLON)=(1.0-ZMRIFPP(JLON)/ZETKE_R(JLON))&
    & /(1.0-ZMRIFPP(JLON))
    ZPHI3TA(JLON)=(1.0-ZMRIFPP(JLON)/ZETKE_P(JLON))&
    & /(1.0-ZMRIFPP(JLON))
  ENDDO

  DO JLON=1,SIZE(PTA)
    ZAUX=SQRT(ZCHI3TA(JLON)*(1.-ZMRIFPP(JLON)))
    ZFMTKE(JLON)=ZCHI3TA(JLON)*ZAUX
    ZFTTKE(JLON)=ZPHI3TA(JLON)*ZAUX
  ENDDO
  IF (LCOEFK_F1) THEN
    DO JLON=1,SIZE(PTA)
      ZFMTKE(JLON)=MIN(ZCHI3TA(JLON),ZFMTKE(JLON))
      ZFTTKE(JLON)=MIN(ZPHI3TA(JLON),ZFTTKE(JLON))
    ENDDO
  ENDIF
  ...
  DO JLON=1,SIZE(PTA)
    ...
    PCH(JLON)=C3TKEFREE*ZFTTKE(JLON)*ZCDNH(JLON)
  ENDDO
