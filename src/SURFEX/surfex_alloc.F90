...
USE MODD_ISBA_n, ONLY : ISBA_S_INIT, ISBA_K_INIT, ISBA_P_INIT, ISBA_PE_INIT, &
...
!
  ...
  !
  CALL AGRI_NP_INIT(YDSURFEX%IM%NAG,NVEGTYPE)
  CALL CH_ISBA_INIT(YDSURFEX%IM%CHI)
  CALL CH_ISBA_NP_INIT(YDSURFEX%IM%NCHI,NVEGTYPE)
  CALL DATA_ISBA_INIT(YDSURFEX%IM%DTI)
  CALL DIAG_EVAP_ISBA_INIT(YDSURFEX%IM%ID%DE)
  CALL DIAG_EVAP_ISBA_INIT(YDSURFEX%IM%ID%DEC)
  CALL DIAG_EVAP_ISBA_PATCH_INIT(YDSURFEX%IM%ID%DEP,NVEGTYPE)
  CALL DIAG_EVAP_ISBA_PATCH_INIT(YDSURFEX%IM%ID%DEPC,NVEGTYPE)  
  CALL DIAG_OPTIONS_INIT(YDSURFEX%IM%ID%O)
  CALL DIAG_INIT(YDSURFEX%IM%ID%D)
  CALL DIAG_INIT(YDSURFEX%IM%ID%DC)
  CALL DIAG_PATCH_INIT(YDSURFEX%IM%ID%DP,NVEGTYPE)
  CALL DIAG_PATCH_INIT(YDSURFEX%IM%ID%DPC,NVEGTYPE)
  CALL DIAG_MISC_ISBA_INIT(YDSURFEX%IM%ID%DM)
  CALL DIAG_MISC_ISBA_PATCH_INIT(YDSURFEX%IM%ID%DMP,NVEGTYPE)
  CALL GR_BIOG_NP_INIT(YDSURFEX%IM%NGB,NVEGTYPE)
  CALL GR_BIOG_INIT(YDSURFEX%IM%GB)
  CALL CANOPY_INIT(YDSURFEX%IM%SB)
  CALL GRID_INIT(YDSURFEX%IM%G)
  CALL GRID_NP_INIT(YDSURFEX%IM%NG,NVEGTYPE)
  CALL SSO_INIT(YDSURFEX%IM%ISS)
  CALL SSO_NP_INIT(YDSURFEX%IM%NISS,NVEGTYPE)
  CALL ISBA_OPTIONS_INIT(YDSURFEX%IM%O)
  CALL ISBA_S_INIT(YDSURFEX%IM%S)
  CALL ISBA_K_INIT(YDSURFEX%IM%K)
  CALL ISBA_NK_INIT(YDSURFEX%IM%NK,NVEGTYPE)
  CALL ISBA_NP_INIT(YDSURFEX%IM%NP,NVEGTYPE)
  CALL ISBA_NPE_INIT(YDSURFEX%IM%NPE,NVEGTYPE)
  CALL PACK_DIAG_ISBA_INIT(YDSURFEX%IM%PKD)
  CALL DIAG_INIT(YDSURFEX%IM%PKD%D)
  CALL DIAG_INIT(YDSURFEX%IM%PKD%DP)
  CALL DIAG_EVAP_ISBA_INIT(YDSURFEX%IM%PKD%DE)
  CALL DIAG_EVAP_ISBA_INIT(YDSURFEX%IM%PKD%DEP)
  CALL DIAG_MISC_ISBA_INIT(YDSURFEX%IM%PKD%DMI) 
  CALL GR_BIOG_INIT(YDSURFEX%IM%PKD%GB)
  !
  CALL DATA_ISBA_INIT(YDSURFEX%GDM%DTI)
  CALL DIAG_EVAP_ISBA_INIT(YDSURFEX%GDM%VD%DE)
  CALL DIAG_EVAP_ISBA_INIT(YDSURFEX%GDM%VD%DEP)
  CALL DIAG_INIT(YDSURFEX%GDM%VD%D)
  CALL DIAG_INIT(YDSURFEX%GDM%VD%DP)
  CALL DIAG_MISC_ISBA_INIT(YDSURFEX%GDM%VD%DM)
  ...
  CALL DIAG_EVAP_ISBA_INIT(YDSURFEX%GRM%VD%DE)
  CALL DIAG_EVAP_ISBA_INIT(YDSURFEX%GRM%VD%DEP)
  CALL DIAG_INIT(YDSURFEX%GRM%VD%D)
  CALL DIAG_INIT(YDSURFEX%GRM%VD%DP)
  CALL DIAG_MISC_ISBA_INIT(YDSURFEX%GRM%VD%DM)
  ...
  CALL DIAG_MISC_TEB_INIT(YDSURFEX%TM%TD%DMTC,NTEB_PATCH_MAX)
  CALL DIAG_MISC_TEB_INIT(YDSURFEX%TM%TD%DMT,NTEB_PATCH_MAX)
  ...
  CALL DIAG_PATCH_INIT(YDSURFEX%DUP,NTILESFC) 
  CALL DIAG_PATCH_INIT(YDSURFEX%DUPC,NTILESFC)
  ...
  CALL DST_NP_INIT(YDSURFEX%NDST,NVEGTYPE)
