...
!
 CHARACTER(LEN=12) :: YREC,CDUM
INTEGER :: IFIELD,JFIELD,JREC
...
IF (.NOT.(LXIOS .AND. (TRIM(HPROGRAM)=='XIOS' ))) THEN
  ! Check the possible existence for this HREC
  YREC = HREC
  SELECT CASE(HREC(1:4))
    CASE("TEB1","TEB2","TEB3","TEB4","TEB5","TEB6","TEB7","TEB8","TEB9")
      YREC=HREC(6:LEN(HREC))
  END SELECT
  DO JREC=1,NCPT_WRITE
    IF ( TRIM(YREC) == TRIM(CREC_TR(JREC)) ) THEN
     ONOWRITE = LNOWRITE(JREC)
     IF (LHOOK) CALL DR_HOOK('TEST_RECORD_LEN',1,ZHOOK_HANDLE)
     RETURN
    ENDIF
  ENDDO
...
IF ( NCPT_WRITE > MAX_NOWRITE_REC) THEN
    WRITE(CDUM,'(I8)')NCPT_WRITE
    CALL ABOR1_SFX('TEST_RECORD_LEN: NCPT_WRITE > MAX_NOWRITE_REC --> '//CDUM)
ENDIF
!
