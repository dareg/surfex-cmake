...
IF (KNPATCH==1) THEN
  ...
  !
  IF (IO%LFLOOD) THEN
    PCPL_EFLOOD(:,KPATCH) = DEK%XLE_FLOOD(:) / PK%XPLVTT(:) + DEK%XLEI_FLOOD(:) / PK%XPLSTT(:)
...
ELSE
  ...
  IF (IO%LFLOOD) THEN
    DO JI=1,KSIZE
      IMASK = KMASK(JI)
      PCPL_EFLOOD(IMASK,KPATCH) = DEK%XLE_FLOOD(JI) / PK%XPLVTT(JI) + DEK%XLEI_FLOOD(JI) / PK%XPLSTT(JI)
