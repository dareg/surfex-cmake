...
IF (KTILE==1) THEN
  ...
  IF (U%CSEA=='SEAFLX') THEN
    CALL UPDATE_ESM_SEAFLUX_n(U%NSIZE_SEA,KSW,ZP_ZENITH,ZP_DIR_ALB, &
...
ELSEIF (KTILE==2) THEN
  ...
  IF (U%CWATER=='WATFLX') THEN   
    CALL UPDATE_ESM_WATFLUX_n(U%NSIZE_WATER,KSW,ZP_ZENITH,ZP_DIR_ALB, &
                              ZP_SCA_ALB,ZP_EMIS,ZP_TRAD,ZP_TSURF   )
  ELSEIF (U%CWATER=='FLAKE ') THEN
    CALL UPDATE_ESM_FLAKE_n(U%NSIZE_WATER,KSW,ZP_ZENITH,ZP_DIR_ALB, &
                            ZP_SCA_ALB,ZP_EMIS,ZP_TRAD,ZP_TSURF   )
  ELSE
