...
!
REAL, DIMENSION(SIZE(I%M%X%XVEGTYPE,1),SIZE(I%M%X%XVEGTYPE,2)) :: ZVEGTYPE
!
...
DO JPATCH=1,I%O%NPATCH
   DO JJ=1,I%IP%NSIZE_NATURE_P(JPATCH)
      ZVEGTYPE(JJ,JPATCH) = I%M%X%XVEGTYPE(I%IP%NR_NATURE_P(JJ,JPATCH),JPATCH)
   ENDDO
ENDDO
!
!-------------------------------------------------------------------------------------
...
DO JPATCH=1,I%O%NPATCH
  !
  IF(I%IP%NSIZE_NATURE_P(JPATCH)>0) CALL TREAT_NATURE(I%IP%NSIZE_NATURE_P(JPATCH),JPATCH)
  !
ENDDO
!-------------------------------------------------------------------------------
!
...
!
PDIR_ALB_WITH_SNOW(:,:,:)=I%I%XDIR_ALB_WITH_SNOW (:,:,:)
PSCA_ALB_WITH_SNOW(:,:,:)=I%I%XSCA_ALB_WITH_SNOW (:,:,:)
!
...
!
REAL, DIMENSION(KSIZE,SIZE(I%R%TSNOW%WSNOW,2)) :: ZLAYERSWE
REAL, DIMENSION(KSIZE,SIZE(I%R%TSNOW%WSNOW,2)) :: ZLAYERRHO
REAL, DIMENSION(KSIZE,SIZE(I%R%TSNOW%WSNOW,2)) :: ZLAYERAGE
!
...
REAL, DIMENSION(KSIZE,SIZE(I%IP%XABC)) :: ZIACAN_SUNLIT, ZIACAN_SHADE, ZFRAC_SUN, ZIACAN
...
!
IMASK(:)=I%IP%NR_NATURE_P(1:KSIZE,KPATCH)
!
 CALL PACK_SAME_RANK(IMASK(:),I%R%TSNOW%WSNOW(:,:,KPATCH),ZLAYERSWE(:,:))
 CALL PACK_SAME_RANK(IMASK(:),I%R%TSNOW%RHO  (:,:,KPATCH),ZLAYERRHO(:,:))
!  
 CALL PACK_SAME_RANK(IMASK(:),I%R%TSNOW%ALB  (:,KPATCH),ZSNOWALB(:))
!
...
!
    CALL PACK_SAME_RANK(IMASK(:),I%R%TSNOW%AGE  (:,:,KPATCH),ZLAYERAGE(:,:))
!
    CALL PACK_SAME_RANK(IMASK(:),I%IP%XVEGTYPE_PATCH(:,NVT_SNOW,KPATCH),ZPERMSNOWFRAC(:))    
!
  ENDIF
!
ELSE
...
IF (HSNOW=='3-L' .OR. HSNOW=='CRO') THEN
  CALL PACK_SAME_RANK(IMASK(:),I%R%TSNOW%ALBVIS  (:,KPATCH),ZSNOWALBVIS(:))
  CALL PACK_SAME_RANK(IMASK(:),I%R%TSNOW%ALBNIR  (:,KPATCH),ZSNOWALBNIR(:))
ENDIF
!   
!-------------------------------------------------------------------------------
!
...
!
IF ( HSNOW=='EBA' ) CALL UNPACK_SAME_RANK(IMASK(:),ZPSNV_A(:),I%R%XPSNV_A(:,KPATCH),ZPUT0) 
!
!-------------------------------------------------------------------------------
!
...
!
      CALL PACK_SAME_RANK(IMASK(:),I%R%XLAI_EFFC(:,KPATCH),ZLAI_EFFC(:))
      CALL PACK_SAME_RANK(IMASK(:),I%R%XFAPARC(:,KPATCH),  ZFAPARC(:)  )
      CALL PACK_SAME_RANK(IMASK(:),I%R%XFAPIRC(:,KPATCH),  ZFAPIRC(:)  )
      CALL PACK_SAME_RANK(IMASK(:),I%R%XMUS(:,KPATCH),     ZMUS(:)     )
!
...
!
      DO JSWB=1,ISWB
         ...
         END WHERE
         !
         CALL RADIATIVE_TRANSFERT(I%O%LAGRI_TO_GRASS, ZVEGTYPE(1:KSIZE,:),           &
              ZALBVIS_VEG, ZALBVIS_TSOIL, ZALBNIR_VEG, ZALBNIR_TSOIL,              &
              ZGLOBAL_SW, ZLAIN, ZZENITH, I%IP%XABC,                                  &
...
! Unpack variable
!
 CALL UNPACK_SAME_RANK(IMASK(:),ZPSNG (:),I%R%XPSNG  (:,KPATCH),ZPUT0)     
 CALL UNPACK_SAME_RANK(IMASK(:),ZPSNV (:),I%R%XPSNV  (:,KPATCH),ZPUT0)     
 CALL UNPACK_SAME_RANK(IMASK(:),ZPSN  (:),I%R%XPSN   (:,KPATCH),ZPUT0)  
 CALL UNPACK_SAME_RANK(IMASK(:),ZEMIST(:),PEMIST (:,KPATCH),ZPUT0)  
 CALL UNPACK_SAME_RANK(IMASK(:),ZDIR_ALB_WITH_SNOW (:,:),I%I%XDIR_ALB_WITH_SNOW (:,:,KPATCH),ZPUT0)  
 CALL UNPACK_SAME_RANK(IMASK(:),ZSCA_ALB_WITH_SNOW (:,:),I%I%XSCA_ALB_WITH_SNOW (:,:,KPATCH),ZPUT0)
!
!-------------------------------------------------------------------------------
...
!
 CALL PACK_SAME_RANK(KMASK(:),I%R%XTG(:,1,KPATCH),ZTG(:))
!
 CALL PACK_SAME_RANK(KMASK(:),PZENITH(:),ZZENITH (:))
 CALL PACK_SAME_RANK(KMASK(:),I%I%XFFLOOD(:),ZFFLOOD (:))
!      
...
ENDWHERE
!
 CALL UNPACK_SAME_RANK(KMASK(:),ZFFG    (:),I%I%XFFG    (:,KPATCH),ZPUT0)     
 CALL UNPACK_SAME_RANK(KMASK(:),ZFFV    (:),I%I%XFFV    (:,KPATCH),ZPUT0)     
 CALL UNPACK_SAME_RANK(KMASK(:),ZFFROZEN(:),I%I%XFFROZEN(:,KPATCH),ZPUT0)     
 CALL UNPACK_SAME_RANK(KMASK(:),PFF     (:),I%I%XFF     (:,KPATCH),ZPUT0) 
 CALL UNPACK_SAME_RANK(KMASK(:),PEMISF  (:),I%I%XEMISF  (:,KPATCH),XUNDEF)
 CALL UNPACK_SAME_RANK(KMASK(:),PALBF   (:),I%I%XALBF   (:,KPATCH),XUNDEF)     
!
