!     #########
SUBROUTINE UPDATE_RAD_ISBA_n (IO, MT, MM, MA, IP, I, IR, &
                              PVEGTYPE, PZENITH, PSW_BANDS, &
...
!
REAL, DIMENSION(SIZE(PVEGTYPE,1),SIZE(PVEGTYPE,2)) :: ZVEGTYPE
!
INTEGER :: JP, ISWB, JJ
...
ZVEGTYPE(:,:) = 0.
DO JP=1,IO%NPATCH
   DO JJ=1,IP%NSIZE_NATURE_P(JP)
      ZVEGTYPE(JJ,JP) = PVEGTYPE(IP%NR_NATURE_P(JJ,JP),JP)
   ENDDO
ENDDO
!
!-------------------------------------------------------------------------------------
...
DO JP=1,IO%NPATCH
  !
  IF(IP%NSIZE_NATURE_P(JP)>0) CALL TREAT_NATURE(IP%NSIZE_NATURE_P(JP),JP)
  !
ENDDO
!-------------------------------------------------------------------------------
!
...
!
REAL, DIMENSION(KSIZE,SIZE(IR%TSNOW%WSNOW,2)) :: ZLAYERSWE
REAL, DIMENSION(KSIZE,SIZE(IR%TSNOW%WSNOW,2)) :: ZLAYERRHO
REAL, DIMENSION(KSIZE,SIZE(IR%TSNOW%WSNOW,2)) :: ZLAYERAGE
!
...
REAL, DIMENSION(KSIZE,SIZE(IP%XABC)) :: ZIACAN_SUNLIT, ZIACAN_SHADE, ZFRAC_SUN, ZIACAN
...
!
IMASK(:)=IP%NR_NATURE_P(1:KSIZE,KPATCH)
!
 CALL PACK_SAME_RANK(IMASK(:),IR%TSNOW%WSNOW(:,:,KPATCH),ZLAYERSWE(:,:))
 CALL PACK_SAME_RANK(IMASK(:),IR%TSNOW%RHO  (:,:,KPATCH),ZLAYERRHO(:,:))
!  
 CALL PACK_SAME_RANK(IMASK(:),IR%TSNOW%ALB  (:,KPATCH),ZSNOWALB(:))
!
...
!  CALL PACK_SAME_RANK(IMASK(:),MM%XLAIGV     (:,KPATCH),ZLAI        (:))
!  CALL PACK_SAME_RANK(IMASK(:),MT%XLAI       (:,KPATCH),ZLAIV       (:))
!  CALL PACK_SAME_RANK(IMASK(:),MM%XZ0LITTER  (:,KPATCH),ZZ0         (:))
!  CALL PACK_SAME_RANK(IMASK(:),MM%XGNDLITTER (:,KPATCH),ZVEG        (:))
  ZVEG(:)=0. ! Set veg=0 for MEB to get bare soil conditions for snow cover and
!            ! flood fraction
  CALL PACK_SAME_RANK(IMASK(:),MM%XH_VEG     (:,KPATCH),ZH_VEG      (:))
!
  IF(PRESENT(PDIR_SW))THEN
!
    CALL PACK_SAME_RANK(IMASK(:),PDIR_SW    (:,:),     ZDIR_SW     (:,:))
    CALL PACK_SAME_RANK(IMASK(:),PSCA_SW    (:,:),     ZSCA_SW     (:,:))
    CALL PACK_SAME_RANK(IMASK(:),MT%XALBNIR_VEG  (:,KPATCH),ZALBNIR_VEG  (:))
    CALL PACK_SAME_RANK(IMASK(:),MA%XALBNIR_SOIL (:,KPATCH),ZALBNIR_SOIL (:))
    CALL PACK_SAME_RANK(IMASK(:),MT%XALBVIS_VEG  (:,KPATCH),ZALBVIS_VEG  (:))
    CALL PACK_SAME_RANK(IMASK(:),MA%XALBVIS_SOIL (:,KPATCH),ZALBVIS_SOIL (:))
!
    CALL PACK_SAME_RANK(IMASK(:),IR%TSNOW%AGE  (:,:,KPATCH),ZLAYERAGE(:,:))
!
    CALL PACK_SAME_RANK(IMASK(:),IP%XVEGTYPE_PATCH(:,NVT_SNOW,KPATCH),ZPERMSNOWFRAC(:))    
!
  ENDIF
!
ELSE
!  CALL PACK_SAME_RANK(IMASK(:),MT%XLAI       (:,KPATCH),ZLAI    (:))
!  CALL PACK_SAME_RANK(IMASK(:),MT%XZ0        (:,KPATCH),ZZ0     (:))
  CALL PACK_SAME_RANK(IMASK(:),MT%XVEG       (:,KPATCH),ZVEG    (:))
!  ZALBNIR_VEG(:)  = XUNDEF
!  ZALBNIR_SOIL(:) = XUNDEF
!  ZALBVIS_VEG(:)  = XUNDEF
!  ZALBVIS_SOIL(:) = XUNDEF
...
!
 CALL PACK_SAME_RANK(IMASK(:),MT%XLAI       (:,KPATCH),ZLAI    (:))
 CALL PACK_SAME_RANK(IMASK(:),MT%XZ0        (:,KPATCH),ZZ0     (:))
 CALL PACK_SAME_RANK(IMASK(:),MT%XEMIS      (:,KPATCH),ZEMIS   (:))
 CALL PACK_SAME_RANK(IMASK(:),MT%XALBNIR    (:,KPATCH),ZALBNIR (:))
 CALL PACK_SAME_RANK(IMASK(:),MT%XALBVIS    (:,KPATCH),ZALBVIS (:))
 CALL PACK_SAME_RANK(IMASK(:),MT%XALBUV     (:,KPATCH),ZALBUV  (:))
!
IF (IR%TSNOW%SCHEME=='3-L' .OR. IR%TSNOW%SCHEME=='CRO') THEN
  CALL PACK_SAME_RANK(IMASK(:),IR%TSNOW%ALBVIS  (:,KPATCH),ZSNOWALBVIS(:))
  CALL PACK_SAME_RANK(IMASK(:),IR%TSNOW%ALBNIR  (:,KPATCH),ZSNOWALBNIR(:))
ENDIF
!   
!-------------------------------------------------------------------------------
!
 CALL ISBA_SNOW_FRAC(IR%TSNOW%SCHEME, ZLAYERSWE, ZLAYERRHO, ZSNOWALB,    &
                     ZVEG, ZLAI, ZZ0,ZPSN(:), ZPSNV_A(:), ZPSNG(:), ZPSNV(:) )  
!
IF ( IR%TSNOW%SCHEME=='EBA' ) CALL UNPACK_SAME_RANK(IMASK(:),ZPSNV_A(:),IR%XPSNV_A(:,KPATCH),ZPUT0) 
!
!-------------------------------------------------------------------------------
!
...
!
      CALL PACK_SAME_RANK(IMASK(:),IR%XLAI_EFFC(:,KPATCH),ZLAI_EFFC(:))
      CALL PACK_SAME_RANK(IMASK(:),IR%XFAPARC(:,KPATCH),  ZFAPARC(:)  )
      CALL PACK_SAME_RANK(IMASK(:),IR%XFAPIRC(:,KPATCH),  ZFAPIRC(:)  )
      CALL PACK_SAME_RANK(IMASK(:),IR%XMUS(:,KPATCH),     ZMUS(:)     )
!
...
!
      DO JSWB=1,ISWB
         ...
         END WHERE
         !
         CALL RADIATIVE_TRANSFERT(IO%LAGRI_TO_GRASS, ZVEGTYPE(1:KSIZE,:),           &
              ZALBVIS_VEG, ZALBVIS_TSOIL, ZALBNIR_VEG, ZALBNIR_TSOIL,              &
              ZGLOBAL_SW, ZLAIN, ZZENITH, IP%XABC,                                  &
...
!
  CALL ISBA_EMIS_MEB(ZPSN, ZPALPHAN, ZSIGMA_F, ZSIGMA_FN, ZEMISSN, ZEMIST                       )
!
ELSE
!        
...
! Unpack variable
!
 CALL UNPACK_SAME_RANK(IMASK(:),ZPSNG (:),IR%XPSNG  (:,KPATCH),ZPUT0)     
 CALL UNPACK_SAME_RANK(IMASK(:),ZPSNV (:),IR%XPSNV  (:,KPATCH),ZPUT0)     
 CALL UNPACK_SAME_RANK(IMASK(:),ZPSN  (:),IR%XPSN   (:,KPATCH),ZPUT0)  
...
!
 CALL PACK_SAME_RANK(KMASK(:),IR%XTG(:,1,KPATCH),ZTG(:))
!
