!   ##########################################################################
SUBROUTINE UTCI_TEB(T, TDU, PTI_BLD, PQI_BLD, PU10, PPS, PREF_SW_GRND, PREF_SW_FAC, &
...
TYPE(DIAG_UTCI_TEB_t), INTENT(INOUT) :: TDU
!
...
ZUIN = 0.5
TDU%XUTCI_IN = UTCI_APPROX(PTI_BLD - XTT, ZEHPA, PTRAD_IN - XTT, ZUIN)
!
! 2-calculation of UTCI_OUTSUN
ZEHPA = T%XQ_CANYON * PPS / (0.622 + 0.378 * T%XQ_CANYON) /100.
TDU%XTRAD_SUN = TRAD_BODY(PSCA_SW, PREF_SW_FAC, PREF_SW_GRND, PEMIT_LW_FAC, PEMIT_LW_GRND, PLW_RAD, &
                          T%XBLD, T%XBLD_HEIGHT, T%XWALL_O_HOR, PDIR_SW, PZENITH )
TDU%XUTCI_OUTSUN = UTCI_APPROX(T%XT_CANYON - XTT, ZEHPA, TDU%XTRAD_SUN - XTT, PU10)
!
! 3-calculation of UTCI_OUTSHADE
ZDIR_SW=0.
ZZENITH=0.
!!TDU%XTRAD_SHADE = TRAD_BODY(PSCA_SW,PREF_SW_FAC, PREF_SW_GRND, &
!!                      PEMIT_LW_FAC, PEMIT_LW_GRND, PLW_RAD,&
!!                      T%XBLD, T%XBLD_HEIGHT, T%XWALL_O_HOR, ZDIR_SW, ZZENITH)
TDU%XTRAD_SHADE = TRAD_BODY(PSCA_SW,PREF_SW_FAC, PREF_SW_GRND, PEMIT_LW_FAC, PEMIT_LW_GRND, PLW_RAD,&
                            T%XBLD, T%XBLD_HEIGHT, T%XWALL_O_HOR)
TDU%XUTCI_OUTSHADE = UTCI_APPROX(T%XT_CANYON - XTT, ZEHPA, TDU%XTRAD_SHADE - XTT, PU10)
