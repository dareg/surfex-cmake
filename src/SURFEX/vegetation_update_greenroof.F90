!     #########
    SUBROUTINE VEGETATION_UPDATE_GREENROOF (DTCO, DTI, IG, I, T, TOP, DTGD, GRM, &
                                            TPTIME,PTSTEP,KLU)  
!   ##########################################################################
!
!!****  *GREENROOF*  
...
!
ZVEG = GRM%TV%M%T%CUR%XVEG
ZZ0 = GRM%TV%M%T%CUR%XZ0
ZALBNIR = GRM%TV%M%T%CUR%XALBNIR
ZALBVIS = GRM%TV%M%T%CUR%XALBVIS
ZALBUV = GRM%TV%M%T%CUR%XALBUV
ZEMIS = GRM%TV%M%T%CUR%XEMIS
ZRSMIN = GRM%TV%M%T%CUR%XRSMIN
ZGAMMA = GRM%TV%M%T%CUR%XGAMMA
ZWRMAX_CF = GRM%TV%M%T%CUR%XWRMAX_CF
ZRGL = GRM%TV%M%T%CUR%XRGL
ZCV = GRM%TV%M%T%CUR%XCV
ZGMES = GRM%TV%M%T%CUR%XGMES
ZBSLAI = GRM%TV%M%T%CUR%XBSLAI
ZLAIMIN = GRM%TV%M%T%CUR%XLAIMIN
ZSEFOLD = GRM%TV%M%T%CUR%XSEFOLD
ZGC = GRM%TV%M%T%CUR%XGC
ZDMAX = GRM%TV%M%X%XDMAX
ZF2I = GRM%TV%M%T%CUR%XF2I
GSTRESS = GRM%TV%M%T%CUR%LSTRESS
ZALBNIR_VEG = GRM%TV%M%T%CUR%XALBNIR_VEG
ZALBVIS_VEG = GRM%TV%M%T%CUR%XALBVIS_VEG
ZALBUV_VEG = GRM%TV%M%T%CUR%XALBUV_VEG
ZALBNIR_SOIL = GRM%TV%M%A%XALBNIR_SOIL
ZALBVIS_SOIL = GRM%TV%M%A%XALBVIS_SOIL
ZALBUV_SOIL = GRM%TV%M%A%XALBUV_SOIL
ZCE_NITRO = GRM%TV%M%T%CUR%XCE_NITRO
ZCF_NITRO = GRM%TV%M%T%CUR%XCF_NITRO
ZCNA_NITRO = GRM%TV%M%T%CUR%XCNA_NITRO
! --------------------------------------------------------------------------------------
...
IF (GRM%TV%O%CPHOTO=='NON' .OR. GRM%TV%O%CPHOTO=='AGS' .OR. GRM%TV%O%CPHOTO=='AST') THEN
     CALL VEGETATION_UPDATE(DTCO, DTI, DTGD, GRM%DTI, IG, I%O, &
                         ...
                         GRM%TV%O%CISBA,(.NOT. GRM%TV%O%LPAR), GRM%TV%O%CPHOTO, .FALSE.,  &
                         GRM%TV%O%LTR_ML, 'GR ',                                  &
                         ZLAI,ZVEG,ZZ0,                                  &
                         ...
                         GRM%TV%O%CALBEDO, ZALBNIR_VEG, ZALBVIS_VEG, ZALBUV_VEG,  &
...
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
GRM%TV%M%T%CUR%XVEG = ZVEG
GRM%TV%M%T%CUR%XZ0 = ZZ0
GRM%TV%M%T%CUR%XALBNIR = ZALBNIR
GRM%TV%M%T%CUR%XALBVIS = ZALBVIS
GRM%TV%M%T%CUR%XALBUV = ZALBUV
GRM%TV%M%T%CUR%XEMIS = ZEMIS
GRM%TV%M%T%CUR%XRSMIN = ZRSMIN
GRM%TV%M%T%CUR%XGAMMA = ZGAMMA
GRM%TV%M%T%CUR%XWRMAX_CF = ZWRMAX_CF
GRM%TV%M%T%CUR%XRGL = ZRGL
GRM%TV%M%T%CUR%XCV = ZCV
GRM%TV%M%T%CUR%XGMES = ZGMES
GRM%TV%M%T%CUR%XBSLAI = ZBSLAI
GRM%TV%M%T%CUR%XLAIMIN = ZLAIMIN
GRM%TV%M%T%CUR%XSEFOLD = ZSEFOLD
GRM%TV%M%T%CUR%XGC = ZGC
GRM%TV%M%X%XDMAX = ZDMAX
GRM%TV%M%T%CUR%XF2I = ZF2I
GRM%TV%M%T%CUR%LSTRESS = GSTRESS
GRM%TV%M%T%CUR%XALBNIR_VEG = ZALBNIR_VEG
GRM%TV%M%T%CUR%XALBVIS_VEG = ZALBVIS_VEG
GRM%TV%M%T%CUR%XALBUV_VEG = ZALBUV_VEG
GRM%TV%M%A%XALBNIR_SOIL = ZALBNIR_SOIL
GRM%TV%M%A%XALBVIS_SOIL = ZALBVIS_SOIL
GRM%TV%M%A%XALBUV_SOIL = ZALBUV_SOIL
GRM%TV%M%T%CUR%XCE_NITRO = ZCE_NITRO
GRM%TV%M%T%CUR%XCF_NITRO = ZCF_NITRO
GRM%TV%M%T%CUR%XCNA_NITRO = ZCNA_NITRO
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!
