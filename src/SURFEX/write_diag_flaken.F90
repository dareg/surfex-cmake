!     #########
SUBROUTINE WRITE_DIAG_FLAKE_n (DTCO, DGU, IOB, U, CHF, DGF, DGMF, F, &
                               HPROGRAM,HWRITE)
!     ###############################################################################
!
...
!        
   IF (DGF%XDIAG_TSTEP==XUNDEF .OR. ABS(NINT(F%TTIME%TIME/DGF%XDIAG_TSTEP)*DGF%XDIAG_TSTEP-F%TTIME%TIME)<1.E-3 ) THEN
      CALL WRITE_DIAG_SEB_FLAKE_n(DTCO, DGU, IOB, U, CHF, DGF, &
                                  HPROGRAM)
      CALL WRITE_DIAG_MISC_FLAKE_n(DTCO, DGU, IOB, U, DGMF, &
                                   HPROGRAM)
   END IF
!        
ENDIF
