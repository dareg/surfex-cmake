!     #########
SUBROUTINE WRITE_DIAG_ISBA_n (DTCO, DGU, IOB, U, IM, DST, &
                              HPROGRAM,HWRITE)
!     ###############################################################################
!
...
IF (HWRITE/='PGD') THEN
  IF (IM%DGI%XDIAG_TSTEP==XUNDEF .OR. &
    ...
    CALL WRITE_DIAG_SEB_ISBA_n(DTCO, DGU, IOB, U, IM%CHI, IM%DGEI, IM%DGI, DST, IM%GB, IM%I, &
                               HPROGRAM)
    CALL WRITE_DIAG_MISC_ISBA_n(DTCO, DGU, IOB, U, IM%DGI, IM%DGMI, IM%I, &
                                HPROGRAM)
  END IF
END IF
!
IF (IM%DGI%LPGD) THEN
  IF (IM%DGI%XDIAG_TSTEP==XUNDEF .OR. &
                ABS(NINT(IM%I%TTIME%TIME/IM%DGI%XDIAG_TSTEP)*IM%DGI%XDIAG_TSTEP-IM%I%TTIME%TIME)<1.E-3 ) THEN
    CALL WRITE_DIAG_PGD_ISBA_n(DTCO, DGU, IOB, U, IM%CHI, IM%DGMI, IM%I, &
                               HPROGRAM)
  END IF
END IF
