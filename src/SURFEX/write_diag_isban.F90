...
IF (HWRITE/='PGD') THEN
  IF (IM%DGI%XDIAG_TSTEP==XUNDEF .OR. &
          ABS(NINT(IM%I%I%TTIME%TIME/IM%DGI%XDIAG_TSTEP)*IM%DGI%XDIAG_TSTEP-IM%I%I%TTIME%TIME)<1.E-3 ) THEN
    CALL WRITE_DIAG_SEB_ISBA_n(DTCO, DGU, U, IM%CHI, IM%DGEI, IM%DGEIC, IM%DGEIP, IM%DGEIPC, &
                               IM%DGI, IM%DGIC, IM%DGIP, IM%DGIPC, DST, IM%GB, IM%I, &
                               HPROGRAM)
    CALL WRITE_DIAG_MISC_ISBA_n(DTCO, DGU, U, IM%DGI, IM%DGIP, IM%DGMI, IM%DGMIP, IM%I, &
                                HPROGRAM)
  END IF
END IF
!
