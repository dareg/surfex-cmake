...
IF (HWRITE/='PGD') THEN
  IF (DGI%XDIAG_TSTEP==XUNDEF .OR. ABS(NINT(I%TTIME%TIME/DGI%XDIAG_TSTEP)*DGI%XDIAG_TSTEP-I%TTIME%TIME)<1.E-3 ) THEN
    CALL WRITE_DIAG_SEB_ISBA_n(BOP, BDD, CHE, CHS, CHN, CHU, CHT, CHW, DTCO, DTS, DTT, &
                                         DTZ, DGF, DGMI, DGMTO, DGO, DGS, DGSI, DGT, DGUT, DGW, F, &
                                         FSB, IOB, ICP, O, S, SSB, UG, U, SV, TCP, TGD, &
                                         TGDO, TGR, TGRO, T, TOP, TVG, W, WSB, &
                               CHI, DGEI, DGI, DGU, DST, GB, I, &
                               HPROGRAM)
    CALL WRITE_DIAG_MISC_ISBA_n(BOP, BDD, CHE, CHI, CHS, CHN, CHU, CHT, CHW, DTCO, DTS, &
                                          DTT, DTZ, DGEI, DGF, DGMTO, DGO, DGS, DGSI, DGU, DGT, DGUT, &
...
IF (DGI%LPGD) THEN
  IF (DGI%XDIAG_TSTEP==XUNDEF .OR. ABS(NINT(I%TTIME%TIME/DGI%XDIAG_TSTEP)*DGI%XDIAG_TSTEP-I%TTIME%TIME)<1.E-3 ) THEN
    CALL WRITE_DIAG_PGD_ISBA_n(BOP, BDD, CHE, CHS, CHN, CHU, CHT, CHW, DTCO, DTS, DTT, &
