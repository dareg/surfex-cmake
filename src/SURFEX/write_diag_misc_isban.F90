!     #########
      SUBROUTINE WRITE_DIAG_MISC_ISBA_n (DTCO, HSELECT, U, OPATCH_BUDGET, PTS, DGIP, &
                                         DGMI, DGMIP, I, HPROGRAM)
!     #################################
!
...
IF (DGMI%LSURF_MISC_BUDGET) THEN
  ...
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XHV(:),IRESP,HCOMMENT=YCOMMENT)
  !
  !        2.2    Snow fractions
  !               --------------
  !
  ...
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XPSNG(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='PSNV_ISBA'
  YCOMMENT='snow fraction over vegetation averaged over tile nature (-)'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XPSNV(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='PSN_ISBA'
  YCOMMENT='total snow fraction averaged over tile nature (-)'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XPSN(:),IRESP,HCOMMENT=YCOMMENT)
  !
  ...
  IF (I%R%TSNOW%SCHEME=='3-L' .OR. I%R%TSNOW%SCHEME=='CRO') THEN
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,PTS(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='TSRAD_ISBA'
    YCOMMENT='total radiative surface temperature (isba+snow) over tile nature'
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,I%R%XTSRAD_NAT(:),IRESP,HCOMMENT=YCOMMENT)
    !
  END IF
  !
  ...
  DO JLAYER=1,I%O%NGROUND_LAYER
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XSWI(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
    !
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XTSWI(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
    !
  END DO
  !
  YRECFM='SWI_T_ISBA'
  YCOMMENT='soil wetness index over the soil column (-)'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XSOIL_SWI(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='TSWI_T_ISBA'
  YCOMMENT='total soil wetness index over the soil column (-)'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XSOIL_TSWI(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='WGTOT_T_ISBA'
  YCOMMENT='total water content (liquid+solid) over the soil column (kg/m2)'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XSOIL_TWG(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='WGI_T_ISBA'
  YCOMMENT='total ice content (solid) over the soil column (kg/m2)'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XSOIL_TWGI(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='WGTOT_ISBA'
  YCOMMENT='total volumetric water content (liquid+solid) over the soil column (m3/m3)'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XSOIL_WG(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='WGI_ISBA'
  YCOMMENT='total volumetric ice content (solid) over the soil column (m3/m3)'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XSOIL_WGI(:),IRESP,HCOMMENT=YCOMMENT)
  !
  IF(I%O%CISBA=='DIF') THEN
    !
    IF (DGMI%LSURF_MISC_DIF)THEN
      ...
      CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFRD2_TSWI(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='WG_D2_ISBA'
      YCOMMENT='liquid water content over comparable FR-DG2 reservoir (m3/m3)'
      CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFRD2_TWG(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='WGI_D2_ISBA'
      YCOMMENT='ice content over comparable FR-DG2 reservoir (m3/m3)'
      CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFRD2_TWGI(:),IRESP,HCOMMENT=YCOMMENT)  
      !
      YRECFM='TSWI_D3_ISBA'
      YCOMMENT='total soil wetness index over comparable FR-DG3 reservoir (-)'
      CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFRD3_TSWI(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='WG_D3_ISBA'
      YCOMMENT='liquid water content over comparable FR-DG3 reservoir (m3/m3)'
      CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFRD3_TWG(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='WGI_D3_ISBA'
      YCOMMENT='ice content over comparable FR-DG3 reservoir (m3/m3)'
      CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFRD3_TWGI(:),IRESP,HCOMMENT=YCOMMENT)  
      !
    ENDIF
    !
    YRECFM='ALT_ISBA'
    YCOMMENT='active layer thickness over permafrost (m)'
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XALT(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FLT_ISBA'
    YCOMMENT='frozen layer thickness over non-permafrost (m)'
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFLT(:),IRESP,HCOMMENT=YCOMMENT)
    !
  ENDIF
  !
  !        2.5    Snow outputs
  ...
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XTWSNOW(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='DSN_T_ISBA'
  YCOMMENT='Total_snow_depth (m)'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XTDSNOW(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='TSN_T_ISBA'
  YCOMMENT='Total_snow_temperature (K)'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XTTSNOW(:),IRESP,HCOMMENT=YCOMMENT)
  !
  !        2.6    SGH scheme
  !               ----------
  !
  IF(I%O%CRUNOFF=='SGH '.OR.I%O%CRUNOFF=='DT92')THEN     
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFSAT(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
  !
  IF(I%O%CRAIN=='SGH ')THEN
    YRECFM='MUF_ISBA'
    YCOMMENT='fraction of the grid cell reached by the rainfall (-)'
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,I%I%XMUF(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
  !
  !        2.7    Flooding scheme
  !               ---------------
  ...
  IF(I%O%LFLOOD)THEN
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFFG(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FFV_ISBA'
    YCOMMENT='flood fraction over vegetation averaged over tile nature (-)'
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFFV(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FF_ISBA'
    YCOMMENT='total flood fraction averaged over tile nature (-)'
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XFF(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FFLOOD_ISBA'
    YCOMMENT='Grdi-cell potential flood fraction (-)'
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,I%I%XFFLOOD(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='PIFLOOD_ISBA'
    YCOMMENT='Grdi-cell Potential_floodplain_infiltration (kg/m2)'
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,I%I%XPIFLOOD(:),IRESP,HCOMMENT=YCOMMENT)
    !
  ENDIF
  !
  !        2.8    Total LAI
  ...
  IF(I%O%CPHOTO/='NON'.OR.I%O%NPATCH>1)THEN        
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DGMI%XLAI(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
  !
  ...
  IF(I%O%LWTD)THEN
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,I%IP%XFWTD(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='WTD_ISBA'
    YCOMMENT='water table depth from RRM model or observation (m)'
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,I%IP%XWTD(:),IRESP,HCOMMENT=YCOMMENT)
    !
  ENDIF
  ...
  IF (LASSIM .AND. CASSIM_ISBA=="EKF  ") THEN
    ...
    DO JVAR = 1,NVAR
      ...
      YCOMMENT="by patch"
      CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,&
              I%R%XINCR(:,I%O%NPATCH*(JVAR-1)+1:I%O%NPATCH*JVAR),IRESP,HCOMMENT=YCOMMENT)
    ENDDO
    !
  ENDIF
  !
