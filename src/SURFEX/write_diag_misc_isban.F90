!     #########
      SUBROUTINE WRITE_DIAG_MISC_ISBA_n (DGI, DGMI, I, &
                                         HPROGRAM)
!     #################################
!
...
IF (DGMI%LSURF_MISC_BUDGET) THEN
  ...
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_HV(:),IRESP,HCOMMENT=YCOMMENT)
  !
  !        2.2    Snow fractions
  !               --------------
  !
  ...
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_PSNG(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='PSNV_ISBA'
  YCOMMENT='snow fraction over vegetation averaged over tile nature (-)'
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_PSNV(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='PSN_ISBA'
  YCOMMENT='total snow fraction averaged over tile nature (-)'
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_PSN(:),IRESP,HCOMMENT=YCOMMENT)
  !
  ...
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_ALBT(:),IRESP,HCOMMENT=YCOMMENT)
  !
  IF (I%TSNOW%SCHEME=='3-L' .OR. I%TSNOW%SCHEME=='CRO') THEN
    !        
    YRECFM='TS_ISBA'
    YCOMMENT='total surface temperature (isba+snow) over tile nature'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGI%XAVG_TS(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='TSRAD_ISBA'
    YCOMMENT='total radiative surface temperature (isba+snow) over tile nature'
    CALL WRITE_SURF(HPROGRAM,YRECFM,I%XTSRAD_NAT(:),IRESP,HCOMMENT=YCOMMENT)
    !
  END IF
  !
  ...
  DO JLAYER=1,I%NGROUND_LAYER
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_SWI(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
    !
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_TSWI(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
    !
  END DO
  !
  YRECFM='SWI_T_ISBA'
  YCOMMENT='soil wetness index over the soil column (-)'
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XSOIL_SWI(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='TSWI_T_ISBA'
  YCOMMENT='total soil wetness index over the soil column (-)'
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XSOIL_TSWI(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='WGTOT_T_ISBA'
  YCOMMENT='total water content (liquid+solid) over the soil column (kg/m2)'
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XSOIL_TWG(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='WGI_T_ISBA'
  YCOMMENT='total ice content (solid) over the soil column (kg/m2)'
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XSOIL_TWGI(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='WGTOT_ISBA'
  YCOMMENT='total volumetric water content (liquid+solid) over the soil column (m3/m3)'
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XSOIL_WG(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='WGI_ISBA'
  YCOMMENT='total volumetric ice content (solid) over the soil column (m3/m3)'
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XSOIL_WGI(:),IRESP,HCOMMENT=YCOMMENT)
  !
  IF(I%CISBA=='DIF') THEN
    !
    IF (DGMI%LSURF_MISC_DIF)THEN
      ...
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFRD2_TSWI(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='WG_D2_ISBA'
      YCOMMENT='liquid water content over comparable FR-DG2 reservoir (m3/m3)'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFRD2_TWG(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='WGI_D2_ISBA'
      YCOMMENT='ice content over comparable FR-DG2 reservoir (m3/m3)'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFRD2_TWGI(:),IRESP,HCOMMENT=YCOMMENT)  
      !
      YRECFM='TSWI_D3_ISBA'
      YCOMMENT='total soil wetness index over comparable FR-DG3 reservoir (-)'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFRD3_TSWI(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='WG_D3_ISBA'
      YCOMMENT='liquid water content over comparable FR-DG3 reservoir (m3/m3)'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFRD3_TWG(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='WGI_D3_ISBA'
      YCOMMENT='ice content over comparable FR-DG3 reservoir (m3/m3)'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFRD3_TWGI(:),IRESP,HCOMMENT=YCOMMENT)  
      !
    ENDIF
    !
    YRECFM='ALT_ISBA'
    YCOMMENT='active layer thickness over permafrost (m)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_ALT(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FLT_ISBA'
    YCOMMENT='frozen layer thickness over non-permafrost (m)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_FLT(:),IRESP,HCOMMENT=YCOMMENT)
    !
  ENDIF
  !
  !        2.5    Snow outputs
  ...
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_TWSNOW(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='DSN_T_ISBA'
  YCOMMENT='Total_snow_depth (m)'
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_TDSNOW(:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='TSN_T_ISBA'
  YCOMMENT='Total_snow_temperature (K)'
  CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_TTSNOW(:),IRESP,HCOMMENT=YCOMMENT)
  !
  !        2.6    SGH scheme
  !               ----------
  !
  IF(I%CRUNOFF=='SGH '.OR.I%CRUNOFF=='DT92')THEN     
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_FSAT(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
  !
  IF(I%CRAIN=='SGH ')THEN
    YRECFM='MUF_ISBA'
    YCOMMENT='fraction of the grid cell reached by the rainfall (-)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,I%XMUF(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
  !
  !        2.7    Flooding scheme
  !               ---------------
  ...
  IF(I%LFLOOD)THEN
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_FFG(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FFV_ISBA'
    YCOMMENT='flood fraction over vegetation averaged over tile nature (-)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_FFV(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FF_ISBA'
    YCOMMENT='total flood fraction averaged over tile nature (-)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_FF(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FFLOOD_ISBA'
    YCOMMENT='Grdi-cell potential flood fraction (-)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,I%XFFLOOD(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='PIFLOOD_ISBA'
    YCOMMENT='Grdi-cell Potential_floodplain_infiltration (kg/m2)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,I%XPIFLOOD(:),IRESP,HCOMMENT=YCOMMENT)
    !
  ENDIF
  !
  !        2.8    Total LAI
  ...
  IF(I%CPHOTO/='NON'.OR.I%NPATCH>1)THEN        
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XAVG_LAI(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
  !
  ...
  IF(I%LWTD)THEN
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,I%XFWTD(:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='WTD_ISBA'
    YCOMMENT='water table depth from RRM model or observation (m)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,I%XWTD(:),IRESP,HCOMMENT=YCOMMENT)
    !
  ENDIF
  ...
  IF(DGI%LPATCH_BUDGET)THEN
    ...
    DO JLAYER=1,I%NGROUND_LAYER
      ...
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XSWI(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='TSWI'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
      YFORM='(A39,I1.1,A4)'
      IF (JLAYER >= 10)  YFORM='(A39,I2.2,A4)'
      WRITE(YCOMMENT,FMT=YFORM) 'total swi (liquid+solid) per patch for layer ',JLAYER,' (-)'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XTSWI(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
      !
    END DO
    !
    IF(I%CISBA=='DIF')THEN
      ...
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XALT(:,:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='FLT_P'
      YCOMMENT='frozen layer thickness over non-permafrost per patch (m)'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFLT(:,:),IRESP,HCOMMENT=YCOMMENT) 
      !
    ENDIF
    !    
    !        3.2    Snow fractions
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XDPSNG(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='PSNV_P'
    YCOMMENT='snow fraction per patch over vegetation'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XDPSNV(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='PSN_P'
    YCOMMENT='total snow fraction per patch'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XDPSN(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    !        3.3    SGH scheme
    !               ----------
    !
    IF(I%CRUNOFF=='DT92')THEN     
      ...
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XDFSAT(:,:),IRESP,HCOMMENT=YCOMMENT)
    ENDIF
    !
    !        3.3    Flood fractions
    !               --------------
    ...
    IF(I%LFLOOD)THEN
      ...
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XDFFG(:,:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='FFV_P'
      YCOMMENT='flood fraction per patch over vegetation'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XDFFV(:,:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='FF_P'
      YCOMMENT='total flood fraction per patch'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XDFF(:,:),IRESP,HCOMMENT=YCOMMENT)
      !
    ENDIF
    !
    !        3.4    Total Albedo
    ...
    !
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XALBT(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    IF (I%TSNOW%SCHEME=='3-L' .OR. I%TSNOW%SCHEME=='CRO') THEN
      YRECFM='TS_P'
      YCOMMENT='total surface temperature (isba+snow) per patch'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGI%XTS(:,:),IRESP,HCOMMENT=YCOMMENT)
      YRECFM='TSRAD_P'
      YCOMMENT='total radiative surface temperature (isba+snow) per patch'
      CALL WRITE_SURF(HPROGRAM,YRECFM,DGI%XTSRAD(:,:),IRESP,HCOMMENT=YCOMMENT)
    ENDIF
    !
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XHV(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    !        3.6  Snow outputs 
    !        -----------------
    !
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XTWSNOW(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='DSN_T_P'
    YCOMMENT='X_Y_DSNOW_TOT (m) per patch'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XTDSNOW(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='TSN_T_P'
    YCOMMENT='X_Y_TSNOW_TOT (k) per patch'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XTTSNOW(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    IF (I%TSNOW%SCHEME=='3-L' .OR. I%TSNOW%SCHEME=='CRO') THEN
      !
      DO JLAYER=1,I%TSNOW%NLAYER
        ...
        CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XSNOWLIQ(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
        !
        ...
        CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XSNOWTEMP(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
        !
      END DO
      !        
    ENDIF
  ...
  IF (LAGRIP) THEN
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XSEUIL(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
  ENDIF
  !
  IF (I%LTR_ML) THEN
    ...
    YRECFM='FAPAR'
    YCOMMENT='FAPAR (-)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFAPAR(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FAPIR'
    YCOMMENT='FAPIR (-)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFAPIR(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FAPAR_BS'
    YCOMMENT='FAPAR_BS (-)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFAPAR_BS(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='FAPIR_BS'
    YCOMMENT='FAPIR_BS (-)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XFAPIR_BS(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='DFAPARC'
    YCOMMENT='DFAPARC (-)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XDFAPARC(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='DFAPIRC'
    YCOMMENT='DFAPIRC (-)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XDFAPIRC(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
    YRECFM='DLAI_EFFC'
    YCOMMENT='DLAI_EFFC (m2/m2)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,DGMI%XDLAI_EFFC(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
  ENDIF
  !  
ENDIF
