!     #########
      SUBROUTINE WRITE_DIAG_MISC_SEAICE_n (DTCO, DGU, U, DGMSI, S, &
                                          HPROGRAM)
!     #################################
!
...
IF (TRIM(S%CSEAICE_SCHEME) == 'GELATO') THEN 
    ...
    CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,'SIT',DGMSI%XSIT(:),IRESP,YCOMMENT)
    !
    YCOMMENT='Sea-ice snow depth (m)'
    CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,'SND',DGMSI%XSND(:),IRESP,YCOMMENT)
    !
    YCOMMENT='Sea mixed layer temp for Glt (K)'
    CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,'SIMLT',DGMSI%XMLT(:),IRESP,YCOMMENT)
    !
ENDIF
!               -------------
!         End of IO
