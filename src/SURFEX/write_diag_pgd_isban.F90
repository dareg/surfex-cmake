!     #########
      SUBROUTINE WRITE_DIAG_PGD_ISBA_n (DTCO, HSELECT, U, CHI, OSURF_DIAG_ALBEDO, &
                          IO, IP, INI, IM, ISS, HPROGRAM)
!     #########################################
!
...
!              -------------------------------
!
REAL, DIMENSION(SIZE(IM%X%XDG,1),SIZE(IM%X%XDG,3)) :: ZWORK ! Work array
REAL, DIMENSION(SIZE(IM%X%XDG,1),SIZE(IM%X%XDG,2)) :: ZDG   ! Work array
REAL, DIMENSION(SIZE(IM%X%XDG,1)            ) :: ZDG2
REAL, DIMENSION(SIZE(IM%X%XDG,1)            ) :: ZDTOT
!
...
IF (IO%CPHOTO=='NON' .OR. IO%CPHOTO=='AGS' .OR. IO%CPHOTO=='AST') THEN
  ...
  !
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XLAI(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
  IF (ISIZE_LMEB_PATCH>0) THEN
    ...
    !
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%M%XLAIGV(:,:),IRESP,HCOMMENT=YCOMMENT)
    !
  ENDIF
  !
ENDIF
...
!
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XVEG(:,:),IRESP,HCOMMENT=YCOMMENT)
!
...
!
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XZ0(:,:),IRESP,HCOMMENT=YCOMMENT)
!
IF (ISIZE_LMEB_PATCH>0) THEN
  !
  YRECFM='GNDLITTER'
  ...
  !
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%M%XGNDLITTER(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='Z0LITTER'
  YCOMMENT='MEB: ground litter roughness length (without snow) (m)'
  !
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%M%XZ0LITTER(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
ENDIF
!
!-------------------------------------------------------------------------------
...
DO JL=1,SIZE(IM%X%XDG,2)
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%X%XDG(:,JL,:),IRESP,HCOMMENT=YCOMMENT)
END DO
!
!* Averaged Soil depth
!
...
!        
  ...
  DO JP=1,IO%NPATCH
     DO JL=1,SIZE(IM%X%XDG,2)
        DO JJ=1,SIZE(IM%X%XDG,1) 
           ZDG(JJ,JL)=ZDG(JJ,JL)+IP%XPATCH(JJ,JP)*IM%X%XDG(JJ,JL,JP)
        ENDDO
     ENDDO
  ENDDO
!
...
!
  ...
  ZDTOT(:)=0.0
  ZWORK(:,:)=XUNDEF
  DO JP=1,SIZE(IM%X%XDG,3)
     DO JJ=1,SIZE(IM%X%XDG,1)
        ZDG2(JJ)=ZDG2(JJ)+IP%XPATCH(JJ,JP)*IM%X%XDG2(JJ,JP)
        JL=IM%X%NWG_LAYER(JJ,JP)
        IF(JL/=NUNDEF)THEN
          ZWORK(JJ,JP)=IM%X%XDG(JJ,JL,JP)
          ZDTOT(JJ)=ZDTOT(JJ)+IP%XPATCH(JJ,JP)*IM%X%XDG(JJ,JL,JP)
        ENDIF
     ENDDO
  ENDDO
!
...
!
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%X%XDROOT(:,:),IRESP,HCOMMENT=YCOMMENT)
!
  YRECFM='DG2_DIF'
  YCOMMENT='DG2 depth in ISBA-DIF'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%X%XDG2(:,:),IRESP,HCOMMENT=YCOMMENT)
!  
  IF(IO%NPATCH>1)THEN
    YRECFM='DG2_DIF_ISBA'
...
!
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IP%XRUNOFFD(:,:),IRESP,HCOMMENT=YCOMMENT)
!
...
!
  DO JL=1,SIZE(IM%X%XROOTFRAC,2)
     ...
     ZWORK(:,:)=XUNDEF
     DO JJ=1,SIZE(IM%X%XDG,1)
        WHERE(JL<=IM%X%NWG_LAYER(JJ,:).AND.IM%X%NWG_LAYER(JJ,:)/=NUNDEF)
              ZWORK(JJ,:)=IM%X%XROOTFRAC(JJ,JL,:)
        ENDWHERE
     ENDDO
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,ZWORK(:,:),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  IF (ISIZE_LMEB_PATCH>0) THEN
    DO JL=1,SIZE(IM%M%XROOTFRACGV,2)
       IF (JL<10) THEN
       ...
       DO JJ=1,SIZE(IM%X%XDG,1)
          WHERE(JL<=IM%X%NWG_LAYER(JJ,:).AND.IM%X%NWG_LAYER(JJ,:)/=NUNDEF)
                ZWORK(JJ,:)=IM%M%XROOTFRACGV(JJ,JL,:)
          ENDWHERE
       ENDDO
       CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,ZWORK(:,:),IRESP,HCOMMENT=YCOMMENT)
    END DO
...
!
  IF(IO%LSOC)THEN
    DO JL=1,SIZE(IM%X%XDG,2)
     ...
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,INI%XFRACSOC(:,JL),IRESP,HCOMMENT=YCOMMENT)
    END DO
  ENDIF
!
ENDIF        
...
DO JL=1,SIZE(IM%X%XDG,2)
  ...
  CALL WRITE_SURF(HSELECT, &
                  HPROGRAM,YRECFM,IP%XWSAT(:,JL),IRESP,HCOMMENT=YCOMMENT)
ENDDO
!
DO JL=1,SIZE(IM%X%XDG,2)
   IF (JL<10) THEN
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IP%XWFC(:,JL),IRESP,HCOMMENT=YCOMMENT)
ENDDO
!
DO JL=1,SIZE(IM%X%XDG,2)
   IF (JL<10) THEN
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IP%XWWILT(:,JL),IRESP,HCOMMENT=YCOMMENT)
ENDDO     
!
!-------------------------------------------------------------------------------
...
IF(IO%CHORT=='SGH'.AND.IO%CISBA/='DIF')THEN
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%X%XD_ICE(:,:),IRESP,HCOMMENT=YCOMMENT)
ENDIF
!
!-------------------------------------------------------------------------------
!
...
!
DO JL=1,SIZE(IP%XVEGTYPE_PATCH,2)
  WRITE(YPAS,'(I2)') JL 
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%X%XVEGTYPE(:,JL),IRESP,HCOMMENT=YCOMMENT)
END DO
!-------------------------------------------------------------------------------
!
!* Fraction of each vegetation type for each patch
!
IF(IO%NPATCH>1.AND.SIZE(IP%XVEGTYPE_PATCH,2)/=SIZE(IP%XVEGTYPE_PATCH,3))THEN
!
  DO JL=1,SIZE(IP%XVEGTYPE_PATCH,2)
    WRITE(YPAS,'(I2)') JL 
    YLVLV=ADJUSTL(YPAS(:LEN_TRIM(YPAS)))
    ...
    CALL WRITE_SURF(HSELECT,  HPROGRAM,YRECFM,IP%XVEGTYPE_PATCH(:,JL,:),IRESP,HCOMMENT=YCOMMENT)
  END DO
!
ENDIF
!
...
YCOMMENT='minimum stomatal resistance (sm-1)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XRSMIN(:,:),IRESP,HCOMMENT=YCOMMENT)
!
YRECFM='GAMMA'
YCOMMENT='coefficient for RSMIN calculation (-)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XGAMMA(:,:),IRESP,HCOMMENT=YCOMMENT)
!
YRECFM='CV'
YCOMMENT='vegetation thermal inertia coefficient (-)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XCV(:,:),IRESP,HCOMMENT=YCOMMENT)
!
YRECFM='RGL'
YCOMMENT='maximum solar radiation usable in photosynthesis (-)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XRGL(:,:),IRESP,HCOMMENT=YCOMMENT)
!
YRECFM='EMIS_ISBA'
YCOMMENT='surface emissivity (-)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XEMIS(:,:),IRESP,HCOMMENT=YCOMMENT)
!
YRECFM='WRMAX_CF'
YCOMMENT='coefficient for maximum water interception (-)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XWRMAX_CF(:,:),IRESP,HCOMMENT=YCOMMENT)
!
IF (ISIZE_LMEB_PATCH>0) THEN
  !
  YRECFM='RSMINGV'
  YCOMMENT='MEB: understory minimum stomatal resistance (sm-1)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%M%XRSMINGV(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='GAMMAGV'
  YCOMMENT='MEB: understory coefficient for RSMIN calculation (-)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%M%XGAMMAGV(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='RGLGV'
  YCOMMENT='MEB: understory maximum solar radiation usable in photosynthesis (-)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%M%XRGLGV(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='WRMAX_CFGV'
  YCOMMENT='MEB: understory coefficient for maximum water interception (-)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%M%XWRMAX_CFGV(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='H_VEG'
  YCOMMENT='MEB: height of vegetation (m)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%M%XH_VEG(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
ENDIF
!
!-------------------------------------------------------------------------------
...
!
   ...
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%A%XALBNIR_SOIL(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!-------------------------------------------------------------------------------
!
   YRECFM='ALBVIS_S'
   YCOMMENT='soil visible albedo (-)'
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%A%XALBVIS_SOIL(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!-------------------------------------------------------------------------------
!
   YRECFM='ALBUV_S'
   YCOMMENT='soil UV albedo (-)'
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%A%XALBUV_SOIL(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!-------------------------------------------------------------------------------
!
!* albedos
!
   ...
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XALBNIR(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!-------------------------------------------------------------------------------
!
   YRECFM='ALBVIS_ISBA'
   YCOMMENT='total visible albedo (-)'
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XALBVIS(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!-------------------------------------------------------------------------------
!
   YRECFM='ALBUV_ISBA'
   YCOMMENT='total UV albedo (-)'
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%T%XALBUV(:,:),IRESP,HCOMMENT=YCOMMENT)
!
END IF
!
!-------------------------------------------------------------------------------
...
IF (CHI%CCH_DRY_DEP=='WES89' .AND. CHI%SVI%NBEQ>0) THEN
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,CHI%XSOILRC_SO2(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
  YRECFM='SOILRC_O3'
  YCOMMENT='bare soil resistance for O3 (?)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,CHI%XSOILRC_O3(:,:),IRESP,HCOMMENT=YCOMMENT)
END IF
!
!-------------------------------------------------------------------------------
!
...
!
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%I%TSEED(:,:),IRESP,HCOMMENT=YCOMMENT)
!
  YRECFM='TREAP'
  YCOMMENT='date of reaping (-)'
!
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%I%TREAP(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!-------------------------------------------------------------------------------
!
!* irrigated fraction
...
!
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%I%XIRRIG(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!-------------------------------------------------------------------------------
!
...
!
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IM%I%XWATSUP(:,:),IRESP,HCOMMENT=YCOMMENT)
!
ENDIF
!-------------------------------------------------------------------------------
!         End of IO
