!     #########
SUBROUTINE WRITE_DIAG_SEAFLUX_n (DTCO, DGU, IOB, U, SM, &   
                                 HPROGRAM,HWRITE)
!     ###############################################################################
!
...
!        
   IF (SM%DGS%XDIAG_TSTEP==XUNDEF .OR. &
      ...
      CALL WRITE_DIAG_SEB_SEAFLUX_n(DTCO, DGU, IOB, U, SM%CHS, SM%DGS, SM%DGSI, SM%S, &
                                    HPROGRAM)
      IF (SM%DGO%LDIAG_OCEAN)  CALL WRITE_DIAG_SEB_OCEAN_n(DTCO, DGU, IOB, U, SM%DGO, &
                                                        HPROGRAM)
      IF (SM%DGSI%LDIAG_SEAICE) CALL WRITE_DIAG_SEB_SEAICE_n(DTCO, DGU, IOB, U, SM%DGS, SM%DGSI, SM%S, &
                                                          HPROGRAM)
   END IF
!        
ENDIF
