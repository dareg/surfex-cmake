...
IF(ID%O%LPATCH_BUDGET.AND.(IO%NPATCH >1))THEN
    ...
    IF (ID%O%N2M>=1) THEN
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XRI(:),ISIZE)
      ENDDO
      !
    END IF
    !
    ...
    IF (ID%O%LSURF_BUDGET) THEN
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XALBT(:),ISIZE)
      ENDDO      
      !
      YRECFM='RN_'
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XRN(:),ISIZE)
      ENDDO      
      !
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XH(:),ISIZE)
      ENDDO      
      !
      YRECFM='LE_'
      YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XLEI(:),ISIZE)
      ENDDO  
      !
      YRECFM='GFLUX_'
      YCOMMENT='X_Y_'//YRECFM//' (W/m2)'      
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XGFLUX(:),ISIZE)
      ENDDO   
      !
      YRECFM='EVAP_'
      YCOMMENT='X_Y_'//YRECFM//' (W/m2)'      
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XEVAP(:),ISIZE)
      ENDDO    
      !
      YRECFM='SUBL_'
      YCOMMENT='X_Y_'//YRECFM//' (W/m2)'      
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XSUBL(:),ISIZE)
      ENDDO   
      !     
      IF (ID%O%LRAD_BUDGET .OR. (ID%O%LSURF_BUDGETC .AND. .NOT.DUO%LRESET_BUDGETC)) THEN
        !
        ...
        DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XSWD(:),ISIZE)
        ENDDO       
        !
        ...
        DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XSWU(:),ISIZE)
        ENDDO
        !
        YRECFM='LWD_'
        YCOMMENT='X_Y_'//YRECFM//' (W/m2)'        
        DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XLWD(:),ISIZE)
        ENDDO  
        !
        YRECFM='LWU_'
        YCOMMENT='X_Y_'//YRECFM//' (W/m2)'        
        DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XLWU(:),ISIZE)
        ENDDO
        !
        DO JSW=1, SIZE(ID%D%XSWBD,2)
          YNUM=ACHAR(48+JSW)
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XFMU(:),ISIZE)
      ENDDO
      !
      YRECFM='FMV_'
      YCOMMENT='X_Y_'//YRECFM//' (Pa)'      
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XFMV(:),ISIZE)
      ENDDO
      !
    END IF
    !
    ...
    IF (ID%O%N2M>=1.AND..NOT.IO%LCANOPY) THEN
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XT2M(:),ISIZE)
      ENDDO
      !
      YRECFM='T2MMIN_'
      YCOMMENT='X_Y_'//YRECFM//' (K)' 
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XT2M_MIN(:),ISIZE)
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XT2M_MAX(:),ISIZE)
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XQ2M(:),ISIZE)
      ENDDO
      !
      YRECFM='HU2M_'
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XHU2M(:),ISIZE)
      ENDDO
      !
      YRECFM='ZON10M_'
      YCOMMENT='X_Y_'//YRECFM//' (M/S)'      
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XZON10M(:),ISIZE)
      ENDDO
      !
      YRECFM='MER10M_'
      YCOMMENT='X_Y_'//YRECFM//' (M/S)'
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XMER10M(:),ISIZE)
      ENDDO
      !    
      YRECFM='W10M_'
      YCOMMENT='X_Y_'//YRECFM//' (M/S)'
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%ND%AL(JP)%XWIND10M(:),ISIZE)
      ENDDO
      !
    END IF
    !
    ...
    IF (ID%O%LSURF_BUDGETC) THEN
      ...
      IF(IO%LGLACIER)THEN              
        ...
        DO JP=1,IO%NPATCH
          CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDEC%AL(JP)%XICEFLUX(:),ISIZE)
        ENDDO
      ENDIF
      !      
      YRECFM='RNC_'
      ...
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XRN(:),ISIZE)
      ENDDO
      !     
      YRECFM='HC_'
      YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XH(:),ISIZE)
      ENDDO
      ! 
      YRECFM='LEC_'
      YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XLE(:),ISIZE)
      ENDDO
      !   
      YRECFM='LEIC_'
      YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XLEI(:),ISIZE)
      ENDDO
      !
      YRECFM='GFLUXC_'
      YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XGFLUX(:),ISIZE)
      ENDDO
      !     
      YRECFM='EVAPC_'
      YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XEVAP(:),ISIZE)
      ENDDO
      !    
      YRECFM='SUBLC_'
      YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
      DO JP=1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XSUBL(:),ISIZE)
      ENDDO      
      !    
      IF (ID%O%LRAD_BUDGET .OR. (ID%O%LSURF_BUDGETC .AND. .NOT.DUO%LRESET_BUDGETC)) THEN
        !
        ...
        DO JP=1,IO%NPATCH
          CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XSWD(:),ISIZE)
        ENDDO
        !      
        YRECFM='SWUC_'
        YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
        DO JP=1,IO%NPATCH
          CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XSWU(:),ISIZE)
        ENDDO
        !     
        YRECFM='LWDC_'
        YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
        DO JP=1,IO%NPATCH
          CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XLWD(:),ISIZE)
        ENDDO
        !     
        YRECFM='LWUC_'
        YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
        DO JP=1,IO%NPATCH
          CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XLWU(:),ISIZE)
        ENDDO
        !
      ENDIF
      ! 
      ...
      DO JP=1,IO%NPATCH
          CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XFMU(:),ISIZE)
      ENDDO
      ! 
      YRECFM='FMVC_'
      YCOMMENT='X_Y_'//YRECFM//' (Pa.s)'
       DO JP=1,IO%NPATCH
          CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,ID%NDC%AL(JP)%XFMV(:),ISIZE)
      ENDDO
      !
    ENDIF
    !
...
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLEG(:),ISIZE)
ENDDO
!
YRECFM='LEGI'//TRIM(HTERM)
YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLEGI(:),ISIZE)
ENDDO
!
YRECFM='LEV'//TRIM(HTERM)
YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLEV(:),ISIZE)
ENDDO
!
...
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLES(:),ISIZE)
ENDDO
!
IF(NPE%AL(1)%TSNOW%SCHEME=='3-L' .OR. NPE%AL(1)%TSNOW%SCHEME=='CRO')THEN 
  YRECFM='LESL'//TRIM(HTERM)
  ...
  DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLESL(:),ISIZE)
ENDDO
  YRECFM='SNDRIF'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (kg/m2/s)'
  DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XSNDRIFT(:),ISIZE)
ENDDO
ENDIF
! 
YRECFM='LER'//TRIM(HTERM)
...
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLER(:),ISIZE)
ENDDO
!
YRECFM='LETR'//TRIM(HTERM)
YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLETR(:),ISIZE)
ENDDO
!
YRECFM='DRAIN'//TRIM(HTERM)
YCOMMENT='X_Y_'//YRECFM//' (Kg/m2/s)'
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XDRAIN(:),ISIZE)
ENDDO
!
IF(IO%CRUNOFF=='SGH'.AND.IO%CISBA=='DIF')THEN
  YRECFM='QSB'//TRIM(HTERM)
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
          NP%AL(JP)%NR_P,NDEA%AL(JP)%XQSB(:),ISIZE)
  ENDDO  
ENDIF
!
YRECFM='RUNOFF'//TRIM(HTERM)
...
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XRUNOFF(:),ISIZE)
ENDDO
!
IF(IO%CHORT=='SGH'.OR.IO%CISBA=='DIF')THEN
  ...
  DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XHORT(:),ISIZE)
  ENDDO
...
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XDRIP(:),ISIZE)
ENDDO
!
YRECFM='RRVEG'//TRIM(HTERM)
YCOMMENT='X_Y_'//YRECFM//' (Kg/m2/s)'
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XRRVEG(:),ISIZE)
ENDDO
!
YRECFM='SNOMLT'//TRIM(HTERM)
YCOMMENT='X_Y_'//YRECFM//' (Kg/m2/s)'
DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XMELT(:),ISIZE)
ENDDO
!
! MEB STUFF
IF (ISIZE_LMEB_PATCH>0) THEN
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLEVCV(:),ISIZE)
  ENDDO
  !
  YRECFM='LESC'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLESC(:),ISIZE)
  ENDDO
  !
  YRECFM='LETRCV'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLETRCV(:),ISIZE)
  ENDDO
  !
  YRECFM='LELIT'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLELITTER(:),ISIZE)
  ENDDO
  !
  YRECFM='LELITI'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLELITTERI(:),ISIZE)
  ENDDO
  !
  YRECFM='DRIPLIT'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XDRIPLIT(:),ISIZE)
  ENDDO
  !
  YRECFM='RRLIT'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XRRLIT(:),ISIZE)
  ENDDO
  !
  YRECFM='LERCV'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLERCV(:),ISIZE)
  ENDDO
  !
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLE_C_A(:),ISIZE)
  ENDDO
  !
  YRECFM='LE_V_C'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLE_V_C(:),ISIZE)
  ENDDO
  !
  YRECFM='LE_G_C'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLE_G_C(:),ISIZE)
  ENDDO
  !
  YRECFM='LE_N_C'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLE_N_C(:),ISIZE)
  ENDDO
  !
  YRECFM='SWNT_V'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XSWNET_V(:),ISIZE)
  ENDDO
  !
  YRECFM='SWNT_G'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XSWNET_G(:),ISIZE)
  ENDDO
  !
  YRECFM='SWNT_N'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XSWNET_N(:),ISIZE)
  ENDDO
  !
  YRECFM='SWNT_NS'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XSWNET_NS(:),ISIZE)
  ENDDO
  !
  YRECFM='LWNT_V'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLWNET_V(:),ISIZE)
  ENDDO
  !
  YRECFM='LWNT_G'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLWNET_G(:),ISIZE)
  ENDDO
  !
  YRECFM='LWNT_N'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLWNET_N(:),ISIZE)
  ENDDO
  !
  YRECFM='SWDN_GN'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XSWDOWN_GN(:),ISIZE)
  ENDDO
  !
  YRECFM='LWDN_GN'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLWDOWN_GN(:),ISIZE)
  ENDDO
  !
  YRECFM='H_V_C'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XH_V_C(:),ISIZE)
  ENDDO
  !
  YRECFM='H_G_C'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XH_G_C(:),ISIZE)
  ENDDO
  !
  YRECFM='H_C_A'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XH_C_A(:),ISIZE)
  ENDDO
  !
  YRECFM='H_N_C'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XH_N_C(:),ISIZE)
  ENDDO
  !
  YRECFM='SR_GN'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (kg/m2/s)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XSR_GN(:),ISIZE)
  ENDDO
  !
  YRECFM='MELTCV'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (kg/m2/s)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XMELTCV(:),ISIZE)
  ENDDO
  !
  YRECFM='FRZCV'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (kg/m2/s)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XFRZCV(:),ISIZE)
  ENDDO
...
IF(LAGRIP)THEN
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XIRRIG_FLUX(:),ISIZE)
  ENDDO
ENDIF
!
IF(IO%LFLOOD)THEN
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XIFLOOD(:),ISIZE)
  ENDDO
  !
  YRECFM='PFLOOD'//TRIM(HTERM)
  YCOMMENT='X_Y_'//YRECFM//' (Kg/m2/s)'
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XPFLOOD(:),ISIZE)
  ENDDO
  !
  ...
  DO JP=1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLE_FLOOD(:),ISIZE)
ENDDO
  !
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XLEI_FLOOD(:),ISIZE)
  ENDDO
  !
ENDIF
!
IF(IO%CPHOTO/='NON')THEN
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XGPP(:),ISIZE)
  ENDDO
  !
  IF (HTERM(1:1)=="C") THEN
    YRECFM='RC_AUTO'//TRIM(HTERM(2:))
  ...
  YCOMMENT='autotrophic respiration per patch (kgCO2/m2/s)'
    DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XRESP_AUTO(:),ISIZE)
  ENDDO
  !
  IF (HTERM(1:1)=="C") THEN
    YRECFM='RC_ECO'//TRIM(HTERM(2:))
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XRESP_ECO(:),ISIZE)
  ENDDO
  !
ENDIF
!
IF(ID%DE%LWATER_BUDGET .OR. OFLAG)THEN 
  ...
  YCOMMENT='change in liquid soil moisture per patch (Kg/m2/s)'
    DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XDWG(:),ISIZE)
  ENDDO
  !
  YRECFM='DWGI'//TRIM(HTERM)
  YCOMMENT='change in solid soil moisture per patch (Kg/m2/s)'
    DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XDWGI(:),ISIZE)
  ENDDO
  !
  YRECFM='DWR'//TRIM(HTERM)
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XDWR(:),ISIZE)
  ENDDO
  !
  YRECFM='DSWE'//TRIM(HTERM)
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XDSWE(:),ISIZE)
  ENDDO
  !
  YRECFM='WATBUD'//TRIM(HTERM)
  ...
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
        NP%AL(JP)%NR_P,NDEA%AL(JP)%XWATBUD(:),ISIZE)
  ENDDO
  !
ENDIF
!
