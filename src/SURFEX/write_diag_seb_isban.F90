...
IF (LSURF_EVAP_BUDGET) THEN
  ...
  IF(TSNOW%SCHEME=='3-L' .OR. TSNOW%SCHEME=='CRO')THEN  
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,XAVG_SNDRIFT(:),IRESP,HCOMMENT=YCOMMENT)    
  ENDIF
  !  
  YRECFM='LER_ISBA'
  ...
  IF(CRUNOFF=='SGH'.AND.CISBA=='DIF')THEN
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,XAVG_QSB(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
  !
  YRECFM='RUNOFF_ISBA'
...
IF (LSURF_BUDGETC) THEN
  ...
  IF(TSNOW%SCHEME=='3-L' .OR. TSNOW%SCHEME=='CRO')THEN  
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,XAVG_SNDRIFTC(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
  !  
  YRECFM='LERC_ISBA'
  YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
  ...
  IF(CRUNOFF=='SGH'.AND.CISBA=='DIF')THEN
    YRECFM='QSBC_ISBA'
    YCOMMENT='X_Y_'//YRECFM//' (Kg/m2)'
    CALL WRITE_SURF(HPROGRAM,YRECFM,XAVG_QSBC(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
  !
  YRECFM='RUNOFFC_ISBA'
  YCOMMENT='X_Y_'//YRECFM//' (Kg/m2)'
...
IF(LPATCH_BUDGET.AND.(NPATCH >1))THEN
    ...
    IF (LSURF_EVAP_BUDGET) THEN
      ...
      IF(TSNOW%SCHEME=='3-L' .OR. TSNOW%SCHEME=='CRO')THEN  
        ...
        YRECFM='SNDRIF_P'
        YCOMMENT='X_Y_'//YRECFM//' (kg/m2/s)'
        CALL WRITE_SURF(HPROGRAM,YRECFM,XSNDRIFT(:,:),IRESP,HCOMMENT=YCOMMENT)
      ENDIF
      !      
      YRECFM='LER_P'
      YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
      ...
      IF(CRUNOFF=='SGH'.AND.CISBA=='DIF')THEN
        YRECFM='QSB_P'
        YCOMMENT='X_Y_'//YRECFM//' (Kg/m2/s)'
        CALL WRITE_SURF(HPROGRAM,YRECFM,XQSB(:,:),IRESP,HCOMMENT=YCOMMENT)
      ENDIF
      !
      YRECFM='RUNOFF_P'
      YCOMMENT='X_Y_'//YRECFM//' (Kg/m2/s)'
...
!        CALL WRITE_SURF(HPROGRAM,YRECFM,XLERGV(:,:),IRESP,HCOMMENT=YCOMMENT)
    ...
    IF (LSURF_BUDGETC) THEN
      ...
      IF(TSNOW%SCHEME=='3-L' .OR. TSNOW%SCHEME=='CRO')THEN  
        ...
        YRECFM='SNDRIFC_P'
        YCOMMENT='X_Y_'//YRECFM//' (Kg/m2)'
        CALL WRITE_SURF(HPROGRAM,YRECFM,XSNDRIFTC(:,:),IRESP,HCOMMENT=YCOMMENT)
      ENDIF      
      !
      YRECFM='LERC_P'
      YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
      ...
      IF(CRUNOFF=='SGH'.AND.CISBA=='DIF')THEN
        YRECFM='QSBC_P'
        YCOMMENT='X_Y_'//YRECFM//' (Kg/m2)'
        CALL WRITE_SURF(HPROGRAM,YRECFM,XQSBC(:,:),IRESP,HCOMMENT=YCOMMENT)
      ENDIF
      !
      YRECFM='RUNOFFC_P'
      YCOMMENT='X_Y_'//YRECFM//' (Kg/m2)'
...
DO JLAYER=1,NGROUND_LAYER
  ...
  YRECFM=YRECFM(:LEN_TRIM(YRECFM))//'_ISBA'
  YCOMMENT='Soil moisture content (kg/m2)' 
...
DO JLAYER=1,IWORK
  ...
  YRECFM=YRECFM(:LEN_TRIM(YRECFM))//'_ISBA'
  YCOMMENT='Soil ice content (kg/m2)' 
...
DO JPATCH=1,NPATCH
   DO JJ=1,INI 
      ZWORK(JJ) = ZWORK(JJ) + XPATCH(JJ,JPATCH) * XWR(JJ,JPATCH)
   ENDDO
ENDDO
!
YRECFM='WR_ISBA'
...
IF(LGLACIER)THEN
  ...
  DO JPATCH=1,NPATCH
    DO JJ=1,INI 
       ZWORK(JJ) = ZWORK(JJ) + XPATCH(JJ,JPATCH) * XICE_STO(JJ,JPATCH)
    ENDDO    
  ENDDO    
  !
  YRECFM='ICE_STO_ISBA'
...
DO JPATCH=1,NPATCH
   DO JJ=1,INI 
      IF(TSNOW%ALB(JJ,JPATCH)/=XUNDEF)THEN
        ZWORK (JJ) = ZWORK (JJ) + XPATCH(JJ,JPATCH) * TSNOW%ALB(JJ,JPATCH)
        ZPATCH(JJ) = ZPATCH(JJ) + XPATCH(JJ,JPATCH)
      ENDIF
   ENDDO
ENDDO
!
...
IF(TSNOW%SCHEME=='3-L' .OR. TSNOW%SCHEME=='CRO')THEN
  ...
  DO JLAYER = 1,TSNOW%NLAYER
    ...
    DO JPATCH=1,NPATCH
       DO JJ=1,INI 
          ZWORK(JJ) = ZWORK(JJ) + XPATCH(JJ,JPATCH) * TSNOW%WSNOW(JJ,JLAYER,JPATCH)
       ENDDO
    ENDDO
    !
    WRITE(YLVL,'(I4)') JLAYER
  ...
  DO JLAYER = 1,TSNOW%NLAYER
    ...
    DO JPATCH=1,NPATCH
       DO JJ=1,INI 
         ZWORK(JJ) = ZWORK(JJ) + XPATCH(JJ,JPATCH) * TSNOW%WSNOW(JJ,JLAYER,JPATCH)/TSNOW%RHO(JJ,JLAYER,JPATCH)
       ENDDO
    ENDDO
    !
    WRITE(YLVL,'(I4)') JLAYER
  ...
  DO JLAYER = 1,TSNOW%NLAYER
    ...
    DO JPATCH=1,NPATCH
       DO JJ=1,INI 
          IF(TSNOW%WSNOW(JJ,JLAYER,JPATCH)>0.)THEN
             ZWORK (JJ) = ZWORK (JJ) + XPATCH(JJ,JPATCH) * TSNOW%TEMP(JJ,JLAYER,JPATCH) 
             ZPATCH(JJ) = ZPATCH(JJ) + XPATCH(JJ,JPATCH)
          ENDIF
       ENDDO
    ENDDO
  ...
  DO JLAYER = 1,TSNOW%NLAYER
    ...
    DO JPATCH=1,NPATCH
       DO JJ=1,INI 
          IF(TSNOW%WSNOW(JJ,JLAYER,JPATCH)>0.)THEN    
             ZWORK (JJ) = ZWORK (JJ) + XPATCH(JJ,JPATCH) * TSNOW%AGE(JJ,JLAYER,JPATCH) 
             ZPATCH(JJ) = ZPATCH(JJ) + XPATCH(JJ,JPATCH)
          ENDIF
       ENDDO
    ENDDO
