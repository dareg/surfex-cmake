...
GRESET=.TRUE.
#ifdef SFX_ARO
GRESET=(NBLOCK>0 .AND. DUO%LRESETMINMAX)
#endif
#ifdef SFX_OL
...
!
REAL, DIMENSION(U%NSIZE_NATURE,SIZE(NPE%AL(1)%XWG,2)) :: ZWG_M    ! ZWG in kg/m2
REAL, DIMENSION(U%NSIZE_NATURE,SIZE(NPE%AL(1)%XWG,2)) :: ZWGI_M   ! ZWGI in kg/m2
!
REAL, DIMENSION(U%NSIZE_NATURE,SIZE(NPE%AL(1)%XWG,2)) :: ZWGTOT_M
!
...
ENDIF
!
! * Compute soil liquid and ice water content (kg/m2 and m3/m3) and total water content
!
ZWG (:,:)=0.0
ZWGI(:,:)=0.0
ZWG_M (:,:)=0.0
...
ENDIF
!
! soil liquid and ice water content (kg/m2)
ZWG_M (:,:) = ZWG (:,:)*XRHOLW
ZWGI_M(:,:) = ZWGI(:,:)*XRHOLW
! total soil moisture (liquid+frozen) in kg/m2
ZWGTOT_M(:,:) = ZWG_M(:,:) + ZWGI_M(:,:)
!
...
IF (LALLOW_ADD_DIM)  THEN 
  ...
  YRECFM='WG_T_ISBA' 
  YCOMMENT='Soil liquid water content (kg/m2)'
  CALL WRITE_SURF(DUO%CSELECT, &
                  HPROGRAM,YRECFM,ZWG_M(:,:),IRESP,YCOMMENT,HNAM_DIM=YWGROUND_LAYER_DIM_NAME)
ELSE
  DO JL=1,IO%NGROUND_LAYER
    WRITE(YLVL,'(I4)') JL
    ...
    YRECFM='WG'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    YRECFM=YRECFM(:LEN_TRIM(YRECFM))//'_T_ISBA'
    YCOMMENT='Soil liquid water content (kg/m2)'
    CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,ZWG_M(:,JL),IRESP,HCOMMENT=YCOMMENT)
  END DO
ENDIF
!
...
IF (LALLOW_ADD_DIM)  THEN 
  ...
  YRECFM='WGI_T_ISBA'
  YCOMMENT='Soil solid water content (kg/m2)'
  CALL WRITE_SURF(DUO%CSELECT, &
                  HPROGRAM,YRECFM,ZWGI_M(:,:),IRESP,YCOMMENT,HNAM_DIM=YWIGROUND_LAYER_DIM_NAME)
ELSE
  DO JL=1,IWORK
    WRITE(YLVL,'(I4)') JL
    ...
    YRECFM=YRECFM(:LEN_TRIM(YRECFM))//'_T_ISBA'
    YCOMMENT='Soil solid water content (kg/m2)'
    CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,ZWGI_M(:,JL),IRESP,HCOMMENT=YCOMMENT)
  END DO
ENDIF
!
...
IF (.NOT. LALLOW_ADD_DIM)  THEN
  DO JL=1,IO%NGROUND_LAYER
    WRITE(YLVL,'(I4)') JL
    YRECFM='WGT'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    YRECFM=YRECFM(:LEN_TRIM(YRECFM))//'_T_ISBA'
    YCOMMENT='Total water content (liquid+solid) (kg/m2)'
    CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,ZWGTOT_M(:,JL),IRESP,HCOMMENT=YCOMMENT)
  END DO
ENDIF
!
