!     #########
      SUBROUTINE WRITE_DIAG_SEB_SEAFLUX_n (DTCO, DGU, U, CHS, DGS, DGSC, DGMSI, S, &
                                           HPROGRAM)
!     #################################
!
...
IF (DGS%LSURF_BUDGET) THEN
   ...
   !
   IF(.NOT.DGMSI%LDIAG_MISC_SEAICE)THEN
     YRECFM='LEI_SEA'
     YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
     CALL WRITE_SURF(DGU, U, &
...
IF (DGS%LSURF_BUDGETC) THEN
   ...
   !
   CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XRN(:),IRESP,HCOMMENT=YCOMMENT)
   !
   YRECFM='HC_SEA'
   YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
   !
   CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XH(:),IRESP,HCOMMENT=YCOMMENT)
   !
   YRECFM='LEC_SEA'
   YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
   !
   CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XLE(:),IRESP,HCOMMENT=YCOMMENT)
   !
   IF(.NOT.DGMSI%LDIAG_MISC_SEAICE)THEN
     YRECFM='LEIC_SEA'
     YCOMMENT='X_Y_'//YRECFM//' (W/m2)'
     CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XLEI(:),IRESP,HCOMMENT=YCOMMENT) 
   ENDIF
   !
   YRECFM='GFLUXC_SEA'
   YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
   !
   CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XGFLUX(:),IRESP,HCOMMENT=YCOMMENT)
   !
   YRECFM='EVAPC_SEA'
   YCOMMENT='X_Y_'//YRECFM//' (kg/m2)'
   !
   CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XEVAP(:),IRESP,HCOMMENT=YCOMMENT)
   !
   YRECFM='SUBLC_SEA'
   YCOMMENT='X_Y_'//YRECFM//' (kg/m2)'
   !
   CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XSUBL(:),IRESP,HCOMMENT=YCOMMENT)
   !
   IF (DGS%LRAD_BUDGET .OR. (DGS%LSURF_BUDGETC .AND. .NOT.DGU%LRESET_BUDGETC)) THEN
      ...
      !
      CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XSWD(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='SWUC_SEA'
      YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
      !
      CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XSWU(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='LWDC_SEA'
      YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
      !
      CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XLWD(:),IRESP,HCOMMENT=YCOMMENT)
      !
      YRECFM='LWUC_SEA'
      YCOMMENT='X_Y_'//YRECFM//' (J/m2)'
      !
      CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XLWU(:),IRESP,HCOMMENT=YCOMMENT)
      !
   ENDIF
   !
   YRECFM='FMUC_SEA'
   ...
   !
   CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XFMU(:),IRESP,HCOMMENT=YCOMMENT)
   !
   YRECFM='FMVC_SEA'
   YCOMMENT='X_Y_'//YRECFM//' (kg/ms)'
   !
   CALL WRITE_SURF(DGU, U, &
                    HPROGRAM,YRECFM,DGSC%XFMV(:),IRESP,HCOMMENT=YCOMMENT)
   !
END IF
!
