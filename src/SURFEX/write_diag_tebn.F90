!     #########
SUBROUTINE WRITE_DIAG_TEB_n (DTCO, DGU, IOB, U, TM, GDM, GRM, &
                             HPROGRAM,HWRITE)
!     ###############################################################################
!
...
!        
   IF (TM%DGT%XDIAG_TSTEP==XUNDEF .OR. &
      ...
      CALL WRITE_DIAG_SEB_TEB_n(DTCO, DGU, IOB, U, TM%CHT, TM%DGT, TM%DGUT, &
                                HPROGRAM)
      DO JTEB_PATCH=1,TM%TOP%NTEB_PATCH
        ...
        CALL WRITE_DIAG_MISC_TEB_n(DTCO, DGU, IOB, U, TM%DGCT, TM%DGMT, TM%DGMTO, TM%T, TM%TOP, &
                                   HPROGRAM,JTEB_PATCH)
      END DO      
   END IF
!
...
IF (TM%DGT%LPGD) THEN
  IF (TM%DGT%XDIAG_TSTEP==XUNDEF .OR. &
    ...
    IF (ASSOCIATED(TM%T%CUR%XBLD)) THEN
      CALL WRITE_DIAG_PGD_TEB_n(DTCO, DGU, IOB, U, TM%B, TM%BOP, TM%T, TM%TOP, TM%TPN, &
                                HPROGRAM)
      IF (TM%TOP%LGARDEN) CALL WRITE_DIAG_PGD_GRDN_n(DTCO, DGU, IOB, U, TM%DGMTO, GDM%TGDPE, GDM%TGDP, GDM%TVG, &
                                                  HPROGRAM)
    ENDIF
  END IF
END IF
