...
!        
   IF (TM%TD%O%XDIAG_TSTEP==XUNDEF .OR. &
      ...
      DO JTEB_P=1,TM%TOP%NTEB_PATCH
        CALL GOTO_WRAPPER_TEB_PATCH(JTEB_P, DMTC=TM%TD%DMTC, DMT=TM%TD%DMT, NT=TM%NT)
        CALL WRITE_DIAG_MISC_TEB_n(DTCO, HSELECT, U, TM%TD%DMTC%CUR, TM%TD%DMT%CUR, TM%TD%MTO, &
                                   GDM%VD%DP%AL(JTEB_P), GDM%VD%DEP%AL(JTEB_P), GDM%VD%DEPC%AL(JTEB_P), &
                                   GRM%VD%DP%AL(JTEB_P), GRM%VD%DEP%AL(JTEB_P), GRM%VD%DEPC%AL(JTEB_P), & 
                                   TM%NT%CUR, TM%TOP, HPROGRAM,JTEB_P)
      END DO      
   END IF
!
ENDIF
...
IF (TM%TD%O%LPGD) THEN
  IF (TM%TD%O%XDIAG_TSTEP==XUNDEF .OR. &
          ABS(NINT(TM%TOP%TTIME%TIME/TM%TD%O%XDIAG_TSTEP)*TM%TD%O%XDIAG_TSTEP-TM%TOP%TTIME%TIME)<1.E-3 ) THEN
    IF (ASSOCIATED(TM%NT%CUR%XBLD)) THEN
      !DO JTEB_P=1,TM%TOP%NTEB_PATCH
      !  CALL GOTO_WRAPPER_TEB_PATCH(JTEB_P, B=TM%B, T=TM%T)            
        CALL WRITE_DIAG_PGD_TEB_n(DTCO, HSELECT, U, TM%B%CUR, TM%BOP, TM%NT%CUR, TM%TOP, TM%TPN, HPROGRAM)
      !ENDDO
      IF (TM%TOP%LGARDEN) &
