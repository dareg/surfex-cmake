...
!        
   IF (TM%TD%O%XDIAG_TSTEP==XUNDEF .OR. &
      ...
      CALL WRITE_DIAG_SEB_TEB_n(DTCO, HSELECT, U, TM%CHT, TM%TD%O, TM%TD%D, TM%TD%U, HPROGRAM)
      DO JTEB_PATCH=1,TM%TOP%NTEB_PATCH
        CALL GOTO_WRAPPER_TEB_PATCH(JTEB_PATCH, DGCT=TM%TD%C, DGMT=TM%TD%M, T=TM%T)
        CALL WRITE_DIAG_MISC_TEB_n(DTCO, HSELECT, U, TM%TD%C%CUR, TM%TD%M%CUR, TM%TD%MO, &
...
IF (TM%TD%O%LPGD) THEN
  IF (TM%TD%O%XDIAG_TSTEP==XUNDEF .OR. &
    ...
    IF (ASSOCIATED(TM%T%CUR%XBLD)) THEN
      ...
      !ENDDO
      IF (TM%TOP%LGARDEN) &
        CALL WRITE_DIAG_PGD_GRDN_n(DTCO, HSELECT, U, TM%TD%MO%LSURF_DIAG_ALBEDO, &
                        GDM%TV%M%X, GDM%TV%M%T%CUR, GDM%TV%M%A, GDM%TV%O, HPROGRAM)
    ENDIF
  END IF
END IF
