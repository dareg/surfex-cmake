!     #########
SUBROUTINE WRITE_DIAG_TEB_n (BDD, CHE, CHI, CHS, CHN, CHU, CHW, DTCO, DTS, DTT, DTZ, &
...
!        
   IF (DGT%XDIAG_TSTEP==XUNDEF .OR. ABS(NINT(TOP%TTIME%TIME/DGT%XDIAG_TSTEP)*DGT%XDIAG_TSTEP-TOP%TTIME%TIME)<1.E-3 ) THEN
      CALL WRITE_DIAG_SEB_TEB_n(BOP, BDD, CHE, CHI, CHS, CHN, CHU, CHW, DTCO, DTS, DTT, &
...
IF (DGT%LPGD) THEN
  IF (DGT%XDIAG_TSTEP==XUNDEF .OR. ABS(NINT(TOP%TTIME%TIME/DGT%XDIAG_TSTEP)*DGT%XDIAG_TSTEP-TOP%TTIME%TIME)<1.E-3 ) THEN
    IF (ASSOCIATED(T%CUR%XBLD)) THEN
      CALL WRITE_DIAG_PGD_TEB_n(BDD, CHE, CHI, CHS, CHN, CHU, CHT, CHW, DTCO, DTS, DTT, &
                                        DTZ, DGEI, DGF, DGI, DGMI, DGMTO, DGO, DGS, DGSI, DGU, DGT, &
                                        DGUT, DGW, F, FSB, GB, IOB, ICP, I, O, S, SSB, &
                                        UG, U, SV, TCP, TGD, TGDO, TGR, TGRO, TVG, W, WSB, &
                                B, BOP, T, TOP, TPN, &
                                HPROGRAM)
      IF (TOP%LGARDEN) CALL WRITE_DIAG_PGD_GRDN_n(BOP, BDD, CHE, CHI, CHS, CHN, CHU, CHT, CHW, DTCO, DTS, &
                                         DTT, DTZ, DGEI, DGF, DGI, DGMI, DGO, DGS, DGSI, DGU, DGT, &
                                         DGUT, DGW, F, FSB, GB, IOB, ICP, I, O, S, SSB, &
