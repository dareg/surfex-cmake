...
INTEGER :: IWIDTH_I_Y
REAL    :: ZTRUNC
REAL :: ZLON0, ZLAT0, ZLON1, ZLAT1, ZLON2, ZLAT2, ZLONR, ZLATR
!
INTEGER :: ILUOUT
...
!
  ! As in acadfa*
  ! >> some dummy variables in FA header... 
  ZLON0 = 0.
  ZLAT0 = 44.
  ZLON1 = 0.
  ZLAT1 = 43.
  ...
  ZSINLA (8)  = ZDY(1)
  ZSINLA (9)  = ZDX(1)*ILON
  ZSINLA (10) = ZDY(1)*ILAT
  ZSINLA (11) = 0.
  ZSINLA (13) = ZLON1*ZRAD
  ZSINLA (14) = ZLAT1*ZRAD
  ZSINLA (15) = ZLON2*ZRAD
  ZSINLA (16) = ZLAT2*ZRAD
  ZSINLA (17) = 0.
  ZSINLA (18) = 0.
!
  INLOPA  (1)  = 0
...
! Reduce verbosity (in case it is not already done)
 ...
 CALL FACADE(CDNOMC,ITYPTR,ZSLAPO,ZCLOPO,ZSLOPO,ZCODIL,ITRONC,INLATI,INXLON, &
              INLOPA,INOZPA,ZSINLA,1,ZREFER,ZAHYBR(0:1),ZBHYBR(0:1),.TRUE.)  
!
