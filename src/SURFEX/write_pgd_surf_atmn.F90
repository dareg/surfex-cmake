...
IF (HPROGRAM.NE.'BINARY' .AND. HPROGRAM.NE.'TEXTE ') THEN
   CALL WRITE_ECOCLIMAP2_DATA(YSC%DGU, YSC%IOB, YSC%U, &
                              HPROGRAM)
ENDIF
!
 CALL WRITE_GRID(YSC%DGU, YSC%IOB, YSC%U, &
                 HPROGRAM,YSC%UG%CGRID,YSC%UG%XGRID_PAR,YSC%UG%XLAT,YSC%UG%XLON,&
                 YSC%UG%XMESH_SIZE,IRESP,YSC%USS%XZ0EFFJPDIR)
!
 CALL WRITESURF_COVER_n(YSC%DGU, YSC%IOB, &
                        YSC%U, &
                        HPROGRAM)
 CALL WRITESURF_SSO_n(YSC%DGU, YSC%IOB, YSC%U, &
                      YSC%USS, &
                      HPROGRAM)
 CALL WRITESURF_DUMMY_n(YSC%DGU, YSC%IOB, YSC%U, &
                        YSC%DUU, &
                        HPROGRAM)
!
YCOMMENT='CH_EMIS'
...
IF (YSC%CHU%LCH_EMIS) THEN
  IF (YSC%CHU%CCH_EMIS=='AGGR') THEN
    CALL WRITESURF_CH_EMIS_n(YSC%DGU, YSC%IOB, YSC%U, &
                             YSC%CHE, &
                             HPROGRAM)
  ELSE IF (YSC%CHU%CCH_EMIS=='SNAP') THEN
    CALL WRITESURF_SNAP_n(YSC%DGU, YSC%IOB, YSC%U, &
                          YSC%CHN, &
                          HPROGRAM)
  ENDIF
ENDIF
