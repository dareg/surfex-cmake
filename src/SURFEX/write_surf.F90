...
!
!
USE MODD_SURFEX_MPI, ONLY : WLOG_MPI, NRANK, npio
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK
USE PARKIND1  ,ONLY : JPRB
!
#ifdef SFX_OL
...
YREC = HREC
!
!if(YREC(1:8)=="TG1_ISBA".and.nrank==npio) print*,'test in'
 CALL TEST_RECORD_LEN(HPROGRAM,YREC,HSELECT,GNOWRITE)
!if(YREC(1:8)=="TG1_ISBA".and.nrank==npio) print*,'test out',GNOWRITE
!if (YREC(1:2)=="TG") print*,YREC,GNOWRITE,NRANK,HPROGRAM
!
...
ELSE
  ...
  IF (HPROGRAM=='BINARY') THEN
#ifdef SFX_BIN
!if(YREC(1:8)=="TG1_ISBA".and.nrank==npio) print*,'write bin in '
    CALL WRITE_SURFX_BIN(HSELECT,YREC,PFIELD,KRESP,HCOMMENT,YDIR)
!if(YREC(1:8)=="TG1_ISBA".and.nrank==npio) print*,'write bin out '
#endif
  ENDIF
  !
  IF (HPROGRAM=='LFI   ') THEN
