...
YCOMMENT='(-)'
 ...
 CALL WRITE_SURF(YSC%DGU, YSC%U, &
                 HPROGRAM,'WRITE_EXT ',LWRITE_EXTERN,IRESP,HCOMMENT=YCOMMENT)          
!
YCOMMENT='s'
 CALL WRITE_SURF(YSC%DGU, YSC%U, &
...
!
 CALL WRITE_GRID(YSC%DGU, YSC%U, &
                 HPROGRAM,YSC%UG%G%CGRID,YSC%UG%G%XGRID_PAR,YSC%UG%G%XLAT,YSC%UG%G%XLON,YSC%UG%G%XMESH_SIZE,IRESP)
!
