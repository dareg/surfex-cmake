...
SUBROUTINE WRITE_SURF_XIOS(HREC, HCOMMENT, KFIELD, PFIELD, OFIELD, HFIELD, PFIELD1, PFIELD2, PFIELD3, HAXIS) 
!!
!!
!!     PURPOSE
!!     --------
...
!     ENDIF
  ...
  ELSE IF (PRESENT(PFIELD2)) THEN 
    ...
    ELSE ! Per-patch fields are written through a loop, and with patch number suffix
      ...
      DO JI=1,SIZE(PFIELD2,2)
        ...
        ELSE
          CALL SFX_XIOS_SEND_BLOCK(YNAME,PFIELD=PFIELD2(:,JI))
        ENDIF
  ...
  ELSE IF (PRESENT(PFIELD3)) THEN 
    ...
    ELSE
      ...
      CALL ABOR1_SFX('INTERFACE TO XIOS CANNOT YET HANDLE 2 AXES IN ADDITION TO HORIZONTAL SPACE AXIS/AXES')
