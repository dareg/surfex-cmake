...
!               ---------------------------
!
 CALL WRITESURF_TEB_CONF_n(TM%CHT, TM%TD%MTO, TM%TD%O, TM%TD%DUT, TM%NT, TM%TOP,HPROGRAM)
!
DO JPATCH=1,TM%TOP%NTEB_PATCH
  CALL GOTO_WRAPPER_TEB_PATCH(JPATCH, B=TM%B, DMTC=TM%TD%DMTC, DMT=TM%TD%DMT, NT=TM%NT)
  CALL WRITESURF_TEB_n(HSELECT, TM%TOP, TM%BOP, TM%NT%CUR, TM%B%CUR, TM%DTT%LDATA_ROAD_DIR, TM%TPN, &
                       GDM%O, GDM%NPE%AL(JPATCH), GRM%O, GRM%NPE%AL(JPATCH), &
