...
YCOMMENT='flag to use SBL levels'
 CALL WRITE_SURF(HPROGRAM,YRECFM,F%LSBL,IRESP,HCOMMENT=YCOMMENT)
!
...
YCOMMENT='number of SBL levels'
 CALL WRITE_SURF(HPROGRAM,YRECFM,FSB%NLVL,IRESP,HCOMMENT=YCOMMENT)
!
!* altitudes
!
DO JLAYER=1,FSB%NLVL
  ...
  CALL WRITE_SURF(HPROGRAM,YRECFM,FSB%XZ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
IF (HWRITE/='PRE') THEN
  !
  ...
  DO JLAYER=1,FSB%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,FSB%XU(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* temperature in SBL
  !
  DO JLAYER=1,FSB%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,FSB%XT(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* humidity in SBL
  !
  DO JLAYER=1,FSB%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,FSB%XQ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Tke in SBL
  !
  DO JLAYER=1,FSB%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,FSB%XTKE(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Monin-Obhukov length
  !
  YRECFM='WAT_SBL_LMO     '
  CALL WRITE_SURF(HPROGRAM,YRECFM,FSB%XLMO(:),IRESP,HCOMMENT=YCOMMENT)
  !
  !* Air pressure in SBL
  !
  DO JLAYER=1,FSB%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,FSB%XP(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
ENDIF
!
