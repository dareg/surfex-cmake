!     #########
      SUBROUTINE WRITESURF_GR_SNOW (HSELECT, HPROGRAM,HSURFTYPE,HPREFIX,TPSNOW  )
!     ##########################################################
!
...
DO JLAYER = 1,TPSNOW%NLAYER
  ...
  IF (TPSNOW%SCHEME=='1-L' .OR. TPSNOW%SCHEME=='D95' .OR. TPSNOW%SCHEME=='EBA' .OR. &
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,TPSNOW%WSNOW(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
    !
    !*       7.    Snow density
    !              ------------
    !
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,TPSNOW%RHO(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
    !
  END IF
  !
  !*       8.    Snow temperature
  ...
  IF (TPSNOW%SCHEME=='1-L') THEN
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,TPSNOW%T(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
    !
  END IF
  !
  IF (TPSNOW%SCHEME=='3-L' .OR. TPSNOW%SCHEME=='CRO') THEN
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,TPSNOW%HEAT(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
    !
    !*       10.    Age parameter
    !              ---------------
    !
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,TPSNOW%AGE(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
    !
  END IF
  !
  IF (TPSNOW%SCHEME=='CRO') THEN
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,TPSNOW%GRAN1(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
    !
    !*       12.    Snow Gran2
    !              ------------
    !
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,TPSNOW%GRAN2(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
    !
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,TPSNOW%HIST(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
    !
  END IF
  !
END DO
...
IF (TPSNOW%SCHEME=='D95' .OR. TPSNOW%SCHEME=='EBA' .OR. TPSNOW%SCHEME=='1-L' .OR. &
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,TPSNOW%ALB(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
END IF
!
IF (LHOOK) CALL DR_HOOK('WRITESURF_GR_SNOW',1,ZHOOK_HANDLE)
