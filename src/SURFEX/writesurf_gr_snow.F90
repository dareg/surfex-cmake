...
YCOMMENT    = '(INTEGER)'
 ...
 CALL WRITE_SURF(DGU, U, &
                HPROGRAM,'NIMPUR',NIMPUR,IRESP,HCOMMENT=YCOMMENT)
...
IF (DGU%LSNOWDIMNC .AND. (HPROGRAM=='OFFLIN')) THEN


  WRITE(YFMT,'(A5,I1,A4)') '(A9,A',ISURFTYPE_LEN,',A8)'    
  WRITE(YCOMMENT,YFMT) 'X_Y_WSNOW',HSURFTYPE,' (kg/m2)'
  CALL WRITE_SURF(DGU, U, HPROGRAM,'WSN_VEG',TPSNOW%WSNOW(:,:,:),IRESP,HCOMMENT=YCOMMENT)  
  
  WRITE(YFMT,'(A5,I1,A4)') '(A9,A',ISURFTYPE_LEN,',A8)'
  WRITE(YCOMMENT,YFMT) 'X_Y_RSNOW',HSURFTYPE,' (kg/m2)'
  CALL WRITE_SURF(DGU, U, HPROGRAM,'SNOWRO',TPSNOW%RHO(:,:,:),IRESP,HCOMMENT=YCOMMENT)  

  WRITE(YFMT,'(A5,I1,A4)') '(A9,A',ISURFTYPE_LEN,',A8)'
  WRITE(YCOMMENT,YFMT) 'X_Y_HSNOW',HSURFTYPE,' (kg/m2)'
  CALL WRITE_SURF(DGU, U, HPROGRAM,'SNOWHEAT',TPSNOW%HEAT(:,:,:),IRESP,HCOMMENT=YCOMMENT)  

  WRITE(YFMT,'(A5,I1,A4)') '(A8,A',ISURFTYPE_LEN,',A8)'
  WRITE(YCOMMENT,YFMT) 'X_Y_SAGE',HSURFTYPE,' (kg/m2)'
  CALL WRITE_SURF(DGU, U, HPROGRAM,'SNOWAGE',TPSNOW%AGE(:,:,:),IRESP,HCOMMENT=YCOMMENT)
  ...
  CALL WRITE_SURF(DGU, U, HPROGRAM,'SNOWGRAN2',TPSNOW%GRAN2(:,:,:),IRESP,HCOMMENT=YCOMMENT)
  
  WRITE(YFMT,'(A5,I1,A4)') '(A8,A',ISURFTYPE_LEN,',A8)'  
  WRITE(YCOMMENT,YFMT) 'X_Y_HIST',HSURFTYPE,' (kg/m2)'
  CALL WRITE_SURF(DGU, U, HPROGRAM,'SNOWHIST',TPSNOW%HIST(:,:,:),IRESP,HCOMMENT=YCOMMENT)  
...
!      !
      DO JIMP=1,NIMPUR
        ...
        CALL WRITE_SURF(DGU, U, &
                     HPROGRAM,YRECFM,TPSNOW%IMPUR(:,JLAYER,JIMP,:),IRESP,HCOMMENT=YCOMMENT)
      ENDDO
