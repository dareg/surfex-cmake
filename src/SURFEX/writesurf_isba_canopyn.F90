!     #########
      SUBROUTINE WRITESURF_ISBA_CANOPY_n (HSELECT, ICP, OCANOPY, HPROGRAM,HWRITE)
!     ####################################
!
...
YCOMMENT='number of canopy levels'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,ICP%NLVL,IRESP,HCOMMENT=YCOMMENT)
!
!* altitudes
!
DO JLAYER=1,ICP%NLVL
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,ICP%XZ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
IF (HWRITE/='PRE') THEN
  !
  ...
  DO JLAYER=1,ICP%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,ICP%XU(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  ...
  DO JLAYER=1,ICP%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,ICP%XT(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* humidity in canopy
  !
  DO JLAYER=1,ICP%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,ICP%XQ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Tke in canopy
  !
  DO JLAYER=1,ICP%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,ICP%XTKE(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Monin-Obhukov length
  !
  YRECFM='ISBA_CAN_LMO'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,ICP%XLMO(:,ICP%NLVL),IRESP,HCOMMENT=YCOMMENT)
  !
  ...
  DO JLAYER=1,ICP%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,ICP%XP(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
ENDIF
!
