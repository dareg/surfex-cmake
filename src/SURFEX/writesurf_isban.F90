...
!
INTEGER :: JJ, JL, JP, JNBIOMASS, JNLITTER, JNSOILCARB, JNLITTLEVS  ! loop counter on levels
...
DO JL=1,IO%NGROUND_LAYER
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,NPE%AL(JP)%XWG(:,JL),KI)  
  ENDDO
END DO
!
...
DO JL=1,IWORK
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,NPE%AL(JP)%XWGI(:,JL),KI)  
  ENDDO
END DO
!
...
DO JP = 1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,NPE%AL(JP)%XWR(:),KI)    
ENDDO
!
!* Glacier ice storage
!
...
IF(IO%LGLACIER)THEN
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,NPE%AL(JP)%XICE_STO(:),KI)   
  ENDDO
ENDIF
!
!* Leaf Area Index
...
IF (IO%CPHOTO/='NON' .AND. IO%CPHOTO/='AST') THEN
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                NP%AL(JP)%NR_P,NPE%AL(JP)%XLAI(:),KI)    
  ENDDO
  !
END IF
!
IF ( TRIM(CASSIM_ISBA)=="ENKF" .AND. (LASSIM .OR. NIE/=0) ) THEN
  DO JVAR = 1,NVAR
    IF ( XADDTIMECORR(JVAR)>0. ) THEN
      ...
      DO JP = 1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                     NP%AL(JP)%NR_P,NP%AL(JP)%XRED_NOISE(:,JVAR),KI)       
      ENDDO
    ENDIF
  ENDDO
ENDIF
...
!
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                  NP%AL(JP)%NR_P,NPE%AL(JP)%XWRL(:),KI)    
  ENDDO
!
!* ice on litter
!
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                  NP%AL(JP)%NR_P,NPE%AL(JP)%XWRLI(:),KI)    
  ENDDO
!
...
!
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                  NP%AL(JP)%NR_P,NPE%AL(JP)%XWRVN(:),KI)    
  ENDDO
...
!
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                  NP%AL(JP)%NR_P,NPE%AL(JP)%XTV(:),KI)    
  ENDDO
!
!* litter temperature
!
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                  NP%AL(JP)%NR_P,NPE%AL(JP)%XTL(:),KI)    
  ENDDO
!
...
!
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                  NP%AL(JP)%NR_P,NPE%AL(JP)%XTC(:),KI)    
  ENDDO
!
...
!
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                  NP%AL(JP)%NR_P,NPE%AL(JP)%XQC(:),KI)    
  ENDDO
!
ENDIF
!
...
DO JP = 1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                  NP%AL(JP)%NR_P,NP%AL(JP)%XPATCH(:),KI)    
ENDDO
!
...
DO JP = 1,IO%NPATCH
  CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                  NP%AL(JP)%NR_P,NPE%AL(JP)%XRESA(:),KI)    
ENDDO
!
!* Land use variables
!
...
!
  DO JL=1,IO%NGROUND_LAYER
    ...
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NP%AL(JP)%XDG(:,JL),KI)    
    ENDDO
  END DO
!
ENDIF
...
IF (IO%CPHOTO/='NON') THEN
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                    NP%AL(JP)%NR_P,NPE%AL(JP)%XAN(:),KI)    
  ENDDO
!
  YRECFM='ANDAY'
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                    NP%AL(JP)%NR_P,NPE%AL(JP)%XANDAY(:),KI)    
  ENDDO  
!
  YRECFM='ANFM'
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                    NP%AL(JP)%NR_P,NPE%AL(JP)%XANFM(:),KI)    
  ENDDO    
!
  YRECFM='LE_AGS'
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                    NP%AL(JP)%NR_P,NPE%AL(JP)%XLE(:),KI)    
  ENDDO    
END IF
!
!
IF (IO%CPHOTO=='NIT' .OR. IO%CPHOTO=='NCB') THEN
  ...
  DO JNBIOMASS=1,IO%NNBIOMASS
    ...
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NPE%AL(JP)%XBIOMASS(:,JNBIOMASS),KI)    
    ENDDO      
  END DO
  !
  !
  DO JNBIOMASS=2,IO%NNBIOMASS
    ...
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NPE%AL(JP)%XRESP_BIOMASS(:,JNBIOMASS),KI)    
    ENDDO      
  END DO
  !
END IF
...
IF (IO%CRESPSL=='CNT') THEN
  ...
  DO JNLITTER=1,IO%NNLITTER
    DO JNLITTLEVS=1,IO%NNLITTLEVS
      ...
      DO JP = 1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                        NP%AL(JP)%NR_P,NPE%AL(JP)%XLITTER(:,JNLITTER,JNLITTLEVS),KI)    
      ENDDO        
    END DO
  END DO
  ...
  DO JNSOILCARB=1,IO%NNSOILCARB
    ...
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NPE%AL(JP)%XSOILCARB(:,JNSOILCARB),KI)    
    ENDDO     
  END DO
!
  DO JNLITTLEVS=1,IO%NNLITTLEVS
    ...
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NPE%AL(JP)%XLIGNIN_STRUC(:,JNLITTLEVS),KI)    
    ENDDO       
  END DO
!
ENDIF
...
IF (CHI%SVI%NDSTEQ > 0)THEN
  DO JSV = 1,NDSTMDE ! for all dust modes
    ...
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NDST%AL(JP)%XSFDSTM(:,JSV),KI)    
    ENDDO     
  END DO
ENDIF
!
