!     #########
      SUBROUTINE WRITESURF_ISBA_n (HSELECT, CHI, DST, &
                                   IO, PPATCH, IR, PLAI, PDG, TPTIME, HPROGRAM,OLAND_USE)
!     #####################################
!
...
DO JLAYER=1,IWORK
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XTG(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
END DO
!
...
DO JLAYER=1,IO%NGROUND_LAYER
   ...
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XWG(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
END DO
!
...
DO JLAYER=1,IWORK
   ...
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XWGI(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)  
END DO
!
...
YCOMMENT='X_Y_WR (kg/m2)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XWR(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!* Glacier ice storage
!
YRECFM = 'GLACIER'
...
IF(IO%LGLACIER)THEN
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XICE_STO(:,:),IRESP,HCOMMENT=YCOMMENT)
ENDIF
!
!* Leaf Area Index
!
IF (IO%CPHOTO/='NON' .AND. IO%CPHOTO/='AGS' .AND. IO%CPHOTO/='AST') THEN
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,PLAI(:,:),IRESP,HCOMMENT=YCOMMENT)
  !
END IF
!
IF ( TRIM(CASSIM_ISBA)=="ENKF" .AND. (LASSIM .OR. NIE/=0) ) THEN
  DO JVAR = 1,NVAR
    IF ( XADDTIMECORR(JVAR)>0. ) THEN
      ...
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XRED_NOISE(:,:,JVAR),IRESP,HCOMMENT=YCOMMENT)
    ENDIF
  ENDDO
ENDIF
!
!* snow mantel
!
 CALL WRITESURF_GR_SNOW(HSELECT, HPROGRAM,'VEG','     ',IR%TSNOW)
!
!
...
!
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XWRL(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!* ice on litter
!
  YRECFM='WRLI'
  YCOMMENT='X_Y_WRLI (kg/m2)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XWRLI(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!* snow intercepted on canopy vegetation leaves
!
  YRECFM='WRVN'
  YCOMMENT='X_Y_WRVN (kg/m2)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XWRVN(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!* canopy vegetation temperature
!
  YRECFM='TV'
  YCOMMENT='X_Y_TV (K)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XTV(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!* litter temperature
!
  YRECFM='TL'
  YCOMMENT='X_Y_TL (K)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XTL(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!* vegetation canopy air temperature
!
  YRECFM='TC'
  YCOMMENT='X_Y_TC (K)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XTC(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!* vegetation canopy air specific humidity
!
  YRECFM='QC'
  YCOMMENT='X_Y_QC (kg/kg)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XQC(:,:),IRESP,HCOMMENT=YCOMMENT)
!
ENDIF
!
!-------------------------------------------------------------------------------
...
CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,PPATCH(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!* patch averaged radiative temperature (K)
!
YRECFM='TSRAD_NAT'
YCOMMENT='X_TSRAD_NAT (K)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XTSRAD_NAT(:),IRESP,HCOMMENT=YCOMMENT)
!
!* aerodynamical resistance
!
YRECFM='RESA'
YCOMMENT='X_Y_RESA (s/m)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XRESA(:,:),IRESP,HCOMMENT=YCOMMENT)
!
!* Land use variables
!
...
!
  DO JLAYER=1,IO%NGROUND_LAYER
    ...
    ENDIF
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,PDG(:,JLAYER,:),IRESP,HCOMMENT=YCOMMENT)
  END DO
!
ENDIF
!
...
IF (IO%CPHOTO/='NON') THEN
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XAN(:,:),IRESP,HCOMMENT=YCOMMENT)
!
  YRECFM='ANDAY'
  YCOMMENT='X_Y_ANDAY (kgCO2/m2/day)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XANDAY(:,:),IRESP,HCOMMENT=YCOMMENT)
!
  YRECFM='ANFM'
  YCOMMENT='X_Y_ANFM (kgCO2/kgair m/s)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XANFM(:,:),IRESP,HCOMMENT=YCOMMENT)
!
  YRECFM='LE_AGS'
  YCOMMENT='X_Y_LE_AGS (W/m2)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XLE(:,:),IRESP,HCOMMENT=YCOMMENT)
END IF
!
!
IF (IO%CPHOTO=='NIT' .OR. IO%CPHOTO=='NCB') THEN
  ...
  DO JNBIOMASS=1,IO%NNBIOMASS
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XBIOMASS(:,JNBIOMASS,:),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !
  DO JNBIOMASS=2,IO%NNBIOMASS
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XRESP_BIOMASS(:,JNBIOMASS,:),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
END IF
!
...
IF (IO%CRESPSL=='CNT') THEN
  ...
  DO JNLITTER=1,IO%NNLITTER
    DO JNLITTLEVS=1,IO%NNLITTLEVS
      ...
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XLITTER(:,JNLITTER,JNLITTLEVS,:),IRESP,HCOMMENT=YCOMMENT)
    END DO
  END DO
  ...
  DO JNSOILCARB=1,IO%NNSOILCARB
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XSOILCARB(:,JNSOILCARB,:),IRESP,HCOMMENT=YCOMMENT)
  END DO
!
  DO JNLITTLEVS=1,IO%NNLITTLEVS
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,IR%XLIGNIN_STRUC(:,JNLITTLEVS,:),IRESP,HCOMMENT=YCOMMENT)
  END DO
!
ENDIF
!
...
IF (CHI%SVI%NDSTEQ > 0)THEN
  DO JSV = 1,NDSTMDE ! for all dust modes
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DST%XSFDSTM(:,JSV,:),IRESP,HCOMMENT=YCOMMENT)
  END DO
ENDIF
!
!-------------------------------------------------------------------------------
...
YRECFM='DTCUR'
YCOMMENT='s'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,TPTIME,IRESP,HCOMMENT=YCOMMENT)
