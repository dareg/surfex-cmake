...
IF (IO%CPHOTO=='NIT' .OR. IO%CPHOTO=='NCB') THEN
  ...
  DO JNB=1,IO%NNBIOMASS
    ...
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NPE%AL(JP)%XBIOMASS(:,JNB),KI)    
    ENDDO      
  END DO
  !
  !
  DO JNB=2,IO%NNBIOMASS
    ...
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NPE%AL(JP)%XRESP_BIOMASS(:,JNB),KI)    
    ENDDO      
  END DO
  !
END IF
...
IF (IO%CRESPSL=='CNT') THEN
  ...
  DO JNL=1,IO%NNLITTER
    DO JNLV=1,IO%NNLITTLEVS
      ...
      DO JP = 1,IO%NPATCH
        CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                        NP%AL(JP)%NR_P,NPE%AL(JP)%XLITTER(:,JNL,JNLV),KI)    
      ENDDO        
    END DO
  END DO
  ...
  DO JNS=1,IO%NNSOILCARB
    ...
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NPE%AL(JP)%XSOILCARB(:,JNS),KI)    
    ENDDO     
  END DO
!
  DO JNLV=1,IO%NNLITTLEVS
    ...
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NPE%AL(JP)%XLIGNIN_STRUC(:,JNLV),KI)    
    ENDDO       
  END DO
!
ENDIF
