...
IF (DTV%LDATA_VEGTYPE) THEN
  ...
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DTV%XPAR_VEGTYPE(:,:),IRESP,HCOMMENT=YCOMMENT)
ENDIF
!
YRECFM='NDATA_TIME'
YCOMMENT='(-)'
...
YRECFM='L_LAI'
YCOMMENT=YRECFM
!we don't write LAI for the 3 first vegtypes (bare soil)
DO JTIME = 1,DTV%NTIME
  DO JV = 1,3
    DTV%LDATA_LAI((JTIME-1)*NVEGTYPE+JV) = .FALSE.
  ENDDO
ENDDO
...
DO JTIME = 1,DTV%NTIME
  DO JV = 1,3
    DTV%LDATA_ALBNIR_VEG((JTIME-1)*NVEGTYPE+JV) = .FALSE.
  ENDDO
ENDDO
...
DO JTIME = 1,DTV%NTIME
  DO JV = 1,3
    DTV%LDATA_ALBVIS_VEG((JTIME-1)*NVEGTYPE+JV) = .FALSE.
  ENDDO
ENDDO
...
DO JLAYER=1,SIZE(DTV%XPAR_DG,2)
  DO JV=1,NVEGTYPE
    IF (DTV%LDATA_DG(JV)) THEN
      WRITE(YRECFM,FMT='(A6,I2.2,A1,I2.2)') 'D_DG_L',JLAYER,'V',JV  
      YCOMMENT='X_Y_'//YRECFM     
      CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,DTV%XPAR_DG(:,JLAYER,JV),IRESP,HCOMMENT=YCOMMENT)
    ENDIF
  END DO
ENDDO
!
...
DO JLAYER=1,SIZE(DTV%XPAR_ROOTFRAC,2)
  DO JV=1,NVEGTYPE
    IF (DTV%LDATA_ROOTFRAC(JV)) THEN
      WRITE(YRECFM,FMT='(A6,I2.2,A1,I2.2)') 'D_RTF_L',JLAYER,'V',JV
      YCOMMENT='X_Y_'//YRECFM
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_ROOTFRAC(:,JLAYER,JV),IRESP,HCOMMENT=YCOMMENT)
    ENDIF
  END DO
ENDDO
!
