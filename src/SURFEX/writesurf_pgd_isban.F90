...
!
IF(I%CISBA=='DIF') THEN
  IF (ALL(I%XSOILGRID(:)==XUNDEF)) THEN
    GSOILGRID = .FALSE.
  ELSE
    GSOILGRID = .TRUE.
  ENDIF
  YCOMMENT = "L_SOILGRID"
  CALL WRITE_SURF(DGU, U, HPROGRAM,"L_SOILGRID",GSOILGRID,IRESP,HCOMMENT=YCOMMENT) 
  IF (GSOILGRID) THEN
...
!
 CALL WRITE_GRID(DGU, U, &
                 HPROGRAM,IG%CGRID,IG%XGRID_PAR,IG%XLAT,IG%XLON,IG%XMESH_SIZE,IRESP)
!
!
!* clay fraction
!
