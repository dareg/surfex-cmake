!     #########
      SUBROUTINE WRITESURF_SEAFLUX_SBL_n (HSELECT, OSBL, SSB, HPROGRAM,HWRITE)
!     ####################################
!
...
YCOMMENT='number of SBL levels'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSB%NLVL,IRESP,HCOMMENT=YCOMMENT)
!
!* altitudes
!
DO JLAYER=1,SSB%NLVL
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSB%XZ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
IF (HWRITE/='PRE') THEN
  !
  ...
  DO JLAYER=1,SSB%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSB%XU(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* temperature in SBL
  !
  DO JLAYER=1,SSB%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSB%XT(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* humidity in SBL
  !
  DO JLAYER=1,SSB%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSB%XQ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Tke in SBL
  !
  DO JLAYER=1,SSB%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSB%XTKE(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Monin-Obhukov length
  !
  YRECFM='SEA_SBL_LMO '
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSB%XLMO(:,SSB%NLVL),IRESP,HCOMMENT=YCOMMENT)
  !
  !* Air pressure in SBL
  !
  DO JLAYER=1,SSB%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSB%XP(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
ENDIF
!
