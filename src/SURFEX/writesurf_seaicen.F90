...
!
YCOMMENT='(-)'
CALL WRITE_SURF(HPROGRAM,'SEAICE_SCHEM',CSEAICE_SCHEME,IRESP,YCOMMENT)
!
!
IF (CSEAICE_SCHEME == 'GELATO') THEN 
   ...
   !
   YCOMMENT='ICEUSTAR ()'
   CALL WRITE_SURF(HPROGRAM,'ICEUSTAR',TGLT%ust(:,1),IRESP,YCOMMENT)
   !
   ...
   DO JK=1,nt
      ...
      CALL WRITE_SURF(HPROGRAM,'ICEAGE'//YCATEG,TGLT%sit(JK,:,1)%age,IRESP,YCOMMENT)
      ! .. Write melt pond volume for type JK
      YCOMMENT='X_Y_ICEVMP'//YCATEG//' (m3)'
      CALL WRITE_SURF(HPROGRAM,'ICEVMP'//YCATEG,TGLT%sit(JK,:,1)%vmp,IRESP,YCOMMENT)
      ! .. Write sea ice surface albedo for type JK
      YCOMMENT='X_Y_ICEASN'//YCATEG//' ([0-1])'
      CALL WRITE_SURF(HPROGRAM,'ICEASN'//YCATEG,TGLT%sit(JK,:,1)%asn,IRESP,YCOMMENT)
      ! .. Write sea ice fraction for type JK
      YCOMMENT='X_Y_ICEFSI'//YCATEG//' ([0-1])'
      CALL WRITE_SURF(HPROGRAM,'ICEFSI'//YCATEG, TGLT%sit(JK,:,1)%fsi,IRESP,YCOMMENT)
      ! .. Write sea ice thickness for type JK
      YCOMMENT='X_Y_ICEHSI'//YCATEG//' (m)'
      CALL WRITE_SURF(HPROGRAM,'ICEHSI'//YCATEG, TGLT%sit(JK,:,1)%hsi,IRESP,YCOMMENT)
      ! .. Write sea ice salinity for type JK
      YCOMMENT='X_Y_ICESSI'//YCATEG//' (psu)'
      CALL WRITE_SURF(HPROGRAM,'ICESSI'//YCATEG, TGLT%sit(JK,:,1)%ssi,IRESP,YCOMMENT)
      ! .. Write sea ice surface temperature for type JK
      YCOMMENT='X_Y_ICETSF'//YCATEG//' (K)'
      CALL WRITE_SURF(HPROGRAM,'ICETSF'//YCATEG, TGLT%sit(JK,:,1)%tsf,IRESP,YCOMMENT)
      ! .. Write snow thickness for type JK
      YCOMMENT='X_Y_ICEHSN'//YCATEG//' (m)'
      CALL WRITE_SURF(HPROGRAM,'ICEHSN'//YCATEG, TGLT%sit(JK,:,1)%hsn,IRESP,YCOMMENT)
      ! .. Write snow density for type JK
      YCOMMENT='X_Y_ICERSN'//YCATEG//' (kg m-3)'
      CALL WRITE_SURF(HPROGRAM,'ICERSN'//YCATEG, TGLT%sit(JK,:,1)%rsn,IRESP,YCOMMENT)
      !
      ...
      DO JL=1,nl
         ...
         CALL WRITE_SURF(HPROGRAM,'ICEH'//YLEVEL, TGLT%sil(JL,JK,:,1)%ent,IRESP,YCOMMENT)
      END DO
   END DO
ELSE
...
!
   DO JMTH=1,INMTH
      ...
      CALL WRITE_SURF(HPROGRAM,YRECFM,XSIC_MTH(:,JMTH),IRESP,HCOMMENT=YCOMMENT)
   ENDDO
!
ENDIF
!
