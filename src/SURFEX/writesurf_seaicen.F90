...
!
YCOMMENT='(-)'
CALL WRITE_SURF(HPROGRAM,'SEAICE_SCHEM',S%CSEAICE_SCHEME,IRESP,YCOMMENT)
!
!
IF (S%CSEAICE_SCHEME == 'GELATO') THEN 
   YCOMMENT='Number of sea-ice layers'
   CALL WRITE_SURF(HPROGRAM,'ICENL',nl,IRESP,YCOMMENT)
   YCOMMENT='Number of ice categories'
   CALL WRITE_SURF(HPROGRAM,'ICENT',nt,IRESP,YCOMMENT)
   !
   !*       1.     Prognostic fields with only space dimension(s) :
   !
   YCOMMENT='ICEUSTAR ()'
   CALL WRITE_SURF(HPROGRAM,'ICEUSTAR',S%TGLT%ust(:,1),IRESP,YCOMMENT)
   !
   ...
   DO JK=1,nt
      ...
      CALL WRITE_SURF(HPROGRAM,'ICEAGE'//YCATEG,S%TGLT%sit(JK,:,1)%age,IRESP,YCOMMENT)
      ! .. Write melt pond volume for type JK
      YCOMMENT='X_Y_ICEVMP'//YCATEG//' (m3)'
      CALL WRITE_SURF(HPROGRAM,'ICEVMP'//YCATEG,S%TGLT%sit(JK,:,1)%vmp,IRESP,YCOMMENT)
      ! .. Write sea ice surface albedo for type JK
      YCOMMENT='X_Y_ICEASN'//YCATEG//' ([0-1])'
      CALL WRITE_SURF(HPROGRAM,'ICEASN'//YCATEG,S%TGLT%sit(JK,:,1)%asn,IRESP,YCOMMENT)
      ! .. Write sea ice fraction for type JK
      YCOMMENT='X_Y_ICEFSI'//YCATEG//' ([0-1])'
      CALL WRITE_SURF(HPROGRAM,'ICEFSI'//YCATEG, S%TGLT%sit(JK,:,1)%fsi,IRESP,YCOMMENT)
      ! .. Write sea ice thickness for type JK
      YCOMMENT='X_Y_ICEHSI'//YCATEG//' (m)'
      CALL WRITE_SURF(HPROGRAM,'ICEHSI'//YCATEG, S%TGLT%sit(JK,:,1)%hsi,IRESP,YCOMMENT)
      ! .. Write sea ice salinity for type JK
      YCOMMENT='X_Y_ICESSI'//YCATEG//' (psu)'
      CALL WRITE_SURF(HPROGRAM,'ICESSI'//YCATEG, S%TGLT%sit(JK,:,1)%ssi,IRESP,YCOMMENT)
      ! .. Write sea ice surface temperature for type JK
      YCOMMENT='X_Y_ICETSF'//YCATEG//' (K)'
      CALL WRITE_SURF(HPROGRAM,'ICETSF'//YCATEG, S%TGLT%sit(JK,:,1)%tsf,IRESP,YCOMMENT)
      ! .. Write snow thickness for type JK
      YCOMMENT='X_Y_ICEHSN'//YCATEG//' (m)'
      CALL WRITE_SURF(HPROGRAM,'ICEHSN'//YCATEG, S%TGLT%sit(JK,:,1)%hsn,IRESP,YCOMMENT)
      ! .. Write snow density for type JK
      YCOMMENT='X_Y_ICERSN'//YCATEG//' (kg m-3)'
      CALL WRITE_SURF(HPROGRAM,'ICERSN'//YCATEG, S%TGLT%sit(JK,:,1)%rsn,IRESP,YCOMMENT)
      !
      ...
      DO JL=1,nl
         ...
         CALL WRITE_SURF(HPROGRAM,'ICEH'//YLEVEL, S%TGLT%sil(JL,JK,:,1)%ent,IRESP,YCOMMENT)
      END DO
   END DO
ELSE
...
!
   DO JMTH=1,INMTH
      ...
      CALL WRITE_SURF(HPROGRAM,YRECFM,S%XSIC_MTH(:,JMTH),IRESP,HCOMMENT=YCOMMENT)
   ENDDO
!
ENDIF
!
...
CALL WRITE_SURF(HPROGRAM,YRECFM,S%XSIC(:),IRESP,HCOMMENT=YCOMMENT)  
!
!
...
!
   DO JMTH=1,INMTH
      ...
      CALL WRITE_SURF(HPROGRAM,YRECFM,S%XSIT_MTH(:,JMTH),IRESP,HCOMMENT=YCOMMENT)
   ENDDO
!
ENDIF
!
