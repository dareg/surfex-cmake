!     ####################################
      SUBROUTINE WRITESURF_SSO_CANOPY_n (HSELECT,SSCP,HPROGRAM,OWRITE)
!     ####################################
!
...
YCOMMENT='number of canopy levels'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSCP%NLVL,IRESP,HCOMMENT=YCOMMENT)
!
!* altitudes
!
DO JLAYER=1,SSCP%NLVL
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSCP%XZ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
!* wind in canopy
!
DO JLAYER=1,SSCP%NLVL
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSCP%XU(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
!* Tke in canopy
!
DO JLAYER=1,SSCP%NLVL
  ...
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SSCP%XTKE(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
