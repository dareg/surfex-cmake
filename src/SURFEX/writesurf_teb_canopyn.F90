...
YCOMMENT='flag to use canopy levels'
 CALL WRITE_SURF(HPROGRAM,YRECFM,TOP%LCANOPY,IRESP,HCOMMENT=YCOMMENT)
!
...
YCOMMENT='number of canopy levels'
 CALL WRITE_SURF(HPROGRAM,YRECFM,TCP%NLVL,IRESP,HCOMMENT=YCOMMENT)
!
!* altitudes
!
DO JLAYER=1,TCP%NLVL
  ...
  CALL WRITE_SURF(HPROGRAM,YRECFM,TCP%XZ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
IF (HWRITE/='PRE') THEN
  !
  ...
  DO JLAYER=1,TCP%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TCP%XU(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  ...
  DO JLAYER=1,TCP%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TCP%XT(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* humidity in canopy
  !
  DO JLAYER=1,TCP%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TCP%XQ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Tke in canopy
  !
  DO JLAYER=1,TCP%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TCP%XTKE(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  ...
  DO JLAYER=1,TCP%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TCP%XLMO(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* mixing length
  !
  IF (ASSOCIATED(TCP%XLM)) THEN
    DO JLAYER=1,TCP%NLVL
      ...
      CALL WRITE_SURF(HPROGRAM,YRECFM,TCP%XLM(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
   END DO
  END IF
  !
  !* dissipative length
  ...
  IF (ASSOCIATED(TCP%XLEPS)) THEN
    DO JLAYER=1,TCP%NLVL
      ...
      YCOMMENT='mixing length (m)'
      CALL WRITE_SURF(HPROGRAM,YRECFM,TCP%XLEPS(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
    END DO
  END IF
  !
  ...
  DO JLAYER=1,TCP%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TCP%XP(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
ENDIF
!
