!     #########
      SUBROUTINE WRITESURF_TEB_CANOPY_n (HSELECT, SB, OCANOPY, HPROGRAM,HWRITE)
!     ####################################
!
...
IF (HWRITE/='PRE') THEN
  ...
  DO JLAYER=1,SB%NLVL
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SB%XLMO(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* mixing length
  !
  IF (ASSOCIATED(SB%XLM)) THEN
    DO JLAYER=1,SB%NLVL
      ...
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SB%XLM(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
   END DO
  END IF
  !
  !* dissipative length
  ...
  IF (ASSOCIATED(SB%XLEPS)) THEN
    DO JLAYER=1,SB%NLVL
      ...
      YCOMMENT='mixing length (m)'
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,SB%XLEPS(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
    END DO
  END IF
  !
