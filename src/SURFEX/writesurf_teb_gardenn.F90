!     #########
      SUBROUTINE WRITESURF_TEB_GARDEN_n (DGU, IOB, U, &
                                          TGD, TGDO, TGDPE, TVG, &
                                         HPROGRAM,HPATCH)
!     #####################################
!
...
YCOMMENT='X_Y_GD_WR (kg/m2)'
 CALL WRITE_SURF(DGU, IOB, U, &
                  HPROGRAM,YRECFM,TGD%CUR%XWR(:),IRESP,HCOMMENT=YCOMMENT)
!
!* Leaf Area Index
!
...
IF (TVG%CPHOTO=='NIT') THEN
  ...
  DO JNBIOMASS=1,TVG%NNBIOMASS
    ...
    CALL WRITE_SURF(DGU, IOB, U, &
                  HPROGRAM,YRECFM,TGD%CUR%XBIOMASS(:,JNBIOMASS),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !
  DO JNBIOMASS=2,TVG%NNBIOMASS
    ...
    CALL WRITE_SURF(DGU, IOB, U, &
                  HPROGRAM,YRECFM,TGD%CUR%XRESP_BIOMASS(:,JNBIOMASS),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
END IF
!
...
YCOMMENT='X_Y_GD_RESA (s/m)'
 CALL WRITE_SURF(DGU, IOB, U, &
                  HPROGRAM,YRECFM,TGD%CUR%XRESA(:),IRESP,HCOMMENT=YCOMMENT)
!
!* snow mantel
!
YRECFM='GD'
