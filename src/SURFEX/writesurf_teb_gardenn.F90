!     #########
      SUBROUTINE WRITESURF_TEB_GARDEN_n (HSELECT, GDO, GDR, PLAI, HPROGRAM,HPATCH)
!     #####################################
!
...
YCOMMENT='X_Y_GD_WR (kg/m2)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,GDR%XWR(:,1),IRESP,HCOMMENT=YCOMMENT)
!
!* Leaf Area Index
!
IF (GDO%CPHOTO/='NON' .AND. GDO%CPHOTO/='AGS' .AND. GDO%CPHOTO/='AST') THEN
  YRECFM=HPATCH//'GD_LAI'
  YRECFM=ADJUSTL(YRECFM)
  YCOMMENT='X_Y_GD_LAI (m2/m2)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,PLAI(:,1),IRESP,HCOMMENT=YCOMMENT)
END IF
!
IF (GDO%CPHOTO=='NIT') THEN
  !
  DO JNBIOMASS=1,GDO%NNBIOMASS
    ...
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,GDR%XBIOMASS(:,JNBIOMASS,1),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !
  DO JNBIOMASS=2,GDO%NNBIOMASS
    WRITE(YLVL,'(I1)') JNBIOMASS
    YRECFM=HPATCH//'GD_RESPI'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    YRECFM=ADJUSTL(YRECFM)
    YFORM='(A16,I1.1,A10)'
    WRITE(YCOMMENT,FMT=YFORM) 'X_Y_RESP_BIOMASS',JNBIOMASS,' (kg/m2/s)'
    CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,GDR%XRESP_BIOMASS(:,JNBIOMASS,1),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
END IF
!
...
YCOMMENT='X_Y_GD_RESA (s/m)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,GDR%XRESA(:,1),IRESP,HCOMMENT=YCOMMENT)
!
!* snow mantel
!
YRECFM='GD'
 CALL WRITESURF_GR_SNOW(HSELECT,HPROGRAM,YRECFM,HPATCH,GDR%TSNOW)
