...
ALLOCATE(ZWORK(SIZE(TGD%CUR%XTG,1)))
!* soil temperatures
!
...
YCOMMENT='X_Y_GD_WR (kg/m2)'
 CALL WRITE_SURF(HPROGRAM,YRECFM,TGD%CUR%XWR(:),IRESP,HCOMMENT=YCOMMENT)
!
!* Leaf Area Index
!
...
IF (TVG%CPHOTO=='NIT') THEN
  ...
  DO JNBIOMASS=1,TVG%NNBIOMASS
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TGD%CUR%XBIOMASS(:,JNBIOMASS),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !
  DO JNBIOMASS=2,TVG%NNBIOMASS
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TGD%CUR%XRESP_BIOMASS(:,JNBIOMASS),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
END IF
!
...
YCOMMENT='X_Y_GD_RESA (s/m)'
 CALL WRITE_SURF(HPROGRAM,YRECFM,TGD%CUR%XRESA(:),IRESP,HCOMMENT=YCOMMENT)
!
!* snow mantel
!
YRECFM='GD'
