...
ALLOCATE(ZWORK(SIZE(GDM%TV%R%CUR%XTG,1)))
!* soil temperatures
!
DO JLAYER=1,GDM%TV%O%NGROUND_LAYER
  ...
  ZWORK=GDM%TV%R%CUR%XTG(:,JLAYER,1)
...
DO JLAYER=1,GDM%TV%O%NGROUND_LAYER
  ...
  ZWORK=GDM%TV%R%CUR%XWG(:,JLAYER,1)
  CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,YRECFM,ZWORK,IRESP,HCOMMENT=YCOMMENT)
END DO
!
...
DO JLAYER=1,GDM%TV%O%NGROUND_LAYER
  ...
  ZWORK=GDM%TV%R%CUR%XWGI(:,JLAYER,1)
  CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,YRECFM,ZWORK,IRESP,HCOMMENT=YCOMMENT)
END DO
!
...
YCOMMENT='X_Y_GD_WR (kg/m2)'
 CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,YRECFM,GDM%TV%R%CUR%XWR(:,1),IRESP,HCOMMENT=YCOMMENT)
!
!* Leaf Area Index
!
IF (GDM%TV%O%CPHOTO/='NON' .AND. GDM%TV%O%CPHOTO/='AGS' .AND. GDM%TV%O%CPHOTO/='AST') THEN
  YRECFM=HPATCH//'GD_LAI'
  YRECFM=ADJUSTL(YRECFM)
  YCOMMENT='X_Y_GD_LAI (m2/m2)'
 CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,YRECFM,GDM%TV%M%T%CUR%XLAI(:,1),IRESP,HCOMMENT=YCOMMENT)
END IF
!
IF (GDM%TV%O%CPHOTO=='NIT') THEN
  !
  DO JNBIOMASS=1,GDM%TV%O%NNBIOMASS
    ...
    CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,YRECFM,GDM%TV%R%CUR%XBIOMASS(:,JNBIOMASS,1),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !
  DO JNBIOMASS=2,GDM%TV%O%NNBIOMASS
    ...
    CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,YRECFM,GDM%TV%R%CUR%XRESP_BIOMASS(:,JNBIOMASS,1),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
END IF
!
...
YCOMMENT='X_Y_GD_RESA (s/m)'
 CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,YRECFM,GDM%TV%R%CUR%XRESA(:,1),IRESP,HCOMMENT=YCOMMENT)
!
!* snow mantel
!
YRECFM='GD'
 CALL WRITESURF_GR_SNOW(DGU, U, &
                        HPROGRAM,YRECFM,HPATCH,GDM%TV%R%CUR%TSNOW)
