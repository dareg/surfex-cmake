!     #########
      SUBROUTINE WRITESURF_TEB_GREENROOF_n (TGR, TGRO, TGRPE, TVG, &
                                            HPROGRAM,HPATCH)
!     #####################################
!
...
ALLOCATE(ZWORK(SIZE(TGR%CUR%XTG,1)))
!
!
!* soil temperatures
!
...
YCOMMENT='X_Y_TWN_WR_GR (kg/m2)'
 CALL WRITE_SURF(HPROGRAM,YRECFM,TGR%CUR%XWR(:),IRESP,HCOMMENT=YCOMMENT)
!
!* Leaf Area Index
!
IF (TVG%CPHOTO/='NON' .AND. TVG%CPHOTO/='AGS' .AND. TVG%CPHOTO/='AST') THEN
 ...
 CALL WRITE_SURF(HPROGRAM,YRECFM,TGRPE%CUR%XLAI(:),IRESP,HCOMMENT=YCOMMENT)
END IF
!
!
!* biomass
...
IF (TVG%CPHOTO=='NIT') THEN
  DO JNBIOMASS=1,TVG%NNBIOMASS
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TGR%CUR%XBIOMASS(:,JNBIOMASS),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !
  DO JNBIOMASS=2,TVG%NNBIOMASS
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TGR%CUR%XRESP_BIOMASS(:,JNBIOMASS),IRESP,HCOMMENT=YCOMMENT)
  END DO
END IF
!
...
YCOMMENT='X_Y_GR_RESA (s/m)'
 CALL WRITE_SURF(HPROGRAM,YRECFM,TGR%CUR%XRESA(:),IRESP,HCOMMENT=YCOMMENT)
!
!* snow mantel
!
YRECFM='GR'
 CALL WRITESURF_GR_SNOW(HPROGRAM,YRECFM,HPATCH,TGR%CUR%TSNOW)
!
