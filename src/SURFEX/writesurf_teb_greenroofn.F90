...
DO JLAYER=1,NLAYER_GR
  WRITE(YLVL,'(I4)') JLAYER
  YRECFM=HPATCH//'GR_WI'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
  YRECFM=ADJUSTL(YRECFM)
  YFORM='(A14,I1.1,A8)'
  IF (JLAYER >= 10)  YFORM='(A14,I2.2,A8)'
  WRITE(YCOMMENT,YFORM) 'X_Y_GR_WI',JLAYER,' (m3/m3)'
  ZWORK=XWGI(:,JLAYER)
  CALL WRITE_SURF(HPROGRAM,YRECFM,ZWORK,IRESP,HCOMMENT=YCOMMENT)
END DO
!
...
IF (CPHOTO=='NIT') THEN
  DO JNBIOMASS=1,NNBIOMASS
    ...
    YRECFM=HPATCH//'GR_BIOMAS'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    YFORM='(A11,I1.1,A8)'
  ...
  DO JNBIOMASS=2,NNBIOMASS
    WRITE(YLVL,'(I4)') JNBIOMASS
    YRECFM=HPATCH//'GR_RESPIR'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    YFORM='(A16,I1.1,A10)'
