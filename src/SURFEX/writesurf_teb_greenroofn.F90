!     #########
      SUBROUTINE WRITESURF_TEB_GREENROOF_n (HSELECT, GRO, GRR, PLAI, HPROGRAM,HPATCH)
!     #####################################
!
...
YCOMMENT='X_Y_TWN_WR_GR (kg/m2)'
 CALL WRITE_SURF(HSELECT, &
                  HPROGRAM,YRECFM,GRR%XWR(:,1),IRESP,HCOMMENT=YCOMMENT)
!
!* Leaf Area Index
!
IF (GRO%CPHOTO/='NON' .AND. GRO%CPHOTO/='AGS' .AND. GRO%CPHOTO/='AST') THEN
  YRECFM=HPATCH//'GR_LAI'
  YRECFM=ADJUSTL(YRECFM)
  YCOMMENT='X_Y_GR_LAI (m2/m2)'
 CALL WRITE_SURF(HSELECT, &
                  HPROGRAM,YRECFM,PLAI(:,1),IRESP,HCOMMENT=YCOMMENT)
END IF
!
!
!* biomass
...
IF (GRO%CPHOTO=='NIT') THEN
  DO JNBIOMASS=1,GRO%NNBIOMASS
    ...
    CALL WRITE_SURF(HSELECT, &
                  HPROGRAM,YRECFM,GRR%XBIOMASS(:,JNBIOMASS,1),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !
  DO JNBIOMASS=2,GRO%NNBIOMASS
    ...
    CALL WRITE_SURF(HSELECT, &
                  HPROGRAM,YRECFM,GRR%XRESP_BIOMASS(:,JNBIOMASS,1),IRESP,HCOMMENT=YCOMMENT)
  END DO
END IF
!
...
YCOMMENT='X_Y_GR_RESA (s/m)'
 CALL WRITE_SURF(HSELECT, &
                  HPROGRAM,YRECFM,GRR%XRESA(:,1),IRESP,HCOMMENT=YCOMMENT)
!
!* snow mantel
!
YRECFM='GR'
 CALL WRITESURF_GR_SNOW(HSELECT, &
                        HPROGRAM,YRECFM,HPATCH,GRR%TSNOW)
!
