!     #########
      SUBROUTINE WRITESURF_TEB_GREENROOF_n (DGGR, DST, TGR, TGRO, TGRPE, TGRP, TVG, &
                                            HPROGRAM,HPATCH)
!     #####################################
!
...
YCOMMENT='X_Y_TWN_WR_GR (kg/m2)'
 CALL WRITE_SURF(HPROGRAM,YRECFM,TGR%XWR(:),IRESP,HCOMMENT=YCOMMENT)
!
!* Leaf Area Index
!
IF (TVG%CPHOTO/='NON' .AND. TVG%CPHOTO/='AGS' .AND. TVG%CPHOTO/='AST') THEN
 ...
 CALL WRITE_SURF(HPROGRAM,YRECFM,TGRPE%XLAI(:),IRESP,HCOMMENT=YCOMMENT)
END IF
!
!
!* biomass
...
IF (TVG%CPHOTO=='NIT') THEN
  DO JNBIOMASS=1,TVG%NNBIOMASS
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TGR%XBIOMASS(:,JNBIOMASS),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !
  DO JNBIOMASS=2,TVG%NNBIOMASS
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,TGR%XRESP_BIOMASS(:,JNBIOMASS),IRESP,HCOMMENT=YCOMMENT)
  END DO
END IF
!
...
YCOMMENT='X_Y_GR_RESA (s/m)'
 CALL WRITE_SURF(HPROGRAM,YRECFM,TGR%XRESA(:),IRESP,HCOMMENT=YCOMMENT)
!
!* snow mantel
!
YRECFM='GR'
 CALL WRITESURF_GR_SNOW(HPROGRAM,YRECFM,HPATCH,TGR%TSNOW)
!
