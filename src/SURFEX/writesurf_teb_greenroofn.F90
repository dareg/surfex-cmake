...
CHARACTER(LEN=5),  INTENT(IN)     :: HPATCH   ! current teb patch
!
...
DO JLAYER=1,IWORK
  ...
  YRECFM=HPATCH//'TWN_GR_TG'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
...
DO JLAYER=1,NLAYER_GR
  WRITE(YLVL,'(I4)') JLAYER
  YRECFM=HPATCH//'TWN_GR_WG'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
  YRECFM=ADJUSTL(YRECFM)
  YFORM='(A13,I1.1,A8)'
...
DO JLAYER=1,NLAYER_GR
  WRITE(YLVL,'(I4)') JLAYER
  YRECFM=HPATCH//'TWN_GR_WGI'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
  YRECFM=ADJUSTL(YRECFM)
  YFORM='(A14,I1.1,A8)'
  IF (JLAYER >= 10)  YFORM='(A14,I2.2,A8)'
  WRITE(YCOMMENT,YFORM) 'X_Y_TWN_GR_WGI',JLAYER,' (m3/m3)'
  ZWORK=XWGI(:,JLAYER)
  CALL WRITE_SURF(HPROGRAM,YRECFM,ZWORK,IRESP,HCOMMENT=YCOMMENT)
END DO
!
...
IF (CPHOTO/='NON' .AND. CPHOTO/='AGS' .AND. CPHOTO/='AST') THEN
  YRECFM=HPATCH//'TWN_GR_LAI'
  YRECFM=ADJUSTL(YRECFM)
  YCOMMENT='X_Y_TWN_GR_LAI (m2/m2)'
...
IF (CPHOTO=='NIT') THEN
  DO JNBIOMASS=1,NNBIOMASS
    ...
    YRECFM='TWN_GR_BIOMASS'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    YFORM='(A11,I1.1,A8)'
  ...
  DO JNBIOMASS=2,NNBIOMASS
    WRITE(YLVL,'(I4)') JNBIOMASS
    YRECFM='TWN_GR_RESP_BIOM'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    YFORM='(A16,I1.1,A10)'
...
!
!
YRECFM=HPATCH//'TWN_GR_RESA'
YRECFM=ADJUSTL(YRECFM)
YCOMMENT='X_Y_TWN_GR_RESA (s/m)'
