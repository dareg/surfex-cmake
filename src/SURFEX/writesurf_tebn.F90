!     #########
      SUBROUTINE WRITESURF_TEB_n (B, BOP, DTT, TGD, TGDO, TGDPE, TGR, TGRO, TGRPE, T, TOP, TPN, TVG, &
...
YCOMMENT='Option for Road orientation in TEB scheme'
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,'ROAD_DIR',TOP%CROAD_DIR,IRESP,YCOMMENT)
YCOMMENT='Option for Wall representation in TEB scheme'
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,'WALL_OPT',TOP%CWALL_OPT,IRESP,YCOMMENT)
!
!*       3.     Prognostic fields:
!               -----------------
!
...
DO JLAYER=1,TOP%NROOF_LAYER
 ...
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,T%CUR%XT_ROOF(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
...
YCOMMENT='WS_ROOF (kg/m2)'
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,T%CUR%XWS_ROOF(:),IRESP,HCOMMENT=YCOMMENT)
!
!* road temperatures
!
...
DO JLAYER=1,TOP%NROAD_LAYER
  ...
  CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,T%CUR%XT_ROAD(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
!* road water content
!
...
YCOMMENT='WS_ROAD (kg/m2)'
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,T%CUR%XWS_ROAD(:),IRESP,HCOMMENT=YCOMMENT)
!
!* wall temperatures
!
...
DO JLAYER=1,TOP%NWALL_LAYER
 IF (TOP%CWALL_OPT=='UNIF') THEN
  ...
  CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,T%CUR%XT_WALL_A(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
 ELSE
  !* Wall A
  ...
  ELSEIF (SIZE(T%CUR%XROAD_DIR)>0) THEN
    ...
    CALL WALLA_DIR(T%CUR%XROAD_DIR(1),YDIR)
  ...
  ELSEIF (SIZE(T%CUR%XROAD_DIR)>0) THEN
    !* wall direction is uniform spatially, one can then indicate it in the comment
    CALL WALLB_DIR(T%CUR%XROAD_DIR(1),YDIR)
    YSTRING=TRIM(YDIR)//'-FACING WALL TEMP. LAYER '
  ELSE
    YSTRING='?-FACING WALL TEMP. LAYER '
  ENDIF
  WRITE(YCOMMENT,'(A,I1.1,A4)') TRIM(YSTRING), JLAYER,' (K)'
  CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,T%CUR%XT_WALL_B(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
 END IF
END DO
!
...
YCOMMENT='TI_BLD (K)'
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,B%CUR%XTI_BLD(:),IRESP,HCOMMENT=YCOMMENT)
!
!
...
YCOMMENT='T_WIN1 (K)'
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,B%CUR%XT_WIN1(:),IRESP,HCOMMENT=YCOMMENT)
!
...
YCOMMENT='QI_BLD (kg/kg)'
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,B%CUR%XQI_BLD(:),IRESP,HCOMMENT=YCOMMENT)
!
  !
  ...
  YCOMMENT='T_WIN2 (K)'
  CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,B%CUR%XT_WIN2(:),IRESP,HCOMMENT=YCOMMENT)
  !
  !* floor temperatures
  !
  DO JLAYER=1,BOP%NFLOOR_LAYER
    ...
    YRECFM=ADJUSTL(YRECFM)
    CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,B%CUR%XT_FLOOR(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  ...
  DO JLAYER=1,BOP%NFLOOR_LAYER
    ...
    YRECFM=ADJUSTL(YRECFM)
    CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,B%CUR%XT_MASS(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO        
  !
ENDIF
!
...
YCOMMENT='TI_ROAD (K)'
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,T%CUR%XTI_ROAD(:),IRESP,HCOMMENT=YCOMMENT)
!
!* snow mantel
!
YRECFM='RF'
...
YCOMMENT='T_CANYON (K)'
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,T%CUR%XT_CANYON(:),IRESP,HCOMMENT=YCOMMENT)
!
...
YCOMMENT='Q_CANYON (kg/kg)'
 CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,T%CUR%XQ_CANYON(:),IRESP,HCOMMENT=YCOMMENT)
!
!
...
IF (TOP%LSOLAR_PANEL) THEN
  ...
  IF (.NOT. ASSOCIATED(TPN%XTHER_PRODC_DAY)) THEN
    ! for PREP cases
    ALLOCATE(TPN%XTHER_PRODC_DAY(SIZE(B%CUR%XTI_BLD)))
    TPN%XTHER_PRODC_DAY=0.
  END IF
  CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,TPN%XTHER_PRODC_DAY(:),IRESP,HCOMMENT=YCOMMENT)
END IF
!-------------------------------------------------------------------------------
!
!*       5.  Time
...
IF (KPATCH==1) THEN
  ...
  YCOMMENT='s'
  CALL WRITE_SURF(DGU, IOB, U, &
                 HPROGRAM,YRECFM,TOP%TTIME,IRESP,HCOMMENT=YCOMMENT)
END IF
!
!
!-------------------------------------------------------------------------------
