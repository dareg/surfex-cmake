...
IF (LWRITE_EXTERN) THEN
  ...
  DO JLAYER=1,TM%TOP%NROOF_LAYER
    ...
    CALL WRITE_SURF(DGU, U, &
                 HPROGRAM,YRECFM,TM%T%CUR%XD_ROOF(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  DO JLAYER=1,TM%TOP%NWALL_LAYER
    WRITE(YRECFM,FMT='(A,I1.1)') 'D_WALL',JLAYER
    YCOMMENT='WALL layer thickness'
    CALL WRITE_SURF(DGU, U, &
                 HPROGRAM,YRECFM,TM%T%CUR%XD_WALL(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  DO JLAYER=1,TM%TOP%NROAD_LAYER
    WRITE(YRECFM,FMT='(A,I1.1)') 'D_ROAD',JLAYER
    YCOMMENT='ROAD layer thickness'
    CALL WRITE_SURF(DGU, U, &
                 HPROGRAM,YRECFM,TM%T%CUR%XD_ROAD(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  IF (TM%TOP%CBEM=='BEM') THEN
    DO JLAYER=1,TM%BOP%NFLOOR_LAYER
      ...
      CALL WRITE_SURF(DGU, U, &
                 HPROGRAM,YRECFM,TM%B%CUR%XD_FLOOR(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
    END DO
  ENDIF
  !
ENDIF
...
!
! Grenn roofs
IF (TM%TOP%LGREENROOF) CALL WRITESURF_TEB_GREENROOF_n(DGU, U, GRM, &
                                                   HPROGRAM,YPATCH)
!
