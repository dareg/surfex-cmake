!     #########
      SUBROUTINE WRITESURF_WATFLUX_SBL_n (HSELECT, OSBL, WSB, HPROGRAM,HWRITE)
!     ####################################
!
...
!
IF (.NOT. OSBL .AND. LHOOK) CALL DR_HOOK('WRITESURF_WATFLUX_SBL_N',1,ZHOOK_HANDLE)
IF (.NOT. OSBL) RETURN
!
!* number of levels
!i
...
YCOMMENT='number of SBL levels'
 CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,WSB%NLVL,IRESP,HCOMMENT=YCOMMENT)
!
!* altitudes
!
DO JLAYER=1,WSB%NLVL
  ...
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,WSB%XZ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
IF (HWRITE/='PRE') THEN
  !
  ...
  DO JLAYER=1,WSB%NLVL
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,WSB%XU(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* temperature in SBL
  !
  DO JLAYER=1,WSB%NLVL
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,WSB%XT(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* humidity in SBL
  !
  DO JLAYER=1,WSB%NLVL
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,WSB%XQ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Tke in SBL
  !
  DO JLAYER=1,WSB%NLVL
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,WSB%XTKE(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Monin-Obhukov length
  !
  YRECFM='WAT_SBL_LMO '
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,WSB%XLMO(:,WSB%NLVL),IRESP,HCOMMENT=YCOMMENT)
  !
  !* Air pressure in SBL
  !
  DO JLAYER=1,WSB%NLVL
    ...
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,WSB%XP(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
ENDIF
!
