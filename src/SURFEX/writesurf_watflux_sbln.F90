!     #########
      SUBROUTINE WRITESURF_WATFLUX_SBL_n(HPROGRAM,HWRITE)
!     ####################################
!
...
YCOMMENT='flag to use SBL levels'
 CALL WRITE_SURF(HPROGRAM,YRECFM,W%LSBL,IRESP,HCOMMENT=YCOMMENT)
!
...
YCOMMENT='number of SBL levels'
 CALL WRITE_SURF(HPROGRAM,YRECFM,WSB%NLVL,IRESP,HCOMMENT=YCOMMENT)
!
!* altitudes
!
DO JLAYER=1,WSB%NLVL
  ...
  CALL WRITE_SURF(HPROGRAM,YRECFM,WSB%XZ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
END DO
!
IF (HWRITE/='PRE') THEN
  !
  ...
  DO JLAYER=1,WSB%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,WSB%XU(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* temperature in SBL
  !
  DO JLAYER=1,WSB%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,WSB%XT(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* humidity in SBL
  !
  DO JLAYER=1,WSB%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,WSB%XQ(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Tke in SBL
  !
  DO JLAYER=1,WSB%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,WSB%XTKE(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
  !* Monin-Obhukov length
  !
  YRECFM='WAT_SBL_LMO '
  CALL WRITE_SURF(HPROGRAM,YRECFM,WSB%XLMO(:),IRESP,HCOMMENT=YCOMMENT)
  !
  !* Air pressure in SBL
  !
  DO JLAYER=1,WSB%NLVL
    ...
    CALL WRITE_SURF(HPROGRAM,YRECFM,WSB%XP(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
  !
ENDIF
!
