...
!!      J. Masek     08/2023   : Namelist variable XRZ0_TO_HEIGHT instead of 
!!                               hardcoded value 0.13
!-------------------------------------------------------------------------------
!
!*       0.     DECLARATIONS
!               ------------
...
USE MODD_TREEDRAG,       ONLY : XZ0_MIN_LIMIT, XZ0_MAX_LIMIT
USE MODD_SURF_ATM,       ONLY : XZ0_OFFSET, XRZ0_TO_HEIGHT
...
DO JTYPE=1,SIZE(PVEGTYPE)
  ZAVG_H = ZAVG_H + PVEGTYPE(JTYPE) / (LOG(XZ0_OFFSET + XRZ0_TO_HEIGHT*ZH_VEG(JTYPE)/ZZREF))**2
END DO
ZAVG_H = MAX(ZAVG_H,0.00001)

ZAVG_H = ZZREF / (EXP(1./SQRT(ZAVG_H)) - XZ0_OFFSET)
!
PZ0  = MAX(0.001, ZAVG_H)
PZ0  = MAX(XZ0_MIN_LIMIT, MIN(XZ0_MAX_LIMIT, PZ0))
...
!
WHERE(ZH_VEG(:)/=XUNDEF) 
  PZ0 (:) = MAX(0.001, XRZ0_TO_HEIGHT*ZH_VEG(:))
  PZ0 (:) = MAX(XZ0_MIN_LIMIT, MIN(XZ0_MAX_LIMIT, PZ0(:)))
END WHERE
!-----------------------------------------------------------------
!
