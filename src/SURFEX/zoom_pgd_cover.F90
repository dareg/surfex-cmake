...
!
ALLOCATE(ZCOVER(INI,COUNT(LCOVER)))
 CALL READ_SURF_COV(HPROGRAM,YRECFM,ZCOVER(:,:),LCOVER,IRESP,HDIR='A')
!
...
IF (IVERSION>=7) THEN
  ...
  !
ELSE
  CALL CONVERT_COVER_FRAC(ZCOVER,LCOVER,ZSEA1(:,1),ZNATURE1(:,1),ZTOWN1(:,1),ZWATER1(:,1))
ENDIF
!
...
!
IL = SIZE(XLAT)
ALLOCATE(XCOVER(IL,COUNT(LCOVER)))
!
 CALL HOR_INTERPOL(ILUOUT,ZCOVER,XCOVER)
!
DEALLOCATE(ZCOVER)
!
ALLOCATE(ZCOVER(IL,COUNT(LCOVER)))
ICPT1 = 0
ICPT2 = 0
DO JCOVER = 1,JPCOVER
  IF (LCOVER(JCOVER)) THEN
    ICPT1 = ICPT1 + 1
    IF (ALL(XCOVER(:,ICPT1)==0.)) THEN
      LCOVER(JCOVER) = .FALSE.
    ELSE
      ICPT2 = ICPT2 + 1
      ZCOVER(:,ICPT2) = XCOVER(:,ICPT1)
    ENDIF
  ENDIF
ENDDO
!
DEALLOCATE(XCOVER)
ALLOCATE(XCOVER(IL,ICPT2))
XCOVER(:,:) = ZCOVER(:,1:ICPT2)
DEALLOCATE(ZCOVER)
!
ALLOCATE(ZSEA2  (IL,1))
...
!
DO JCOVER=1,SIZE(XCOVER,2)
  WHERE(ZSUM(:)/=0.)  XCOVER(:,JCOVER) = XCOVER(:,JCOVER)/ZSUM(:)
END DO
!
DO JCOVER=1,SIZE(XCOVER,2)
