...
!
 CALL PREP_OUTPUT_GRID(ILUOUT,UG%CGRID,UG%XGRID_PAR,UG%XLAT,UG%XLON)
!
!------------------------------------------------------------------------------
!
!*      3.     Reading of cover
...
ALLOCATE(U%LCOVER(JPCOVER))
 ...
 CALL READ_SURF(HPROGRAM,YRECFM,U%LCOVER(:),IRESP,HDIR='-')
!
...
IF (IVERSION>=7) THEN
  ...
  !
ELSE
  CALL CONVERT_COVER_FRAC(ZCOVER,U%LCOVER,ZSEA1(:,1),ZNATURE1(:,1),ZTOWN1(:,1),ZWATER1(:,1))
ENDIF
!
...
!
 CALL HOR_INTERPOL(ILUOUT,ZCOVER,U%XCOVER)
!
DEALLOCATE(ZCOVER)
!
