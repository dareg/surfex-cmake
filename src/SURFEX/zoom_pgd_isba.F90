...
!
 ...
 CALL READ_SURF(&
                HINIFILETYPE,'PATCH_NUMBER',I%O%NPATCH,IRESP)
 CALL READ_SURF(&
                HINIFILETYPE,'GROUND_LAYER',I%O%NGROUND_LAYER,IRESP)
 CALL READ_SURF(&
                HINIFILETYPE,'ISBA',I%O%CISBA,IRESP)
IF (IVERSION >= 7) THEN
  CALL READ_SURF(&
                HINIFILETYPE,'PEDOTF',I%O%CPEDOTF,IRESP)
ELSE
  I%O%CPEDOTF = 'CH78'
ENDIF
 CALL READ_SURF(&
                HINIFILETYPE,'PHOTO',I%O%CPHOTO,IRESP)
!
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=2) THEN
  !
  CALL READ_SURF(&
                HINIFILETYPE,'TR_ML',I%O%LTR_ML,IRESP)
  !
ELSE 
  I%O%LTR_ML = .FALSE.
ENDIF
...
IF(I%O%CISBA=='DIF') THEN
  ...
  IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=2) THEN
    CALL READ_SURF(&
                HINIFILETYPE,'SOILGRID',I%O%XSOILGRID,IRESP,HDIR='-')
  ELSE
...
IF (IVERSION>=6) THEN
  CALL READ_SURF(&
                HPROGRAM,'NBIOMASS',I%O%NNBIOMASS,IRESP)
ELSE
