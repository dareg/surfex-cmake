...
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
   CALL READ_SURF(HPROGRAM,'SOCP',I%O%LSOCP,IRESP)
ELSE
   I%O%LSOCP=.FALSE.
ENDIF
!
...
!
  CALL READ_SURF(HPROGRAM,'SOC_TOP',I%P%XSOC(:,1),IRESP)
  CALL READ_SURF(HPROGRAM,'SOC_SUB',I%P%XSOC(:,2),IRESP)
!
...
!
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
   CALL READ_SURF(HPROGRAM,'PERMAFROST',I%O%LPERM,IRESP)
ELSE
   I%O%LPERM=.FALSE.
ENDIF
!
...
!  
  ...
  CALL READ_SURF(HPROGRAM,'PERM',I%P%XPERM(:),IRESP)
!
ELSE
!  
...
!
IF (IVERSION>=8) THEN
   CALL READ_SURF(HPROGRAM,'GWKEY',I%O%LGW,IRESP)
ELSE
   I%O%LGW=.FALSE.
ENDIF
!
...
!  
  ...
  CALL READ_SURF(HPROGRAM,'GWFRAC',I%P%XGW(:),IRESP)
!
ELSE
!  
  ALLOCATE(I%P%XGW (0))
...
!
IF (IVERSION>7 .OR. (IVERSION==7 .AND. IBUGFIX>=3)) THEN
   CALL READ_SURF(HPROGRAM,'NO',I%O%LNOF,IRESP)
ELSE
   I%O%LNOF = .FALSE.
ENDIF
!
...
IF (CHI%LCH_NO_FLUX) THEN
  ...
  IF (I%O%LNOF) THEN
    ...
    CALL READ_SURF(HPROGRAM,'PH',I%P%XPH(:),IRESP)
    !
    ALLOCATE(I%P%XFERT(INI))
    CALL READ_SURF(HPROGRAM,'FERT',I%P%XFERT(:),IRESP)
    !
  ELSE
...
!* interpolations
!
 CALL HOR_INTERPOL(DTCO, U, ILUOUT,ZSAND,I%P%XSAND)
 CALL HOR_INTERPOL(DTCO, U, ILUOUT,ZCLAY,I%P%XCLAY)
...
!
 ...
 CALL READ_PGD_ISBA_PAR_n(DTCO, U, DTI, IG%NDIM, I%O, &
