...
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
   CALL READ_SURF(HPROGRAM,'SOCP',I%LSOCP,IRESP)
ELSE
   I%LSOCP=.FALSE.
ENDIF
!
...
!
  CALL READ_SURF(HPROGRAM,'SOC_TOP',I%XSOC(:,1),IRESP)
  CALL READ_SURF(HPROGRAM,'SOC_SUB',I%XSOC(:,2),IRESP)
!
...
!
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
   CALL READ_SURF(HPROGRAM,'PERMAFROST',I%LPERM,IRESP)
ELSE
   I%LPERM=.FALSE.
ENDIF
!
...
!  
  ...
  CALL READ_SURF(HPROGRAM,'PERM',I%XPERM(:),IRESP)
!
ELSE
!  
  ALLOCATE(I%XPERM (0))
...
!
IF (IVERSION>=8) THEN
   CALL READ_SURF(HPROGRAM,'GWKEY',I%LGW,IRESP)
ELSE
   I%LGW=.FALSE.
ENDIF
!
...
!  
  ALLOCATE(I%XGW (INI))
  CALL READ_SURF(HPROGRAM,'GWFRAC',I%XGW(:),IRESP)
!
ELSE
!  
  ALLOCATE(I%XGW (0))
...
!
IF (IVERSION>7 .OR. (IVERSION==7 .AND. IBUGFIX>=3)) THEN
   CALL READ_SURF(HPROGRAM,'NO',I%LNOF,IRESP)
ELSE
   I%LNOF = .FALSE.
ENDIF
!
...
IF (CHI%LCH_NO_FLUX) THEN
  ...
  IF (I%LNOF) THEN
    !
    ALLOCATE(I%XPH(INI))
    CALL READ_SURF(HPROGRAM,'PH',I%XPH(:),IRESP)
    !
    ALLOCATE(I%XFERT(INI))
    CALL READ_SURF(HPROGRAM,'FERT',I%XFERT(:),IRESP)
    !
  ELSE
...
!* interpolations
!
 CALL HOR_INTERPOL(ILUOUT,ZSAND,I%XSAND)
 CALL HOR_INTERPOL(ILUOUT,ZCLAY,I%XCLAY)
