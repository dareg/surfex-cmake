...
!
 CALL PGD_GRID(&
               YSC%UG, YSC%U, &
               HPROGRAM,HFILE,HFILETYPE,.TRUE.,YSC%UG%G%CGRID,YSC%UG%G%NGRID_PAR,YSC%UG%G%XGRID_PAR)
!
...
IF (YSC%U%NDIM_TOWN>0)                                 &
  CALL ZOOM_PGD_TOWN(YSC%TM%B, YSC%TM%DGCT, YSC%TM%DGMT, YSC%TM%T, YSC%GDM%TV%R, &
