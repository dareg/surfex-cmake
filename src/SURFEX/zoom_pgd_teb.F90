!     ###########################################################
      SUBROUTINE ZOOM_PGD_TEB (B, DGCT, DGMT, T, TGDR, TGDPE, TGRR, TGRPE, BOP, &
                               BDD, DTB, DTCO, DTT, UG, U, TGDO, TGDP, TG, TOP, &
...
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
  ...
  CALL READ_SURF(HPROGRAM,'GD_ISBA',TGDO%CISBA,IRESP)
  CALL READ_SURF(HPROGRAM,'GD_PHOTO',TGDO%CPHOTO,IRESP)
  CALL READ_SURF(HPROGRAM,'GD_PEDOTF',TGDO%CPEDOTF,IRESP)
  TGDO%NNBIOMASS=1
  IF (TGDO%CPHOTO=='NIT') TGDO%NNBIOMASS=3  
ELSE
  CALL READ_SURF(HPROGRAM,'TWN_LAYER',TGDO%NGROUND_LAYER,IRESP)
  CALL READ_SURF(HPROGRAM,'TWN_ISBA',TGDO%CISBA,IRESP)
  CALL READ_SURF(HPROGRAM,'TWN_PHOTO',TGDO%CPHOTO,IRESP)
  CALL READ_SURF( HPROGRAM,'TWN_PEDOTF',TGDO%CPEDOTF,IRESP)
  CALL READ_SURF(HPROGRAM,'TWN_NBIOMASS',TGDO%NNBIOMASS,IRESP)
ENDIF
!
!* sand
!
...
ALLOCATE(TGDP%XCLAY(ILU,TGDO%NGROUND_LAYER))
 CALL HOR_INTERPOL(DTCO, U, ILUOUT,ZIN,TGDP%XCLAY)
DEALLOCATE(ZIN)
!
!* runoff & drainage
!
...
!
 CALL READ_SURF(HPROGRAM,'PAR_GARDEN',TGDO%LPAR,IRESP)
!
!!
IF (TGDO%LPAR) THEN
  WRITE(ILUOUT,*) 'ERROR'
