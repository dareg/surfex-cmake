!     ###########################################################
      SUBROUTINE ZOOM_PGD_TEB (B, DGCT, DGMT, T, TGDR, TGDPE, TGRR, TGRPE, &
...
!
  CALL GOTO_WRAPPER_TEB_PATCH(B, DGCT, DGMT, T, TGDR, TGDPE, &
                                TGRR, TGRPE, 1)
!-------------------------------------------------------------------------------
!
...
IF (IVERSION>7 .OR. IVERSION==7 .AND. IBUGFIX>=3) THEN
  ...
  CALL READ_SURF(&
                HPROGRAM,'GD_ISBA',TGDO%CISBA,IRESP)
  CALL READ_SURF(&
                HPROGRAM,'GD_PHOTO',TGDO%CPHOTO,IRESP)
  CALL READ_SURF(&
                HPROGRAM,'GD_PEDOTF',TGDO%CPEDOTF,IRESP)
  TGDO%NNBIOMASS=1
  IF (TGDO%CPHOTO=='NIT') TGDO%NNBIOMASS=3  
ELSE
  CALL READ_SURF(&
                HPROGRAM,'TWN_LAYER',TGDO%NGROUND_LAYER,IRESP)
  CALL READ_SURF(&
                HPROGRAM,'TWN_ISBA',TGDO%CISBA,IRESP)
  CALL READ_SURF(&
                HPROGRAM,'TWN_PHOTO',TGDO%CPHOTO,IRESP)
  CALL READ_SURF(&
                HPROGRAM,'TWN_PEDOTF',TGDO%CPEDOTF,IRESP)
  CALL READ_SURF(&
                HPROGRAM,'TWN_NBIOMASS',TGDO%NNBIOMASS,IRESP)
ENDIF
!
!* sand
!
...
!
 CALL READ_SURF(&
                HPROGRAM,'PAR_GARDEN',TGDO%LPAR,IRESP)
!
!!
IF (TGDO%LPAR) THEN
  WRITE(ILUOUT,*) 'ERROR'
