...
!     ###########################################################
      SUBROUTINE ZOOM_PGD_TEB (B, DMTC, DMT, T, BOP, &
...
ALLOCATE(K%XSAND(ILU,IO%NGROUND_LAYER))
 CALL HOR_INTERPOL(DTCO, U, ILUOUT,ZIN,K%XSAND)
DEALLOCATE(ZIN)
!
!* clay
!
...
ALLOCATE(K%XCLAY(ILU,IO%NGROUND_LAYER))
 CALL HOR_INTERPOL(DTCO, U, ILUOUT,ZIN,K%XCLAY)
DEALLOCATE(ZIN)
!
!* runoff & drainage
!
...
ALLOCATE(K%XRUNOFFB(ILU))
 CALL HOR_INTERPOL(DTCO, U, ILUOUT,ZIN,ZOUT)
K%XRUNOFFB(:) = ZOUT(:,1)
!
IF (IVERSION<=3) THEN
  K%XWDRAIN = 0.
